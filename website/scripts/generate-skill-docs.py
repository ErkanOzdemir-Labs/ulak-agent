...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/ErkanOzdemir-Labs/ulak-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    ulak_meta = (fm.get("metadata") or {}).get("ulak") or {}
    tags = ulak_meta.get("tags") or []
    related = ulak_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `ulak skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Ulak Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Ulak ships with a large built-in skill library copied into `~/.ulak/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Ulak also syncs bundled skills on `ulak update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.ulak/skills/` tree, it is still shipped with Ulak; restore it with `ulak skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with ulak-agent — install via ulak skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with ulak-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "ulak skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "ulak skills install official/blockchain/solana",
        "ulak skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "ulak skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
