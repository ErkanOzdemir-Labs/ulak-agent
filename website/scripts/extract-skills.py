...
2. The unified Ulak Skills Index at ``website/static/api/skills-index.json``,
    ...
    for base_dir, source_label in LOCAL_SKILL_DIRS:
        ...
        for root, _dirs, files in os.walk(base_path):
            ...
            if isinstance(metadata, dict):
                ulak_meta = metadata.get("ulak", {})
                if isinstance(ulak_meta, dict):
                    tags = ulak_meta.get("tags", [])
            if not tags:
                tags = fm.get("tags", [])
