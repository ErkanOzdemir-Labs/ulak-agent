...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Ulak already has applies. The plugin never sees a
...
ulak-agent itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Ulak provider id (e.g. "openrouter")
...
using Ulak built-in auxiliary tasks; it does not permit another plugin's
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Ulak provider
        # (must be one Ulak already knows about — same names as
        # `ulak model` and config.yaml model.provider).
        ...
        # plugin sends — Ulak does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.ulak/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Ulak' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Ulak subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/ErkanOzdemir-Labs/ulak-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/ErkanOzdemir-Labs/ulak-agent/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/ErkanOzdemir-Labs/ulak-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/ErkanOzdemir-Labs/ulak-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
