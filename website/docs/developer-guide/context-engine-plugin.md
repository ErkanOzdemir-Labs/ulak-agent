...
Users select your engine via `ulak plugins` → Provider Plugins → Context Engine, or by editing `config.yaml`:
...
When `compression.context_timeout_seconds > 0` (the default), Ulak runs the
