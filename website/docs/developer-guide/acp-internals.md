...
The ACP adapter wraps Ulak' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
ulak acp / ulak-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.ulak/.env
  -> configure stderr logging
  -> construct UlakACPAgent
...
- `allow_once` -> Ulak `once`
- `allow_always` -> Ulak `always`
- reject options -> Ulak `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Ulak tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["ulak-acp"])
...
Instead it reuses Ulak' runtime resolver:

- `acp_adapter/auth.py`
- `ulak_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Ulak provider/credentials. It also always advertises a terminal setup auth method (`ulak-setup`, args `--setup`) so first-run ACP clients can open Ulak' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.ulak/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `ulak-acp` toolset definition
- `ulak_cli/main.py` — `ulak acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `ulak-acp` script
