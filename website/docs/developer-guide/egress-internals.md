...
description: "How the iron-proxy egress firewall integrates with Ulak — module layout, lifecycle, security invariants, and extension points"
---

# Egress proxy internals

This page covers the architecture of the egress credential-injection firewall (`ulak egress` / iron-proxy) from a contributor / plugin author's perspective. End-user setup + usage docs live at [Egress proxy](../user-guide/egress/iron-proxy.md).
...
ulak_cli/proxy_cli.py               Wizard + slash command handlers.
                                       `ulak egress {install,setup,start,stop,
                                       status,disable,config}`.  Wires the
                                       core module into argparse.

ulak_cli/subcommands/egress.py:_dispatch_egress
                                       Top-level subparser dispatcher.
                                       dest='egress_command' (intentionally
                                       disjoint from the inbound OAuth
                                       `ulak proxy` subparser, which uses
                                       dest='proxy_command').

ulak_cli/config.py: proxy schema    The `proxy:` block in DEFAULT_CONFIG.
...
tools/environments/docker.py
  ...
  DockerEnvironment.__init__          Docker-side merge logic: collision
                                       ...
                                       _ULAK_EGRESS_NODE_OPTIONS_APPEND
...
tests/test_iron_proxy_e2e.py          Live E2E (gated on ULAK_RUN_E2E=1).
...
ulak egress install
  -> agent.proxy_sources.iron_proxy.install_iron_proxy(force=...)
       ...
       Stage into ~/.ulak/bin/.iron-proxy_XXXX, chmod 755, os.replace
         to ~/.ulak/bin/iron-proxy (atomic).
       _VERSION_CACHE.pop(target) so a forced reinstall re-probes
         --version on next call.

ulak egress setup [--from-bitwarden | --no-bitwarden] [--rotate-tokens]
  -> proxy_cli.cmd_setup
...
| Path | Mode | Test |
|---|---|---|
| `~/.ulak/proxy/` (dir) | `0o700` | `test_proxy_state_dir_is_0o700` |
...
`merge_mappings(existing, discovered, rotate=False)` MUST return prior tokens for providers that overlap.  Re-running `ulak egress setup` cannot silently 401 running sandboxes.  `--rotate-tokens` is the explicit opt-in.
...
`cmd_setup` MUST NOT downgrade `credential_source: bitwarden` to `env` on re-run without an explicit `--no-bitwarden` flag.  Running `ulak egress setup` (no flag) preserves whatever was previously configured.
...
If the provider uses SigV4 / SDK-minted OAuth / request signatures, a static header swap cannot cover it.  Add the env var to `_NON_BEARER_PROVIDERS` so the wizard and `ulak egress status` warn about it:
...
4. Sets the seven env vars (HTTPS_PROXY, NO_PROXY, REQUESTS_CA_BUNDLE, SSL_CERT_FILE, CURL_CA_BUNDLE, NODE_EXTRA_CA_CERTS, ULAK_EGRESS_PROXY) and the per-mapping `ULAK_PROXY_TOKEN_<NAME>` vars.
5. Distributes the CA cert into the sandbox at a path the runtime will trust (typically `/etc/ssl/certs/ulak-egress-ca.crt`).
...
ULAK_RUN_E2E=1 scripts/run_tests.sh tests/test_iron_proxy_e2e.py

# Live PTY smoke against `ulak egress`
ULAK_HOME=/tmp/ulak-egress-test python3 -m ulak_cli.main egress --help
ULAK_HOME=/tmp/ulak-egress-test python3 -m ulak_cli.main egress setup --help
```
...
- User-facing setup + troubleshooting: [Egress proxy](https://ulak-agent.github.com/ErkanOzdemir-Labs/docs/user-guide/egress/iron-proxy)
- Docker backend internals: [Docker](https://ulak-agent.github.com/ErkanOzdemir-Labs/docs/user-guide/docker)
- Bitwarden Secrets Manager integration: [`ulak secrets bitwarden`](https://ulak-agent.github.com/ErkanOzdemir-Labs/docs/user-guide/secrets/bitwarden)
- CLI command reference: [`ulak egress`](https://ulak-agent.github.com/ErkanOzdemir-Labs/docs/reference/cli-commands#ulak-egress)
- Sandbox-injected environment variables: [Egress proxy (sandbox-injected)](https://ulak-agent.github.com/ErkanOzdemir-Labs/docs/reference/environment-variables#egress-proxy-sandbox-injected)
