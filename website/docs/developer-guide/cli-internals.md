...
sidebar_position: 15
title: "CLI Internals"
description: "How ulak_cli is shaped: slash dispatch, config loaders, the skin engine, the transactional update pipeline, and process-identity rules"
---

# CLI Internals

Companion to `ulak_cli/AGENTS.md` (the rules) — this page holds the longer explanations.

## Update pipeline

The stage-by-stage contract (`plan → snapshot → apply → restart-per-kind → verify → report`) and the
field failure each stage guards are documented in `ulak_cli/AGENTS.md`; user-facing behaviour
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line` (gateway run),
  `ulak_cli.update_cmd._ulak_holder_subcommand` (top-level subcommand of any Ulak argv). Never
...
Built-in skins (`_BUILTIN_SKINS` in `ulak_cli/skin_engine.py`): `default` (classic gold/kawaii),
...
User skins are `~/.ulak/skins/<name>.yaml` with the same keys, activated with `/skin <name>` or
`display.skin: <name>`; the full YAML template is in the
[Skins & Themes](../user-guide/features/skins.md) user guide.

## Profiles: multi-instance support

Ulak supports profiles — fully isolated instances, each with its own `ULAK_HOME` (config, API
keys, memory, sessions, skills, gateway). `_apply_profile_override()` in `ulak_cli/main.py` sets
`ULAK_HOME` before any module imports, so every `get_ulak_home()` reference scopes to the active
profile. Profile operations are HOME-anchored (`_get_profiles_root()` returns
`Path.home() / ".ulak" / "profiles"`, not `get_ulak_home() / "profiles"`) so
`ulak -p coder profile list` sees all profiles regardless of which one is active — intentional.
