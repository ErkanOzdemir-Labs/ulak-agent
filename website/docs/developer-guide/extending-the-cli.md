...
description: "Build wrapper CLIs that extend the Ulak TUI with custom widgets, keybindings, and layout changes"
---

# Extending the CLI

Ulak exposes protected extension hooks on `UlakCLI` so wrapper CLIs can add widgets, keybindings, and layout customizations without overriding the `run()` method or the TUI construction in `ulak_cli/cli_tui_mixin.py` (where these hooks are defined; `UlakCLI` in `cli.py` mixes it in). This keeps your extension decoupled from internal changes.
...
```python
#!/usr/bin/env python3
"""my_cli.py — Example wrapper CLI that extends Ulak."""
...
Called after Ulak registers its own keybindings and before the layout is built. Add your keybindings to `kb`.
