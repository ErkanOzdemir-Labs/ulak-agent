...
Plugins can launch and supervise fresh Ulak child sessions without importing
...
active agent turn fails closed with `No active Ulak parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Ulak can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Ulak's existing unsafe-tool block remains enforced.
