...
Ulak Agent uses a dual compression system and Anthropic prompt caching to
...
Configure via `ulak plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.

For building a context engine plugin, see [Context Engine Plugins](/developer-guide/context-engine-plugin).

## Dual Compression System

Ulak has two separate compression layers that operate independently:
...
value is kept per `model@host` in `~/.ulak/cache/image_token_costs.json` and
...
compression:
  ...
  codex_app_server_auto: native  # native|ulak|off for Codex app-server thread compaction
...
| `codex_app_server_auto` | `native` | `native`, `ulak`, `off` | Thread-compaction mode for Codex app-server sessions (see below) |
...
OAuth (`provider: openai-codex`) and the model is gpt-5.5, Ulak raises the
trigger to **85%** (~231K) and shows a notice with the opt-out command. The
notice is shown once per profile — a marker under `$ULAK_HOME`
...
```bash
ulak config set compression.codex_gpt55_autoraise false
```

To keep the 85% autoraise but hide only the one-time notice:

```bash
ulak config set compression.codex_gpt55_autoraise_notice false
```
...
for ChatGPT-subscription accounts (live-verified Aug 2026). Ulak keeps the
...
`gpt-5.4-900k`). These are Ulak-side aliases: the suffix is stripped before
...
thread context, so Ulak' auxiliary summarizer cannot shrink it — rewriting the
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  the default `native` lets the app-server decide when to compact and Ulak
  records the resulting compaction events (compression counters, session
  events). Set `ulak` to let Ulak' compression threshold initiate
  app-server compaction, or `off` to disable Ulak-initiated automatic
  compaction entirely (codex may still compact natively).

Ulak' local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep Ulak' summary compressor.
...
encrypted `compaction` output item. Ulak captures that item into the
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. Ulak
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   Ulak can avoid; user-facing docs for `/model`, fallback providers, and
