...
title: "Desktop Plugin SDK (@ulak/plugin-sdk)"
description: "Extend the native Ulak Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Ulak Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `UlakPlugin`.
It imports one module — `@ulak/plugin-sdk` — and gets everything: the app's
...
`$ULAK_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Ulak. This page is the **native
desktop app** (`ulak desktop`) SDK — the `@ulak/plugin-sdk` module and
`$ULAK_HOME/desktop-plugins/`. The **web dashboard** (`ulak dashboard`) has
its own, unrelated plugin system on `window.__ULAK_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Ulak Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$ULAK_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$ULAK_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `UlakPlugin` contract, appear in **Settings → Plugins**,
...
[`ulak-example-plugins`](https://github.com/ErkanOzdemir-Labs/ulak-example-plugins)
repo.

## Quick start — your first plugin

Create `$ULAK_HOME/desktop-plugins/hello/plugin.js` (that's `~/.ulak/...`
by default, or `~/.ulak/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.ulak/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@ulak/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Ulak' }),
      jsx('div', {
...
The only importable specifiers are `@ulak/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `UlakPlugin`:

```ts
interface UlakPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `ulak.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@ulak/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@ulak/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@ulak/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@ulak/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@ulak/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@ulak/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@ulak/plugin-sdk'
...
`window.ulak.send('get-price eth')` (or a declarative
`<button data-ulak-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@ulak/plugin-sdk'
...
for requests; `targetProfile` is the backend Ulak profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Ulak
...
ctx.os.notify({
  ...
  // Body click → focus Ulak + navigate. Same vocabulary as OS deep links:
  activate: 'ulak://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'ulak://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `ulak://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `ulak://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@ulak/plugin-sdk'
...
a package into `~/.ulak/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.ulak/plugins` is not reachable as a filesystem —
...
the `ulak://` scheme — a plain anchor on your website or README:
...
`dashboard/` subfolder of a regular Ulak plugin and declare it in a
`manifest.json`:
...
ulak-agent codebase directly (`ulak_state`, `ulak_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.ulak/`)
...
(`ulak.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `UlakPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@ulak/plugin-sdk` alias.
2. It's still lint-fenced to `@ulak/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@ulak/plugin-sdk`, `react`,
...
| Plugin contract | `UlakPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `UlakOpenTarget`, `Contribution` |
...
### Agents: the `ulak-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`ulak-desktop-plugins`** skill — it carries the same contract as this page in
...
`$ULAK_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `ulak logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@ulak/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.ulak/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.ulak/logs/errors.log` for
