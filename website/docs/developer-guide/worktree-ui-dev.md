...
| Variable | Meaning |
|----------|---------|
| `ULAK_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `ULAK_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `ULAK_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Ulak itself — they're private to these helpers. The variables Ulak *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `ulak --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_ulak_root)" || { echo "htui: not in a Ulak checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$ULAK_MAIN_CHECKOUT/.venv/bin/python" -m ulak_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `ULAK_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_ulak_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `ULAK_TUI_DIR` are mutually exclusive
`ULAK_TUI_DIR` points Ulak at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `ulak --tui --dev` exits with an error. Run `unset ULAK_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_ulak_root)" || { echo "hgui: not in a Ulak checkout" >&2; return 1; }
  deps="${ULAK_GUI_DEPS_CHECKOUT:-$ULAK_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _ulak_link_deps "$desktop" "$deps/apps/desktop"
    _ulak_link_deps "$root" "$deps"
  else
  ...
  trap '_ulak_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    ULAK_DESKTOP_ULAK_ROOT="$root" \
    ULAK_DESKTOP_PYTHON="$ULAK_MAIN_CHECKOUT/.venv/bin/python" \
    ULAK_DESKTOP_IGNORE_EXISTING=1 \
    ULAK_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `ULAK_DESKTOP_ULAK_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `ulak`. |
| `ULAK_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `ULAK_DESKTOP_IGNORE_EXISTING` | Ignores any `ulak` on `PATH` so it can't shadow the worktree. |
| `ULAK_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Ulak checkout.
_ulak_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/ulak_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_ulak_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'ulak_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `ulak --tui --dev` and the `ULAK_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/ErkanOzdemir-Labs/ulak-agent/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `ULAK_*` variable Ulak reads
