...
Ulak runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.ulak/plugins/`, registering a backend the user selects
...
| `ulak setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `ulak status` / `ulak doctor` | `doctor_checks()` |
...
```python title="~/.ulak/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.ulak"    # where synced cache files land, or None
...
```yaml title="~/.ulak/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
ulak plugins enable acmebox
ulak config set terminal.backend acmebox
```
