...
description: "How to build a memory provider plugin for Ulak Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give Ulak Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `ulak plugins`.
:::

## Installation Layouts

Ulak discovers memory providers from four sources, in this precedence order:

| Source | Location | Notes |
|---|---|---|
| Bundled | `plugins/memory/<name>/` | Ships with Ulak. Closed to new providers — see [CONTRIBUTING](https://github.com/ErkanOzdemir-Labs/ulak-agent/blob/main/CONTRIBUTING.md). |
| User | `$ULAK_HOME/plugins/<name>/` | Dropped in by the user, per profile. |
| Project | `./.ulak/plugins/<name>/` | Opt-in via `ULAK_ENABLE_PROJECT_PLUGINS=1`. |
| Package | `ulak_agent.memory_providers` entry point | `pip install`, nothing to copy. |
...
Ulak, in `$ULAK_HOME/plugins/<name>/` when installed by a user, or in
`./.ulak/plugins/<name>/` for a project-local one:
...
`ulak_agent.memory_providers` group. The entry-point name is the provider
name users select in `memory.provider`; its value points to the provider's
`register(ctx)` function:

```toml title="pyproject.toml"
[project.entry-points."ulak_agent.memory_providers"]
...
package layout. No copy under `$ULAK_HOME/plugins/` is required.

A package entry point gets everything a directory install does, including the
two files Ulak reads from disk rather than importing — `config_schema.py`
(the dashboard config panel) and `cli.py` (your `ulak <provider>`
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          ulak_home (str): Active ULAK_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `ulak memory setup` | **Yes** |
| `save_config(values, ulak_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
The gate binds to every compaction authority, not just the Ulak
...
required checkpoint cannot be guaranteed there. The checkpoint-aware Ulak
...
`get_config_schema()` returns a list of field descriptors used by `ulak memory setup`:
...
Every field in `get_config_schema()` is prompted during `ulak memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$ULAK_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::

## Save Config

```python
def save_config(self, values: dict, ulak_home: str) -> None:
    """Write non-secret config to your native location."""
    import json
    from pathlib import Path
    config_path = Path(ulak_home) / "my-provider.json"
...
the `messages` parameter; Ulak will continue calling them with the legacy
signature.
...
All storage paths **must** use the `ulak_home` kwarg from `initialize()`, not hardcoded `~/.ulak`:

```python
# CORRECT — profile-scoped
from ulak_constants import get_ulak_home
data_dir = get_ulak_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.ulak/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `ulak my-provider status`, `ulak my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `ulak <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `ulak --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: ulak my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the ulak my-provider argparse tree.
