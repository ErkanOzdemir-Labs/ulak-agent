...
sidebar_position: 1
title: "Architecture"
description: "Ulak Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Ulak Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ ulak_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # UlakCLI facade — mixins in ulak_cli/cli_*_mixin.py
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── ulak_state.py           # SQLite session/state database facade (+ ulak_state_*.py siblings)
├── ulak_constants.py       # ULAK_HOME, profile-aware paths
...
│
├── ulak_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — `ulak` subcommands (parsers in subcommands/, main_*.py)
...
│   ├── skills_config.py      # ulak skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # ulak tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # ulak gateway start/stop
│
...
```text
User input → UlakCLI.process_input()
...
Three discovery sources: `~/.ulak/plugins/` (user), `.ulak/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `ulak plugins` or `config.yaml`.
...
Exposes Ulak as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`ulak -p <name>`) gets its own ULAK_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
