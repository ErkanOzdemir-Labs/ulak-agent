...
Ulak fetches curated model lists for **OpenRouter** and **Nous Portal** from a JSON manifest hosted alongside the docs site. This lets maintainers update picker lists without shipping a new `ulak-agent` release.

When the manifest is unreachable (offline, network blocked, hosting failure), Ulak silently falls back to the in-repo snapshot that ships with the CLI. The manifest never breaks the picker — worst case you see whatever list was bundled with your installed version.

## Live manifest URL

```
https://ulak-agent.github.com/ErkanOzdemir-Labs/docs/api/model-catalog.json
```
...
- **`version`** — integer schema version. Future schemas bump this; Ulak refuses manifests with versions it doesn't understand and falls back to the hardcoded snapshot.
- **`metadata`** — free-form dict at the manifest, provider, and model level. Any keys. Ulak ignores unknown fields, so you can annotate entries (`"tier": "paid"`, `"tags": [...]`, etc.) without coordinating a schema change.
...
- **`default`** — exactly one entry per provider may carry `"default": true`. That model is the **silent default**: what Ulak lands on when the user never selected a model (GUI onboarding confirm card, `provider` configured with no `model`, empty `model.default`). Read cache-only at runtime (`get_default_model_from_cache`) so hot resolution paths never hit the network; when no cached manifest exists, Ulak falls back to the in-repo `PREFERRED_SILENT_DEFAULT_MODEL` constant, which must match the labeled entry. This lets maintainers rotate the silent default without shipping a release. It is deliberately a capable low-cost model, never the priciest flagship.
...
| When | What happens |
|---|---|
| `/model` or `ulak model` | Fetches if disk cache is stale, else uses cache |
...
Cache location: `~/.ulak/cache/model_catalog.json`.
...
The exclusion is matched case-insensitively against every key a provider can surface under — the Ulak id and models.dev id (built-in mapped providers), the overlay pid and resolved Ulak slug (overlay providers), and the canonical slug (canonical providers) — so a single entry like `copilot` hides the provider regardless of which section emits it. It is honored by every `/model` picker surface: the gateway interactive/text pickers, the TUI picker, and the interactive `ulak model` CLI picker. An empty list (or omitting the key) has no effect.
...
# editing OPENROUTER_MODELS or _PROVIDER_MODELS["nous"] in ulak_cli/models.py).
