...
of fields, and Ulak schedules it as a cron job — no cron syntax required.
...
- **CLI, TUI, and messengers** — type `/blueprint <name>` (e.g.
  `/blueprint morning-brief`) and Ulak asks you for what it needs, one
...
A blueprint is just a skill with a `metadata.ulak.blueprint` block in its
