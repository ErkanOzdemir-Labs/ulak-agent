...
This page covers all commands related to [Ulak profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `ulak profile` without a subcommand shows help.
...
```bash
ulak profile use <name>
```

Sets `<name>` as the active profile. All subsequent `ulak` commands (without `-p`) will use this profile.
...
ulak profile use default
```

## `ulak profile create`

```bash
ulak profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `ulak profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `ulak update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.ulak`), use `ulak skills opt-out` / `ulak skills opt-in`. |
...
ulak profile create work2 --clone-from work

# Clone everything from a specific profile
ulak profile create work2-backup --clone-from work --clone-all
```

## `ulak profile describe`

```bash
ulak profile describe [<name>] [options]
```
...
ulak profile describe researcher

# Set it explicitly
ulak profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
ulak profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
ulak profile describe --all --auto
```

## `ulak profile delete`

```bash
ulak profile delete <name> [options]
```
...
```bash
ulak profile delete mybot
ulak profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.ulak`) cannot be deleted — use `ulak uninstall` to remove everything.
:::

## `ulak profile show`

```bash
ulak profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Ulak home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ ulak profile show work
Profile: work
Path:    ~/.ulak/profiles/work
...
```bash
ulak profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Ulak installation.
...
```bash
ulak profile alias work
# Creates/updates ~/.local/bin/work

ulak profile alias work --name mywork
# Creates ~/.local/bin/mywork

ulak profile alias work --remove
...
```bash
ulak profile rename <old-name> <new-name>
```
...
```bash
ulak profile rename mybot assistant
# ~/.ulak/profiles/mybot → ~/.ulak/profiles/assistant
...
```bash
ulak profile export <name> [options]
```
...
```bash
ulak profile export work
# Creates work.tar.gz in the current directory

ulak profile export work -o ./work-2026-03-29.tar.gz
```
...
```bash
ulak profile import <archive> [options]
```
...
```bash
ulak profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

ulak profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `ulak -p telemetry`). |
...
ulak profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
ulak profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
ulak profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
ulak profile install ./telemetry/
```

### `ulak profile update`

```bash
ulak profile update <name> [--force-config] [--yes]
```
...
```bash
ulak profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Ulak version, author, env var requirements, the source URL/path, and
...
`ulak profile list` also shows the distribution name and version in a
`Distribution` column, and `ulak profile show <name>` / `delete <name>`
...
ulak profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
ulak profile install https://github.com/your-org/internal-assistant.git
```
...
`ulak_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Ulak version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Ulak can clone from.
3. Tell recipients to run `ulak profile install <your-repo-url>`.
...
```bash
ulak -p <name> <command> [options]
ulak --profile <name> <command> [options]
```

Global flag to run any Ulak command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
ulak -p work chat -q "Check the server status"
ulak --profile dev gateway start
ulak -p personal skills list
ulak -p work config edit
```

## `ulak completion`

```bash
ulak completion <shell>
```
...
- `ulak profile <TAB>` — subcommands (list, use, create, etc.)
- `ulak profile use <TAB>` — profile names
- `ulak -p <TAB>` — profile names
