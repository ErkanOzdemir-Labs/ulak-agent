...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Ulak Agent — no API key required"
---

# MiniMax OAuth

Ulak Agent supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Ulak automatically refreshes your session.
...
# → Ulak opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.ulak/auth.json` and are refreshed automatically before each session.
...
```bash
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.ulak/.env
```
...
```bash
ulak auth add minimax-oauth --no-browser
```

Ulak will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

Ulak implements a PKCE browser OAuth flow against the MiniMax OAuth endpoints:

1. Ulak generates a PKCE verifier / challenge pair and a random state value.
...
4. Ulak polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.ulak/auth.json` under the `minimax-oauth` key.
...
```bash
ulak config set model.default MiniMax-M2.7
ulak config set model.provider minimax-oauth
```

## Configuration Reference

After login, `~/.ulak/config.yaml` will contain entries similar to:
...
```bash
ulak --provider minimax-oauth    # canonical
ulak --provider minimax-portal   # alias
ulak --provider minimax-global   # alias
ulak --provider minimax_oauth    # alias (underscore form)
```
...
To use `minimax-oauth` as the active provider, set `model.provider: minimax-oauth` in `config.yaml` (use `ulak setup` for the guided flow), or pass `--provider minimax-oauth` for a single invocation:

```bash
ulak --provider minimax-oauth
```
...
Ulak refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Ulak marks the session as requiring re-login.
...
**Fix:** run `ulak auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Ulak raises a timeout error.

**Fix:** re-run `ulak auth add minimax-oauth` (or `ulak model`). The flow starts fresh.

### State mismatch (possible CSRF)

Ulak detected that the `state` value returned by the authorization server does not match what it sent.
...
If `ulak` cannot open a browser window, use `--no-browser`:

```bash
ulak auth add minimax-oauth --no-browser
```

Ulak prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `ulak model` and select MiniMax (OAuth), or run `ulak auth add minimax-oauth`.
...
```bash
ulak auth logout minimax-oauth
```
...
- [ulak doctor](../reference/cli-commands.md)
