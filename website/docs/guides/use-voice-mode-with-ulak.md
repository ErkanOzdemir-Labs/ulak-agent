---
sidebar_position: 8
title: "Use Voice Mode with Ulak"
description: "A practical guide to setting up and using Ulak voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Ulak
...
- you want Ulak sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Ulak.
...
## Step 1: make sure normal Ulak works first
...
Ulak supports both local and cloud speech stacks.
...
### If you use `ulak setup`

If you choose NeuTTS in the setup wizard, Ulak checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Ulak transcribes and responds
...
- asking Ulak to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Ulak loop.

## Tuning CLI behavior

### Silence threshold

If Ulak starts/stops too aggressively, tune:
...
Ulak stays a normal chat bot, but can speak replies.
...
- you want Ulak to function like a portable research or ops assistant
...
Ulak joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Ulak detects speech boundaries
- transcripts are posted in the associated text channel
- Ulak responds in text and audio
...
1. get text Ulak working
2. run `ulak setup tts` to enable voice support
