...
This tutorial walks you through setting up a Telegram bot powered by Ulak Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Ulak Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.ulak/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Ulak itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Ulak Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_ulak_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Ulak Agent. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u ulak-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `ulak gateway install` to update the plist.
:::
...
```bash
ulak gateway stop && ulak gateway start
```
...
ulak pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.ulak/.env`:
...
Control how much detail the bot shows when using tools. In `~/.ulak/config.yaml`:
...
Customize how the bot communicates by editing `~/.ulak/SOUL.md`:

For a full guide, see [Use SOUL.md with Ulak](/guides/use-soul-with-ulak).
...
```bash
# From the CLI
ulak cron list          # View all scheduled jobs
ulak cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u ulak-gateway` (Linux) or `~/.ulak/logs/gateway.log` (macOS) |
| Cron job output | `~/.ulak/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.ulak/cron/jobs.json` |
| Pairing data | `~/.ulak/pairing/` |
| Session history | `~/.ulak/sessions/` |
