...
description: "Diagnose and fix common Ulak cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`ulak gateway` for foreground, or `ulak gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `ulak cron tick`.
...
```bash
date
ulak cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.ulak/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.ulak/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.ulak/.env` |
...
| `local` | Write access to `~/.ulak/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `ulak cron list` for updated `last_error` field (if available).
...
### Check 5: Relay-fronted platforms (Ulak Cloud / Team Gateway)
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `ulak skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `ulak skills list`.
...
2. `~/.ulak/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `ulak cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Ulak config directory). Verify:
```bash
ls ~/.ulak/scripts/your-script.py   # Must exist
ulak cron edit <job_id> --script ~/.ulak/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `ulak skills install <skill-name>`.
...
Jobs are stored in `~/.ulak/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.ulak/cron/jobs.json
chmod 600 ~/.ulak/cron/jobs.json   # Your user should own it
```
...
```bash
ulak cron list                    # Show all jobs, states, next_run times
ulak cron run <job_id>            # Schedule for next tick (for testing)
ulak cron edit <job_id>           # Fix configuration issues
ulak logs                         # View recent Ulak logs
ulak skills list                  # Verify installed skills
```
...
1. Run the job with `ulak cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.ulak/logs/agent.log` for scheduler messages and `~/.ulak/logs/errors.log` for warnings
3. Open an issue at [github.com/ErkanOzdemir-Labs/ulak-agent](https://github.com/ErkanOzdemir-Labs/ulak-agent) with:
