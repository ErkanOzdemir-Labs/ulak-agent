...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `ulak send`."
---

# Pipe Script Output to Messaging Platforms

`ulak send` is a small, scriptable CLI that pushes a message to any
messaging platform Ulak is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | ulak send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `ulak
...
ulak send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
ulak send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
ulak send --list

# Filter by platform
ulak send --list telegram
```
...
Any platform Ulak ships adapters for works as a target:
...
`ulak send` resolves the message body in this order:

1. **Positional argument** — `ulak send --to telegram "hi"`
2. **`--file PATH`** — `ulak send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | ulak send --to telegram`

When stdin is a TTY (no pipe), Ulak does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  ulak send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `ulak send` reuses your Ulak config, the same script works on
any host where Ulak is installed — no need to export bot tokens into
...
`ulak send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  ulak send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | ulak send \
...
```bash
./train.py --epochs 200 && \
  ulak send --to telegram "training done" || \
  ulak send --to telegram "training failed (exit $?)"
```
...
ulak send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(ulak send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `ulak send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `ulak send` calls
the platform's REST endpoint directly using credentials from
`~/.ulak/.env` and `~/.ulak/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `ulak gateway start` and retry.
...
The listing is built from `~/.ulak/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`ulak gateway
...
| Approach | Multi-platform | Reuses Ulak creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `ulak send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`ulak send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `ulak send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `ulak send` shares with cron delivery.
