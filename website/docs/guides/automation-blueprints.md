...
Copy-paste blueprints for common automation patterns. Each blueprint uses Ulak's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`ulak webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `ulak webhook subscribe`) |
...
```bash
ulak cron create "0 2 * * *" \
  "You are a project manager triaging the ErkanOzdemir-Labs/ulak-agent GitHub repo.

1. Run: gh issue list --repo ErkanOzdemir-Labs/ulak-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
ulak webhook subscribe github-pr-review \
  --events "pull_request" \
...
ulak cron create "0 9 * * 1" \
  "Scan the ErkanOzdemir-Labs/ulak-agent repo for documentation drift.

1. Run: gh pr list --repo ErkanOzdemir-Labs/ulak-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (ulak_cli/commands.py, ulak_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (ulak_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
ulak cron create "0 6 * * *" \
  "Run a dependency security audit on the ulak-agent project.

1. cd ~/.ulak/ulak-agent && source .venv/bin/activate
...
```bash
ulak webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
ulak webhook subscribe alert-triage \
...
```python title="~/.ulak/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Ulak-Monitor/1.0"})
...
ulak cron create "every 30m" \
  ...
  --script ~/.ulak/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
ulak cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Ulak Agent development." \
...
```bash
ulak webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
ulak webhook subscribe stripe-payments \
...
```bash
ulak cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the ulak-agent codebase.
