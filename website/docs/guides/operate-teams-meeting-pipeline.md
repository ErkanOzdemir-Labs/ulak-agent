...
```bash
ulak teams-pipeline token-health
ulak teams-pipeline token-health --force-refresh
```
...
```bash
ulak teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
ulak teams-pipeline maintain-subscriptions
ulak teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Ulak cron (recommended if you already run the Ulak gateway)

Ulak ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.ulak/scripts/`. First create the script:

```bash
mkdir -p ~/.ulak/scripts
cat > ~/.ulak/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec ulak teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.ulak/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
ulak cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/ulak-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Ulak Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=ulak
EnvironmentFile=/etc/ulak/env
ExecStart=/usr/local/bin/ulak teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/ulak-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Ulak Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now ulak-teams-pipeline-maintain.timer
systemctl list-timers ulak-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/ulak teams-pipeline maintain-subscriptions >> /var/log/ulak/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.ulak/.env` at the top of a wrapper script that crontab calls.
...
```bash
ulak teams-pipeline subscriptions   # should show expirationDateTime advanced
ulak teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
ulak teams-pipeline list
ulak teams-pipeline list --status failed
ulak teams-pipeline show <job-id>
```

### Replay a stored job

```bash
ulak teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
ulak teams-pipeline fetch --meeting-id <meeting-id>
ulak teams-pipeline fetch --join-web-url "<join-url>"
ulak teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `ulak teams-pipeline maintain-subscriptions --dry-run`
- inspect `ulak teams-pipeline list --status failed`
...
- run `ulak teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `ulak teams-pipeline run`
...
- [ ] `ulak teams-pipeline validate` returns an OK snapshot
- [ ] `ulak teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Ulak cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
