...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `ulak setup --portal`. See [Nous Portal](/integrations/nous-portal).
:::
...
2. **Ulak spins up** a fresh agent session with your prompt
...
- **Gateway running** — the gateway daemon handles cron execution:
  ...
  sudo ulak gateway install --system   # Linux servers: boot-time system service
  # or
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.ulak/cron/output/` and you can read them anytime.
:::
...
Ulak will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Ulak Agent
```
...
Before creating cron jobs, ensure Ulak has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Ulak what you want:
...
Ulak will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Ulak to delegate each topic to a sub-agent:
...
Ulak will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
