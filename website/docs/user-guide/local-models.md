...
Ulak can run open models entirely on your own machine. It downloads and
...
sizes, GPU layers, or quantization. You pick a model; Ulak does the rest.
...
2. Click **Install runtime**. Ulak downloads the official llama.cpp
...
That's the whole flow. The server starts and stops with Ulak, restarts
survive app restarts, and switching back to a cloud provider is one click
in the model picker.

## How Ulak picks what to download
...
Models ship in several quality grades (quantizations). Ulak picks the
...
guarantees. Below 4-bit the quality loss is too severe, so Ulak never
...
Local models live or die by memory placement, so Ulak manages it
...
- **Every recommended model gets at least a 64K context window.** When a
  model is larger than your GPU's memory, Ulak deliberately places the
...
- Results show download counts and a per-file fit check sized to your
  ...
  your GPU.
- Anything you download behaves exactly like a catalog model — Ulak
...
If a llama-server is already running on your machine, Ulak detects it
...
[Run Ulak Locally with Ollama](/guides/local-ollama-setup) and
...
```yaml
local_runtime:
  enabled: false     # true = start the managed server with Ulak.
                     # The desktop "Use" button sets this automatically.
  backend: auto      # auto | cuda | metal | vulkan | hip | cpu
  tag: b10362        # pinned llama.cpp release; Ulak updates it with
                     # each release after re-validation
```

Models and runtime builds live under the Ulak home directory
