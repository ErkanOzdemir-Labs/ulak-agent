...
title: "Git Worktrees"
description: "Run multiple Ulak agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Ulak Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Ulak so each session has a clean, isolated working directory.

## Why Use Worktrees with Ulak?

Ulak treats the **current working directory** as the project root:

- CLI: the directory where you run `ulak` or `ulak chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.ulak/config.yaml`
...
Ulak creates `.worktrees/my-experiment/` inside the repo (branch
`ulak/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `ulak-<id>`
...
git worktree add ../repo-feature feature/ulak-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/ulak-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Ulak there:

```bash
cd ../repo-feature

# Start Ulak in the worktree
ulak
```
...
git worktree add ../repo-experiment-a feature/ulak-a
git worktree add ../repo-experiment-b feature/ulak-b
```
...
- Works on its own branch (`feature/ulak-a` vs `feature/ulak-b`).
...
- Ulak checkpoint data under `~/.ulak/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Ulak experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/ulak-checkpoints-docs`, `feature/ulak-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Ulak from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `ulak -w` (Automatic Worktree Mode)

Ulak has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `ulak/ulak-<hash>`).
...
```bash
ulak -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `ulak -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Ulak session its own clean checkout.
