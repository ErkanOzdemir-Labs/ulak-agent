...
sidebar_position: 2
title: "TUI"
description: "Launch the modern terminal UI for Ulak — mouse-friendly, rich overlays, and non-blocking input."
---

# TUI

The TUI is the modern front-end for Ulak — a terminal UI backed by the same Python runtime as the [Classic CLI](cli.md). Same agent, same sessions, same slash commands; a cleaner, more responsive surface for interacting with them.

It's the recommended way to run Ulak interactively.
...
ulak --tui -c
ulak --tui --continue
ulak --tui --resume latest

# Resume a specific session by ID or title
ulak --tui -r 20260409_000000_aa11bb
ulak --tui --resume "my t0p session"
...
```bash
export ULAK_TUI=1
ulak          # now uses the TUI
ulak chat     # same
```

Or make it the persistent default in `~/.ulak/config.yaml`:
...
With `display.interface: tui`, a bare `ulak` (and `ulak chat`) launches the TUI. Explicit flags always win — run `ulak --cli` to drop back to the classic REPL for a single invocation, or `ulak --tui` / `ULAK_TUI=1` to force the TUI when the config default is `cli`.
...
- **Instant first frame** — the banner paints before the app finishes loading, so the terminal never feels frozen while Ulak is starting.
...
- **Node.js** ≥ 20 — the TUI runs as a subprocess launched from the Python CLI. `ulak doctor` verifies this.
- **TTY** — like the classic CLI, piping stdin or running in non-interactive environments falls back to single-query mode.

On first launch Ulak installs the TUI's Node dependencies into `ui-tui/node_modules` (one-time, a few seconds). Subsequent launches are fast. If you pull a new Ulak version, the TUI bundle is rebuilt automatically when sources are newer than the dist.

:::tip Working across git worktrees?
Contributors who run `ulak --tui --dev` from many worktrees can share one `node_modules` instead of installing per checkout — see [TUI & Desktop from Worktrees](../developer-guide/worktree-ui-dev.md).
:::

### External prebuild

Distributions that ship a prebuilt bundle (Nix, system packages) can point Ulak at it:

```bash
export ULAK_TUI_DIR=/path/to/prebuilt/ui-tui
ulak --tui
```
...
| `/reload` | Re-reads `~/.ulak/.env` into the running TUI process so newly added API keys take effect without a restart |
...
Use the live session switcher when you want one terminal to act as a dispatcher for several TUI sessions. It lists only sessions that are currently live in this TUI process; closed sessions remain saved transcripts and can still be reopened with `/resume` or `ulak --tui --resume <id-or-title>`.
...
<img alt="Ulak TUI Session Orchestrator with one live session and a +new row" src="/docs/img/docs/tui-session-orchestrator/session-orchestrator.png" />

<video controls muted loop playsInline src="/docs/img/docs/tui-session-orchestrator/session-orchestrator-demo.mp4" title="Ulak TUI Session Orchestrator demo" style={{maxWidth: '100%'}}></video>
...
1. `ULAK_TUI_THEME` env var — highest priority. Values: `light`, `dark`, or a raw 6-char background hex (e.g. `ffffff`, `1a1a2e`).
...
```bash
export ULAK_TUI_THEME=light
```

## Busy indicator styles

The status-bar busy indicator is pluggable — the default rotates Ulak' kawaii face palette every 2.5 seconds during agent work. Pick a different style via config or the `/indicator` slash command:
...
By default, `ulak --tui` starts a fresh session each launch. To re-attach to the most recent TUI session automatically (useful when your terminal or SSH connection drops unexpectedly), opt in:

```bash
export ULAK_TUI_RESUME=1          # most-recent TUI session
# or:
export ULAK_TUI_RESUME=<session-id>   # specific session
```
...
- **Working directory with git branch** — `~/projects/ulak-agent (docs/two-week-gap-sweep)`. The branch suffix updates when you `git checkout` in a side terminal (mtime-cached) so the TUI reflects your actual active branch, not whatever it was at launch.
...
- **`⚠ YOLO`** — visible warning whenever YOLO mode is on (`ulak --yolo`, `/yolo`, or `ULAK_YOLO_MODE=1`). The same badge also appears in the startup banner so you cannot launch an auto-approving session without noticing.

## Configuration

The TUI respects all standard Ulak config: `~/.ulak/config.yaml`, profiles, personalities, skins, quick commands, credential pools, memory providers, tool/skill enablement. No TUI-specific config file exists.
...
Sessions are shared between the TUI and the classic CLI — both write to the same `~/.ulak/state.db`. You can start a session in one, resume in the other. The session picker surfaces sessions from both sources, with a source tag.
...
You may see a `ULAK_TUI_GATEWAY_URL` env var referenced in the codebase or logs. This is an **internal wiring detail of the web dashboard**, not a user-facing remote-attach knob. When you open the dashboard's "Chat" tab (`ulak dashboard` → `/chat`), the dashboard's web server spawns an embedded TUI child process and injects `ULAK_TUI_GATEWAY_URL` so that child attaches to the dashboard's own in-process `tui_gateway` over a loopback WebSocket (`/api/ws`). The `/api/ws` endpoint exists only inside the dashboard server (`ulak_cli/web_server.py`) and is bound to that process's lifetime and auth.

There is no general "point any TUI at any standalone gateway port" mode. In particular, the OpenAI-compatible API server (`ulak gateway` / the `api_server` platform) does **not** serve `/api/ws` — it's the model-backend surface (`/v1/chat/completions`, `/v1/models`, …) and deliberately does not expose the TUI's JSON-RPC control channel. Setting `ULAK_TUI_GATEWAY_URL` to that port will 404.

If you want multiple surfaces to share one set of sessions, use the shared `~/.ulak/state.db` (see [Sessions](sessions.md)) or the web dashboard's embedded chat (see [Web Dashboard](features/web-dashboard.md#chat)) — not a hand-set gateway URL.

## Reverting to the classic CLI

Launching `ulak` (without `--tui`) stays on the classic CLI by default. To make a machine prefer the TUI, set `display.interface: tui` in `~/.ulak/config.yaml` (persistent) or `ULAK_TUI=1` in your shell profile (per-shell). To go back, set `interface: cli` / unset the env var, or pass `ulak --cli` for a one-off.

If the TUI fails to launch (no Node, missing bundle, TTY issue), Ulak prints a diagnostic and falls back — rather than leaving you stuck.
