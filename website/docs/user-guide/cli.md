...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Ulak Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Ulak Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `ulak setup --portal` — and you're ready to `ulak chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Ulak also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `ulak --tui` — see the [TUI](tui.md) guide.
:::
...
ulak chat --query-file prompt.txt
ulak chat --query-file - < prompt.txt

# With a specific model
ulak chat --model "anthropic/claude-sonnet-4"

# With a specific provider
ulak chat --provider nous        # Use Nous Portal
ulak chat --provider openrouter  # Force OpenRouter

# With specific toolsets
ulak chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
ulak -s ulak-agent-dev,github-auth
ulak chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
ulak --continue             # Resume the most recent CLI session (-c)
ulak --resume <session_id>  # Resume a specific session by ID (-r)
ulak --resume latest        # Resume the most recent session (same as -c)
ulak --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
ulak chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
ulak -w                         # Interactive mode in worktree
ulak -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`ulak -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
ulak worktree list              # audit: age, size, verdict, reason per tree
ulak worktree prune             # remove safe trees + delete merged branches
ulak worktree prune --dry-run   # show the plan without changing anything
ulak worktree prune --trees-only     # leave local branches alone
ulak worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running ulak session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.ulak/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
launches `ulak -w` for days — no longer accumulate merged scratch trees
...
The `ulak plugins` commands manage native Ulak plugins and portable Agent
...
ulak plugins disable <plugin-name>
ulak plugins update <plugin-name>
ulak plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Ulak currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Ulak CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Ulak CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `ULAK_YOLO_MODE` is on (either `ulak --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
On the `openai-codex` provider, `/usage` also shows any banked usage-limit resets on your ChatGPT account ("You have N resets banked - use /usage reset to activate"). `/usage reset` redeems one banked reset, fully restoring your 5-hour and weekly limits. Ulak refuses to redeem while your limits aren't exhausted (a banked reset restores the full allowance, so spending it early wastes it) — pass `/usage reset --force` to redeem anyway.

### Session Resume Display

When resuming a previous session (`ulak -c` or `ulak --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Ulak to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.ulak/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Ulak supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Ulak replies |
...
```bash
ulak -s ulak-agent-dev,github-auth
ulak chat -s github-pr-workflow -s github-auth
```

Ulak loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.ulak/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.ulak/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Ulak..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Ulak also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Ulak recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Ulak — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Ulak is working, Ulak prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Ulak to the background — just like any terminal process. The shell prints a confirmation:

```
Ulak Agent has been suspended. Run `fg` to bring Ulak Agent back.
```
...
Resume this session with:
  ulak --resume 20260225_143052_a1b2c3
...
```bash
ulak --continue                          # Resume the most recent CLI session
ulak -c                                  # Short form
ulak -c "my project"                     # Resume a named session (latest in lineage)
ulak --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
ulak --resume "refactoring auth"         # Resume by title
ulak --resume latest                     # Resume the most recent session (same as -c)
ulak --resume latest --in ./my-project   # Latest session for ./my-project's workspace
ulak -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `ulak sessions rename <id> <title>` from the command line. Use `ulak sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Ulak's SQLite state database under `~/.ulak/state.db`. The database keeps:
...
Ulak immediately confirms the task and gives you back the prompt:
