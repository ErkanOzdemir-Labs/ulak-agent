...
`~/.ulak/config.yaml`, `~/.ulak/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/ulak`:

```text
/etc/ulak/
├── config.yaml     # managed config layer (wins over ~/.ulak/config.yaml)
└── .env            # managed env layer (wins over ~/.ulak/.env + shell)
```
...
The location can be relocated with the `ULAK_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `ULAK_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Ulak.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export ULAK_MANAGED_DIR=/opt/org/ulak-policy
```

:::warning
A user who can set `ULAK_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `ulak doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/ulak/config.yaml` (managed) | `/etc/ulak/.env` (managed) |
| 2 | `~/.ulak/config.yaml` (user) | `~/.ulak/.env` (user) |
...
```bash
ulak config        # shows a header naming the managed source + the pinned keys
ulak doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Ulak refuses and names the source:

```bash
$ ulak config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/ulak/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `ulak config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/ulak

# Pin some config values for every user on this machine
sudo tee /etc/ulak/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/ulak/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/ulak
sudo chmod 0644 /etc/ulak/config.yaml /etc/ulak/.env
```

Changes take effect on the next Ulak start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`ulak doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Ulak as `root`), managed scope is advisory.
