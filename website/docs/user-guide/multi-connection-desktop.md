...
# Connecting Desktop to Many Ulak Instances

Register every Ulak backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and Ulak Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another Ulak gateway…"**) deep-links straight to
...
list of Ulak gateways. Its intro says it plainly: *"Manage this device and
every Ulak gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The Ulak runtime managed by this app." | automatic |
| **Remote gateway** | "A Ulak gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A Ulak install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **Ulak Cloud** | "A hosted instance discovered through your Ulak Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the Ulak Cloud sign-in/discovery flow at
  the top of the Gateways page — the **Ulak Cloud** kind in the add-connection
...
Gateways are machines or hosted backends; profiles are isolated Ulak agents
...
connection is registered) dispatches `ulak update` to every eligible
...
- **Ulak Cloud** instances are skipped with a *"Managed by Ulak Cloud"*
...
panel, ⌘K **Update Ulak**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `ulak serve` is running on the remote host, the
...
- **"Update Ulak Desktop to chat with agents on other connections"** — the
