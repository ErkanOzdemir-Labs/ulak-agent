...
sidebar_position: 30
title: "Ulak Relay"
description: "Connect Ulak to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# Ulak Relay (Connector)
...
Ulak Relay is not a chat platform itself — it is a **connector system** that
...
secret. `ulak gateway enroll` redeems a **single-use enrollment token**
...
1. Resolves a fresh Nous Portal access token from your existing login
   (`~/.ulak/auth.json`) — this proves which Nous org (tenant) you own. If
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.ulak/.env`:
...
`ulak gateway enroll` refuses to run in managed/hosted installs — there the
...
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.ulak/.env`) | Connector relay WebSocket URL. Presence enables the relay platform. |
...
token. Re-login with `ulak auth add nous` (or `ulak setup`) and retry.
...
startup. Restart it (`ulak gateway restart`).
