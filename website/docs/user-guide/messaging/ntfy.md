...
ntfy makes a great lightweight push channel for Ulak: subscribe to a topic from the [ntfy mobile app](https://ntfy.sh/docs/subscribe/phone/), send messages to the topic to talk to the agent, get the response back on your phone.

> Run `ulak gateway setup` and pick **ntfy** for a guided walk-through.

## Prerequisites

- A topic name (any unique string — `ulak-myname-2026` works fine)
...
That's it. No SDK, no daemon, no Node.js. The adapter uses `httpx` which is already a Ulak dependency.
...
Add these to `~/.ulak/.env`:

```
NTFY_TOPIC=ulak-myname-2026
NTFY_ALLOWED_USERS=ulak-myname-2026
NTFY_HOME_CHANNEL=ulak-myname-2026
```
...
ntfy has no native authenticated user identity. The `title` field on a published message is **publisher-controlled** and can be anything the sender wants. The Ulak adapter does NOT use `title` for authorization — it would let any publisher who knows the topic spoof an allowed user.
...
- Or **pick a long, unguessable topic name** (`ulak-7d4f9c8b-2026`) and treat it as the shared secret. This is the lightest setup but the topic name leaks via any logs or screenshots.
...
1. Pick a topic name: `ulak-myname-2026`
2. On your phone: install the [ntfy app](https://ntfy.sh/docs/subscribe/phone/), tap **+**, enter `ulak-myname-2026`
3. On the host:
   ```bash
   echo 'NTFY_TOPIC=ulak-myname-2026' >> ~/.ulak/.env
   echo 'NTFY_ALLOWED_USERS=ulak-myname-2026' >> ~/.ulak/.env
...
Or target a specific topic explicitly via the cron job's `deliver:` field, or from a shell script with the [`ulak send` CLI](/guides/pipe-script-output):

```bash
ulak send ntfy:alerts-channel "Done!"
```
...
ntfy clients render markdown when the publisher sets the `X-Markdown: true` header. To enable for outgoing Ulak replies:
