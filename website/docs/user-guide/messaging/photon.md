...
Connect Ulak to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Ulak — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Ulak runs it in a
...
ulak photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Ulak Agent` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.ulak/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.ulak/.env`
...
(device token, dashboard project id) lives in `~/.ulak/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Ulak
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Ulak replies with a pairing code. Approve it with:

```bash
ulak pairing approve photon <CODE>
```

Use `ulak pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.ulak/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.ulak/.env`):
...
By default Ulak responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Ulak` and
`@Ulak agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Ulak will reply.
...
assigned iMessage line Ulak uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run ulak photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `ulak photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `ulak photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `ulak photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Ulak sends images, voice
  ...
  media.
- **Native polls are supported.** Ulak sends poll content through
...
- **Read receipts are supported.** The sidecar marks an inbound iMessage
  read after forwarding it to Ulak, so the sender sees `Read` without
  waiting for a model/tool turn. Inbound receipts for Ulak-sent messages
...
- **Message effects are supported.** Ulak sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `ulak send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<ulak-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Ulak is set
...
| `PHOTON_MENTION_PATTERNS` | Ulak wake words  | JSON list / comma / newline regex patterns for group mentions |
