...
Connect Ulak to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Ulak supports two WeCom integration modes:
...
> Run `ulak gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Ulak decrypts the message, queues it for the agent
...
(Use `ulak gateway start` only after `ulak gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Ulak and the token the
...
in `~/.ulak/.env` values around `=` will also break signature checks. After
fixing, restart `ulak gateway run`.
...
Check `ulak gateway run` logs for the bound host/port. If the adapter bound to
