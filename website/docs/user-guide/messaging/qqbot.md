...
Connect Ulak to QQ via the **Official QQ Bot API (v2)** — supporting private (C2C), group @-mentions, guild, and direct messages with voice transcription.
...
Set the required environment variables in `~/.ulak/.env`:
...
For fine-grained control, add platform settings to `~/.ulak/config.yaml`:
