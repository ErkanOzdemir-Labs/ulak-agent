...
sidebar_position: 12
title: "Google Chat"
description: "Set up Ulak Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Ulak Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Ulak
...
> Run `ulak gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Ulak can read (e.g.,
`~/.ulak/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a ulak restart)
...
- Principal: `ulak-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Ulak calls
...
- **App name**: whatever you want users to see ("Ulak" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/ulak-chat-events`.
...
`ADDED_TO_SPACE` event that Ulak uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Ulak

Add the Google Chat section to `~/.ulak/.env`:
...
Send "hola" in the test DM. The bot posts a "Ulak is thinking…" marker, then
...
`~/.ulak/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Ulak is thinking…").
...
Thread support: when a user replies inside a thread, Ulak detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Ulak session.
...
3. Download the JSON. Move it onto the host that runs Ulak.
4. Register the client with Ulak (run under the profile you want it scoped to):
...
ulak -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Ulak home (e.g.
`~/.ulak/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.ulak/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.ulak/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Ulak isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `ulak gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Ulak will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.ulak/google_chat_user_tokens/<sanitized_email>.json` (filesystem
