...
If you only run one Ulak agent, you don't need this page — see
...
[Connecting Desktop to Many Ulak Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Ulak agents that should all
...
(`ai.ulak.gateway-<name>.plist`) or systemd user service
(`ulak-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
ulak profile create coder
ulak profile create personal-bot
ulak profile create research
...
```bash
ulak config set gateway.multiplex_profiles true
ulak gateway restart
```

Equivalently, in the default profile's `~/.ulak/config.yaml`:
...
You do **not** run `ulak gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `ulak gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.ulak/profiles/coder/.env`; the default listener key is rejected.
...
home). `ulak status` reports the multiplexer and the profiles it serves;
`ulak status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/ulak-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: ulak-gateways {start|stop|restart|status|list}"
}
...
```bash
ulak-gateways start      # start every configured profile
ulak-gateways stop       # stop every configured profile
ulak-gateways restart    # restart all
ulak-gateways status     # status across all
ulak-gateways list       # delegates to `ulak gateway list`
```

:::tip
The `default` profile is targeted with `ulak gateway <action>` (no `-p`),
not `ulak -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `ulak -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.ulak.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/ulak-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.ulak.gateway.plist` /
`ulak-gateway.service`.
...
```bash
# Default profile
tail -f ~/.ulak/logs/gateway.log
tail -f ~/.ulak/logs/gateway.error.log

# Named profile
tail -f ~/.ulak/profiles/<name>/logs/gateway.log
tail -f ~/.ulak/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.ulak/logs/gateway.log ~/.ulak/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
ulak logs -f                  # follow default profile
ulak -p coder logs -f         # follow one profile
ulak logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
ulak profile list             # profiles + model + gateway state
ulak-gateways status          # full status across every profile
launchctl list | grep ulak    # macOS — PIDs and labels
systemctl --user list-units 'ulak-gateway-*'   # Linux — units
```
...
The default profile uses `~/.ulak/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
ulak config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.ulak/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=ulak --why="gateways running" \
  sleep infinity &
...
`ulak-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.ulak/.env ~/.ulak/profiles/*/.env
```

## Updating the code

`ulak update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `ulak gateway start` after a previous `ulak gateway stop`. The
...
```bash
ps -ef | grep "ulak_cli.*-p <profile>"
cat ~/.ulak/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.ulak.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.ulak.gateway-<profile>.plist

# Linux
systemctl --user restart ulak-gateway-<profile>.service
```

### Health check

```bash
ulak doctor                  # default profile
ulak -p <profile> doctor     # one profile
```
