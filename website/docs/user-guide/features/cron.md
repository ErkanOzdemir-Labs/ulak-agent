...
Schedule tasks to run automatically with natural language or cron expressions. Ulak exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to Ulak itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::tip
**Which model does a cron job run on?** Resolution at fire time is: per-job pin → `cron.model` in `config.yaml` → the global default from `ulak model`.

- **Per-job pin** — set by *you* via the dashboard, `ulak cron create/edit --model … --provider …`, or by editing `~/.ulak/cron/jobs.json`. Once set, it sticks until you change it. The agent's `cronjob` tool cannot set or change per-job models — inference pins are user-owned.
- **`cron.model` / `cron.model_provider`** — a cron-fleet default: every unpinned job runs on this model, independent of your chat model. Set it once (`ulak config set cron.model <name>`) and switching your chat model with `ulak model` or `/model` never touches your cron fleet.
- **Global default** — only when neither of the above is set does a job follow `ulak model`. In this case Ulak **snapshots** the provider and model at creation, and if the global default later changes the job **fails closed**: it skips the run, makes no inference call, and alerts you **once** — the job stays skipped (and silent) on subsequent ticks until you act or the config is restored (#44585). For recurring or otherwise repeatable jobs, pin the provider/model explicitly (`ulak cron edit <job_id> --provider <provider> --model <model>`) to proceed. A consumed finite one-shot cannot be updated; create a new future one-shot with an explicit provider and model instead. This prevents an unattended job from silently inheriting a switch to a paid provider/model. Setting `cron.model` (or a per-job pin) is the deliberate way to route cron spend, and the drift guard does not engage for an axis covered by it. Operators who instead want unpinned jobs to track the changing global default can [disable the drift guard](#letting-unpinned-jobs-track-global-defaults).
...
`ulak setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
**Per-job reasoning effort.** A job can pin its own thinking level, independent of the model pin: one of `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, `ultra`. When set, it overrides both the global `agent.reasoning_effort` and per-model `agent.reasoning_overrides` for that job's runs (`none` disables thinking). Set it via `ulak cron create/edit --reasoning-effort high`; pass an empty string on edit to clear the pin and follow config again. (It is deliberately not exposed on the agent's `cronjob` tool — model configuration stays a user decision.) Levels a model doesn't support are clamped or omitted by the provider at request time — pinning `xhigh` on a model that caps at `high` runs at `high`. The pin has no effect on `no_agent` jobs (there is no LLM call to tune). Use it to run heavy scheduled analyses at `high` while cheap recurring jobs run at `minimal`, without touching your global default.
:::

:::warning
Cron-run sessions cannot recursively create more cron jobs. Ulak disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
ulak cron create "every 2h" "Check server status"
ulak cron create "every 1h" "Summarize new feed items" --skill blogwatcher
ulak cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Ulak will use the unified `cronjob` tool internally.
...
Or: `ulak config set cron.preflight false`
...
```bash
ulak config set cron.model_drift_guard false
```
...
ulak cron create "every 1d at 09:00" \
...
```bash
ulak cron edit <job_id> --schedule "every 4h"
ulak cron edit <job_id> --prompt "Use the revised task"
ulak cron edit <job_id> --skill blogwatcher --skill maps
ulak cron edit <job_id> --add-skill maps
ulak cron edit <job_id> --remove-skill blogwatcher
ulak cron edit <job_id> --clear-skills
```
...
```bash
ulak cron list
ulak cron pause <job_id_or_name>
ulak cron resume <job_id_or_name>
ulak cron run <job_id_or_name>
ulak cron remove <job_id_or_name>
ulak cron edit <job_id_or_name> [...flags]
ulak cron status
ulak cron tick
```
...
```bash
ulak gateway install     # Install as a user service
sudo ulak gateway install --system   # Linux: boot-time system service for servers
ulak gateway             # Or run in foreground
...
1. loads jobs from `~/.ulak/cron/jobs.json`
...
A file lock at `~/.ulak/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.

### Execution history

Ulak records each claimed cron attempt in the profile-local
`~/.ulak/cron/executions.db` before executor or provider dispatch. Attempts
move through `claimed`, `running`, and one immutable terminal state:
`completed`, `failed`, or `unknown`. After restart, Ulak marks an abandoned
...
Inspect recent attempts with `ulak cron runs [job-id] --limit 20` (alias:
...
in a row and suggesting you fix, pause (`ulak cron pause <job>`), or remove
it. Any successful run resets the streak, and `ulak cron list` shows the
...
```bash
ulak cron incidents                 # list incidents (newest activity first)
ulak cron incidents --state alerted # filter: detected | alerted | closed
ulak cron incidents ack <id>        # acknowledge — stop re-pinging
```
...
### Fleet health check: `ulak cron doctor`

`ulak cron doctor` is a read-only health check over every active job. It
...
- script missing, not a file, or resolving outside `ULAK_HOME/scripts`,
...
`ulak cron incidents` (durable failure records) and `ulak cron runs`
...
| `"local"` | Save to local files only (`~/.ulak/cron/output/`) | Default on CLI |
...
`last_delivery_error`. `ulak cron list` shows it in yellow as
`delivery_failed: <reason>`, `ulak cron doctor` reports it as a delivery
...
- `bot-chat:<profile>` targets another profile **on the same machine**. Names are validated against `ulak profile list` when the job is created; profiles on other gateways or machines can never be targeted, so same-named profiles across machines are unambiguous.
...
and in `ulak cron doctor` as `last delivery unverified (...)`. The marker is
...
If the agent's final response contains `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.ulak/cron/output/`), but no message is sent to the delivery target.
...
Or set the `ULAK_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 3600s default.
...
Or set the `ULAK_CRON_MEDIA_SEND_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 300s default. A timed-out attachment is recorded in the job's run status as a partial delivery failure (the text still delivers).
...
`.sh` / `.bash` files run under `bash` from `PATH` when available, otherwise `/bin/bash` (important on Windows Git Bash). Anything else runs under the current Python interpreter (`sys.executable`). Scripts must resolve inside `$ULAK_HOME/scripts/` — relative names, absolute paths, and `~`-prefixed paths are accepted when the resolved target stays in that directory; paths that escape it are rejected. Subprocess env is sanitized (`_sanitize_subprocess_env`): provider API credentials and other Ulak-managed secrets are **not** inherited by cron scripts.

### The agent sets these up for you

The `cronjob` tool's schema exposes `no_agent` to Ulak directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

Ulak will write the check script to `~/.ulak/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.ulak/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.ulak/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.ulak/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.ulak/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, Ulak reads Job 1's most recent output from `~/.ulak/cron/output/{job1_id}/*.md`
...
From the CLI: `ulak cron create "every 6h" "Scan for news" --continuity`, and `ulak cron edit <job_id> --continuity` / `--no-continuity` to toggle it on an existing job. The same toggle appears in the dashboard's cron editor and the desktop Bot Mode routine dialog.
...
- `ulak cron list` — a red `⚠ Missed scheduled fire:` line under the job
- The dashboard job view

The stamp always reflects **current** auto-fire health: it is overwritten by newer misses and cleared automatically by the next successful run. If you see it, the job and its schedule are fine — the gateway side of the fire path needs attention (most commonly, restart the gateway through its supervisor so it loads the full profile environment: `ulak gateway restart`).
...
Runtimes that can't receive detached results (one-shot `ulak -z`, `ulak
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `ulak tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `ulak tools` cron-platform config wins; otherwise Ulak falls back to the built-in defaults. This matters for cost control: carrying `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Ulak should even invoke the agent. Emit a final stdout line of the form:
...
```bash
#!/bin/bash
# ~/.ulak/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.ulak/scripts/.feed-changed.last"
...
:::tip
Ulak's own `~/.ulak/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::

Credit: this recipe set was prompted by @iankar8's exploration in [#2654](https://github.com/ErkanOzdemir-Labs/ulak-agent/pull/2654), which proposed adding sql/file/command triggers as a parallel mechanism. The `script` + `wakeAgent` gate already covers all three cases at $0, so the work landed as documentation instead.
...
Jobs are stored in `~/.ulak/cron/jobs.json`. Output from job runs is saved to `~/.ulak/cron/output/{job_id}/{timestamp}.md`.

Job definitions are plain JSON on disk: they survive `ulak update`, gateway restarts, and machine reboots. A job that was mid-run during a restart is marked `unknown` in the execution ledger — it is not automatically retried, but the job's next scheduled tick fires normally. See [Execution history](#execution-history) for details.

:::tip
Ask the agent to manage jobs through the `cronjob` tool, `ulak cron edit`, or `/cron` — not by patching `jobs.json` directly. Direct edits can fail silently when [file write safety](../security.md#file-write-safety) blocks the path (for example when `ULAK_WRITE_SAFE_ROOT` is set), and the [file-mutation verifier](../configuration.md#file-mutation-verifier) footer is the authoritative signal that nothing was saved.
:::

Jobs may store `model` and `provider` as `null`. When those fields are omitted, Ulak resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
