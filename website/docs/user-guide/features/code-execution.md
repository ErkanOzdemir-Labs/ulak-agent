...
The `execute_code` tool lets the agent write Python scripts that call Ulak tools programmatically, collapsing multi-step workflows into a single LLM turn. The script runs in a child process on the agent host, communicating with Ulak over a Unix domain socket RPC.

## How It Works

1. The agent writes a Python script using `from ulak_tools import ...`
2. Ulak generates a `ulak_tools.py` stub module with RPC functions
3. Ulak opens a Unix domain socket and starts an RPC listener thread
4. The script runs in a child process — tool calls travel over the socket back to Ulak
5. Only the script's `print()` output is returned to the LLM; intermediate tool results never enter the context window

```python
# The agent can write scripts like:
from ulak_tools import web_search, web_extract
...
```python
from ulak_tools import search_files, read_file
import json
...
```python
from ulak_tools import search_files, read_file, patch
...
```python
from ulak_tools import terminal, read_file
import json
...
`execute_code` has two execution modes controlled by `code_execution.mode` in `~/.ulak/config.yaml`:

| Mode | Working directory | Python interpreter |
|------|-------------------|--------------------|
| **`project`** (default) | The session's working directory (same as `terminal()`) | Active `VIRTUAL_ENV` / `CONDA_PREFIX` python, falling back to Ulak's own python |
| `strict` | A temp staging directory isolated from the user's project | `sys.executable` (Ulak's own python) |
...
### `ULAK_*` variables in the child

The child process receives only a small, fixed set of operational `ULAK_*`
...
(plus `ULAK_RPC_DIR` / `ULAK_RPC_SOCKET` / `TZ` / `HOME`, which Ulak
injects explicitly so the RPC channel works).

:::note Behavior change
Earlier versions passed **any** variable whose name began with `ULAK_`
through to the child. That broad prefix was removed for security hardening: it
could leak `ULAK_*`-named configuration that doesn't match a secret substring
(for example `ULAK_BASE_URL`, `ULAK_KANBAN_DB`, or a `ULAK_*_WEBHOOK`
endpoint) into arbitrary sandboxed code.

If an `execute_code` script — or a repo/plugin module it imports at import time
— relied on a `ULAK_*` variable outside the four operational names above, it
...
the secret-stripping guarantee (Ulak-managed provider credentials can never
...
**Diagnosing it.** When the child drops one or more non-allowlisted `ULAK_*`
variables, Ulak emits a one-line `debug` log naming them and pointing at the
`env_passthrough` escape hatch. Run with debug logging (`ulak logs --level
DEBUG`, or check `~/.ulak/logs/agent.log`) and look for
`execute_code: dropped N non-allowlisted ULAK_* var(s)` if a script behaves
as though a `ULAK_*` variable is missing.

Ulak always writes the script and the auto-generated `ulak_tools.py` RPC stub into a temp staging directory that is cleaned up after execution. In `strict` mode the script also *runs* there; in `project` mode it runs in the session's working directory (the staging directory stays on `PYTHONPATH` so imports still resolve). The child process runs in its own process group so it can be cleanly killed on timeout or interruption.
...
**Rule of thumb:** Use `execute_code` when you need to call Ulak tools programmatically with logic between calls. Use `terminal` for running shell commands, builds, and processes.

## Platform Support

Code execution is available on **Linux, macOS, and Windows**. On Linux and macOS the RPC channel uses a Unix domain socket; on Windows, where `AF_UNIX` is unreliable, Ulak automatically falls back to a loopback TCP socket for the sandbox RPC transport. Remote terminal backends (Docker/SSH/Modal/etc.) use a file-based RPC transport instead and additionally require Python 3 inside the backend.
