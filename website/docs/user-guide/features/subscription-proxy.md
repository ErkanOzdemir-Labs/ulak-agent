...
chat completions — use your Ulak-managed provider subscription as their
...
| Use case | "Use Ulak as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Ulak stores
the refresh token in `~/.ulak/auth.json` — the same place all Ulak
...
Model:      Ulak-4-70B    # or Ulak-4.3-36B, Ulak-4-405B
```
...
interface in `ulak_cli/proxy/adapters/`.
...
If you see `not logged in`, run `ulak portal`. If you see
...
supports; check `portal.github.com/ErkanOzdemir-Labs/models`.
...
[portal.github.com/ErkanOzdemir-Labs](https://portal.github.com/ErkanOzdemir-Labs).
...
`ulak_cli/proxy/adapters/<provider>.py` and registering it in
