...
Ulak runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Ulak
...
1. Ulak captures a baseline of current diagnostics for the file.
...
…). Ulak auto-detects the binary on PATH or in
`<ULAK_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Ulak at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<ULAK_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`ulak lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Ulak installs both packages together when you
run `ulak lsp install typescript` or auto-install fires on first
use.

## CLI

```
ulak lsp status          # service state + per-server install status
ulak lsp list            # registry, optionally --installed-only
ulak lsp install <id>    # eagerly install one server
ulak lsp install-all     # try every server with a known recipe
ulak lsp restart         # tear down running clients
ulak lsp which <id>      # print resolved binary path
```

`ulak lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <ULAK_HOME>/lsp/bin
...
When `install_strategy: auto`, Ulak installs binaries into
`<ULAK_HOME>/lsp/bin/`. NPM packages land in
`<ULAK_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Ulak-owned and is
...
**single process** per Ulak process: the first Python project spawns
...
**`ulak lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<ULAK_HOME>/lsp/bin/`. Run
`ulak lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `ulak lsp status`**
...
When `ulak lsp status` shows a `Backend warnings` section, install
...
Check `~/.ulak/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `ulak lsp restart` to clear the set;
