...
Ulak Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
ulak memory setup      # interactive picker + configuration
ulak memory status     # check what's active
ulak memory off        # disable external provider
```

You can also select the active memory provider via `ulak plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.ulak/config.yaml`:
...
When a memory provider is active, Ulak automatically:
...
**Setup Wizard:**
```bash
ulak memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `ulak honcho setup` command still works (it now redirects to `ulak memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
**Config:** `$ULAK_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$ULAK_HOME/honcho.json` > `~/.ulak/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/ErkanOzdemir-Labs/ulak-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/ulak).
...
:::tip Migrating from `ulak honcho`
If you previously used `ulak honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Ulak profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Ulak profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Ulak profile. Host key `ulak` → default; `ulak.<profile>` for others. |
...
```bash
ulak profile create coder --clone
```

`--clone` creates a `ulak.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Ulak profile, creates host blocks for any profile without one, inherits settings from the default `ulak` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `ulak memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/ErkanOzdemir-Labs/ulak-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/ulak).
...
# Then configure Ulak
ulak memory setup    # select "openviking"
# Or manually:
ulak config set memory.provider openviking
```

`ulak memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.ulak/.env`, and for named profiles use
`~/.ulak/profiles/<profile>/.env`.
...
Peer identity is optional. By default, Ulak sends no peer ID and writes
...
Set `memory.openviking.agent: ulak` to restore the old peer-scoped writes.
...
Ulak sends `User-Agent: openviking-memory-ulak/<version>` on OpenViking
...
**Setup (Platform):**
```bash
ulak memory setup    # select "mem0" → "Platform"
# Or manually:
ulak config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.ulak/.env
```

**Setup (OSS):**
```bash
ulak memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
ulak memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
ulak memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
ulak memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
ulak memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.ulak/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.ulak/.env
```
...
**Config:** `$ULAK_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.ulak/.env`.
...
| `user_id` | `ulak-user` | User identifier |
| `agent_id` | `ulak` | Agent identifier |
...
**Switching modes:** Re-run `ulak memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
ulak memory setup    # select "hindsight"
# Or manually:
ulak config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.ulak/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p ulak ui start`

**Config:** `$ULAK_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `ulak` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Ulak Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/ErkanOzdemir-Labs/ulak-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
ulak memory setup    # select "holographic"
# Or manually:
ulak config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.ulak-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$ULAK_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
ulak memory setup    # select "retaindb"
# Or manually:
ulak config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.ulak/.env
```
...
# Then configure Ulak
ulak memory setup    # select "byterover"
# Or manually:
ulak config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$ULAK_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [cloud API key](http://app.supermemory.ai/integrations?connect=ulak), or a [self-hosted server](https://supermemory.ai/docs/self-hosting/overview) |
...
**Setup:**
```bash
ulak memory setup    # select "supermemory"
# Or manually:
ulak config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.ulak/.env
```
...
Before running `ulak memory setup`, set `base_url` in
`$ULAK_HOME/supermemory.json`:
...
Then run `ulak memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$ULAK_HOME/supermemory.json`
...
| `container_tag` | `ulak` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `ulak-{identity}` → `ulak-coder`) to isolate memories per Ulak profile
...
| **Requires** | `pip install ulak-memori` + `ulak-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install ulak-memori
ulak-memori install
ulak config set memory.provider memori
ulak memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `ulak-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$ULAK_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$ULAK_HOME/` so each profile has its own credentials
