...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Ulak's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Ulak instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
ulak memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `ulak` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Ulak syncs them back at session init.
...
The `ulak honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `ulak memory setup honcho` (or run `ulak memory setup` and pick it from the list); the `ulak honcho` subcommand then appears on the next invocation.

```bash
ulak memory setup honcho    # Configure Honcho directly (works before activation)
ulak honcho status          # Connection status, config, and key settings
ulak honcho setup           # Redirects to `ulak memory setup` (post-activation alias)
ulak honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
ulak honcho peer            # Show or update peer names + dialectic reasoning level
ulak honcho mode            # Show or set recall mode (hybrid/context/tools)
ulak honcho tokens          # Show or set token budget for context and dialectic
ulak honcho identity        # Seed or show the AI peer's Honcho identity
ulak honcho sync            # Sync Honcho config to all existing profiles
ulak honcho peers           # Show peer identities across all profiles
ulak honcho sessions        # List known Honcho session mappings
ulak honcho map             # Map current directory to a Honcho session name
ulak honcho enable          # Enable Honcho for the active profile
ulak honcho disable         # Disable Honcho for the active profile
ulak honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `ulak honcho`

If you previously used the standalone `ulak honcho setup`:
...
No re-login or re-setup needed. Run `ulak memory setup` and select "honcho" — the wizard detects your existing config.
