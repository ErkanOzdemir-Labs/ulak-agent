...
Ulak can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<ULAK_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Ulak renders the sprite at full fidelity when your
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `ulak pets list [query] [--limit N]` |
| List installed pets | `ulak pets list --installed` |
| Install a pet | `ulak pets install <slug> [--select] [--force]` |
| Set the active pet | `ulak pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `ulak pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `ulak pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `ulak pets off` |
| Remove an installed pet | `ulak pets remove <slug>` |
| Diagnose setup | `ulak pets doctor` |

`ulak pets show` flags:
...
Beyond installing pre-made pets from the gallery, Ulak can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `ulak pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `ulak tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `ULAK_PET_IMAGE_PROVIDER` env var (e.g. `ULAK_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Ulak is
...
Run `ulak pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Ulak uses its own
  profile-scoped `<ULAK_HOME>/pets/` instead — install through `ulak pets`.

## See also

- The [`ulak-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-ulak-agent.md)
