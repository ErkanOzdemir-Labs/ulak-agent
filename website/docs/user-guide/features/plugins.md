...
title: "Plugins"
description: "Extend Ulak with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Ulak has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/developer-guide/adding-tools) page is for built-in Ulak
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Ulak Plugin](/developer-guide/plugins)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.ulak/plugins/` with a `plugin.yaml` and Python code:

```
~/.ulak/plugins/my-plugin/
...
Start Ulak — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.ulak/plugins/hello-world/plugin.yaml`**
...
**`~/.ulak/plugins/hello-world/__init__.py`**

```python
"""Minimal Ulak plugin — registers a tool and a hook."""
...
Drop both files into `~/.ulak/plugins/hello-world/`, restart Ulak, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

The model-facing tool description belongs in `schema["description"]`. The optional `ctx.register_tool(description=...)` value is separate `ToolEntry` registry metadata: when omitted, it defaults to the schema description, but Ulak does not copy it back into a schema that lacks `description`. Prefer defining the text once in the schema. If you provide both values, keep them synchronized; the model sees the schema value.

Project-local plugins under `./.ulak/plugins/` are disabled by default. Enable them only for trusted repositories by setting `ULAK_ENABLE_PROJECT_PLUGINS=true` before starting Ulak.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `ulak <plugin> <subcommand>` |
...
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `ulak plugins install` |
| Distribute via pip | `[project.entry-points."ulak_agent.plugins"]` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Ulak — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
| User | `~/.ulak/plugins/` | Personal plugins |
| Project | `.ulak/plugins/` | Project-specific plugins (requires `ULAK_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `ulak_agent.plugins` entry_points | Distributed packages |
| Nix | `services.ulak-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, Ulak also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.ulak/plugins/model-providers/<name>/` and `~/.ulak/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `ulak plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.ulak/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
ulak plugins                    # interactive toggle (space to check/uncheck)
ulak plugins enable <name>      # add to allow-list
ulak plugins disable <name>     # remove from allow-list + add to disabled
```

After `ulak plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

For a reproducible install, pin a full immutable commit (tags, branches, and
abbreviated SHAs are not accepted):

```bash
ulak plugins install owner/repo --ref 0123456789abcdef0123456789abcdef01234567
```

Ulak checks out the commit detached, verifies that `HEAD` exactly matches the
requested SHA, and records the canonical source, installed revision, and pin
status in the current profile. `ulak plugins update` refuses to move a pinned
plugin; choose a new exact commit explicitly with
`ulak plugins install <source> --force --ref <new-commit>`. The
...
Several categories of plugin bypass `plugins.enabled` — they're part of Ulak' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.ulak/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.ulak/plugins/`.

### Approval transports

An approval transport changes **where a human sees and answers** an existing
Ulak tool-approval request. It does not decide whether a command needs
...
`present` may be synchronous or async. Ulak runs it on a bounded worker and
...
`transport_fallback: builtin`. Without that exact opt-in, Ulak never
materializes the prompt on another surface.

Ulak still owns hardline blocks, sudo-stdin protection, user deny rules,
...
When you upgrade to a version of Ulak that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.ulak/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)

## Available hooks

Plugins can register the 26 lifecycle events currently accepted by `ulak_cli.plugins.VALID_HOOKS`. The **[Event Hooks catalog](/user-guide/features/hooks#shipped-plugin-hook-catalog)** is canonical for exact timing, return handling, payload fields, and privacy notes.
...
Ulak has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.ulak/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Ulak also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Ulak Plugin](/developer-guide/plugins) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Ulak Plugin](/developer-guide/plugins) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Ulak Plugin](/developer-guide/plugins) · [Extending the CLI](/developer-guide/extending-the-cli) |
| A **subcommand** for `ulak <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| An **STT backend** (any CLI — whisper.cpp, custom whisper binary, local ASR CLI) | Config-driven (recommended) — declare under `stt.providers.<name>` with `type: command` in `config.yaml`, or set `ULAK_LOCAL_STT_COMMAND` for the legacy single-command escape hatch. OR Python backend plugin — `ctx.register_transcription_provider()` for Python-SDK engines (OpenRouter, SenseAudio, Gemini-STT, etc.). | [STT Setup](/user-guide/features/tts#stt-custom-command-providers) · [Python plugin guide](/user-guide/features/tts#python-plugin-providers-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Ulak auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `ulak skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.ulak/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `ulak plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
```bash
ulak plugins                               # unified interactive UI
ulak plugins list                          # table: enabled / disabled / not enabled
ulak plugins search <term>                 # search the community plugin index
ulak plugins install <name>                # install by index name (resolved to repo @ pinned ref)
ulak plugins install user/repo             # install from Git, then prompt Enable? [y/N]
ulak plugins install user/repo --enable    # install AND enable (no prompt)
ulak plugins install user/repo --no-enable # install but leave disabled (no prompt)
ulak plugins update my-plugin              # pull latest (local edits are autostashed and re-applied)
ulak plugins remove my-plugin              # uninstall
ulak plugins enable my-plugin              # add to allow-list
ulak plugins disable my-plugin             # remove from allow-list + add to disabled
ulak plugins capabilities [my-plugin]      # declared vs granted capabilities
```

### One-click install links (Desktop)

Ulak Desktop registers the `ulak://` URL scheme, so a website, README, or
chat message can link straight to a plugin install:

```
ulak://plugin/install?repo=owner/repo            # main install link
ulak://plugin/install?repo=owner/repo&enable=1   # enable the agent plugin after install
ulak://plugin/install?repo=owner/repo&force=1    # replace an existing install
```

Clicking one opens Ulak and shows a **confirmation dialog** — the repo id,
...
Install from Git**. Legacy `ulak://plugin-agent/…` and
`ulak://plugin-desktop/…` URLs route into the same dialog. In dev builds
(`npm run dev`) the scheme is `ulak-dev://`.

Websites need no SDK — a normal anchor works:

```html
<a href="ulak://plugin/install?repo=owner/repo&enable=1">Install in Ulak</a>
```

MCP servers have the equivalent link form — see
[Add to Ulak link](/reference/mcp-config-reference#add-to-ulak-link).
...
When a plugin declares capabilities, `ulak plugins install` (and
`ulak plugins enable`) shows the list with one-line risk descriptions and
...
granted, `ulak plugins update` surfaces the additions and asks again. New
...
`ulak plugins enable <id>` interactively to grant them later.

Inspect the state at any time:

```bash
ulak plugins capabilities             # all plugins with declared/granted capabilities
ulak plugins capabilities my-plugin   # one plugin, declared vs granted
```
...
not a code audit, and Ulak has not reviewed the plugin's code. Only install
...
`ulak plugins search <term>` searches the **community plugin index** — a
static, machine-readable JSON catalog of community plugins. Matching is fuzzy
across name, description, and tags:

```bash
ulak plugins search telegram               # fuzzy search
ulak plugins search                        # browse the whole index
ulak plugins search --capability platform  # filter by declared capability
ulak plugins search media --json           # machine-readable output
ulak plugins search --refresh              # bypass the 24h local cache
```
...
```bash
ulak plugins install ulak-media-studio
```
...
(`https://raw.githubusercontent.com/ErkanOzdemir-Labs/ulak-plugin-index/main/index.json`,
overridable via `ulak config set plugins.index_url <url>`). Fetches are
cached under `~/.ulak/cache/plugin_index.json` for 24 hours; when the
remote is unreachable the stale cache is used, and when there is no cache at
all a bundled seed copy ships with Ulak — so search works fully offline.

**Index entry format.** Each entry is a JSON object:

```json
{
  "name": "ulak-media-studio",
  "description": "Generative media workspace plugin.",
  "author": "ErkanOzdemir-Labs",
  "tags": ["media", "image-gen"],
  "repo": "ErkanOzdemir-Labs/ulak-media-studio",
  "ref": "<40-char commit SHA>",
  "subdir": null,
  "homepage": "https://github.com/ErkanOzdemir-Labs/ulak-media-studio",
...
(`ulak_cli/data/plugin_index.json` in the repo) is the format reference.

**Submitting a plugin.** The index is maintained as a plain JSON file —
submit a pull request to the
[ulak-plugin-index](https://github.com/ErkanOzdemir-Labs/ulak-plugin-index)
...
A **plugin pack** is a declarative, shareable YAML file (`ulak-pack.yaml`)
...
version: 1.0.0
plugins:
  - name: ulak-media-studio            # bare community-index name…
...
```bash
ulak plugins pack show ./ulak-pack.yaml     # dry-run review
ulak plugins pack install ./ulak-pack.yaml  # review → confirm → install
ulak plugins pack export > ulak-pack.yaml   # snapshot the current install
ulak plugins pack export --enabled-only       # only plugins.enabled
```
...
same pinned install path as `ulak plugins install --ref <sha>` and record
...
capability-consent prompt — identical to a single `ulak plugins install`.
...
provenance (installed via `ulak plugins install`). Local-only plugins are
...
auto-installed — install those manually for now (`ulak skills`). Wiring
skill-hub ids into pack install is a documented follow-up seam.

### Install-time security scanning

Every `ulak plugins install` and `ulak plugins update` runs a static
...
On `ulak plugins update`, a dangerous verdict on the updated tree
...
Running `ulak plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `ulak plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
- Ulak reuses that session's stored platform, chat, thread, profile, and conversation history. Plugins cannot supply a new chat route through this API.
- Ulak rechecks the stored route against the gateway's current authorisation rules before dispatch.
- Routes that relied only on an adapter-time or upstream authorisation decision are rejected unless Ulak can revalidate them from current core allowlists, pairing, or explicit allow-all configuration.
...
- The route and conversation are pinned while dispatch is pending. Ulak drops the request if topic recovery changes the route or the session rotates before handling starts.
...
:::warning
Only grant gateway injection to plugins you trust. Ulak checks this host API permission and restricts it to existing session routes, but Python plugins run in-process and this setting is not a sandbox.
:::
...
`ctx.call_mcp()` lets a plugin call a tool on one of the user's configured MCP servers — synchronously, from any hook or tool handler — routing through Ulak' existing native MCP client (same connections, trust-tier gates, circuit breaker, and reconnect logic as model-invoked MCP tools; never a parallel client).
