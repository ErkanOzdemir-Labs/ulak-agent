...
When Ulak Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.ulak/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.ulak/config.yaml` (switchable per session
via `/personality`).
...
If you use Ulak' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.ulak/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Ulak Agent's deliverable
