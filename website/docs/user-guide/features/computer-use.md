...
Ulak Agent can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Ulak integration. It
...
**Fresh installs already have the driver.** The Ulak installer
...
- **`ulak tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
ulak computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `ulak computer-use
status` to verify the install.

Already have cua-driver? Ulak reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `ulak update`, and the first
`computer_use` call of a session, Ulak checks the local version and
...
selected with `ULAK_CUA_DRIVER_CMD` stays
under your control, so Ulak reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Ulak autodetection is a
planned cua-driver follow-up, so currently point Ulak at that directory or
...
interface. The built-in toolset provides Ulak actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `ulak computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Ulak host identity. |
...
```
ulak -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.ulak/config.yaml`.

## Permission modes and logged-in browser profiles

Ulak maps its existing approval UX onto cua-driver's immutable runtime
modes. Permission mode and capability manifest approval are launch settings.
They cannot change after the runtime starts:

| Ulak session | cua-driver mode | Human intervention |
|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Ulak approvals; Cua stops at its protected boundary |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Ulak risk acceptance; no runtime Cua prompts |
...
computer_use:
  ...
  capability_manifest: ~/.ulak/cua-manifest.yaml
```
...
for the format). Ulak launches a private runtime with
...
identity instead of resetting with every Ulak build), and Ulak verifies
...
resetting or closing the Ulak session, cancellation cleanup, or process exit
closes that transport session. Ulak also stops private runtimes that it
launched for bounded or unrestricted access. One Ulak
conversation cannot change another runtime's mode or grants. Bounded and
unrestricted modes use a private
embedded service under the Ulak host identity.
...
## `ulak computer-use doctor` — your first triage stop

`ulak computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ ulak computer-use doctor
...
Ulak run declares a public cua-driver **session name** (something like
`ulak-3a7b9c14d2e8`). The name labels cursor identity and related state, so
...
without it. Ulak disables it automatically where it is a known failure
...
Ulak keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Ulak-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. Ulak
autodetection is a planned cua-driver follow-up, so currently point Ulak at
...
These are **platform deep dives, not duplicates of the Ulak skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Ulak
...
image capture also saves a bounded, shareable copy under Ulak' image cache
...
Ulak applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.ulak/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Ulak applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Ulak agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Ulak agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Ulak warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Ulak from inside the RDP / console
...
On Linux, native Wayland support remains an explicit opt-in. Ulak passes the
...
```
ULAK_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
ULAK_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Ulak disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Ulak sets
...
When it's on, `ulak computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Ulak at a binary you built from source instead
of the published release. Ulak resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`ULAK_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Ulak testing (see notes).
...
### Option B — point Ulak straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`ULAK_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
ULAK_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
ULAK_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Ulak is using your build

- `ulak computer-use status` prints the resolved binary path and
  version.
- `ulak computer-use doctor` confirms the binary is reachable and
...
- **Ulak spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Ulak YOLO, Ulak instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Ulak
  change needed either way.
- **Local builds skip the version check.** Ulak warns when the
...
**First action when anything's off: run `ulak computer-use doctor`.**
...
Run `ulak computer-use install` to fetch the cua-driver binary, or
run `ulak tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `ulak computer-use doctor` will
...
- **Ulak-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Ulak `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Ulak autodetection is a
  planned follow-up; currently point Ulak at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Ulak among them)
