...
- **Plugin / integration authors** wanting to add a new lane shape (a CLI worker that wraps Codex / Claude Code / OpenCode, a containerised review worker, a non-Ulak service that pulls tasks via the API).

If you're writing the worker code itself — the agent that runs *inside* a lane — the kanban lifecycle and reference details are injected into the worker's system prompt automatically (the `KANBAN_GUIDANCE` block in [`agent/prompt_builder.py`](https://github.com/ErkanOzdemir-Labs/ulak-agent/blob/main/agent/prompt_builder.py)).

## The hierarchy

```text
Ulak Kanban  =  canonical task lifecycle + audit trail
...
Ulak Kanban owns lifecycle truth — `ready` → `running` → `review` / `blocked` / `done` / `archived`. Worker lanes execute work but never own that truth; everything they do flows back through the kanban kernel via the `kanban_*` tools (or, for non-Ulak external workers, via the API). Reviewers gate the transition from "code change written" to "task done."
...
The dispatcher matches `task.assignee` against either a Ulak profile name (the default lane shape) or a registered non-spawnable identifier (the plugin lane shape — see [Adding an external CLI worker lane](#adding-an-external-cli-worker-lane) below). Tasks whose assignee doesn't resolve are left on `ready` with a `skipped_nonspawnable` event so a board operator can fix them; they are not silently dropped or executed by an arbitrary fallback.

### 2. A spawn mechanism

For Ulak profile lanes, the dispatcher's `_default_spawn` runs `ulak -p <assignee> chat -q <prompt>` (or the equivalent module form when the `ulak` shim isn't on `$PATH`) inside the task's pinned workspace, with these env vars set:

| Variable | Carries |
|---|---|
| `ULAK_KANBAN_TASK` | the task id the worker is operating on |
| `ULAK_KANBAN_DB` | absolute path to the per-board SQLite file |
| `ULAK_KANBAN_BOARD` | board slug |
| `ULAK_KANBAN_WORKSPACES_ROOT` | root of the board's workspace tree |
| `ULAK_KANBAN_WORKSPACE` | absolute path to *this* task's workspace |
| `ULAK_KANBAN_RUN_ID` | the current run's id (for the lifecycle gate) |
| `ULAK_KANBAN_CLAIM_LOCK` | the claim lock string (`<host>:<pid>:<uuid>`) |
| `ULAK_PROFILE` | the worker's own profile name (for `kanban_comment` author attribution) |
| `ULAK_TENANT` | tenant namespace, if the task has one |

For non-Ulak lanes (registered via a plugin), the plugin supplies its own `spawn_fn` callable that gets `task`, `workspace`, and `board` and returns an optional pid for crash detection.
...
The historical issue for this is [#19931](https://github.com/ErkanOzdemir-Labs/ulak-agent/issues/19931) and the closed-not-merged Codex-specific PR [#19924](https://github.com/ErkanOzdemir-Labs/ulak-agent/pull/19924) — those describe the original architecture proposal but didn't land a runner.
...
- [`KANBAN_GUIDANCE`](https://github.com/ErkanOzdemir-Labs/ulak-agent/blob/main/agent/prompt_builder.py) — the worker + orchestrator lifecycle injected into every kanban worker's system prompt.
