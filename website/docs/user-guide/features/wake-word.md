...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey Ulak' wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey Ulak")

The wake word turns Ulak into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, Ulak listens in the background for a spoken
trigger phrase. Say it, and Ulak starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** Ulak backend (for example a
...
Ulak supports **client capture** for that case:
...
|--------|------|---------|-------|
| **openWakeWord** (default) | Free | None | Local ONNX models. Ships a bundled **"hey ulak"** model (default); also supports `hey_jarvis`, `alexa`, `hey_mycroft`, … and custom models |
...
By default the phrase is **"hey ulak"** — a model for it ships with Ulak, so
...
```bash
cd ~/.ulak/ulak-agent && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `ulak` session:
...
desktop ear button — also writes `wake_word.enabled` to `~/.ulak/config.yaml`,
...
wake_word:
  ...
  phrase: "hey ulak"        # cosmetic label only — detection is keyed by the model/keyword below
  ...
  openwakeword:
    model: hey_ulak         # bundled default; OR a built-in name OR a path to a custom .onnx/.tflite
...
- **`confirmation_frames`** (default `3`, openWakeWord only) — how many
  *consecutive* over-threshold frames are required before the wake fires. A real
  "hey ulak" holds a high score across several frames; an ambient blip spikes
...
- **`sensitivity`** (default `0.6`) — the detection threshold, `0.0`–`1.0`.
  ...
  **all** engines — for openWakeWord it's the raw per-frame score threshold, for
  ...
  if real "hey ulak" utterances are missed.
...
default) to let Ulak choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which Ulak installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three Ulak surfaces, and `surface` picks which one
...
"Hey Ulak" works out of the box — the bundled openWakeWord model
(`model: hey_ulak`) is the default. To wake on something else, the easiest
...
command (`ulak -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.ulak/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
syllables with an uncommon word ("ulak" qualifies) beat common words like
"hello" or "stop".
:::

### Option C — Porcupine (custom keyword in seconds)

Create a "Hey Ulak" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
wake_word:
  ...
  phrase: "hey ulak"
  porcupine:
    keyword: ~/.ulak/wakewords/hey_ulak.ppn
```

Set your access key in `~/.ulak/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `ulak tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `ulak-agent[wake]`).
...
but the phrase never fires. Ulak detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the Ulak
backend (it may appear as your terminal, `python`, or Ulak), then toggle the
...
```bash
ulak config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
ulak config set wake_word.input_device null
```
