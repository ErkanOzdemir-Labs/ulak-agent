...
Type `@` followed by a reference to inject content directly into your message. Ulak expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Ulak env: `$ULAK_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$ULAK_HOME/skills/.hub/`
