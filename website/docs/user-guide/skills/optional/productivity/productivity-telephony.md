...
| | |
|---|---|
| Source | Optional — install with `ulak skills install official/productivity/telephony` |
...
This optional skill gives Ulak practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${ULAK_HOME:-~/.ulak}/.env`
...
It does **not** turn Ulak into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Ulak memory
...
### 1) "I want Ulak to own a real phone number"
Use **Twilio**.
...
- pairs well with Ulak `text_to_speech` plus a public file host or tunnel
...
```bash
SCRIPT="$(find ~/.ulak/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
ulak skills search telephony
ulak skills install official/productivity/telephony
```
...
Then save credentials into Ulak:
...
3. Buy it and save it into `${ULAK_HOME:-~/.ulak}/.env` + state:
```bash
python "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Ulak calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Ulak's existing `text_to_speech` support.

Use this when:
- you want the call to use Ulak's configured TTS voice rather than Twilio `<Say>`
...
Recommended Ulak TTS -> Twilio Play workflow:

1. Generate the audio with Ulak `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Ulak to create the message audio with `text_to_speech`
...
Important note:
- Ulak TTS is great for prerecorded outbound messages
...
- Ulak STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Ulak memory.
...
- Do not store arbitrary third-party phone numbers in Ulak memory.
...
3. persist that number to `${ULAK_HOME:-~/.ulak}/.env`
