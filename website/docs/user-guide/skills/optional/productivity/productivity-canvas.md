...
| | |
|---|---|
| Source | Optional — install with `ulak skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Ulak Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${ULAK_HOME:-~/.ulak}/.env`:
...
```bash
CANVAS="python $ULAK_HOME/skills/productivity/canvas/scripts/canvas_api.py"
