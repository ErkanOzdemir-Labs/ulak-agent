...
| | |
|---|---|
| Source | Optional — install with `ulak skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${ULAK_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ULAK_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ULAK_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client ulak
```
...
```bash
PUBLISH="${ULAK_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client ulak
```
...
```bash
DRIVE="${ULAK_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `ulak`)    |
