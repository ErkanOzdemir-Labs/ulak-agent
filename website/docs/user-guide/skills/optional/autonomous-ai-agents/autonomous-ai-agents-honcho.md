---
title: "Honcho — Configure and troubleshoot Honcho memory for Ulak"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Ulak"
---
...
Configure and troubleshoot Honcho memory for Ulak.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `ulak skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Ulak

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Ulak profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/ulak#running-honcho-locally-with-ulak

### Verify

```bash
ulak honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Ulak profile's AI peer
...
Honcho models conversations as interactions between **peers**. Ulak creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Ulak instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Ulak run |
| `global` | Single session across all directories |

Manual override: `ulak honcho map my-project-name`
...
Each Ulak profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block ulak.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `ulak.coder` host block in `honcho.json`
...
```bash
ulak honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Ulak when Honcho memory is active.
...
Config file: `$ULAK_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `ulak honcho setup`. Ensure `memory.provider: honcho` is in `~/.ulak/config.yaml`.

### Memory not persisting across sessions
Check `ulak honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `ulak profile create <name> --clone`. For existing profiles: `ulak honcho sync`.
...
| Command | Description |
|---------|-------------|
| `ulak honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `ulak honcho status` | Show resolved config, connection test, peer info for active profile |
| `ulak honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `ulak honcho disable` | Disable Honcho for the active profile |
| `ulak honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `ulak honcho peers` | Show peer identities across all profiles |
| `ulak honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `ulak honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `ulak honcho sessions` | List known directory-to-session-name mappings |
| `ulak honcho map <name>` | Map current working directory to a Honcho session name |
| `ulak honcho identity` | Seed AI peer identity or show both peer representations |
| `ulak honcho sync` | Create host blocks for all Ulak profiles that don't have one yet |
| `ulak honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Ulak + Honcho |
| `ulak memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `ulak memory status` | Show active memory provider and config |
| `ulak memory off` | Disable external memory provider |
