...
| | |
|---|---|
| Source | Optional — install with `ulak skills install official/security/1password` |
| Path | `optional-skills/security\1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Ulak Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Ulak terminal calls (desktop app flow only)
...
### Service Account (recommended for Ulak)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${ULAK_HOME:-~/.ulak}/.env` (the skill will prompt for this on first load).
...
## Ulak Execution Pattern (desktop app flow)

Ulak terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/ulak-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/ulak-op.sock"
