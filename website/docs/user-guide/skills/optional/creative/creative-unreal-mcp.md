...
| | |
|---|---|
| Source | Optional — install with `ulak skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Ulak MCP catalog. The
...
Two halves, in this order: the editor side must be up before Ulak connects.
...
### One-time, Ulak side
    ...
    ulak mcp install unreal-engine
...
`~/.ulak/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Ulak — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Ulak connects
...
2. Start the Ulak session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Ulak
they appear as:
...
then happens at `ulak mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Ulak runs batched calls
...
- **Start order matters.** Editor + server up first, then the Ulak
...
- **Timeouts on long operations.** Ulak' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.ulak/config.yaml` for
