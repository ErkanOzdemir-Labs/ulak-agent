...
| | |
|---|---|
| Source | Optional — install with `ulak skills install official/creative/pixel-art` |
...
import sys
import os
sys.path.insert(0, os.path.expanduser("~/.ulak/skills/creative/pixel-art/scripts"))
...
```bash
cd ~/.ulak/skills/creative/pixel-art/scripts
...
- ffmpeg on PATH (only needed for video — Ulak installs package this)
