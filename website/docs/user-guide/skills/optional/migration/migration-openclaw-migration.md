---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Ulak"
...
Import an OpenClaw setup (memories, skills) into Ulak.
...
# OpenClaw -> Ulak Migration

Use this skill when a user wants to move their OpenClaw setup into Ulak Agent with minimal manual cleanup.
...
```bash
ulak claw migrate              # Full interactive migration
ulak claw migrate --dry-run    # Preview what would be migrated
ulak claw migrate --preset user-data   # Migrate without secrets
ulak claw migrate --overwrite  # Overwrite existing conflicts
ulak claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `ulak setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_ulak.py` to:

- import `SOUL.md` into the Ulak home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Ulak memory entries
- merge OpenClaw command approval patterns into Ulak `command_allowlist`
- migrate Ulak-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Ulak working-directory configuration
- copy OpenClaw skills into `~/.ulak/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Ulak workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.ulak/tts/`
- archive non-secret docs that do not have a direct Ulak destination
...
- `scripts/openclaw_to_ulak.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.ulak/skills/migration/openclaw-migration/scripts/openclaw_to_ulak.py`

Do not guess a shorter path like `~/.ulak/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.ulak/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Ulak-compatible secrets, currently:
...
Ulak CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Ulak. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Ulak workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Ulak skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.ulak/skills/migration/openclaw-migration/scripts/openclaw_to_ulak.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.ulak/skills/migration/openclaw-migration/scripts/openclaw_to_ulak.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.ulak/skills/migration/openclaw-migration/scripts/openclaw_to_ulak.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.ulak/skills/migration/openclaw-migration/scripts/openclaw_to_ulak.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.ulak/skills/migration/openclaw-migration/scripts/openclaw_to_ulak.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.ulak/skills/migration/openclaw-migration/scripts/openclaw_to_ulak.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Ulak unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Ulak targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Ulak destination. Unsupported auth blobs must still be reported as skipped.
...
- Ulak persona state imported
- Ulak memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.ulak/skills/openclaw-imports/`
