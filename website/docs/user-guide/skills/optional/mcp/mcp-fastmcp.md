...
| | |
|---|---|
| Source | Optional — install with `ulak skills install official/mcp/fastmcp` |
...
| Related skills | [`ulak-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-ulak-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Ulak or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Ulak. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.ulak/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.ulak/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Ulak integration, either:

- configure the server in `~/.ulak/config.yaml` using the `native-mcp` skill, or
...
### Ulak cannot see the deployed server

The server-building part may be correct while the Ulak config is not. Load the `native-mcp` skill and configure the server in `~/.ulak/config.yaml`, then restart Ulak.
