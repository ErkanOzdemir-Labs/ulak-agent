...
| | |
|---|---|
| Source | Optional — install with `ulak skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain\hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), Ulak Agent |
| License | MIT |
...
The script reads `${ULAK_HOME:-~/.ulak}/.env` for two optional defaults:
...
Helper script: `~/.ulak/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
```bash
python ~/.ulak/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py <command> [args]
```
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${ULAK_HOME:-~/.ulak}/.env`.
...
```bash
python ~/.ulak/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py dexs

python ~/.ulak/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py \
