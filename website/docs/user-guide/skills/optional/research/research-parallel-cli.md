...
| | |
|---|---|
| Source | Optional — install with `ulak skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a Ulak core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Ulak native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Ulak native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Ulak native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended Ulak usage patterns
...
- Do not silently choose Parallel when Ulak native tools are already sufficient.
