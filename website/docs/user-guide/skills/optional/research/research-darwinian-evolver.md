...
| | |
|---|---|
| Source | Optional — install with `ulak skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research\darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Ulak Agent |
| License | MIT |
...
import upstream classes into Ulak itself.
...
```bash
mkdir -p ~/.ulak/cache/darwinian-evolver && cd ~/.ulak/cache/darwinian-evolver
...
```bash
cd ~/.ulak/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.ulak/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.ulak/skills/research/darwinian-evolver
DE_DIR=~/.ulak/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Ulak core.
   Custom driver scripts under `~/.ulak/skills/...` are user-side and fine.
