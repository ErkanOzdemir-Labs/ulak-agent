...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`ulak-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-ulak-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Ulak terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Ulak itself, `model.provider: openai-codex` uses Ulak-managed Codex
OAuth from `~/.ulak/auth.json` after `ulak auth add openai-codex`. For the
...
When invoking the Codex CLI from a Ulak gateway/service context (for example,
