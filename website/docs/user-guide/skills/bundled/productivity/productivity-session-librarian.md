...
| List/filter by metadata (age, source, cost, tokens, workspace) | `ulak sessions list` / `stats` via terminal |
| Rename | `ulak sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `ulak sessions archive <filters>` |
| Delete (destructive) | `ulak sessions delete` / `ulak sessions prune <filters>` |
| Export before deleting anything valuable | `ulak sessions export --session-id <id> --format md` |
...
`ulak sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `ulak sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `ulak sessions list`
