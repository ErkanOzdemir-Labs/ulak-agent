...
description: "Which operating systems, distribution methods, and features Ulak Agent supports."
---

# Platform Support

Ulak Agent maintains support for many platforms and distribution methods, but we can't support every possible install method.
...
| ----------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **macOS** (Apple Silicon)                                                     | [Ulak Desktop](https://ulak-agent.github.com/ErkanOzdemir-Labs/), [`install.sh`](./installation.md#linux--macos--wsl2--android-termux) |
| [**Windows 10 / 11**](../user-guide/windows-native.md) (x86_64, aarch64)      | [Ulak Desktop](https://ulak-agent.github.com/ErkanOzdemir-Labs/), [`install.ps1`](./installation.md#windows-native)                    | A few features are [not available](../user-guide/windows-native.md#feature-matrix).                                                                       |
...
| [**Docker Container**](../user-guide/docker.md#quick-start) (x86_64, aarch64) | [`docker pull`](../user-guide/docker.md#quick-start)                                                                           | Docker installs do not support `ulak update`. Updating is done by running a new image.                                                                  |
...
- installs via `pypi` (e.g. `uv tool install ulak-agent`, `pip install ulak-agent`, etc.)
- installs via `brew` (`brew install ulak-agent`)
