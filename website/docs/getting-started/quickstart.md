---
sidebar_position: 1
title: "Ulak Agent Quickstart"
description: "Your first conversation with Ulak Agent — from install to chatting in under 5 minutes"
---

# Ulak Agent Quickstart

This guide gets you from zero to a working Ulak setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Ulak Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Ulak Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Ulak for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Ulak working on my machine | `ulak setup` | Run a real chat and verify it responds |
| I already know my provider | `ulak model` | Save the config, then start chatting |
| I want a bot or always-on setup | `ulak gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `ulak model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `ulak model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Ulak cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Ulak Agent
### With the Ulak Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the Ulak Desktop installer](https://ulak-agent.github.com/ErkanOzdemir-Labs/) from our website and run it.

### Without Ulak Desktop:
For a command-line only install without Ulak Desktop, run:
...
The single most important setup step. Use `ulak model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `ulak setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `ulak update`. Re-enable anything later with `ulak tools`, seed skills with `ulak skills opt-in --sync`, or tune settings with `ulak setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `ulak model` |
| **OpenAI Codex** | ChatGPT or Codex subscription, uses Codex models | Device code auth via `ulak model` → **ChatGPT or Codex Subscription** |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `ulak model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `ulak_cli/models.py` may change between releases) | `ulak model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `ulak model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `ulak model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `ulak model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `ulak model` (requires `copilot` CLI + `copilot login`) |
...
Ulak Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `ulak model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Ulak separates secrets from normal config:

- **Secrets and tokens** → `~/.ulak/.env`
- **Non-secret settings** → `~/.ulak/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
ulak config set model anthropic/claude-opus-4.6
...
```bash
ulak            # classic CLI
ulak --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Ulak ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `ulak` vs `ulak --tui`.
:::
...
```bash
ulak --continue    # Resume the most recent session
ulak -c            # Short form
```
...
```bash
ulak gateway setup    # Interactive platform configuration
```
...
- `ulak tools` — tune tool access per platform
- `ulak skills` — browse and install reusable workflows
...
```bash
ulak config set terminal.backend docker    # Docker isolation
ulak config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `ulak egress setup && ulak egress start`; `ulak setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the Ulak install directory (the curl installer placed it at
# ~/.ulak/ulak-agent on Linux/macOS or %LOCALAPPDATA%\ulak\ulak-agent on Windows):
cd ~/.ulak/ulak-agent
...
Skills are on-demand instruction documents that teach Ulak how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Ulak ships with a catalog of bundled skills already installed in `~/.ulak/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
ulak skills browse                      # list everything available
ulak skills search kubernetes           # find skills by keyword
ulak skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `ulak skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Ulak ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.ulak/ulak-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Ulak opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `ulak model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `ulak gateway setup` and check `ulak gateway status` |
| `ulak --continue` can't find old session | Switched profiles or session never saved | Check `ulak sessions list` and confirm you're in the right profile |
...
| `ulak doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `ulak model`
3. `ulak setup`
4. `ulak sessions list`
5. `ulak --continue`
6. `ulak gateway status`
...
|---------|-------------|
| `ulak` | Start chatting |
| `ulak model` | Choose your LLM provider and model |
| `ulak tools` | Configure which tools are enabled per platform |
| `ulak setup` | Full setup wizard (configures everything at once) |
| `ulak doctor` | Diagnose issues |
| `ulak update` | Update to latest version |
| `ulak gateway` | Start the messaging gateway |
| `ulak --continue` | Resume last session |
...
- **[Moving to another machine](/reference/faq#exporting-ulak-to-another-machine)** — `ulak backup` migrates your whole setup (or [a single profile](/reference/faq#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch
