...
> **Heads up:** Native Windows runs Ulak without WSL — CLI, gateway, TUI, and tools all work natively. If you'd rather use WSL2, the Linux/macOS one-liner above works there too. Found a bug? Please [file issues](https://github.com/ErkanOzdemir-Labs/ulak-agent/issues).
...
If your antivirus (Bitdefender, Windows Defender, etc.) quarantines `uv.exe` from the Ulak `bin` folder (`%LOCALAPPDATA%\ulak\bin\uv.exe`), this is a **false positive**. The file is Astral's `uv` — the Rust Python package manager Ulak bundles to manage its Python environment. ML-based antivirus engines commonly flag unsigned Rust binaries that download and install packages.
...
# Run verification
$uv = "$env:LOCALAPPDATA\ulak\bin\uv.exe"
...
**To whitelist Ulak:**
- **Windows Defender:** Run PowerShell as Admin → `Add-MpPreference -ExclusionPath "$env:LOCALAPPDATA\ulak\bin"`
- **Bitdefender:** Add an exception in the Bitdefender console (Protection > Antivirus > Settings > Manage Exceptions)
- Whitelist the **folder**, not the file hash — Ulak updates `uv` and the hash changes every version
...
```bash
ulak              # Interactive CLI — start a conversation
...
ulak config get   # Print individual config values
ulak gateway      # Start the messaging gateway (Telegram, Discord, etc.)
ulak setup        # Run the full setup wizard (configures everything at once)
ulak claw migrate # Migrate from OpenClaw (if coming from OpenClaw)
ulak update       # Update to the latest version
...
Ulak has two entry points: start the terminal UI with `ulak`, or run the gateway and talk to it from Telegram, Discord, Slack, WhatsApp, Signal, or Email. Once you're in a conversation, many slash commands are shared across both interfaces.

| Action                         | CLI                                           | Messaging platforms                                                              |
| ------------------------------ | --------------------------------------------- | -------------------------------------------------------------------------------- |
| Start chatting                 | `ulak`                                      | Run `ulak gateway setup` + `ulak gateway start`, then send the bot a message |
...
If you're coming from OpenClaw, Ulak can automatically import your settings, memories, skills, and API keys.

**During first-time setup:** The setup wizard (`ulak setup`) automatically detects `~/.openclaw` and offers to migrate before configuration begins.

**Anytime after install:**

```bash
ulak claw migrate              # Interactive migration (full preset)
...
- **Skills** — user-created skills → `~/.ulak/skills/openclaw-imports/`
...
See `ulak claw migrate --help` for all options, or use the `openclaw-migration` skill for an interactive agent-guided migration with dry-run previews.
...
`~/.ulak/ulak-agent`). This matches the layout used by `ulak update`, the
...
source ~/.ulak/venvs/ulak-dev/bin/activate
uv pip install -e ".[all,dev]"
scripts/run_tests.sh
```
...
- 🔌 [computer-use-linux](https://github.com/avifenesh/computer-use-linux) — Linux desktop-control MCP server for Ulak and other MCP hosts, with AT-SPI accessibility trees, Wayland/X11 input, screenshots, and compositor window targeting.
- 🔌 [UlakClaw](https://github.com/AaronWong1999/ulakclaw) — Community WeChat bridge: Run Ulak Agent and OpenClaw on the same WeChat account.
