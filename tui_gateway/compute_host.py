...
class ComputeHost:
    ...
    def __init__(
        ...
        self._heartbeat_secs = (
            ...
            else float(os.environ.get("ULAK_COMPUTE_HOST_HEARTBEAT_SECS") or "15"))
    ...
    def _run_real_turn(self, frame: dict[str, Any]) -> None:
        ...
        try:
            ...
            with contextlib.suppress(Exception):
                import ulak_undo
                ulak_undo.on_user_message_appended(session["session_key"])
            with contextlib.suppress(Exception):
    ...
    def _build_server_session(self, server: Any, frame: dict[str, Any], sid: str) -> dict:
        ...
        try:
            if profile_home:
                from ulak_constants import set_ulak_home_override
            ...
            if home_token is not None:
                with contextlib.suppress(Exception):
                    from ulak_constants import reset_ulak_home_override
...
def _default_workers() -> int:
    try:
        return max(2, int(os.environ.get("ULAK_TUI_RPC_POOL_WORKERS") or "8"))
    except (TypeError, ValueError):
        return 8


def run_host(stdin: Any = None, stdout: Any = None) -> None:
    os.environ["ULAK_COMPUTE_HOST_CHILD"] = "1"
    ...
    host.emit({
        ...
        "ulak_home": os.environ.get("ULAK_HOME", "")})
