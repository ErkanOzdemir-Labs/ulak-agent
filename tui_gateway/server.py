...
from ulak_constants import (
    get_ulak_home, get_ulak_home_override, reset_ulak_home_override, set_ulak_home_override)
from ulak_cli.env_loader import load_ulak_dotenv
from utils import is_truthy_value
from tools.environments.local import ulak_subprocess_env
...
load_ulak_dotenv(ulak_home=_ulak_home, project_env=Path(__file__).parent.parent / ".env")
...
_CRASH_LOG = os.path.join(_ulak_home, "logs", "tui_gateway_crash.log")
...
with contextlib.suppress(Exception):
    from ulak_cli.banner import prefetch_update_check
...
_SLASH_WORKER_TIMEOUT_S = max(5.0, env_float("ULAK_TUI_SLASH_TIMEOUT_S", 45.0))
...
def _resolve_ws_orphan_reap_grace() -> float:
    ...
    return _ws_orphan_setting("ULAK_TUI_WS_ORPHAN_REAP_GRACE_S", "ws_orphan_reap_grace_s", 20.0)
...
_WS_ORPHAN_ACTIVITY_STALE_S = _ws_orphan_setting("ULAK_TUI_WS_ORPHAN_ACTIVITY_STALE_S", "ws_orphan_activity_stale_s", 600.0)
...
_rpc_pool_workers = max(2, env_int("ULAK_TUI_RPC_POOL_WORKERS", 8))
...
_current_runtime_session_record: contextvars.ContextVar[dict | None] = contextvars.ContextVar(
    "ulak_gateway_runtime_session_record", default=None)
...
_current_rpc_method: contextvars.ContextVar[str] = contextvars.ContextVar("ulak_gateway_rpc_method", default="")
...
def _prepend_tool_paths(env: dict[str, str]) -> dict[str, str]:
    ...
    ~/.local/bin to PATH so slash_worker children resolve Ulak-managed CLIs under the Desktop's minimal PATH."""
    managed_bin = ""
    with contextlib.suppress(Exception):
        managed_bin = str(Path(get_ulak_home()) / "bin")
...
class _SlashWorker:
    """Persistent UlakCLI subprocess for slash commands."""
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the Ulak agent → needs provider credentials. Tier-1 secrets
        ...
        # launch ULAK_HOME (#40677). The override goes through the build_subprocess_env factory's `extra`
        # (applied last, always wins) instead of a hand-rolled env["ULAK_HOME"] assignment.
        ...
        env = _prepend_tool_paths(build_subprocess_env(
            ulak_subprocess_env(inherit_credentials=True), scrub_secrets=False,
            inherit_profile_home=False, extra={"ULAK_HOME": str(profile_home)} if profile_home else None))
        ...
        # Prepend the Ulak venv bin dir and the user-local bin dir to PATH so slash_worker child processes
        # can resolve Ulak-managed CLIs (browser-use, uvx) even when the parent gateway was launched with
...
_SESSION_TTL_S = max(0.0, env_float("ULAK_TUI_SESSION_TTL_S", float(6 * 3600)))
...
_EXIT_FLUSH_BUDGET_S = max(0.0, env_float("ULAK_TUI_EXIT_FLUSH_BUDGET_S", 5.0))
_INCREMENTAL_FLUSH_INTERVAL_S = max(0.0, env_float("ULAK_TUI_SESSION_FLUSH_INTERVAL_S", _REAPER_SCAN_S))
...
# backend, so calls carry ``profile`` → open that profile's db and bind its ULAK_HOME (ContextVar
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    try:
        ...
        return None
    if home.resolve() == Path(_ulak_home).resolve() or not home.exists():
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s ULAK_HOME around a handler (pets/projects resolve via
    ``get_ulak_home``, so app-global remote mode still hits the focused profile). No-op for launch.
...
def _bind_build_profile_scopes(profile_home: str) -> "_TurnScopes":
    """Bind a session profile's ULAK_HOME / secret / terminal scopes for an agent build. Fail-open per
    ...
    scopes = _TurnScopes()
    scopes.home = set_ulak_home_override(profile_home)
    with contextlib.suppress(Exception):
...
def _release_build_profile_scopes(scopes: "_TurnScopes") -> None:
    if scopes.home is not None:
        reset_ulak_home_override(scopes.home)
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    def _build() -> None:
        ...
        try:
            ...
            # Global-remote: bind the session profile's ULAK_HOME and hand the agent that profile's db —
...
def _active_config_path() -> Path:
    ...
    return Path(override if isinstance(override, str) and override else _ulak_home) / "config.yaml"
...
def _set_session_context(session_key: str, cwd: str | None = None, *, ui_session_id: str = "") -> list:
    with contextlib.suppress(Exception):
        ...
        # Live conversation id for subprocess ULAK_SESSION_ID: an explicitly empty contextvar is authoritative
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ.update(ULAK_GATEWAY_SESSION="1", ULAK_EXEC_ASK="1", ULAK_INTERACTIVE="1")
...
_TOUR_BRIDGE_UNAVAILABLE = json.dumps({
    "success": False,
    "error": ("No Ulak Desktop window answered the tour request. The tour is driven by the desktop app's "
              "renderer, which updates separately from this backend, so an app build older than the tour tool "
              "has nothing listening. Update the Ulak Desktop app and start a new session. Do not retry tour "
              "in this session.")})
...
def _env_model_seed() -> str:
    """The launch-scoped model seed (``ulak --tui -m``, hosted provisioning); "" when unset."""
    return (os.environ.get("ULAK_MODEL", "") or os.environ.get("ULAK_INFERENCE_MODEL", "")).strip()
...
def _resolve_model() -> str:
    ...
    with contextlib.suppress(Exception):
        from ulak_cli.models import get_preferred_silent_default_model
        return get_preferred_silent_default_model()
    return "z-ai/glm-5.2"


def _resolve_session_platform() -> str:
    """``ULAK_DESKTOP=1`` without ``ULAK_DESKTOP_TERMINAL`` → "desktop" (chat panel; the agent then
    suggests TUI-only slash commands), else "tui" (embedded terminal pane or standalone ``ulak --tui``)."""
    desktop = is_truthy_value(os.environ.get("ULAK_DESKTOP"))
    return "desktop" if desktop and not is_truthy_value(os.environ.get("ULAK_DESKTOP_TERMINAL")) else "tui"
...
def _config_model_target() -> tuple[str, str]:
    """(model, provider) selected by config.yaml — and ONLY config: the ULAK_MODEL launch seed fed into
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    if explicit_provider := os.environ.get("ULAK_TUI_PROVIDER", "").strip():
        return model, explicit_provider
    if not (explicit_model := _env_model_seed()):
        return model, None
    with contextlib.suppress(Exception):
        from ulak_cli.models import detect_static_provider_for_model
        cfg = _load_cfg().get("model") or {}
        current_provider = ((str(cfg.get("provider") or "").strip().lower() if isinstance(cfg, dict) else "")
                            or os.environ.get("ULAK_INFERENCE_PROVIDER", "").strip().lower() or "auto")
...
from ulak_state import _BARE_BILLING_PROVIDERS


def _is_routable_provider(provider: str) -> bool:
    with contextlib.suppress(Exception):
        from ulak_cli.runtime_provider import is_routable_provider
...
def _stored_session_runtime_overrides(row: dict | None) -> dict:
    ...
    if provider and not _is_routable_provider(provider):
        healed = None
        try:
            from ulak_cli.runtime_provider import canonical_custom_identity
...
def _persist_live_session_system_prompt(session: dict | None) -> None:
    ...
    if live is None or not hasattr(live[0], "_build_system_prompt") or not hasattr(live[2], "update_system_prompt"):
        return
    agent, session_key, db = live
    # Re-bind the session's profile ULAK_HOME (the build's finally reset it → root profile's SOUL.md/skills)
    ...
    home_token = set_ulak_home_override(profile_home) if profile_home else None
...
def _load_reasoning_config(model: str = "") -> dict | None:
    """Via the shared chokepoint :func:`ulak_constants.resolve_reasoning_config` (per-model override >
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("ULAK_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    """Toolsets that exist because of the CLIENT (both off ``_ULAK_CORE_TOOLS``; this is the one gate).
    ``platform`` is the SESSION's source, never a process env var: the desktop may drive a URL/cloud
    backend where ``ULAK_DESKTOP`` is unset (AGENTS.md surface rule)."""
...
def _resolve_explicit_toolsets(explicit: list[str], validate_toolset) -> list[str] | None | bool:
    """Resolve a ULAK_TUI_TOOLSETS pin: list, None for "all", False when nothing was valid."""
    ...
    if any(name in {"all", "*"} for name in built_in):
        if ignored := [name for name in explicit if name not in {"all", "*"}]:
            _tui_notice(f"[tui] ULAK_TUI_TOOLSETS=all enables every toolset; ignoring additional entries: {', '.join(ignored)}")
        return None
    if not unresolved:
        return built_in
    try:  # (enabled, disabled) MCP server names from raw config; both empty on any failure
        ...
        from ulak_cli.tools_config import _parse_enabled_flag
    ...
    if unknown:
        _tui_notice(f"[tui] ignoring unknown ULAK_TUI_TOOLSETS entries: {', '.join(unknown)}")
    if disabled:
        _tui_notice("[tui] ignoring disabled MCP servers in ULAK_TUI_TOOLSETS "
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    """The agent's toolsets for this session (None = all): an explicit ULAK_TUI_TOOLSETS pin; else the
    ...
    explicit = [item.strip() for item in os.environ.get("ULAK_TUI_TOOLSETS", "").split(",") if item.strip()]
    ...
    if explicit and validate_toolset is not None:
        ...
        if resolved is not False:
            return resolved
        fallback_notice = "[tui] no valid ULAK_TUI_TOOLSETS entries; using configured CLI toolsets"
    try:
        from ulak_cli.config import load_config
        ...
        if fallback_notice is not None:
            _tui_notice("[tui] no valid ULAK_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets")
        return None
...
def _get_usage(agent) -> dict:
    ...
    # Dev-only live credits-spent readout, gated on ULAK_DEV_CREDITS so the payload stays clean otherwise.
    if is_truthy_value(os.environ.get("ULAK_DEV_CREDITS")):
        with contextlib.suppress(Exception):
...
def _probe_config_health(cfg: dict) -> str:
    ...
    if isinstance(display_cfg, dict):
        ...
        if personality and personality not in {"default", "none", "neutral"}:
            with contextlib.suppress(Exception):
                from ulak_cli.personality import available_personalities
...
def _project_info_for_cwd(cwd: str) -> dict | None:
    ...
    if not str(cwd or "").strip():
        return None
    try:
        from ulak_cli import projects_db as pdb
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    if live_agent:
        ...
        with contextlib.suppress(Exception):
            from ulak_cli.banner import get_available_skills
    ...
    with contextlib.suppress(Exception):
        from ulak_cli.banner import get_update_result
        from ulak_cli.config import recommended_update_command
...
def _startup_system_prompt(cfg: dict, task_id: str) -> str:
    """Config ephemeral system prompt + ULAK_TUI_SKILLS preload block. Hard-fails only when EVERY requested
    skill is missing (cli.py parity): a typo'd name must not auto-block the Kanban task."""
    from ulak_cli.config import resolve_ephemeral_system_prompt_from_config
    ...
    if missing_skills:
        ...
        logger.warning("Unknown skill(s) requested, skipping: %s. Continuing with: %s. "
                       "List available skills with `ulak skills list`.", missing_display, ", ".join(loaded_skills))
    if skills_prompt:
...
def _make_agent(
    ...
    for _mod in ("ulak_cli.mcp_startup", "tui_gateway.entry"):
        with contextlib.suppress(Exception):
    ...
    ignore_rules = is_truthy_value(os.environ.get("ULAK_IGNORE_RULES"))
    agent = AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("ULAK_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("ULAK_TUI_PASS_SESSION_ID")),
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            # Profile-scoped ULAK_HOME (None = launch); SessionBranch copies the parent's (same state.db).
...
def _load_resume_transcript(db, stored_id: str) -> tuple[list, list, list]:
    ...
    from ulak_state import SessionResumeTooLargeError
    prefix_fits = True
...
def _pet_gen_root():
    ...
    root = get_ulak_home() / "cache" / "pet-gen"
...
try:
    _PET_REFERENCE_MAX_BYTES = max(1, int(os.environ.get("ULAK_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)))
except (TypeError, ValueError):
...
def _spawn_trees_root():
    root = get_ulak_home() / "spawn-trees"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
_CLI_EXEC_BLOCKED = {
    ("setup",): "`ulak setup` needs a full terminal — run it outside the TUI",
    ("gateway",): "`ulak gateway` is long-running — run it in another terminal",
    ("sessions", "browse"): "`ulak sessions browse` is interactive — use /resume here, or run browse in another terminal",
    ("config", "edit"): "`ulak config edit` needs $EDITOR in a real terminal",
}
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `ulak` is interactive — use `/ulak chat -q …` or run `ulak` in another terminal"
...
def _resolve_name(name: str) -> str:
    with contextlib.suppress(Exception):
        from ulak_cli.commands import resolve_command
