...
_SESSION_OWNERSHIP_UNAVAILABLE = "Ulak could not safely reserve this session. Try again."
...
def _other_runtime_lease_guard(session_id: str, session: dict):
    ...
    try:
        from ulak_cli.active_sessions import active_session_liveness_guard, release_active_session_liveness_guard
    except Exception as exc:
...
def _transfer_active_session_slot(sid: str, session: dict, *, new_session_id: str) -> bool:
    ...
    if lease is None:
        return True
    try:
        from ulak_cli.active_sessions import transfer_active_session
    ...
    for sid, session in owned:
        ...
        with _session_resume_lock, _sessions_lock:
            ...
            if current.get("close_on_disconnect"):
                ...
                # `ulak --tui` keeps real _stdio. UNLESS another window (pop-out viewer) still shows the session:
