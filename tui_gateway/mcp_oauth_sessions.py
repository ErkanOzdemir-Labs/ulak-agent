...
land. Reuses ``ulak mcp login``'s probe under ``force_interactive_oauth`` plus
...
def _start_loopback_listener(flow) -> "http.server.HTTPServer":
    ...
    class _Handler(http.server.BaseHTTPRequestHandler):
        def do_GET(self):  # noqa: N802 — stdlib naming
            ...
            body = b"<h1>Authorization received</h1><p>You can close this tab and return to Ulak.</p>"
            status = 200
            try:
...
def _probe_with_rollback(
    server_name: str, cfg: dict, ulak_home: str, flow, reconnect_live: bool) -> None:
    """Run the OAuth probe; on ANY failure restore the prior token file + manager entry."""
    from ulak_cli.mcp_config import _oauth_tokens_present, _probe_single_server, _save_mcp_server
    from tools.mcp_oauth import UlakTokenStorage
    from tools.mcp_oauth_manager import get_manager
    manager = get_manager()
    storage = UlakTokenStorage(server_name)
    backup = storage.snapshot()
    previous_entry = None
    try:
        previous_entry = manager.remove(server_name, ulak_home=ulak_home)
        ...
        manager.restore_entry(server_name, previous_entry, ulak_home=ulak_home)
        raise


def _worker(
        session_id: str, ulak_home: str, server_name: str, cfg: dict, reconnect_live: bool) -> None:
    ...
    try:
        ...
        home_token = set_ulak_home_override(ulak_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(ulak_home)))
        try:
            with force_interactive_oauth(), dashboard_oauth_flow(flow):
                _probe_with_rollback(server_name, cfg, ulak_home, flow, reconnect_live)
        finally:
...
def start_flow(
    ulak_home: str, server_name: str, cfg: dict, *, reconnect_live: bool = False,
    ...
    with _sessions_lock:
        ...
        if any(r["server_name"] == server_name and r["ulak_home"] == ulak_home for r in active):
            raise RuntimeError(f"MCP OAuth for '{server_name}' is already in progress")

    session_id = secrets.token_urlsafe(24)
    flow = DashboardOAuthFlow(
        flow_id=session_id, server_name=server_name, profile=None, ulak_home=ulak_home,
    ...
    rec = {
        "session_id": session_id, "server_name": server_name, "ulak_home": ulak_home,
        "flow": flow, "httpd": httpd, "created_at": time.time()}
    with _sessions_lock:
        _sessions[session_id] = rec
    threading.Thread(
        target=_worker, args=(session_id, ulak_home, server_name, dict(cfg), reconnect_live),
