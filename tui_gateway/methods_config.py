...
from ulak_constants import display_ulak_home as _display_ulak_home
...
def _cfg_get_provider(params):
    from ulak_cli.models import list_available_providers, normalize_provider
...
_CONFIG_GETTERS = {
    ...
    "profile": lambda params: {"home": str(_ulak_home), "display": _display_ulak_home()},
...
def _readiness_check(rid, params, probe):
    ...
    optional ``profile`` param's ULAK_HOME + ``.env`` secret scope (ContextVars: concurrent checks
    ...
    try:
        ...
        def probe(profile, scoped):
            ...
            if (not provider_configured and provider == "bedrock"
                ...
                return fail("No Ulak provider is configured.", source)
...
def _(rid, params: dict) -> dict:
    ...
    force-redaction pipeline as ``ulak debug share --nous``; redaction is NOT client-controllable
    ...
    try:
        from ulak_cli.debug import _redact_log_text, build_nous_bundle, collect_share_bundle
