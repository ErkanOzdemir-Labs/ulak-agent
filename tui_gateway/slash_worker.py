"""Persistent slash-command worker — one UlakCLI per TUI session.

Protocol: reads JSON lines from stdin {id, command}, writes {id, ok, output|error} to stdout.
"""

# Stop a ``utils/`` (or ``proxy/``, ``ui/``) package in the launch directory from shadowing Ulak's own
...
# retry loop. ``ulak_bootstrap`` lives at the repo root (no collision risk), so importing it first is safe.
# ``ulak_bootstrap`` lives at the repo root, so importing it is safe before the guard runs (its name won't
...
_WATCHDOG_POLL_S = max(0.05, env_float("ULAK_SLASH_WATCHDOG_POLL_S", 2.0))
_ORPHAN_GRACE_S = max(0.0, env_float("ULAK_SLASH_WATCHDOG_GRACE_S", 5.0))
...
def _prepare_slash_worker_runtime() -> None:
    """Start bounded MCP discovery before UlakCLI snapshots tools: each slash_worker child is its
    own process — the parent ``ulak serve`` discovery thread does not populate this registry.

    See #61891.
    """
    from ulak_cli.mcp_startup import start_background_mcp_discovery, wait_for_mcp_discovery
...
def _run(cli: UlakCLI, command: str) -> str:
...
def main():
    ...
    args = p.parse_args()
    os.environ["ULAK_SESSION_KEY"] = args.session_key
    os.environ["ULAK_INTERACTIVE"] = "1"
    # Start before the (hundreds-of-ms) UlakCLI build — that window is itself an orphan risk if the
    # gateway dies mid-spawn.
    _start_parent_death_watchdog(os.getppid())
    _prepare_slash_worker_runtime()
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = UlakCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
