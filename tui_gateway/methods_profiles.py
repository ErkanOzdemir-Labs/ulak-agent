...
only have the ws door), on the same `ulak_cli.profiles` primitives. Bodies are rebound onto
...
def _pin_profile_model(profile_dir, provider, model) -> None:
    _lazy("ulak_cli.web_routers.profiles", "_write_profile_model")(profile_dir, provider, model)
...
def _ulak_home_scope(path):
    """Scope config/auth resolution to ``path`` for the block."""
    token = set_ulak_home_override(str(path))
    try:
        yield
    finally:
...
def _resurrect_recoverable_canonical(db, profile_path, session_id):
    ...
    try:
        ...
        try:
            ...
            _best_effort(lambda: _lazy("ulak_state_registry", "release_or_close")(wdb))
    except Exception:
        return False
...
def _canonical_session_row(db, profile_path):
    ...
    and its click target describe the same session (ulak-agent#88200) with no client-side pointer
    involved.
    """
    try:
...
def _latest_profile_session_rows(db):
    ...
    (ulak-agent#90268). Workers heartbeat ``last_activity_at`` every ≤60s while running (#72016), so a
...
def _profile_session_fields(row, profile_path):
    ...
    if _try(db_path.exists, False):
        db = _try(lambda: _lazy("ulak_state", "SessionDB")(db_path=db_path, read_only=True), None)
    try:
...
def _(rid, params: dict) -> dict:
    """List Ulak profiles. ``include_sessions`` (default true) adds ``last_session`` /
...
def _mirror_voice_sections(path) -> bool:
    ...
    try:
        from ulak_cli.config import load_config_readonly, read_user_config_raw, save_config
        src_cfg = load_config_readonly() or {}
        sections = {k: src_cfg[k] for k in ("stt", "tts", "voice") if src_cfg.get(k)}
        if not sections:
            return False
        with _ulak_home_scope(path):
...
def _inherit_launch_model(path) -> bool:
    ...
    from ulak_cli.config import load_config_readonly, read_user_config_raw
    with _ulak_home_scope(path):
...
def _mirror_launch_credentials(path, params: dict) -> dict:
    ...
    if not share_auth:  # a copy forks token state: the first refresh in either store strands the other
        ...
        if mirrored["auth"]:
            ...
            _best_effort(lambda: _lazy("ulak_cli.auth", "strip_cloned_single_use_oauth_grants")(path))
...
def _describe_toolsets(cfg):
    """``(toolsets, pinned_set)`` as the `ulak tools` checklist presents them (the raw registry
    ...
    default_off = _try(lambda: _lazy("ulak_cli.tools_config", "_DEFAULT_OFF_TOOLSETS"), set())
    toolsets_out = []
...
def _(rid, params: dict) -> dict:
    ...
    if err is not None:
        return err
    with _ulak_home_scope(profile_dir):
        from ulak_cli.config import load_config
        from ulak_cli.skills_config import get_disabled_skills
        cfg = load_config() or {}
        ...
        meta = _try(lambda: _lazy("ulak_cli.profiles", "read_profile_meta")(profile_dir), {})
        return _ok(rid, {
...
def _configure_model(profile_dir, params, applied):
    ...
    if not is_truthy_value(params.get("confirm_expensive_model", False)):
        warn = _lazy("ulak_cli.model_selection_guards", "combined_selection_warning")
...
def _configure_cfg_sections(profile_dir, params, applied) -> None:
    ...
    if want_mcp:  # launch catalog read BEFORE the home override flips config resolution
        load_launch = _lazy("ulak_cli.config", "load_config_readonly")
    ...
    with _ulak_home_scope(profile_dir):
        ...
        if isinstance(params.get("disabled_skills"), list):
            try:
                from ulak_cli.skills_config import save_disabled_skills
...
def _(rid, params: dict) -> dict:
    ...
    if isinstance(params.get("description"), str):
        write_meta = _lazy("ulak_cli.profiles", "write_profile_meta")
