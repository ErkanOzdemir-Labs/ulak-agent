...
def _legacy_group_fence_error(rid, session, params):
    ...
    try:
        ...
        peer = False
        if not hosted:
            from ulak_constants import named_profile_home
    ...
    if hosted or peer:
        ...
        return _err(
            rid, 4122, f"This room is managed by {owner}. Update Ulak Desktop to continue it.")
    return None
...
def _parse_truncation_params(rid, sid, session, params, history):
    ...
    return None, None, _err(
        ...
        "(update your Ulak client if a rewind was intended)")
...
def _(rid, params: dict) -> dict:
    from ulak_cli.input_sanitize import sanitize_user_prompt_text
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from ulak_cli.clipboard import has_clipboard_image, save_clipboard_image
...
def _spawn_side_agent(
    ...
    def run():
        ...
        # Bug #50233: ephemeral agent threads don't inherit the session's ULAK_HOME override (the
        ...
        # Bug #50233: ephemeral preview-restart agent threads don't inherit the session's ULAK_HOME
...
_PREVIEW_RESTART_RULES = (
    "Restart exactly the app intended for the Preview URL, not Ulak Desktop itself.",
    ...
    "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Ulak/Desktop process cwd.",
    ...
    "Before declaring success, verify the Preview URL responds with the intended app, not Ulak Desktop. If it serves Ulak/Desktop UI or another unrelated app, stop that process and report failure.",
