...
activate with `ulak --tui` or `ULAK_TUI=1`. `tui_gateway` is ALSO the backend the Desktop app
...
themselves come from `ulak_cli/commands.py` (`ulak_cli/AGENTS.md`).
...
npm run dev       # watch mode (rebuilds ulak-ink + tsx --watch)
npm start         # production
npm run build     # full build (ulak-ink + tsc)
