...
def _relay_root() -> Path:
    ...
    home = Path(os.getenv("ULAK_HOME") or os.path.expanduser("~/.ulak"))
    return home.parent.parent if home.parent.name == "profiles" else home


# Per-attempt turn timeout and attempt ceiling for bot_relay.deliver. The Desktop client mirrors
# both (apps/desktop/src/plugins/ulak-bots/relay.ts: RELAY_TURN_ATTEMPT_MS / RELAY_TURN_MAX_ATTEMPTS)
...
def _(rid, params: dict, _root=_relay_root, _run=_run_delivery) -> dict:
    ...
    GATEWAY via the one-turn ``ulak -p <profile> chat -c "Bot Chat"`` transport local DMs use →
    ...
    try:
        ...
        resolved = "default" if profile.lower() == "ulak" else profile
        ...
        fd, tmp = tempfile.mkstemp(prefix="ulak-relay-dm-", suffix=".txt", text=True)
        try:
