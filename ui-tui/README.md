...
React + Ink terminal UI for Ulak. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ULAK_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `ulak setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.ulak/.ulak_history` or under `ULAK_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `ulak setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/ulak-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external ulak setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.ulak/tui-parent.log
    ...
    types/
      ulak-ink.d.ts            type declarations for @ulak/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent UlakCLI subprocess for slash commands
```
