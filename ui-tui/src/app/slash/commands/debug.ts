...
import { terminalBackgroundHex } from '@ulak/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $ULAK_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see ULAK_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['ULAK_TUI_BACKGROUND', process.env.ULAK_TUI_BACKGROUND ?? '(unset)'],
            ['ULAK_TUI_THEME', process.env.ULAK_TUI_THEME ?? '(unset)'],
