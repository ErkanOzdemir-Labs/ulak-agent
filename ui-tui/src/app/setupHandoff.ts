import type { RunExternalProcess } from '@ulak/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`ulak ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching ulak: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`ulak ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
