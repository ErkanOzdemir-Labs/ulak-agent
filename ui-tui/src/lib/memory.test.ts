...
const ENV_KEYS = ['ULAK_AUTO_HEAPDUMP', 'ULAK_HEAPDUMP_DIR', 'ULAK_HEAPDUMP_MAX_BYTES'] as const
...
describe('performHeapDump auto opt-in gate (#21767)', () => {
  ...
  beforeEach(() => {
    ...
    dir = mkdtempSync(join(tmpdir(), 'ulak-heapdump-test-'))
    process.env.ULAK_HEAPDUMP_DIR = dir
  })
  ...
  it('writes diagnostics only for auto-high without ULAK_AUTO_HEAPDUMP', async () => {
  ...
  it('writes diagnostics only for auto-critical without ULAK_AUTO_HEAPDUMP', async () => {
  ...
  it('writes both diagnostics and snapshot for auto-high when ULAK_AUTO_HEAPDUMP=1', async () => {
    process.env.ULAK_AUTO_HEAPDUMP = '1'
  ...
  it('accepts truthy spellings (true|yes|on, case-insensitive) as opt-in', async () => {
    for (const value of ['true', 'YES', 'On']) {
      process.env.ULAK_AUTO_HEAPDUMP = value
      const result = await performHeapDump('auto-high')
  ...
  it('writes both for manual triggers regardless of ULAK_AUTO_HEAPDUMP', async () => {
...
describe('heapdump retention guard (#21767)', () => {
  ...
  beforeEach(() => {
    savedDir = process.env.ULAK_HEAPDUMP_DIR
    savedMax = process.env.ULAK_HEAPDUMP_MAX_BYTES
    delete process.env.ULAK_AUTO_HEAPDUMP
    dir = mkdtempSync(join(tmpdir(), 'ulak-heapdump-prune-'))
    process.env.ULAK_HEAPDUMP_DIR = dir
  })

  afterEach(() => {
    if (savedDir === undefined) {
      delete process.env.ULAK_HEAPDUMP_DIR
    } else {
      process.env.ULAK_HEAPDUMP_DIR = savedDir
    }

    if (savedMax === undefined) {
      delete process.env.ULAK_HEAPDUMP_MAX_BYTES
    } else {
      process.env.ULAK_HEAPDUMP_MAX_BYTES = savedMax
    }
  ...
  it('evicts oldest files when total bytes exceed the cap, retaining the newest', async () => {
    ...
    process.env.ULAK_HEAPDUMP_MAX_BYTES = String(2 * 1024)
