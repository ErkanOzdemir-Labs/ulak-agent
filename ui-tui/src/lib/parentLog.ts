...
const logDir = join(process.env.ULAK_HOME?.trim() || join(homedir(), '.ulak'), 'logs')
...
// a real ~/.ulak (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('ulak-tui: parent lifecycle log unavailable\n')
    }
  }
}
