...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.ULAK_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Ulak on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via ULAK_TUI_TRUECOLOR=1.
