 ...
 * `ulak-boot-background` / `ulak-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports ULAK_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.ulak matches get_ulak_home()'s default.
const bootFilePath = () => join(process.env.ULAK_HOME ?? join(homedir(), '.ulak'), 'tui-theme-boot.json')

// Never touch the user's real ~/.ulak from test runs (the TS suite has no
// ULAK_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into ULAK_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.ULAK_TUI_THEME || env.ULAK_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.ULAK_TUI_BACKGROUND
  ) {
    env.ULAK_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.ULAK_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.ULAK_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into ULAK_TUI_THEME.
