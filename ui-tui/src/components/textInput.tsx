import type { InputEvent, Key } from '@ulak/ink'
import * as Ink from '@ulak/ink'
 ...
 * but that accepts `key.meta` on macOS — and ulak-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.ULAK_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which ulak-ink decodes as meta+'d'; without this branch it
