...
/**
 * User widget apps — Ulak authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$ULAK_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.ulak/plugins/`: files under ULAK_HOME execute
...
const widgetsDir = () => join(process.env.ULAK_HOME?.trim() || join(homedir(), '.ulak'), 'tui-widgets')
 ...
 *  every change, so a widget Ulak writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Ulak) ever writes must hot-load too — a 10s poll here read
