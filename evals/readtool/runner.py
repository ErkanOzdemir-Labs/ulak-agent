"""Run the read-tool eval through the REAL Ulak AIAgent.

For each task: fresh temp ULAK_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    ulak_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".ulak"
    ...
    try:
        ...
        shutil.rmtree(ulak_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.ulak/.env; set +a  — then relaunch."
        )
