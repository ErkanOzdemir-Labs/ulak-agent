...
# Remove all inherited Ulak/config and credential env before real imports.
for key in list(os.environ):
    if key.startswith('ULAK_') or key.endswith(('_API_KEY', '_TOKEN')):
        os.environ.pop(key, None)
home = tempfile.TemporaryDirectory(prefix='review-goaldup-')
os.environ['ULAK_HOME'] = home.name
os.environ['NO_PROXY'] = '*'
os.environ['TZ'] = 'UTC'
...
from ulak_cli import cli_commands_mixin, goals
...
server._ulak_home = Path(home.name)
...
with patch('ulak_cli.goals.draft_contract', return_value=None), contextlib.redirect_stdout(io.StringIO()):
