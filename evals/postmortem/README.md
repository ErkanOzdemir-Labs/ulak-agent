...
- **`forensics/`** reads a *copy* of a Ulak `state.db` (plus rotated `agent.log*` and git) and
...
- A Ulak checkout with its venv (`.venv/bin/python`). NeMo Relay is not required for the forensics;
  it is what the run itself used for the wire captures in `live_ab/cache_prefix_wire.py`.
- For forensics: a **copy** of `~/.ulak/state.db` (never point at the live file; `sqlite3 state.db ".backup copy.db"`
  or `cp` while Ulak is idle) and, optionally, the rotated `~/.ulak/logs/agent.log*`.
- For `--live` probes: real credentials in `ULAK_HOME` (they spend cents per run).
...
$P -m evals.postmortem.forensics.logcalls   --db state_copy.db --logs "$HOME/.ulak/logs/agent.log*"
...
Ulak recorded (an estimator, not an invoice).
...
```bash
# offline probes (fake providers, temp ULAK_HOME), main vs a branch or integration checkout:
...
The instrument behind #104284 / #104421 and ErkanOzdemir-Labs/api#227. N concurrent real `AIAgent`
...
Forensic lanes: five parallel Ulak subagents (2026-09-04), rewritten here to take `--db`/`--root`
