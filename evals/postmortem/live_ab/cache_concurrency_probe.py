...
Runs N independent Ulak ``AIAgent`` sessions concurrently, each the same ~8-step tool loop whose
...
Results that motivated ulak-agent #104284 / #104421 and ErkanOzdemir-Labs/api#227 (2026-09-05/06,
...
Usage:
  ...
  providers: nous (Portal creds from ULAK_HOME), openrouter (OPENROUTER_API_KEY or --api-key),
             anthropic (ANTHROPIC_API_KEY or --api-key)
Cost: ~$50 per 20x6 arm on Fable 5.1 at the 5m tier. Every run starts from the real Ulak request
...
def _parse():
    ...
    ap.add_argument("--repo", required=True, help="ulak-agent checkout to import from")
    ...
    ap.add_argument("--wire", choices=["chat", "native"], default=None,
                    help="force the wire (default: what Ulak would pick for the provider/model)")
...
os.environ.setdefault("ULAK_HOME", os.path.expanduser("~/.ulak"))
import anthropic
...
def creds():
    ...
    from ulak_cli.auth_nous import resolve_nous_runtime_credentials
    c = resolve_nous_runtime_credentials()
    return dict(api_key=c["api_key"], base_url=c.get("base_url") or "https://inference-api.github.com/ErkanOzdemir-Labs/v1", provider="nous")
CRED = creds()
