...
message prefix between call N and N+1. If Ulak strips prior-turn thinking, call N+1's messages[:k]
...
#   python cache_prefix_wire.py <repo_root> <A|B> [--ulak-home DIR]   (default ULAK_HOME: the real one, for credentials)
sys.path.insert(0, sys.argv[1])
os.environ.setdefault("ULAK_HOME", os.path.expanduser("~/.ulak"))
if "--ulak-home" in sys.argv:
    os.environ["ULAK_HOME"] = sys.argv[sys.argv.index("--ulak-home") + 1]
...
time.sleep(1)
log = subprocess.run(f"grep -h '\\[{sid}\\]' ~/.ulak/logs/agent.log | grep 'API call #'", shell=True, capture_output=True).stdout.decode("utf-8", "replace")
