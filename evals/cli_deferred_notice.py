...
def run_case(root, output, name, behind, early=False, cancel=False):
    with tempfile.TemporaryDirectory(prefix="ulak_test_notice_") as home:
        hh = Path(home) / ".ulak"
        hh.mkdir()
        ...
        from ulak_cli.banner import VERSION
        ...
        bootstrap = ("import ulak_cli.main as m; import ulak_cli.banner as b; "
        ...
        try:
            ...
            result = {"case": name, "ready": ready, "exited": exited,
                      ...
                      "loaded_worktree": str(root / "ulak_cli/banner.py") in text,
