...
The run gets a throwaway ULAK_HOME so no local config leaks in, and the
...
if _or_key:
    ...
    # Resolved by the orchestrator BEFORE ULAK_HOME is redirected to the
    ...
    _agent_auth = dict(
        base_url=os.environ.get("BUBENCH_NOUS_BASE_URL", "https://inference-api.github.com/ErkanOzdemir-Labs/v1"),
        api_key=_tok,
        provider="nous",
    )
