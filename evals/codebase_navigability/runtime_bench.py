...
Each probe runs in a FRESH subprocess with an isolated ULAK_HOME so nothing is cached across reps.
...
ENV = {**os.environ, "ULAK_HOME": HOME, "PYTHONPATH": TREE, "PYTHONDONTWRITEBYTECODE": "0", "OPENROUTER_API_KEY": "sk-bench-placeholder",
       "ULAK_SKIP_UPDATE_CHECK": "1", "NO_COLOR": "1", "TERM": "dumb", "COLUMNS": "120"}
for k in list(ENV):
    if k.startswith(("ULAK_SESSION", "ULAK_PROFILE")): ENV.pop(k)
...
IMPORT_TARGETS = ["run_agent", "cli", "ulak_cli.main", "gateway.run", "tools.registry", "ulak_state", "tui_gateway.server", "ulak_cli.web_server", "model_tools", "agent.prompt_builder"]
probe = r'''
...
tree=os.getcwd(); foreign=[m for m,mod in list(sys.modules.items()) if getattr(mod,"__file__",None) and "ulak-agent" in mod.__file__ and not mod.__file__.startswith(tree) and "site-packages" not in mod.__file__]
...
# 2. CLI end-to-end startup: `ulak --version`, `ulak --help`, `ulak doctor --help`, `ulak config get model` (no network)
CLI = {"version": ["ulak_cli/main.py", "--version"], "help": ["ulak_cli/main.py", "--help"], "config_get": ["ulak_cli/main.py", "config", "get", "model"], "tools_list": ["ulak_cli/main.py", "tools", "--help"], "skills_help": ["ulak_cli/main.py", "skills", "--help"]}
cli = {}
...
try:
    ...
    T("resolve_toolset_ulak_default_ms", lambda: resolve_toolset("ulak-default"), 200)
...
try:
    import ulak_state, tempfile, uuid
    from pathlib import Path
    db=ulak_state.SessionDB(Path(tempfile.mkdtemp())/"s.db") if hasattr(ulak_state,"SessionDB") else None
    if db:
        sid=str(uuid.uuid4())
