"""Drive the real ``ulak fallback add`` under a Linux PTY into an ordinary picker error.
...
def _persisted_model(root: Path, env: dict) -> dict:
    ...
    out = subprocess.run(
        [sys.executable, "-c", "import json; from ulak_cli.config import load_config; "
...
def run(root: Path, output: Path) -> dict:
    with tempfile.TemporaryDirectory(prefix="ulak_test_fallback_") as home:
        hh = Path(home) / ".ulak"
        hh.mkdir()
        ...
        env = {"PATH": os.environ["PATH"], "HOME": home, "ULAK_HOME": str(hh),
        ...
        proc = subprocess.Popen([sys.executable, "-m", "ulak_cli.main", "fallback", "add"],
