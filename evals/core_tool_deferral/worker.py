...
# --- hermetic env BEFORE any ulak import -------------------------------
for var in list(os.environ):
    if var.endswith(("_API_KEY", "_TOKEN")) and var != "OPENROUTER_API_KEY":
        os.environ.pop(var, None)
os.environ.pop("FAL_KEY", None)
os.environ.pop("ULAK_PROFILE", None)

tmp_root = tempfile.mkdtemp(prefix=f"ab-{ARM}-{TASK_ID}-")
ulak_home = os.path.join(tmp_root, ".ulak")
workspace = os.path.join(tmp_root, "ws")
os.makedirs(ulak_home)
os.makedirs(workspace)
with open(os.path.join(ulak_home, "config.yaml"), "w", encoding="utf-8") as f:
    f.write("model:\n  provider: openrouter\n  model: %s\n" % MODEL)
...
PREAMBLE = ("You are running inside the Ulak desktop app on the user's machine. "
...
ctx = {
    "workspace": workspace, "ulak_home": ulak_home,
