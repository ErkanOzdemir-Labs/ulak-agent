#!/usr/bin/env python3
"""Live test harness for Ulak Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any ulak imports.
ORIGINAL_HOME = os.environ.get("ULAK_HOME")
ORIGINAL_AUTH = Path.home() / ".ulak" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.ulak/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.ulak/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="ulak_ts_live_"))
    ulak_home = home_dir / ".ulak"
    ulak_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, ulak_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".ulak" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, ulak_home / ".env")
        ...
        load_ulak_dotenv(ulak_home=str(Path.home() / ".ulak"))
    ...
    (ulak_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return ulak_home
...
def reset_module_state():
    """Drop cached modules so the new ULAK_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "ulak_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original ULAK_HOME
    if ORIGINAL_HOME is not None:
        os.environ["ULAK_HOME"] = ORIGINAL_HOME
    else:
