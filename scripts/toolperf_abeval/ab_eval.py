...
measured in the production session DB) through `ulak chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    ULAK_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Ulak home with credentials for the
...
(tracker: ErkanOzdemir-Labs/ulak-agent#77056).
"""
import json
import os
import re
...
""".strip(),
            ...
            env.update({
                ...
                "ULAK_HOME": str(HOME),
                "ULAK_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "ulak_cli.main", "chat", "--query", q,
