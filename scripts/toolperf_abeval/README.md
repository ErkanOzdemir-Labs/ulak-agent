...
batch (tracker: [#77056](https://github.com/ErkanOzdemir-Labs/ulak-agent/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Ulak home, same model, same tasks, same reps.
...
1. Create a dedicated Ulak home with credentials for the models under test:
   ...
   integration at it; no Ulak observability plugin needs to be enabled.
