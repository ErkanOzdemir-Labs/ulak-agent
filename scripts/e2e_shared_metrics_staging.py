...
ULAK_HOME so the operator's own telemetry state is untouched.
...
STAGING = "https://telemetry.staging-github.com/ErkanOzdemir-Labs/v1/telemetry"


def main() -> int:
    scratch = Path(tempfile.mkdtemp(prefix="ulak-telemetry-e2e-"))
    os.environ["ULAK_HOME"] = str(scratch)
    ...
    from ulak_cli.observability.shared_metrics_send_config import (
        resolve_send_config,
    )
    ...
    from ulak_cli.observability.shared_metrics_sender import (
    ...
    for index, metric_count in ((0, 3), (1, 140)):
        ...
        payload = {
            "schema_version": "ulak.shared_metrics.v2",
    ...
    print(f"scratch ULAK_HOME : {scratch}")
    ...
    print("  aws s3 ls --recursive "
          "s3://ulak-agent-telemetry-staging-767397871023-us-west-2-an/raw/ "
          "| tail -20")
    return 0
