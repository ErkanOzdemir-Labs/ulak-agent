# ============================================================================
# Ulak Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$UlakHome = $(if ($env:ULAK_HOME) { $env:ULAK_HOME } else { "$env:LOCALAPPDATA\ulak" }),
    [string]$InstallDir = $(if ($env:ULAK_HOME) { "$env:ULAK_HOME\ulak-agent" } else { "$env:LOCALAPPDATA\ulak\ulak-agent" }),
    ...
    # builds apps/desktop into a launchable Ulak.exe.
    #
    # Why opt-in:
    #   * Ulak-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Ulak.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Ulak.exe
    ...
    #     `ulak desktop` already builds on demand.
...
# ULAK_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[ulak] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'UlakInstall.LongPath').Type) {
            Add-Type -Namespace 'UlakInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [UlakInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -UlakHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('UlakHome')) {
    $UlakHome = ConvertTo-LongPath $UlakHome
} else {
    $UlakHome = ConvertTo-LongPath $(
        if ($env:ULAK_HOME) { $env:ULAK_HOME } else { "$env:LOCALAPPDATA\ulak" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:ULAK_HOME) { "$env:ULAK_HOME\ulak-agent" } else { "$env:LOCALAPPDATA\ulak\ulak-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: UlakHome=$UlakHome InstallDir=$InstallDir"
}
...
$RepoUrlSsh = "git@github.com:ErkanOzdemir-Labs/ulak-agent.git"
$RepoUrlHttps = "https://github.com/ErkanOzdemir-Labs/ulak-agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Ulak Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Ulak Agent.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $UlakHome)) {
        New-Item -ItemType Directory -Force -Path $UlakHome | Out-Null
    }
    $envFile = Join-Path $UlakHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `ulak update`'s npx
    ...
    $prefixDir = Join-Path $UlakHome "node"
...
function Install-Uv {
    # Ulak owns its own uv at $UlakHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (ulak_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `ulak update` stay in sync.
    $managedUv = Join-Path $UlakHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $UlakHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $UlakHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $UlakHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $UlakHome "bin"
        ...
        # (ulak_cli/managed_uv.py looks only at $UlakHome\bin\uv.exe).
...
# Put the Ulak-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone ulak-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Ulak-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Ulak-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("ulak-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and ULAK_HOME wasn't propagated).
...
function Initialize-ManagedPythonEnvironment {
    # Python used by Ulak belongs to the checkout, never to another
    # application or a user-level uv configuration. Keep this aligned with
    # ulak_cli.managed_uv.managed_python_env(), which owns the update path.
    foreach ($name in @(
    ...
    $managedRoot = Join-Path $InstallDir ".ulak-runtime\python"
...
function Resolve-AvailablePythonVersion {
    # Return the path and minor version of the first Ulak-managed interpreter
    ...
    #
    # Under Ulak-Setup.exe each stage runs in a fresh powershell.exe. The
    ...
    foreach ($ver in $candidates) {
        ...
        try {
            ...
            throw "Failed to resolve Ulak-managed Python $ver`: $_"
        } finally {
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Ulak setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Ulak uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\ulak\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Ulak needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Ulak copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\ulak\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``ULAK_GIT_BASH_PATH`` (User scope) so Ulak can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Ulak-managed PortableGit install instead..."
    }

    # Download PortableGit into $UlakHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $UlakHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Ulak features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Ulak needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``ULAK_GIT_BASH_PATH`` (User env scope) so Ulak can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $UlakHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $UlakHome\git\usr\bin\bash.exe
    $candidates += "$UlakHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$UlakHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("ULAK_GIT_BASH_PATH", $candidate, "User")
            $env:ULAK_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set ULAK_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Ulak may not find Git Bash."
    Write-Info "If needed, set ULAK_GIT_BASH_PATH manually to your bash.exe path."
}
...
# range used to provision the Ulak-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if (Test-NodeVersionOk $version) {
        ...
        Write-Warn "Node.js $version is unsupported (Ulak requires Node 22.22+, 24.11+, or 26+)"
        return $false
    }
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy Ulak requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using a Ulak-managed Node.js installation instead..."

    # Prefer a Ulak-managed Node from a previous run over a too-old system one.
    $managedNode = "$UlakHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$UlakHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$UlakHome\node"
        Write-Success "Node.js $version found (Ulak-managed)"
        ...
        Update-ManagedNpm "$UlakHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Ulak-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $UlakHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $UlakHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\ulak-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$UlakHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$UlakHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$UlakHome\node") {
                    try {
                        Rename-Item "$UlakHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Ulak-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$UlakHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $UlakHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\ulak-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `ulak
                    # update` path (#4735).
                    ...
                    $stashName = "ulak-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. ulak-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``ulak update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Ulak behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running ulak processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/ErkanOzdemir-Labs/ulak-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/ErkanOzdemir-Labs/ulak-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/ErkanOzdemir-Labs/ulak-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\ulak-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\ulak-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `ulak update` (see the notes at the shared clone-path
    ...
    # next `ulak update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Ulak-Setup.exe
    ...
    if (-not $resolvedPython) {
        throw "Ulak-managed Python is unavailable. Run install.ps1 -Stage python first."
    }
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running ulak process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running ulak processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Ulak_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # venv\Scripts\, so its image name is python/pythonw, not ulak.exe.
            ...
            # Ulak children re-exec through .ulak-runtime, so killing only
            ...
            # user (see ulak_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Ulak " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `ulak update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Ulak (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `ulak.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install ulak-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and ulak.exe broken with
    ...
    # here, not 30 seconds later when the user runs `ulak`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Ulak processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Ulak processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Ulak and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register ulak.exe in dist-info/RECORD but fail to
        ...
        # `ulak` command while ulak-agent.exe / ulak-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m ulak_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `ulak dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\ulak_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `ulak dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `ulak dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: ulak_cli/web_server.py"
        }
    }
...
function Install-UlakCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the ulak launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring ulak.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "ulak.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the ulak command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # ulak_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("ulak", "ulak-acp")) {
    ...
    $requiredExe = Join-Path $Destination "ulak.exe"
    $requiredCmd = Join-Path $Destination "ulak.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the ulak command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up ulak command..."
    
    if ($NoVenv) {
        $ulakBin = "$InstallDir"
    } else {
        # $UlakHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `ulak update`'s autostash
        ...
        # installer staged under ulak-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-UlakCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $ulakBin = "$UlakHome\bin"
        Install-UlakCommandLaunchers -Root $InstallDir -Destination $ulakBin | Out-Null
    }
    ...
    #   ulak-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The ulak-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept ulak via $ulakBin)"
        }
    }
    
    if ($currentPath -notlike "*$ulakBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$ulakBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $ulakBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set ULAK_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\ulak instead
    # of the Unix default ~/.ulak
    $currentUlakHome = [Environment]::GetEnvironmentVariable("ULAK_HOME", "User")
    if (-not $currentUlakHome -or $currentUlakHome -ne $UlakHome) {
        [Environment]::SetEnvironmentVariable("ULAK_HOME", $UlakHome, "User")
        Write-Success "Set ULAK_HOME=$UlakHome"
    }
    $env:ULAK_HOME = $UlakHome
    
    # Update current session
    $env:Path = "$ulakBin;$env:Path"
    
    Write-Success "ulak command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.ulak-bootstrap-complete which tells the Ulak
    ...
    $markerPath = Join-Path $InstallDir ".ulak-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the ULAK_HOME directory structure ($UlakHome, default %LOCALAPPDATA%\ulak)
    New-Item -ItemType Directory -Force -Path "$UlakHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$UlakHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$UlakHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$UlakHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$UlakHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$UlakHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$UlakHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$UlakHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$UlakHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$UlakHome\.env"
    ...
    # Create config.yaml
    $configPath = "$UlakHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Ulak's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$UlakHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in ulak_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $UlakHome"
    
    # Seed bundled skills into $UlakHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $UlakHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $UlakHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$UlakHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $UlakHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Ulak-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'ulak setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\ulak-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\ulak-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\ulak-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `ulak tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $UlakHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only Ulak' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Ulak' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'ulak tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `ulak
...
# paths still lazy-install via install_cua_driver() (ulak_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `ulak computer-use install` runs. Bounded
        ...
        # Ulak' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'ulak tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: ulak computer-use install"
            }
        } else {
...
# the final `electron` -> `Ulak` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Ulak.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_ulak_desktop command
    # resolves apps/desktop/release/win-unpacked/Ulak.exe directly,
    ...
    # enforces the supported Node lines and prepends the Ulak-managed Node to
    ...
    # The --dir mode produces an unpacked Ulak.exe in
    ...
    # are NOT trying to work around). The Ulak icon + product name are
    # stamped onto Ulak.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\ulak-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Ulak.exe",
        "$desktopDir\release\win-arm64-unpacked\Ulak.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Ulak.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Ulak icon + identity are stamped onto Ulak.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, ulak-agent#38216).
    ...
    #    Ulak.exe. We deliberately do NOT point them at `ulak desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Ulak.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Ulak.lnk')
        )
        ...
        # with the Ulak icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.ulak/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'ulak setup'."
        return
    }
    ...
    # Run ulak setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m ulak_cli.main setup
    } else {
        python -m ulak_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $ulakCmd = "$InstallDir\venv\Scripts\ulak.exe"
    if (-not (Test-Path $ulakCmd)) {
        $ulakCmd = "ulak"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$UlakHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'ulak whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: ulak gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$UlakHome\logs\gateway.log"
            Start-Process -FilePath $ulakCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$UlakHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: ulak gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: ulak gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$UlakHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$UlakHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$UlakHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$UlakHome\ulak-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   ulak              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   ulak setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   ulak config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   ulak config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   ulak gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   ulak update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Ulak repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    # Managed Python lives under $InstallDir\.ulak-runtime, so the checkout
...
if ($IncludeDesktop) {
    ...
    # (Ulak-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Ulak to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://ulak-agent.github.com/ErkanOzdemir-Labs/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
