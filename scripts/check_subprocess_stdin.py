...
When Ulak runs in TUI mode, the gateway child process communicates with
...
# ``get_ulak_home() / "plugins"`` (user) and ``./.ulak/plugins/`` (project,
# gated behind ``ULAK_ENABLE_PROJECT_PLUGINS``) — see
# ``ulak_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``ulak_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import ulak_constants (this script
    ...
    # plugins in get_ulak_home()/plugins/ and project plugins in
    # ./.ulak/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_ulak_home() / "plugins"]
    if os.environ.get("ULAK_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".ulak" / "plugins")
