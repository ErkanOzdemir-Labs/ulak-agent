...
#
# The whole job: wait for the Desktop to exit, run `ulak update`, tell the
...
# the ULAK_DESKTOP_CHILD_PID reaper-exclusion dance dies with it.
#
# CONTRACT (keep in sync with apps/desktop/electron/main.ts):
#   bash scripts/desktop-update/posix.sh
#     --install-root <path>    repo checkout (ULAK_HOME/ulak-agent)
...
ULAK_HOME="${INSTALL_ROOT:+$(dirname "$INSTALL_ROOT")}"
ULAK_HOME="${ULAK_HOME:-${TMPDIR:-/tmp}}"
MARKER="$ULAK_HOME/.ulak-update-in-progress"
LOG_DIR="$ULAK_HOME/logs"; mkdir -p "$LOG_DIR" 2>/dev/null || true
LOG="$LOG_DIR/desktop-update-handoff.log"
RESULT="$ULAK_HOME/.ulak-update-result.json"
STATUS="${TMPDIR:-/tmp}/ulak-update-status.$$"
...
notify_fallback() { # status message — renderer-free recovery surface.
  ...
  if [ "$(uname)" = "Darwin" ]; then
    /usr/bin/osascript -e "display notification \"$(printf '%s' "$2" | sed 's/"/\\"/g')\" with title \"Ulak update\"" 2>/dev/null && return 0
  else
    if command -v notify-send >/dev/null 2>&1; then
      notify-send -u critical "Ulak update" "$2" 2>/dev/null && return 0
    fi
    local p
    if command -v zenity >/dev/null 2>&1; then
      zenity --warning --title="Ulak update" --text="$2" 2>/dev/null &
      p=$!; sleep 1
      kill -0 "$p" 2>/dev/null && return 0
      wait "$p" 2>/dev/null
    fi
    if command -v kdialog >/dev/null 2>&1; then
      kdialog --title "Ulak update" --sorry "$2" 2>/dev/null &
      p=$!; sleep 1
      kill -0 "$p" 2>/dev/null && return 0
      wait "$p" 2>/dev/null
    fi
...
find_browser() {
  ...
  # email included — inside this window that is titled "Ulak" (#88410).
...
start_ui() {
  ...
  # teardown TERM killed the shim ~1s into `ulak update` (2026-08-14 16:44,
  ...
  "$py" -c 'import os, signal, sys; os.setsid(); signal.signal(signal.SIGTERM, signal.SIG_DFL); os.execv(sys.argv[1], sys.argv[1:])' \
    "$browser" --app="http://127.0.0.1:$port/" --user-data-dir="${TMPDIR:-/tmp}/ulak-update-ui-$$" \
...
stop_ui() { # error/manual outcomes keep the window up briefly so a watching
  ...
  if [ "${1:-}" = "leave-window" ]; then
    sleep "${ULAK_UPDATE_SHIM_GRACE_SECONDS:-15}"
  fi
...
linux_gate() {
  ...
  # _desktop_linux_userns_sandbox_available() probe in ulak_cli/main.py).
  ...
  GATE=manual GATE_MSG="Update complete, but the rebuilt app can't relaunch itself (its sandbox helper needs root ownership). Reopen Ulak to finish."
}

mac_swap() {
  local rebuilt="" c
  for c in "$INSTALL_ROOT/apps/desktop/release/mac-arm64/Ulak.app" \
           "$INSTALL_ROOT/apps/desktop/release/mac/Ulak.app"; do
    [ -d "$c" ] && { rebuilt="$c"; break; }
  done
  ...
  if [ "$FINAL_CODE" -eq 0 ] && [ -n "$rebuilt" ] && [ -d "$RELAUNCH_TARGET" ] && [ "$rebuilt" != "$RELAUNCH_TARGET" ]; then
    ...
    if ! /usr/bin/ditto "$rebuilt" "$RELAUNCH_TARGET.new"; then
      ...
      if mv "$RELAUNCH_TARGET.old" "$RELAUNCH_TARGET"; then
        ...
        FINAL_CODE=7 FINAL_MSG="The update finished but installing the new app failed and the previous app could not be restored. Reinstall Ulak (the rebuilt app is at $rebuilt)."
  ...
  if [ -n "$DONE_NOTE" ]; then
    if [ "$(uname)" = "Darwin" ]; then
      ...
      if ! launch_app; then
        ...
        FINAL_MSG="$DONE_NOTE Ulak also could not reopen itself - open it manually."
        write_result
      fi
    fi
    ...
    FINAL_MSG="Update complete. Reopen Ulak to finish (it could not restart itself)."
    MANUAL=1
    write_result
    publish "manual" "$FINAL_MSG"; stop_ui leave-window
  fi
...
# (`venv/bin/ulak` has a `python3` shebang), so no Python-side heal —
...
# (ulak_cli/macos_tcc_anchor.ensure_tcc_anchor, which re-anchors whenever
...
# touched; and when it does restore symlinks, the very `ulak update` run it
...
tcc_pick_update_invoke() { # sets UPDATE_INVOKE; safety net past a failed heal
  ...
  # launchd gateway proves `venv/bin/python -m ulak_cli.main` works when
  # every alias entrypoint is bricked — drive the update the same way.
  local bin="$1"
  UPDATE_INVOKE=("$bin/ulak")
  if ! tcc_probe_python "$bin/python3" && tcc_probe_python "$bin/python"; then
    UPDATE_INVOKE=("$bin/python" -m ulak_cli.main)
  fi
}
...
if [ "$SELF_TEST_UI" -eq 1 ]; then
  ...
  sleep "${ULAK_SELFTEST_HOLD_SECONDS:-6}"
  RELAUNCH_TARGET=""
  if [ -n "${ULAK_SELFTEST_FAIL:-}" ]; then FINAL_MSG="self-test error state"
...
# died two seconds after starting `ulak update`. Re-exec through a one-shot
...
if [ "$HANDOFF_DAEMONIZED" -ne 1 ]; then
  ...
  # EXIT: that would publish a false failure and relaunch Ulak while the
...
# so the shim server and the later `ulak update` subprocess both inherit
...
# update_lock.py (the `ulak update` child adopts it via process ancestry).
# The Desktop supplies one acquisition time for the whole ownership chain.
NOW="$(date +%s)"
STARTED_AT="${ULAK_UPDATE_STARTED_AT:-$NOW}"
...
if [ "$DESKTOP_PID" -gt 0 ] 2>/dev/null; then
  ...
  if kill -0 "$DESKTOP_PID" 2>/dev/null; then
    FINAL_CODE=4 FINAL_MSG="Update aborted: the Ulak window (pid $DESKTOP_PID) did not exit within 30s. Nothing was changed. Close Ulak fully and try again."
...
ULAK_BIN="$INSTALL_ROOT/venv/bin/ulak"
[ -x "$ULAK_BIN" ] || { FINAL_CODE=3 FINAL_MSG="Update aborted: $ULAK_BIN is missing. The install needs repair (run the Ulak installer or ulak doctor)."; log "$FINAL_MSG"; exit 3; }

# Heal a venv the reverted TCC anchor left bricked BEFORE invoking the CLI:
# venv/bin/ulak execs venv/bin/python3, so a dead alias kills every attempt
...
tcc_pick_update_invoke "$INSTALL_ROOT/venv/bin"
if [ "${UPDATE_INVOKE[0]}" != "$ULAK_BIN" ]; then
  log "venv/bin/python3 still unbootable; invoking the update via ${UPDATE_INVOKE[*]}"
fi

# Run FROM the install root: `ulak update` resolves the tree it mutates
...
# "close all Ulak windows" sentinel.
KEEP_STASH=""
if "${UPDATE_INVOKE[@]}" update --help 2>/dev/null | grep -q -- '--keep-stash'; then
  KEEP_STASH="--keep-stash"
else
  log "installed ulak predates --keep-stash; running without it"
fi
...
printf '%s\n' "$OUT" >> "$LOG" 2>/dev/null
log "ulak update exit code: $CODE"

if [ "$CODE" -ne 0 ] && [ "$CODE" -ne 2 ]; then
  # Retry once: update-boundary class (fresh code on disk, stale in memory).
  # Exit 2 ("close all Ulak windows") is not retryable.
  #
  ...
  if printf '%s' "$OUT" | grep -q "CODE UPDATE SKIPPED"; then
    log "ulak update skipped (checkout parked on a non-target branch); not retrying"
    FINAL_CODE=8
...
# Truthful completion: `ulak update` calls a GUI build failure non-fatal
...
if [ "$CODE" -eq 0 ] && printf '%s' "$OUT" | grep -q "Desktop build failed"; then
  log "desktop build failed inside ulak update; retrying build"
  publish_stage "Rebuilding Desktop"
  "${UPDATE_INVOKE[@]}" desktop --force-build --build-only >> "$LOG" 2>&1 || {
    FINAL_CODE=6 FINAL_MSG="Code and dependencies updated, but the Desktop app rebuild failed - you are running the previous build. Run ulak desktop --force-build from a terminal to retry."
    exit 6
  }
fi
...
if [ "$CODE" -eq 0 ]; then FINAL_CODE=0 FINAL_MSG="Update complete."
  ...
  FINAL_CODE="$CODE" FINAL_MSG="Update failed (exit $CODE). Run ulak debug share in a terminal to send a report."
  ...
  if ! tcc_probe_python "$INSTALL_ROOT/venv/bin/python3" \
    ...
    FINAL_MSG="Update failed: the Python interpreter inside $INSTALL_ROOT/venv cannot start (heal state: $TCC_HEAL_STATE). Reinstall the runtime with the Ulak installer, or run ulak doctor --fix from a terminal if any ulak command still works."
  fi
fi
exit "$FINAL_CODE"
