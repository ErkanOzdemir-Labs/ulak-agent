...
# (%ULAK_HOME%\ulak-setup.exe). That binary has no self-update path --
# copy_self_to_ulak_home deliberately no-ops during --update -- so every
...
#
# This script lives in the repo checkout, so EVERY `ulak update` refreshes
...
#     -InstallRoot <path>   repo checkout (ULAK_HOME\ulak-agent)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] Ulak.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .ulak-update-in-progress in place (tests)
#
...
# .ulak-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim ULAK_HOME\.ulak-update-in-progress with OUR pid as
# step 0 (the wrapper cmd.exe pid the Desktop saw is useless -- it exits
# immediately), retaining ULAK_UPDATE_STARTED_AT from the Desktop hand-off.
# ulak_cli/update_lock.py's ancestry rule lets our
# `ulak update` child adopt the claim; electron/update-marker.ts parks a
...
# pass our foreground right on to the new Ulak.exe pid.
try {
    Add-Type -Namespace UlakHandoff -Name Win32 -MemberDefinition @'
...
$UlakHome = if ($InstallRoot) { Split-Path -Parent $InstallRoot } else { $TempDir }
$MarkerPath = Join-Path $UlakHome ".ulak-update-in-progress"
$LogDir = Join-Path $UlakHome "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $UlakHome ".ulak-update-result.json"
$script:Ui = $null
$script:UiStage = "Ulak will open once done."   # until the first gate; matches ui.html
...
function Stop-UiServer([switch]$LeaveWindow) {
    ...
    # profile plus any stale ulak-update-ui-* leftovers from interrupted
    ...
    # update marker (.ulak-update-in-progress) serialises hand-offs, so no
    # other run's profile can be in active use here.
    try {
        $profileDirs = @()
        if ($script:UiServer.Profile) { $profileDirs += $script:UiServer.Profile }
        Get-ChildItem -LiteralPath $TempDir -Directory -Filter "ulak-update-ui-*" -ErrorAction SilentlyContinue |
...
function Publish-UiProgress([string]$Message) {
    ...
    # stderr remain asynchronously drained in Invoke-UlakStep and are never
...
function Show-ProgressWindow {
    ...
    if ($htmlPath -and $browser) {
        ...
        if ($server) {
            try {
                ...
                $browserProfile = Join-Path $TempDir ("ulak-update-ui-{0}" -f $PID)
                $browserArgs = @(
    ...
    try {
        ...
        try {
            $form.Activate()
            if ($script:Win32) { [UlakHandoff.Win32]::SetForegroundWindow($form.Handle) | Out-Null }
        } catch {}
        [System.Windows.Forms.Application]::DoEvents()
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `ulak debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"ulak debug share`" in a terminal to send a report."
        ...
        try {
            $ui.Form.Activate()
            if ($script:Win32) { [UlakHandoff.Win32]::SetForegroundWindow($ui.Form.Handle) | Out-Null }
        } catch {}
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old Ulak.exe before writing the replacement,
    ...
    # DO NOT spawn Ulak.exe as our child: Electron/Chromium calls
    ...
    try {
        ...
        if ($r -and $r.ReturnValue -eq 0) {
            ...
            try {
                if ($script:Win32) {
                    [UlakHandoff.Win32]::AllowSetForegroundWindow([int]$r.ProcessId) | Out-Null
                    ...
                    while ((Get-Date) -lt $deadline) {
                        ...
                        if ($hwnd -ne [System.IntPtr]::Zero) {
                            [UlakHandoff.Win32]::ShowWindow($hwnd, 9) | Out-Null  # SW_RESTORE
                            [UlakHandoff.Win32]::SetForegroundWindow($hwnd) | Out-Null
    ...
    if (-not $spawned) {
        ...
        # verify by watching for a fresh Ulak process.
        try {
            ...
            while ((Get-Date) -lt $explorerDeadline) {
                ...
                if ($fresh.Count -gt 0) {
                    ...
                    try {
                        if ($script:Win32) {
                            [UlakHandoff.Win32]::AllowSetForegroundWindow([int]$fresh[0].Id) | Out-Null
...
# closes it. `ulak update` deliberately runs its build steps with stdout
# inherited (ulak_cli/main.py, the tee-stderr runner), so the tree under a
...
if ($env:ULAK_UPDATE_PIPE_DRAIN_SECONDS) {
    $parsedGrace = 0
    if ([int]::TryParse($env:ULAK_UPDATE_PIPE_DRAIN_SECONDS, [ref]$parsedGrace) -and $parsedGrace -ge 0) {
...
if ($env:ULAK_UPDATE_STEP_IDLE_SECONDS) {
    $parsedIdle = 0
    if ([int]::TryParse($env:ULAK_UPDATE_STEP_IDLE_SECONDS, [ref]$parsedIdle) -and $parsedIdle -gt 0) {
        $script:StepIdleTimeoutSeconds = $parsedIdle
    }
}

# Silence on the pipes is NOT silence in the update. `ulak update` captures
# the (very loud) Electron/vite build into logs/update.log instead of its own
# stdout (ulak_cli/update_cmd.py, the update-log tee), so a real update is
...
if ($env:ULAK_UPDATE_PROGRESS_LOG) {
    $script:StepProgressLogPath = $env:ULAK_UPDATE_PROGRESS_LOG
}
...
public static class UlakUpdateJob {
...
function Invoke-UlakStep([string]$Exe, [string[]]$UlakArgs, [string]$Tag) {
    ...
    # afterwards, where `ulak debug share` picks it up.
    #
    ...
    # .ulak-update-result.json, clearing .ulak-update-in-progress,
    ...
    # Ulak" until the user killed something by hand. Losing the tail of a
    ...
    $arguments = ($UlakArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    ...
    try {
        ...
        $started = [UlakUpdateJob]::StartAssigned($Exe, $arguments)
    } finally {
    ...
    while ($true) {
        ...
        if ($proc.HasExited) {
            ...
            # Quiet pipes are how a healthy `ulak update` looks for 40+
            ...
            if ($currentLogStamp -ne $progressLogStamp) {
                ...
                $stalled = [UlakUpdateJob]::TerminateAndWait($job, 124, 10000)
                if (-not $stalled) {
...
# the page can be QA'd in any browser; ULAK_SELFTEST_FAIL=1 exercises the
# error state, ULAK_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:ULAK_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:ULAK_SELFTEST_HOLD_SECONDS }
    Publish-UiProgress "Testing quiet update"
    Start-Sleep -Seconds $hold
    if ($env:ULAK_SELFTEST_FAIL) {
...
# -SelfTestPipeDrain: prove Invoke-UlakStep survives a leaked pipe ------
# The #90455 deadlock needs no update, no checkout and no Ulak install to
...
#            update log, the shape of every real `ulak update` build (output
...
if ($SelfTestPipeDrain) {
    ...
    if ($env:ULAK_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:ULAK_SELFTEST_HOLD_SECONDS }
    $floodKb = 8192
    if ($env:ULAK_SELFTEST_FLOOD_KB) { $floodKb = [int]$env:ULAK_SELFTEST_FLOOD_KB }
    # $PSHOME is this interpreter's own directory -- no hardcoded system path.
    $powershell = Join-Path $PSHOME "powershell.exe"
    $stamp = [Guid]::NewGuid().ToString("N")
    $childPs1 = Join-Path $TempDir "ulak-pipe-drain-$stamp.ps1"
    $floodPs1 = Join-Path $TempDir "ulak-pipe-flood-$stamp.ps1"
    $pidFile = Join-Path $TempDir "ulak-pipe-drain-$stamp.pid"
    $stallPs1 = Join-Path $TempDir "ulak-step-stall-$stamp.ps1"
    $stallPidFile = Join-Path $TempDir "ulak-step-stall-$stamp.pid"
    $stallGrandchildPidFile = Join-Path $TempDir "ulak-step-stall-grandchild-$stamp.pid"
    $logStallPs1 = Join-Path $TempDir "ulak-step-logstall-$stamp.ps1"
    $logStallProgress = Join-Path $TempDir "ulak-step-logstall-$stamp.update.log"
...
'@
    ...
    # merely loud. `ulak update` is this shape -- the Electron/vite build
...
'@
    ...
    $res = Invoke-UlakStep $powershell @(
    ...
    $flood = Invoke-UlakStep $powershell @(
    ...
    $stall = Invoke-UlakStep $powershell @(
    ...
    try {
        $logstall = Invoke-UlakStep $powershell @(
    ...
    if ($stallAlive) { $problems += "stalled child pid $stallPid remained alive after Invoke-UlakStep returned" }
    if ($stallGrandchildAlive) { $problems += "stalled descendant pid $stallGrandchildPid remained alive after Invoke-UlakStep returned" }
...
try {
    ...
    try {
        ...
        $startedAt = 0L
        $hasStartedAt = [int64]::TryParse($env:ULAK_UPDATE_STARTED_AT, [ref]$startedAt)
    ...
    Publish-UiProgress "Waiting for Ulak to close"
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the Ulak window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close Ulak fully and try again."
    ...
    Publish-UiProgress "Preparing Ulak files"
    $shim = Join-Path $InstallRoot "venv\Scripts\ulak.exe"
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the Ulak install open (venv\Scripts\ulak.exe locked after 20s). Nothing was changed. Close other Ulak windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # --force skips only the ulak.exe shim guard, which step 2 just PROVED
    ...
    #
    # DRIVE THE UPDATE THROUGH venv\Scripts\python.exe, NOT venv\Scripts\ulak.exe.
    # `uv pip install -e .` has to replace the console-script shims, so
    # _quarantine_running_ulak_exe must first rename the running ulak.exe
    # out of the way. On Windows that rename fails whenever ANY child process
    # spawned from that ulak.exe is still alive: a child inherits a handle on
    # the parent image, and the resulting sharing violation is indistinguishable
    # from a user leaving a second Ulak window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at a Ulak.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m ulak_cli.main update` puts the
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the Ulak installer or `ulak doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    $updateArgs = @("-m", "ulak_cli.main", "update", "--yes", "--gateway", "--force", "--branch", $Branch)
    ...
    # collides with the "close all Ulak windows" sentinel.
    try {
        $updateHelp = & $pythonExe -m ulak_cli.main update --help 2>$null | Out-String
        if ($updateHelp -match "--keep-stash") {
            $updateArgs += "--keep-stash"
        } else {
            Write-HandoffLog "installed ulak predates --keep-stash; running without it"
        }
    } catch {
    ...
    $res = Invoke-UlakStep $pythonExe $updateArgs "update"
    Write-HandoffLog "ulak update exit code: $($res.Code)"

    $retryPolicyPath = Join-Path $PSScriptRoot "retry-policy.ps1"
    if (Test-Path -LiteralPath $retryPolicyPath) {
        . $retryPolicyPath
        $shouldRetry = Test-UlakUpdateShouldRetry -ExitCode $res.Code -InstallRoot $InstallRoot
    } else {
    ...
    # `ulak update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        Write-HandoffLog "ulak update reported a desktop build failure (non-fatal there, fatal here); retrying build"
        Publish-UiProgress "Rebuilding Desktop"
        $rebuild = Invoke-UlakStep $pythonExe @("-m", "ulak_cli.main", "desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `ulak desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `ulak debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    #   3. only then the terminal UI state — done means "Ulak is back",
    ...
    if (-not $script:TreeSafeToFinalize) {
        ...
        $finalCode = 7
        $finalMsg = "Update recovery could not stop every updater process. Ulak was not restarted to avoid overlapping the active install. Wait for it to finish or restart Windows, then reopen Ulak."
        ...
        if ($finalCode -ne 0) {
            ...
            Publish-UiProgress "Opening Ulak"
            ...
            if (-not $cameBack -and $RelaunchExe) {
                ...
                $finalMsg = "Update complete. Reopen Ulak to finish (it could not restart itself)."
