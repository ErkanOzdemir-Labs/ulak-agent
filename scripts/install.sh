...
# Python-driven tool session (e.g. Ulak terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:ErkanOzdemir-Labs/ulak-agent.git"
REPO_URL_HTTPS="https://github.com/ErkanOzdemir-Labs/ulak-agent.git"
ULAK_HOME="${ULAK_HOME:-$HOME/.ulak}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${ULAK_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$ULAK_INSTALL_DIR"
...
#   code at /usr/local/lib/ulak-agent, command at /usr/local/bin/ulak,
#   data still at /root/.ulak (ULAK_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$ULAK_HOME/.no-bundled-skills so future"
            echo "                   'ulak update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Ulak.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.ulak/ulak-agent"
            echo "                   default (root, Linux): /usr/local/lib/ulak-agent"
            echo "  --ulak-home PATH  Data directory (default: ~/.ulak, or \$ULAK_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Ulak installs the code under"
            echo "  /usr/local/lib/ulak-agent and links the command into"
            echo "  /usr/local/bin/ulak (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$ULAK_HOME"
            echo "  (default /root/.ulak).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$ULAK_HOME/ulak-agent are preserved in-place."
...
print_banner() {
    ...
    echo "│             ➶ Ulak Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Ulak Agent.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `ulak update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Ulak-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Ulak Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install ulak command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `ulak` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $ULAK_HOME/ulak-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $ULAK_HOME/ulak-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/ulak-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $ULAK_HOME/ulak-agent — then preserve it)
#
# Always no-op when the user set --dir or $ULAK_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in ULAK_HOME.
    if is_termux; then
        INSTALL_DIR="$ULAK_HOME/ulak-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$ULAK_HOME/ulak-agent/.git" ]; then
            INSTALL_DIR="$ULAK_HOME/ulak-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/ulak-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/ulak-agent"
        ...
        # /usr/local/bin/ulak wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/ulak"
        log_info "  Data:    $ULAK_HOME (unchanged)"
...
# Point a Ulak-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $ULAK_HOME/node/bin — which is NOT on
...
# Ulak's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Ulak-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$ULAK_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$ULAK_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$ULAK_HOME/node/etc/npmrc"
}

get_ulak_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/ulak" ]; then
        echo "$link_dir/ulak"
    else
        echo "ulak"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://ulak-agent.github.com/ErkanOzdemir-Labs/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Ulak owns its own uv at $ULAK_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (ulak_cli/managed_uv.py) looks in the same
    # place, so install.sh and `ulak update` stay in sync.
    local _managed_uv="$ULAK_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $ULAK_HOME/bin ..."
    mkdir -p "$ULAK_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/ulak-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/ulak-uv-installer.$$.sh")"
    ...
    # directly into $ULAK_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$ULAK_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
check_python() {
    if [ "$DISTRO" = "termux" ]; then
        ...
        # Ulak currently declares requires-python >=3.11,<3.14.  Termux can
        ...
        log_error "Termux Python $PYTHON_FOUND_VERSION is not supported; Ulak requires Python >=3.11,<3.14"
...
# replaced with the Ulak-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Ulak-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Ulak-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Ulak-managed Node from a previous run over a too-old system one.
    if [ -x "$ULAK_HOME/node/bin/node" ] && [ -x "$ULAK_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$ULAK_HOME/node/bin/node" --version)"; then
        export PATH="$ULAK_HOME/node/bin:$PATH"
        log_success "Node.js $("$ULAK_HOME/node/bin/node" --version) found (Ulak-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Ulak-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is unsupported (Ulak requires Node 22.22+, 24.11+, or 26+) — installing Ulak-managed Node $NODE_VERSION..."
...
# Download and adopt one Node release line (e.g. 26) into ~/.ulak/node/.
#
...
install_node_line() {
    ...
    log_info "Extracting to ~/.ulak/node/..."
    ...
    # Place into ~/.ulak/node/ and symlink binaries into the same bin dir
    # the ulak command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$ULAK_HOME/node"
    mkdir -p "$ULAK_HOME"
    mv "$extracted_dir" "$ULAK_HOME/node"
    rm -rf "$tmp_dir"
    ...
    ln -sf "$ULAK_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$ULAK_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$ULAK_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    if ! installed_ver=$("$ULAK_HOME/node/bin/node" --version 2>&1); then
    ...
    log_success "Node.js $installed_ver installed to ~/.ulak/node/"
    HAS_NODE=true
    return 0
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Ulak install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Ulak Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `ulak update` path
                # (#4735).
                ...
                local stash_name
                stash_name="ulak-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``ulak update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Ulak behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up ulak command..."

    if [ "$USE_VENV" = true ]; then
        ULAK_BIN="$INSTALL_DIR/venv/bin/python"
        ULAK_ENTRYPOINT="$INSTALL_DIR/ulak"
    else
        ULAK_BIN="$(which ulak 2>/dev/null || echo "")"
        if [ -z "$ULAK_BIN" ]; then
            log_warn "ulak not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$ULAK_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$ULAK_ENTRYPOINT" ]; }; then
        log_warn "Ulak launcher prerequisites not found"
    ...
    # Create a user-facing shim for the ulak command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $ULAK_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$ULAK_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/ulak"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/ulak" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ULAK_BIN" "$ULAK_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/ulak" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/ulak"
    log_success "Installed ulak launcher → $command_link_display_dir/ulak"

    # Also expose `ulak-agent`. The `ulak-agent` console script declared in
    ...
    rm -f "$command_link_dir/ulak-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/ulak-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ULAK_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/ulak-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ULAK_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/ulak-agent"
    log_success "Installed ulak-agent launcher → $command_link_display_dir/ulak-agent"

    # Also expose `ulak-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `ulak-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Ulak as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/ulak-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/ulak-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ULAK_BIN" "$ULAK_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/ulak-acp" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/ulak-acp"
    log_success "Installed ulak-acp launcher → $command_link_display_dir/ulak-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "ulak command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v ulak' \
                >/dev/null 2>&1; then
        ...
        log_info "ulak not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Ulak Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Ulak Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Ulak Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so ulak works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.ulak directory structure (config at top level, code in subdir)
    mkdir -p "$ULAK_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.ulak/.env (top level, easy to find)
    if [ ! -f "$ULAK_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$ULAK_HOME/.env"
            log_success "Created ~/.ulak/.env from template"
        else
            touch "$ULAK_HOME/.env"
            log_success "Created ~/.ulak/.env"
        fi
    else
        log_info "~/.ulak/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$ULAK_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.ulak/config.yaml (top level, easy to find)
    if [ ! -f "$ULAK_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$ULAK_HOME/config.yaml"
            log_success "Created ~/.ulak/config.yaml from template"
        fi
    else
        log_info "~/.ulak/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in ulak_cli/default_soul.py — the
    ...
    if [ ! -f "$ULAK_HOME/SOUL.md" ]; then
        cat > "$ULAK_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.ulak/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.ulak/"

    # Seed bundled skills into ~/.ulak/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `ulak update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'ulak update'." \
            > "$ULAK_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $ULAK_HOME/.no-bundled-skills"
        log_info "  Future 'ulak update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.ulak/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.ulak/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$ULAK_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$ULAK_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.ulak/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Ulak Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Ulak will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Ulak Agent browser tools — explicit browser override."
...
# install — the same closure `ulak update` installs
# (ulak_cli/main.py::_update_node_dependencies). Prebuilt/partial checkouts
...
install_node_deps() {
    ...
    # Keep the checkout clean so `ulak update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `ulak tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if [ -x "$ULAK_HOME/bin/browser-use" ]; then
        log_success "Browser Use CLI already installed"
        return 0
    fi

    log_info "Installing Browser Use CLI (default browser backend)..."
    # UV_TOOL_BIN_DIR keeps the binary inside Ulak' managed bin dir, where
    # the browser tool resolves it — no reliance on the user's PATH.
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$ULAK_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'ulak tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `ulak tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `ulak computer-use install` on their own).
    ...
    # Same upstream installer `ulak computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the Ulak install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Ulak'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'ulak tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: ulak computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'ulak setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m ulak_cli.main setup < /dev/tty
    else
        python -m ulak_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$ULAK_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Ulak to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$ULAK_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'ulak whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                ULAK_CMD="$(get_ulak_command_path)"
                $ULAK_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'ulak whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'ulak gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $ULAK_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $ULAK_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: ulak gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: ulak gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $ULAK_CMD gateway > "$ULAK_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.ulak/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: ulak gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: ulak gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.ulak-bootstrap-complete, which tells the Ulak
    ...
    local marker_path="$INSTALL_DIR/.ulak-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $ULAK_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $ULAK_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $ULAK_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}ulak${NC}              Start chatting"
    echo -e "   ${GREEN}ulak setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}ulak config${NC}       View/edit configuration"
    echo -e "   ${GREEN}ulak config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}ulak gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}ulak update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'ulak' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'ulak' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'ulak' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$ULAK_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$ULAK_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$ULAK_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$ULAK_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`Ulak` rename
...
# _purge_electron_build_cache() used by `ulak desktop`; install.sh was the only
...
install_desktop() {
    ...
    # supported Node lines and prepends the Ulak-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `ulak desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Ulak" ]; then
            app="$desktop_dir/release/linux-unpacked/Ulak"
        elif [ -x "$desktop_dir/release/linux-unpacked/ulak" ]; then
            app="$desktop_dir/release/linux-unpacked/ulak"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Ulak.app" \
            "$desktop_dir/release/mac/Ulak.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `ulak desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading ULAK_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if ULAK_HOME="$ULAK_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from ulak_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `ulak update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Ulak-managed Node
            # provisioned during prerequisites/node-deps (at $ULAK_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $ULAK_HOME.
    # $ULAK_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'ulak update' on this host install.
