...
VERSION_FILE = REPO_ROOT / "ulak_cli" / "__init__.py"
...
LEGACY_AUTHOR_MAP = {
    ...
    "pi@ulak.local": "Elektrofussel",  # PR #61675 salvage (honcho: defaultHost + private-range local URL detection)
    ...
    "kavi@local.ulak": "kavioavio",  # Issue #46544 / PR #47705 evolution (smart DENY exact-operation owner override)
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level ULAK_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (ulak journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "ulak.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.ulak/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "moonsong@erkanozdemir-labs.local": "Tranquil-Flow",  # PR #52623 salvage (auxiliary Anthropic base_url host validation; #52608)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows ulak-shim resolution + prefer --update on recovery; #52378)
    ...
    "jake@github.com/ErkanOzdemir-Labs": "simpolism",
    ...
    "teknium@github.com/ErkanOzdemir-Labs": "teknium1",
    ...
    "274096618+ulak-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "shannon@github.com/ErkanOzdemir-Labs": "shannonsands",
    ...
    "ulak@github.com/ErkanOzdemir-Labs": "ErkanOzdemir-Labs",
    ...
    "ben@github.com/ErkanOzdemir-Labs": "benbarclay",
    ...
    "emozilla@github.com/ErkanOzdemir-Labs": "emozilla",
    ...
    "jonny@github.com/ErkanOzdemir-Labs": "yoniebans",
    "jake@github.com/ErkanOzdemir-Labs": "simpolism",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on ulak mcp add)
    ...
    "ulak@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "github@nadyaulak.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (ULAK_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (ULAK_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: ulak-eval community link)
    ...
    "ulakagent26@gmail.com": "ulakagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+ulakagent26@users.noreply.github.com": "ulakagent26",
    ...
    "clement@github.com/ErkanOzdemir-Labs": "lemassykoi",  # PR #27042 (model-switch probe keyless providers)
    ...
    "vigo@ulak": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Ulak runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level ULAK_HOME state files)
    ...
    "annguyen@github.com/ErkanOzdemir-Labs": "annguyenNous",  # PR #52844 co-author
    ...
    "zhchl@ulak-agent.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Ulak
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "ulak@github.com/ErkanOzdemir-Labs"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/ErkanOzdemir-Labs/ulak-agent",
    ...
    lines.append(f"# Ulak Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Ulak Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Ulak Agent Release Tool")
    ...
    print(f"{'='*60}")
    print("  Ulak Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Ulak Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Ulak Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Ulak Agent v{new_version} ({calver_date})' "
