# Behavioral test for install.ps1's ulak launcher staging (PR #92092,
...
#
# The test lifts the real Install-UlakCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $UlakHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-UlakCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-UlakCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'ulak-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'ulak-agent'
    ...
    $missingThrew = $false
    try {
        Install-UlakCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing ulak.exe fails the launcher stage'
    ...
    $ulakV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'ulak.exe'), $ulakV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-UlakCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $ulakV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'ulak.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'ulak-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'ulak.exe'), $ulakV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'ulak-acp.exe'), $acp)
    Install-UlakCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $ulakV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'ulak.exe'))) `
        'installer refreshes an existing Ulak launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'ulak-acp.exe'))) `
    ...
    Install-UlakCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'ulak.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'ulak.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'ulak.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'ulak.exe')) -and $cmdBody.Contains('%*')) `
