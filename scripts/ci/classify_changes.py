...
# tree generates llms.txt — the index every LLM (Ulak included, via the
# ulak-agent skill) reads to learn what Ulak can do — and
...
_MCP_CATALOG_FILES = {"ulak_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Ulak-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``ulak serve`` backend, the Docker image)
