"""Run a real Ulak CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_ulak_executable(ulak_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "ulak",
        Path(".venv") / "Scripts" / "ulak.exe",
    ):
        candidate = ulak_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("ulak")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Ulak executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Ulak source checkout containing .venv/bin/ulak",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated ULAK_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["ulak.client.active"] != [
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['ulak.client.active']}"
        )
    [model] = by_name["ulak.model_route.count"]
    expected_model = {
        "name": "ulak.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['ulak.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["ulak.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['ulak.task_run.started']}"
        )
    [terminal] = by_name["ulak.task_run.finished"]
    ...
    [tool] = by_name["ulak.tool_call.count"]
    ...
    lifecycle = by_name["ulak.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["ulak.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Ulak development environment requires jsonschema"
        ) from exc
    ...
    if metrics["ulak.client.active"] != [
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['ulak.client.active']}"
        )
    [model] = metrics["ulak.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['ulak.model_route.count']}"
        )
    [terminal] = metrics["ulak.task_run.finished"]
    ...
    [tool] = metrics["ulak.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["ulak.skill.lifecycle.count"]
    ...
    loads = metrics["ulak.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    ulak_repo = args.ulak_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    ulak = _resolve_ulak_executable(ulak_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="ulak-relay-shared-metrics-"))
    home = root / "ulak-home"
    ...
    try:
        ...
        env = os.environ.copy()
        env["ULAK_HOME"] = str(home)
        python_paths = [str(ulak_repo)]
    ...
    (root / "ulak.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "ulak.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Ulak exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Ulak model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Ulak did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Ulak did not print the mock model response")
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "ulak.shared_metrics.v2.schema.json",
    )

    print("Ulak -> NeMo Relay shared-metrics smoke test passed")
