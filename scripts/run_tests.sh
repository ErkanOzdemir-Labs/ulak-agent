#!/usr/bin/env bash
# Canonical test runner for ulak-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.ulak/...)
#
# Usage:
...
# (ULAK_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.ulak/ulak-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.ulak/ulak-agent/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${ULAK_PYTHON:-}" ] && [ -x "$ULAK_PYTHON" ] \
    && "$ULAK_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: ULAK_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `ulak` binary rather
  # than the devShell hook.
  PYTHON="$ULAK_PYTHON"
  echo "▶ no local venv — using Nix dev venv via ULAK_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and ULAK_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.ulak/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.ulak"
...
#
#   * ULAK_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * ULAK_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     ULAK_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# ULAK_RUN_SLOW_PET_TESTS / ULAK_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no ULAK_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in ULAK_TEST_IMAGE ULAK_TEST_WORKERS ULAK_TEST_PATHS \
  ULAK_TEST_FILE_TIMEOUT ULAK_TEST_FILE_RETRIES ULAK_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${ULAK_RUN_SLOW_PET_TESTS:+ULAK_RUN_SLOW_PET_TESTS="$ULAK_RUN_SLOW_PET_TESTS"} \
  ${ULAK_E2E_BROWSER:+ULAK_E2E_BROWSER="$ULAK_E2E_BROWSER"} \
