...
unprivileged `ulak` user, giving the layout most people have —
$ULAK_HOME/ulak-agent plus a ~/.local/bin launcher. Pass --root for the FHS
...
Environment:
  ULAK_DEV_SANDBOX_DIR    Sandbox directory name, relative to the repo root
                            (default: .ulak-sandbox).

Examples:
  # create a sandbox, install this branch as `main`, and then drop to a shell,
  # skipping `ulak setup` & the browser tools for speed.
...
INSTALL_REF=""
UPSTREAM_URL="${ULAK_DEV_SANDBOX_UPSTREAM:-https://github.com/ErkanOzdemir-Labs/ulak-agent.git}"
...
GIT_ROOT="${ULAK_SANDBOX_SOURCE_ROOT:-$(git rev-parse --show-toplevel)}"
...
SANDBOX_DIR_NAME="${ULAK_DEV_SANDBOX_DIR:-.ulak-sandbox}"
...
if [ "$PERSISTENT" = true ]; then
  ...
  SANDBOX_ROOT="$(mktemp -d -t ulak-sandbox.XXXXXX)"
...
if [ -n "$INSTALL_REF" ]; then
  ...
  UPSTREAM_REPO="$(mktemp -d -t ulak-sandbox-upstream.XXXXXX)"
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  mkdir -p "$SANDBOX_ROOT/root/http/ulak-agent.github.com/ErkanOzdemir-Labs"
  if [ -n "$INSTALL_REF" ]; then
    git -C "$UPSTREAM_REPO" show "$UPSTREAM_COMMIT:scripts/install.sh" \
      > "$SANDBOX_ROOT/root/http/ulak-agent.github.com/ErkanOzdemir-Labs/install.sh"
  else
    cp -a "$INSTALLER_PATH" "$SANDBOX_ROOT/root/http/ulak-agent.github.com/ErkanOzdemir-Labs/install.sh"
  fi
  set -- bash -c '
    set +e
    curl -fsSL https://ulak-agent.github.com/ErkanOzdemir-Labs/install.sh | bash -s -- "$@"
    install_status=$?
    if [ "$install_status" -eq 0 ] && [ -f /work/promote-main ]; then
      next_main=$(cat /work/promote-main)
      if git --git-dir=/work/repos/ulak-agent.git update-ref refs/heads/main "$next_main"; then
...
# A user-level install writes the `ulak` launcher to ~/.local/bin and the
# checkout to $ULAK_HOME; both live under the sandbox HOME, which is bound
...
SOURCE_REF="$COMMIT"
SNAPSHOT_REPO=""
FAKE_REPO="$SANDBOX_ROOT/root/repos/ulak-agent.git"
git -C "$SANDBOX_ROOT/root/repos" init --bare -q ulak-agent.git
if [ -n "$INSTALL_REF" ]; then
...
if [ -n "$(git -C "$GIT_ROOT" status --porcelain)" ]; then
  ...
  SNAPSHOT_REPO="$(mktemp -d -t ulak-sandbox-snapshot.XXXXXX)"
  git -C "$SNAPSHOT_REPO" init -q
  git -C "$SNAPSHOT_REPO" fetch -q "$GIT_ROOT" "$COMMIT"
  git -C "$SNAPSHOT_REPO" config user.name 'Ulak sandbox'
...
if [ -n "$INSTALL_REF" ]; then
  git --git-dir="$FAKE_REPO" fetch -q --force "$SOURCE_REPO" \
    "$SOURCE_REF:refs/ulak-sandbox/next"
