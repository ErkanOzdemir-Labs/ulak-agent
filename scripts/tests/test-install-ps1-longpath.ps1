...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[ulak] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[ulak`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[ulak`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        UlakHome = $(if ($paths) { $paths.ulak_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\ulak\ulak-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-ulak'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-ulak') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
