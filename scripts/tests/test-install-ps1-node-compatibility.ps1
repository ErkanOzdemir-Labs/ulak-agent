...
# changing PATH, installing software, or touching the user's Ulak home.

$repoRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path))
$installScript = Join-Path $repoRoot 'scripts\install.ps1'
$testRoot = Join-Path $env:TEMP ("ulak-node-compatibility-test-" + [Guid]::NewGuid().ToString('N'))
$UlakHome = Join-Path $testRoot 'home'
$InstallDir = Join-Path $testRoot 'missing-checkout'
. $installScript -UlakHome $UlakHome -InstallDir $InstallDir
