...
# 5. Symlinks the 'ulak' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Ulak Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/ulak-uv-install.$$.log")"
...
if is_termux; then
    # Ulak currently declares requires-python >=3.11,<3.14. Termux can expose
    ...
    if [ -z "${PYTHON_PATH:-}" ]; then
        if command -v python >/dev/null 2>&1; then
            ...
            echo -e "${RED}✗${NC} Termux Python $PYTHON_FOUND_VERSION is not supported; Ulak requires Python >=3.11,<3.14"
...
# ============================================================================
# PATH setup — symlink ulak into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up ulak command..."

ULAK_BIN="$SCRIPT_DIR/venv/bin/ulak"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ULAK_BIN" "$COMMAND_LINK_DIR/ulak"
echo -e "${GREEN}✓${NC} Symlinked ulak → $COMMAND_LINK_DISPLAY_DIR/ulak"
...
# ============================================================================
# Seed bundled skills into ~/.ulak/skills/
# ============================================================================

ULAK_SKILLS_DIR="${ULAK_HOME:-$HOME/.ulak}/skills"
mkdir -p "$ULAK_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.ulak/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ULAK_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  ulak status        # Check configuration"
if is_termux; then
    echo "  ulak gateway       # Run gateway in foreground"
else
    echo "  ulak gateway install # Install gateway service (messaging + cron)"
fi
echo "  ulak cron list     # View scheduled jobs"
echo "  ulak doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m ulak_cli.main setup
fi
