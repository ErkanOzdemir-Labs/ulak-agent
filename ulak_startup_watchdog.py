...
timeout. Config is env-only (``ULAK_STARTUP_WATCHDOG=0``,
``ULAK_STARTUP_WATCHDOG_TIMEOUT_S``) because config.yaml parsing is itself in scope.
...
ENV_STARTUP_WATCHDOG = "ULAK_STARTUP_WATCHDOG"
ENV_STARTUP_WATCHDOG_TIMEOUT_S = "ULAK_STARTUP_WATCHDOG_TIMEOUT_S"
...
# Module singleton: the arm sites (ulak_cli.main / ulak_cli.gateway /
...
def _process_ulak_home() -> Path:
    """ULAK_HOME for diagnostic files — stdlib-only replica of the ulak_constants default."""
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<ULAK_HOME>/logs/gateway-startup-watchdog.log``."""
    base = home if home is not None else _process_ulak_home()
    return base.joinpath(*_DUMP_RELATIVE)


def startup_watchdog_disabled() -> bool:
    """True when ``ULAK_STARTUP_WATCHDOG`` opts out explicitly."""
...
def arm_startup_watchdog(
    ...
    *,
    ...
    ``ULAK_STARTUP_WATCHDOG=0`` or when the thread could not be started.
    """
    global _handle
    try:
