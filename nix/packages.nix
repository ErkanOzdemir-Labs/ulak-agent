# nix/packages.nix — Ulak Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./ulak-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.ulakTui;
        web = full.ulakWeb;
        desktop = full.ulakDesktop;

        update-npm-lockfile = full.ulakNpmLib.updateNpmLockfile;
      };
    };
}
