...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Ulak derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            ulak-agent = prev.ulak-agent.overrideAttrs (_old: {
              ULAK_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $ULAK_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$ULAK_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        ulak-agent = prev.ulak-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "ulak-agent-env" {
    ulak-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "ulak-agent-editable-env" {
    ulak-agent = dependency-groups;
  };
}
