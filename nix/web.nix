# nix/web.nix — Ulak Web Dashboard (Vite/React) frontend build
{ ulakNpmLib, ... }:
ulakNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @ulak/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../ulak_cli/web_dist for the
