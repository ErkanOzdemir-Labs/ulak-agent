# nix/homeManagerModules.nix — the Home Manager module for ulak-agent
#
# This module is the user-level equivalent of nixosModules.default. Ulak is
...
#
# `services.ulak-agent` is the same option set on both modules. All of the
...
#   changed   addToSystemPackages      -> programs.ulak-agent.enable and
#                                        home.sessionVariables
#   added     programs.ulak-agent    the CLI and the desktop application,
#                                      because Home Manager separates an
#                                      installation from a daemon
#   changed   stateDir (+ "/.ulak")  -> ulakHome, set directly
#
# To use the module:
#   imports = [ ulak-agent.homeManagerModules.default ];
#   programs.ulak-agent = {
#     enable = true;          # the ulak CLI on your PATH
...
{ inputs, ... }:
{
  flake.homeManagerModules.default =
    ...
    let
      ...
      cfgPrograms = config.programs.ulak-agent;
      ...
      # not reach it, and the application would open ~/.ulak while the
      # services use ulakHome. Thus the launcher carries the value itself.
      #
      # ULAK_MANAGED rides along only when the services are enabled. That
      ...
      desktopEnvironment = {
        ULAK_HOME = cfg.ulakHome;
      }
      // lib.optionalAttrs cfg.enable {
        inherit (processEnvironment) ULAK_MANAGED;
      }
      // lib.optionalAttrs desktopUsesService {
        ULAK_DESKTOP_REMOTE_URL = "http://${cfg.backend.host}:${toString cfg.backend.port}";
      };
      ...
      # token the desktop resolver throws ("ULAK_DESKTOP_REMOTE_URL is set
      # but ULAK_DESKTOP_REMOTE_TOKEN is not"), so the two variables travel
      ...
      desktopRun = lib.optional desktopUsesService ''
        if [ -r ${lib.escapeShellArg cfg.backend.sessionTokenFile} ]; then
          ULAK_DESKTOP_REMOTE_TOKEN="$(tr -d '\r\n' < ${lib.escapeShellArg cfg.backend.sessionTokenFile})"
          export ULAK_DESKTOP_REMOTE_TOKEN
        else
          echo "ulak-desktop: cannot read the session token at ${cfg.backend.sessionTokenFile}." >&2
          echo "ulak-desktop: the application starts its own backend instead of the one of the service." >&2
        fi
      '';
    ...
    in
    {
      # ── programs.ulak-agent — the installation ───────────────────────
      # Home Manager separates "install this application for me" from "run
      # this daemon". Ulak needs both, and a person can want one without
      # the other: an application with no gateway, or a headless gateway on
      # a machine with no display.
      #
      # `services.ulak-agent` stays the authority for the state and the
      # configuration. This module reads ulakHome and the backend address
      # from it, and never the reverse.
      options.programs.ulak-agent = {
        enable = lib.mkEnableOption ''
          the Ulak Agent command line application.

          This adds `ulak` to home.packages, and exports ULAK_HOME with
          home.sessionVariables. An interactive shell then uses the same
          state as `services.ulak-agent`
        '';

        package = lib.mkOption {
          type = lib.types.package;
          default = effectivePackage;
          defaultText = lib.literalExpression "config.services.ulak-agent.package";
          description = ''
            The ulak-agent package to install.

            The default follows `services.ulak-agent.package`, and applies
        ...
        desktop = {
          enable = lib.mkEnableOption ''
            the Ulak Desktop application (Electron).

            This adds `ulak-desktop` to home.packages, with an XDG
            launcher entry on Linux. The launcher starts the same Ulak
            runtime that `package` gives, and reads the ULAK_HOME of
            `services.ulak-agent`. Thus the application, the interactive
            shell and the services share one state directory.

            The Electron application carries its own Ulak runtime with
            the usual distribution. This module gives it the Nix package
            instead, with ULAK_DESKTOP_ULAK. It installs no second copy
            of Ulak, and it downloads nothing on the first start
          '';

          package = lib.mkOption {
            type = lib.types.package;
            default = cfgPrograms.package.ulakDesktop;
            defaultText = lib.literalExpression "config.programs.ulak-agent.package.ulakDesktop";
            description = ''
              The ulak-desktop package to use.

              The default follows `package`, and thus also
              `services.ulak-agent.extraPythonPackages` and
              `extraDependencyGroups`, because the desktop application is a
              passthru of the agent package. A package that you set here
              carries its own Ulak runtime, and this module cannot make
      ...
      options.services.ulak-agent =
        ...
        // {
          ulakHome = lib.mkOption {
            type = lib.types.str;
            default = "${config.home.homeDirectory}/.ulak";
            defaultText = lib.literalExpression ''"''${config.home.homeDirectory}/.ulak"'';
            description = ''
              The value of ULAK_HOME. This state directory holds
              config.yaml, .env, auth.json, the sessions, the skills, the
              memory and the cron jobs.

              The NixOS module takes a `stateDir` and adds `/.ulak` to it.
              This module sets ULAK_HOME directly. Thus an existing
              ~/.ulak continues to work, and you can give the directory any
              name.
            '';
            example = "/home/alice/.ulak-work";
          };

          # `installPackage` moved to `programs.ulak-agent.enable`. The
          ...
          # quiet removal gives them a machine with no `ulak` and no
          ...
          installPackage = lib.mkOption {
            ...
            visible = false;
            description = ''
              Removed. Use `programs.ulak-agent.enable` instead.
            '';
          };
      ...
      config = lib.mkMerge [
        ...
        (lib.mkIf cfgPrograms.enable {
          ...
          home.sessionVariables.ULAK_HOME = cfg.ulakHome;
        })

        # A launcher from the desktop menu reads no shell profile, so the
        # ULAK_HOME that `programs.enable` exports does not reach it. Home
        # Manager writes only systemd.user.sessionVariables into
        # environment.d, and this module does not put ULAK_HOME there,
          ...
          assertions = [
            {
              ...
              # with no `ulak` on the PATH and no message.
        ...
        (lib.mkIf cfg.enable (
          lib.mkMerge [
                ...
                ++ [
                  {
                    ...
                    message = "services.ulak-agent.backend.waitFor = \"interface\" works on Linux only. Use \"hostname\" on Darwin.";
                  }
                ];
            }
            ...
            # ── Activation: directories, config, secrets, documents ────────
            {
              ...
              home.activation.ulakAgentSetup =
                lib.hm.dag.entryAfter
                  ...
                  (
                    common.mkStateScript {
                      inherit pkgs cfg;
                      inherit (cfg) ulakHome workingDirectory;
                      run = "$DRY_RUN_CMD ";
            ...
            (lib.mkIf (isLinux && cfg.gateway.enable) {
              systemd.user.services.ulak-agent = mkUnit {
            ...
            (lib.mkIf (isLinux && cfg.backend.mode != "none") {
              systemd.user.services.ulak-backend = mkUnit {
            ...
            (lib.mkIf (isDarwin && cfg.gateway.enable) {
              launchd.agents.ulak-agent = mkAgent {
                argv = common.gatewayArgv cfg;
                logName = "ulak-agent";
              };
            })
            ...
            (lib.mkIf (isDarwin && cfg.backend.mode != "none") {
              launchd.agents.ulak-backend = mkAgent {
                argv = common.backendArgv { inherit pkgs cfg; };
                logName = "ulak-backend";
              };
            })
          ]
        ))
