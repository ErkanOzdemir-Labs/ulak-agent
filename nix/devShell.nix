...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      ulakNpmLib = self'.packages.default.passthru.ulakNpmLib;
      ...
      ulakAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "ulak" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../ulak} $out/bin/ulak
          '')
        ...
        shellHook = ''
          ${ulakAgentDevShellHook}
          ${ulakNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export ULAK_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "Ulak Agent dev shell in $ULAK_PYTHON_SRC_ROOT"
          echo "Ready. Run 'ulak' or 'sandbox ulak' to start."
        '';
      };
    };
}
