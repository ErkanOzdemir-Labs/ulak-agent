# nix/nixosModules.nix — the NixOS module for ulak-agent
#
...
#
# Container mode: ulak runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $ULAK_HOME/.env
# and read by ulak at startup — no container recreation needed for env changes.
#
# Tool resolution: the ulak wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      cfg = config.services.ulak-agent;
      common = import ./moduleCommon.nix { inherit lib; };

      effectivePackage = common.effectivePackage cfg;
      ulak-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

      ulakHome = "${cfg.stateDir}/.ulak";
      ...
      # ULAK_HOME via addToSystemPackages, so they can save settings through the
      ...
      # ulak user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "ulak-container-entrypoint" ''
        set -eu

        ULAK_UID="''${ULAK_UID:?ULAK_UID must be set}"
        ULAK_GID="''${ULAK_GID:?ULAK_GID must be set}"

        # ── Group: ensure a group with GID=$ULAK_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$ULAK_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="ulak"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$ULAK_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$ULAK_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$ULAK_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$ULAK_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="ulak"
          TARGET_HOME="/home/ulak"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$ULAK_UID" -g "$ULAK_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$ULAK_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$ULAK_UID:$ULAK_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure ULAK_HOME is owned by the target user.
        ...
        if [ -n "''${ULAK_HOME:-}" ] && [ -d "$ULAK_HOME" ]; then
          find "$ULAK_HOME" \! -user "$ULAK_UID" -exec chown "$ULAK_UID:$ULAK_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/ulak-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/ulak-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/ulak ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/ulak
          chmod 0440 /etc/sudoers.d/ulak
        fi
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$ULAK_UID" --regid="$ULAK_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $ULAK_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "ulak-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        ulak_bin=${containerDataDir}/current-package/bin/ulak
      '';

      # Default: /var/lib/ulak/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the ulak group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit ulakHome; };

      unitPath = common.processPath { inherit pkgs cfg; };

    in
    {
      options.services.ulak-agent =
        common.sharedOptions {
          defaultPackage = ulak-agent;
          defaultPackageText = lib.literalExpression "ulak-agent.packages.\${system}.default";
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              type = types.str;
              default = "/var/lib/ulak";
              description = "State directory. Contains .ulak/ subdir (ULAK_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the ulak CLI to environment.systemPackages and export
                ULAK_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.ulak symlink to the service
                  stateDir. These users are automatically added to the ulak group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          (lib.mkIf (cfg.mcpServers != { }) {
            services.ulak-agent.settings.mcp_servers = common.mcpServersToConfig cfg.mcpServers;
          })
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the ulak CLI to system PATH and export ULAK_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.ulak/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.ULAK_HOME = ulakHome;
          })
          ...
          # ── Assertions ─────────────────────────────────────────────────────
          {
            assertions =
              common.pluginNameAssertions {
                inherit cfg;
                optionPath = "services.ulak-agent";
              }
              ++ common.workspaceFilesAssertions {
                inherit cfg;
                opt = options.services.ulak-agent.workingDirectory;
                optionPath = "services.ulak-agent";
              }
              ...
              ++ [
                {
                  ...
                  message = "services.ulak-agent: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the ulak user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.ulak-agent: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed ulak
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure ulak is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${ulakHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${ulakHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."ulak-agent-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${ulakHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${ulakHome} ${cfg.workingDirectory}
                  ...
                  find ${ulakHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${ulakHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${ulakHome}/$_subdir"
                    chmod 2770 "${ulakHome}/$_subdir"
                    find "${ulakHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg ulakHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${ulakHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${ulakHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${ulakHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.ulak";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${ulakHome}" ]; then
                                rm -f "${symlinkPath}"
                                echo "ulak-agent: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.ulak -> stateDir/.ulak for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.ulak-agent = {
              description = "Ulak Agent Gateway";
              wantedBy = [ "multi-user.target" ];
              after = [ "network-online.target" ];
              wants = [ "network-online.target" ];

              # cfg.environment and cfg.environmentFiles are written to
              # $ULAK_HOME/.env by the activation script. load_ulak_dotenv()
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.ulak-agent = {
              description = "Ulak Agent Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  ULAK_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  ULAK_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env ULAK_UID="$ULAK_UID" \
                    --env ULAK_GID="$ULAK_GID" \
                    --env ULAK_HOME=${containerDataDir}/.ulak \
                    --env ULAK_MANAGED=true \
                    --env HOME=${containerHomeDir} \
                    ${lib.concatStringsSep " " cfg.container.extraOptions} \
                    ${cfg.container.image} \
                    ${containerDataDir}/current-package/bin/ulak gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
