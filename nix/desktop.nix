# nix/desktop.nix — Ulak Desktop (Electron) app build + wrapper
#
# `ulakAgent` is the fully-built `.#default` package — it ships the
# `ulak` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `ULAK_DESKTOP_ULAK` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Ulak CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # ULAK_HOME and ULAK_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "ulak-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "ulak-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = ulakNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "ulak-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe ulakNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/ulak-desktop $out/bin
    cp -r ${renderer}/* $out/share/ulak-desktop/
    ...
    substituteInPlace $out/share/ulak-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/ulak-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # ULAK_DESKTOP_ULAK to the absolute path of the nix-built `ulak`
    # binary so the desktop's resolver step 4 ("existing Ulak CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/ulak-desktop \
      --add-flags "$out/share/ulak-desktop" \
      --set ULAK_DESKTOP_ULAK "${lib.getExe ulakAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/ulak.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../ulak_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/ulak-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/ulak.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/ulak.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Ulak Agent";
    homepage = "https://github.com/ErkanOzdemir-Labs/ulak-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "ulak-desktop";
  };
}
