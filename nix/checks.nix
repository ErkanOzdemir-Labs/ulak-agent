...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      ulak-agent = self'.packages.default;
      ulakVenv = ulak-agent.ulakVenv;
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.ulak-agent = programs;
              services.ulak-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.ulak-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.ulak-agent);
      ...
      configKeys = pkgs.runCommand "ulak-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${ulakVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "ulak-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "ulak-build-package" { } ''
          echo "PASS: package built at ${ulak-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "ulak-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.ULAK_TEST = "1";
                environmentFiles = [ "/run/secrets/ulak-env" ];
                ulakHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "ulak" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "ulak" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.ulakAgentSetup.data;

            failures =
              lib.optional (names != [
                "ulak-agent"
                "ulak-backend"
              ]) "expected ulak-agent + ulak-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/ulak gateway" (argvOf "ulak-agent")
              ) "gateway process does not run `ulak gateway`: ${argvOf "ulak-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/ulak serve" (argvOf "ulak-backend")
              ) "backend process does not run `ulak serve`: ${argvOf "ulak-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "ulak-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/ulak-check/.ulak" (envOf n)) names
              ) "gateway and backend must share one ULAK_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.ULAK_HOME or null != "/home/ulak-check/.ulak"
              ) "programs.ulak-agent.enable must export ULAK_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "ulak-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/ulak-check/.ulak/SOUL.md" activation
              ) "ulakHomeFiles must install into ULAK_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads ULAK_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "ULAK_MANAGED=home-manager" (envOf n)) names
              ) "processes must report ULAK_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "ulak-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "ulak-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            cases = [
              ...
              {
                name = "ulakHomeFiles needs no directory";
                ok = accepts { ulakHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "ulak-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one ULAK_HOME ───────────────
        # `programs.enable` exports ULAK_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.ulak
        # while the services use the ULAK_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/ulak-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                ulakHome = "/home/ulak-check/.ulak-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "ulak-desktop") cfg.home.packages;
            ...
            # second Ulak runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "ulak-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.ulak-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.ulak-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one ulak-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "ULAK_HOME" != "/home/ulak-check/.ulak-work"
              ) "the launcher must carry ULAK_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "ULAK_HOME")})"
              ++ lib.optional (
                setValue "ULAK_MANAGED" != "home-manager"
              ) "the launcher must report ULAK_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "ULAK_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "ULAK_DESKTOP_ULAK" != "${lib.head agentPackages}/bin/ulak"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "ULAK_DESKTOP_ULAK")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "ULAK_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "ULAK_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "ULAK_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "ULAK_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "ULAK_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "ulak-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares ULAK_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set ULAK_HOME" wrapper
              ) "the launcher must carry ULAK_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "ULAK_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "ULAK_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? ulak-backend || cfg.launchd.agents ? ulak-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "ulak-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `ulak`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.ulak-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.ulak-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "ulak-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "ulak-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "ulak" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."ulak-agent-setup".text;

            failures =
              lib.optional (names != [
                "ulak-agent"
                "ulak-backend"
              ]) "expected ulak-agent + ulak-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/ulak gateway" (execOf "ulak-agent")
              ) "gateway unit does not run `ulak gateway`: ${execOf "ulak-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/ulak dashboard" (execOf "ulak-backend")
              ) "backend unit does not run `ulak dashboard`: ${execOf "ulak-backend"}"
              ++ lib.optional (
                units.ulak-agent.environment.ULAK_HOME != units.ulak-backend.environment.ULAK_HOME
              ) "gateway and backend must share one ULAK_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/ulak/.ulak/SOUL.md" activation
              ) "ulakHomeFiles must install into ULAK_HOME";
          ...
          in
          pkgs.runCommand "ulak-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs ulak, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.ulak-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/ulak serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec ulak directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "ulak-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "ulak-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec ulak, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/ulak dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "ulak-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        # $ULAK_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          ...
          in
          pkgs.runCommand "ulak-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'ULAK_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--ulak-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "ulak-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "ulak-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${ulak-agent}/bin/ulak || (echo "FAIL: ulak binary missing"; exit 1)
          test -x ${ulak-agent}/bin/ulak-agent || (echo "FAIL: ulak-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${ulak-agent}/bin/ulak --version 2>&1 | grep -qi "ulak" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "ulak-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in ulak ulak-agent ulak-acp; do
            test -x ${ulak-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "ulak-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking ulak --help ==="
          ${ulak-agent}/bin/ulak --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${ulak-agent}/bin/ulak --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "ulak-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${ulak-agent}/share/ulak-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${ulak-agent}/share/ulak-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "ULAK_BUNDLED_SKILLS" ${ulak-agent}/bin/ulak || \
            (echo "FAIL: ULAK_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: ULAK_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${ulak-agent}/share/ulak-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${ulak-agent}/share/ulak-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "ULAK_OPTIONAL_SKILLS" ${ulak-agent}/bin/ulak || \
            (echo "FAIL: ULAK_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, ULAK_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "ulak-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${ulak-agent}/share/ulak-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${ulak-agent}/share/ulak-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "ULAK_BUNDLED_PLUGINS" ${ulak-agent}/bin/ulak || \
            (echo "FAIL: ULAK_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: ULAK_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "ulak-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${ulak-agent}/share/ulak-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${ulak-agent}/share/ulak-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${ulak-agent}/share/ulak-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "ULAK_BUNDLED_LOCALES" ${ulak-agent}/bin/ulak || \
            (echo "FAIL: ULAK_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: ULAK_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + ULAK_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && ULAK_BUNDLED_LOCALES=${ulak-agent}/share/ulak-agent/locales \
            ${ulakVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with ULAK_BUNDLED_LOCALES set"; exit 1)
        ...
        # ULAK_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "ulak-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${ulak-agent}/share/ulak-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${ulak-agent}/share/ulak-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "ULAK_OPTIONAL_MCPS" ${ulak-agent}/bin/ulak || \
            (echo "FAIL: ULAK_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: ULAK_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${ulak-agent}/bin/ulak mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: ulak mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "ulak-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${ulak-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${ulak-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "ULAK_TUI_DIR" ${ulak-agent}/bin/ulak || \
            (echo "FAIL: ULAK_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: ULAK_TUI_DIR set in wrapper"
        ...
        # Verify ULAK_NODE is set in wrapper and points to Node 26+
        # (Ulak pins its toolchain to Node 26 everywhere)
        ulak-node = pkgs.runCommand "ulak-node-version" { } ''
          set -e
          echo "=== Checking ULAK_NODE in wrapper ==="
          grep -q "ULAK_NODE" ${ulak-agent}/bin/ulak || \
            (echo "FAIL: ULAK_NODE not set in wrapper"; exit 1)
          echo "PASS: ULAK_NODE present in wrapper"

          ULAK_NODE=$(sed -n "s/^export ULAK_NODE='\(.*\)'/\1/p" ${ulak-agent}/bin/ulak)
          test -x "$ULAK_NODE" || (echo "FAIL: ULAK_NODE=$ULAK_NODE not executable"; exit 1)
          echo "PASS: ULAK_NODE executable at $ULAK_NODE"

          NODE_MAJOR=$("$ULAK_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Ulak requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All ULAK_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify ULAK_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "ulak-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ULAK_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking ULAK_MANAGED guards ==="
          check_blocked "config set" ${ulak-agent}/bin/ulak config set model foo
          check_blocked "config edit" ${ulak-agent}/bin/ulak config edit
        ...
        extra-python-packages = let
          ...
          ulakWithExtra = ulak-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "ulak-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${ulakWithExtra}/bin/ulak || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${ulakWithExtra}/bin/ulak || \
          ...
          if grep -q "PYTHONPATH" ${ulak-agent}/bin/ulak; then
        ...
        extra-dependency-groups = let
          ulakWithGroups = ulak-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "ulak-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${ulakWithGroups}"
          echo "venv: ${ulakWithGroups.ulakVenv}"
        ...
        messaging-variant = pkgs.runCommand "ulak-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.ulakVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "ulak-config-roundtrip" {
          ...
          merge_and_load() {
            local ulak_home="$1"
            export ULAK_HOME="$ulak_home"
            ${configMergeScript} ${nixSettings} "$ulak_home/config.yaml"
            ${ulakVenv}/bin/python3 -c '
import json, sys
