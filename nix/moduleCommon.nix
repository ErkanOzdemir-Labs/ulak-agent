...
#
# `services.ulak-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  ulakHome, programs.ulak-agent (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "ulak-config-attrs";
    description = "Ulak YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $ULAK_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Ulak Agent";
      ...
      package = mkOption {
        ...
        description = "The ulak-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Ulak configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `ulak config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $ULAK_HOME/.env. Ulak reads that file at each start, with
          load_ulak_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."ulak/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Ulak refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Ulak reads SOUL.md and memories/ from ULAK_HOME, so put those
          files in `ulakHomeFiles`.
      ...
      ulakHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into ULAK_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Ulak reads SOUL.md and the memory files from ULAK_HOME and not
          from the working directory. Declare those files here, or Ulak
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the ulak plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. Ulak discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          ulak_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as ulak (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in ulak-agent's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `ulak gateway`.";
      };
      ...
      # ── The backend: `ulak serve` or `ulak dashboard` ──────────────
      # `ulak serve` and `ulak dashboard` are the same entry point,
      # ulak_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Ulak Desktop
      ...
      # controls an external gateway, with `ulak gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Ulak
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            ULAK_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. Ulak Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."ulak/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `ulak` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.ulak-agent.installPackage was removed. Ulak now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.ulak-agent.enable = ${lib.boolToString (value != false)};  # the ulak CLI, and ULAK_HOME for your shells
        programs.ulak-agent.desktop.enable = true;  # the desktop application

      `services.ulak-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "ulak-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "ulak-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "ulak-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "ulak-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "ulak-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # ULAK_MANAGED variable of the service. The value tells the shell
    ...
    in
    ''
      # Directories. The service units and Ulak make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${ulakHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Ulak
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${ulakHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${ulakHome}/config.yaml
            ${run}chmod ${modes.config} ${ulakHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "ulak-managed" managedSystem} ${ulakHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${ulakHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${ulakHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${ulakHome}/auth.json"
        else
          ''
            if [ ! -e ${ulakHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${ulakHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree ulakHome cfg.ulakHomeFiles}
      ...
      ${run}find ${ulakHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "ulak-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${ulakHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  #
  # `exec` on the last line keeps ulak as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "ulak-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "ulak-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "ulak-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          ULAK_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export ULAK_DASHBOARD_SESSION_TOKEN

          if [ -z "$ULAK_DASHBOARD_SESSION_TOKEN" ]; then
            echo "ulak-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "ulak-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "ulak-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "ulak-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "ulak-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "ulak-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "ulak-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Ulak Agent web dashboard and desktop backend"
    else
      "Ulak Agent backend for Ulak Desktop";

  # The environment that each Ulak process needs, from either module.
  #
  # managedSystem gives the value of ULAK_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give Ulak an identity and a memory, use
          ${optionPath}.ulakHomeFiles instead. Those files go to
          ULAK_HOME. Ulak reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of ULAK_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
