...
Author widget apps for the Ulak TUI (`ulak --tui`): glanceable ambient
...
- The TUI must be in use (`ulak --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.ulak/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.ulak/logs/tui_gateway_crash.log` if a widget never appears.
