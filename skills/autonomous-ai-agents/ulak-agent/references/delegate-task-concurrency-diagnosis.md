...
**three** code paths in Ulak that cap a batch. If none of them fired, the
cap came from the **model itself** — not from Ulak — and the user's
narration of "the runtime caps at N" is the model rationalising its own
choice.

## The three real caps in Ulak
...
3. **Cost-warning** — same `_get_max_concurrent_children()`. When the
   ...
   log line** — it does not cap anything. Easy to mis-read as "Ulak is
   refusing my value."
...
ulak config get delegation.max_concurrent_children

# 2. Did Ulak' truncator or rejector actually fire?
grep -E "Truncated.*delegate_task|Too many tasks" ~/.ulak/logs/agent.log | tail
# If neither line appears, neither cap path executed.

# 3. Confirm the resolver returns what config says (in venv with ulak on path)
...
appears, **the cap is the model**, not Ulak.
