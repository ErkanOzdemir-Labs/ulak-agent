...
Set via `ulak model` (picker) or `ulak setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://ulak-agent.github.com/ErkanOzdemir-Labs/docs/integrations/providers
...
| nous | OAuth device code | `ulak auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `ulak auth add openai-codex` |
| qwen-oauth | OAuth | `ulak auth add qwen-oauth` |
| minimax-oauth | OAuth | `ulak auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`ulak auth`).
Fallback chain when the primary fails: `ulak fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`ulak_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   ulak config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
