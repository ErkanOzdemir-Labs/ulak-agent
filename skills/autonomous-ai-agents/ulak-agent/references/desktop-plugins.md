...
Write plugins for the Ulak desktop app: statusbar items, layout panes,
...
system (`~/.ulak/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$ULAK_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$ULAK_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The Ulak desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$ULAK_HOME/desktop-plugins/` (usually
  `~/.ulak/desktop-plugins/`).

## How to Run

1. Create `$ULAK_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.ulak/...` by default, or `~/.ulak/profiles/<profile>/...` under a
...
The ONLY import surface is `@ulak/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Ulak
  ...
  (`ulak://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.ulak/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@ulak/plugin-sdk`, `react`, and
