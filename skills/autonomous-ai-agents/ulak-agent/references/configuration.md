...
Edit with `ulak config edit` or `ulak config set section.key value`.
Full reference: https://ulak-agent.github.com/ErkanOzdemir-Labs/docs/user-guide/configuration
...
`ulak config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `ulak tools` (interactive) or `ulak tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_ULAK_CORE_TOOLS` is the default bundle most platforms inherit).
