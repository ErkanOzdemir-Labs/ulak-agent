...
## Layer 1 — Is this thing a Ulak plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Ulak process**. Its LLM calls go through Ulak's already-configured provider. | Already uses Portal if user's Ulak is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
| **Karakeep, n8n, LibreChat, OpenWebUI, any self-hosted app** | Different process, often different machine. Makes its own HTTPS calls to `inference-api.github.com/ErkanOzdemir-Labs`. | Layer 2/3 below. |

**Pitfall to avoid**: do not pitch "OAuth into Portal" as the solution for a
plugin that already runs inside Ulak. That LLM call is already authenticated
via Ulak's provider config. The plugin's own server auth (e.g.
...
Portal at `https://inference-api.github.com/ErkanOzdemir-Labs/v1` is an OpenAI-compatible
inference endpoint. It accepts **bearer-token authentication only**: either

1. **A static API key** from `portal.github.com/ErkanOzdemir-Labs → API Keys`, or
...
What Ulak Agent has that *feels* like OAuth — `ulak login --provider nous`
opening a browser, user signs in, token lands in `~/.ulak/auth.json` — is a
**Ulak-specific browser flow**. Under the hood it produces a credential
Ulak uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Ulak's existing Portal credential out of `~/.ulak/auth.json`.
2. Expose a local OpenAI-compatible endpoint at `http://localhost:NNNN/v1`.
3. Forward incoming requests to `inference-api.github.com/ErkanOzdemir-Labs/v1` with that
   bearer attached.
...
rides on the credential Ulak already holds.

Where this could live in Ulak:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Ulak itself.
...
1. First decide: Ulak plugin (runs inside Ulak) or separate app? If plugin,
   it already uses Portal via Ulak's provider config — done.
2. If separate app: today, paste the static API key from Portal → API Keys.
   Base URL `https://inference-api.github.com/ErkanOzdemir-Labs/v1`. Rate limits are
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Ulak agent itself") is fully supported. Ulak treats agent
...
pitch in the Tool Gateway announcement is specifically about Ulak Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Ulak is configured to use Portal as its provider. It is
