...
For occasional contributors and PR authors. Full developer docs: https://ulak-agent.github.com/ErkanOzdemir-Labs/docs/developer-guide/
...
Config: `~/.ulak/config.yaml` (settings), `~/.ulak/.env` (API keys) — both under `$ULAK_HOME` when it is set.
...
`_ULAK_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_ulak_home()` for paths,
never hardcode `~/.ulak`. For custom/local-only tools, write a plugin in
`~/.ulak/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `ulak_cli/commands.py`
...
- Tests auto-redirect `ULAK_HOME` to temp dirs — never touch real `~/.ulak/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/ulak_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_ulak_home()` from `ulak_constants` for all paths (profile-safe)
