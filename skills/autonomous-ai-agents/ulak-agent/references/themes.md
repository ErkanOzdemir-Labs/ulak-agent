# Themes / Skins — Author a Ulak Color Theme

Author a Ulak **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`ulak_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.ulak/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Ulak itself.
...
- Write access to the Ulak home dir — `~/.ulak` by default, or `$ULAK_HOME`
  / the active profile's dir. Skins live in `<ulak-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `ulak config set`).
...
3. `write_file` it to `<ulak-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `ulak config set display.skin default` (or they can `/skin default`).
...
```
ulak skin set <key> <hex>      # e.g. ulak skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `ulak skin set background "#08201f"` changes
only the background; `ulak skin use <name>` / `ulak skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.ulak`** when a profile is active — resolve the real home
  from `$ULAK_HOME` first, falling back to `~/.ulak`.
...
- **Never hand-edit `config.yaml` to activate.** Use `ulak config set
...
- **You apply it, not the user.** `ulak config set display.skin <name>` is
...
- `read_file` the written `<ulak-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `ulak config get display.skin` and confirm it reports `<name>`.
