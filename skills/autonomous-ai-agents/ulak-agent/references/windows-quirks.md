...
Ulak runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`ulak config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually Ulak's env scrubber dropping
...
Ulak-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every Ulak tool and most Windows APIs. Prefer forward slashes in code
