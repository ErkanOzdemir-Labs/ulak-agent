...
Ledger path resolution (first wins):
  --ledger PATH
  $ULAK_CITATION_LEDGER
  $ULAK_HOME/cache/citations/ledger.json
"""
...
from _ulak_home import get_ulak_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("ULAK_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_ulak_home() / "cache" / "citations" / "ledger.json"
