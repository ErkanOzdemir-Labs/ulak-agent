#!/usr/bin/env bash
# GitHub environment detection helper for Ulak Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _ulak_env="${ULAK_HOME:-$HOME/.ulak}/.env"; [ -f "$_ulak_env" ] && grep -q "^GITHUB_TOKEN=" "$_ulak_env" 2>/dev/null; then
