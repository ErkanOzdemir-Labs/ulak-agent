#!/usr/bin/env python3
"""Google Workspace API CLI for Ulak Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Ulak-facing JSON contract and falls back to the Python client
...
from _ulak_home import get_ulak_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ULAK_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Ulak Agent")
