"""Shared constants for Ulak Agent.
...
_UNSET = object()
_ULAK_HOME_OVERRIDE: ContextVar[str | object] = ContextVar("_ULAK_HOME_OVERRIDE", default=_UNSET)
...
def set_ulak_home_override(path: str | Path | None) -> Token:
    """Set a context-local Ulak home override and return its reset token.

    Deliberately does not mutate ``os.environ`` (shared by every thread in the process).
    """
    value: str | object = _UNSET if path is None else str(path)
    return _ULAK_HOME_OVERRIDE.set(value)


def reset_ulak_home_override(token: Token) -> None:
    """Restore the previous context-local Ulak home override."""
    _ULAK_HOME_OVERRIDE.reset(token)


def get_ulak_home_override() -> str | None:
    """Return the active context-local Ulak home override, if any."""
    override = _ULAK_HOME_OVERRIDE.get()
    return str(override) if override is not _UNSET and override else None


def _get_platform_default_ulak_home() -> Path:
    """Return the platform-native default Ulak home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "ulak"
    return Path.home() / ".ulak"


def _warn_profile_fallback_once() -> None:
    """Warn once when ULAK_HOME is unset but a non-default profile is sticky-active (wrong fallback)."""
    global _profile_fallback_warned
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_ulak_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[ULAK_HOME fallback] ULAK_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass ULAK_HOME explicitly "
            f"(see issue #18594)."
        )
...
def get_ulak_home() -> Path:
    """Ulak home: context-local override → ``ULAK_HOME`` env var → platform default."""
    override = get_ulak_home_override()
    if override:
        return Path(override)
    if not os.environ.get("ULAK_HOME", "").strip():
        _warn_profile_fallback_once()
    return get_process_ulak_home()
...
def ulak_home_key(path: str | Path | None = None) -> str:
    """Stable registry key for a Ulak home/profile dir.
    ...
    """
    candidate = Path(path) if path is not None else get_ulak_home()
    raw = str(candidate)
...
def reset_ulak_home_key_cache() -> None:
    """Forget every remembered home key (for tests that move a home dir on disk)."""
    _HOME_KEY_CACHE.clear()


def get_process_ulak_home() -> Path:
    """Ulak home of the running process, ignoring task overrides.
    ...
    return Path(val) if val else _get_platform_default_ulak_home()


# get_default_ulak_root() memo keyed on (native home, ULAK_HOME) so it stays
# fresh when a test or plugin mutates ULAK_HOME; saves ~80us/call at 31+ sites.
_default_ulak_root_memo: "tuple[str, str, Path] | None" = None


def get_default_ulak_root() -> Path:
    """Root Ulak dir for profile-level ops: ``<root>`` when ``ULAK_HOME=<root>/profiles/<name>``."""
    global _default_ulak_root_memo
    native_home = _get_platform_default_ulak_home()
    env_home = os.environ.get("ULAK_HOME", "")
    memo = _default_ulak_root_memo
    ...
    if env_home:
        ...
        try:
            env_path.resolve().relative_to(native_home.resolve())  # under ~/.ulak (normal or profile mode)
        except ValueError:  # Docker/custom root: <root>/profiles/<name> -> <root>, else ULAK_HOME itself
            result = env_path.parent.parent if env_path.parent.name == "profiles" else env_path
    _default_ulak_root_memo = (str(native_home), env_home, result)
    return result
...
# Files marking a real Ulak home; arbitrary dirs with a ``profiles`` segment lack them.
_ULAK_HOME_MARKERS = ("config.yaml", ".env", "state.db")


def _is_ulak_profiles_root(profiles_dir: Path) -> bool:
    """True when *profiles_dir* is provably ``<ulak-home>/profiles``.

    Accepts the classic ``~/.ulak`` layout, a root carrying Ulak-home marker files, a
    ...
    try:
        if (profiles_dir / _DELETED_PROFILES_DIR).is_dir() or any(
            (root / marker).exists() for marker in _ULAK_HOME_MARKERS
        ):
            return True
    except OSError:
        pass
    try:
        return root.resolve(strict=False) == get_default_ulak_root().resolve(strict=False)
    except OSError:
        return False
...
def named_profile_home(path: str | Path) -> Path | None:
    ...
    Requires ``<name>`` not to start with ``.`` and the ``profiles`` parent to be a real Ulak home;
    ...
    for candidate in (current, *current.parents):
        if (candidate.parent.name == "profiles" and not candidate.name.startswith(".")
                and _is_ulak_profiles_root(candidate.parent)):
            return candidate
        if candidate.name == ".ulak":  # default home: a coincidental profiles/ ancestor is not a root
            return None
    return None
...
def mkdir_under_ulak_home(path: str | Path) -> Path:
...
def _packaged_dir(env_var: str, default: Path | None, subdir: str) -> Path:
    ...
    return Path(override) if override else default if default is not None else get_ulak_home() / subdir


def get_optional_skills_dir(default: Path | None = None) -> Path:
    """Return the optional-skills directory, honoring package-manager wrappers."""
    return _packaged_dir("ULAK_OPTIONAL_SKILLS", default, "optional-skills")


def get_optional_mcps_dir(default: Path | None = None) -> Path:
    """Return the optional-mcps directory, honoring package-manager wrappers (``ULAK_OPTIONAL_MCPS``)."""
    return _packaged_dir("ULAK_OPTIONAL_MCPS", default, "optional-mcps")


def get_bundled_skills_dir(default: Path | None = None) -> Path:
    """Return the bundled skills directory, honoring package-manager wrappers (``ULAK_BUNDLED_SKILLS``)."""
    return _packaged_dir("ULAK_BUNDLED_SKILLS", default, "skills")


def get_ulak_dir(new_subpath: str, old_name: str, *, home: Path | None = None) -> Path:
    """Resolve a Ulak subdirectory, honouring a populated legacy ``<old_name>/`` (no migration).
    ...
    """
    home = home or get_ulak_home()
    old_path = home / old_name
    return old_path if _legacy_path_has_content(old_path) else home / new_subpath


def iter_ulak_node_dirs(home: Path | None = None) -> list[Path]:
    """Ulak-managed Node dirs in lookup order; both Windows and POSIX shapes so migrated installs work.

    Keep in sync with ulakManagedNodePathEntries() in apps/desktop/electron/backend-env.ts.
    """
    node_dir = (home or get_ulak_home()) / "node"
...
def _iter_managed_node_candidates(names: list[str], home: Path | None = None):
    ...
    for directory in iter_ulak_node_dirs(home):
        for name in names:
...
def _version_probe_ok(path: str) -> bool:
    """True when ``<path> --version`` exits 0 under the Ulak-managed Node PATH."""
    result = _run_version_probe([path, "--version"], env=with_ulak_node_path())
    return result is not None and result.returncode == 0


_ULAK_NODE_TARGET_MAJOR = int(os.environ.get("ULAK_NODE_TARGET_MAJOR", "22"))
_managed_node_heal_attempted = False
_NODE_BOOTSTRAP_SCRIPT = Path(__file__).resolve().parent / "scripts" / "lib" / "node-bootstrap.sh"

# Install tree root (this file lives at <install_root>/ulak_constants.py). Used by secure_parent_dir() to
# skip chmod on the install dir — chmodding it 0700 breaks ulak-user traversal in Docker (UID 10000). See
# #25821, #93050.
...
def ulak_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Ulak-managed node/npm/npx shim exists on disk."""
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        "→ Ulak-managed Node.js is in use by a running app; deferring its "
        "upgrade until the app is closed (re-run `ulak update` afterwards).", flush=True,
    )
...
def _stage_windows_node_zip(home: Path, node_arch: str) -> Path | None:
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_ULAK_NODE_TARGET_MAJOR}.x/"
    index_bytes = _fetch_url(index_url, 60)
    if index_bytes is None:
        return None
    pattern = rf"node-v{_ULAK_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip"
...
def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    """Redownload the portable Node zip into ``%ULAK_HOME%\\node`` on Windows.
    ...
    (images are mapped with ``FILE_SHARE_DELETE`` — the same mechanism as the ulak.exe quarantine); when
...
def _run_node_bootstrap(func: str, *, timeout: int, **extra_env: str) -> bool:
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ULAK_HOME": str(get_ulak_home()), **extra_env},
...
def bootstrap_ulak_managed_node() -> str | None:
    """Install a Ulak-managed Node tree under ``$ULAK_HOME/node`` and return its npm path.

    Ulak never modifies a user-owned toolchain (system, nvm, brew, Nix) that fails ``engines``.
    """
    existing = find_ulak_node_executable("npm")
    if existing:
        return existing
    if sys.platform == "win32":
        ok = _heal_managed_node_windows()
    else:
        # ULAK_NODE_SKIP_LINKS=1 keeps node/npm/npx out of ~/.local/bin: never shadow the user toolchain.
        ok = _run_node_bootstrap("_nb_install_bundled_node", timeout=600, ULAK_NODE_SKIP_LINKS="1")
    if not ok:
        return None
    return _first_runnable_managed(_candidate_node_command_names("npm"))[0]


def heal_ulak_managed_node() -> bool:
    """Redownload Ulak-managed Node when the tree exists but is broken; at most once per process.
    ...
    if _managed_node_heal_attempted or not ulak_managed_node_tree_present():
        return False
    if sys.platform == "win32":
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    for candidate in _iter_managed_node_candidates(_candidate_node_command_names("node"), home):
        ...
        if "-" in version:
            return True
        return major < _ULAK_NODE_TARGET_MAJOR
    return False


def find_ulak_node_executable(command: str) -> str | None:
    """Ulak-managed Node/npm path, healing broken/outdated trees; heal failure still returns old Node."""
    ...
    if needs_heal and heal_ulak_managed_node():
...
def find_node_executable(command: str) -> str | None:
    ...
    """
    managed = find_ulak_node_executable(command)
    if managed:
        return managed
    if ulak_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_ulak_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Ulak-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    parts = [p for p in merged.get("PATH", "").split(os.pathsep) if p]
    for entry in reversed([str(path) for path in iter_ulak_node_dirs() if path.is_dir()]):
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    local ``node_modules/agent-browser/bin/...`` binary, which then disappears on the next ``ulak update``
...
def display_ulak_home() -> str:
    """User-facing ``~/`` display string for ULAK_HOME (``~/.ulak/profiles/coder``)."""
    home = get_ulak_home()
    try:  # as_posix(): str() on Windows yields chimeras like ~/AppData\Local\ulak/skills/
...
def secure_parent_dir(path: Path) -> None:
    """Chmod ``0o700`` on *path*'s parent, refusing ``/`` and top-level dirs (misresolved ULAK_HOME)."""
    parent = path.parent.resolve()
    if parent == Path("/") or len(parent.parts) < 3:
        return
    # Refuse the install tree: chmod 0700 breaks ulak-user traversal in Docker (UID 10000).
    # A credential file here means ULAK_HOME misresolved; surface it (caused production lockouts).
    # See #25821, #93050.
    if parent == _INSTALL_ROOT or _INSTALL_ROOT in parent.parents:
        ...
        logging.getLogger(__name__).warning(
            ...
            "ulak-agent install directory (%s). Credential files are "
            "normally stored under the ulak home directory instead.", parent, _INSTALL_ROOT,
        )
        return
    with contextlib.suppress(OSError):
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{ULAK_HOME}/home`` when the profile-home directory exists."""
    ulak_home = get_ulak_home_override() or (env or {}).get("ULAK_HOME") or os.getenv("ULAK_HOME")
    if not ulak_home:
        return None
    profile_home = os.path.join(ulak_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates = [_env_get(env, "ULAK_REAL_HOME"), _env_get(env, "HOME")]
    with contextlib.suppress(Exception):
        import pwd
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """The OS user's real home, avoiding the Ulak profile HOME.

    ``HOME`` belongs to the OS account and external CLIs keeping credentials under ``~``; a parent
    already running with ``HOME={ULAK_HOME}/home`` is repaired back when possible.
    """
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    ``{ULAK_HOME}/home``; ``real``: always real HOME; ``profile``: always the profile home.
    """
    env = env or {}
    profile_home = _profile_home_path(env)
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Ulak' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["ULAK_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Map a Windows-host cwd (drive path or ``\\\\wsl.localhost\\`` UNC) to POSIX when Ulak runs in WSL.
...
def is_container() -> bool:
    ...
    See: ErkanOzdemir-Labs/ulak-agent#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under ULAK_HOME."""
    return get_ulak_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under ULAK_HOME."""
    return get_ulak_home() / "skills"


def get_env_path() -> Path:
    """Return the path to the ``.env`` file under ULAK_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_ulak_ipv4_patched", False):
        return
    ...
    def _ipv4_getaddrinfo(host, port, family=0, type=0, proto=0, flags=0):
        ...
        return _original_getaddrinfo(host, port, family, type, proto, flags)
    _ipv4_getaddrinfo._ulak_ipv4_patched = True  # type: ignore[attr-defined]
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    Canonical helper for venv layout. This was open-coded in seven places across four ``ulak_cli`` modules
    ...
    lived 2400 lines away in another function. A few sites outside ``ulak_cli``
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* ships with Ulak (exact first segment; ``startswith`` would claim ``agentops``)."""
    root = str(name).split(".")[0] if name else ""
    return bool(root) and (root in FIRST_PARTY_MODULE_ROOTS or root.startswith("ulak_"))
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    return [
        ...
        "    ulak update",
        "If that also fails, reinstall: https://ulak-agent.github.com/ErkanOzdemir-Labs",
    ]
