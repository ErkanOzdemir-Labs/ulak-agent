"""Migrate Ulak MCP server config and Codex's installed curated plugins into ~/.codex/config.toml.
"""
...
MIGRATION_END_MARKER = "# end ulak-agent managed section"
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Ulak config.")
...
# Ulak MCP keys codex understands (transport stdio/http, timeouts, general). Any other key is
# dropped with a warning: ``sampling`` has no codex equivalent; the rest are unknown Ulak keys.
_KNOWN_ULAK_KEYS = {
...
# (ulak key, codex key, skip note) — timeouts are emitted as floats or skipped when non-numeric.
_TIMEOUT_KEYS = (
...
def _translate_one_server(name: str, ulak_cfg: dict) -> tuple[Optional[dict], list[str]]:
    """Translate one Ulak MCP server config to codex's inline-table dict.

    Returns ``(codex_entry, skipped_keys)``; ``codex_entry`` is None when the config is unusable.
    stdio (``command``) wins over ``url`` when both are set. Ulak' ``transport: sse`` hint is
    informational only — codex auto-negotiates. ``enabled`` is emitted only when explicitly false
    (codex defaults to true).
    """
    if not isinstance(ulak_cfg, dict):
        return None, []
    skipped: list[str] = []
    out: dict[str, Any] = {}
    if ulak_cfg.get("command"):
        if ulak_cfg.get("url"):
            skipped.append("url (both command and url set; preferring stdio)")
        out["command"] = str(ulak_cfg["command"])
        if ulak_cfg.get("args"):
            out["args"] = [str(a) for a in ulak_cfg["args"]]
        if ulak_cfg.get("env"):
            out["env"] = _str_map(ulak_cfg["env"])
        if ulak_cfg.get("cwd"):
            out["cwd"] = str(ulak_cfg["cwd"])
    elif ulak_cfg.get("url"):
        out["url"] = str(ulak_cfg["url"])
        if ulak_cfg.get("headers"):
            out["http_headers"] = _str_map(ulak_cfg["headers"])
        if ulak_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]
    for ulak_key, codex_key, note in _TIMEOUT_KEYS:
        if ulak_key in ulak_cfg:
            try:
                out[codex_key] = float(ulak_cfg[ulak_key])
            except (TypeError, ValueError):
                skipped.append(note)
    if ulak_cfg.get("enabled") is False:
        out["enabled"] = False
    for key in ulak_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            skipped.append(f"{key} (no codex equivalent)")
        elif key not in _KNOWN_ULAK_KEYS:
            skipped.append(f"{key} (unknown Ulak key)")
    return out, skipped
...
# env-var passthrough (ULAK_HOME, PYTHONPATH) could carry one in pathological cases.
_TOML_ESCAPES = (
    ("\\", "\\\\"), ('"', '\\"'), ("\b", "\\b"), ("\t", "\\t"),
    ("\n", "\\n"), ("\f", "\\f"), ("\r", "\\r"))
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out += ["# (no MCP servers, plugins, or permissions configured by Ulak)", MIGRATION_END_MARKER]
        return "\n".join(out) + "\n"
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(codex_home=str(codex_home) if codex_home else None) as client:
            client.initialize(client_name="ulak-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    Such dirs are reaped between sessions; a ULAK_HOME pointing there burned into
...
def _build_ulak_tools_mcp_entry() -> dict:
    """Codex stdio entry launching Ulak' own tool surface as an MCP server (browser/web/
    delegate_task/vision/memory/skills call-backs).

    ULAK_HOME passes through only IF SET, read from os.environ (not get_ulak_home()): when
    unset the codex subprocess must inherit its launcher's runtime ULAK_HOME (systemd, gateway,
    kanban), not a migrate-time default burned into config.toml that pins the wrong profile. The
    pytest-tempdir guard keeps a sibling test's monkeypatched ULAK_HOME out of the user's real
    config. PYTHONPATH passes through so a worktree-launched ulak finds the branch's modules.
    """
    import sys
    env: dict[str, str] = {}
    # ULAK_HOME passes through IF SET so the MCP subprocess sees the same config / auth / sessions DB as
    # the parent CLI. Read from os.environ (not get_ulak_home()) on purpose: when the env var is unset we
    # want codex's subprocess to inherit whatever ULAK_HOME its launcher sets at runtime (systemd unit,
    # gateway, kanban dispatcher, custom shell), rather than burning the migrate-time resolved default into
    # config.toml — that would override the launcher's ULAK_HOME and pin the subprocess to the wrong
    # profile. The pytest-tempdir guard below catches the issue #26250 Bug C scenario: a sibling test's
    # monkeypatch.setenv("ULAK_HOME", tmp_path) would otherwise leak a transient pytest tempdir into the
    # user's real ~/.codex/config.toml and silently brick codex once the tempdir is GC'd.
    ulak_home = os.environ.get("ULAK_HOME") or ""
    if ulak_home and not _looks_like_test_tempdir(ulak_home):
        env["ULAK_HOME"] = ulak_home
    if os.environ.get("PYTHONPATH"):
        env["PYTHONPATH"] = os.environ["PYTHONPATH"]
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["ULAK_QUIET"] = "1"
    env["ULAK_REDACT_SECRETS"] = env.get("ULAK_REDACT_SECRETS", "true")
    return {
...
def migrate(
    ulak_config: dict, *, codex_home: Optional[Path] = None, dry_run: bool = False,
    discover_plugins: bool = True, default_permission_profile: Optional[str] = ":workspace",
    expose_ulak_tools: bool = True) -> MigrationReport:
    """Translate Ulak mcp_servers config + Codex curated plugins into ~/.codex/config.toml.
    ...
    approval prompt on every write. ``expose_ulak_tools`` registers Ulak' own tool surface
    (agent/transports/ulak_tools_mcp_server.py, launched on demand by codex over stdio) as an MCP
    ...
    ulak_servers = (ulak_config or {}).get("mcp_servers") or {}
    if not isinstance(ulak_servers, dict):
        report.errors.append("mcp_servers in Ulak config is not a dict; cannot migrate.")
        return report
    translated: dict[str, dict] = {}
    for raw_name, cfg in ulak_servers.items():
        name = str(raw_name)
    ...
    if expose_ulak_tools:
        translated["ulak-tools"] = _build_ulak_tools_mcp_entry()
        if "ulak-tools" not in report.migrated:
            report.migrated.append("ulak-tools")
