"""Bounded product contract for the first Ulak shared-metrics slice."""
...
SCHEMA_KEY = "ulak.metrics.schema_version"
SCHEMA_VERSION = "ulak.metrics.event.v2"
MODEL_CALL_SCOPE = "ulak.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "ulak.task_run"
TOOL_CALL_SCOPE = "ulak.tool_call"
CLIENT_ACTIVE_MARK = "ulak.client.active"
TOOL_APPROVAL_MARK = "ulak.tool_approval"
SKILL_LIFECYCLE_MARK = "ulak.skill.lifecycle"
SKILL_LOAD_MARK = "ulak.skill.load"
SUBSCRIBER_NAME = "ulak.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "ulak.client.active"
LEGACY_MODEL_CALL_METRIC = "ulak.model_call.count"
MODEL_ROUTE_METRIC = "ulak.model_route.count"
TASK_STARTED_METRIC = "ulak.task_run.started"
TASK_FINISHED_METRIC = "ulak.task_run.finished"
TOOL_CALL_METRIC = "ulak.tool_call.count"
TOOL_APPROVAL_METRIC = "ulak.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "ulak.skill.lifecycle.count"
SKILL_LOAD_METRIC = "ulak.skill.load.count"
...
CLIENT_RESOURCE_KEYS = frozenset({"architecture", "ulak_version", "install_method", "os_family"})
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Ulak installation method."""
    normalized = _norm(value)
    return _allowlisted("nixos" if normalized == "nix" else normalized, CLIENT_INSTALL_METHODS)


def client_resource(
    ulak_version: Any, *, os_name: Any, architecture: Any, install_method: Any
) -> dict[str, str]:
    """Build the bounded client resource attached to aggregate packages."""
    version = str(ulak_version or "").strip()
    return {
        "architecture": client_architecture(architecture),
        "ulak_version": version if 0 < len(version) <= 64 else "unknown",
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("ulak_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Ulak logical scope."""
    metadata = _relay_metadata(
        event, RUNTIME_SCHEMA_KEY, RUNTIME_SCHEMA_VERSION, "ulak.call_role"
    )
    call_role = (metadata or {}).get("ulak.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from ulak_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Ulak terminal state to bounded (outcome, end_reason, termination)."""
...
_TOOL_CATEGORY_PREFIXES = (
    ...
    (("discord", "email", "feishu", "ulak-yuanbao", "slack", "sms"), "communication"),
)


def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Ulak registry toolset metadata to a low-cardinality category."""
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Ulak tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Ulak."""
