...
The provider list shown by ``ulak model`` (CLI/TUI) and the desktop Settings → Providers tabs
...
come from :data:`ulak_cli.auth.PROVIDER_REGISTRY` (credential truth); ``display_name`` /
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `ulak model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """One descriptor per provider in the ``ulak model`` universe (:data:`CANONICAL_PROVIDERS`,
    ...
    PROVIDER_REGISTRY = _safe_import("ulak_cli.auth", "PROVIDER_REGISTRY", {})
    OPTIONAL_ENV_VARS = _safe_import("ulak_cli.config", "OPTIONAL_ENV_VARS", {})
    ...
    ULAK_OVERLAYS = _safe_import("ulak_cli.providers", "ULAK_OVERLAYS", {})
    try:
