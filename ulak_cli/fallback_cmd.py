"""ulak fallback — manage the fallback provider chain (tried in order when the primary fails).

Subcommands: ``list`` (default), ``add`` (same picker as `ulak model`), ``remove``, ``clear``.
"""
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if chain is None:
        print("  Add one with:  ulak fallback add\n")
        return
    print()
    ...
    print("  Docs: https://ulak-agent.github.com/ErkanOzdemir-Labs/docs/user-guide/features/fallback-providers\n")


def cmd_fallback_add(args) -> None:
    """Launch the same picker as `ulak model`, then append the selection to the chain."""
    from ulak_cli.main import _require_tty, select_provider_and_model
    from ulak_cli.config import load_config, save_config
    ...
    print("\n  Adding a fallback provider.  The picker below is the same one used by\n"
          "  `ulak model` — select the provider + model you want as a fallback.\n")
    ...
    print("  Run `ulak fallback list` to view, or `ulak fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``ulak fallback [subcommand]``."""
