"""Dispatcher and maintenance verbs for ``ulak kanban``: ``dispatch``,
...
from ulak_cli.kanban_output import _err, _fmt_ts, _print_json
...
_DAEMON_DEPRECATED = (
    "ulak kanban daemon: DEPRECATED — the dispatcher now runs\ninside the gateway. To use "
    "kanban:\n\n    ulak gateway start       # starts the gateway + embedded dispatcher\n\nReady "
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"successfully. Check profile health (venv, PATH, credentials) and `ulak "
                    f"kanban list --status ready` / `ulak kanban list --status blocked` for "
