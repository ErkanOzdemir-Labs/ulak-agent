"""Image-authored deployment provenance for immutable Ulak runtimes.

The image bakes ``/etc/ulak/image-provenance.json`` outside ``$ULAK_HOME`` and the checkout, so a
...
IMAGE_PROVENANCE_PATH = Path("/etc/ulak/image-provenance.json")
