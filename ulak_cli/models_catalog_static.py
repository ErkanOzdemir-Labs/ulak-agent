...
maps. Split out of ``ulak_cli.models``.
"""
...
def _codex_curated_models() -> list[str]:
    ...
    synthesis) so the gateway /model picker and the CLI ``ulak model`` flow share one source."""
    from ulak_cli.codex_models import DEFAULT_CODEX_MODELS, _finalize_codex_models
...
# Mirrors the xAI-direct IDs from $ULAK_HOME/models_dev_cache.json; the cache overrides it on the
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Ulak-curated xAI models missing from models.dev, right after the pinned headline."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    """Offline curated floor for xAI / xAI OAuth pickers: $ULAK_HOME/models_dev_cache.json
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # (see ulak_cli/model_setup_flows.py); validated live against a GCP project (global region).
    "vertex": [
...
# lists, displays, or iterates providers (ulak model, /model, list_authenticated_providers)
# derives from it. slug = internal ID (config.yaml, --provider); label = short display name;
# tui_desc = longer description for the `ulak model` picker.
# ---------------------------------------------------------------------------
...
# ...) fold under one top-level row in the INTERACTIVE PICKERS (``ulak model``, setup wizard,
...
# Offline/fresh-install fallback for the model Ulak silently lands on when the user never picked
...
# (GUI onboarding / ``ulak model``) uses the tier-aware ``get_recommended_default_model`` in
# ulak_cli/web_server.py + ``partition_nous_models_by_tier``, which may hit the Portal.
...
# OpenRouter-style ids -> Copilot ids. Dash-notation Claude ids are accepted too: Ulak' default
