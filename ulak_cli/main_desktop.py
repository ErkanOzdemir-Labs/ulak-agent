"""Desktop (Electron) app: build/stamp, stage-and-swap pack, exe integrity gate, macOS signing/TCC, Linux sandbox, launch (ulak gui/desktop).
...
from ulak_cli.main_tui_launch import _npm_lifecycle_env
...
def _desktop_stamp_path() -> Path:
    """Path of the desktop build stamp under $ULAK_HOME."""
    from ulak_constants import get_ulak_home
    return get_ulak_home() / "desktop-build-stamp.json"
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/Ulak.app/Contents/MacOS/Ulak → …/Contents/Resources
    resources = (
...
def _desktop_packaged_executable_in(release_dir: Path) -> Optional[Path]:
    ...
    *release_dir* is electron-builder's ``directories.output`` — the live ``apps/desktop/release`` or a
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Ulak.app/Contents/MacOS/Ulak"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / d / "Ulak.exe" for d in ("win-unpacked", "win-ia32-unpacked", "win-arm64-unpacked")
        ]
    else:
        candidates = [
            release_dir / d / n for d in ("linux-unpacked", "linux-arm64-unpacked") for n in ("ulak", "Ulak")
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # mtime can hand a wrong-architecture Ulak.exe to the launcher. Prefer
        ...
        # wrong-architecture Ulak.exe to the launcher, which Windows rejects with "This app can't run on
        # your computer" (#69179).
...
# PLACE: before-pack.mjs wipes ``release/<platform>- unpacked`` (or the mac ``Ulak.app``) and the Electron
# unpack + asar + rename then rebuild it. Any failure after that wipe — corrupt cached zip, blocked
# download, missing dep, disk full — leaves the user with NO app, and ``ulak update`` used to report
...
# (Desktop → ulak-setup --update → `ulak update` → `ulak desktop --build-only` → relaunch) rebuilds
# Ulak.exe on the end user's machine and used to verify only that the file EXISTS before declaring
...
# Ulak.exe is caught BEFORE the updater replaces the working app, and the previous build can be restored
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    emulated process arch) and rejected a correctly-built ARM64 ``Ulak.exe``. Binding
...
def _windows_native_machine() -> str:
    ...
    chain runs an x64 ulak-setup.exe (and thus x64 Python) on Windows-on-ARM devices, where
...
def _ensure_desktop_exe_launchable(desktop_dir: Path, packaged_executable: Optional[Path]) -> tuple:
    ...
    print(f"✗ The built Ulak.exe failed its integrity check: {error}\n    at: {packaged_executable}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Ulak.exe from backup.")
        print("    Your existing version was kept and still works. Run `ulak desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `ulak desktop --force-build` to rebuild, or re-run the Ulak")
...
def _redownload_electron_dist(project_root: Path, env: dict, *, mirror: Optional[str] = None) -> bool:
    ...
    if not installer.is_file():
        return False
    from ulak_constants import find_node_executable, with_ulak_node_path
    ...
    dl_env = with_ulak_node_path(env)
    if mirror:
...
def _desktop_macos_relaunchable_fixup(
    ...
    "Ulak is damaged" and loses every grant. Clear quarantine xattrs, then sign
    ...
    if exe is None:
        return True
    # exe = .../Ulak.app/Contents/MacOS/Ulak  ->  app bundle = .../Ulak.app
    app = exe.parents[2]
...
def _macos_create_signing_identity(
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="ulak-tcc-"))
    try:
        ...
        def _export_p12(extra_args: list) -> None:
            subprocess.run(
                [
                    ...
                    "-out", str(p12), "-passout", "pass:ulaklocal",
                ],
                capture_output=True, check=True)
...
def _desktop_macos_setup_tcc_identity(identity: str = "Ulak Local Signing") -> bool:
    ...
    print(
        ...
        "stuck, reset it with:  tccutil reset All com.erkanozdemir-labs.ulak"
    )
    return True
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Ulak Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Ulak Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Ulak Desktop (Linux only, best-effort).

    ``Exec`` and ``Icon`` are absolute so the entry works outside a login shell.
    ``ulak uninstall --gui`` removes it.
    """
    from ulak_cli.main import PROJECT_ROOT
    try:
        from ulak_cli.linux_desktop_entry import install_desktop_entry, is_supported
        if not is_supported():
            return
...
def _install_desktop_workspace_deps(npm: str, env: dict) -> None:
    ...
    from ulak_cli.main import PROJECT_ROOT
    from ulak_cli.main_web_build import _run_npm_install_deterministic
    ...
    nixos_env = with_ulak_node_path(_nixos_build_env())
...
def _run_desktop_pack_with_recovery(
    ...
    if build_result.returncode != 0 and staging_dir is not None and _staged_exe() is None:
        ...
        if restored:
            ...
            for p in purged:
                print(f"    - {p}")
            # The purge can't remove a win-unpacked tree whose Ulak.exe is
...
def _promote_staged_desktop_app(desktop_dir: Path, staging_dir: Path) -> Path:
    ...
    # Ulak.exe Windows cannot load. Verified on the STAGED exe, so a failure
...
def _build_desktop_app(desktop_dir: Path, *, source_mode: bool, npm: str, env: dict) -> Optional[Path]:
    ...
    if not source_mode:
        ...
        # A running desktop instance holds Ulak.exe locked on Windows, so the
    ...
    if build_result.returncode != 0:
        ...
        if sys.platform == "win32":
            print("  If this says \"Access is denied\" on Ulak.exe, close any")
            print("  running Ulak desktop window and retry.")
        print("  If the log shows Electron download retries, rebuild via a mirror:")
        print("    ELECTRON_MIRROR=<mirror-base-url> ulak desktop --force-build")
...
def _desktop_launch_env(args: argparse.Namespace) -> tuple[dict, list[str]]:
    ...
    from ulak_constants import with_ulak_node_path
    # with_ulak_node_path() copies os.environ when called with no arg.
    env = with_ulak_node_path()
    for attr, key in (
        ("fake_boot", "ULAK_DESKTOP_BOOT_FAKE"), ("ignore_existing", "ULAK_DESKTOP_IGNORE_EXISTING")):
        if getattr(args, attr, False):
            env[key] = "1"
    if getattr(args, "ulak_root", None):
        env["ULAK_DESKTOP_ULAK_ROOT"] = str(Path(args.ulak_root).expanduser().resolve())
    cwd = getattr(args, "cwd", None)
    env["ULAK_DESKTOP_CWD"] = str(Path(cwd).expanduser().resolve()) if cwd else os.getcwd()

    config_electron_flags, config_disable_gpu, config_password_store, config_ozone_hint = (
        _desktop_launch_options())
    if config_disable_gpu != "auto" and "ULAK_DESKTOP_DISABLE_GPU" not in os.environ:
        env["ULAK_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if sys.platform == "linux" and "ULAK_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
            config_password_store if config_password_store != "auto" else _detect_linux_password_store()
        )
        if password_store:
            env["ULAK_DESKTOP_PASSWORD_STORE"] = password_store
...
def cmd_gui(args: argparse.Namespace):
    ...
    from ulak_cli.main import PROJECT_ROOT
    from ulak_cli.main_install_repair import _resolve_node_runtime_npm
    desktop_dir = PROJECT_ROOT / "apps" / "desktop"
    ...
    if getattr(args, "setup_tcc_identity", False):
        identity = getattr(args, "identity", None) or "Ulak Local Signing"
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  ulak gui")
            sys.exit(1)
    ...
    if source_mode:
        print("→ Launching Ulak Desktop from source build...")
    ...
    if not source_mode:
        print(f"→ Launching packaged Ulak Desktop: {' '.join(launch_command)}")
