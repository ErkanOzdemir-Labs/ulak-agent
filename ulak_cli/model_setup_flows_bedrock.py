...
Imports of ulak_cli.auth / config / models stay lazy (tests patch them at call time).
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    from ulak_cli.auth import _resolve_api_key_provider_secret, ProviderConfig
    from ulak_cli.config import save_env_value
    from ulak_cli.models import _PROVIDER_MODELS
    mantle_base_url = f"https://bedrock-mantle.{region}.api.aws/v1"

    # Check env var and credential pool (keys added via `ulak auth`)
