...
Split out of ``ulak_cli.models``. OpenRouter and
...
``ulak_cli.models`` — tests reset them there — and are read/written by attribute name.
...
# that must never block on HTTP — so without a disk copy, `ulak -p`, a cron job, or a freshly
...
def _reasoning_caps_disk_path() -> Path:
    ...
    return get_ulak_home() / "cache" / "reasoning_caps.json"
...
def _fetch_reasoning_caps_catalog(url: str, timeout: float) -> Optional[Caps]:
    ...
    m = _origin()
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": m._ULAK_USER_AGENT})
...
class _CapsSource:
    """One catalog's cache slots on ``ulak_cli.models`` plus how to name its URL.
...
def refresh_reasoning_caps_async(provider: Optional[str]) -> None:
    ...
    """
    src = {"nous": _NOUS_CAPS, "nous-portal": _NOUS_CAPS, "erkanozdemir-labs": _NOUS_CAPS,
