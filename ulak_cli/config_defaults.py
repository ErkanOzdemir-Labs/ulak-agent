"""Default configuration data for Ulak Agent: DEFAULT_CONFIG and OPTIONAL_ENV_VARS.

Pure-data leaf module — must not import from ulak_cli.config. Comments are the user-facing
docs of config.yaml.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["ulak-cli"],
    # journal_mode: SQLite journal mode for every Ulak DB. "wal" default; use "delete" on
    ...
    "session": {
        # Per-terminal `ulak -c`: each CLI session writes a breadcrumb under
        # $ULAK_HOME/terminal-sessions/<terminal-id>, so bare -c/--continue resumes THIS
    ...
    "agent": {
        ...
        # Default 30 min is a safety valve for wedged agents, not a target latency — an interactive `ulak
        ...
        # Ulak-level retry attempts for API errors (connection drops, timeouts, 5xx) wrapping the
        ...
        # wrapping Ulak (sandbox runner, managed platform) can describe proxy/credential/ mount
        # layout without editing SOUL.md. Env ULAK_ENVIRONMENT_HINT overrides it.
        "environment_hint": "",
        ...
        # share or `ulak doctor` (status channel only; prompt cache untouched). 0 = no escalation
        ...
        # gateway status (`ulak status` / fleet monitoring). Retries never stop — a signal, not a
        ...
        # ULAK_LOCAL_STREAM_STALE_TIMEOUT overrides.
    ...
    "terminal": {
        ...
        # sandboxes). Empty = TMPDIR/TMP/TEMP if set, else ULAK_HOME/cache/terminal (auto-pruned
        ...
        # ULAK_HOME/home; "real" = force real HOME; "profile" = force ULAK_HOME/home when it
        ...
        # ["/home/user/.ulak/cache/documents:/output"]. For gateway MEDIA delivery, write to
        ...
        # default for images whose entrypoints must start as root (e.g. the bundled Ulak image,
        # which drops to `ulak` via s6-setuidgid). When on, SETUID/SETGID caps are omitted.
    ...
    "web": {
        ...
        # parallel, firecrawl, keenable; tavily is opt-in keyless via `ulak tools`, not a ring
        # member). Set by the `ulak tools` picker. "free" = always anonymous endpoint even with a
    ...
    "browser": {
        ...
        # Consent to browse with the user's REAL logins locally: runs on a Ulak-managed SNAPSHOT
        # of the ACTIVE default-Chromium profile (Local State -> profile.last_used; cookies, logins,
        # prefs copied and re-synced per fresh session) driven by Ulak' packaged Chromium. The
        # snapshot dir sidesteps Chrome 136+'s default-profile debugging block and never contends
        # with the running browser. Turning off deletes ~/.ulak/browser-profile/ so credentials
        ...
        # runs `ulak browser close-profile` (kills that profile's browser tree, unsaved tabs lost)
    ...
    # write_file/patch call); restore with /rollback. Opt-in via `ulak chat --checkpoints` or
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ~/.ulak/checkpoints/ size (MB); the oldest checkpoint per project
        ...
        # must not guess. Orphans: `ulak checkpoints prune` (`--keep-orphans` to skip).
        "auto_prune": True,
        "retention_days": 7,
        "min_interval_hours": 24,
    },
    # Hard cap (chars) for one auto-loaded context file (SOUL.md, AGENTS.md, CLAUDE.md, .ulak.md,
    ...
    # Same bound for single-query mode (``ulak -q/-z``). With only ONE turn there is no
    ...
    "compression": {
        ...
        # Ulak' summarizer cannot shrink it. native = codex decides; ulak = Ulak' threshold
    ...
    "auxiliary": {
        ...
        # of skills); route cheaper via `ulak model` → auxiliary → Curator.
        "curator": _aux(600),
    ...
    "display": {
        ...
        # Interface bare `ulak`/`ulak chat` launches: "cli" (prompt_toolkit REPL) | "tui" (Ink).
        # Flags win: `--cli` forces the REPL, `--tui` / ULAK_TUI=1 forces the TUI.
        "interface": "cli",
        # `ulak --tui` auto-resumes the most recent human-facing session (like `ulak -c`).
        # ULAK_TUI_RESUME=<id> always wins.
        ...
        # `path`/`paths` = ULAK_HOME-relative YAML files/dirs (or conventional status_phrases.yaml
        ...
        # Gateway runtime footer on the FINAL message, e.g. `model · 68% · ~/projects/ulak`.
        ...
        # CLI/TUI/desktop, managed with `ulak pets`. No effect on prompt caching.
        "pet": {
            "enabled": False,
            "slug": "",   # active pet slug in get_ulak_home()/pets/; empty → first installed
    ...
    "dashboard": {
        ...
        # client disconnects. 0 = park forever. Env: ULAK_TUI_WS_ORPHAN_REAP_GRACE_S.
        ...
        # newest message older than ULAK_TUI_SESSION_TTL_S, default 6h) with
        ...
        # plugin. Env ULAK_DASHBOARD_OAUTH_CLIENT_ID / ULAK_DASHBOARD_PORTAL_URL win when
        ...
        # no-op. Env ULAK_DASHBOARD_BASIC_AUTH_USERNAME / _PASSWORD_HASH / _PASSWORD / _SECRET /
        ...
        # ULAK_DASHBOARD_DRAIN_SECRET; no-op unless >=256-bit, weak secrets rejected
        # (fail-closed). scope = capability label; min_secret_chars in url-safe-b64 chars.
        "drain_auth": {"scope": "drain", "min_secret_chars": 43},
        # Public URL (env ULAK_DASHBOARD_PUBLIC_URL): full authority (scheme + host + optional
        # prefix, e.g. https://example.com/ulak) for the OAuth redirect_uri; its hostname is
    ...
    "tts": {
        ...
        "piper": {
            ...
            # (~/.ulak/cache/piper-voices/), use_cuda, length_scale (2.0 = twice as slow),
    ...
    # "Hey Ulak" hands-free wake word: always-on, on-device hotword detection that starts a fresh
    ...
    "wake_word": {
        ...
        "openwakeword": {
            # "hey_ulak" | built-in openWakeWord name ("hey_jarvis", "alexa", ...) | path to a
    ...
    # plugins/context_engine/<name>/ or ~/.ulak/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # ~/.ulak/cache/delegation/ with a head+tail window + read_file offset footer, nothing
    ...
    "moa": {
        ...
        # <ulak_home>/moa-traces/<session_id>.jsonl (or trace_dir) for auditing.
        "save_traces": False,
        "trace_dir": "",
    ...
    # and resolved; read-only — creation goes to ~/.ulak/skills/ unless create_dir redirects it.
    "skills": {
        ...
        # ULAK_HOME, scanned alongside the local dir.
        "create_dir": "",
        # In a git checkout, <root>/.ulak/skills/ and <root>/.agents/skills/ load as the
        # highest-precedence tier — ONLY if the root is in trusted_project_dirs. false = no scan, no
        # untrusted-skills notice.
        "project_discovery": True,
        # Trusted project roots; managed by `ulak skills trust` / `untrust`.
        "trusted_project_dirs": [],
        # Substitute ${ULAK_SKILL_DIR} / ${ULAK_SESSION_ID} in SKILL.md content.
        "template_vars": True,
        ...
        # Advisory NVIDIA SkillEvaluator Tier 1 scan on `ulak skills install` (alongside the
        ...
        "write_approval": False,
        # Audit ledger: every skill mutation appends to ~/.ulak/skills/.curator_ledger.jsonl with
        # before/after hashes (blobs under ~/.ulak/.curator_backups/blobs/); powers `ulak
    ...
    # daemon. `ulak curator status` shows the last run.
    "curator": {
        "enabled": True,
        ...
        # aux-model cost. `ulak curator run --consolidate` overrides once.
        "consolidate": False,
        # Also prune bundled built-ins (a suppression list stops `ulak update` restoring them);
        ...
        "prune_builtins": True,
        # TTL purge of skills/.archive/: 0 = never; > 0 lets the explicit `ulak curator purge`
        # delete older archived skills (never automatic; logged in the ledger).
        "archive_ttl_days": 0,
        # Before every real (non-dry-run) pass, snapshot ~/.ulak/skills/ to
        # ~/.ulak/skills/.curator_backups/<utc-iso>/skills.tar.gz (`ulak curator rollback`).
        "backup": {
            "enabled": True,
    ...
    # peerName, sessions, enabled); ulak-specific overrides only here.
    "honcho": {},
    ...
    #   cron job, a -q session (ULAK_INTERACTIVE=1 but nobody to answer), or an unattended
    ...
    "approvals": {
        ...
        # export ULAK_INTERACTIVE=1 (for interactive sudo prompts) but have NO user waiting to answer
        ...
        # Approve" → false. ULAK_TUI_NO_CONFIRM=1 skips the TUI modal.
    ...
    # are given.
    "platform_hints": {},
    # Plugin system. `enabled`/`disabled` lists are written by `ulak plugins enable|disable` and
    ...
    "plugins": {
        ...
        # 2026-09-14 removal date (see COMPAT_MANIFEST.md, `ulak plugins compat`). Stopgap only: the
    ...
    # approvals persist in ~/.ulak/shell-hooks-allowlist.json. Schema + examples:
    # website/docs/user-guide/features/hooks.md.
    "hooks": {},
    # Auto-accept shell-hook registrations without a TTY prompt (also --accept-hooks or
    # ULAK_ACCEPT_HOOKS=1). Gateway/cron/non-interactive runs need one of these to pick up
    # newly-added hooks.
    ...
    "security": {  # Security: pre-exec scanning via tirith plus related guards.
        ...
        # .ulak config) always need human approval, even under yolo. Extra patterns are fnmatch
        ...
        # banner. Add via `ulak doctor --ack <id>`; remove by editing the list. Catalog:
        # ulak_cli/security_advisories.py.
        "acked_advisories": [],
    ...
    "cron": {
        ...
        # an installed provider (plugins/cron_providers/<name>/ or $ULAK_HOME/plugins/ <name>/),
        ...
        "chronos": {
            ...
            "portal_url": "https://portal.github.com/ErkanOzdemir-Labs",
        ...
        # Env override: ULAK_CRON_MAX_PARALLEL.
        ...
        "output_retention": 50,
        # Timeout (seconds) for a no-agent cron script. Env: ULAK_CRON_SCRIPT_TIMEOUT. Keep in
        ...
        # job's dispatch guard forever. Env: ULAK_CRON_SESSION_DB_TIMEOUT. 0 = unlimited.
        "session_db_timeout_seconds": 10,
        # Timeout (seconds) per media attachment send during gateway delivery; large attachments
        # (long TTS audio, big exports) need more than 30s. Env: ULAK_CRON_MEDIA_SEND_TIMEOUT.
    ...
    # promotes dependency-satisfied todos to ready, and fires `ulak -p <assignee> chat -q ...` per
    ...
    "kanban": {
        ...
        # Auto-run the decomposer on Triage tasks every tick. False = manual via `ulak kanban
    ...
    "code_execution": {  # execute_code settings (programmatic tool calls).
        ...
        # relative paths resolve. strict = isolated temp dir with ulak-agent's own python
    ...
    # by tool_search / tool_describe / tool_call bridges and surfaced on demand. Core Ulak tools
    ...
    "logging": {  # File logging to ~/.ulak/logs/: agent.log captures INFO+, errors.log WARNING+.
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://ulak-agent.github.com/ErkanOzdemir-Labs/docs/api/model-catalog.json",
        # Disk cache TTL in minutes. The gateway refreshes in the background on this cadence; the
        # CLI refetches on the next /model or `ulak model` once the cache is older. Network
    ...
    # defaults (200K context, tools on, vision/reasoning off) and get patched. Provider keys: Ulak
    ...
    "monitoring": {
        ...
        "gateway_health_export": {  # Gateway health & diagnostics export.
            ...
            "resource_attributes": {
                "service.name": "ulak-gateway", "deployment.environment.name": "production"
            },
        },
    ...
    "gateway": {  # Gateway settings (messaging platforms: Telegram, Discord, Slack, ...).
        ...
        # to ULAK_GATEWAY_PLATFORM_CONNECT_TIMEOUT, which wins if set explicitly.
        ...
        # run_gateway() bridges these to ULAK_STARTUP_WATCHDOG / ULAK_STARTUP_WATCHDOG_TIMEOUT_S
        # and re-arms the live handle; explicit env wins.
        "startup_watchdog": True,
        "startup_watchdog_timeout_seconds": 300,
        # Keep writing the legacy ~/.ulak/sessions/sessions.json mirror of the routing index
        ...
        # (ULAK_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent AND a wakeUrl is
        ...
        # ULAK_GATEWAY_MAX_STARTS / ULAK_GATEWAY_START_WINDOW_S.
        ...
        # credential/system denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.ulak/.env, auth.json). True:
        # files must be under the Ulak cache, media_delivery_allow_dirs, or fresher than
        # trust_recent_files_seconds — recommended for public-facing gateways so prompt injection
        # can't exfiltrate host secrets. Bridged to ULAK_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra roots (project/scratch dirs, mounted shares) from which bare file paths may be
        # uploaded; the Ulak cache is always trusted. List of absolute paths or one
        # os.pathsep-separated string; tildes expanded. Bridged to ULAK_MEDIA_ALLOW_DIRS. Honored
        # in both modes.
        ...
        # pure-allowlist mode. Bridged to ULAK_MEDIA_TRUST_RECENT_FILES. Only consulted when
        # strict is true.
        "trust_recent_files": True,
        # Recency window in seconds; 600 covers a multi-tool turn. Bridged to
        # ULAK_MEDIA_TRUST_RECENT_SECONDS. Only consulted when strict is true.
    ...
    # Automatic cleanup of ~/.ulak/state.db, which otherwise grows without bound and slows FTS5
    ...
    "sessions": {
        ...
        "auto_prune": True,
        # Inactive days of ended-session history to keep (= `ulak sessions prune`).
        ...
        # processes).
        "min_interval_hours": 24,
        # Legacy ~/.ulak/sessions/session_{sid}.json snapshots rewritten every turn. state.db is
        ...
        # stay until `ulak sessions optimize-storage` runs, since the rebuild is disk-heavy on
        # large DBs. advise = `ulak update` prints a one-line notice with reclaimable size when a
        ...
        # (native/fts5_cjk/build.sh → ~/.ulak/lib/libfts5_cjk.so), 1-2 char CJK terms get exact
        # index matches instead of LIKE scans. True = use when present (inert otherwise); False =
        # never load/serve it. Bridged to ULAK_CJK_FTS.
        "cjk_fts": True,
        ...
        # Max active messages per session for in-memory export (`ulak sessions export`); checked
    ...
    "telemetry": {
        "shared_metrics": {
            ...
            "endpoint": "https://telemetry.github.com/ErkanOzdemir-Labs/v1/telemetry",
        },
    },

    "doctor": {
        # Per-probe timeout (seconds) for `ulak doctor --live` real-call probes.
        "live_probe_timeout": 10,
    },

    "updates": {
        # Pre-update backup. quick = snapshot small critical state (pairing JSONs, cron jobs,
        # config.yaml, .env, auth.json, profile DBs) into <ULAK_HOME>/state-snapshots/, skipping
        # files >1 GiB; restore via ``/snapshot``. full = quick PLUS a ``ulak backup`` zip in
        # <ULAK_HOME>/backups/ (``ulak import`` restores; slow on large homes; ``--backup``
        ...
        # update cleanly. `ulak update --switch-branch` overrides to switch for one run.
        "parked_branch_strategy": "switch",
        # Refresh an installed cua-driver during `ulak update` (best-effort, macOS only). Turn off
    ...
    "lsp": {
        ...
        "wait_mode": "document",
        "wait_timeout": 5.0,
        # Missing server binaries: auto = install via npm/go/pip into <ULAK_HOME>/lsp/bin/ on
    ...
    # (SuperGrok OAuth or XAI_API_KEY) AND the toolset is enabled in `ulak tools`.
    "x_search": {
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # ~/.ulak/.env (or the shell), never in config.yaml.
            ...
            # Last-good fallback for NETWORK/TIMEOUT outages: AES-GCM cache under ~/.ulak/cache/,
            ...
            # Auto-download bws into ~/.ulak/bin/ on first use; False = bws must be on PATH.
            "auto_install": True,
    ...
    "computer_use": {
        # cua-driver's upstream PostHog telemetry defaults ON; Ulak sets
        ...
        # #47032; Linux/WSL2 idle spin #28152). cua-driver ≥ 0.6.x supports --no-overlay; Ulak also calls
    ...
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned binary into ~/.ulak/bin/; False = iron-proxy on PATH.
        "auto_install": True,
    ...
    "desktop": {  # Ulak Desktop (Electron) launch options; only affect `ulak desktop`.
        ...
        # Bridged to ULAK_DESKTOP_DISABLE_GPU: auto = disable GPU only on remote displays
        ...
        # to ULAK_DESKTOP_PASSWORD_STORE; ignored off-Linux.
        "password_store": "auto",
    ...
    "nous": {
        ...
        # ErkanOzdemir-Labs/api#227), so chat is the default until that is fixed.
        "anthropic_wire": "chat",
    },
    ...
    "local_runtime": {
        # Off = detection-only (Ulak still finds an external llama-server you run).
        "enabled": False,
        # Pinned llama.cpp release tag; bumped by Ulak releases after validation.
        "tag": "b10679",
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": _prov(
        ...
        "OAuth2, not a static API key — this points at the credentials Ulak mints short-lived "
    ...
    "AZURE_FOUNDRY_BASE_URL": _prov(
        "Azure Foundry base URL (set via 'ulak model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_DOMAIN": _tool(
        ...
        "hosts, e.g. github.com/ErkanOzdemir-Labs -> firecrawl-gateway.github.com/ErkanOzdemir-Labs",
    ...
    "TOOL_GATEWAY_USER_TOKEN": _tool(
        ...
        "read from the Ulak auth store)", "Tool-gateway user token", None, advanced=True),
    ...
    "PORCUPINE_ACCESS_KEY": _tool(
        "Picovoice access key for the Porcupine 'Hey Ulak' wake word engine (optional; "
    ...
    "ULAK_LANGFUSE_PUBLIC_KEY": _tool("Langfuse project public key (pk-lf-...)",
        "Langfuse public key", "https://cloud.langfuse.com", password=False),
    "ULAK_LANGFUSE_SECRET_KEY": _tool("Langfuse project secret key (sk-lf-...)",
        "Langfuse secret key", "https://cloud.langfuse.com"),
    "ULAK_LANGFUSE_BASE_URL": _tool("Langfuse server URL (default: https://cloud.langfuse.com)",
    ...
    "SLACK_ALLOWED_USERS": _msg(
        "Comma-separated Slack member IDs allowed to use Ulak, e.g. U01ABC2DEF3. Without "
    ...
    "MATRIX_USER_ID": _msg("Matrix user ID (e.g. @ulak:example.org)",
    ...
    "MATRIX_DEVICE_ID": _msg(
        "Stable Matrix device ID for E2EE persistence across restarts (e.g. ULAK_BOT)",
    ...
    "IRC_CHANNEL": _msg("IRC channel to join (e.g. #ulak)", "IRC channel", None),
    "IRC_NICKNAME": _msg("Bot nickname on IRC (default: ulak-bot)", "IRC nickname", None),
    ...
    "API_SERVER_MODEL_NAME": _msg(
        "Model name advertised on /v1/models. Defaults to the profile name (or 'ulak-agent' "
    ...
    "GATEWAY_PROXY_URL": _msg(
        "URL of a remote Ulak API server to forward messages to (proxy mode). When set, the "
        ...
        "Remote Ulak API server URL (e.g. http://192.168.1.100:8642)", None, advanced=True),
    "GATEWAY_PROXY_KEY": _msg(
        "Bearer token for authenticating with the remote Ulak API server (proxy mode). Must "
    ...
    # ULAK_TOOL_PROGRESS_MODE (deprecated; use display.tool_progress) is intentionally NOT listed:
    # this dict feeds user-facing surfaces (dashboard keys page, setup checklists), so deprecated
    # knobs stay in config._EXTRA_ENV_KEYS only. ULAK_TOOL_PROGRESS is unsupported.
    "ULAK_PREFILL_MESSAGES_FILE": _setting(
        "Path to JSON file with ephemeral prefill messages for few-shot priming",
        "Prefill messages file path", None),
    "ULAK_EPHEMERAL_SYSTEM_PROMPT": _setting(
