...
def import_foreign_session(source: str, path, db=None) -> str:
    """Import one foreign session into the Ulak SessionDB; returns the new Ulak session id.
...
def pick_foreign_session(source: Optional[str] = None, *, limit: int = 25) -> Optional[ForeignSession]:
    ...
    if not sys.stdin.isatty():
        print("Non-interactive terminal — pass the file path directly:\n"
              "  ulak sessions import --from claude|codex <path>")
        return None
    try:
...
def run_sessions_import(args, db=None) -> Optional[str]:
    """`ulak sessions import` entry point. Returns new session id or None."""
    ...
    print(f"  Continue it with:  ulak --resume {session_id}")
    return session_id
