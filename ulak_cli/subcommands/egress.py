...
def build_egress_parser(subparsers) -> None:
    ...
    # OUTBOUND egress firewall (iron-proxy); `ulak proxy` is the separate INBOUND
    ...
    egress_parser = subparsers.add_parser(
        ...
        description="Manage iron-proxy, the optional TLS-intercepting egress firewall "
            ...
            "https://ulak-agent.github.com/ErkanOzdemir-Labs/docs/user-guide/egress/iron-proxy")

    from ulak_cli import proxy_cli as _proxy_cli
    _proxy_cli.register_cli(egress_parser)

    def _dispatch_egress(args):  # noqa: ANN001
        # dest='egress_command' stays disjoint from ``ulak proxy`` (dest='proxy_command').
