...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``invalid choice``. Registered WITHOUT ``help=`` so it is omitted from ``ulak --help``
    ...
    login_parser = subparsers.add_parser(
        "login",
        description="Deprecated. Use `ulak auth` to manage credentials, "
    ...
    login_parser.add_argument(
        "--provider", default=None, help="(deprecated) Provider name; ignored — see `ulak model`")
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: ulak-cli)")
