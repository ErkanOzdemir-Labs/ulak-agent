...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Ulak Agent. Use 'ulak debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    ulak debug share              Upload debug report (asks for confirmation)
    ulak debug share --yes        Skip confirmation (for scripts/CI)
    ulak debug share --lines 500  Include more log lines
    ulak debug share --expire 30  Keep paste for 30 days
    ulak debug share --local      Print report locally (no upload)
    ulak debug share --no-redact  Disable upload-time secret redaction
    ulak debug share --nous       Upload to Nous-internal storage (private)
    ulak debug delete <url>       Delete a previously uploaded paste
""")
    ...
    delete_parser = debug_sub.add_parser(
        "delete", help="Delete a paste uploaded by 'ulak debug share'")
