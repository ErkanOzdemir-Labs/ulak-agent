"""``ulak fallback`` subcommand parser."""
...
def build_fallback_parser(subparsers) -> None:
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description="Manage the fallback provider chain.  Fallback providers are tried "
            ...
            "https://ulak-agent.github.com/ErkanOzdemir-Labs/docs/user-guide/features/fallback-providers")
    fallback_subparsers = fallback_parser.add_subparsers(dest="fallback_command")
    fallback_subparsers.add_parser(
        "list", aliases=["ls"], help="Show the current fallback chain (default when no subcommand)")
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `ulak model`) and append to the chain")
    fallback_subparsers.add_parser(
