"""``ulak dashboard`` / ``ulak serve`` subcommand parsers.
...
def _add_server_runtime_args(parser) -> None:
    ...
    # manager / PID file: they scan the process table for `ulak dashboard|serve`
    # cmdlines and SIGTERM them — the same path `ulak update` uses.
    parser.add_argument(
        "--stop", action="store_true", help="Stop all running Ulak web server processes and exit")
    parser.add_argument(
        "--status", action="store_true", help="List running Ulak web server processes and exit")
...
def build_serve_parser(
    *, cmd_dashboard: Callable, add_help: bool = True, exit_on_error: bool = True,
    ...
    parser = argparse.ArgumentParser(
        prog="ulak serve",
        description="Run the Ulak backend server - the JSON-RPC/WebSocket gateway the "
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Ulak Agent web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically")
    # Compat shim: desktop shells <= 0.15.x spawn `ulak dashboard --no-open --tui ...`;
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Ulak backend server (headless; powers the desktop app and remote backends)",
        description="Run the Ulak backend server — the JSON-RPC/WebSocket gateway the "
            "desktop app and remote clients connect to. Headless: it never opens "
            "a browser UI.")
    _configure_serve_parser(serve_parser, cmd_dashboard=cmd_dashboard)

    # `register` is nested so bare `ulak dashboard` keeps launching the server.
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description="Register this install as a self-hosted dashboard with your Nous "
            ...
            "ULAK_DASHBOARD_OAUTH_CLIENT_ID into ~/.ulak/.env, and prints "
            "how to engage the login gate. Requires being logged in (ulak setup).")
    ...
    dashboard_register_parser.add_argument(
        ...
        help="Optional public HTTPS OAuth redirect URI for the dashboard, e.g. "
            "https://ulak.example.com/auth/callback. Omit for localhost-only use.")
    dashboard_register_parser.add_argument(
        ...
        help="Override the Nous Portal base URL for registration (default: the "
            ...
            "portal. Also settable via ULAK_DASHBOARD_PORTAL_URL. Mainly for "
