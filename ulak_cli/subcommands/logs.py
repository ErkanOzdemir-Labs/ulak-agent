...
def build_logs_parser(subparsers, *, cmd_logs: Callable) -> None:
    ...
    logs_parser = subparsers.add_parser(
        "logs", help="View and filter Ulak log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log / gui.log / desktop.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    ulak logs                    Show last 50 lines of agent.log
    ulak logs -f                 Follow agent.log in real time
    ulak logs errors             Show last 50 lines of errors.log
    ulak logs gateway -n 100     Show last 100 lines of gateway.log
    ulak logs gui -f             Follow gui.log in real time
    ulak logs desktop -f         Follow desktop.log (Electron app boot/backend)
    ulak logs --level WARNING    Only show WARNING and above
    ulak logs --session abc123   Filter by session ID
    ulak logs --component tools  Only show tool-related lines
    ulak logs --since 1h         Lines from the last hour
    ulak logs --since 30m -f     Follow, starting from 30 min ago
    ulak logs list               List available log files with sizes
""")
