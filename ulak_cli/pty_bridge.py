"""PTY bridge for `ulak dashboard` chat tab.
...
``ulak_cli.web_server``. POSIX-only: depends on ``fcntl``, ``termios`` and ``ptyprocess`` (native
...
# ui-tui/packages/ulak-ink/src/ink/termio/host.ts — keep the two in sync.
PTY_HOST_ENV = "ULAK_PTY_HOST"
        ...
        if not _PTY_AVAILABLE:
            if sys.platform.startswith("win"):
                raise PtyUnavailableError("Pseudo-terminals are unavailable on this platform. "
                                          "Ulak Agent supports Windows only via WSL.")
