...
resolved through the module object so test patches on ``ulak_cli.setup.<name>`` take effect."""

import logging
from tools import tool_backend_helpers
from ulak_cli import nous_subscription
...
_TTS_SUMMARY_ROWS = {
    ...
    "neutts": ("NeuTTS", "neutts", "run 'ulak setup tts'"),
    "kittentts": ("KittenTTS", "kittentts", "run 'ulak setup tts'")}
...
_STT_SUMMARY_DEFAULT = ("Local Whisper", "faster_whisper", "run 'ulak tools' → Speech-to-Text")
...
_EDIT_WIZARD_ROWS = (
    ("ulak setup", "          Re-run the full wizard"), ("ulak setup model", "    Change model/provider"),
    ("ulak setup terminal", " Change terminal backend"), ("ulak setup gateway", "  Configure messaging"),
    ("ulak setup tools", "    Configure tool providers"))
_EDIT_CONFIG_ROWS = (
    ("ulak config", "         View current settings"), ("ulak config edit", "    Open config in your editor"),
    ("ulak config set <key> <value>", ""))
_READY_ROWS = (
    ("ulak", "              Start chatting"), ("ulak gateway", "      Start messaging gateway"),
    ("ulak doctor", "       Check for issues"))
...
def _vision_row(config, feats):
    ...
    try:
        ...
        ok = False
    return ("Vision (image analysis)", ok, None if ok else "run 'ulak setup' to configure")
...
def _video_gen_row(config, feats):
    # Opt-in via `ulak tools` → Video Generation. Only show the row when a plugin reports
...
def _modal_row(config, feats):
    ...
    if _setup.cfg_get(config, "terminal", "backend") == "modal":
        ...
        return ("Modal Execution", False, "run 'ulak setup terminal'")
...
def _spotify_row(config, feats):
    # OAuth via ulak auth spotify — check auth.json, not env vars
    try:
...
def _print_setup_summary(config: dict, ulak_home):
    ...
    try:
        ...
        print()
        _setup.print_warning("No inference provider is configured — Ulak cannot chat yet.")
        _setup._info("  Finish this one step with either of:",
              "    ulak model            (pick any provider/model)",
              "    ulak setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if available_count < len(tool_status):
        _setup.print_warning("Some tools are disabled. Run 'ulak setup tools' to configure them,")
    ...
    for label, value in (("Settings:", f"  {_setup.get_config_path()}"), ("API Keys:", f"  {_setup.get_env_path()}"),
                         ("Data:", f"      {ulak_home}/cron/, sessions/, logs/")):
...
import ulak_cli.setup as _setup  # noqa: E402  (bottom: ulak_cli.setup imports this module)
