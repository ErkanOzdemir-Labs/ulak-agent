"""Top-level argparse construction for the ulak CLI.

Only the top-level parser and the ``chat`` subparser live here. Every other subparser (model,
gateway, sessions, …) is built by ``ulak_cli/subcommands/<group>.py`` and wired in
...
# (it sets ``ULAK_HOME`` and strips itself from ``sys.argv``), so it isn't on the parser.
...
# live parser fails. The derived path is authoritative; tests/ulak_cli/
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    argparse surface — the drift that made ``ulak --reasoning high chat …`` misread ``high`` as
...
def _inherited_flag(parser, *args, **kwargs):
    """``parser.add_argument`` + tag the Action ``inherit_on_relaunch`` for ``ulak_cli.relaunch``."""
...
_EPILOGUE = """
Examples:
    ulak                        Start interactive chat
    ulak chat -q "Hello"        Single query mode
    ulak --tui                  Launch the modern TUI (or set display.interface: tui)
    ulak --cli                  Force the classic REPL (overrides display.interface: tui)
    ulak -c                     Resume the most recent session
    ulak -c "my project"        Resume a session by name (latest in lineage)
    ulak --resume <session_id>  Resume a specific session by ID
    ulak --resume latest        Resume the most recent session (same as -c)
    ulak --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    ulak setup                  Run setup wizard
    ulak logout                 Clear stored authentication
    ulak auth add <provider>    Add a pooled credential
    ulak auth list              List pooled credentials
    ulak auth remove <p> <t>    Remove pooled credential by index, id, or label
    ulak auth reset <provider>  Clear exhaustion status for a provider
    ulak model                  Select default model
    ulak fallback [list]        Show fallback provider chain
    ulak fallback add           Add a fallback provider (same picker as `ulak model`)
    ulak fallback remove        Remove a fallback provider from the chain
    ulak config                 View configuration
    ulak config edit            Edit config in $EDITOR
    ulak config set model gpt-4 Set a config value
    ulak gateway                Run messaging gateway
    ulak -s ulak-agent-dev,github-auth
    ulak -w                     Start in isolated git worktree
    ulak gateway install        Install gateway background service
    ulak sessions list          List past sessions
    ulak sessions browse        Interactive session picker
    ulak sessions rename ID T   Rename/title a session
    ulak logs                   View agent.log (last 50 lines)
    ulak logs -f                Follow agent.log in real time
    ulak logs errors            View errors.log
    ulak logs --since 1h        Lines from the last hour
    ulak debug share             Upload debug report for support
    ulak console                Open the safe Ulak command console
    ulak update                 Update to latest version
    ulak dashboard              Start web UI dashboard (port 9119)
    ulak dashboard --stop       Stop running dashboard processes
    ulak dashboard --status     List running dashboard processes
...
def _add_top_level_flags(parser: argparse.ArgumentParser) -> None:
    ...
    inherited(parser, "-m", "--model", default=None, help=(
        ...
        "Applies to -z/--oneshot and --tui. Also settable via ULAK_INFERENCE_MODEL env var."))
    inherited(parser, "--provider", default=None, help=(
        "Provider override for this invocation (e.g. openrouter, anthropic). "
        "Applies to -z/--oneshot and --tui. The persistent provider lives in config.yaml "
        "under model.provider — use `ulak setup` or edit the file to change it."))
    ...
    inherited(parser, "--accept-hooks", action="store_true", default=False, help=(
        ...
        "without a TTY prompt.  Equivalent to ULAK_ACCEPT_HOOKS=1 or "
    ...
    inherited(parser, "--ignore-user-config", action="store_true", default=False,
              help="Ignore ~/.ulak/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)")
...
def _build_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    ``ulak -m foo chat`` argparse first sets ``args.model`` from the top-level parser, then
    ...
    unless the flag is actually passed after the subcommand (tests/ulak_cli/
    test_argparse_flag_propagation.py).
    """
    chat_parser = subparsers.add_parser(
        "chat", help="Interactive chat with the agent",
        description="Start an interactive chat session with Ulak Agent")
    ...
    inherited(chat_parser, "--accept-hooks", action="store_true", default=SUPPRESS, help=(
        "Auto-approve any unseen shell hooks declared in config.yaml "
        "without a TTY prompt (see also ULAK_ACCEPT_HOOKS env var and "
    ...
    inherited(chat_parser, "--ignore-user-config", action="store_true", default=SUPPRESS,
              help="Ignore ~/.ulak/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.")
    ...
    inherited(chat_parser, "--safe-mode", action="store_true", default=SUPPRESS,
              help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Ulak itself.")
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="ulak", description="Ulak Agent - AI assistant with tool-calling capabilities",
