"""Gateway subcommand for ulak CLI.

Handles: ulak gateway [run|start|stop|restart|status|install|uninstall|setup]
"""

import asyncio
import contextlib
from ulak_cli.cli_output import line_input
import json
import logging
import os
import shlex
...
from ulak_cli import setup_platforms
...
# display_ulak_home is imported lazily: ulak_constants may be a cached pre-update version.
...
from ulak_cli.colors import Colors, color
...
def _get_service_pids(all_profiles: bool = False) -> set:
    ...
    ``all_profiles`` widens the current profile's unit/label to the whole ``ulak-gateway*`` /
    ``ai.ulak.gateway*`` fleet so update/reaper never kill a sibling's service gateway as "manual".

    ``all_profiles`` widens the launchd branch to every installed ``ai.ulak.gateway*`` LaunchAgent — the
    ...
    the ``ulak-gateway*`` fleet glob.
    """
    pids: set = set()

    # --- systemd (Linux): user and system scopes ---
    if supports_systemd_services():
        pattern = "ulak-gateway*" if all_profiles else get_service_name()
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                # Belt-and-suspenders for the EXCLUDE use case (#74075): a bare ``launchctl list`` prefix
                # scan also catches ai.ulak.gateway* agents the label derivation can't map (renamed
    ...
    if is_macos():
        ...
        if all_profiles:
            # Whole fleet, mirroring the systemd ``ulak-gateway*`` glob above.
            # Every gateway LaunchAgent, not just the invoking profile's — mirrors the systemd branch's
            # ``ulak-gateway*`` pattern above. The update path restarts the whole fleet, and its
        ...
        for label in sorted(labels):
            ...
            if pid is not None and pid > 0:
                pids.add(pid)
        if all_profiles:
            # Prefix scan also catches ai.ulak.gateway* agents the label derivation can't map
            ...
            try:
                ...
                if result.returncode == 0:
                    for line in result.stdout.strip().splitlines():
                        parts = line.split()
                        if len(parts) >= 3 and parts[-1].startswith("ai.ulak.gateway"):
                            try:
                                pid = int(parts[0])
                                if pid > 0:
                                    pids.add(pid)
...
# its full budget and `ulak update` can deadlock. Two witnesses classify the loop BEFORE any
...
# fail", and `ulak update` can deadlock behind it. The loop publishes a liveness signal precisely for this
...
def _get_ancestor_pids() -> set[int]:
    """PIDs of this process and its ancestors, so scans never count the invoking ``ulak`` CLI as a gateway.

    Walks from the current PID up to PID 1 (init) so that process-table scans never match the calling CLI
    process or any of its parents. This prevents ``ulak gateway status`` from falsely counting the
    ``ulak`` CLI that invoked it as a running gateway instance (see #13242).
    """
...
def _scan_gateway_pids(
    ...
    # Exclude the entire ancestor chain so the CLI process that invoked this scan (e.g. ``ulak gateway
    ...
    current_home = str(get_ulak_home().resolve())
    # Forward slashes on both sides of the ULAK_HOME= match (mirrors gateway.status).
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"ulak_home={current_home_lc}" in command_lc
            )

        # Default profile: accept unless argv advertises another profile. ULAK_HOME may come via
        # env (invisible to wmic/CIM), so only a non-matching explicit ULAK_HOME= disqualifies.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        return not ("ulak_home=" in command_lc and f"ulak_home={current_home_lc}" not in command_lc)
...
def _windows_process_listing() -> str | None:
    ...
    # failure or timeout (result is None) trips the fallback. ``ulak update`` hung exactly there on
    ...
    from ulak_cli._subprocess_compat import bounded_probe_run
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    """Find running gateway PIDs for the current profile, or every profile with ``all_profiles`` (``ulak update``)."""
...
def find_profile_gateway_processes(exclude_pids: set | None = None, *, strict: bool = False) -> list[ProfileGatewayProcess]:
    """Return running gateway PIDs mapped to Ulak profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid, get_running_pid_identity_strict
        from ulak_cli.profiles import list_profiles
    except Exception:
        if strict:
            raise
        return processes
...
def find_windows_gateway_services(
    *, psutil_module=None, profile_processes: list[ProfileGatewayProcess] | None = None
    ...
    command lines, so identity = Ulak's own PID file + a parent chain ending at a running SCM service
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "ulak_cli.main"]
    if profile != "default":
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``ulak update``: ``--external-supervisor`` gateways
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    if old_pid <= 0 or not run_argv:
        return False
    from ulak_cli._subprocess_compat import windows_detach_flags_without_breakaway, windows_detach_popen_kwargs
    ...
    # normalizes the interpreter and captures a stable cwd + env overlay (ULAK_HOME,
    ...
    if sys.platform == "win32":
        try:
            from ulak_cli.gateway_windows import windowless_gateway_restart_spec
    ...
    watcher = textwrap.dedent(
        ...
        import sys
        import time
        from ulak_cli._subprocess_compat import (
        ...
        try:
            ...
            from pathlib import Path
            _log_dir = Path(get_ulak_home()) / "logs"
        ...
        # ULAK_HOME) the windowless base interpreter needs to import ulak_cli. Empty on POSIX.
        if _respawn_cwd:
...
def _ulak_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """``ULAK_HOME`` from the on-disk unit file — what refresh/compare already read, and reliable under ``sudo``."""
    ...
    for line in text.splitlines():
        ...
        if body.startswith("Environment="):
            ...
            if body.startswith("ULAK_HOME="):
                return body.split("=", 1)[1].strip().strip('"') or None
    return None


def _sync_ulak_home_from_systemd_unit(system: bool) -> None:
    """Adopt a system-scope unit's ``ULAK_HOME``: under ``sudo`` it is stripped and HOME=/root, so
    get_ulak_home() would pick the wrong profile for runtime-status/PID reads."""
    if not system:
        return
    # On-disk unit first; ``systemctl show`` for units that only exist in the manager.
    unit_home = (_ulak_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        env_line = _systemctl_show(("Environment",), system=True).get("Environment", "")
        unit_home = _parse_kv_pairs(env_line.split()).get("ULAK_HOME", "").strip()
    if unit_home and os.environ.get("ULAK_HOME", "").strip() != unit_home:
        os.environ["ULAK_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {sudo}ulak gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {sudo}ulak gateway restart{scope_flag}")
...
def _s6_gateway_snapshot(gateway_pids: tuple[int, ...]) -> GatewayRuntimeSnapshot | None:
    ...
    from ulak_cli.service_manager import detect_service_manager, get_service_manager
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from ulak_constants import is_container
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: ulak gateway stop")
    else:
        print("⚠ Gateway process is running for this profile, but the service is not active")
        print(pids_line)
        print("  This is usually a manual foreground/tmux/nohup run, so `ulak gateway`")
        print("  can refuse to start another copy until this process stops.")


def _print_other_profiles_gateway_status() -> None:
    """Print other profiles' running gateways at the bottom of ``ulak gateway status``."""
    try:
        from ulak_cli.profiles import get_active_profile_name
...
def _gateway_list() -> None:
    ...
    try:
        from ulak_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    if is_windows():
        try:
            from ulak_cli.gateway_windows import get_task_name  # profile-aware task name
            _task_name = get_task_name()
        except Exception:
            _task_name = "Ulak_Gateway"
...
def _reaper_exclusion_pids(extra_exclude: set | None) -> set[int]:
    ...
    with contextlib.suppress(Exception):
        ...
        # cover the whole ai.ulak.gateway* fleet — not just the current profile's label — or a sibling
...
from ulak_constants import is_container, is_termux, is_wsl
...
def _gw_windows():
    """Lazily import :mod:`ulak_cli.gateway_windows` (Windows-only service backend)."""
    from ulak_cli import gateway_windows
...
def _gateway_detached_env() -> bool:
    return _truthy_env(os.getenv("ULAK_GATEWAY_DETACHED"))
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    """True for detached Windows gateway runs that should ignore Ctrl+C (``ULAK_GATEWAY_DETACHED=1``
...
def _windows_gateway_breakaway_state() -> bool | None:
    ...
    if not is_windows():
        return None
    from ulak_cli._subprocess_compat import _WINDOWS_GATEWAY_BREAKAWAY_ENV
...
_SERVICE_BASE = "ulak-gateway"
SERVICE_DESCRIPTION = "Ulak Agent Gateway - Messaging Platform Integration"
...
def _profile_suffix() -> str:
    """Service-name suffix for ULAK_HOME: "" for the default root, the profile name for
    ``<root>/profiles/<name>``, else a short hash of the path."""
    import hashlib
    from ulak_constants import get_default_ulak_root
    home = get_ulak_home().resolve()
    default = get_default_ulak_root().resolve()
    if home == default:
        return ""
    # Fallback: short hash for arbitrary ULAK_HOME paths
    return _profile_name_from_home(home, default) or hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(ulak_home: str | None = None, default_root: str | Path | None = None) -> str:
    """``--profile <name>`` for ``<root>/profiles/<name>``, else "". *ulak_home*/*default_root* let a
    sudo/root process generate a unit for another user (the defaults would refer to root)."""
    from ulak_constants import get_default_ulak_root
    home = Path(ulak_home or str(get_ulak_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_ulak_root().resolve()
    if home == default:
        return ""
...
def get_service_name() -> str:
    """Systemd service name: ``ulak-gateway`` for default ULAK_HOME, ``ulak-gateway-<profile>``
    or ``-<hash>`` otherwise."""
...
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("ulak.service",)
...
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "ulak_cli.main gateway",
    "ulak_cli/main.py gateway",
    "gateway/run.py",
    " ulak gateway ",
    "/ulak gateway ",
)
...
def _find_legacy_ulak_units() -> list[tuple[str, Path, bool]]:
    """``[(unit_name, unit_path, is_system)]`` for legacy gateway units (e.g. ``ulak.service``), which
    ...
    Detects unit files installed by older Ulak versions that used a different service name (e.g. When both
    a legacy unit and the current ``ulak-gateway.service`` are active, they fight over the same bot token
...
def has_legacy_ulak_units() -> bool:
    """Return True when any legacy Ulak gateway unit files exist."""
    return bool(_find_legacy_ulak_units())


def print_legacy_unit_warning() -> None:
    """Warn about installed legacy gateway units; prints nothing when there are none."""
    legacy = _find_legacy_ulak_units()
    if not legacy:
        return
    print_warning("Legacy Ulak gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        print_info(f"    {path}  ({_service_scope_label(is_system)} scope)")
    print_info("  These run alongside the current ulak-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    ulak gateway migrate-legacy")


def remove_legacy_ulak_units(interactive: bool = True, dry_run: bool = False) -> tuple[int, list[Path]]:
    ...
    legacy = _find_legacy_ulak_units()
    if not legacy:
        print("No legacy Ulak gateway units found.")
        return 0, []

    print()
    print("Legacy Ulak gateway unit(s) found:")
    for name, path, is_system in legacy:
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: ulak gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo ulak gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    ulak gateway uninstall")
    print_info("    sudo ulak gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: ulak gateway install"
            )
            return scope, False
...
def ensure_gateway_service(context: str = "setup") -> bool:
    """Install and start a user-scope gateway service without prompting (``ulak setup``/``import``).
    ...
    if is_container():
        ...
        print_info("   ulak gateway run          # Run as container main process")
        print_info("")
    ...
    if not (supports_systemd or is_macos() or is_windows()):
        ...
        print_info("  Run the gateway in the foreground with: ulak gateway")
        return False
    ...
    try:
        ...
        print_info("  You can retry manually: ulak gateway install")
...
def get_launchd_plist_path() -> Path:
    """``~/Library/LaunchAgents/ai.ulak.gateway[-<profile>].plist`` under the real account home."""
    import pwd
    suffix = _profile_suffix()
    name = f"ai.ulak.gateway-{suffix}" if suffix else "ai.ulak.gateway"
...
def launchd_gateway_labels_for_install() -> list[str]:
    ...
    the profile layout, NOT by globbing ``~/Library/LaunchAgents``, so a sandboxed ULAK_HOME never
    ...
    for profile in list_profiles():
        if profile.is_default:
            root_label.append("ai.ulak.gateway")
        elif _re.match(r"^[a-z0-9][a-z0-9_-]{0,63}$", profile.name):
            profile_labels.append(f"ai.ulak.gateway-{profile.name}")
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            from ulak_constants import venv_python_path
        except ImportError:
            # Update-boundary: a gateway restarted mid-update can hold a stale ulak_constants
            # without this symbol; see _reload_ulak_constants() in ulak_cli/managed_uv.py.
            from ulak_cli.managed_uv import _reload_ulak_constants
            venv_python_path = _reload_ulak_constants().venv_python_path
...
def _ulak_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current ULAK_HOME (root's, under sudo) to the target user's equivalent:
    ``/root/.ulak[/profiles/x]`` → ``/home/alice/.ulak[/profiles/x]``; custom paths kept as-is."""
    current_ulak_raw = os.environ.get("ULAK_HOME", "").strip()
    current_ulak = Path(current_ulak_raw).expanduser() if current_ulak_raw else get_ulak_home()
    # Keep paths lexical: resolving a non-existent path can bake a different ULAK_HOME into the unit.
    current_default = Path.home() / ".ulak"
    target_default = Path(target_home_dir) / ".ulak"
    try:
        # Default ~/.ulak or a profile/subdir of it → preserve the relative structure under the target.
        return str(target_default / current_ulak.relative_to(current_default))
    except ValueError:
        return str(current_ulak)  # Completely custom path (not under ~/.ulak) — keep as-is
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    ulak_home = get_ulak_home()
    extras = (project_root / "node_modules" / ".bin", ulak_home / "node" / "bin", ulak_home / "node_modules" / ".bin")
    for extra in extras:
        if _is_dir(extra):
...
def _stable_service_working_dir() -> str:
    """WorkingDirectory that won't disappear under systemd (ULAK_HOME, else PROJECT_ROOT). cwd is
...
def _systemd_watchdog_seconds(ulak_home: str | Path | None = None) -> int:
    ...
    if ulak_home is not None:
        from ulak_constants import (reset_ulak_home_override, set_ulak_home_override)
        override_token = set_ulak_home_override(ulak_home)
        reset_home_override = reset_ulak_home_override
    try:
...
def _append_node_dir_for_service(path_entries: list[str], ulak_root: Path | None = None) -> None:
    """Append the Node dir a service unit should use: managed ``<ulak_root>/node`` (profile-scoped)
    first — a unit survives reboots, so baking a shell-PATH Node is permanent breakage — else PATH lookup."""
    from ulak_constants import (ulak_managed_node_tree_present, iter_ulak_node_dirs)
    managed_node_present = ulak_managed_node_tree_present(ulak_root)
    for directory in iter_ulak_node_dirs(ulak_root) if managed_node_present else ():
        entry = str(directory)
        try:
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        ulak_home = _ulak_home_for_target_user(home_dir)
        # Profile arg relative to the TARGET user's ~/.ulak when ulak_home lives under it.
        target_root = Path(home_dir) / ".ulak"
        try:
            Path(ulak_home).resolve().relative_to(target_root.resolve())
            profile_arg = _profile_arg(ulak_home, default_root=target_root)
        except ValueError:
            profile_arg = _profile_arg(ulak_home)
        # Remap paths under the calling user's home (/root/) to the target user's so the service can read them.
        python_path = _remap_path_for_user(python_path, home_dir)
        working_dir = str(ulak_home) if ulak_home else _remap_path_for_user(working_dir, home_dir)
        ...
        _append_node_dir_for_service(_target_node_entries, Path(ulak_home) if ulak_home else None)
        ...
        ulak_home = str(get_ulak_home().resolve())
        profile_arg = _profile_arg(ulak_home)
        user_home = Path.home()
        identity_lines = env_lines = ""
        wanted_by = "default.target"

    watchdog_seconds = _systemd_watchdog_seconds(ulak_home)
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}{identity_lines}ExecStart={python_path} -m ulak_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
{env_lines}Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="ULAK_HOME={ulak_home}"
Environment="ULAK_SUPERVISED_CHILD=1"
Restart=always
RestartSec=5
...
def _normalize_launchd_plist_for_comparison(text: str) -> str:
    ...
    import re
    return re.sub(
        r"(<key>PATH</key>\s*<string>)(.*?)(</string>)", r"\1__ULAK_PATH__\3",
        _normalize_service_definition(text), flags=re.S,
    )


def systemd_unit_is_current(system: bool = False) -> bool:
    # ULAK_HOME sync chokepoint for every compare/regenerate path: under `sudo … --system` it is often
    # stripped to /root/.ulak, so refresh would rewrite a correct unit and status warn forever.
    # Idempotent; the os.environ mutation persists for later runtime reads (restart's PID/drain).
    _sync_ulak_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Temp-dir ULAK_HOME baked into a systemd unit / launchd plist, or None. A temp home means a
    test/E2E harness generated it; installing it leaves the gateway "running" but deaf to every platform."""
    import re
    import tempfile
    candidates = re.findall(r'ULAK_HOME=([^"\n]+)', definition)
    candidates += re.findall(r"<key>ULAK_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S)
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp ULAK_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(f"✗ Refusing to write the gateway {kind}: ULAK_HOME resolves to a temporary directory ({temp_home}).")
    print(
        "  This usually means a test/E2E environment exported ULAK_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # systemd_unit_is_current is the ULAK_HOME-sync chokepoint; its env mutation persists for the regenerate below.
    ...
    # pytest-tmp ULAK_HOME baked into the developer's real unit breaks their gateway on next reboot.
    if not system and any(m in new_unit for m in ("/pytest-of-", '/ulak_test"', "/ulak_test/")):
        return False

    # Structural variant: refuse ANY temp-dir ULAK_HOME (manual E2E homes lack the pytest markers).
    if _refuse_temp_home_service_write(new_unit, "systemd unit"):
        return False

    unit_path.write_text(new_unit, encoding="utf-8")
    _run_systemctl(["daemon-reload"], system=system, check=True, timeout=30)
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Ulak install")
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo ulak gateway uninstall --system")
    print_info("         ulak gateway install")
    print_info("         ulak gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("ULAK_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def _get_cron_drain_timeout() -> float:
    ...
    See #82161.
    """
    return _agent_timeout_setting("ULAK_CRON_DRAIN_TIMEOUT", "cron_drain_timeout", parse_cron_drain_timeout)
...
def _get_restart_exit_wait_budget() -> float:
    ...
    return resolve_restart_exit_wait_budget(
        ...
        _agent_timeout_setting(
            "ULAK_RESTART_AFTER_TURN_TIMEOUT", "restart_after_turn_timeout", parse_restart_after_turn_timeout
        ),
    )
...
def systemd_install(
    ...
    # Existing system units already pin ULAK_HOME; adopt it before any regenerate.
    if unit_path.exists():
        _sync_ulak_home_from_systemd_unit(system=system)
    ...
    print()
    print("Next steps:")
    print(f"  {sudo}ulak gateway start{scope_flag}              # Start the service")
    print(f"  {sudo}ulak gateway status{scope_flag}             # Check status")
...
def _print_service_not_installed(system: bool) -> None:
    ...
    print(f"  Run: {sudo}ulak gateway install{scope_flag}")
...
def systemd_start(system: bool = False):
    ...
    # ULAK_HOME sync happens in refresh's systemd_unit_is_current gate; the unit is guaranteed to exist here.
    refresh_systemd_unit_if_needed(system=system)
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `ulak gateway status` or logs for final shutdown state."
        )
        return
...
def systemd_restart(system: bool = False):
    ...
    # ULAK_HOME sync happens in refresh's systemd_unit_is_current gate; its os.environ mutation
    ...
    if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
        ...
        print(
            ...
            # graceful shutdown, so waiting the full drain budget only stalls the restart (and `ulak
...
def _systemd_reset_and_run(action: str, *, system: bool, previous_pid) -> None:
    ...
    try:
        ...
        print(
            ...
            "check `ulak gateway status` or logs for final state."
        )
        return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {sudo}ulak gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if result.stdout.strip() == "active":
        ...
        print(f"  Run: {sudo}ulak gateway start{scope_flag}")
    ...
    if active_state == "activating" and unit_props.get("SubState", "") == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {sudo}ulak gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {user_flag}reset-failed {svc} && {sudo}ulak gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.ulak.gateway-{suffix}" if suffix else "ai.ulak.gateway"
...
# at all and we degrade to a detached background process (the `nohup ulak gateway run` workaround). See
# #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_ulak_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_ulak_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build ``python -m ulak_cli.main [--profile X] gateway run --replace``, honoring the active profile."""
    return [get_python_path(), "-m", "ulak_cli.main", *_profile_arg().split(), "gateway", "run", "--replace"]
...
def _timestamped_stderr_gateway_command(error_log: Path, *, external_supervisor: bool = False) -> list[str]:
    ...
    (launchd ProgramArguments only) adds ``--external-supervisor`` so ``ulak update`` hands back to
    ...
    ``--external-supervisor`` so ``ulak update`` sees the flag on the live grandchild argv and hands the
    ...
    return [get_python_path(), "-m", "ulak_cli.stderr_timestamp", "--error-log", str(error_log), "--", *inner]
...
def _spawn_detached_gateway() -> bool:
    ...
    the `nohup ulak gateway run --replace` workaround but keeps it CLI-managed: stdout goes to
    ...
    """
    from ulak_cli._subprocess_compat import windows_detach_popen_kwargs
    log_dir = get_ulak_home() / "logs"
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    from ulak_constants import display_ulak_home as _dhh
    ...
    print(f"  Try manually: nohup ulak gateway run --replace > {_dhh()}/logs/gateway.log 2>&1 &")
    if exit_on_failure:
        sys.exit(1)
    return False
...
def generate_launchd_plist() -> str:
    ...
    nofile_block = ""
    try:
        from ulak_cli.resource_limits import configured_nofile_soft_limit
...
<dict>
    ...
    <dict>
        ...
        <key>ULAK_HOME</key>
        <string>{ulak_home}</string>
        <key>ULAK_SUPERVISED_CHILD</key>
        <string>1</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Ulak install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  ulak gateway status             # Check status")
    from ulak_constants import display_ulak_home as _dhh
...
def launchd_stop():
    ...
    _mark_planned_stop()
    # bootout unloads the definition so KeepAlive doesn't respawn; `ulak gateway start` re-bootstraps.
    try:
...
def _wait_for_gateway_exit(timeout: float = 10.0, force_after: float | None = 5.0) -> bool:
    ...
    ULAK_HOMEs work) to exit; SIGKILL it after ``force_after`` s of graceful waiting."""
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
            ...
            # only stalls the restart (and `ulak update`). Bounded SIGTERM → SIGKILL, ~10s.
...
def launchd_status(deep: bool = False):
    ...
    # Ulak PID may be a detached fallback process; when launchd IS supervising both PIDs match — don't double-count.
    from gateway.status import get_running_pid
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Ulak install")
    else:
        print("⚠ Service definition is stale relative to the current Ulak install")
        print("  Run: ulak gateway start")
    ...
    if not service_listed:
        ...
        if fallback_pid:
            ...
            print("  Cron jobs will fire. Stop with: ulak gateway stop")
        else:
    ...
    if deep:
        log_file = get_ulak_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _is_official_docker_checkout() -> bool:
    return str(PROJECT_ROOT) == "/opt/ulak" and (PROJECT_ROOT / "docker" / "entrypoint.sh").is_file()
...
def _running_under_gateway_supervisor() -> bool:
    ...
    (shells inherit "0"), s6 ULAK_S6_SUPERVISED_CHILD, or ``--external-supervisor``."""
...
def named_profile_served_by_running_multiplexer(profile_name: str | None = None) -> bool:
    ...
    try:
        from ulak_constants import get_default_ulak_root
        default_root = get_default_ulak_root()
    except Exception:
        return False
    ...
    try:
        ...
        cfg = {}
        if cfg_path.exists():
            from ulak_cli.config import read_user_config_raw
        ...
        from ulak_cli.profiles import normalize_profile_name
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    "already running". Same user-facing contract; never scans other ULAK_HOME roots."""
    ...
    try:
        ...
        return
    if pid is None:
        # get_running_pid() filters by the current profile's ULAK_HOME; warn if the PID file
        ...
        try:
            ...
            if stale is not None and not _pid_record_belongs_to_current_profile(stale):
                logger.warning(
                    "PID file belongs to another profile (ulak_home=%s). "
                    "The old gateway may still be running under that profile.",
                    stale.get("ulak_home", "<unknown>"),
                )
        except Exception:
            pass
        return
    ...
    print("  Use 'ulak gateway restart' to replace it,")
    print("  or 'ulak gateway stop' first.")
    print("  Or use 'ulak gateway run --replace' to auto-replace.")
    sys.exit(1)


def _guard_official_docker_root_gateway() -> None:
    """Refuse gateway startup when the official Docker privilege drop was bypassed."""
    if not hasattr(os, "geteuid") or os.geteuid() != 0 or _truthy_env(os.getenv("ULAK_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error("Refusing to run the Ulak gateway as root inside the official Docker image.")
    print(
        "  The image entrypoint normally drops privileges to the 'ulak' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/ulak/docker/entrypoint.sh before the Ulak command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$ULAK_HOME and break later non-root dashboard/gateway runs."
    )
    print("  Set ULAK_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
def _apply_startup_watchdog_config() -> None:
    ...
    try:
        from ulak_startup_watchdog import (
        ...
        try:
            from ulak_cli.config import load_config as _sw_load_config
...
def _make_exit_diag():
    ...
    ``asyncio.run()`` can return, for chasing silent Windows gateway deaths. ULAK_GATEWAY_EXIT_DIAG=0 opts out."""
    from datetime import datetime as _dt, timezone as _tz

    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("ULAK_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from ulak_constants import get_ulak_home as _ghh
            log_dir = _ghh() / "logs"
            log_dir.mkdir(parents=True, exist_ok=True)
            line = {
...
def _respawn_storm_backoff() -> None:
    ...
    DEFAULT_CONFIG ``gateway.respawn_storm``; ULAK_GATEWAY_MAX_STARTS / ULAK_GATEWAY_START_WINDOW_S
    ...
    try:
        ...
        try:
            ...
            pass
        try:
            _max_starts = int(os.environ["ULAK_GATEWAY_MAX_STARTS"])
        except (KeyError, ValueError):
            pass
        try:
            _win = float(os.environ["ULAK_GATEWAY_START_WINDOW_S"])
        except (KeyError, ValueError):
            pass
        ...
        if _storm is not None:
            ...
            try:
                from ulak_startup_watchdog import kick_startup_watchdog
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # Detached Windows runs (ULAK_GATEWAY_DETACHED=1, or non-TTY for older wrappers) ignore
    ...
    # exit-code-75 respawn (stale-code or /restart), which bypasses `ulak gateway restart`.
    if supports_systemd_services():
        try:
...
_PLATFORMS = [
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. ulak) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL",
             ...
             "password": False, "help": "Channel ID where Ulak delivers cron results and notifications."},
    ...
    {"key": "weixin", "label": "Weixin / WeChat", "emoji": "💬", "token_var": "WEIXIN_ACCOUNT_ID"},
    {
        ...
        "setup_instructions": [
            ...
            "5. Ulak connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: ulak pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Ulak will connect automatically over WebSocket",
        ],
        "vars": [
...
_UNAUTHORIZED_ACCESS_CHOICES = {
    ...
    False: (1,
        ...
        "Use DM pairing (unknown users request access, you approve with 'ulak pairing approve')",
...
def _prompt_unauthorized_access(*, is_email: bool) -> None:
    ...
    if access_idx == 0:
        ...
        print_info("  Approve with: ulak pairing approve <platform> <code>")
    elif is_email:
        print_success("  Unknown email senders will be ignored.")
    else:
        print_info("  Skipped — configure later with 'ulak gateway setup'")
...
def _telegram_auto_setup(token_var: str) -> tuple[bool, object]:
    ...
    if prompt("  Choice [1/2]", default="1").strip() != "1":
        return False, None
    try:
        from ulak_cli.telegram_managed_bot import (
...
def _setup_standard_platform(platform: dict):
    ...
    from ulak_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
...
def _running_under_s6() -> bool:
    from ulak_cli.service_manager import detect_service_manager
...
def _setup_weixin():
    ...
    print()
    _print_info_lines(
        "  1. Ulak will open Tencent iLink QR login in this terminal.",
        "  2. Use WeChat to scan and confirm the QR code.",
        "  3. Ulak will store the returned account_id/token in ~/.ulak/.env.",
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `ulak gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_ulak_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 2:
        ...
        if access_idx == 0:
            print_info(
                "  Unknown DM users can request access and you approve them with `ulak pairing approve`."
            )
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown users can request access; approve with `ulak pairing approve`.")
    elif access_idx == 1:
...
def _builtin_setup_fn(key: str):
    """Resolve a built-in platform's setup function; late-bound to dodge the ulak_cli.setup cycle."""
    from ulak_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    if required:
        print_info(f"  Set these env vars in ~/.ulak/.env: {', '.join(required)}")
    else:
...
def _setup_service_action(
    ...
    try:
        ...
        if backend is not None:
            ...
            print_info("Start manually: ulak gateway")
    except UserSystemdUnavailableError as e:
...
_WIZARD_NO_SERVICE_LINES = {
    "wsl": (
        "  WSL detected but systemd is not running.", "  Run in foreground: ulak gateway run",
        "  For persistence:   tmux new -s ulak 'ulak gateway run'",
        "  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'",
    ),
    "termux": (
        "  Termux does not use systemd/launchd services.", "  Run in foreground: ulak gateway run",
        "  Or start it manually in the background (best effort): nohup ulak gateway run >{home}/logs/gateway.log 2>&1 &",
    ),
    "unsupported": (
        "  Service install not supported on this platform.", "  Run in foreground: ulak gateway run",
    ),
}
...
def _wizard_service_status_block() -> None:
    ...
    if supports_systemd_services() and has_legacy_ulak_units():
        print_legacy_unit_warning()
        print()
...
def _wizard_install_service(backend: str) -> None:
    ...
    if not (start_now or start_on_login):
        ...
        print_info("  You can install later: ulak gateway install")
        if supports_systemd_services():
            print_info("  Or as a boot-time service: sudo ulak gateway install --system")
        print_info("  Or run in foreground:  ulak gateway run")
        return
    try:
        ...
        print_info("  You can try manually: ulak gateway install")
...
def gateway_setup():
    ...
    if any(_is_progress(_platform_status(p)) for p in _all_platforms()):
        ...
        print()
        print_info("No platforms configured. Run 'ulak gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(action: str, profile: str | None = None) -> bool:
    ...
    from ulak_cli.service_manager import (
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    ``ULAK_S6_SUPERVISED_CHILD`` (set by ``S6ServiceManager._render_run_script``) marks the supervised
    child, which must run in foreground or we'd recurse run → start → run; ``--no-supervise`` /
    ULAK_GATEWAY_NO_SUPERVISE=1 opts out (CI smoke, debugging)."""
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("ULAK_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    # ULAK_S6_SUPERVISED_CHILD: we ARE the supervised child; fall through so the gateway starts.
    if no_supervise or os.environ.get("ULAK_S6_SUPERVISED_CHILD"):
        return False
    ...
    print(
        ...
        "  dashboard supervised alongside if ULAK_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or ULAK_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    try:
        ...
        # down the supervised gateway cleanly). Prefer `sleep infinity` (matches the static main-ulak
        # service's pattern in docker/s6-rc.d/main-ulak/run, and frees the Python interpreter — the
...
def _refuse_from_inside_gateway(verb: str, reason: str) -> None:
    ...
    if _is_supervised_gateway_process():
        print_error(
            ...
            f"Use `ulak gateway {verb}` from a shell outside the running gateway."
        )
        sys.exit(1)
...
_WSL_FOREGROUND_HINT = (
    "", "  ulak gateway run                              # direct foreground",
    "  tmux new -s ulak 'ulak gateway run'         # persistent via tmux",
    "  nohup ulak gateway run > ~/.ulak/logs/gateway.log 2>&1 &  # background",
)
...
_NO_BACKEND_MESSAGES = {
    ("install", "termux"): (1,
        "Gateway service installation is not supported on Termux.", "Run manually: ulak gateway"),
    ("install", "wsl"): (1,
    ...
    ("install", "s6"): (None,
        ...
        "  ulak profile create <name>     # creates the s6 service slot",
        "  ulak -p <name> gateway start   # bring it up via s6",
        "  ulak status                    # see currently-supervised gateways"),
    ("install", "container"): (0,
        ...
        "To run the gateway: ulak gateway run"),
    ("install", "unsupported"): (1,
        "Service installation not supported on this platform.", "Run manually: ulak gateway run"),
    ("uninstall", "termux"): (1,
        "Gateway service uninstall is not supported on Termux because there is no managed service to remove.",
        "Stop manual runs with: ulak gateway stop"),
    ("uninstall", "s6"): (None,
        "Per-profile gateways are auto-unregistered when you delete the profile.", "",
        "  ulak profile delete <name>     # tears down the s6 service slot",
        "  ulak -p <name> gateway stop    # stop without deleting the profile"),
    ...
    ("start", "container"): (0,
        ...
        "Or run the gateway directly: ulak gateway run"),
...
def _install_systemd_from_cli(args, *, force: bool, system: bool, run_as_user) -> None:
    if is_wsl():
        ...
        _print_info_lines(
            "  Consider running in foreground instead: ulak gateway run",
            "  Or use tmux/screen for persistence: tmux new -s ulak 'ulak gateway run'",
        )
        print()
...
def _cmd_restart(args):
    ...
    if supports_systemd_services():
        ...
        if linger_ok is not True:
            ...
            _print_lines(
                ...
                "  Then restart the gateway:", "    ulak gateway restart",
            )
            return
    ...
    if service_configured:
        _print_lines(
            ...
            "  Fix the service, then retry: ulak gateway start",
        )
        sys.exit(1)
...
# ``ulak gateway status`` hints for a manually-run / stopped gateway, keyed by host kind.
_STATUS_RUNNING_HINTS = {
    ...
    "windows": ("To install as a Windows Scheduled Task (auto-start on login):", "  ulak gateway install"),
    "other": (
        "To install as a service:", "  ulak gateway install", "  sudo ulak gateway install --system",
    ),
}
_STATUS_STOPPED_HINTS = {
    "termux": (
        "  nohup ulak gateway run > ~/.ulak/logs/gateway.log 2>&1 &  # Best-effort background start",
    ),
    "wsl": (
        "  tmux new -s ulak 'ulak gateway run'         # persistent via tmux",
        "  nohup ulak gateway run > ~/.ulak/logs/gateway.log 2>&1 &  # background",
    ),
    "windows": ("  ulak gateway install  # Install as Windows Scheduled Task (auto-start on login)",),
    "other": (
        "  ulak gateway install  # Install as user service",
        "  sudo ulak gateway install --system  # Install as boot-time system service",
    ),
}
...
def _cmd_status(args):
    ...
    if not snapshot.running and named_profile_served_by_running_multiplexer():
        ...
        print("  Manage it from the default profile: ulak gateway status")
        ...
        if pids:
            ...
            print()
            print("To start:")
            print("  ulak gateway run      # Run in foreground")
...
def _cmd_migrate_legacy(args):
    """Stop, disable, and remove legacy Ulak gateway unit files (e.g. ulak.service)."""
    dry_run = getattr(args, "dry_run", False)
    yes = getattr(args, "yes", False)
    if not supports_systemd_services() and not is_macos():
        print("Legacy unit migration only applies to systemd-based Linux hosts.")
        return
    remove_legacy_ulak_units(interactive=not yes, dry_run=dry_run)
