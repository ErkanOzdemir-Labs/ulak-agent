...
_DEFAULT_PLATFORM_TOOLSETS = {"cli": "ulak-cli"}
...
def _has_agent_browser() -> bool:
    ...
    from ulak_constants import agent_browser_runnable
    ...
    try:
        ...
        # Ulak-managed Node dirs ($ULAK_HOME/node, prepended to PATH at runtime but usually absent
        ...
        search_paths = [None, with_ulak_node_path().get("PATH", ""), str(local_bin_dir) if local_bin_dir.is_dir() else ""]
        return any(
...
def _local_stt_backend_available() -> bool:
    ...
    if get_env_value("ULAK_LOCAL_STT_COMMAND"):
        return True
    try:
...
def prompt_enable_tool_gateway(config: Dict[str, object], *, force_fresh: bool = True) -> set[str]:
    ...
    if not unconfigured and not has_direct:
        return set()
    try:
        from ulak_cli.setup import prompt_checklist
    except Exception:
        return set()
...
# Inline Nous Portal login for the Tool Gateway picker (`ulak tools`)
