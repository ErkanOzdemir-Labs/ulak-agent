...
Split out of ``ulak_cli.kanban_db``; origin-resident helpers are reached
...
if TYPE_CHECKING:
    from ulak_cli.kanban_db import Task
...
# ``ULAK_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``.
...
class DispatchResult:
    ...
    Code terminal like ``orion-cc``), not a Ulak profile. Expected steady-state
    ...
    if sweep.exited_hook_payloads and _kb._kanban_observer_consumed("on_kanban_worker_exited"):
        ...
        for hook_fields in sweep.exited_hook_payloads:
            ...
            _kb._fire_kanban_lifecycle_hook(
                ...
                # fire in the DISPATCHER process (gateway-embedded dispatcher or ``ulak kanban
...
def _profile_exists_fn() -> Optional[Callable[[str], bool]]:
    """``ulak_cli.profiles.profile_exists``, or ``None`` when it cannot be
    imported (local import avoids a cycle; callers fall back to trusting the
    assignee)."""
    try:
        from ulak_cli.profiles import profile_exists
    except Exception:
        return None
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    """True iff a ready+assigned+unclaimed task maps to a real Ulak profile.
...
def review_dispatch_enabled() -> bool:
    """Whether review tasks dispatch automatically. Default true (Ulak ships
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so ``ULAK_KANBAN_DB`` (pins every
...
def _dispatch_lane_task(
    ...
    *,
    ...
    # would fail ``ulak -p <assignee>`` at startup and loop ready→crash→ready
    ...
    if guard_reason is not None:
        ...
        # Event so ``ulak kanban tail`` shows why the task looks stuck.
...
def _module_ulak_argv() -> list[str]:
    """Interpreter-bound Ulak CLI invocation (``ulak_cli.main`` is the
    console-script target — there is no top-level ``ulak`` package)."""
    return [sys.executable, "-m", "ulak_cli.main"]


def _absolute_ulak_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Ulak shim."""
...
def _ulak_path_argv(path: str) -> list[str]:
    """argv for a resolved Ulak executable path. Windows batch shims
    ...
    if _kb._IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_ulak_argv()
    return [_absolute_ulak_path(path)]


def _resolve_ulak_argv() -> list[str]:
    """Resolve the ``ulak`` invocation as argv for ``Popen``: ``$ULAK_BIN``
    (path-like -> absolute; bare names keep PATH semantics, never a
    same-directory file), then ``which("ulak")`` (Windows: safe PATH search,
    batch shims fall back to the module form), then ``sys.executable -m
    ulak_cli.main`` for shim-less environments (cron, systemd ``User=``,
    launchd). Mirrors ``gateway.run._resolve_ulak_bin``; local because
    ``ulak_cli`` sits below ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("ULAK_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _ulak_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _ulak_path_argv(resolved_env_bin)
        return _module_ulak_argv()

    ulak_bin = _safe_which_no_cwd("ulak") if _kb._IS_WINDOWS else shutil.which("ulak")
    if ulak_bin:
        return _ulak_path_argv(ulak_bin)
    return _module_ulak_argv()
...
def _resolve_worker_cli_toolsets(ulak_home: Optional[str]) -> Optional[list[str]]:
    ...
    tools when ``ULAK_KANBAN_TASK`` is set.
    """
    if not ulak_home:
        return None
    try:
        ...
        token = set_ulak_home_override(ulak_home)
        try:
            cfg = load_config()
        ...
        _kb._log.debug(
            "kanban worker: could not resolve CLI toolsets for ULAK_HOME=%r (%s)",
            ulak_home,
            exc,
        )
        return None
...
def _worker_argv(task: Task, profile_arg: str, ulak_home: Optional[str]) -> list[str]:
    """Build the ``ulak -p <profile> --cli ... chat -q ...`` worker command."""
    cmd = [
        *_resolve_ulak_argv(),
        "-p", profile_arg,
        ...
        "--cli",
        # Workers run under a profile-scoped ULAK_HOME and so see that
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(ulak_home)
    if worker_toolsets:
...
def _open_worker_log(task: Task, board: Optional[str]):
    ...
    `ulak kanban log` reads its own file and boards sharing task ids don't
    collide."""
...
def _default_spawn(task: Task, workspace: str, *, board: Optional[str] = None) -> Optional[int]:
    """Fire-and-forget ``ulak -p <profile> chat -q ...`` subprocess.
    ...
    ``ULAK_KANBAN_DB`` / ``ULAK_KANBAN_BOARD`` / workspaces_root to the
    board the task was claimed from, so workers cannot see other boards.
    """
    if not task.assignee:
        raise ValueError(f"task {task.id} has no assignee")

    from ulak_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    # Inject ULAK_HOME so the worker reads the profile-scoped config.yaml:
    # without it the child's get_ulak_home() falls back to the DEFAULT
    # profile root because `ulak -p` applies its override before
    # ulak_constants is imported.
    try:
        env["ULAK_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # No profile dir (isolated test fixtures) — the CLI resolves it from
        # ULAK_PROFILE (set below) instead.
        pass
    if task.tenant:
        env["ULAK_TENANT"] = task.tenant
    env["ULAK_KANBAN_TASK"] = task.id
    env["ULAK_KANBAN_WORKSPACE"] = workspace
    ...
    env["ULAK_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["ULAK_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["ULAK_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["ULAK_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    if task.goal_mode:
        env["ULAK_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["ULAK_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # match after `ulak -p` rewrites ULAK_HOME (symlink / Docker layouts).
    env["ULAK_KANBAN_DB"] = str(_kb.kanban_db_path(board=board))
    env["ULAK_KANBAN_WORKSPACES_ROOT"] = str(_kb.workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["ULAK_KANBAN_WORKSPACES_ROOT"])
    # Board slug — defense-in-depth pin if a path is resolved without the
    # DB / workspaces env vars.
    env["ULAK_KANBAN_BOARD"] = _kb._normalize_board_slug(board) or _kb.get_current_board()
    # kanban_comment reads ULAK_PROFILE for its default author; `-p` alone
    # doesn't set the env var.
    env["ULAK_PROFILE"] = profile_arg
    # `--cli` is the highest-precedence TUI override; dropping ULAK_TUI covers
    # older ulak builds on PATH that predate the flag's precedence.
    env.pop("ULAK_TUI", None)

    cmd = _worker_argv(task, profile_arg, env.get("ULAK_HOME"))
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`ulak` executable not found on PATH. "
            "Install Ulak Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    the gateway dispatcher and ``ulak kanban dispatch`` — the standalone
...
from ulak_cli import kanban_db as _kb  # noqa: E402
from ulak_cli import kanban_db_connect as _kbc  # noqa: E402
from ulak_cli import kanban_db_workspace as _kbw  # noqa: E402
