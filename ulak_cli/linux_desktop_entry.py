"""Install and remove the Linux desktop entry (``ulak.desktop``).
...
DESKTOP_ENTRY_NAME = "ulak.desktop"
...
def _can_import_ulak_cli(interpreter: Path) -> bool:
    """Whether *interpreter* can import ``ulak_cli.main`` unaided.
...
def _running_interpreter_fallback() -> str:
    """The RUNNING interpreter — it has ``ulak_cli`` importable by definition."""
    return os.path.abspath(sys.executable)


def resolve_exec_command(project_root: Optional[Path] = None) -> str:
    """Build the absolute ``Exec=`` command line for ``ulak desktop``.

    Prefer the real ``ulak`` launcher; fall back to ``<python> -m ulak_cli.main desktop``.
    """
    from ulak_cli.relaunch import resolve_ulak_bin

    bin_path = _resolve_ulak_bin_for_desktop_entry(resolve_ulak_bin, checkout_root=project_root)
    interpreter = _running_interpreter()
    if not _can_import_ulak_cli(Path(interpreter)):
        ...
        # The candidate interpreter cannot actually import ulak_cli.main (checked in isolated mode from a
    ...
    argv = [interpreter, "-m", "ulak_cli.main", "desktop"]
    if bin_path:
        resolved = Path(bin_path).resolve()
        # A Python launcher whose shebang points OUTSIDE the venv (e.g. the repo's `ulak` script
...
def _inside_checkout(candidate: str, checkout_root: Path, original_argv0: str) -> bool:
    ...
    try:
        ...
        return False
    # Anything shipped in the tree (e.g. the repo `ulak` script) is checkout-internal. Compare
    ...
    for root in {checkout_root, resolved_root}:
        if root is not None and (path == root or root in path.parents):
            return True
    # The `python -m ulak_cli.main` relaunch context surfaces the invoking interpreter as
...
def _resolve_ulak_bin_for_desktop_entry(
    resolve_fn=None,
    checkout_root: Optional[Path] = None,
) -> Optional[str]:
    """Resolve the launcher binary for the persisted ``.desktop`` entry.

    Wraps :func:`ulak_cli.relaunch.resolve_ulak_bin` with one rule: an ``argv[0]`` inside this
    ...
    if resolve_fn is None:
        from ulak_cli.relaunch import resolve_ulak_bin as resolve_fn
    ...
    # An external primary (another install's /opt/.../bin/ulak, a venv console script) wins
    ...
    # /opt/.../bin/ulak from another install method, or a venv console script). It must be evaluated
    ...
    # argv[0] was checkout-internal AND PATH had no `ulak` — common in stripped systemd user
...
def _known_wrapper_candidates():
    ...
    (``/usr/local/bin``), and user (``~/.local/bin``). The wrapper is always named ``ulak``.
    """
    candidates = []
    prefix = os.environ.get("PREFIX")
    if prefix:
        candidates.append(Path(prefix) / "bin" / "ulak")
    if hasattr(os, "geteuid") and os.geteuid() == 0:
        candidates.append(Path("/usr/local/bin/ulak"))
    candidates.append(Path.home() / ".local" / "bin" / "ulak")
    return candidates
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Ulak\n"
        "GenericName=Ulak Desktop\n"
        "Comment=Launch Ulak Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
...
def _hicolor_icon_dest(subdir: str) -> Path:
    return _xdg_data_home() / "icons" / "hicolor" / subdir / "apps" / "ulak.png"
...
def _install_icon_to_hicolor(icon: Path) -> bool:
    """Install the app icon into the user's hicolor tree so ``Icon=ulak`` resolves without an
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Ulak desktop entry and return its path.
    ...
    icon_value = str(icon) if icon.is_file() else "ulak"
