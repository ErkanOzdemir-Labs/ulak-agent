"""Interactive setup wizard for Ulak Agent (config lives in ~/.ulak/).
...
``ulak_cli.setup.<name>`` keep working.
"""
...
from ulak_cli.curses_ui import MenuNavigationEvent, MenuNavigationStart
# Config helpers are re-exported (tests patch them on this module). display_ulak_home is
# imported lazily at call sites (stale-module safety during ulak update).
from ulak_cli.config import (
    cfg_get, DEFAULT_CONFIG, get_ulak_home, get_config_path, get_env_path, load_config, save_config,
    save_env_value, remove_env_value, get_env_value, ensure_ulak_home,
)
from ulak_cli.colors import Colors, color
from ulak_cli.cli_output import print_error, print_info, print_success, print_warning
...
_DOCS_BASE = "https://ulak-agent.github.com/ErkanOzdemir-Labs/docs"
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Ulak Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    _info("The interactive wizard cannot be used here.", None,
          "Configure Ulak using environment variables or config commands:",
          "  ulak config set model.provider custom",
          "  ulak config set model.base_url http://localhost:8080/v1",
          "  ulak config set model.default your-model-name", None,
          "Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.",
          "Run 'ulak setup' in an interactive terminal to use the full wizard.", None)
...
_SETUP_NAVIGATION: ContextVar[_SetupNavigationState | None] = ContextVar("ulak_setup_navigation", default=None)
...
def _setup_navigation_scope():
    ...
    from ulak_cli.curses_ui import reset_menu_navigation_handler, set_menu_navigation_handler
...
def run_setup_action_with_navigation(
    ...
    ``ulak model`` that use the wizard's pickers outside ``run_setup_wizard``."""
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from ulak_cli.curses_ui import curses_radiolist
...
def is_noninteractive() -> bool:
    ...
    with ``stdin=DEVNULL`` and ``ULAK_NONINTERACTIVE=1`` (``ulak_cli/web_server.py``), where a
    prompt that aborts on EOF would kill the spawned action — callers fall back to their default."""
    return os.environ.get("ULAK_NONINTERACTIVE", "").strip().lower() in {"1", "true", "yes", "on"}


def prompt_yes_no(question: str, default: bool = True) -> bool:
    """Prompt for yes/no. Ctrl+C exits; empty input, ``ULAK_NONINTERACTIVE=1`` or a
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from ulak_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    print()
    _prompt_and_save_env_var(var, "  ✓ Saved", "  Skipped (configure later with 'ulak setup')")
...
def setup_model_provider(config: dict, *, quick: bool = False):
    """Configure the inference provider and default model via the ``ulak model`` flow (one code
    ...
    from ulak_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: ulak model")
    ...
    # keep safe defaults (configure via `ulak auth add` / `ulak setup tts`).
    config.clear()
...
def _apply_default_agent_settings(config: dict):
    ...
    # config.yaml is authoritative for max_turns (the gateway bridges it into ULAK_MAX_ITERATIONS);
    # a stale .env entry silently shadowing it caused the 60-vs-500 bug, so drop it.
    remove_env_value("ULAK_MAX_ITERATIONS")
    ...
    _info("  Max iterations: 150", "  Tool progress: all", "  Compression threshold: 0.50",
          ...
          "  Run `ulak setup agent` later to customize.")
...
_TOOL_PROGRESS_HELP = (
    ...
    "  log     — Silent in chat; write every tool call to ~/.ulak/logs/tool_calls.log (gateway only)",
)
_SESSION_RESET_HELP = (
...
def setup_agent_settings(config: dict):
    ...
    if max_iter is None:
        ...
        # config.yaml only; gateway/run.py derives ULAK_MAX_ITERATIONS from agent.max_turns.
...
def setup_tools(config: dict, first_install: bool = False):
    """`ulak setup tools` == `ulak tools`: platform selection → toolset toggles → provider keys.
    ``first_install`` selects the simplified flow (no platform menu, prompts for all missing keys)."""
    from ulak_cli.tools_config import tools_command
...
def _record_send_consent_change(*, enabled: bool) -> None:
    ...
    try:
        from ulak_cli.observability.shared_metrics import SharedMetricsStore
        from ulak_cli.observability.shared_metrics_sender import reconcile_send_consent
        from ulak_cli.sqlite_util import write_txn
...
# ulak_cli.setup. They import this module lazily inside bodies, so this is cycle-free.

from ulak_cli.setup_tts import setup_tts  # noqa: E402
from ulak_cli.setup_terminal import setup_terminal_backend  # noqa: E402
from ulak_cli.setup_platforms import setup_gateway  # noqa: E402
from ulak_cli.setup_summary import _print_setup_summary  # noqa: E402,F401
from ulak_cli.setup_migration import _offer_openclaw_migration, _skip_configured_section  # noqa: E402
from ulak_cli.setup_quick import _run_portal_one_shot, _run_quick_setup  # noqa: E402
...
def _run_setup_section(config: dict, section: str) -> None:
    """``ulak setup <section>``: run one SETUP_SECTIONS entry under the banner."""
    ...
    if entry is None:
        ...
        return
    label, func = entry
    _print_banner(f"│     ⚕ Ulak Setup — {label:<34s} │")
...
def _run_full_setup(config: dict, ulak_home, *, is_existing: bool, migration_ran: bool) -> None:
    ...
    _info(f"Config file:  {get_config_path()}", f"Secrets file: {get_env_path()}",
          f"Data folder:  {ulak_home}", f"Install dir:  {PROJECT_ROOT}", None,
          "You can edit these files directly or use 'ulak config edit'")
    if migration_ran:
    ...
    def _gateway_step() -> None:
        ...
        from ulak_cli.gateway import ensure_gateway_service
...
def _run_setup_wizard_impl(args):
    ...
    ``ulak setup <section>`` from SETUP_SECTIONS."""
    ...
    from ulak_cli.auth import get_active_provider
    is_existing = bool(get_env_value("OPENROUTER_API_KEY") or get_env_value("OPENAI_BASE_URL")
                       or get_active_provider() is not None)
    _print_banner("│             ⚕ Ulak Agent Setup Wizard                │",
                  "├─────────────────────────────────────────────────────────┤",
                  "│  Let's configure your Ulak Agent installation.       │",
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_setup_steps([("Quick Setup", lambda: _run_quick_setup(config, ulak_home))])
            return
        print_header("Reconfigure", gap=True)
        print_success("You already have Ulak configured.")
        _info("Running the full wizard — each prompt shows your current value.",
              ...
              "Tip: jump straight to a section with 'ulak setup model|terminal|",
        ...
        migration_ran = _offer_openclaw_migration(ulak_home)  # before configuration begins
        if migration_ran:
            config = load_config()
        setup_mode = prompt_choice("How would you like to set up Ulak?", [label for label, _ in _FIRST_TIME_MODES], 0)
        label, runner = _FIRST_TIME_MODES[setup_mode]
        if runner is not None:
            from ulak_cli import setup_quick
            _run_setup_steps([(label, lambda: getattr(setup_quick, runner)(config, ulak_home, is_existing))])
            return
    _run_full_setup(config, ulak_home, is_existing=is_existing, migration_ran=migration_ran)
    ...
    _print_setup_summary(config, ulak_home)
...
_PLUGIN_COMPAT_LAZY = {
    'get_nous_subscription_features': ('ulak_cli.nous_subscription', 'get_nous_subscription_features'),
    'get_optional_skills_dir': ('ulak_constants', 'get_optional_skills_dir'),
