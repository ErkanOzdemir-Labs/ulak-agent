...
All HttpOnly, ``SameSite=Lax`` unless noted, Path = proxy prefix or /: ``ulak_session_at``
(access token; Max-Age = token TTL), ``ulak_session_rt`` (rotating refresh token; written only
when the provider returned one, always cleared on logout/expiry), ``ulak_session_provider``
(non-secret routing hint so an RT is not handed to the wrong provider), ``ulak_session_pkce``
...
cookies set on such a 302, crbug 40508226), ``ulak_sso_attempt`` (auto-SSO loop guard, 60 s).
...
SESSION_AT_COOKIE = "ulak_session_at"
SESSION_RT_COOKIE = "ulak_session_rt"
SESSION_PROVIDER_COOKIE = "ulak_session_provider"
PKCE_COOKIE = "ulak_session_pkce"
SSO_ATTEMPT_COOKIE = "ulak_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    """``Path=/ulak`` under a proxy prefix (no leak to sibling apps), else ``/``."""
...
def set_session_cookies(
    ...
    Nous Portal issues a 24h rotating refresh token (ulak #37247); a provider that omits it returns
