...
def _strip_ulak_path_lines(content: str) -> str:
    """Drop the ``# Ulak Agent`` marker (+ its PATH line) and any ulak PATH line; squash blank runs."""
    new_lines = []
    skip_next = False
    for line in content.split('\n'):
        if '# Ulak Agent' in line or '# ulak-agent' in line:
            skip_next = True
            continue
        if skip_next and ('ulak' in line.lower() and 'PATH' in line):
            skip_next = False
            continue
        skip_next = False
        if 'ulak' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
            continue
...
def remove_path_from_shell_configs():
    """Remove Ulak PATH entries from shell configuration files."""
    removed_from = []
    for config_path in (c for c in (Path.home() / n for n in _SHELL_RC_NAMES) if c.exists()):
        try:
            content = config_path.read_text(encoding="utf-8")
            new_content = _strip_ulak_path_lines(content)
...
def remove_wrapper_script():
    """Remove the ulak wrapper script if it exists."""
    def _unlink_ours(wrapper: Path) -> bool:
        # Only our wrapper (contains a ulak_cli / ulak-agent reference).
        content = wrapper.read_text(encoding="utf-8")
        return _unlink_if('ulak_cli' in content or 'ulak-agent' in content, wrapper)

    candidates = (
        bin_dir / name
        for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin"))
        for name in ("ulak", "ulak-acp", "ulak-agent"))
...
def remove_node_symlinks(ulak_home: Path) -> list:
    ...
    node_dir = (ulak_home / "node").resolve()
...
def uninstall_gateway_service():
    ...
    try:
        from ulak_cli.gateway import kill_gateway_processes, find_gateway_pids
...
def _remove_systemd_gateway() -> bool:
    ...
    from ulak_cli.gateway import _systemctl_cmd, get_service_name, get_systemd_unit_path
...
def _remove_launchd_gateway() -> bool:
    ...
    from ulak_cli.gateway import get_launchd_plist_path
...
def _remove_windows_gateway() -> bool:
    ...
    from ulak_cli import gateway_windows as gw
...
# ULAK_HOME / ULAK_GIT_BASH_PATH (HKCU\Environment), User-scope PATH entries
# (%LOCALAPPDATA%\ulak\git\{cmd,bin,usr\bin}, ...\ulak\node), PortableGit + Node copies
...
def _ulak_path_markers(ulak_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Prefixes identifying Ulak-owned User-PATH entries (prefix match sweeps git\cmd, git\bin,
    ...
    root = str(ulak_home).rstrip("\\/")
    subs = ("ulak-agent", "git", "node", "venv") + (("bin",) if include_managed_bin else ())
    return [f"{root}\\{sub}" for sub in subs]


def remove_path_from_windows_registry(ulak_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Ulak-owned entries from User-scope PATH in the registry (see ``_ulak_path_markers``)."""
    markers = tuple(m.lower() for m in _ulak_path_markers(ulak_home, include_managed_bin=include_managed_bin))
...
def remove_ulak_env_vars_windows() -> list[str]:
    """Delete ULAK_HOME and ULAK_GIT_BASH_PATH from User-scope env vars."""
    def edit(winreg, key, removed):
        for name in ("ULAK_HOME", "ULAK_GIT_BASH_PATH"):
            try:
...
def remove_portable_tooling_windows(ulak_home: Path) -> list[Path]:
    """Delete the PortableGit / Node / gateway-service dirs the Windows installer created under
    ``ulak_home`` (isolated from any system Git / Node, so nothing else breaks)."""
    targets = (ulak_home / sub for sub in ("git", "node", "gateway-service"))
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``ulak`` launchers install.ps1 staged in the managed ``bin`` dir: every mode
    ...
    try:
        ...
        from ulak_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from ulak_constants import get_default_ulak_root
        bin_dir = get_default_ulak_root() / "bin"
    except Exception as e:
...
def _is_default_ulak_home(ulak_home: Path) -> bool:
    """Return True when ``ulak_home`` points at the default (non-profile) root."""
    try:
        from ulak_constants import get_default_ulak_root
        return ulak_home.resolve() == get_default_ulak_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    out to ``ulak -p <name> gateway stop|uninstall`` because service names / unit paths derive
    from the current ULAK_HOME and can't be switched in-process."""
    name = profile.name
    log_info(f"Uninstalling profile '{name}'...")

    # 1. Gateway service, via `python -m ulak_cli.main` (the `ulak` wrapper may be half-gone).
    ulak_invocation = [sys.executable, "-m", "ulak_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                ulak_invocation + ["gateway", subcmd], capture_output=True, text=True,
    ...
    # 3. Wipe the profile's ULAK_HOME directory.
    _rmtree_step(profile.path, indent="  ", fully=False)


def run_gui_uninstall(args):
    """``ulak uninstall --gui``: remove the desktop app's built artifacts, packaged bundle
    (best-effort) and Electron userData — never config/sessions/.env, the agent or its venv."""
    from ulak_cli.gui_uninstall import agent_is_installed, gui_install_summary, uninstall_gui
    ulak_home = get_ulak_home()
    summary = gui_install_summary(ulak_home)
    ...
    if not summary["gui_installed"]:
        print("No Ulak Chat GUI installation was found.")
        print(f"  Checked: {ulak_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Ulak agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(ulak_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Ulak agent at {ulak_home / 'ulak-agent'}")
        print(f"  • Your config, sessions, and secrets under {ulak_home}")
        print()
    ...
    print()
    uninstall_gui(ulak_home)

    print()
    _print_box("│            ✓ Chat GUI Uninstalled!                      │", Colors.GREEN)
    print()
    print("The Ulak agent is still installed. Run 'ulak' to use the CLI,")
    print("or 'ulak uninstall' to remove the agent too.")
    print()


def run_uninstall(args):
    """Interactive/``--yes`` uninstall: full removes code and ``~/.ulak/``; keep-data removes only
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            project_root=project_root, ulak_home=ulak_home, full_uninstall=full_flag)
        return

    # Named profiles (only when uninstalling from the default root) are offered for cleanup too,
    # instead of leaving zombie ULAK_HOMEs and systemd units behind.
    is_default_profile = _is_default_ulak_home(ulak_home)
    ...
    print(f"  Config:  {ulak_home / 'config.yaml'}")
    print(f"  Secrets: {ulak_home / '.env'}")
    print(f"  Data:    {ulak_home / 'cron/'}, {ulak_home / 'sessions/'}, {ulak_home / 'logs/'}")
    print()
    ...
    # services, alias wrappers, ULAK_HOME dirs) — otherwise they leave zombie services behind.
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Ulak data!", Colors.RED, Colors.BOLD))
        print(color("   Including: configs, API keys, sessions, scheduled jobs, logs", Colors.RED))
        if remove_profiles:
            print(color(f"   Plus {n_profiles} profile(s): {profile_names}", Colors.RED))
    else:
        print("This will remove the Ulak code but keep your configuration and data.")
    ...
    _perform_uninstall(
        project_root=project_root, ulak_home=ulak_home, full_uninstall=full_uninstall,
        remove_profiles=remove_profiles, named_profiles=named_profiles)


def _print_uninstall_dry_run(*, project_root: Path, ulak_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Ulak PATH entries from shell configs / Windows User PATH")
    print("  • Ulak wrapper scripts and Ulak-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if not full_uninstall:
        print(f"  • Keep Ulak config/data: {ulak_home}")
    else:
        print(f"  • Ulak config/data: {ulak_home}")
        profiles = _discover_named_profiles() if _is_default_ulak_home(ulak_home) else []
        if profiles:
...
def _perform_uninstall(
    *,
    ...
    the checkout -> (Windows) PortableGit/Node -> optionally ``$ULAK_HOME`` and named profiles."""
    print()
    ...
    # 2-3b. PATH entries, wrapper, Windows launchers, node symlinks. Windows: ulak_home is
    #    %VAR%-expanded because install.ps1 writes literal C:\Users\<u>\...; ulak\bin (launchers +
    ...
    windows = _is_windows()
    sweep_managed_bin = windows and full_uninstall and _is_default_ulak_home(ulak_home)
    for on_this_platform, label, remove, success_fmt, none_msg in (
        ...
        (windows, "Removing PATH entries from Windows User environment...",
         lambda: remove_path_from_windows_registry(
             Path(os.path.expandvars(str(ulak_home))), include_managed_bin=sweep_managed_bin),
         "Removed from User PATH: {}", "No Ulak-owned PATH entries in User environment"),
        (windows, "Removing ULAK_HOME / ULAK_GIT_BASH_PATH User env vars...",
         remove_ulak_env_vars_windows, "Removed User env var: {}", "No Ulak-set User env vars to remove"),
        (True, "Removing ulak command...", remove_wrapper_script, "Removed {}", "No wrapper script found"),
        (windows, "Removing Windows ulak launchers...",
         remove_windows_bin_launchers, "Removed {}", "No Windows ulak launchers found"),
        (True, "Removing Ulak-managed node/npm/npx symlinks...",
         lambda: remove_node_symlinks(ulak_home), "Removed {}", "No Ulak-managed node/npm/npx symlinks found"),
    ):
        if on_this_platform:
    ...
    #     .env (safe in keep-data mode); the packaged app + Electron userData live OUTSIDE ULAK_HOME.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from ulak_cli.gui_uninstall import uninstall_gui
        if not uninstall_gui(ulak_home):
    ...
    if windows:
        _remove_step(
            ...
            lambda: remove_portable_tooling_windows(ulak_home), "Removed {}",
            "No Windows installer artifacts to remove")

    # 5. Optionally remove ~/.ulak/ data directory (and named profiles)
    if full_uninstall:
        ...
        _rmtree_step(ulak_home)
    else:
        log_info(f"Keeping configuration and data in {ulak_home}")
...
_REINSTALL_HINT = {
    True: "  iex (irm https://ulak-agent.github.com/ErkanOzdemir-Labs/install.ps1)",
    False: "  curl -fsSL https://ulak-agent.github.com/ErkanOzdemir-Labs/install.sh | bash"}
...
def main(argv=None) -> int:
    """``python -m ulak_cli.uninstall --mode <gui|lite|full>``. Imports only stdlib +
    ``ulak_constants`` + ``ulak_cli.colors``, so it runs under a bare system Python (no venv)."""
    import argparse
    parser = argparse.ArgumentParser(prog="python -m ulak_cli.uninstall")
    parser.add_argument(
