...
_MANAGED_FILES_ROOT_ENV = "ULAK_DASHBOARD_FILES_ROOT"
...
def _default_ulak_root_is_opt_data() -> bool:
    raw = os.environ.get("ULAK_HOME", "").strip()
    if not raw:
        return False
    try:
        from ulak_constants import get_default_ulak_root

        root = get_default_ulak_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """True when the dashboard should not offer ``ulak update``.

    Containerized dashboards are updated by the outer launcher/image — except a
    ``git`` install (bind-mounted checkout, e.g. the ulak-webui image), where
    ...
    if _default_ulak_root_is_opt_data():
        return True
    try:
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # install still browses its home). Lock to /opt/data only when the Ulak
    # root actually IS /opt/data or ULAK_DASHBOARD_FILES_ROOT is set.
    if _default_ulak_root_is_opt_data():
