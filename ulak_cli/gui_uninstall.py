"""Ulak Desktop (Chat GUI) uninstaller: removes only GUI state — built Electron artifacts, the packaged
...
def desktop_userdata_dir() -> Path:
    """Electron ``app.getPath('userData')`` for an app named "Ulak" on each platform (GUI-only state)."""
    home = Path.home()
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Ulak"
    if sys.platform == "win32":
        return _env_dir("APPDATA", home / "AppData" / "Roaming") / "Ulak"
    return _env_dir("XDG_CONFIG_HOME", home / ".config") / "Ulak"


def source_built_gui_artifacts(ulak_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``ulak desktop`` inside the checkout (same ``ulak-agent/`` layout
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Ulak" product."""
    home = Path.home()
    if sys.platform == "darwin":
        return [Path("/Applications/Ulak.app"), home / "Applications" / "Ulak.app"]
    if sys.platform == "win32":
        ...
        return [local_base / "Programs" / "Ulak", local_base / "ulak-desktop"] + (
            [Path(program_files) / "Ulak"] if program_files else [])
    ...
    from ulak_cli.linux_desktop_entry import desktop_entry_path
    ...
    return [desktop_entry_path(), data_base / "applications" / "Ulak.desktop"] + [
        icons / size / "apps" / "ulak.png"
        for size in ("scalable", "24x24", "32x32", "48x48", "256x256", "512x512", "1024x1024")]


def agent_is_installed(ulak_home: Path) -> bool:
    """True when a usable Python agent install exists under ULAK_HOME (gates the desktop UI's options).
    Package source or a venv alone is enough — a source checkout without a venv is still "the agent is here"."""
    return any((ulak_home / "ulak-agent" / sub).is_dir() for sub in ("ulak_cli", "venv", ".venv"))


def gui_is_installed(ulak_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    return any(p.exists() for p in (*source_built_gui_artifacts(ulak_home), *packaged_gui_app_paths(), desktop_userdata_dir()))


def gui_install_summary(ulak_home: "Path | None" = None) -> dict:
    ...
    return {"ulak_home": str(home), "agent_installed": agent_is_installed(home),
...
def uninstall_gui(ulak_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    if sys.platform.startswith("linux"):
        # The desktop entry was removed above but the menu caches still list it; reindex so Ulak
        # disappears from the launcher.
        try:
            from ulak_cli.linux_desktop_entry import desktop_entry_path, refresh_desktop_databases
        ...
        log_info("If you installed the desktop via a .deb / .rpm package, remove it with your package manager "
                 "(e.g. 'sudo apt remove ulak' or 'sudo dnf remove ulak'). AppImage builds are a single "
