"""Helpers for loading Ulak .env files consistently across entrypoints."""
...
# Once-per-process guards: load_ulak_dotenv() runs repeatedly (user + project env, gateway hot-reload,
...
# env-var name → source label ("bitwarden", …) for externally injected credentials; setup / `ulak
...
# ULAK_HOME paths already pulled external secrets for: load_ulak_dotenv() runs at import time from
...
# Behavioral routing keys a parent Ulak process injects into child env that silently redirect a profile
...
def get_secret_source_values(ulak_home: str | os.PathLike) -> dict[str, str]:
    """Return the external-secret value snapshot for ``ulak_home``."""
    return dict(_SECRET_SOURCE_VALUES_BY_HOME.get(str(Path(ulak_home).resolve()), {}))


def hydrate_profile_secret_sources(ulak_home: str | os.PathLike) -> dict[str, str]:
    ...
    with _SECRET_SOURCE_CACHE_LOCK:
        return _hydrate_profile_secret_sources(Path(ulak_home))
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Mirror load_ulak_dotenv()'s .op.env bootstrap (1Password token lives in gitignored .op.env)
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `ulak setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    if not path.exists():
        return
    try:
        from ulak_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_ulak_dotenv(
    *,
    ulak_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
    load_external_secrets: bool = True,
) -> list[Path]:
    """Load Ulak env files: ``~/.ulak/.env`` overrides stale shell exports; project ``.env`` is a dev
    fallback that only fills gaps when the user env exists (and overrides shell vars when it does not)."""
    home_path = Path(ulak_home or os.getenv("ULAK_HOME", Path.home() / ".ulak"))
    ...
    if is_multiplex_active() and get_ulak_home_override() is not None:
        ...
        if load_external_secrets:
            from ulak_cli import _early_recovery
    ...
    # Windows (#73381, #86735). 2. A fresh ``ulak update`` retry just completed a deferred dependency
    ...
    # ~/.ulak/.env win on every reload and flip the backend mid-session in long-lived processes.
    ...
    # with override=True — so a stale TERMINAL_ENV=docker left in ~/.ulak/.env (e.g. written by an older
    # `ulak setup` before the user switched terminal.backend in config.yaml) silently wins again on every
    # reload. Startup launchers bridge config→env once, but long-lived processes (gateway per-turn reload,
    # cron standalone runs) call load_ulak_dotenv() repeatedly and used to flip the effective backend back
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    try:
        if Path(home_path).resolve() != _process_ulak_home().resolve():
            return
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    tokens), BEFORE Ulak reads credentials; failures never block startup. Precedence/conflicts/provenance
    ...
    # load_ulak_dotenv() calls per startup don't re-fetch / re-print — error retries within one process
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # populating it lets main.py's early bridge and ulak_logging reuse one parse instead of 3-4.
    if home_path == _process_ulak_home():
        try:
...
def _process_ulak_home() -> Path:
    """The ULAK_HOME the running process was launched under.

    Must be the *true* process home, ignoring any context-local
    ``set_ulak_home_override`` a per-request task has installed. Both
    ...
    * ``_load_secrets_config`` uses it to decide whether the shared
    ...
    ``ulak_constants.get_process_ulak_home()`` is the override-immune
