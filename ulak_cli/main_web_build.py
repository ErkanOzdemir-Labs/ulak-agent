...
# to the checkout (NOT in ULAK_HOME): __pycache__ is per-checkout state shared
# by every profile.
...
def _record_bytecode_fingerprint() -> None:
    ...
    from ulak_cli.main import PROJECT_ROOT, _read_git_revision_fingerprint
    try:
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    Update-time clears can't close the stale-bytecode class: ``ulak update`` runs
    ...
    pull inside ``ulak update``, a manual ``git pull``, a ZIP update, a file-sync restore) while
    ...
    """
    from ulak_cli.main import PROJECT_ROOT, _clear_bytecode_cache, _read_git_revision_fingerprint
    try:
        fingerprint = _read_git_revision_fingerprint(PROJECT_ROOT)
        if not fingerprint:
...
def _web_dist_dir(web_dir: Path) -> Path:
    """Vite outputs to ``ulak_cli/web_dist/`` (vite.config.ts outDir), NOT ``web/dist/``."""
    return _web_project_root(web_dir) / "ulak_cli" / "web_dist"
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    Content hash, NOT mtime: ``git checkout`` / ``ulak update`` rewrite source
...
def _web_ui_stamp_path() -> Path:
    """Path of the web UI build stamp under $ULAK_HOME."""
    from ulak_constants import get_ulak_home
    return get_ulak_home() / "web-ui-build-stamp.json"
...
def _run_with_idle_timeout(
    ...
    half-state with the ``ulak`` launcher present but ``ulak_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    """``PYTHON=`` env for node-gyp on NixOS (bare PATH lookup fails outside nix-shell): the ulak
...
def _run_npm_install_deterministic(
    ...
    run_env["PATH"] = with_ulak_node_path(run_env)["PATH"]
...
def _web_npm_install_context(web_dir: Path) -> tuple[Path, tuple[str, ...]]:
    ...
    closure as ``ulak update``'s ``_update_node_dependencies()`` (ui-tui + web +
    ...
    # When running from the workspace root, this must name the SAME closure as `ulak update`'s
...
def _report_web_build_failure(step: str, result: subprocess.CompletedProcess, *, fatal: bool) -> bool:
    ...
    _console_print(f"  {'✗' if fatal else '⚠'} Web UI {step} failed" + ("" if fatal else " (ulak web will not be available)"))
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    soft warning (used by ``ulak web``). Returns True when the build succeeded
    ...
    if not npm:
        ...
        return not fatal
    build_env = _npm_lifecycle_env(with_ulak_node_path())
