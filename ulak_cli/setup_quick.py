"""Streamlined setup flows: the Nous Portal one-shot (`ulak portal`), first-time quick setup,
Blank Slate setup and the `--quick` missing-items pass. Names from setup.py are imported lazily
per function so test patches on ``ulak_cli.setup`` take effect."""
...
logger = logging.getLogger("ulak_cli.setup")
...
def _blank_slate_done(config: dict, ulak_home, tools_line: str, *extra: str, intro: str | None = None) -> None:
    """Shared Blank Slate epilogue: success banner, the "enable later" hints, then the summary."""
    from ulak_cli.setup import _info, _print_setup_summary, print_success
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    _info(*([intro] if intro else []), tools_line, "  Seed skills:         ulak skills opt-in --sync",
          "  Add MCP servers:     ulak mcp add", *extra, "  Tune agent settings: ulak setup agent", None)
...
def _reload_config_into(config: dict, *, dict_only: bool = False) -> None:
    ...
    from ulak_cli.setup import load_config
...
def _run_nous_flow(config: dict, *, context: str, cancel_exc: tuple, cancel_lines: tuple, print_error) -> bool:
    ...
    single source of truth shared with ``ulak model``. False when cancelled or failed (the
    message is already printed)."""
    from ulak_cli.setup import _info
    try:
        from ulak_cli.model_setup_flows import _model_flow_nous
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Nous Portal setup (``ulak setup --portal`` / ``ulak portal``)."""
    from ulak_cli.setup import _info, _print_banner, print_error, print_info, print_success
    _print_banner("│     ⚕ Ulak Setup — Nous Portal (one-shot)             │")
    _info(None, "  One subscription, 300+ models, plus the Tool Gateway:",
          ...
          "  Sign up: https://portal.github.com/ErkanOzdemir-Labs/manage-subscription", None)

    def _on_error(exc: Exception) -> None:
        print()
        print_error(f"  Nous Portal setup encountered an error: {exc}")
        print_info("  You can retry later with `ulak portal`.")

    if not _run_nous_flow(config, context="`ulak portal`", cancel_exc=(KeyboardInterrupt, EOFError, SystemExit),
                          cancel_lines=(None, "  Setup cancelled.", "  You can retry later with `ulak portal`."),
    ...
    _info("  Run `ulak portal info` to inspect routing.", "  Run `ulak` to start chatting.")


def _run_first_time_quick_setup(config: dict, ulak_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice("Connect a messaging platform? (Telegram, Discord, etc.)", [
        "Set up messaging now (recommended)", "Skip — set up later with 'ulak setup gateway'",
    ], 0)
    if gateway_choice == 0:
        ...
        # platforms come alive as soon as tokens are added later (e.g. via `ulak import`).
        from ulak_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    print()
    print_success("Setup complete! You're ready to go.")
    _info(None, "  Configure all settings:    ulak setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  ulak setup gateway")
...
def _print_macos_fda_tip() -> None:
    ...
    _info(None, "  macOS tip: silence ALL folder permission prompts with one switch —",
          ...
          "  your terminal (and Ulak.app if you use Desktop), or run:",
          "    open \"x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles\"",
          "  The grant is permanent — it survives every Ulak update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``vision`` (``read_file`` can't read images) and ``skills`` (the seeded ``ulak-agent`` skill
    ...
    try:
        ...
        from ulak_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
        all_keys = {k for k, _, _ in CONFIGURABLE_TOOLSETS}
        all_keys.update(_get_plugin_toolset_keys())
        # Plain TOOLSETS entries catch recovered toolsets like ``kanban``. Skip "ulak-*" platform
        ...
        for k, tdef in TOOLSETS.items():
            if k.startswith("ulak-") or (isinstance(tdef, dict) and (tdef.get("includes") or tdef.get("posture"))):
                continue
...
def _blank_slate_minimize_config(config: dict):
    """Turn OFF every optional config feature; all opt back in via ``ulak setup agent``."""
...
def _run_blank_slate_setup(config: dict, ulak_home, is_existing: bool):
    ...
    _info("Everything starts OFF. First we force-enable only what's required",
          ...
          "essential `ulak-agent` skill is always kept so the agent",
          "can help you drive and configure Ulak itself.", None)
    ...
    if path != 0:
        _blank_slate_walkthrough(config, ulak_home)
        return
    save_config(config)
    # Blank Slate means no bundled skills; record the opt-out so future `ulak update` runs
    # don't re-inject them.
    _set_bundled_skills_opt_out(True, "skill opt-out")
    _blank_slate_done(config, ulak_home, "  Enable tools:        ulak tools", "  Enable plugins:      ulak plugins",
                      intro="Enable anything later, on demand:")


def _blank_slate_walkthrough(config: dict, ulak_home):
    ...
    def _opted_out(_result) -> None:
        _info("No skills seeded (except the essential `ulak-agent`",
              "skill). A .no-bundled-skills marker keeps future",
              "`ulak update` runs from re-injecting them. Opt back in any",
              "time with `ulak skills opt-in --sync`.")
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `ulak tools`.")
    ...
    for header, question, yes_msg, no_msg in (
        ("Plugins", "Review and enable built-in plugins now?",
         "Manage plugins with `ulak plugins list` / `ulak plugins install`.",
         "No plugins enabled. Add later with `ulak plugins`."),
        ("MCP Servers", "Add an MCP server now?",
         "Add servers with `ulak mcp add <name> --url ... | --command ...`.",
         "No MCP servers configured. Add later with `ulak mcp add`."),
    ):
    ...
    if prompt_yes_no("Connect a messaging platform (Telegram, Discord, …)?", default=False):
        setup_gateway(config)
    save_config(config)
    _blank_slate_done(config, ulak_home, "  Enable more tools:   ulak tools")


def _run_quick_setup(config: dict, ulak_home):
    ...
    from ulak_cli.config import (get_missing_env_vars, get_missing_config_fields, check_config_version)
    ...
    if not (missing_required or missing_optional or missing_config or current_ver < latest_ver):
        ...
        _info(None, "Run 'ulak setup' and choose 'Full Setup' to reconfigure,",
    ...
    if missing_messaging:  # checklist, then prompt for each selected platform's vars
        ...
        _info("Connect Ulak to messaging apps to chat from anywhere.",
              "You can configure these later with 'ulak setup gateway'.")
