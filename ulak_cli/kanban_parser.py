"""Argparse tree for ``ulak kanban …`` (``build_parser``).
...
def _triage_sweep_args(verb: str, Verb: str, noun: str):
    ...
    return (
        ...
        _arg("--author",
             help=f"Author name recorded on the audit comment (default: $ULAK_PROFILE or '{noun}')"),
...
_BOARD_SPECS = [
    ...
    _cmd("import", [
        ...
        "Import a .tar.gz produced by `ulak kanban boards export`. The board always lands as a "
...
# Top-level ``ulak kanban <action>`` records, in ``--help`` order.
_SPECS = [
    ...
    _cmd("create", [
        ...
        _arg("--project",
             help="Link to a project (id or slug). Anchors the task's "
                  ...
                  "deterministic branch. See `ulak project list`."),
        _TENANT,
        _PRIORITY,
    ...
    _cmd("list", [
        _arg("--mine", action="store_true", help="Filter by $ULAK_PROFILE as assignee"),
        _arg("--assignee"),
    ...
    _cmd("comment", [
        ...
        _arg("--author", help="Author name (default: $ULAK_PROFILE or 'user')"),
    ...
    _cmd("attach", [
        ...
        _arg("--author", help="uploaded_by label (default: $ULAK_PROFILE or 'user')"),
    ...
    ], help="DEPRECATED — dispatcher now runs in the gateway. Use `ulak gateway start`."),
    _cmd("watch", [
    ...
    _cmd("assignees", [_json_flag()],
         help="List known profiles + per-profile task counts (union of ~/.ulak/profiles/ and current assignees on the board)"),
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description="Durable SQLite-backed task board shared across Ulak profiles. "
                    "Tasks are claimed atomically, can depend on other tasks, and "
                    "are executed by a named profile in an isolated workspace. "
                    "See https://ulak-agent.github.com/ErkanOzdemir-Labs/docs/user-guide/features/kanban "
                    "or docs/ulak-kanban-v1-spec.pdf for the full design.",
    )
    # --board scopes every subcommand to one board's DB; when omitted the
    # resolution is ULAK_KANBAN_BOARD, then the persisted current-board
    ...
    kanban_parser.add_argument("--board", default=None, metavar="<slug>",
                               help="Board slug to operate on. Defaults to the current board (set "
                                    "via `ulak kanban boards switch <slug>` or the "
                                    "ULAK_KANBAN_BOARD env var). Use `ulak kanban boards "
