"""Shell completion script generation for ulak CLI. Walks the live argparse parser tree, so
    ...
    for cmd, info in subcommands:
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_ulak_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;")
            ...
            cases.append(
                ...
                f"            ;;")
    cases_str = "\n".join(cases)
    return f"""# Ulak Agent bash completion
# Add to ~/.bashrc:
#   eval "$(ulak completion bash)"

_ulak_profiles() {{
    local profiles_dir="$HOME/.ulak/profiles"
...
_ulak_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_ulak_profiles)" -- "$cur"))
        return
    fi
...
complete -F _ulak_completion ulak
"""
    ...
    for cmd, info in subcommands:
        ...
        if cmd == "profile":
            ...
            sub_cases.append(
                ...
                f"                    ;;")
    sub_cases_str = "\n".join(sub_cases)
    return f"""#compdef ulak
# Ulak Agent zsh completion
# Add to ~/.zshrc:
...
_ulak_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.ulak/profiles" ]]; then
        profiles+=($HOME/.ulak/profiles/*(N/:t))
    fi
...
_ulak() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_ulak_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'ulak command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Ulak Agent fish completion",
        "# Add to your config:",
        "#   ulak completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __ulak_profiles",
        "    echo default",
        "    if test -d $HOME/.ulak/profiles",
        "        for d in $HOME/.ulak/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c ulak -f -s p -l profile"
        " -d 'Profile name' -xa '(__ulak_profiles)'",
        "",
        ...
        if cmd == "profile":  # profile names for the actions that take one
            for action in sorted(_PROFILE_NAME_ACTIONS):
                lines.append(
                    ...
                    f"-a '(__ulak_profiles)' -d 'Profile name'")
    lines.append("")
    return "\n".join(lines)
