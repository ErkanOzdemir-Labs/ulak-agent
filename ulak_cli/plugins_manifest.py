...
Split out of :mod:`ulak_cli.plugins`; validation warns and never fails a load.
"""
...
# and continue loading. ``capabilities``/``emits``/``listens``/``ulak``/``depends`` are reserved.
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "license", "homepage", "tags", "capabilities", "emits", "listens", "ulak", "depends",
}

# Highest manifest schema version this Ulak understands.
...
def _plugins_debug() -> bool:
    from ulak_cli import plugins as _origin
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Ulak "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `ulak plugins enable %s` if it is installed.",
                    k, dep_id, dep_id,
                )
            elif resolved == k:
...
class PluginManifest:
    ...
    kind: str = "standalone"
    # Path-derived registry key used by plugins.enabled/disabled and `ulak plugins list`: ``disk-cleanup``
    ...
    # VALIDATED AND SURFACED ONLY — Ulak never auto-installs these (isolation design for the install seam
