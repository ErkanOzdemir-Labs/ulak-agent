"""Profile distributions — shareable, packaged Ulak profiles via git.
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            ulak_requires=_str(data, "ulak_requires"), author=_str(data, "author"),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        optional = (
            ("description", self.description), ("ulak_requires", self.ulak_requires),
...
def check_ulak_requires(spec: str, current_version: str) -> None:
    ...
    if not _VERSION_OPS[op](_parse_semver(current_version), _parse_semver(target)):
        raise DistributionError(f"This distribution requires Ulak {op}{target}, but you have {current_version}.")
    ...
    *workdir* (``.git`` removed); a local directory is used in place."""
    ...
    if _looks_like_git_url(src_str):
        ...
        missing = (
            ...
            "This repository is not a Ulak profile distribution."
        )
...
def plan_install(source: str, workdir: Path, override_name: Optional[str] = None) -> InstallPlan:
    ...
    from ulak_cli.profiles import _canon_valid, get_profile_dir
    ...
    if manifest is None:
        raise DistributionError(
            f"No {MANIFEST_FILENAME} found at the distribution root — this source is not a Ulak distribution."
        )
    check_ulak_requires(manifest.ulak_requires, ulak_version)  # fail fast
    canon = _canon_valid(override_name or manifest.name)
    if canon == "default":
        raise DistributionError(
            "Cannot install a distribution as 'default' — that is the built-in "
            "root profile (~/.ulak).  Pass --name <name> to install under a new profile."
        )
    manifest.name = canon
...
def _bootstrap_user_dirs(target: Path) -> None:
    ...
    from ulak_cli.profiles import _PROFILE_DIRS
...
def install_distribution(
    ...
    from ulak_cli.profiles import check_alias_collision, create_wrapper_script
    with tempfile.TemporaryDirectory(prefix="ulak_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `ulak profile update` to upgrade in place, or pass --force to overwrite."
            )
...
def _existing_profile(profile_name: str) -> Tuple[str, Path]:
    ...
    from ulak_cli.profiles import _existing_profile_dir
...
def update_distribution(profile_name: str, force_config: bool = False) -> InstallPlan:
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`ulak profile install <source> --name {canon} --force`."
        )
    with tempfile.TemporaryDirectory(prefix="ulak_dist_update_") as tmp:
