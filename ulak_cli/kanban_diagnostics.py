...
_TRIAGE_SLOTS = {
    ...
    False: (
        ...
        "`ulak kanban specify`, which uses auxiliary.triage_specifier.",
    ),
}
...
def _rule_triage_aux_unavailable(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    actions = [_cli_hint(
        f"Configure {primary_slot}", f"ulak config set {primary_slot}.provider auto", suggested=True,
    )]
    if not fallback_explicit and not main_visible:
        actions.append(_cli_hint(
            f"Or configure fallback {fallback_slot}", f"ulak config set {fallback_slot}.provider auto",
        ))
    if not auto_decompose:
        cmd = f"ulak kanban specify {task_id}"
...
def _rule_repeated_failures(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if most_recent_outcome == "spawn_failed" and assignee and assignee != "default":
        ...
        doctor, auth = f"ulak -p {assignee} doctor", f"ulak -p {assignee} auth"
...
def _rule_review_dependency_deadlock(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if task_id:
        actions.append(_cli_hint(
            "Complete the finished implementation phase", f"ulak kanban complete {task_id}",
            suggested=True,
        ))
    if task_id and child_ids:
        actions.append(_cli_hint(
            "Or unlink the incorrectly gated reviewer", f"ulak kanban unlink {task_id} {child_ids[0]}",
        ))
...
def _rule_block_unblock_cycling(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    actions: list[DiagnosticAction] = []
    if task_id:
        cmd = f"ulak kanban events {task_id}"
...
def _rule_stranded_in_ready(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    actions = [
        ...
        _cli_hint("Check dispatcher status", "ulak kanban diagnostics"),
    ]
    return [Diagnostic(
