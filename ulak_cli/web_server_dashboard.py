...
from ulak_cli.config import cfg_get, get_process_ulak_home
from utils import env_var_enabled

# Same logger the code used before extraction (record parity).
_log = logging.getLogger("ulak_cli.web_server")


def _normalise_prefix(raw: Optional[str]) -> str:
    """Normalise an X-Forwarded-Prefix header value (single source of truth lives in
    ``ulak_cli.dashboard_auth.prefix`` so gate, OAuth, cookies and SPA mount agree)."""
    from ulak_cli.dashboard_auth.prefix import normalise_prefix
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    target palette instead of flashing the bundle's default Ulak Teal until
...
_HEADLESS_MSG = (
    "Headless backend (ulak serve): web UI disabled — use "
    "`ulak dashboard` for the browser UI."
)
...
def mount_spa(application: FastAPI):
    ...
    proxy (``X-Forwarded-Prefix: /ulak``) the served index.html is rewritten so absolute
    asset URLs and the runtime ``__ULAK_BASE_PATH__`` honour that prefix without a rebuild.
    ...
    """
    from ulak_cli.web_server import WEB_DIST, _DASHBOARD_EMBEDDED_CHAT_ENABLED, app
    from ulak_cli.web_deps import _server

    # `ulak serve` is the headless backend: it must NEVER serve the browser SPA, even if a
    # dist is lying around, so only the JSON-RPC/WS/API surface is reachable.
    if os.environ.get("ULAK_SERVE_HEADLESS") == "1":

        @application.get("/{full_path:path}")
        async def no_frontend(full_path: str):
            # Desktop token handshake: the Electron shell boots by fetching `/` and reading
            # ``window.__ULAK_SESSION_TOKEN__`` for /api/ws auth. When headless 404'd every
            # path, a renderer whose spawn token no longer matched (e.g. after `ulak update`)
            ...
            if full_path == "" and not gated:
                return HTMLResponse(
                    ...
                    f"window.__ULAK_SESSION_TOKEN__={json.dumps(_server()._SESSION_TOKEN)};"
                    "window.__ULAK_AUTH_REQUIRED__=false;"
    ...
    # A missing WEB_DIST is deliberately NOT a mount-time terminal state (#82614): a long-lived `ulak
    ...
    def _serve_index(prefix: str = ""):
        ...
        cookie auth; ``__ULAK_AUTH_REQUIRED__`` tells it which scheme to use for /api/pty
        ...
        token_js = "" if gated else f'window.__ULAK_SESSION_TOKEN__="{_server()._SESSION_TOKEN}";'
        bootstrap_script = (
            f"<script>{token_js}"
            f"window.__ULAK_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            f'window.__ULAK_BASE_PATH__="{prefix}";'
            f"window.__ULAK_AUTH_REQUIRED__={'true' if gated else 'false'};"
            f"</script>"
        )
        if prefix:
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Ulak Teal",         "description": "Classic dark teal — the canonical Ulak look"},
    {"name": "default-large", "label": "Ulak Teal (Large)", "description": "Ulak Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    ~/.ulak/, the same trust level as config.yaml. Empty asset values are dropped so a
...
def _discover_user_themes() -> list:
    ...
    Uses the process launch home, not ``get_ulak_home()``, so a transient profile override
    from embedded chat does not hide themes under the server's own ``ULAK_HOME``.
    """
    themes_dir = get_process_ulak_home() / "dashboard-themes"
...
def _dashboard_plugin_search_dirs() -> List[tuple]:
    ...
    When the process itself is profile-scoped (``ULAK_HOME=<root>/profiles/<name>``) the
    launch home has no ``plugins/`` — user plugins live in the ulak root — so the default
    ...
    # context-local ULAK_HOME override (e.g. embedded /chat under --open-profile). #87197: when the
    # process itself is profile-scoped (``--profile <name>`` sets ``ULAK_HOME=<root>/profiles/<name>``),
    # the launch home is the profile directory, which has no ``plugins/`` — user plugins are installed in
    # the ulak root (``~/.ulak/plugins``). Scan the default root as well (``get_default_ulak_root()``
    # unwraps ``<root>/profiles/<name>`` → ``<root>`` and returns a custom ``ULAK_HOME`` unchanged when it
    # *is* the root), mirroring how ``ulak_cli.plugins`` resolves plugin install locations. The
    # ``seen_names`` dedupe below keeps profile-local plugins (if any) authoritative over same-named root
    # plugins.
    user_plugin_roots = [get_process_ulak_home() / "plugins"]
    root_plugins = get_default_ulak_root() / "plugins"
    ...
    # ``true`` / ``yes`` / ``on``) so the gate matches ``ulak_cli/plugins.py`` and the documented user
    # contract.
    if env_var_enabled("ULAK_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".ulak" / "plugins", "project"))
    return search_dirs
...
def _discover_dashboard_plugins() -> list:
    ...
    project plugin sources — same three sources as ``ulak_cli.plugins``."""
    plugins = []
    seen_names: set = set()
...
def _merged_plugins_hub(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    """
    from ulak_cli.web_server_memory import _discover_memory_provider_statuses, _normalize_memory_provider_name
    from ulak_cli.web_server import _get_dashboard_plugins
    from ulak_cli.config import get_ulak_home, load_config
    global _plugins_hub_cache, _plugins_hub_cache_expires_at
    now = time.monotonic()
    ...
    from ulak_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_ulak_home() / "plugins").resolve()
...
def _plugin_api_mount_skip_reason(plugin: Dict[str, Any], enabled_set: set, disabled_set: set) -> Optional[str]:
    ...
    disable; project plugins (``./.ulak/plugins/``) ship with the CWD and are
...
def _mount_plugin_api_routes():
    ...
    (``./.ulak/plugins/``) ship with the CWD and are therefore attacker-controlled in any threat model
    ...
    """
    from ulak_cli.web_server import _get_dashboard_plugins, app
    try:
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"ulak_dashboard_plugin_{plugin['name']}"
