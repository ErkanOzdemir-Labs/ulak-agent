...
DEFAULT_API_URL = "https://setup.ulak-agent.github.com/ErkanOzdemir-Labs"
TELEGRAM_ONBOARDING_URL_ENV = "TELEGRAM_ONBOARDING_URL"
DEFAULT_BOT_NAME = "Ulak Agent"
...
def auto_setup_telegram_bot_result(
    api_url: str | None = None, manager_bot: str = "UlakSetupBot",
    ...
    print(f"\n  Contacting Ulak Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Ulak Telegram onboarding service.\n"
...
DEFAULT_MANAGER_BOT = "UlakSetupBot"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``ulak_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``ulak_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"ulak_{generate_username_slug()}_bot"
