...
from ulak_cli.pty_session import RegistryFull
from ulak_cli.web_deps import LateState, late
from ulak_cli.web_server_chat import (
...
_active_session_file_for_channel = late("_active_session_file_for_channel", "ulak_cli.web_server_chat")
_profile_scope = late("_profile_scope", "ulak_cli.web_server_profiles")
_resolve_chat_argv_async = late("_resolve_chat_argv_async", "ulak_cli.web_server_chat")
_resolve_profile_dir = late("_resolve_profile_dir", "ulak_cli.web_server_profiles")
_ws_auth_reason = late("_ws_auth_reason", "ulak_cli.web_server_chat")
_ws_client_reason = late("_ws_client_reason", "ulak_cli.web_server_chat")
_ws_host_origin_reason = late("_ws_host_origin_reason", "ulak_cli.web_server_chat")
...
def _ws_auth_mode() -> str:
    ...
    from ulak_cli.web_server_chat import _LOOPBACK_HOSTS
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from ulak_cli.console_engine import UlakConsoleEngine

        engine = UlakConsoleEngine(output_limit=_CONSOLE_OUTPUT_LIMIT)
        ...
        try:
            ...
            if command_id == command_generation:
                ...
                await out.error_then_complete(
                    "Command timed out. Ulak Console returned to the prompt.", line, command_id, "timeout",
                )
        except Exception as exc:
            if command_id == command_generation:
                pending_confirmation = None
...
async def pty_ws(ws: WebSocket) -> None:
    from ulak_cli.web_server_chat import PTY_REGISTRY, PtyBridge, PtyUnavailableError, _PTY_BRIDGE_AVAILABLE, _RESIZE_RE
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Ulak inside WSL2 to use the dashboard's /chat "
    ...
    if raw_resume and env:
        registry_resume = env.get("ULAK_TUI_RESUME") or raw_resume
...
async def gateway_ws(ws: WebSocket) -> None:
    ...
    await handle_ws(
        ws,
        auth_identity=getattr(ws, "_ulak_auth_identity", None),
        subprotocol=getattr(ws, "_ulak_ws_subprotocol", None),
    )


# --- /api/pub + /api/events: the PTY-side tui_gateway.entry opens /api/pub
# (ULAK_TUI_SIDECAR_URL from /api/pty's env) and writes every dispatcher emit
