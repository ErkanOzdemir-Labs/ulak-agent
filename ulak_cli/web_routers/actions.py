"""Gateway restart/drain, Ulak update and background-action status dashboard routes.

Extracted from ``ulak_cli.web_server``; helpers/state that tests monkeypatch on
...
from ulak_cli.config import format_docker_update_message, recommended_update_command_for_method
from ulak_cli.web_deps import LateState, late
from ulak_cli.web_server_gateway import _ACTION_LOG_FILES
from ulak_cli.web_routers._common import http_failure
...
detect_install_method = late("detect_install_method", "ulak_cli.config")
get_ulak_home = late("get_ulak_home", "ulak_cli.config")
_ACTION_COMMANDS = LateState("_ACTION_COMMANDS", "ulak_cli.web_server_gateway")
_ACTION_IDS = LateState("_ACTION_IDS", "ulak_cli.web_server_gateway")
_ACTION_PROCS = LateState("_ACTION_PROCS", "ulak_cli.web_server_gateway")
_ACTION_RESULTS = LateState("_ACTION_RESULTS", "ulak_cli.web_server_gateway")
...
def _server_path(name: str) -> Path:
    ...
    import ulak_cli.web_server_gateway as gw
...
_UPDATE_ACTION_COMPLETED_RE = re.compile(r"^=== ulak-update completed ([0-9a-f]{32}) ===$")

_MANAGED_EXTERNALLY_MESSAGE = "Ulak updates are managed outside this dashboard in containerized environments."
...
def _durable_completed_update_action_id(lines: List[str]) -> Optional[str]:
    ...
    for index, line in enumerate(lines):
        if line.startswith("=== ulak update started "):
            last_start = index
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``ulak gateway restart`` in the background."""
...
def _update_refused(error: str, message: str, update_command: str) -> Dict[str, Any]:
    _record_completed_action("ulak-update", message, exit_code=1)
    return {
        "ok": False, "pid": None, "name": "ulak-update", "error": error, "message": message,
        "update_command": update_command,
    }


@router.post("/api/ulak/update")
async def update_ulak():
    """Kick off ``ulak update`` in the background."""
    ...
    existing = _ACTION_PROCS.get("ulak-update")
    if existing is not None and existing.poll() is None:
        response = {"ok": True, "pid": existing.pid, "name": "ulak-update", "already_running": True}
        action_id = _ACTION_IDS.get("ulak-update")
        if action_id:
            response["action_id"] = action_id
        return response

    action_id = secrets.token_hex(16)
    with http_failure("Failed to spawn ulak update", 500, "Failed to start update"):
        proc = _spawn_ulak_action(["update"], "ulak-update", env_overrides={"ULAK_ACTION_ID": action_id})
    return {"ok": True, "pid": proc.pid, "name": "ulak-update", "action_id": action_id}
...
_NON_APPLYABLE_MESSAGES = {
    ...
    "apt": lambda: "Ulak is managed by Termux APT; run `pkg upgrade ulak-agent`.",
}


@router.get("/api/ulak/update/check")
async def check_ulak_update(force: bool = False):
    """Report whether a Ulak update is available, without applying it.
    ...
    try:
        from ulak_cli.banner import check_for_updates

        if force:
            with contextlib.suppress(OSError):
                (get_ulak_home() / ".update_check").unlink()
...
async def get_action_status(name: str, lines: int = 200):
    ...
    if name == "ulak-update":
        ...
        if durable_update_action_id:
            marker = f"=== ulak-update completed {durable_update_action_id} ==="
...
def _latest_update_receipt_summary() -> Optional[Dict[str, Any]]:
    """Compact summary of the latest receipt (written by EVERY ``ulak update`` run,
    incl. refused/failed), or None; never raises. Steps/skips stay in the full endpoint.

    Phase-1 bullet 3 (#91277): the receipt (written by EVERY ``ulak update`` run since #91283, including
...
@status_router.get("/api/ulak/update/receipt")
async def get_update_receipt():
    ...
    receipt = _read_latest_receipt()
    if not receipt:
        raise HTTPException(status_code=404, detail="No update receipt found (no `ulak update` run recorded).")
