"""``ulak dashboard register`` — register a self-hosted dashboard OAuth client.
...
write ``ULAK_DASHBOARD_OAUTH_CLIENT_ID`` (+ portal/public URL when warranted) into ``.env``
...
_DEFAULT_PORTAL = "https://portal.github.com/ErkanOzdemir-Labs"
    ...
    try:
        ...
        if exc.code == 401:
            message = ("Nous Portal rejected the access token (401). "
                       "Try `ulak auth add nous` to re-authenticate.")
        elif exc.code == 403:
...
def _print_post_register_hint(
    *, client_id: str, portal_base_url: str, custom_redirect_uri: Optional[str],
    ...
    from ulak_cli.config import get_env_path
    print(f"\n  Wrote to {get_env_path()}:\n    ULAK_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
    if wrote_portal_url:
        print("    ULAK_DASHBOARD_PORTAL_URL=" + str(portal_base_url))
    if public_url:
        print("    ULAK_DASHBOARD_PUBLIC_URL=" + str(public_url))
    print(
        "\n  Heads up — Nous login only *engages* on a non-loopback bind. A plain\n"
        "  `ulak dashboard` (localhost) leaves the gate off and serves locally\n"
...
def _public_url_from_redirect(redirect_uri: Optional[str]) -> str:
    ...
    ``/auth/callback`` to ULAK_DASHBOARD_PUBLIC_URL, so the raw URI would double the path."""
    try:
...
def cmd_dashboard_register(args) -> None:
    ...
    from ulak_cli.auth import AuthError, resolve_nous_access_token
    from ulak_cli.config import is_managed, save_env_value
    # Managed installs get the client id stamped in by the orchestrator (save_env_value refuses).
    if is_managed():
        print("✗ `ulak dashboard register` is not available in a managed/hosted install.\n"
    ...
    try:
        ...
        if isinstance(exc, AuthError) and getattr(exc, "relogin_required", False):
            print("✗ You're not logged into Nous Portal.\n"
                  "  Run `ulak setup` (or `ulak auth add nous`) first, then retry.")
        else:
    ...
    portal_override = getattr(args, "portal_url", None) or os.environ.get("ULAK_DASHBOARD_PORTAL_URL")
    ...
    stored = _env_value("ULAK_DASHBOARD_OAUTH_CLIENT_ID")
    ...
    try:  # client_id is load-bearing: fatal on failure
        save_env_value("ULAK_DASHBOARD_OAUTH_CLIENT_ID", client_id)
    except Exception as exc:
        print(f"✗ Failed to write ULAK_DASHBOARD_OAUTH_CLIENT_ID to .env: {exc}\n"
              f"  Set it manually:  ULAK_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
        sys.exit(1)
    # Explicit portal → always persist (the user asked); inferred → only if unset AND non-default.
    existing_portal = _env_value("ULAK_DASHBOARD_PORTAL_URL")
    should_write_portal = (
    ...
    wrote_portal_url = should_write_portal and _save_env_quietly("ULAK_DASHBOARD_PORTAL_URL", portal_base_url)
    ...
    wrote_public_url = bool(
        public_url
        and _env_value("ULAK_DASHBOARD_PUBLIC_URL") != public_url
        and _save_env_quietly("ULAK_DASHBOARD_PUBLIC_URL", public_url))
