"""Configuration management for Ulak Agent: config.yaml / .env loading, saving,
validation, migration, and the ``ulak config`` command."""
...
from ulak_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
from ulak_cli.secret_prompt import masked_secret_prompt
# Re-export from ulak_constants — canonical definition lives there.
from ulak_constants import get_ulak_home, get_process_ulak_home  # noqa: F401
...
_PARSE_FAILURE_FALLBACK_MSG = {
    ...
    "refuse-write": (
        ...
        "Fix the YAML (ulak config edit) and retry.")}
...
def _warn_config_parse_failure(
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  ulak config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
# interpreter init (PYTHON*, NODE_*: Ulak restarts through them), PATH (fix tool lookup
...
# and Ulak runtime-location / security-policy flags (config.yaml is the supported surface).
#
# ``ULAK_*`` overall is NOT blocked — many integration credentials use that prefix
# (ULAK_LANGFUSE_PUBLIC_KEY, ULAK_SPOTIFY_CLIENT_ID, ...). The denylist is name-by-name so
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Ulak runtime location
    "ULAK_HOME", "ULAK_PROFILE", "ULAK_CONFIG", "ULAK_ENV",
    "ULAK_CONFIG_PATH", "ULAK_ENV_PATH",
    ...
    "ULAK_COPILOT_ACP_COMMAND", "ULAK_COPILOT_ACP_ARGS",
    # Security policy / approval-routing context — set via their dedicated controls only.
    "ULAK_YOLO_MODE", "ULAK_ACCEPT_HOOKS", "ULAK_REDACT_SECRETS",
    "ULAK_INTERACTIVE", "ULAK_EXEC_ASK", "ULAK_GATEWAY_SESSION",
    "ULAK_CRON_SESSION", "ULAK_SINGLE_QUERY_SESSION",
    "ULAK_SESSION_KEY", "ULAK_SESSION_PLATFORM"})
...
def validate_env_var_name_for_write(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "or Ulak runtime location and security policy (ULAK_HOME, ULAK_YOLO_MODE, ...) "
            "cannot be persisted via the env writer. If you really need this, edit ~/.ulak/.env "
            "directly.")
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # back-compat fallback. The boolean ULAK_TOOL_PROGRESS variant is unsupported (its only
    # consumer, the v3->4 migration, is below the v12 support floor); doctor flags it as ignored.
    "ULAK_TOOL_PROGRESS_MODE",
    ...
    "ULAK_LANGFUSE_ENV", "ULAK_LANGFUSE_RELEASE", "ULAK_LANGFUSE_SAMPLE_RATE",
    "ULAK_LANGFUSE_MAX_CHARS", "ULAK_LANGFUSE_CAPTURE", "ULAK_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY", "LANGFUSE_SECRET_KEY", "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so profiles can use different backends.
    "ULAK_ACP_AUTH_METHOD", "ULAK_ACP_AUTO_APPROVE", "ULAK_COPILOT_ACP_COMMAND",
    "ULAK_COPILOT_ACP_ARGS", "COPILOT_CLI_PATH", "COPILOT_ACP_BASE_URL"})
...
# ULAK_MANAGED). Module-level so tests can patch it without touching /nix/store.
...
def get_managed_system() -> Optional[str]:
    ...
    Signals: ULAK_MANAGED env var (systemd service) or a ``.managed`` marker file in
    ULAK_HOME (NixOS activation script — interactive shells don't see the service env)."""
    marker = os.getenv("ULAK_MANAGED", "").strip().lower() or None
    managed_marker = get_ulak_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Ulak is running in package-manager-managed mode."""
...
_NIX_UPDATE_MSG = (
    "Update Ulak through the Nix source that installed it "
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Ulak was installed: apt/docker/nix/nixos/home-manager/git/unknown.
    Order: code-scoped ``<install tree>/.install_method`` stamp (authoritative) -> legacy
    ``$ULAK_HOME/.install_method`` -> managed marker -> /nix/store path -> .git dir -> unknown.
    The stamp lives next to the code because ULAK_HOME is shared data: a container and a host
    install can bind-mount the same home, so a home-scoped ``docker`` stamp would make the host
    ``ulak update`` refuse to run. A legacy ``docker`` value is therefore ignored unless we are
    ...
    the code; - the published ``erkanozdemir-labs/ulak-agent`` image bakes a ``docker`` stamp into
    ``/opt/ulak`` at build time. An unsupported manual install dropped into a container (no stamp) falls
    through to the ``.git`` checks and behaves like any off-path install. See issue #34397.
    """
    # The stamp is a property of the running code tree (parent of ulak_cli/), NOT of $ULAK_HOME,
    ...
    method = _install_method_stamp(get_ulak_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Import-safe wrapper around ``ulak_constants.is_container``."""
    try:
...
_UPDATE_COMMAND_BY_METHOD = {
    "docker": "docker pull erkanozdemir-labs/ulak-agent:latest",
    "apt": "pkg upgrade ulak-agent",  # "apt" == Termux APT by contract; uses Termux's `pkg`.
}
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    return _UPDATE_COMMAND_BY_METHOD.get(method, "ulak update")
...
# Shared by ``cmd_update`` and ``_cmd_update_check`` (ulak_cli/main.py) so the wording never
# forks. The published image excludes ``.git``, so the git update path can never succeed there
# and the generic "reinstall via install.sh" fallback would install a NEW host-side Ulak.
_DOCKER_UPDATE_MESSAGE = """\
✗ ``ulak update`` doesn't apply inside the Docker container.

Ulak Agent runs as a published image (erkanozdemir-labs/ulak-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull erkanozdemir-labs/ulak-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate ulak-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm erkanozdemir-labs/ulak-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/erkanozdemir-labs/ulak-agent/tags
  • Your config and session history live under ``$ULAK_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``ulak update`` inside Docker."""
    return _DOCKER_UPDATE_MESSAGE


def format_managed_message(action: str = "modify this Ulak installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this Ulak installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Ulak.")
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ULAK_HOME/.container-mode.
    ...
    already inside the container, or when ULAK_DEV=1 is set. Only FileNotFoundError is
    swallowed; other errors (permissions, malformed data) propagate."""
    if os.environ.get("ULAK_DEV") == "1":
        return None
    ...
    try:
        info = {}
        with open(get_ulak_home() / ".container-mode", "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
    ...
    return {
        ...
        "container_name": info.get("container_name", "ulak-agent"),
        "exec_user": info.get("exec_user", "ulak"),
        "ulak_bin": info.get("ulak_bin", "/data/current-package/bin/ulak")}


# ---- Config paths / ULAK_HOME skeleton ----

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_ulak_home() / "config.yaml"
...
def require_parseable_user_config(*, ignore_user_config: bool = False) -> None:
    ...
    states; ``--ignore-user-config`` / ULAK_IGNORE_USER_CONFIG=1 remain authoritative."""
    if ignore_user_config or os.environ.get("ULAK_IGNORE_USER_CONFIG") == "1":
        return
...
def _resolve_ulak_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read ULAK_UID / ULAK_GID (set by Docker deployments); (None, None) if unset/invalid/Windows.
    The entrypoint chowns ULAK_HOME once, but subdirs created at runtime (``profiles/<name>/``)
    need the same chown or they land root:root and block later uid-mapped workers.

    Docker containers running Ulak commonly set these to map the in-container user to a host user so
    ...
    return _env_int("ULAK_UID"), _env_int("ULAK_GID")


def _chown_to_ulak_uid(path) -> None:
    """Chown ``path`` to ``ULAK_UID:ULAK_GID`` when set; EPERM/ENOENT are non-fatal (the
    ...
    :func:`ensure_ulak_home` on Docker deployments. See #34107.
    """
    uid, gid = _resolve_ulak_uid_gid()
...
def _secure_dir(path):
    """chmod a directory owner-only (0700) and apply ULAK_UID/GID ownership. No-op when managed.
    ULAK_HOME_MODE (e.g. 0701) overrides the mode so a web server can traverse ULAK_HOME to
    a served subdirectory without directory listings.

    Also applies ``ULAK_UID``/``ULAK_GID``-based ownership when those env vars are set (#34107 — Docker
    ...
    if is_managed():
        return
    try:
        mode = int(os.environ.get("ULAK_HOME_MODE", "").strip() or "700", 8)
    except ValueError:
        mode = 0o700
    try:
        os.chmod(path, mode)
        ...
        pass
    _chown_to_ulak_uid(path)


def _is_container() -> bool:
    """Detect Docker/Podman/LXC (or ULAK_CONTAINER / ULAK_SKIP_CHMOD opt-out).
    ...
    if (os.environ.get("ULAK_CONTAINER") or os.environ.get("ULAK_SKIP_CHMOD")
...
_ULAK_HOME_ENSURED: set = set()
_ULAK_HOME_SUBDIRS = (
    "cron", "sessions", "logs", "logs/curator", "memories",
    "pairing", "hooks", "image_cache", "audio_cache", "skills")


def ensure_ulak_home():
    """Ensure the ~/.ulak directory skeleton exists with secure permissions.
    ...
    from ulak_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _ULAK_HOME_ENSURED and home.is_dir():
        return
    if is_managed():
        ...
        try:
            if not home.is_dir():
                raise RuntimeError(f"ULAK_HOME {home} does not exist.")
        ...
        _secure_dir(home)
        for subdir in _ULAK_HOME_SUBDIRS:
            d = home / subdir
            d.mkdir(parents=True, exist_ok=True)
            _secure_dir(d)
        _ensure_default_soul_md(home)

    _ULAK_HOME_ENSURED.add(key)


# ---- Config loading/saving ----

from ulak_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: E402,F401
from ulak_cli.config_providers import (  # noqa: E402,F401  (re-exported; callers/tests use ulak_cli.config.<name>)
...
# Back-compat re-exports — :mod:`ulak_cli.personality` owns personality/overlay semantics.
from ulak_cli.personality import (  # noqa: E402,F401
...
def _split_key_path(key: str) -> list[str]:
    ...
    ``ulak config set`` uses ``.`` as the nesting separator, so a key that itself contains a literal dot
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `ulak config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    return (
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # A malformed SKILL.md must never break `ulak update`; this prompting is a nicety.
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (ulak_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard
    "platform_toolsets",     # written by the setup wizard
    "known_plugin_toolsets", # ulak_cli/tools_config.py toolset-save flow
    ...
    for _key in ("backend", "search_backend", "extract_backend"):
        ...
        if note:
            _issue(issues, "warning",
                   ...
                   "Run 'ulak tools' and pick a different Web Search & Extract provider")
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'ulak setup' to create a valid config")]
    ...
    if cp and not config.get("model"):
        _issue(issues, "warning",
               "custom_providers defined but no 'model' section — Ulak won't know which provider to use",
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'ulak doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars() -> None:
    ...
    if lines:
        from ulak_constants import display_ulak_home

        hint_path = display_ulak_home()
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from ulak_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ ulak config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
...
def _disable_suspicious_mcp_servers(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if not isinstance(raw_mcp_servers, dict):
        return
    try:
        from ulak_cli.mcp_security import validate_mcp_server_entry
    except Exception:
        return
    mcp_touched = False
...
def _warn_invalid_platform_toolsets(results: Dict[str, Any], quiet: bool) -> None:
    ...
    try:
        ...
        from ulak_cli.toolset_validation import validate_platform_toolsets
        from ulak_cli.toolset_scope import toolset_allowed_for_platform
...
def _offer_list(heading: str, items: List[str], question: str) -> bool:
    ...
    if not _ask_yes_no(question):
        print("  Set later with: ulak config set <key> <value>")
        return False
    print()
    return True
...
def _env_expand_match(m: re.Match) -> str:
    ...
    if inner.startswith("env:"):
        logger.warning(
            "Config ref %r: %s is not set (check ~/.ulak/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    made against a different environment (load before ``load_ulak_dotenv()``, in-process
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    LiteLLM users reach for, and ``ulak config set`` blindly accepts any dotted key — so
    ...
    from OpenAI-compatible backends) — while display paths (``ulak status``/``dump``) read ``name`` and
...
# the ULAK_MAX_ITERATIONS env bridge, works in every ``<``/``>=``/``max - used`` comparison in
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    """Whether the remote SSH shell must expand *cwd* itself: ``~`` expanded on the Ulak host
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # load_ulak_dotenv() pins unexpanded literals for the process lifetime.
            # Without this, a load_config() that ran before load_ulak_dotenv() pins unexpanded literals
...
#   openai-codex (OAuth — ulak auth) — OpenAI Codex
#   nous         (OAuth — ulak auth) — Nous Portal
...
def save_config(
    ...
    """Save configuration to ~/.ulak/config.yaml.
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Ulak writes itself (bare, 'single', or "double" with
    ``\\"`` / ``\\\\`` escapes)."""
...
def load_env() -> Dict[str, str]:
    """Load ~/.ulak/.env as a dict (memoised; ``get_env_value()`` runs hundreds of times per
...
def sanitize_env_file() -> int:
    """Rewrite ~/.ulak/.env with normalized line formatting; returns the number of changed lines."""
    env_path = get_env_path()
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.ulak/.env (also matching ``export KEY=`` lines, so a save
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    so two endpoints on one host get separate slots. The fixed ``ULAK_CUSTOM_`` prefix keeps the
    ...
    return f"ULAK_CUSTOM_{slug}_API_KEY" if slug else "ULAK_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.ulak/.env and os.environ; True if it was found and removed."""
...
def save_env_value_secure(key: str, value: str) -> Dict[str, Any]:
    ...
    from ulak_cli.credential_lifecycle import save_provider_env_credential
...
def reload_env() -> int:
    """Re-read ~/.ulak/.env into os.environ; returns count of vars changed.
    Removes deleted vars only when known to Ulak (OPTIONAL_ENV_VARS and _EXTRA_ENV_KEYS) so
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` (scope-aware) or ``~/.ulak/.env``.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a Ulak-managed credential preferring ``~/.ulak/.env`` over ``os.environ``, so a
...
def _show_model_section(config: Dict[str, Any]) -> None:
    ...
    # Read the .env FILE directly so a stale ULAK_MAX_ITERATIONS ghost is caught even when the
    # gateway bridge already overrode os.environ.
    try:
        env_ghost = load_env().get("ULAK_MAX_ITERATIONS")
    except Exception:
        env_ghost = None
    if env_ghost is not None and str(env_ghost).strip() != str(cfg_max_turns).strip():
        print(color(f"                ⚠ .env has stale ULAK_MAX_ITERATIONS={env_ghost} "
                    f"(run 'ulak doctor --fix' to remove)", Colors.YELLOW))
...
def _show_display_section(config: Dict[str, Any]) -> None:
    ...
    try:
        from ulak_cli.personality import active_personality_name
...
def show_config():
    ...
    print(color("│              ⚕ Ulak Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from ulak_cli.auth import get_anthropic_key
    ...
    print(color("  ulak config edit     # Edit config file", Colors.DIM))
    print(color("  ulak config set <key> <value>", Colors.DIM))
    print(color("  ulak setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    Mirrors the scheduler's precedence: a truthy configured model wins over ``ULAK_MODEL``; the
    ...
    return provider, configured_model or _model_assignment_text(env.get("ULAK_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "model/provider. Inspect with `ulak cron list`, then pin the intended values with "
        "`ulak cron edit <job_id> --provider <provider> --model <model>`.")
...
def _coerce_config_set_value(key: str, value: str) -> Any:
    """Auto-coerce a ``ulak config set`` string to bool/None/int/float/list/dict.
...
def _redirect_platform_display_key(key: str) -> tuple[str, Optional[str]]:
    ...
    Before #71047 a write such as ``ulak config set platforms.telegram.streaming false`` landed on a key
...
def _guard_section_overwrite(key: str, value: Any, user_config: Dict[str, Any], force: bool) -> str:
    ...
    err += [
        ...
        f"    ulak config set {key}.<sub-key> <value>",
        "  Or use --force to replace the entire section:",
        f"    ulak config set --force {key} {value!r}"]
...
def _touch_skin_file(key: str, value: Any) -> None:
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_ulak_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
            pass
...
def _print_unknown_key_notice(key: str, suggestion: Optional[str]) -> None:
    print(color(
        ...
        "but Ulak may not read it.", Colors.YELLOW))
    if suggestion:
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    if key.strip().lower() in ("model.api_base", "api_base"):
        # Normalize the api_base → base_url alias at set-time too (issue #8919), so a fresh `ulak config
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # See #51071.
        from ulak_cli.credential_lifecycle import remove_provider_env_credential
...
_USAGE_GET = ("Usage: ulak config get <key> [--json]", [
    "ulak config get model", "ulak config get terminal.backend",
    "ulak config get skills.config --json"], None)
_USAGE_SET = ("Usage: ulak config set [--force] <key> <value>", [
    "ulak config set model anthropic/claude-sonnet-4", "ulak config set terminal.backend docker",
    "ulak config set OPENROUTER_API_KEY sk-or-..."], [
    "", "  --force: skip the unknown-key notice for unrecognized keys,",
    "           and allow a scalar to replace a whole mapping section"])
_USAGE_UNSET = ("Usage: ulak config unset <key>", [
    "ulak config unset model", "ulak config unset terminal.backend",
    "ulak config unset OPENROUTER_API_KEY"], None)
...
def _cmd_config_check(args):
    ...
    if missing_config:
        ...
        print("    Run 'ulak config migrate' to add them")
...
_CONFIG_USAGE = """Available commands:
  ulak config           Show current configuration
  ulak config edit      Open config in editor
  ulak config get <key>          Print a resolved config value
  ulak config set <key> <value>   Set a config value
  ulak config unset <key>        Remove a config value
  ulak config check     Check for missing/outdated config
  ulak config migrate   Update config with new options
  ulak config path      Show config file path
  ulak config env-path  Show .env file path"""
...
def _inject_platform_plugin_env_vars() -> None:
    ...
    Chat etc. are configurable in ``ulak config`` UI without the core knowing they exist.
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``ulak_cli/`` — i.e. the git checkout for source
    installs, ``/opt/ulak`` inside the published image. It is a property of
    the running interpreter, NOT of ``$ULAK_HOME``, which is why a
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$ULAK_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/ulak`` in the published image, which instead bakes the stamp at
...
_PLUGIN_COMPAT_LAZY = {
    'normalize_route_base_url': ('ulak_cli.route_identity', 'normalize_route_base_url'),
}
