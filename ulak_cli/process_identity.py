...
Three layers make every long-lived Ulak process positively identifiable, so reapers (``ulak
update``, Desktop startup sweeps) never guess lineage from PPID archaeology or cmdline matching:
1. spawn tags (``ULAK_SPAWN`` env stamped by the spawner); 2. a ``(pid, create_time)`` ledger;
...
def install_id(project_root: Optional[Path] = None) -> str:
    ...
    if project_root is None:
        try:
            from ulak_constants import PROJECT_ROOT as _root
...
def build_spawn_tag(purpose: str, *, project_root: Optional[Path] = None) -> str:
    """Value for the child's ``ULAK_SPAWN`` env var, stamped by the spawner."""
...
def parse_spawn_tag(raw: object) -> Optional[SpawnTag]:
    """Parse a ``ULAK_SPAWN`` value; ``None`` for anything malformed."""
...
def _ledger_path() -> Path:
    ...
    try:
        ...
        return Path(get_default_ulak_root()) / LEDGER_FILENAME
    except Exception:
        from ulak_cli.config import get_ulak_home

        return Path(get_ulak_home()) / LEDGER_FILENAME
...
def register_self(purpose: str, *, project_root: Optional[Path] = None, detail: Optional[dict] = None) -> bool:
    ...
    try:
        ...
        # 10 tokens: enough for `ulak serve --host X --port N --profile P` while bounding
...
def _desktop_spawner_identity() -> tuple[Optional[int], Optional[float]]:
    """Spawner ``(pid, create_time)`` from the Electron app's ULAK_PARENT_PID (+ optional
    ...
    try:
        spawner_pid = int(os.environ.get("ULAK_PARENT_PID", ""))
    except (TypeError, ValueError):
        spawner_pid = 0
    if spawner_pid <= 0:
        return None, None
    marker = os.environ.get("ULAK_PARENT_START_MARKER", "")
...
def register_child(pid: int, purpose: str, *, project_root: Optional[Path] = None) -> bool:
    ...
    helpers: arbitrary ``npx``/binary servers never import Ulak code). Records the child's
