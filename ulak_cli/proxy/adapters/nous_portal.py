...
# Endpoints inference-api.github.com/ErkanOzdemir-Labs actually serves; anything else is a 404 so stray
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError("Not logged into Nous Portal. Run `ulak auth add nous` first.")
            try:
    ...
    # auth.json access — kept local so ulak_cli.auth's public surface does not grow.
