...
Mixed into :class:`ulak_cli.plugins.PluginManager`. ``_resolve_hook_callback_timeout`` stays on
...
from ulak_cli.middleware import OBSERVER_SCHEMA_VERSION
...
PLUGIN_SECTIONS_START = "<!-- ulak-plugin-sections:start -->"
PLUGIN_SECTIONS_END = "<!-- ulak-plugin-sections:end -->"
...
def format_system_prompt_section(section_id: str, content: str) -> str:
    ...
    return (
        ...
        f"<!-- ulak-plugin-section-chars:{len(content)} -->\n\n{content}")
...
# Reserved event namespace prefix — only core may publish ``ulak:<event>``.
ULAK_EVENT_NAMESPACE = "ulak"
...
class PluginDispatchMixin:
    ...
    def invoke_hook(self, hook_name: str, **kwargs: Any) -> List[Any]:
        ...
        """
        from ulak_cli.plugins import _resolve_hook_callback_timeout
    ...
    def _run_hook_callback_bounded(
        ...
        thread = threading.Thread(target=_runner, name=f"ulak-hook-{callback_name}"[:40], daemon=True)
        thread.start()
    ...
    def _ensure_event_worker_locked(self) -> None:
        ...
        worker = threading.Thread(
            target=self._event_worker_loop, args=(self._event_queue,), name="ulak-plugin-events",
            daemon=True,
        )
    ...
    def _deliver_event(self, item: _QueuedPluginEvent) -> None:
        ...
        from ulak_cli.plugins import resolve_plugin_command_result
        with self._event_lock:
