...
def _warm_gateway_module() -> None:
    ...
    for mod in (
        "ulak_cli.gateway",
        "ulak_cli.auth",  # provider auth state → copilot_auth → subprocess
        "ulak_cli.copilot_auth",
        "ulak_cli.runtime_provider",
        "ulak_cli.skin_engine",  # resolve_skin() config + skin engine init
        "ulak_cli.inventory",  # provider catalogs + models.dev cache
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    """Publish the port through an atomic ready file when ``ULAK_DESKTOP_READY_FILE`` is set.
    ...
    """
    target = os.environ.get("ULAK_DESKTOP_READY_FILE")
    if not target:
        return
...
def _is_serve_orphaned(
    ...
    *,
    ...
    ``ULAK_PARENT_PID`` is the Electron PID, not necessarily our PPID (the
    Windows ``ulak.exe`` launcher adds shims), so never compare getppid().
...
def _start_parent_death_watchdog() -> None:
    ...
    """
    raw_pid = os.environ.get("ULAK_PARENT_PID")
    # Empty inherited values mean "absent", not "marker present but blank".
    start_marker = os.environ.get("ULAK_PARENT_START_MARKER") or None
    nonce = os.environ.get("ULAK_PARENT_NONCE") or None
    ...
    if has_marker and (not _valid_parent_start_marker(start_marker or "") or not nonce or nonce != nonce.strip()):
        ...
        _log.warning(
            "Parent-death watchdog disabled: unusable ULAK_PARENT_START_MARKER=%r / nonce; "
    ...
    try:
        poll = max(0.5, float(os.environ.get("ULAK_SERVE_WATCHDOG_POLL_S", "2.0")))
    except (TypeError, ValueError):
        poll = 2.0
...
def _report_port_in_use(host: str, port: int) -> None:
    ...
    print(
        ...
        "'ulak serve' / 'ulak dashboard' backend or the Ulak gateway. "
