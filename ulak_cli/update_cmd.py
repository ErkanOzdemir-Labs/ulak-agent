"""Ulak update pipeline: dispatchers (``_cmd_update_impl``/``_cmd_update_check``) + git plumbing.

Each concern lives in ``update_cmd_<concern>.py`` and is re-imported here so
``ulak_cli.update_cmd.<name>`` keeps resolving (and stays monkeypatchable). Imports are one-way:
main -> update_cmd -> update_cmd_*; ``_m()`` resolves ``ulak_cli.main`` at call time.
"""
...
from ulak_cli.config import get_ulak_home  # noqa: F401  (re-exported; patched via update_cmd)
from ulak_cli.update_cmd_common import _best_effort
from ulak_constants import get_default_ulak_root, venv_python_path

# Re-exports: every split-module name stays reachable (and monkeypatchable) as update_cmd.<name>.
from ulak_cli.update_abort_recovery import (  # noqa: F401
...
from ulak_cli.update_cmd_windows import (  # noqa: F401
    ...
    _handoff_reapable_backend_pids, _ulak_holder_subcommand, _holder_value_flags,
...
from ulak_cli.update_cmd_fleet import (  # noqa: F401
...
from ulak_cli.update_cmd_zip import (  # noqa: F401
...
from ulak_cli.update_cmd_stash import (  # noqa: F401
...
from ulak_cli.update_cmd_config import (  # noqa: F401
...
from ulak_cli.update_cmd_deps import (  # noqa: F401
...
from ulak_cli.update_cmd_git import (  # noqa: F401
...
from ulak_cli.update_cmd_maint import (  # noqa: F401
    ...
    _print_verified_update_completion, _purge_stale_ulak_modules, _read_project_version,
...
def _m():
    """Lazy ``ulak_cli.main`` handle: keeps main-side test patches effective, import one-way."""
    from ulak_cli import main
    return main
...
_UPDATE_CRITICAL_FILES = (
    "ulak_cli/main.py", "ulak_cli/config.py", "ulak_cli/__init__.py",
    "ulak_cli/web_server.py", "cli.py", "run_agent.py", "model_tools.py", "toolsets.py",
    "ulak_constants.py")
...
def _record_update_step(step: str, ok: bool, detail: str = "") -> None:
    ...
    with suppress(Exception):
        from ulak_cli.update_receipt import record_step
...
def _validate_python_files_syntax(root, relpaths) -> tuple[bool, str | None, str | None]:
    ...
    import tempfile
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="ulak-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    from ulak_constants import get_ulak_home  # noqa: F811  (deliberate: constants variant)
    home = get_ulak_home()
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    See #87331.
    """
    print("✗ Cannot continue the update: live Ulak launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["ulak.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically Ulak")
    print("  Desktop, a gateway, or another ulak REPL — and mutating the venv")
    print("  now would strand it half-updated.")
    print("  The dependency install has been deferred: close the process(es)")
    print("  above, then run any `ulak` command to finish it automatically.")
...
def _invalidate_update_cache():
    ...
    default_home = get_default_ulak_root()
...
def _format_concurrent_instances_message(matches: list[tuple[int, str]], scripts_dir: Path) -> str:
    """Explanation + remediation hint for the Windows concurrent-ulak.exe gate."""
    shim = scripts_dir / "ulak.exe"
    lines = [
...
def _classify_concurrent_instance(pid: int) -> str:
    ...
    from ulak_cli._scan_venv_blockers import _is_pausable_gateway  # noqa: PLC0415
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """``ulak update --check``: fetch and report without installing. ``branch_explicit`` is
    ...
    from ulak_cli.update_contract import evaluate_update_admission, record_refusal_receipt
    ...
    from ulak_cli.gitlock import clear_stale_git_locks, clear_stale_tmp_packs
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            return
        from ulak_cli.banner import _github_compare_behind
...
def _repair_venv_on_current_checkout(
    *, assume_yes, gateway_mode, pre_update_snapshot_id, desktop_dir,
    ...
    _write_update_incomplete_marker()
    from ulak_cli.managed_uv import ensure_uv
    repair_uv = ensure_uv()
    ...
    if not healthy_after:
        ...
        print("  Close all Ulak windows/gateways and re-run: ulak update")
        return False
...
def _repair_current_checkout(
    *, assume_yes, gateway_mode, pre_update_snapshot_id, desktop_dir,
    ...
    from ulak_cli.managed_uv import ensure_uv, update_managed_uv
    runtime_repairs = []
    ...
    if handed_off_sync:
        print("→ Finishing the dependency install handed off by ulak.exe...")
    elif not healthy:
    ...
    if runtime_repaired is not None and not _m()._is_windows():
        ...
        print(
            "  Any running Ulak gateways, Desktop backends, or other "
...
def _reconcile_diverged_checkout(git_cmd, branch: str, pre_pull_sha) -> None:
    ...
    if not has_common_ancestor and pre_pull_sha:
        ...
        rescue_ref = (
            f"refs/ulak-update-backups/orphan-{branch}-"
...
def _rollback_if_pulled_syntax_error(git_cmd, pre_pull_sha) -> None:
    ...
    if pre_pull_sha:
        ...
        if rollback_result.returncode == 0:
            ...
            print("  Try ``ulak update`` again later once a fix lands.")
        else:
...
def _pull_updates(
    ...
    # critical-path file (PR #28452 incident: orphan merge-conflict markers in ulak_cli/config.py bricked
    # every user who ran ``ulak update`` for the 7 minutes between the bad commit and the fix landing).
...
class _UpdateOptions:
    """Resolved ``ulak update`` inputs (flags, config, pre-update snapshots)."""
...
def _begin_update_receipt_and_plan(args):
    ...
    pre-update plan (None if the probe failed); ``sys.exit(2)`` when a non-gateway ulak.exe
    holds the venv shim."""
    ...
    with _best_effort('Update receipt unavailable: %s'):
        ...
        from ulak_cli.update_receipt import begin_update_receipt
    ...
    # Plan phase (#91277 Phase 2): snapshot the pre-update fleet — every running Ulak runtime, its
    ...
    with _best_effort('Update plan phase failed: %s'):
        from ulak_cli.update_inventory import collect_runtime_inventory, record_plan_in_receipt
    ...
    # Windows: another ulak.exe holding the venv shim means WinError 32 spam and a
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        concurrent = _m()._detect_concurrent_ulak_instances(scripts_dir) if scripts_dir is not None else []
...
def _prepare_git_command() -> tuple[bool, list, bool]:
    ...
    if use_zip_update and sys.platform != "win32":
        ...
        print("  curl -fsSL https://ulak-agent.github.com/ErkanOzdemir-Labs/install.sh | bash")
        sys.exit(1)
...
def _verify_head_after_pull(
    ...
    # step re-detaches to the SHA). Before this guard, ``ulak update`` printed "✓ Code updated!" and
    # reinstalled deps + rebuilt the desktop app against the stale tree — no error, no warning, ``ulak
    ...
    if pre_pull_sha and post_pull_sha == pre_pull_sha:
        ...
        print(
            ...
            f"git -C {_m().PROJECT_ROOT} checkout {branch} && ulak update")
        _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
        sys.exit(1)
        ...
        if _called_process_error_is_python_dep_install(e):
            ...
            if _m()._is_windows():
                ...
                print(
                    ...
                    '"from ulak_cli.main import main; main()" update --yes')
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # Any venv python still running (typically the Desktop `ulak serve` backend) keeps .pyd
    ...
    # Self-lock deferral moved: the venv-holder sweep above excludes this process by design (a CLI `ulak
