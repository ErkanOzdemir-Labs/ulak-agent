"""``ulak doctor`` — diagnose (and with --fix, repair) a Ulak install.
...
from ulak_cli.config import get_env_path, get_ulak_home, get_project_root
from ulak_cli.env_loader import load_ulak_dotenv
from ulak_constants import display_ulak_home

PROJECT_ROOT = get_project_root()
ULAK_HOME = get_ulak_home()
_DHH = display_ulak_home()  # user-facing display path (e.g. ~/.ulak or ~/.ulak/profiles/coder)

# Load environment variables from ~/.ulak/.env so API key checks work
_env_path = get_env_path()
load_ulak_dotenv(ulak_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from ulak_cli.colors import Colors, color
from ulak_cli.doctor_report import Finding, _section, check_bool, check_info, doctor_check, warn_on_error
from ulak_cli.doctor_connectivity import _has_healthy_oauth_fallback_for_apikey_provider, build_probes, run_probes
from ulak_cli.doctor_tools import _safe_which

from ulak_cli.doctor_config import (
    _check_config_drift,
    _check_config_file,
    _check_env_file,
    _check_mcp_security,
...
from ulak_cli.doctor_platform import (
    _check_certificates,
...
from ulak_cli.doctor_tools import (
    _check_git_and_rg,
...
from ulak_cli.doctor_state import (
...
def _check_auth_providers(should_fix: bool, f: Finding) -> None:
    ...
    with warn_on_error("Auth provider status", "(could not check: {e})"):
        from ulak_cli.auth import get_nous_auth_status_local, get_codex_auth_status, get_minimax_oauth_auth_status
        _login_row("Nous Portal auth", get_nous_auth_status_local())
        # Native OAuth is Ulak' own device-code flow; the Codex CLI only imports existing ~/.codex/auth.json
    ...
    with warn_on_error(""):  # xAI OAuth separately, so an import failure cannot disrupt the rows already printed
        from ulak_cli.auth import get_xai_oauth_auth_status
...
def _ack_advisory(ack_target: str) -> None:
    """`ulak doctor --ack <id>`: persist the ack and return without running diagnostics."""
    from ulak_cli.security_advisories import ADVISORIES, ack_advisory
    ...
    if ack_advisory(ack_target):
        ...
        print(color(f"  ✗ Failed to persist ack for {ack_target}. Check ~/.ulak/config.yaml is writable.", Colors.RED))
        sys.exit(1)
...
def _print_summary(should_fix: bool, total: Finding) -> None:
    ...
    if should_fix and total.fixed > 0:
        ...
        print(numbered)
        if not should_fix:
            print(color("  Tip: run 'ulak doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
...
def run_doctor(args):
    ...
    os.environ.setdefault("ULAK_INTERACTIVE", "1")
    ...
    with warn_on_error(""):
        from ulak_cli.doctor_live import maybe_run_live_checks
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'OPENROUTER_MODELS_URL': ('ulak_constants', 'OPENROUTER_MODELS_URL'),
    'STATE_DB_SIZE_WARN_BYTES': ('ulak_cli.doctor_state', 'STATE_DB_SIZE_WARN_BYTES'),
    'agent_browser_runnable': ('ulak_constants', 'agent_browser_runnable'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'check_certificates': ('ulak_cli.doctor_platform', 'check_certificates'),
    'check_macos_full_disk_access': ('ulak_cli.doctor_platform', 'check_macos_full_disk_access'),
    'check_macos_tcc_anchor': ('ulak_cli.doctor_platform', 'check_macos_tcc_anchor'),
    'check_macos_tcc_grants': ('ulak_cli.doctor_platform', 'check_macos_tcc_grants'),
    'collect_deprecated_config_keys': ('ulak_cli.doctor_config', 'collect_deprecated_config_keys'),
    'collect_deprecated_env_vars': ('ulak_cli.doctor_config', 'collect_deprecated_env_vars'),
    'collect_relay_plugin_cutover_findings': ('ulak_cli.doctor_config', 'collect_relay_plugin_cutover_findings'),
    'describe_vercel_auth': ('ulak_cli.vercel_auth', 'describe_vercel_auth'),
    'detect_install_method': ('ulak_cli.config', 'detect_install_method'),
    'is_nix_install_method': ('ulak_cli.config', 'is_nix_install_method'),
    'managed_scope_check': ('ulak_cli.doctor_config', 'managed_scope_check'),
    'recommended_update_command_for_method': ('ulak_cli.config', 'recommended_update_command_for_method'),
    'report_deprecated_config_and_env': ('ulak_cli.doctor_config', 'report_deprecated_config_and_env'),
}
