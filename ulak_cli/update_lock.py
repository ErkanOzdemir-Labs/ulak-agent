"""Cross-process mutual exclusion for in-flight Ulak updates.
...
MARKER_NAME = ".ulak-update-in-progress"

# Set by an orchestrating updater (Tauri `ulak-setup --update`) to its own pid before
# spawning `ulak update` as a child stage; the parent holds the marker for its whole run,
...
HANDOFF_PID_ENV = "ULAK_UPDATE_HANDOFF_PID"
...
# Tauri updater (UPDATE_EXIT_CONCURRENT in update.rs) to show "Ulak is still running".
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Ulak home (never the context-local profile override): the Rust
    updater resolves ``$ULAK_HOME`` or the platform default and the desktop pins that same
    ...
    return get_process_ulak_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``ulak update`` as a (grand)child, so a live marker
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Ulak update is already running (PID {holder.pid}, "
