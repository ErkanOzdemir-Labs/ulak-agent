"""On-demand supply-chain audit for Ulak Agent installs.
...
def _discover_plugins(ulak_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.ulak/plugins``. Plugins typically don't install
...
def _discover_components(
    *, skip_venv: bool = False, skip_plugins: bool = False, skip_mcp: bool = False, ulak_home: Optional[Path] = None
) -> list[Component]:
    """Discover all scannable components across the enabled sources."""
    home = ulak_home or Path(get_ulak_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `ulak security audit`."""
    home = Path(get_ulak_home())
    ...
    components = _discover_components(ulak_home=home, **skips)
    ...
    try:
        findings = run_audit(ulak_home=home, components=components)
