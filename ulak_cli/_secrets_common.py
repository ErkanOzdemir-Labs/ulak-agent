"""Helpers shared by the Bitwarden and 1Password ``ulak secrets`` CLIs.

Import-light on purpose: ``ulak_cli.secrets_cli`` must stay free of the Bitwarden backend
...
def require_enabled(console: Console, cfg: dict, product: str, command: str) -> bool:
    ...
    console.print(f"[yellow]{product} integration is disabled.  Run "
                  f"`ulak secrets {command} setup` first.[/yellow]")
    return False
...
def rotate_token(
    ...
    console.print(f"[green]✓[/green] stored in {env_path()} as {token_env}.  "
                  "Takes effect on the next Ulak invocation.")
    if disabled_note:
