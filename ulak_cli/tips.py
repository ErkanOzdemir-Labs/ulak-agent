...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or ulak -c.",
    ...
    "Ctrl+Z suspends Ulak to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "ulak -c resumes your most recent CLI session. ulak -c \"project name\" resumes by title.",
    "ulak -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "ulak -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "ulak chat -t web,terminal enables only specific toolsets for a focused session.",
    "ulak chat -s github-pr-workflow preloads a skill at launch.",
    "ulak chat -q \"query\" runs a single non-interactive query and exits.",
    "ulak chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "ulak chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "ulak --yolo bypasses all dangerous command approval prompts for the entire session.",
    "ulak chat --source telegram tags the session for filtering in ulak sessions list.",
    "ulak -p work chat runs under a specific profile without changing your default.",
    # --- CLI Subcommands ---
    "ulak doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "ulak dump outputs a compact setup summary — great for bug reports.",
    "ulak config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "ulak config edit opens config.yaml in your default editor.",
    "ulak config check scans for missing or stale configuration options.",
    "ulak sessions browse opens an interactive session picker with search.",
    "ulak sessions stats shows session counts by platform and database size.",
    "ulak sessions prune --older-than 30 cleans up old sessions.",
    "ulak skills search react --source skills-sh searches the skills.sh public directory.",
    "ulak skills check scans installed hub skills for upstream updates.",
    "ulak skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "ulak skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "ulak mcp add github --command npx adds MCP servers from the command line.",
    "ulak mcp serve runs Ulak itself as an MCP server for other agents.",
    "ulak auth add lets you add multiple API keys for credential pool rotation.",
    "ulak completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "ulak logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "ulak backup creates a zip backup of your entire Ulak home directory.",
    "ulak profile create coder creates an isolated profile that becomes its own command.",
    "ulak profile create work --clone copies your current config and keys to a new profile.",
    "ulak update syncs new bundled skills to ALL profiles automatically.",
    "ulak gateway install sets up Ulak as a system service (systemd/launchd).",
    "ulak memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "ulak webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: ulak tools disables unused tools, ulak skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "ulak models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    # --- Configuration ---
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as ulak -w).",
    ...
    "ULAK_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Ulak tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'ulak profile create coder' creates the 'coder' command.",
    "ulak profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Ulak prints a resume command with session ID and stats.",
    "ulak sessions export backup.jsonl exports all sessions for backup or analysis.",
    "ulak -r SESSION_ID resumes any specific past session by its ID.",
    # --- Memory ---
    ...
    "ulak skills install official/security/1password installs optional skills from the repo.",
    ...
    "/plan writes a markdown implementation plan to .ulak/plans/ without executing anything.",
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: ulak cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Ulak runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "ulak gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Ulak via a local macOS server.",
    ...
    "Context files (.ulak.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Ulak loads project context from .ulak.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "ulak mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "ulak mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "ulak mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — ulak tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.ulak/checkpoints/ — your project's .git is never touched.",
    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "ulak chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "ulak plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "ulak acp runs Ulak as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "ULAK_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "ulak auth add nous or ulak auth add openai-codex sets up OAuth-based providers.",
    ...
    "ulak status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.ulak/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Ulak your own.",
    ...
    "Before a gateway session reset, Ulak auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.ulak/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Ulak state via ULAK_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "ULAK_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in ULAK_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.ulak/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'ulak profile create ls' would be rejected.",
    "ulak profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "ulak claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "ULAK_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via ulak tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.ulak/.env.",
    "Skills from trusted repos (ErkanOzdemir-Labs) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.ulak/skills/.hub/quarantine/ holds skills pending security review.",
    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Ulak auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Ulak config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Ulak is working.',
    ...
    '/reload reloads ~/.ulak/.env into the running session — pick up new API keys without restarting.',
    ...
    "ULAK_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.ulak/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.ulak/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    # --- Curator ---
    'ulak curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "ulak curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'ulak curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',
    # --- Credential Pools & Routing ---
    'ulak auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'ULAK_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "ULAK_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'ulak dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'ulak dashboard embeds the full Ulak TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.ulak/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.ulak/dashboard-plugins/ — no npm build required.',
    ...
    'ULAK_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'ULAK_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'ULAK_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'ULAK_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.ulak/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.ulak/skills/ so drop-in skills appear without restarting the session.',
    ...
    'ulak -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'ulak chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'ulak chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'ulak chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "ulak chat --source tool tags programmatic chats so they don't clutter ulak sessions list.",
    'ulak dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'ulak sessions rename <ID> "new title" renames any past session; ulak sessions delete <ID> removes one.',
    'ulak import restores a full Ulak backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'ulak fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'ulak pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'ulak setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'ulak status --deep runs the full health sweep across every component; plain ulak status is the quick view.',
    # --- Agent Behavior Env Vars ---
    'ULAK_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'ULAK_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.ulak/plugins/ — trust-gated by design.',
    "ULAK_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'ULAK_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'ULAK_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'ULAK_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'ULAK_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'ULAK_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'ULAK_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',
    # --- Gateway Behavior Env Vars ---
    'ULAK_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'ULAK_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'ULAK_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'ULAK_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Ulak block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from ulak sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.ulak/dashboard-plugins/.',
]
...
# Task-oriented example prompts for the empty composer. Kept generic — Ulak is
