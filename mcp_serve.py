"""
Ulak MCP Server — expose messaging conversations as MCP tools (`ulak mcp serve`).
...
Matches OpenClaw's 9-tool channel bridge surface plus the Ulak-specific
channels_list. Client config: {"mcpServers": {"ulak": {"command": "ulak", "args": ["mcp", "serve"]}}}
"""
...
logger = logging.getLogger("ulak.mcp_serve")
...
def _ulak_home() -> Path:
    try:
        ...
        return Path(os.environ.get("ULAK_HOME", Path.home() / ".ulak"))


def _get_sessions_dir() -> Path:
    return _ulak_home() / "sessions"


def _read_state_db_mtime() -> float:
    try:
        return (_ulak_home() / "state.db").stat().st_mtime
...
def _load_channel_directory() -> dict:
    ...
    return _read_json(_ulak_home() / "channel_directory.json")
...
class EventBridge:
    ...
    event queue with waiter support (the Ulak analogue of OpenClaw's WebSocket
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Ulak MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(f"MCP server requires the 'mcp' package. Install with: {sys.executable} -m pip install 'mcp'")
    mcp = MCPServer("ulak", instructions=(
        "Ulak Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Ulak MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
