...
Defenses 1-2 (``_ULAK_GATEWAY`` guard on ``ulak gateway stop|restart`` /
...
persisted to ``<ULAK_HOME>/gateway/restart_loop.json`` and CHAIN while gaps stay
...
def _state_path():
    return get_ulak_home() / "gateway" / "restart_loop.json"
