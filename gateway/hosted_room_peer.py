...
def _gateway_room_grant_secret_for_home(home_value: str) -> bytes:
    ...
    from ulak_cli.install_identity import _fsync_directory
    ...
    return hmac.new(material, b"ulak-hosted-room-installation-grant-v1", hashlib.sha256).digest()
...
def gateway_room_grant_secret(root: Path | str | None = None) -> bytes:
    ...
    if root is None:
        ...
        # Profile routing uses a context-local ULAK_HOME override; the process environment
        # retains the installation root and is the authority here.
        root = os.environ.get("ULAK_HOME") or get_ulak_home()
...
def derive_room_grant_secret(api_key: str) -> bytes:
    ...
    return hmac.new(api_key.encode("utf-8"), b"ulak-hosted-room-grant-v1", hashlib.sha256).digest()
...
def catalog_mapping(
    *, installation_id: str, protocol_versions: Iterable[int] = (PROTOCOL_VERSION,),
    ...
    persistent_process = bool(persistent_process and os.getenv("ULAK_DESKTOP") != "1")
    profile = str(target_profile or "").strip() or (os.getenv("ULAK_PROFILE") or "default").strip() or "default"
...
def _configured_room_link_url() -> str | None:
    ...
    if (override := os.getenv("ULAK_ROOM_LINK_URL")) is not None:
        return override
