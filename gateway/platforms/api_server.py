...
def _prefix_names_served_profile(profile: str) -> bool:
    ...
    try:
        from ulak_cli.profiles import profile_matches_home
...
_STATIC_FEATURE_FLAGS = {
    ...
    "session_continuity_header": "X-Ulak-Session-Id",
    "session_key_header": "X-Ulak-Session-Key"}
...
_BROWSER_CONTROL_WS_PROTOCOL = "ulak-browser-control-v1"
_BROWSER_CONTROL_TICKET_PROTOCOL_PREFIX = "ulak-browser-control-ticket."
...
def _ulak_version() -> str:
    """Canonical Ulak version: ``ulak_cli.__version__`` (dist-info can be stale on
...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    from ulak_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error, _get_model_config
    try:
    ...
    max_tokens = None
    env_max_tokens = os.environ.get("ULAK_MAX_TOKENS")
    if env_max_tokens:
...
def _request_agent_overrides(
    ...
    The virtual model (``ulak-agent``) means "gateway default". A bare ``model`` without
    ``provider`` is honored only when ``allow_bare_model`` (generic clients hardcode "gpt-4o";
    OpenAI-compatible handlers pass the ``direct_model_requests`` opt-in, Ulak-native
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            ...
            with suppress(Exception):
                ...
                db_path = str(get_ulak_home() / "response_store.db")
        ...
        from ulak_state_wal import apply_wal_with_fallback
...
def _derive_chat_session_id(system_prompt: Optional[str], first_user_message: str) -> str:
    ...
    turns of an Open WebUI-style conversation), so one Ulak session/sandbox is reused."""
...
class APIServerAdapter(OpenAICompatRoutesMixin, BasePlatformAdapter):
    """aiohttp server routing OpenAI-format requests through ulak-agent's AIAgent."""
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # executing model. Requests that send an explicit ``provider`` — and the Ulak-native session-chat
    ...
    def _resolve_model_name(explicit: str) -> str:
        """Advertised /v1/models name: explicit override > active profile name > "ulak-agent"
        (precedence owned by ``ulak_cli.model_switch.resolve_effective_model``)."""
        from ulak_cli.model_switch import resolve_effective_model
        profile_name = ""
        with suppress(Exception):
        ...
        return resolve_effective_model(explicit, profile_name, "ulak-agent")
    ...
    def _expected_api_key(self) -> str:
        ...
        try:
            ...
            from ulak_cli.auth import has_usable_secret
        ...
        if not profile:
            with suppress(Exception):
                ...
                if is_multiplex_active():
                    ...
                    return _profile_runtime_scope(get_ulak_home())
            return nullcontext()
        from gateway.run import _profile_runtime_scope
        from ulak_cli.profiles import get_profile_dir
    ...
    def _declared_conversation_session(self, gateway_session_key: Optional[str]) -> Optional[str]:
        """Resolve the live session a client declared with ``X-Ulak-Session-Key`` (the key
    ...
    def _parse_session_key_header(
        ...
        """Validate ``X-Ulak-Session-Key`` (per-channel memory scope) -> ``(key_or_None, None)``
        or ``(None, error)``. Requires API-key auth so a client can't guess another scope."""
        raw = request.headers.get("X-Ulak-Session-Key", "").strip()
        if not raw:
            return None, None
        if not self._api_key:
            logger.warning(
                "X-Ulak-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping.")
            return None, _error_response(
                "X-Ulak-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        """SessionDB for the active profile home (the runtime scope redirects ``get_ulak_home()``
        per profile). Sync, for ``_create_agent``; handlers use ``_ensure_session_db_async``."""
        if self._session_db is not None:
            return self._session_db
        try:
            from ulak_constants import get_ulak_home
            return self._open_and_cache_session_db(get_ulak_home())
    ...
    def _stored_session_model(self, session: Any) -> Optional[str]:
        ...
        "ulak-agent" upstream as a provider model id 400s)."""
    ...
    def _normalize_session_source(value: Any) -> str:
        ...
        allowed = {"api_server", "ulak_browser", "browser", "cli", "telegram", "discord", "slack", "desktop", "dashboard"}
        if text not in allowed:
            return "api_server"
        return "ulak_browser" if text == "browser" else text
    ...
    def _recover_or_record_model(self, model: str, runtime_kwargs: Dict[str, Any], gateway_session_key) -> str:
        ...
        # No model.default but a provider resolved (e.g. `ulak auth add` without `ulak model`).
        if not model and runtime_kwargs.get("provider"):
            with suppress(Exception):
                from ulak_cli.models import get_default_model_for_provider
    ...
    def _select_agent_runtime(
        ...
        # ulak_cli.model_switch.resolve_effective_model.
        from ulak_cli.model_switch import resolve_effective_model
        if session_override:
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "ulak-agent", "version": _ulak_version()})
    ...
    async def _handle_health_detailed(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "status": readiness["status"], "readiness": readiness, "platform": "ulak-agent",
            "version": _ulak_version(), "gateway_state": gw_state,
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — ulak-agent plus configured model_routes aliases (alias + resolved
        ...
        def _model(mid: str, root: str, parent) -> Dict[str, Any]:
            return {"id": mid, "object": "model", "created": now, "owned_by": "ulak", "permission": [],
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        ...
        try:
            from ulak_cli.inventory import build_model_options_payload, load_picker_context
    ...
    def _artifact_store_for(self, profile: str) -> ArtifactStore:
        ...
        (``<ULAK_HOME>/plugin-data/.../artifacts``-style controlled root), so artifacts never escape the
        ...
        try:
            ...
            # Unscoped fallback (tests/manual wiring): controlled root under the Ulak home.
            try:
                from ulak_state import get_ulak_home
                root = Path(get_ulak_home()) / "artifacts" / "browser-control"
            except Exception:
    ...
    async def _handle_toolsets(self, request: "web.Request") -> "web.Response":
        ...
        try:
            ...
            from ulak_cli.tools_config import (
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Ulak sessions."""
        db = await self._ensure_session_db_async()
        if db is None:
        ...
        # Exact-title lookup (`ulak peer dm` -> canonical "Bot Chat"). include_hidden is honored
        ...
        if title_filter and not sessions:
            # A canonical Bot Chat auto-archived by the orphan reaper would make `ulak peer dm`
            ...
            try:
                ...
                # which would fail `ulak peer dm` resolution and mint transient sessions — same accident
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty Ulak session row. Existence check, insert and
        ...
        # would persist "ulak-agent" and later send it to the provider literally.
        ...
        return web.json_response({"object": "ulak.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "ulak.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "ulak.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "ulak.session", "session": self._session_response(fork)}, status=201)
    ...
    def _session_headers(session_id: str, gateway_session_key: Optional[str]) -> Dict[str, str]:
        """``X-Ulak-Session-Id`` (+ ``X-Ulak-Session-Key`` when declared) response headers."""
        headers = {"X-Ulak-Session-Id": session_id}
        if gateway_session_key:
            headers["X-Ulak-Session-Key"] = gateway_session_key
        return headers
    ...
    def _effective_turn_runtime(self, runtime_request: Dict[str, Any], result: Any, usage: Any) -> Dict[str, Any]:
        ...
        return self._sanitize_runtime_metadata(
            ...
            # gateway/status readers resolve process-level paths and do NOT follow the ULAK_HOME
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"object": "ulak.session.chat.completion",
    ...
    async def _handle_session_model_lock(self, request: "web.Request") -> "web.Response":
        ...
        runtime = self._sanitize_runtime_metadata(
            ...
            model_lock="accepted")
        return web.json_response(
            {"object": "ulak.session.model_lock", "session_id": session_id, "runtime": runtime})
    ...
    async def _handle_run_job(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        # Optional transient per-run context (standalone `ulak cron run` /
    ...
    def _turn_runtime_metadata(
        ...
        runtime = dict(getattr(agent, "_ulak_api_runtime", {}) or {})
