...
def _get_ulak_version() -> str:
    """Return the ulak-agent package version, or 'dev' if unavailable."""
    try:
...
def build_user_agent() -> str:
    """``QQBotAdapter/<qqbot_version> (Python/<py_version>; <os>; Ulak/<ulak_version>)``."""
    v = sys.version_info
    return (f"QQBotAdapter/{QQBOT_VERSION} (Python/{v.major}.{v.minor}.{v.micro}; "
            f"{platform.system().lower()}; Ulak/{_get_ulak_version()})")
