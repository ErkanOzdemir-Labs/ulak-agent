...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.ulak to the active profile home."""
    ...
    if raw == "~/.ulak" or raw.startswith("~/.ulak/"):
        return ulak_home / raw[len("~/.ulak/"):]
    path = Path(raw).expanduser()
    return path if path.is_absolute() else ulak_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script; must live under ULAK_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from ulak_constants import get_ulak_home
    scripts_root = (get_ulak_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.ulak" or raw_text.startswith("~/.ulak/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        Non-zero exit, empty/``[SILENT]`` stdout, or a ``[SILENT]``/``__ulak_ignore__`` flag drops the
        ...
        if not isinstance(transformed, dict):
            ...
            return False, None
        silenced = transformed.get("[SILENT]") is True or transformed.get("__ulak_ignore__") is True
