...
There are two ways to add a platform to the Ulak gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.ulak/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Ulak code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `ulak gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `ulak_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Ulak core.
...
"ulak-your-platform": {
    ...
    "tools": _ULAK_CORE_TOOLS,
    "includes": []
},
```

And add it to the `ulak-gateway` composite:

```python
"ulak-gateway": {
    "includes": [..., "ulak-your-platform"]
}
```
...
## 12. Status Display (`ulak_cli/status.py`)
...
## 13. Gateway Setup Wizard (`ulak_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ ulak_cli/ toolsets.py \
  --include="*.py" -l | sort -u
