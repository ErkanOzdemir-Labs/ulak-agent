...
``<ULAK_HOME>/state/gateway.heartbeat`` so supervisors can tell "process alive" from "loop
...
def _process_ulak_home() -> Path:
    """ULAK_HOME for process-level identity files (ignore profile overrides)."""
    val = os.environ.get("ULAK_HOME", "").strip()
    return Path(val) if val else get_ulak_home()


def _home(home: Optional[Path]) -> Path:
    return home if home is not None else _process_ulak_home()
...
def get_loop_tick_socket_path(home: Optional[Path] = None, pid: Optional[int] = None) -> Path:
    """``<ULAK_HOME>/state/gateway.loop-tick.<pid>.sock`` — PID-suffixed so a stale node from a
