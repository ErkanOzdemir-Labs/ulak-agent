...
All hooks write atomic JSON payloads under ``<ulak_home>/pending_messages/``:
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active ULAK_HOME."""
    from ulak_constants import get_ulak_home
    flush_dir = get_ulak_home() / "pending_messages"
...
def spool_dropped_transcript_message(session_id: str, message: Dict[str, Any]) -> Optional[Path]:
    ...
    message under ``<ulak_home>/pending_messages/``), so a runtime cap rotation no longer silently
