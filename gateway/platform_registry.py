...
class PlatformEntry:
    ...
    required_env: list = field(default_factory=list)  # ``ulak setup`` display
    ...
    plugin_name: str = ""  # owning manifest so ``ulak gateway setup`` can auto-enable it
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved ULAK_HOME and overlay the
        ...
        # `ulak` invocation, so the import happens only when a lookup asks for it.
