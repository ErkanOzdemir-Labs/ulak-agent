...
``validate_media_delivery_path`` reads ``ULAK_MEDIA_DELIVERY_STRICT`` (gateway.strict),
``ULAK_MEDIA_ALLOW_DIRS`` (gateway.media_delivery_allow_dirs) and
``ULAK_MEDIA_TRUST_RECENT_FILES`` (gateway.trust_recent_files).  Every delivery
entrypoint (gateway startup, ``ulak cron run``, ``ulak send``) calls
...
_FLAG_ENVS = (("strict", "ULAK_MEDIA_DELIVERY_STRICT"), ("trust_recent_files", "ULAK_MEDIA_TRUST_RECENT_FILES"))
_ALLOW_DIRS_ENV = "ULAK_MEDIA_ALLOW_DIRS"
