...
boundary. POSIX: ``$ULAK_HOME/gateway.sock`` (or a temp-dir socket + ``gateway.sock.path`` pointer
file when the home path exceeds ``sun_path``); Windows: named pipe ``\\\\.\\pipe\\ulak-gateway-<hash>``.
...
def windows_pipe_name(home: Path) -> str:
    """Per-ULAK_HOME named pipe path (Windows transport)."""
    return rf"\\.\pipe\ulak-gateway-{_home_hash(home)}"
...
def _fallback_socket_path(home: Path) -> Path:
    ...
    name = f"ulak-gw-{_home_hash(home)}.sock"
...
def _detect_supervisor() -> str:
    ...
    if env.get("INVOCATION_ID"):
        return "systemd"
    if sys.platform == "darwin" and (env.get("XPC_SERVICE_NAME", "").startswith("ai.ulak")
                                     or env.get("LAUNCHD_SOCKET")):
        return "launchd"
    if env.get("ULAK_DESKTOP_MANAGED"):
        return "desktop"
...
def build_identify_payload() -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        **{k: record.get(k) for k in ("kind", "pid", "start_time", "ulak_home")},
        "profile": _profile_label_for_home(record.get("ulak_home") or ""),
