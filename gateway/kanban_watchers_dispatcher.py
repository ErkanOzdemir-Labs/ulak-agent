...
def _kbc():
    from ulak_cli import kanban_db_connect
    return kanban_db_connect


def _kbd():
    from ulak_cli import kanban_db_dispatch
...
class _KanbanDispatcher:
        ...
        try:
            ...
            if self.is_corrupt_board_db_error(exc):
                ...
                logger.error(
                    ...
                    "then run `ulak kanban init` if you need a fresh board.",
                    slug, fingerprint[0],
                )
                return None
    ...
    def auto_decompose_tick(self, auto_decompose_per_tick: int) -> int:
        ...
        """
        try:
            from ulak_cli import kanban_decompose as _decomp
        ...
        for slug in self._board_slugs():
            ...
            prev_env = os.environ.get("ULAK_KANBAN_BOARD")
            try:
                os.environ["ULAK_KANBAN_BOARD"] = slug
                try:
                ...
                if prev_env is None:
                    ...
                    os.environ["ULAK_KANBAN_BOARD"] = prev_env
        return successes
