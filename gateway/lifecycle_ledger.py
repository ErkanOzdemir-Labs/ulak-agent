...
``<ULAK_HOME>/state/gateway.lifecycle.json`` closes the gap:
...
def _process_ulak_home() -> Path:
    """ULAK_HOME for process-level identity files (ignore task overrides)."""
...
def _home_path(home: Optional[Path], *relative: str) -> Path:
    return (_process_ulak_home() if home is None else home).joinpath(*relative)


def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<ULAK_HOME>/state/gateway.lifecycle.json``."""
...
def _report_unclean_exit(evidence: Dict[str, Any], home: Optional[Path]) -> None:
    ...
    if verdict not in ("ok", "absent"):
        logger.error(
            ...
            "missing until it is repaired. Run `ulak doctor`.",
            verdict,
        )
