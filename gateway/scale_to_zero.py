...
SCALE_TO_ZERO_ENV = "ULAK_SCALE_TO_ZERO"  # stamped by NAS when the Labs toggle is on
...
def dashboard_client_heartbeat_path(ulak_home: Optional[os.PathLike | str] = None):
    """Path of the dashboard-client liveness marker under ULAK_HOME."""
    if ulak_home is None:
        from ulak_constants import get_ulak_home
        ulak_home = get_ulak_home()
    return Path(ulak_home) / DASHBOARD_CLIENT_HEARTBEAT_REL
