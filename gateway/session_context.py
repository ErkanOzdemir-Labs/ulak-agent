"""Session-scoped context variables for the Ulak gateway.

Replaces the old ``os.environ``-based ``ULAK_SESSION_*`` state with task-local ``ContextVar``s
...
_SESSION_VARS = (
    ...
    "ULAK_SESSION_PLATFORM", "ULAK_SESSION_SOURCE", "ULAK_SESSION_CHAT_ID",
    "ULAK_SESSION_CHAT_TYPE", "ULAK_SESSION_CHAT_NAME", "ULAK_SESSION_THREAD_ID",
    "ULAK_SESSION_USER_ID", "ULAK_SESSION_USER_ID_ALT", "ULAK_SESSION_USER_NAME",
    "ULAK_SESSION_SCOPE_ID", "ULAK_SESSION_KEY", "ULAK_SESSION_ID",
    "ULAK_UI_SESSION_ID", "ULAK_SESSION_MESSAGE_ID", "ULAK_SESSION_PROFILE",
    "ULAK_BROWSER_CONTROL_PRINCIPAL", "ULAK_BROWSER_CONTROL_TRANSPORT_FAMILY",
    "ULAK_CRON_SESSION",
))
...
_SESSION_ASYNC_DELIVERY = ContextVar("ULAK_SESSION_ASYNC_DELIVERY", default=_UNSET)

# Cron auto-delivery vars, set per-job in run_job() so concurrent jobs don't clobber.
_CRON_AUTO_DELIVER_PLATFORM = ContextVar("ULAK_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID = ContextVar("ULAK_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID = ContextVar("ULAK_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)
...
def set_current_session_id(session_id: str) -> None:
    """Synchronize ``ULAK_SESSION_ID`` across ContextVar and ``os.environ`` (tools read it
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session var by legacy ``ULAK_SESSION_*`` name; drop-in for os.getenv.  The
...
# Surfaces that are not a human chat channel (gateway binds ULAK_SESSION_PLATFORM, CLI/TUI/
# desktop bind ULAK_SESSION_SOURCE, so both are consulted).  Default-deny: an unrecognized
...
def session_is_messaging_surface() -> bool:
    ...
    ``ULAK_PLATFORM``, then the session platform, then the session source)."""
    platform = os.getenv("ULAK_PLATFORM") or get_session_env("ULAK_SESSION_PLATFORM", "")
    idents = (platform, get_session_env("ULAK_SESSION_SOURCE", ""))
...
def declare_stateless_channel() -> None:
    ...
    See ErkanOzdemir-Labs/ulak-agent#53027 and #63142.
    """
...
def async_delivery_supported() -> bool:
    ...
    (``ULAK_KANBAN_TASK``: one-shot subprocesses whose parent disappears after the turn)."""
