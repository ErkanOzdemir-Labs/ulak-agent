...
``gateway.<key>`` form (what ``ulak config set gateway.<key>`` produces) is consulted only when the
...
#
# Top-level settings are also accepted nested under ``gateway:`` (what ``ulak config set
...
def load_yaml_layer(home: Path, gw_data: dict) -> None:
    ...
    # ulak_cli.config.load_config, so a managed session_reset / quick_commands / stt would otherwise be ignored).
