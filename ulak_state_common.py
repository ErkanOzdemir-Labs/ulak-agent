"""Shared constants and helpers for the SessionDB family of modules.  Lives outside ulak_state so
...
# optimized via ``ulak sessions optimize-storage``. A legacy DB sits at
...
--
-- A host-declared conversation key (X-Ulak-Session-Key / build_session_key)
...
# and must not serve reads until `ulak sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `ulak db optimize`. This is
...
# claims progress under SQLite transaction authority).  Mirrors `ulak_state_repair._cross_process_repair_lock`:
...
# `<db>.repair.lock` (offline schema surgery, minutes in VACUUM).  Lives here: mixins cannot import ulak_state.

# ── Cross-process full-FTS-rebuild admission (single authority) ────────────── Several independent Ulak
# processes routinely share one state.db (gateway service, the Desktop app's `ulak serve` backend,
...
# authority and is intentionally multi-process. Semantics mirror `ulak_state_repair._cross_process_repair_lock`
...
# indeterminate liveness still defers. It lives here (not ulak_state) because the search/schema mixins
# cannot import ulak_state (cycle). The lock file is `<db>.fts_rebuild.lock`, distinct from
...
def _describe_lock_holder(record) -> str:
    ...
    if not isinstance(record, dict) or "pid" not in record:
        return "unknown (no holder record; pre-fix writer or non-Ulak)"
    age = ""
