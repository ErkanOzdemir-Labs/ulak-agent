# Ulak Agent Security Policy

This document describes Ulak Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/ErkanOzdemir-Labs/ulak-agent/security/advisories/new)
or **security@github.com/ErkanOzdemir-Labs**. Do not open public issues for
security vulnerabilities. **Ulak Agent does not operate a bug
bounty program.**
...
- Environment details (`ulak --version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Ulak Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Ulak Agent's documentation
...
Ulak Agent supports two OS-level isolation postures. They address
...
Ulak Agent supports this in two ways:

- **Ulak Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Ulak Agent's in-process heuristics
...
Ulak Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Ulak Agent
itself. Bugs in Ulak Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Ulak Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Ulak Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Ulak Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  ulak-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Ulak Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Ulak Agent. Bugs in Ulak
...
- Ulak Agent includes supply-chain guards for MCP server
