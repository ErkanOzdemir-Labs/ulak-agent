...
Usage:
  ...
  <server_name> matches the files in $ULAK_HOME/mcp-tokens/<server>.json etc.
...
def _ulak_home():
    # Prefer Ulak' own resolver (profile-safe); fall back to env then ~/.ulak.
    try:
        from ulak_constants import get_ulak_home
        return str(get_ulak_home())
    except Exception:
        return os.environ.get("ULAK_HOME") or os.path.expanduser("~/.ulak")


def _tokens_dir():
    return os.path.join(_ulak_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "ulak-diag", "version": "1.0"}},
        },
        headers={
