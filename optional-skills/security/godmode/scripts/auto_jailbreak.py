...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ULAK_HOME", os.path.expanduser("~/.ulak")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ULAK_HOME", Path.home() / ".ulak")) / "skills" / "red-teaming" / "godmode"
...
ULAK_HOME = Path(os.getenv("ULAK_HOME", Path.home() / ".ulak"))
CONFIG_PATH = ULAK_HOME / "config.yaml"
PREFILL_PATH = ULAK_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Ulak models — already uncensored, just needs clean prompt
    "ulak": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "ulak" in model_lower or "nous" in model_lower:
        return "ulak"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Ulak config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.ulak/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Ulak for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Ulak for changes to take effect.")
