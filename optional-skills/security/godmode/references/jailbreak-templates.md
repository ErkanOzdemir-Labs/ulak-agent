...
## 5. GODMODE FAST — Ulak 4 405B (Zero Refusal)

**Model:** `erkanozdemir-labs/ulak-4-405b`
**Strategy:** Ulak models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Ulak Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.ulak/config.yaml`:
