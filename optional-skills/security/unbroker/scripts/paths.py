...
All per-subject data lives under PDD_DATA_DIR (default: $ULAK_HOME/unbroker),
which is the same trust boundary Ulak uses for .env and OAuth tokens.
"""
...
def ulak_home() -> Path:
    return Path(os.environ.get("ULAK_HOME") or (Path.home() / ".ulak"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else ulak_home() / "unbroker"
