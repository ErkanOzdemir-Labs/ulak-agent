...
Usage (via cron with --no-agent):
    ...
    ulak cron create ulak-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$ULAK_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name ulak-issues --repo ErkanOzdemir-Labs/ulak-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Ulak .env file
(``${ULAK_HOME:-~/.ulak}/.env``) to avoid the 60 req/hr
