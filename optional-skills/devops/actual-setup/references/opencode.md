...
is typically only in `~/.ulak/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike Ulak,
...
`ulak-custom-providers` skill, pitfall 2). Ulak hits this because it forwards
