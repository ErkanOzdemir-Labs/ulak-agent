...
- `generate_video()` split: the Pollinations text-to-image call was removed
  (Ulak uses its own `image_generate` + `pixel_art()` pipeline for base
...
- Pollinations text-to-image generation (`pixelart_image.py`,
  `generate_base()` in `pixelart_video.py`) — Ulak has `image_generate`
...
- License: MIT (inherits from ulak-agent repo)
- Original author of the skill shell: dodo-reach
- Expansion with palettes + video: Ulak Agent contributors
