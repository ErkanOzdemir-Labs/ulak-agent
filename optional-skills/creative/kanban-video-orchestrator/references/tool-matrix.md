...
Maps each role archetype to the Ulak skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public ulak-agent
...
## Ulak skills relevant to video production

### Visual / rendering skills (`ulak-agent/skills/creative/`)
...
### Design / pre-production skills (`ulak-agent/skills/creative/`)
...
### Audio / media skills (`ulak-agent/skills/creative/` + `skills/media/`)
...
These are **not** Ulak skills but external CLIs / APIs that profiles invoke.
...
## Built-in Ulak tools for media review

These are native Ulak tools — not invoked via terminal but through their own
...
key is present in `${ULAK_HOME:-~/.ulak}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Ulak profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${ULAK_HOME:-~/.ulak}/.env` → environment variable.
...
When a new Ulak-public video skill ships:
