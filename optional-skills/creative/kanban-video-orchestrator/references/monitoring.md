...
ulak kanban watch --tenant <project-slug>

# Snapshot of the board
ulak kanban list --tenant <project-slug>
ulak kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
ulak kanban stats --tenant <project-slug>

# Visual dashboard (browser)
ulak dashboard

# Inspect a specific task (includes comments + events)
ulak kanban show <task-id>

# Follow a single task's event stream
ulak kanban tail <task-id>
```

Verify available subcommands with `ulak kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `ulak kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `ulak kanban block <id>` then `ulak kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `ulak kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `ulak kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `ulak profile list` (profile exists?), `ulak status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(ulak kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
ulak kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
ulak kanban block <task-id>
ulak kanban archive <task-id>

# Diagnose what happened
ulak kanban show <task-id>      # task body, comments, recent events
ulak kanban tail <task-id>      # follow the live event stream
ulak kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `ulak kanban create` for the director
...
while true; do
    clear
    ulak kanban list --tenant <slug>
    echo "---"
    ulak kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `ulak kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `ulak profile list` and start
