...
the actual `setup.sh` that creates the project workspace, configures Ulak
...
> [ErkanOzdemir-Labs/kanban-video-pipeline](https://github.com/ErkanOzdemir-Labs/kanban-video-pipeline).
...
2. **Create profiles** — `ulak profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.ulak/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `ulak kanban create` assigned to the director
...
```bash
ulak profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.ulak/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.ulak/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Ulak
...
Each profile gets a `SOUL.md` at `~/.ulak/profiles/<name>/SOUL.md` that
...
```bash
ulak kanban create "Direct production of <video title>" \
    --assignee director \
...
the Ulak `.env` (`${ULAK_HOME:-$HOME/.ulak}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _ulak_env="${ULAK_HOME:-$HOME/.ulak}/.env"
    if grep -q "^${var}=" "$_ulak_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_ulak_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_ulak_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY ulak ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY ulak OPENROUTER_API_KEY || exit 1
# ...
```
