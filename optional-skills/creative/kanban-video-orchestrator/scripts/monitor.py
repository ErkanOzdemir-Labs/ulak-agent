#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `ulak kanban list` and
...
def ulak_available() -> bool:
    return shutil.which("ulak") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed ulak CLI."""
    try:
        out = subprocess.run(
            ["ulak", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `ulak kanban list`
    out = subprocess.run(
        ["ulak", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    rows = []
...
def main():
    ...
    if not ulak_available():
        print("ERROR: 'ulak' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
