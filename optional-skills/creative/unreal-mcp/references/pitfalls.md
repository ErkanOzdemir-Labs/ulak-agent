...
### 1. Start order: editor first, Ulak second

Ulak probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Ulak session.
...
restart the editor, restart the Ulak session.
...
server fails to bind (Output Log) or Ulak' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.ulak/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Ulak

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Ulak' connection
lives in `~/.ulak/config.yaml` via `ulak mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Ulak — just don't
mistake it for the Ulak setup step.
...
explicitly warns against overlapping calls. Ulak executes same-turn tool
...
### 9. Timeouts: Ulak gives up before Unreal does

Ulak' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Ulak runs elsewhere (SSH backend,
