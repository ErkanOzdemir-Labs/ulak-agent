...
ULAK_HOME_DIR="${ULAK_HOME:-$HOME/.ulak}"
ULAK_CFG="${ULAK_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Ulak config has twozero_td MCP entry ──
if [[ ! -f "$ULAK_CFG" ]]; then
    echo -e " ${FAIL} Ulak config not found at ${ULAK_CFG}"
    manual_steps+=("Create ${ULAK_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ULAK_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Ulak config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Ulak config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ULAK_CFG} manually"); }
    manual_steps+=("Restart Ulak session to pick up config change")
fi
