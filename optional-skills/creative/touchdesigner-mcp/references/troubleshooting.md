...
## 5. Ulak Configuration

### Config location

`$ULAK_HOME/config.yaml` (defaults to `~/.ulak/config.yaml` when `ULAK_HOME` is unset)
...
Restart the Ulak session for changes to take effect. The MCP connection is
