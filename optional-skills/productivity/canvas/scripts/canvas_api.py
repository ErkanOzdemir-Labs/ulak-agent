#!/usr/bin/env python3
"""Canvas LMS API CLI for Ulak Agent.
...
def _check_config():
    ...
    if missing:
        ulak_env = os.path.join(
            os.environ.get("ULAK_HOME", os.path.expanduser("~/.ulak")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {ulak_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Ulak Agent"
    )
