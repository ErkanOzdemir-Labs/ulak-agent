...
_ULAK_WEBHOOK_SAFE_TOOLS = ["web_search", "web_extract", "vision_analyze", "clarify"]
...
def _bundle(description, extras=()):
    """A `ulak-*` platform bundle: the shared core tools plus optional platform extras."""
    return _ts(description, _ULAK_CORE_TOOLS + list(extras))


def _core_without(*excluded, kanban=True):
    """_ULAK_CORE_TOOLS minus *excluded* (and, unless kanban=True, every kanban_* tool); order preserved."""
    return [t for t in _ULAK_CORE_TOOLS if t not in excluded and (kanban or not t.startswith("kanban_"))]
...
TOOLSETS = {
    ...
    "x_search": _ts(
        ...
        "(SuperGrok OAuth or XAI_API_KEY). Off by default; enable in `ulak tools` → "
        "X (Twitter) Search.",
        ["x_search"],
    ),
    ...
    "video_gen": _ts(
        ...
        "separate tools. Configure via ``ulak tools`` → Video Generation.",
    ...
    "browser": _ts(
        ...
        "iframes, hold-click)",
        [t for t in _ULAK_CORE_TOOLS if t.startswith("browser_")],
    ),
    "cronjob": _ts(
    ...
    "kanban": _ts(
        ...
        "the kanban dispatcher (ULAK_KANBAN_TASK env set). The dispatcher runs "
        ...
        [t for t in _ULAK_CORE_TOOLS if t.startswith("kanban_")],
    ),
    ...
    # config (see the non-configurable-toolset recovery loop in ulak_cli/tools_config.py).
    "coding": _ts(
    ...
    # Full Ulak toolsets (CLI + messaging platforms). All share the core tools;
    # there is deliberately no agent-callable send_message tool. ulak-acp is the
    ...
    "ulak-cli": _bundle("Full interactive CLI toolset - all default tools plus cronjob management"),

    # Mirrors ulak-cli; `ulak tools` platform config filters it down and
    # _get_platform_tools() drops _DEFAULT_OFF_TOOLSETS unless user-enabled.
    "ulak-cron": _bundle("Default cron toolset - same core tools as ulak-cli; gated by `ulak tools`"),
    "ulak-telegram": _bundle("Telegram bot toolset - full access for personal use (terminal has safety checks)"),
    "ulak-discord": _bundle(
        "Discord bot toolset - full access (terminal has safety checks via dangerous "
        "command approval)",
        ["discord", "discord_admin"],
    ),
    "ulak-whatsapp": _bundle("WhatsApp bot toolset - similar to Telegram (personal messaging, more trusted)"),
    "ulak-slack": _bundle("Slack bot toolset - full access for workspace use (terminal has safety checks)"),
    "ulak-signal": _bundle("Signal bot toolset - encrypted messaging platform (full access)"),
    "ulak-bluebubbles": _bundle("BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server"),
    "ulak-homeassistant": _bundle("Home Assistant bot toolset - smart home event monitoring and control"),
    "ulak-email": _bundle("Email bot toolset - interact with Ulak via email (IMAP/SMTP)"),
    "ulak-mattermost": _bundle("Mattermost bot toolset - self-hosted team messaging (full access)"),
    "ulak-matrix": _bundle("Matrix bot toolset - decentralized encrypted messaging (full access)"),
    "ulak-dingtalk": _bundle("DingTalk bot toolset - enterprise messaging platform (full access)"),
    "ulak-feishu": _bundle("Feishu/Lark bot toolset - enterprise messaging via Feishu/Lark (full access)", _FEISHU_TOOLS),
    "ulak-weixin": _bundle("Weixin bot toolset - personal WeChat messaging via iLink (full access)"),
    "ulak-qqbot": _bundle("QQBot toolset - QQ messaging via Official Bot API v2 (full access)"),
    "ulak-wecom": _bundle("WeCom bot toolset - enterprise WeChat messaging (full access)"),
    "ulak-wecom-callback": _bundle("WeCom callback toolset - enterprise self-built app messaging (full access)"),
    "ulak-yuanbao": {
        "description": "Yuanbao Bot 元宝消息平台工具集 - 群信息、成员查询、私聊、贴纸表情",
        "tools": _ULAK_CORE_TOOLS + _YUANBAO_TOOLS,
        "module": "tools.yuanbao_tools",
        "includes": [],
    },
    "ulak-sms": _bundle("SMS bot toolset - interact with Ulak via SMS (Twilio)"),
    "ulak-webhook": _ts("Webhook toolset - receive and process external webhook events", _ULAK_WEBHOOK_SAFE_TOOLS),
    "ulak-gateway": _ts(
        "Gateway toolset - union of all messaging platform tools",
        [],
        includes=[
            "ulak-telegram", "ulak-discord", "ulak-whatsapp", "ulak-slack",
            "ulak-signal", "ulak-bluebubbles", "ulak-homeassistant", "ulak-email",
            "ulak-sms", "ulak-mattermost", "ulak-matrix", "ulak-dingtalk",
            "ulak-feishu", "ulak-wecom", "ulak-wecom-callback", "ulak-weixin",
            "ulak-qqbot", "ulak-webhook", "ulak-yuanbao",
        ],
    ),
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """A bundle's tools minus _ULAK_CORE_TOOLS (one level of includes).

    Disabling a `core + extras` bundle must not strip the core tools every other
    toolset shares. One `includes` pass suffices (only ulak-gateway nests
    bundles). Unknown names: full resolution minus core.
    """
    core = set(_ULAK_CORE_TOOLS)
...
def _plugin_platform_bundle(name: str) -> List[str]:
    """Implicit `ulak-<platform>` bundle for a registered plugin platform: core
    tools plus whatever the plugin registered under the platform name. [] otherwise."""
    if not name.startswith("ulak-"):
        return []
    platform_name = name[len("ulak-"):]
    try:
