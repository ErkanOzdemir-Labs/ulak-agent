...
```bash
ulak plugins enable security-guidance
# or edit ~/.ulak/config.yaml manually:
plugins:
  enabled:
    - security-guidance
```
...
* **No LLM diff review.** Anthropic's layer 2 spawns an auxiliary LLM call
  on every agent turn that touched files. On ulak that would route
...
* **No project-local rules file.** Anthropic's `.claude/claude-security-guidance.md`
  ...
  add an analogous `.ulak/security-guidance.md` once layer 2 lands.
...
* `__init__.py`, `plugin.yaml`, `README.md`, and tests are original work by
  ErkanOzdemir-Labs, MIT-licensed alongside the rest of ulak-agent.
