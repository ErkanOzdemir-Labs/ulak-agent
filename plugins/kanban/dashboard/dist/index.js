/**
 * Ulak Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.ulak/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__ULAK_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__ULAK_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "ulak-kanban-dot-triage",
    todo: "ulak-kanban-dot-todo",
    ready: "ulak-kanban-dot-ready",
    running: "ulak-kanban-dot-running",
    blocked: "ulak-kanban-dot-blocked",
    review: "ulak-kanban-dot-review",
    done: "ulak-kanban-dot-done",
    archived: "ulak-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-ulak-task";
  ...
  const DOCS_URL = "https://ulak-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://ulak-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "ulak.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="ulak-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])ulak-kanban-md-code\1/i.test(attrs)) {
      return ' class="ulak-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "ulak-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "ulak-kanban-md",
  ...
  // standard `drop` event and our `ulak-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("ulak-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("ulak-kanban-column--drop");
          if (target) target.classList.add("ulak-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("ulak-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("ulak-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("ulak-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__ULAK_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "ulak-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "ulak-kanban-attention",
        "ulak-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "ulak-kanban-attention-bar" },
        h("span", { className: "ulak-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "ulak-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "ulak-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "ulak-kanban-attention-row",
                  "ulak-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "ulak-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "ulak-kanban-attention-row-id" }, task.id),
                h("span", { className: "ulak-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "ulak-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "ulak-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "ulak-kanban-diag-action-btn",
      action.suggested ? "ulak-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " ulak-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".ulak-kanban-drawer-comment-row input, .ulak-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "ulak-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("ulak-kanban-diag", sevClass) },
      h("div", { className: "ulak-kanban-diag-header" },
        h("span", { className: "ulak-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "ulak-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "ulak-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "ulak-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "ulak-kanban-diag-data-row" },
                  h("span", { className: "ulak-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "ulak-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "ulak-kanban-diag-data-row" },
                h("span", { className: "ulak-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "ulak-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "ulak-kanban-diag-reassign-row" },
            h("span", { className: "ulak-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "ulak-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "ulak-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "ulak-kanban-diag-msg",
              msg.ok ? "ulak-kanban-diag-msg--ok" : "ulak-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "ulak-kanban-section" },
      h("div", { className: "ulak-kanban-section-head-row" },
        h("span", { className: "ulak-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "ulak-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "ulak-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "ulak-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "ulak-kanban-docs-link",
      title: "Open Ulak Kanban docs in a new tab",
      "aria-label": "Ulak Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `ulak kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "ulak-kanban-msg-ok" : "ulak-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "ulak-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "ulak-kanban-boardswitcher" },
      h("div", { className: "ulak-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "ulak-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "ulak-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "ulak-kanban-dialog-title" },
        ...
        h("div", { className: "ulak-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Ulak profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "ulak-kanban-bulk" },
      h("span", { className: "ulak-kanban-bulk-count" },
      ...
      h("div", { className: "ulak-kanban-bulk-priority",
      ...
      h("div", { className: "ulak-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Ulak profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "ulak-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("ulak-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("ulak-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "ulak-kanban-trash",
        dragOver ? "ulak-kanban-trash--drop" : "",
        props.draggingTaskId ? "ulak-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "ulak-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "ulak-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".ulak-kanban-card")) return true;
      if (target.closest && target.closest(".ulak-kanban-column-add")) return true;
      if (target.closest && target.closest(".ulak-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("ulak-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("ulak-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".ulak-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ...
      className: cn(
        "ulak-kanban-columns",
        isScrollable ? "ulak-kanban-columns--scrollable" : "",
        isPanning ? "ulak-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("ulak-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("ulak-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "ulak-kanban-column",
        dragOver ? "ulak-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "ulak-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "ulak-kanban-col-check",
        ...
        h("span", { className: cn("ulak-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "ulak-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "ulak-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "ulak-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "ulak-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "ulak-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "ulak-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "ulak-kanban-lane" },
                  h("div", { className: "ulak-kanban-lane-head" },
                    h("span", { className: "ulak-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "ulak-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "ulak-kanban-card--stale-red";
    if (age >= tier.amber) return "ulak-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".ulak-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "ulak-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "ulak-kanban-card",
        props.selected ? "ulak-kanban-card--selected" : "",
        props.failed ? "ulak-kanban-card--failed" : "",
        props.draggingSource ? "ulak-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "ulak-kanban-card-content" },
          h("div", { className: "ulak-kanban-card-row" },
            h("label", {
              className: "ulak-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "ulak-kanban-card-check",
            ...
            h("span", { className: "ulak-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or ulak kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "ulak-kanban-warning-badge",
                    "ulak-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "ulak-kanban-needs-assignee",
          ...
          h("div", { className: "ulak-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "ulak-kanban-card-row ulak-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "ulak-kanban-assignee",
                            title: `Assigned to Ulak profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "ulak-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "ulak-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "ulak-kanban-ago",
  ...
  // centered modal reusing the ulak-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "ulak-kanban-dialog ulak-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Ulak profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Ulak profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "ulak-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "ulak-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "ulak-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "ulak-kanban-drawer-close",
        ...
        data ? h("div", { className: "ulak-kanban-drawer-comment-foot" },
          h("div", {
            className: "ulak-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "ulak-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "ulak-kanban-section" },
      h("div", { className: "ulak-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "ulak-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "ulak-kanban-drawer-body" },
      h("div", { className: "ulak-kanban-drawer-title" },
        h("span", { className: cn("ulak-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "ulak-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "ulak-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "ulak-kanban-section" },
            h("div", { className: "ulak-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "ulak-kanban-section" },
            h("div", { className: "ulak-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "ulak-kanban-done-no-result ulak-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "ulak-kanban-section" },
            h("div", { className: "ulak-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "ulak-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "ulak-kanban-section" },
        h("div", { className: "ulak-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "ulak-kanban-comment" },
            h("div", { className: "ulak-kanban-comment-head" },
              h("span", { className: "ulak-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "ulak-kanban-diag-action-btn",
      ...
      h("div", { className: "ulak-kanban-section" },
        h("div", { className: "ulak-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "ulak-kanban-comment" },
                h("div", { className: "ulak-kanban-comment-head" },
                  h("span", { className: "ulak-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "ulak-kanban-comment-ago" },
      ...
      h("div", { className: "ulak-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "ulak-kanban-event",
              isDiag ? "ulak-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "ulak-kanban-event-header" },
                  h("span", { className: "ulak-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "ulak-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "ulak-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "ulak-kanban-event-header-plain" },
                  h("span", { className: "ulak-kanban-event-kind" }, e.kind),
                  h("span", { className: "ulak-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "ulak-kanban-event-phantom-row" },
                  h("span", { className: "ulak-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "ulak-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "ulak-kanban-event-payload" },
    ...
    return h("div", { className: "ulak-kanban-section" },
      h("div", { className: "ulak-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "ulak-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `ulak-kanban-run--${r.outcome || r.status || "ended"}`
          : "ulak-kanban-run--active";
        return h("div", { key: r.id, className: cn("ulak-kanban-run", outcomeClass) },
          h("div", { className: "ulak-kanban-run-head" },
            h("span", { className: "ulak-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "ulak-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "ulak-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "ulak-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "ulak-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "ulak-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "ulak-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "ulak-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "ulak-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "ulak-kanban-pre ulak-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "ulak-kanban-meta-row" },
      h("span", { className: "ulak-kanban-meta-label" }, props.label),
      h("span", { className: "ulak-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "ulak-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "ulak-kanban-meta-row" },
        h("span", { className: "ulak-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "ulak-kanban-meta-value ulak-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "ulak-kanban-meta-row" },
        h("span", { className: "ulak-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "ulak-kanban-meta-value ulak-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "ulak-kanban-meta-row" },
        h("span", { className: "ulak-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "ulak-kanban-meta-value ulak-kanban-editable",
    ...
    return h("div", { className: "ulak-kanban-meta-row" },
      h("span", { className: "ulak-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "ulak-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "ulak-kanban-section" },
      h("div", { className: "ulak-kanban-section-head-row" },
        h("span", { className: "ulak-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "ulak-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "ulak-kanban-section" },
      h("div", { className: "ulak-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "ulak-kanban-deps-row" },
        h("span", { className: "ulak-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "ulak-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "ulak-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "ulak-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "ulak-kanban-dep-chip-x",
      ...
      h("div", { className: "ulak-kanban-deps-row" },
        h("span", { className: "ulak-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "ulak-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "ulak-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "ulak-kanban-section" },
      ...
      h("div", { className: "ulak-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "ulak-kanban-home-sub ulak-kanban-home-sub--on"
              : "ulak-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__ULAK_PLUGINS__ && typeof window.__ULAK_PLUGINS__.register === "function") {
    window.__ULAK_PLUGINS__.register("kanban", KanbanPage);
  }
})();
