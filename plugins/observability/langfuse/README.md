...
This plugin ships bundled with Ulak but is **opt-in** — it only loads when
...
ulak tools  # → Langfuse Observability
...
Set these in `~/.ulak/.env` (or via `ulak tools`):
...
ulak chat -q "hello"              # then check Langfuse for a "Ulak turn" trace
```

Generation observations include the Ulak system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `ULAK_LANGFUSE_MAX_CHARS`).

## Optional tuning

```bash
ULAK_LANGFUSE_ENV=production       # environment tag
ULAK_LANGFUSE_RELEASE=v1.0.0       # release tag
ULAK_LANGFUSE_SAMPLE_RATE=0.5      # sample 50% of traces
ULAK_LANGFUSE_MAX_CHARS=12000      # max chars per field (default: 12000)
ULAK_LANGFUSE_CAPTURE=sanitized    # content capture mode (see below)
ULAK_LANGFUSE_DEBUG=true           # verbose plugin logging
```

## Capture modes

`ULAK_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
...
```bash
ulak plugins disable observability/langfuse
```
