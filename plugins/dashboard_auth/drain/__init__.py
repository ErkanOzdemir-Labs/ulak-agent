...
secret (``ULAK_DASHBOARD_DRAIN_SECRET``, env-only — it is a credential); an inbound bearer
...
from ulak_cli.dashboard_auth import DashboardAuthProvider, Session, TokenPrincipal
...
def _settings() -> dict:
    ...
    secret = os.environ.get("ULAK_DASHBOARD_DRAIN_SECRET", "").strip()
    if not secret:
        raise SkipRegistration(
            "ULAK_DASHBOARD_DRAIN_SECRET is not set. Set a per-agent >=256-bit secret "
    ...
    if reason is not None:
        raise SkipRegistration(
            f"ULAK_DASHBOARD_DRAIN_SECRET rejected — {reason}. The drain endpoint stays disabled (fail-closed).",
            level="warning")
...
def register(ctx) -> None:
    ...
    reason) when ``ULAK_DASHBOARD_DRAIN_SECRET`` is unset or fails the entropy gate. On
    ...
    try:
        from ulak_cli.dashboard_auth.token_auth import register_token_route
