...
``ULAK_DASHBOARD_BASIC_AUTH_*`` env vars (env wins when non-empty; see ``_settings``).
"""
...
from ulak_cli.dashboard_auth import DashboardAuthProvider, InvalidCredentialsError, RefreshExpiredError, Session
...
def _resolve_secret(cfg_section: dict) -> bytes:
    ...
    raw = resolve_env_or_cfg("ULAK_DASHBOARD_BASIC_AUTH_SECRET", cfg_section.get("secret"))
    if not raw:
        logger.info(
            ...
            "ULAK_DASHBOARD_BASIC_AUTH_SECRET) for stable sessions.")
...
def _settings() -> dict:
    ...
    username = setting("ULAK_DASHBOARD_BASIC_AUTH_USERNAME", "username")
    password_hash = setting("ULAK_DASHBOARD_BASIC_AUTH_PASSWORD_HASH", "password_hash")
    plaintext = setting("ULAK_DASHBOARD_BASIC_AUTH_PASSWORD", "password")
    ttl_raw = setting("ULAK_DASHBOARD_BASIC_AUTH_TTL_SECONDS", "session_ttl_seconds")
    if not username:
        raise SkipRegistration(
            "dashboard.basic_auth.username is not set (and ULAK_DASHBOARD_BASIC_AUTH_USERNAME "
    ...
    plaintext_from_env = os.environ.get("ULAK_DASHBOARD_BASIC_AUTH_PASSWORD", "").strip()
...
_PLUGIN_COMPAT_LAZY = {
    'LoginStart': ('ulak_cli.dashboard_auth', 'LoginStart'),
}
