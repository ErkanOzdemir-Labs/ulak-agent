...
when a client_id (``dashboard.oauth.client_id`` / ``ULAK_DASHBOARD_OAUTH_CLIENT_ID``, shape
...
from ulak_cli.dashboard_auth import LoginStart, ProviderError, Session
...
_DEFAULT_PORTAL_URL = "https://portal.github.com/ErkanOzdemir-Labs"
...
def _settings() -> dict:
    ...
    client_id = resolve_env_or_cfg("ULAK_DASHBOARD_OAUTH_CLIENT_ID", section.get("client_id", ""))
    portal_url = resolve_env_or_cfg("ULAK_DASHBOARD_PORTAL_URL", section.get("portal_url", "")) or _DEFAULT_PORTAL_URL
    if not client_id:
        raise SkipRegistration(
            "ULAK_DASHBOARD_OAUTH_CLIENT_ID is not set (and dashboard.oauth.client_id "
            "in config.yaml is empty). The Nous Portal provisions this env var (shape "
            "'agent:{instance_id}') when it deploys a Ulak Agent instance — set it to "
    ...
    if not client_id.startswith("agent:"):
        raise SkipRegistration(
            f"ULAK_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match the contract "
...
_PLUGIN_COMPAT_LAZY = {
    'DashboardAuthProvider': ('ulak_cli.dashboard_auth', 'DashboardAuthProvider'),
    'InvalidCodeError': ('ulak_cli.dashboard_auth', 'InvalidCodeError'),
    'RefreshExpiredError': ('ulak_cli.dashboard_auth', 'RefreshExpiredError'),
    'classify_jwks_lookup_error': ('ulak_cli.dashboard_auth', 'classify_jwks_lookup_error'),
}
