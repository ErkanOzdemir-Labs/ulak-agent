"""Email platform adapter for the Ulak gateway: users talk to Ulak by sending email; IMAP (polled)
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        try:
            ...
            _ulak_version = "0"
        imap.xatom("ID", f'("name" "ulak-agent" "version" "{_ulak_version}" '
                         '"vendor" "ErkanOzdemir-Labs" "support-email" "noreply@github.com/ErkanOzdemir-Labs")')
...
class EmailAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing := [name for name, value in required if not value]:
            message = f"Not configured — missing {', '.join(missing)}. Set it via `ulak gateway setup` (env) or platforms.email in config.yaml."
...
async def _standalone_send(pconfig, chat_id, message, *, thread_id=None, media_files=None, force_document=False):
    ...
    try:
        ...
        for key, value in (("From", address), ("To", chat_id), ("Subject", "Ulak Agent"), ("Date", formatdate(localtime=True))):
            msg[key] = value
