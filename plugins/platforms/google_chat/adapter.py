...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        try:
            from ulak_constants import get_ulak_home as _get_ulak_home
            _ulak_home = _get_ulak_home()
        except (ModuleNotFoundError, ImportError):
            _ulak_home = _Path.home() / ".ulak"
        self._thread_count_store = _ThreadCountStore(_ulak_home / "google_chat_thread_counts.json")
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        from ulak_constants import get_ulak_home as _get_ulak_home
        return _get_ulak_home() / "google_chat_bot_id.json"
    ...
    async def _build_message_event(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> Optional[MessageEvent]:
        """Parse a Chat API message into a ulak MessageEvent."""
    ...
    async def send_clarify(
        ...
        def _button(text: str, choice: str) -> Dict[str, Any]:
            return {"text": text, "action": "ulak_clarify", "parameters": {"clarify_id": clarify_id, "choice": choice}}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Ulak is thinking…' marker (Chat has no typing API); ``send()``
        ...
        body = _thread_body(getattr(self.config, "typing_status_text", None) or "Ulak is thinking…", thread_id)
...
Walkthrough:
  ...
  3. Create a Pub/Sub topic (e.g. ulak-chat-events) and a Pull subscription.
...
def interactive_setup() -> None:
    """``ulak setup`` wizard: print GCP instructions, prompt for env vars, persist to ``~/.ulak/.env``."""
    from ulak_cli.cli_output import print_info, print_success, print_warning, prompt, prompt_yes_no
    ...
    print()
    print_success("Google Chat configuration saved to ~/.ulak/.env")
    print_info("Restart the gateway: ulak gateway restart")
...
async def _standalone_send(
    ...
    gateway runner is not in-process, e.g. ``ulak cron``). Needs SA credentials and a
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `ulak setup` to install Google Chat support.",
        ...
        platform_hint=(
            ...
            "confirmations instead. While you are generating a response, a 'Ulak is thinking…' marker message "
