# A2A — Agent-to-Agent protocol for Ulak
...
peer (another Ulak, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
ulak gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Ulak serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.ulak/a2a_audit.jsonl`.
- Conversations persist to `~/.ulak/a2a_conversations/` — they survive context
