...
_DEFAULT_DESCRIPTION = "Ulak Agent — a general-purpose agent reachable over A2A."
...
def _default_agent_name() -> str:
    ...
    try:
        import socket
        return f"ulak-{socket.gethostname()}"
    except Exception:
        return "ulak-agent"
...
def _active_profile_name() -> str:
    try:
        ...
        return os.getenv("ULAK_PROFILE", "default") or "default"
...
def _profile_home(profile: str) -> Optional[str]:
    ...
    if profile and profile != "default":
        return os.path.expanduser(f"~/.ulak/profiles/{profile}")
    with contextlib.suppress(Exception):
        from ulak_cli.config import get_ulak_home
        return str(get_ulak_home())
    return None
...
class A2AAdapter(BasePlatformAdapter):
    ...
    def _load_served_agents(self, extra: dict) -> dict[str, dict]:
        ...
        for key, val in items:
            ...
            agents[slug] = {
                ...
                "name": str(val.get("name") or f"Ulak {slug}"),
                "description": str(val.get("description") or f"Ulak profile '{profile or slug}' exposed over A2A."),
    ...
    def _forward_to_profile(self, agent: dict, peer: str, context_id: str, framed_text: str) -> tuple[str, str]:
        """Forward a routed task to another local profile via ``ulak chat``. First contact creates a
        ...
        with self._forward_lock(key):
            ...
            cmd = ["ulak", "chat", "-q", framed_text, "-Q", "--source", "a2a"] + (["--resume", session_id] if session_id else [])
            env = {**os.environ, "ULAK_A2A_PEER": peer}
