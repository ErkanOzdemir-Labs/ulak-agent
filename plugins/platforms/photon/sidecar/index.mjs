// Ulak Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Ulak): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Ulak -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Ulak-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `ulak photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Ulak can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "ulak-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Ulak'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Ulak allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Ulak, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-ulak-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Ulak supplied them so a known-good
