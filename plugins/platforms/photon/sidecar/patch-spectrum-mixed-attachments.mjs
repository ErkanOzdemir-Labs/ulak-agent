...
// `message.content.text` before Ulak can see it. We rewrite the two inbound
...
const MARKER = "Ulak patch: Preserve mixed text + attachment iMessage payloads";
...
export function patchSpectrumTs(root = scriptDir()) {
  ...
  for (const file of files) {
    ...
    // Ulak to patch; keep the legacy v8 path below for older pinned installs.
    if (
