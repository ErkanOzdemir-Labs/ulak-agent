...
Small Node helper that bridges Ulak Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Ulak is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Ulak-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Ulak can receive inbound messages
...
The Ulak plugin's `ulak photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Ulak is Python, so replies go through this sidecar
