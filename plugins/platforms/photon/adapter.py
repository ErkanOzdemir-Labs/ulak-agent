...
if TYPE_CHECKING:  # type checkers see httpx as always-imported; runtime keeps it optional
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Ulak dep
...
# Out-of-process senders (cron, `ulak send`) need the live sidecar's port + spawn-time
...
# suppress those so Ulak sees the link once.
...
_DEFAULT_MENTION_PATTERNS = [r"(?<![\w@])@?ulak\s+agent\b[,:\-]?", r"(?<![\w@])@?ulak\b[,:\-]?"]
...
def _runtime_record_path() -> Path:
    from ulak_constants import get_ulak_home  # honors profile overrides
    return get_ulak_home() / "runtime" / _RUNTIME_RECORD_NAME
...
def check_requirements() -> bool:
    ...
    if not sidecar_deps_installed():
        ...
        logger.debug("photon: spectrum-ts not installed at %s%s — run: ulak photon setup", _sidecar_dir(), hint)
        return False
    return True


def _sidecar_deps_stale() -> bool:
    """True when node_modules predates the lockfile (`ulak update` rewrites it without
...
def _reinstall_sidecar_deps() -> None:
    ...
    if not npm:
        ...
        return
    from ulak_cli._subprocess_compat import windows_hide_flags  # no console flash on Windows
...
def _attachment_body(space_id: str, safe_path: str, *, kind: str, name: Optional[str] = None,
    ...
    so optional keys are only sent when Ulak supplied them."""
    body: Dict[str, Any] = {
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _sidecar_headers(self) -> Dict[str, str]:
        return {"X-Ulak-Sidecar-Token": self._sidecar_token}
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not self._project_id or not self._project_secret:
            self._set_fatal_error(
                "MISSING_CREDENTIALS",
                "PHOTON_PROJECT_ID and PHOTON_PROJECT_SECRET are required. Run: ulak photon setup",
                retryable=False)
            return False
    ...
    def _pid_is_sidecar(cls, pid: int) -> bool:
        """True if ``pid``'s command line is a Photon sidecar (any Ulak checkout)."""
    ...
    async def _ensure_sidecar_deps(self) -> None:
        ...
        if not sidecar_deps_installed():
            # Hosted images have no CLI for `ulak photon setup`: connect bootstraps deps itself.
            ...
            if not sidecar_deps_installed():
                ...
                raise PhotonSidecarStartupError(
                    ...
                    f"Run: cd {_sidecar_dir()} && npm ci   (or `ulak photon setup`)",
                    code="SIDECAR_DEPS_MISSING", retryable=False)
        # `ulak update` bumps the lockfile without reinstalling node_modules; the sidecar
    ...
    async def _start_sidecar(self) -> None:
        ...
        from ulak_cli._subprocess_compat import windows_hide_flags  # hide child console on Windows
...
def _standalone_token_from_record(port: int) -> Tuple[Optional[str], int, str]:
    ...
    return None, port, (
        "Photon standalone send requires a running sidecar. Start the Ulak gateway (which spawns "
        f"the sidecar and records its address under <ulak-home>/runtime/{_RUNTIME_RECORD_NAME}), "
...
async def _standalone_send(
    ...
    headers = {"X-Ulak-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the Ulak plugin loader at startup."""
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: ulak photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."),
        setup_fn=_cli.gateway_setup,  # surfaces Photon in the unified `ulak gateway setup` wizard
