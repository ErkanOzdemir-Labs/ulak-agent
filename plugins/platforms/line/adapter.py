...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        path is rechecked against allowed roots (tempdir, ``/tmp``→``/private/tmp`` on macOS, ULAK_HOME).
        ...
        try:
            ...
            ulak_home = Path(get_ulak_home()).resolve()
        except Exception:
            ulak_home = Path.home().joinpath(".ulak").resolve()
        resolved = path.resolve()
        if not any(resolved.is_relative_to(r) for r in (Path(tempfile.gettempdir()).resolve(), Path("/tmp").resolve(), ulak_home)):
...
def is_connected(config) -> bool:
    """Surface in ``ulak status`` even before the adapter is instantiated."""
    return validate_config(config)


def _env_enablement() -> Optional[Dict[str, Any]]:
    """Seed PlatformConfig.extra from env-only setups so ``ulak status`` sees them."""
    if not _env_credentials_present():
        return None
    seeded: Dict[str, Any] = {}
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``ulak setup line`` (writes ``~/.ulak/.env``)."""
    print("\nLINE Messaging API setup\n------------------------\n"
          "Create a Messaging API channel at https://developers.line.biz/console/\nthen copy the values below.\n")
    try:
        from ulak_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("ulak_cli.config not available; set LINE_* vars manually in ~/.ulak/.env")
        return
