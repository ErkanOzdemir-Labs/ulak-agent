...
(enabled/policy/allow_from) falling back independently. Config ~/.ulak/feishu_comment_rules.json (mtime-cached,
hot-reload); pairing store ~/.ulak/feishu_comment_pairing.json."""
...
# long after profile/ULAK_HOME overrides have been applied, so freezing is safe.
RULES_FILE = get_ulak_home() / "feishu_comment_rules.json"
PAIRING_FILE = get_ulak_home() / "feishu_comment_pairing.json"
...
def _main() -> int:
    try:
        __import__("ulak_cli.env_loader", fromlist=["load_ulak_dotenv"]).load_ulak_dotenv()
    except Exception:
        pass
