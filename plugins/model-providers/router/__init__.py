...
``api_mode="codex_responses"`` + the ``api.router.com`` host mandate in ``ulak_cli/providers.py``
...
def _resolve_api_key() -> str:
    ...
    try:
        from ulak_cli.config import get_env_value_prefer_dotenv as prefer_dotenv
    except Exception:
        prefer_dotenv = None
...
def _disk_path() -> Optional[Path]:
    try:
        ...
        return get_ulak_home() / "cache" / "router_catalog.json"
    except Exception:
        return None
