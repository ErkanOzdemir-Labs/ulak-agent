...
An out-of-tree ACP provider (``~/.ulak/plugins/model-providers/`` or a pip entry point)
...
copilot_acp = CopilotACPProfile(
    ...
    # ulak_cli/auth.py), so existing setups keep working.
    process_command="copilot",
    process_args=("--acp", "--stdio"),
    process_command_env_vars=("ULAK_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH"),
    process_args_env_var="ULAK_COPILOT_ACP_ARGS",
)
