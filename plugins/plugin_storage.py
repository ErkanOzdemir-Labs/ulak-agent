"""Per-plugin persistent storage: ``<ulak home>/plugin-data/<name>/``.

Plugins must NOT park state in ``<ulak home>/plugins/<name>/`` (the install dir, deleted by
...
# Mirrors the plugin-name shape `ulak plugins install` accepts (no separators/traversal).
...
def plugin_data_dir(name: str) -> Path:
    """Return (and create) ``<ulak home>/plugin-data/<name>/``; resolves ``get_ulak_home()`` on
    every call so it follows the active profile — don't cache across profile switches."""
    from ulak_constants import get_ulak_home
    root = get_ulak_home() / "plugin-data" / _validate_name(name)
