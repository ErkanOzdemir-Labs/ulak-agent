...
One active meeting at a time, recorded in ``$ULAK_HOME/workspace/meetings/.active.json``
...
def _root() -> Path:
    return Path(get_ulak_home()) / "workspace" / "meetings"
...
def start(url: str, *, out_dir: Optional[Path] = None, headed: bool = False,
          auth_state: Optional[str] = None, guest_name: str = "Ulak Agent", duration: Optional[str] = None,
    ...
    for name in ("transcript.txt", "status.json"):
        with contextlib.suppress(OSError):
            (out / name).unlink()
    env = {**os.environ, "ULAK_MEET_URL": url, "ULAK_MEET_OUT_DIR": str(out),
           "ULAK_MEET_GUEST_NAME": guest_name}
    for value, var in (
        (headed and "1", "ULAK_MEET_HEADED"),
        (auth_state, "ULAK_MEET_AUTH_STATE"),
        (duration, "ULAK_MEET_DURATION"),
        (mode, "ULAK_MEET_MODE"),  # bot defaults to transcribe when unset (v1 behavior)
        (realtime_model, "ULAK_MEET_REALTIME_MODEL"),
        (realtime_voice, "ULAK_MEET_REALTIME_VOICE"),
        (realtime_instructions, "ULAK_MEET_REALTIME_INSTRUCTIONS")):
        if value:
            env[var] = value
    ...
    if not realtime_api_key:
        ...
        realtime_api_key = get_secret("ULAK_MEET_REALTIME_KEY") or get_secret("OPENAI_API_KEY")
    if realtime_api_key:
        env["ULAK_MEET_REALTIME_KEY"] = realtime_api_key
