...
Let the ulak agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where ulak runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `ulak meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `ulak meet` CLI |
...
| `cli.py` | `ulak meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ULAK_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `ulak meet node {run,list,approve,remove,status,ping}` |
...
ulak meet install                                      # pip + Chromium
ulak meet setup                                        # preflight
ulak meet auth                                         # optional
ulak meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
ulak meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.ulak/.env
ulak meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
ulak meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
ulak meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.ulak/.env
ulak meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, ulak will **not** switch your system audio input automatically — the
...
ulak plugins enable google_meet
ulak meet node run --display-name my-mac --host 0.0.0.0 --port 18789
