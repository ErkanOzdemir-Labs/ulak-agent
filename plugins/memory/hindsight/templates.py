...
catalog, filter to the ``ulak`` integration, apply a manifest via
...
def fetch_ulak_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``ulak`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "ulak" in (e.get("integrations") or [])]
