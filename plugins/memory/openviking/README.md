...
- OpenViking server running and reachable from Ulak

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Ulak can identify older servers that expose the legacy status-only health
...
connection values into Ulak, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.ulak/.env`; for a named profile use
...
OpenViking's server config is separate from Ulak:
...
Ulak-side provider config is read from environment variables in the active
profile's `.env`:
...
When `OPENVIKING_API_KEY` is set, Ulak lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Ulak sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
Ulak also sends `User-Agent: openviking-memory-ulak/<version>` on
OpenViking requests. This standard harness identifier contains the Ulak
...
does not ask for a peer ID. Without a configured peer, Ulak sends neither
...
order remains environment, linked OpenViking config, then Ulak YAML. To use
...
on the old implicit `ulak` peer now use user memory for new writes. Without
...
Set `agent: ulak` to restore peer-scoped writes. Memories written at user
...
`viking_remember` creates a one-shot `ulak-remember-<random>` OpenViking
...
not advertised or used. The one-shot session is separate from the live Ulak
...
Use the same OpenViking profile and credentials as Ulak for manual recovery.
...
Ulak built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Ulak action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under user memory, or the configured peer memory directory |

Built-in `replace` and `remove` operations are not mirrored because Ulak
...
file URIs, such as
`viking://user/default/peers/ulak/memories/preferences/mem_abc123.md` (any
