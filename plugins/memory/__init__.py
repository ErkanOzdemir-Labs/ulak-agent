...
``$ULAK_HOME/plugins/<name>/``, project ``./.ulak/plugins/<name>/`` (opt-in via
ULAK_ENABLE_PROJECT_PLUGINS), then ``ulak_agent.memory_providers`` entry points.
...
ENTRY_POINTS_GROUP = "ulak_agent.memory_providers"
...
_USER_NAMESPACE = "_ulak_user_memory"
...
def _get_project_plugins_dir() -> Optional[Path]:
    """``./.ulak/plugins/`` or None. Gated on ULAK_ENABLE_PROJECT_PLUGINS like the
    PluginManager scan: a repo you merely ``cd`` into must not offer a memory backend."""
    try:
        from ulak_cli.plugins import _env_enabled

        if not _env_enabled("ULAK_ENABLE_PROJECT_PLUGINS"):
            return None
        d = Path.cwd() / ".ulak" / "plugins"
...
def find_provider_dir(name: str) -> Optional[Path]:
    ...
    provider silently loses its dashboard panel and ``ulak <provider>`` commands."""
...
def _entry_point_package_dir(entry_point) -> Optional[Path]:
    ...
    if entry_point is None:
        return None
    try:
        from ulak_cli.plugins import resolve_module_origin
...
class _ProviderCollector:
    ...
    def _plugin_context(self):
        ...
        if self._context is None:
            from ulak_cli.plugins import PluginContext, PluginManifest, get_plugin_manager
    ...
    try:
        ...
        if cli_mod is None:
            if not _is_bundled(plugin_dir):
                # cli.py imports as _ulak_user_memory.<name>.cli, usually before the
