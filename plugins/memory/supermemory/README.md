...
- Hosted: API key from [app.supermemory.ai/integrations?connect=ulak](http://app.supermemory.ai/integrations?connect=ulak)
...
Before running `ulak memory setup`, add the local endpoint to
...
Then run `ulak memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$ULAK_HOME/supermemory.json`
...
| `container_tag` | `ulak` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `ulak-{identity}` → `ulak-coder`). |
...
`https://api.supermemory.ai`. Ulak resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: ulak`, and document writes stamp
`metadata.sm_source: ulak`. This is a **functional routing key, not telemetry**:
it groups Ulak-written memories into a dedicated "Ulak" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Ulak profile:

```json
{
  "container_tag": "ulak-{identity}"
}
```

For a profile named `coder`, this resolves to `ulak-coder`. The default profile resolves to `ulak-default`. Without `{identity}`, all profiles share the same container.
