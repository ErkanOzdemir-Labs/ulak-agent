...
_DEFAULT_CONTAINER_TAG = "ulak"
_VALID_SEARCH_MODES = ("hybrid", "memories", "documents")
_DEFAULT_BASE_URL = "https://api.supermemory.ai"
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=ulak"
...
def _load_supermemory_config(ulak_home: Optional[str] = None) -> dict:
    """Defaults overlaid with $ulak_home/supermemory.json (None = defaults only), every key normalized."""
    config = {k: (list(d) if isinstance(d, list) else d) for k, (d, _) in _CONFIG_SPEC.items()}
    if ulak_home is not None:
        config.update({k: v for k, v in _read_json_dict(Path(ulak_home) / "supermemory.json").items() if v is not None})
    for key, (_, normalize) in _CONFIG_SPEC.items():
        config[key] = normalize(config[key])
    return config


def _save_supermemory_config(values: dict, ulak_home: str) -> None:
    from utils import atomic_json_write
    config_path = Path(ulak_home) / "supermemory.json"
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(api_key=api_key, base_url=self._base_url, timeout=timeout, max_retries=0,
                                   default_headers={"x-sm-source": "ulak"})

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Ulak writes into the "Ulak" Space in the Supermemory app so the user
        # can filter / bulk-manage them per source agent (a routing key for the user, not telemetry).
        merged = {"sm_source": "ulak", **(metadata or {})}
...
def _probe_supermemory_connection(api_key: str, ulak_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(ulak_home)
...
class SupermemoryMemoryProvider(MemoryProvider):
    def __init__(self):
        self._api_key = self._session_id = self._ulak_home = ""
    ...
    def get_config_schema(self):
        # Only the API key is prompted during `ulak memory setup`; other options live in supermemory.json / env.
    ...
    def save_config(self, values, ulak_home):
        ...
        _save_supermemory_config(sanitized, ulak_home)

    def get_status_config(self, provider_config: dict) -> dict:
        from ulak_constants import get_ulak_home
        return {"summary": _format_connection_summary(_probe_supermemory_connection(get_secret("SUPERMEMORY_API_KEY", "") or "", str(get_ulak_home())))}

    def post_setup(self, ulak_home: str, config: dict) -> None:
        from ulak_cli.config import save_config
        from ulak_cli.memory_setup import _prompt, _write_env_vars
        ...
        save_config(config)
        if val:
            _write_env_vars({"SUPERMEMORY_API_KEY": val}, ulak_home=ulak_home)
        ...
        status = _probe_supermemory_connection(api_key, ulak_home)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from ulak_constants import get_ulak_home
        self._ulak_home = kwargs.get("ulak_home") or str(get_ulak_home())
        self._session_id, self._turn_count, self._session_turns = session_id, 0, []
        config = _load_supermemory_config(self._ulak_home)
