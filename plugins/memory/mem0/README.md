...
Behavioral settings live in `$ULAK_HOME/mem0.json` (set them via `ulak memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.ulak/.env`.
...
| `user_id` | `ulak-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Ulak session and call `mem0_search` — it connects to your server.
...
```bash
ulak memory setup mem0 --mode oss \
...
```bash
ulak memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$ULAK_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.ulak/mem0_qdrant"}}
  }
}
```
...
```bash
ulak memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$ULAK_HOME/mem0.json`).
