...
Config file resolution: $ULAK_HOME/honcho.json -> ~/.honcho/config.json -> env vars
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Ulak profile."""
...
def resolve_active_host() -> str:
    """Honcho host key: ULAK_HONCHO_HOST env, else the active profile. The config's
    ``defaultHost`` is honored only for the default profile so named profiles stay isolated."""
    explicit = os.environ.get("ULAK_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
    try:
...
def resolve_config_path() -> Path:
    """Active Honcho config path: $ULAK_HOME/honcho.json -> default profile's honcho.json
    ...
    local_path = get_ulak_home() / "honcho.json"
    if local_path.exists():
        return local_path
    default_path = _get_default_ulak_home() / "honcho.json"
...
def _connection_fields(look: _HostLookup, host: str, path: Path) -> dict[str, Any]:
    ...
    # read it before the flat Ulak spellings.
...
class HonchoClientConfig:
    ...
    # re-resolving resolve_config_path()/get_ulak_home() later — those resolvers read a ContextVar that
    ...
    @classmethod
    def from_env(cls, workspace_id: str = "ulak", host: str | None = None) -> HonchoClientConfig:
        ...
        return cls(
            ...
            config_path=resolve_config_path(), ulak_home=get_ulak_home(),
        )
    ...
    def from_global_config(cls, host: str | None = None, config_path: Path | None = None) -> HonchoClientConfig:
        ...
        derives it from the active Ulak profile."""
        resolved_host = host or resolve_active_host()
        ...
        return cls(
            ...
            config_path=path, ulak_home=get_ulak_home(),
        )
...
def spawn_context_thread(target, *, name: str, daemon: bool = True, args: tuple = ()) -> "_threading.Thread":
    ...
    (set_ulak_home_override) and a plain Thread starts EMPTY, so ambient resolution on it
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError("Honcho API key not found. Get your API key at https://app.honcho.dev, "
                         "then run 'ulak honcho setup' or set HONCHO_API_KEY. "
...
def _build_client(config: HonchoClientConfig) -> "Honcho":
    ...
    try:
        ...
        raise ImportError("honcho-ai is required for Honcho integration. Install it with: pip install honcho-ai  "
                          "(or run `ulak honcho setup` to configure).")
