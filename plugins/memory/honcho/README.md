...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/ulak>
...
```bash
ulak memory setup honcho   # configure Honcho directly (works on a fresh install)
ulak memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
ulak config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.ulak/.env
```

> `ulak honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `ulak memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `ulak model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ULAK_HOME/honcho.json` | Profile-local (isolated Ulak instances) |
| 2 | `~/.ulak/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Ulak profile: `ulak` (default) or `ulak_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `ulak honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Ulak session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`ulak-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`ulak`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-ulak-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening ulak in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Ulak session ID (timestamp + hex). Every `ulak` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Ulak profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`ulak`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Ulak profile: `ulak` (default) or `ulak_<profile>` (e.g. `ulak -p coder` -> host key `ulak_coder`). Older `ulak.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `ULAK_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `ulak-agent`) |
...
|---------|-------------|
| `ulak memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `ulak honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `ulak memory setup`) |
| `ulak honcho status` | Show resolved config for active profile |
| `ulak honcho enable` / `disable` | Toggle Honcho for active profile |
| `ulak honcho mode <mode>` | Change recall or observation mode |
| `ulak honcho peer --user <name>` | Update user peer name |
| `ulak honcho peer --ai <name>` | Update AI peer name |
| `ulak honcho tokens --context <N>` | Set context token budget |
| `ulak honcho tokens --dialectic <N>` | Set dialectic max chars |
| `ulak honcho map <name>` | Map current directory to a session name |
| `ulak honcho sync` | Create host blocks for all Ulak profiles |
