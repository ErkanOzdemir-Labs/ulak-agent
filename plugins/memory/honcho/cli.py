"""``ulak honcho`` subcommands: setup wizard, status, peers, sessions, identity, migrate."""
...
from ulak_cli.config import cfg_get

RULE = "─" * 40
REASONING_LEVELS = ("minimal", "low", "medium", "high", "max")
_RETRY_HINT = "  Re-run 'ulak honcho setup' to retry, or choose an API key instead.\n"
...
def _local_config_path() -> Path:
    ...
    return get_ulak_home() / "honcho.json"
...
def _label(host: str) -> str:
    return f"[{host}] " if host != "ulak" else ""
...
def _sync_profiles(verbose: bool) -> int:
    ...
    cfg = _read_config()
    if not cfg:
        return say("  No Honcho config found. Run 'ulak honcho setup' first.\n") or 0
    default_block, has_key = _default_block_and_key(cfg)
    if not default_block and not has_key:
        return say("  Honcho not configured on default profile. Run 'ulak honcho setup' first.\n") or 0
...
def sync_honcho_profiles_quiet() -> int:
    """Sync host blocks for all profiles from `ulak update`; no output, no exceptions."""
...
def _resolve_effective_identity_mapping(cfg: dict, ulak_host: dict) -> tuple[bool, dict, str, bool, bool]:
...
def _apply_runtime_prefix(ulak_host: dict, current_prefix: str, prefix_from_root: bool, label: str) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        ulak_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(ulak_host: dict) -> None:
    print(f"  resolved →\n    pinUserPeer       = {bool(ulak_host.get('pinUserPeer'))}\n"
          f"    userPeerAliases   = {ulak_host.get('userPeerAliases') or '{}'}\n"
          f"    runtimePeerPrefix = {ulak_host.get('runtimePeerPrefix') or '(none)'}")


def _configure_raw_identity_mapping(ulak_host, current_pin, current_aliases, current_prefix,
    ...
    _scrub_identity_mapping(ulak_host)
    ulak_host["pinUserPeer"] = pin
    if pin:
        return
    ...
    while entry := _prompt("    alias", default="").strip():
        ...
        if rid and peer:
            aliases[rid] = peer
    if aliases:
        ulak_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(ulak_host, current_prefix, prefix_from_root,
                          "runtimePeerPrefix — namespace for unknown IDs (blank for none)")


def _setup_identity_mapping(cfg: dict, ulak_host: dict, current_peer: str) -> None:
    ...
    current_pin, current_aliases, current_prefix, aliases_from_root, prefix_from_root = (
        _resolve_effective_identity_mapping(cfg, ulak_host))
    ...
    if gw_platforms:
        ...
        notice, question = (
            ("\n  Gateway identity mapping routes platform users to memory peers.",
             "Running the Ulak gateway (Telegram/Discord/etc.)? (y/N)") if gw_platforms is None else
    ...
    peer_target = ulak_host.get("peerName") or current_peer or "user"
    ...
    if shape == "raw":
        _configure_raw_identity_mapping(ulak_host, current_pin, current_aliases, current_prefix,
        ...
        if shape == "multi" and aliases_from_root:
            prior_aliases = {}
        _scrub_identity_mapping(ulak_host)  # each shape starts from a clean slate
        ulak_host["pinUserPeer"] = shape == "single"
        if shape == "single":
    ...
    _echo_identity_mapping(ulak_host)
...
def _headless() -> tuple[bool, bool]:
    """(is_remote, can_open_browser) — degrades safely if ulak_cli internals move."""
    try:
        from ulak_cli.auth import _can_open_graphical_browser, _is_remote_session
        return _is_remote_session(), _can_open_graphical_browser()
    except Exception:
        return False, True


def _apply_grant_to_host(ulak_host: dict, cred) -> None:
    """Store an OAuth grant on the host block; the wizard's final save persists it."""
    ulak_host["apiKey"] = cred.access_token
    ulak_host["oauth"] = cred.oauth_block()
    if cred.consent_peer_name:  # default the peer prompt to the consent name
        ulak_host["peerName"] = cred.consent_peer_name
    print("  Authorized — token saved. Let's finish configuring.\n")


def _setup_local_auth(cfg: dict, ulak_host: dict) -> None:
    ...
    if new_url := _prompt("Base URL", default=cfg.get("baseUrl") or "http://localhost:8000"):
        cfg["baseUrl"] = new_url
    current_host_key = ulak_host.get("apiKey", "")
    ...
    if new_local_key:
        ulak_host["apiKey"] = new_local_key
...
def _setup_device_login(ulak_host: dict, write_path: Path, *, open_browser: bool) -> bool:
    ...
    try:
        ...
        print(" approved")
        _apply_grant_to_host(ulak_host, cred)
        return True
    return False


def _setup_browser_login(ulak_host: dict, write_path: Path) -> bool:
    ...
    try:
        cred = authorize_via_loopback(config_path=write_path, source="ulak-cli", apply_config=False, open_url=_open)
    except Exception as e:
...
def _setup_cloud_auth(cfg: dict, ulak_host: dict, write_path: Path) -> bool:
    ...
    existing_oauth = OAuthCredential.from_host_block(ulak_host)
    ...
    if device_available and method in {"device", "d"}:
        return _setup_device_login(ulak_host, write_path, open_browser=can_browse and not is_remote)
    if method in {"oauth", "o"}:
        return _setup_browser_login(ulak_host, write_path)
    ...
    print("\n  No API key configured. Get yours at https://app.honcho.dev\n"
          "  Run 'ulak honcho setup' again once you have a key.\n")
    return False
...
def _choice_step(ulak_host, key, current, label, valid, fallback=None) -> None:
    """Prompt for one of ``valid``; an invalid answer writes ``fallback`` (None = keep current)."""
    new = _prompt(label, default=current)
    if new in valid:
        ulak_host[key] = new
    elif fallback is not None:
        ulak_host[key] = fallback


def _setup_tuning(cfg: dict, ulak_host: dict) -> None:
    ...
    _choice_step(ulak_host, "observationMode", _pref(ulak_host, cfg, "observationMode", "directional"),
    ...
    new_wf = _prompt("Write frequency", default=str(_pref(ulak_host, cfg, "writeFrequency", "async")))
    ulak_host["writeFrequency"] = _first_parsed([new_wf], int, new_wf if new_wf in {"async", "turn", "session"} else "async")

    _menu("Recall mode", *(f"{m:<7} -- {desc}" for m, desc in _MODES.items()))
    raw_recall = _pref(ulak_host, cfg, "recallMode", "hybrid")
    _choice_step(ulak_host, "recallMode", raw_recall if raw_recall in _MODES else "hybrid", "Recall mode", _MODES)

    current_ctx_tokens = _pref(ulak_host, cfg, "contextTokens")
    ...
    _menu("Dialectic cadence",
          ...
          "Recommended: 1-5.")
    new_dialectic = _prompt("Dialectic cadence", default=str(_pref(ulak_host, cfg, "dialecticCadence") or "2"))
    if (val := _first_parsed([new_dialectic], int, None)) is None:
        ulak_host["dialecticCadence"] = 2
    elif val >= 1:
        ulak_host["dialecticCadence"] = val
    ...
    _choice_step(ulak_host, "dialecticReasoningLevel", _pref(ulak_host, cfg, "dialecticReasoningLevel") or "low",
                 "Reasoning level", REASONING_LEVELS, "low")

    _menu("Session strategy", *(f"{s:<13} -- {desc}" for s, desc in _STRATEGIES.items()))
    _choice_step(ulak_host, "sessionStrategy", _pref(ulak_host, cfg, "sessionStrategy", "per-session"),
...
def cmd_setup(args) -> None:
    ...
    print(f"\nHoncho memory setup\n{RULE}\n  Honcho gives Ulak persistent cross-session memory.\n  Config: {write_path}")
    ...
    ulak_host = cfg.setdefault("hosts", {}).setdefault(_host_key(), {})
    _migrate_pin_key(cfg)  # canonicalize legacy pinPeerName before detection/writes
    _migrate_pin_key(ulak_host)
    ...
    if is_local:
        _setup_local_auth(cfg, ulak_host)
    elif not _setup_cloud_auth(cfg, ulak_host, write_path):
        return

    # --- 3. Identity ---
    current_peer = ulak_host.get("peerName") or cfg.get("peerName", "")
    for key, label, default in (
        ("peerName", "Your name (user peer)", current_peer or os.getenv("USER", "user")),
        ("aiPeer", "AI peer name", _pref(ulak_host, cfg, "aiPeer", "ulak")),
        ("workspace", "Workspace ID", _pref(ulak_host, cfg, "workspace", "ulak")),
    ):
        if new := _prompt(label, default=default):
            ulak_host[key] = new

    _setup_identity_mapping(cfg, ulak_host, current_peer)

    _setup_tuning(cfg, ulak_host)
    ulak_host["enabled"] = True
    ulak_host.setdefault("saveMessages", True)
    _write_config(cfg)
    ...
    try:  # auto-enable Honcho as memory provider in config.yaml
        ...
        ulak_config = load_config()
        ulak_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(ulak_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}\n  Run: ulak config set memory.provider honcho")
  ...
  Other commands:
    ulak honcho status     -- show full config
    ulak honcho mode       -- change recall/observation mode
    ulak honcho tokens     -- tune context and dialectic budgets
    ulak honcho peer       -- update peer names
    ulak honcho map <name> -- map this directory to a session name
""")
...
def _active_profile_name() -> str:
    """Active Ulak profile name (respects --target-profile override)."""
    if _profile_override:
...
def _all_profile_host_configs() -> list[tuple[str, str, dict]]:
    ...
    cfg = _read_config()
    # _host_block (not hosts.get) keeps legacy dot-form keys ("ulak.work") readable.
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: ulak honcho setup\n")
        return
...
Honcho peers
{RULE}
  User peer:   {_pref(ulak, cfg, 'peerName') or '(not set)'}
    Your identity in Honcho. Messages you send build this peer's card.
  AI peer:     {_pref(ulak, cfg, 'aiPeer') or _host_key()}
    Ulak' identity in Honcho. Seed with 'ulak honcho identity <file>'.
    Dialectic calls ask this peer questions to warm session context.

  Dialectic reasoning:  {_pref(ulak, cfg, 'dialecticReasoningLevel') or 'low'}  ({', '.join(REASONING_LEVELS)})
  Dialectic cap:        {_pref(ulak, cfg, 'dialecticMaxChars') or 600} chars
""")
...
def _show_or_set_choice(args, *, attr: str, key: str, noun: str, title: str, choices: dict,
    ...
    if value is None:
        ...
        return print(f"\n  Set with: ulak honcho {attr} [{'|'.join(choices)}]\n")
...
{RULE}
  ...
  Context     {_pref(ulak, cfg, 'contextTokens') or '(Honcho default)'} tokens
  ...
  Dialectic   {_pref(ulak, cfg, 'dialecticMaxChars') or 600} chars, reasoning: {_pref(ulak, cfg, 'dialecticReasoningLevel') or 'low'}
    AI-to-AI inference. Ulak asks Honcho's AI peer a question
  ...
  Set with: ulak honcho tokens [--context N] [--dialectic N]
""")
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        return print("  No API key configured. Run 'ulak honcho setup' first.\n")
    ...
    if getattr(args, "show", False):
        ...
        print(ai_rep.get("representation") or ai_rep.get("card")
              or "  No representation built yet.\n  Run 'ulak honcho identity <file>' to seed one.")
        print()
        return
...
{RULE}
  ...
  AI peer:   {hcfg.ai_peer}
    ...
    ulak honcho identity --show        — show both peer representations
    ulak honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt
""")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Ulak + Honcho."""
    ...
    print("\nHoncho migration: OpenClaw native memory → Ulak\n" + "─" * 50)
    print("""
...
""")
    if has_key:
        ...
        print("""  Honcho is a cloud memory service that gives Ulak persistent memory
  across sessions. You need an API key to use it.

  1. Get your API key at https://app.honcho.dev
  2. Run:  ulak honcho setup
     Paste the key when prompted.
""")
        if _yes(_prompt("  Run 'ulak honcho setup' now?", default="y")):
            cmd_setup(args)
            cfg = _read_config()
            has_key = bool(cfg.get("apiKey", ""))
        else:
            print("\n  Run 'ulak honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  No OpenClaw native memory files found in cwd or ~/.openclaw/.\n"
              ...
              "  or seed them manually:  ulak honcho identity <path/to/file>")
...
""")
    if user_files:
        ...
        print("""
  These are picked up automatically the first time you run 'ulak'
  with Honcho configured and no prior session history.
  (Ulak calls migrate_memory_files() on first session init.)

  If you want to migrate them now without starting a session:""")
        print("    ulak honcho migrate  — this step handles it interactively\n" * len(user_files), end="")
        if has_key:
            _offer("  Upload user memory files to Honcho now?", _migrate_upload, user_files)
        else:
            print("  Run 'ulak honcho setup' first, then re-run this step.")
    else:
...
Step 4  Seed AI identity files → Honcho AI peer
  ...
  In Ulak, they are seeded once into Honcho's AI peer through the
...
""")
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'ulak honcho setup' first, then seed manually:")
            print("\n".join(f"    ulak honcho identity {f}" for f in agent_files))
    else:
        print("  No agent identity files detected.\n  To seed manually:  ulak honcho identity <path/to/SOUL.md>")
...
Step 5  What changes vs. OpenClaw native memory
  ...
  Session naming
    ...
    Ulak:   per-session by default — each run gets its own session
              Map a custom name:  ulak honcho map <session-name>

Step 6  Next steps
""")
    if not has_key:
        print("  1. ulak honcho setup              — configure API key (required)\n"
              "  2. ulak honcho migrate            — re-run this walkthrough")
    else:
        print("""  1. ulak honcho status             — verify Honcho connection
  2. ulak                           — start a session
     (user memory files auto-uploaded on first turn if not done above)
  3. ulak honcho identity --show    — verify AI peer representation
  4. ulak honcho tokens             — tune context and dialectic budgets
  5. ulak honcho mode               — view or change memory mode""")
    print()
...
_SUBCOMMANDS = (
    ("setup", "Initial Honcho setup (redirects to ulak memory setup)", None, ()),
    ...
    ("migrate", "Step-by-step migration guide from openclaw-honcho to Ulak Honcho", cmd_migrate, ()),
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":  # honcho setup goes through the unified memory-provider path
        print("\n  Honcho is configured via the memory provider system.\n  Running 'ulak memory setup'...\n")
        from ulak_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``ulak honcho`` argparse subcommand tree on the ``ulak honcho`` parser."""
