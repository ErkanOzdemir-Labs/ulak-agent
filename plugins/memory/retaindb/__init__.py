...
class _Client:
    ...
    def _headers(self, path: str, json_body: bool = True) -> dict:
        ...
        return {"Authorization": f"Bearer {token}", "x-sdk-runtime": "ulak-plugin",
...
class RetainDBMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._user_id, self._session_id, self._agent_id = "default", "", "ulak"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Project: RETAINDB_PROJECT > config.yaml > ulak-<profile> > "default" (API auto-creates "default").
        project = os.environ.get("RETAINDB_PROJECT") or cfg.get("project")
        if not project:
            profile_name = os.path.basename(str(kwargs.get("ulak_home", "")))
            project = f"ulak-{profile_name}" if profile_name not in {"", ".ulak"} else "default"
        ...
        self._agent_id = kwargs.get("agent_id", "ulak") or "ulak"
