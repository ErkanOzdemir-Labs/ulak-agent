"""ulak-memory-store — holographic memory plugin (MemoryProvider): structured fact storage with entity
resolution, trust scoring, and HRR-based compositional retrieval. Original plugin by dusterbloom (PR #2351).
Config in $ULAK_HOME/config.yaml under plugins.ulak-memory-store: db_path ($ULAK_HOME/memory_store.db),
...
def _load_plugin_config() -> dict:
    try:
        from ulak_cli.config import load_config_readonly  # canonical: managed-scope overlay + ${VAR} expansion
        return cfg_get(load_config_readonly(), "plugins", "ulak-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, ulak_home):
        """Write config to config.yaml under plugins.ulak-memory-store."""
        config_path = Path(ulak_home) / "config.yaml"
        try:
            import yaml
            from ulak_cli.config import read_user_config_raw  # raw read: merged defaults must not be persisted
            existing = read_user_config_raw(config_path)
            existing.setdefault("plugins", {})["ulak-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        return [
            {"key": "db_path", "description": "SQLite database path", "default": f"{display_ulak_home()}/memory_store.db"},
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _ulak_home = str(get_ulak_home())
        db_path = self._config.get("db_path", _ulak_home + "/memory_store.db")
        if isinstance(db_path, str):  # expand $ULAK_HOME so paths resolve to the active profile
            db_path = db_path.replace("$ULAK_HOME", _ulak_home).replace("${ULAK_HOME}", _ulak_home)
