...
after 14 days; empty dirs under ULAK_HOME always. Prompt-only: research
(keep 10 newest, > 30 days), chrome-profile > 14 days, any file > 500 MB.
Scope: strictly ULAK_HOME and /tmp/ulak-*; never ~/.ulak/logs/ or system dirs.
"""
...
def _state_file(name: str) -> Path:
    """``$ULAK_HOME/disk-cleanup/<name>`` — deliberately outside ``$ULAK_HOME/logs/``."""
    return get_ulak_home() / "disk-cleanup" / name


def is_safe_path(path: Path) -> bool:
    """Accept only paths under ULAK_HOME or ``/tmp/ulak-*`` (rejects /mnt/c etc.)."""
    with contextlib.suppress(ValueError, OSError):
        path.resolve().relative_to(get_ulak_home())
        return True
    parts = path.parts
    return len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("ulak-")
...
# Top-level ULAK_HOME dirs whose empty subdirs are never swept (last row: user project trees).
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    "logs", "memories", "sessions", "cron", "cronjobs",
    "cache", "skills", "plugins", "disk-cleanup", "optional-skills",
    "ulak-agent", "backups", "profiles", ".worktrees",
    "patches", "projects", "skins", "themes", "contributors"})

_EMPTY_DIR_SWEEP_PRUNE_DIRS = frozenset({
    ".git", "node_modules", "venv", ".venv", "site-packages", "__pycache__"})

# Top-level ULAK_HOME entries guess_category() never auto-tracks: state, logs, memory,
...
@functools.lru_cache(maxsize=1)  # built lazily so ULAK_HOME resolves once
def _protected_cron_paths() -> frozenset:
    ...
    return frozenset(str(x) for parent in ("cron", "cronjobs") for base in (get_ulak_home() / parent,)
                     for x in (base, base / "output", base / "jobs.json", base / ".tick.lock"))


# Paths under $ULAK_HOME that must NEVER be deleted by quick(), regardless of what the stored category
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ULAK_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    for item, p, age in _live_items(load_tracked(), datetime.now(timezone.utc), log_stale=True):
        ...
        if err is None:
            ...
            errors.append(err)
            new_tracked.append(item)
    empty_removed = _sweep_empty_dirs(get_ulak_home())
...
def _sweep_empty_dirs(ulak_home: Path) -> int:
    """Remove empty dirs under ULAK_HOME without recursing into durable/heavy trees (a full
    rglob over a checkout+venv under ULAK_HOME can stall the gateway loop for minutes).
    Iterative post-order so parents emptied by child removal are caught."""
    removed = 0
    stack: List[Tuple[Path, bool]] = [
        (top, False) for top in _subdirs(ulak_home, _EMPTY_DIR_PROTECTED_TOP_LEVEL | _EMPTY_DIR_SWEEP_PRUNE_DIRS)]
    while stack:
...
def guess_category(path: Path) -> Optional[str]:
    ...
    if not is_safe_path(path):
        return None
    with contextlib.suppress(ValueError):  # not under ULAK_HOME (/tmp/ulak-*) — name rules only
        rel = path.resolve().relative_to(get_ulak_home())
