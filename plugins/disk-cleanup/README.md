...
Auto-tracks and cleans up ephemeral files created during Ulak Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ULAK_HOME` and `/tmp/ulak-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ULAK_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ULAK_HOME` or `/tmp/ulak-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ULAK_HOME/disk-cleanup/` is itself excluded
- `$ULAK_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
