...
A cron job that restarts/stops the gateway from inside the gateway (``ulak gateway restart``,
``launchctl kickstart ai.ulak.gateway``, ``systemctl restart ulak-gateway``) kills the process,
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: destructive `ulak gateway` ops. `start` is excluded: starting from inside a
    # gateway is benign and a job may legitimately start a sibling profile. The lookbehind keeps
    # `ulak` from being a path component or word tail (`/docs/ulak gateway restart-notes.md`)
    # while every real command position (text start, whitespace, `;`/`&`/`|`, `$(`, backtick,
    # U+FFFD) still matches.
    # See #77173.
    r"(?:(?<![/\w.\-])ulak\s+gateway\s+(?:restart|stop|uninstall)\b)"
    # Branch B: launchctl ops anchored on a ulak-gateway label so unrelated ulak services stay
    ...
    # -l ai.ulak.gateway-<suffix> -- <helper-script>` (or `launchctl bootstrap gui/<uid> <plist>`) creates
    ...
    # Branch C: systemctl ops on a ulak-gateway unit.
...
# split across continuation lines (e.g. `launchctl submit \` / `  -l ai.ulak.gateway-... \` / `  -- ...`,
...
# Branch A2: `ulak -p <profile> gateway restart|stop` (also `--profile <name>` /
...
# `ulak -p venus gateway restart` operates on a sibling profile's gateway and is a legitimate fleet
...
# `$label` next to the verb. These verbs act on an EXISTING job, so the ulak-gateway label anchor
...
_ULAK_GATEWAY_LABEL_RE = re.compile(r"(?i)\bulak[.\-]?gateway\b")
...
def _current_profile_name() -> Optional[str]:
    """Profile running the guard: ``ULAK_PROFILE_NAME``/``ULAK_PROFILE`` env first, then
    ``ulak_cli.profiles.get_active_profile_name`` (from ``ULAK_HOME``); ``None`` if neither."""
    for env_name in ("ULAK_PROFILE_NAME", "ULAK_PROFILE"):
...
def _contains_launchctl_gateway_lifecycle(normalized_text: str) -> bool:
    ...
    return bool(_LAUNCHCTL_LIFECYCLE_VERBS_RE.search(normalized_text)) and bool(
        _ULAK_GATEWAY_LABEL_RE.search(normalized_text)
    )
...
def contains_gateway_lifecycle_command(text: str) -> bool:
    ...
    ``ulak``/``gateway`` token) — this function is the single choke point
    ...
    # <<'EOF'`) are masked before scanning (#88336): a runbook line "a human can run: ulak gateway
    ...
    # Profile-flag form (#78028): `ulak -p <profile> gateway restart|stop` bypasses Branch A because the
    # selector sits between `ulak` and `gateway`. It is only the same foot-gun when the named profile IS
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    ``<ULAK_HOME>/scripts/``) so the guard scans the file that will actually run."""
    ...
    if raw.is_absolute():
        return raw
    try:
        return get_ulak_home() / "scripts" / raw
    except (RuntimeError, OSError):
        # get_ulak_home() falls back to Path.home(), which raises when neither ULAK_HOME nor
...
def check_gateway_lifecycle(prompt: Optional[str], script: Optional[str] = None) -> None:
    ...
    if script:
        ...
        if resolved_script is not None and _on_cloud_path(resolved_script):
            raise GatewayLifecycleBlocked(
                ...
                "(e.g. ~/.ulak/scripts/) and recreate the job."
            )
    ...
    if python_script:
        ...
        # The direct command regex below still scans the full text, so a literal `ulak gateway restart`
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `ulak gateway restart` from a shell outside "
