...
def _open_continuable_cron_thread(job: dict, adapter, chat_id: str, loop) -> Optional[str]:
    ...
    if not callable(create_thread) or loop is None:
        return None
    thread_name = f"Ulak — {job.get('name') or job.get('id', 'cron')}"
    try:
...
def cron_delivery_targets() -> list[dict]:
    ...
    try:
        from ulak_cli.profiles import list_profile_names
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str) -> Optional[str]:
    ...
    ``ulak [-p <profile>] chat --in ~ -c "Bot Chat" --create-if-missing -Q --query-file`` — the
    ...
    if ulak_bin:
        ...
        try:
            ...
            found = _ilu.find_spec("ulak_cli") is not None
        except Exception:
            found = False
        if not found:
            return "bot-chat delivery failed: ulak CLI not resolvable"
        argv = [sys.executable, "-m", "ulak_cli.main"]
    ...
    if profile:
        argv += ["-p", profile]
        # -p owns profile resolution; this scheduler's ULAK_HOME must not shadow it.
        env.pop("ULAK_HOME", None)
    ...
    try:
        with tempfile.NamedTemporaryFile(
            "w", encoding="utf-8", suffix=".txt", prefix="ulak-cron-botchat-", delete=False,
        ) as fh:
            fh.write(message)
            query_file = fh.name
...
def _deliver_result(
    ...
    # job's own attempt: a worker's script may dispatch another job in-process (`ulak cron run`),
    # and that nested delivery must not be keyed under the outer execution id.
    external_execution = os.environ.get("_ULAK_CRON_EXTERNAL_WORKER", "")
    ...
    # persisted as ``last_delivery_unverified`` so `ulak cron list` shows it.
    ...
    # at boot; standalone runs (`ulak cron run`) did not, silently dropping files. Idempotent.
