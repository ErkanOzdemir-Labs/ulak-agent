...
# Sensitive write targets, matched via ~ / $HOME / $ULAK_HOME spellings. The resolved absolute
# home is folded into these forms at detection time by _normalize_command_for_detection(), so no
# import-time path snapshot (stale once ULAK_HOME is set after import) lives in the patterns.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_ULAK_ENV_PATH = (
    r'(?:~\/\.ulak/|(?:\$home|\$\{home\})/\.ulak/|(?:\$ulak_home|\$\{ulak_home\})/)' r'\.env\b'
)
# ~/.ulak/config.yaml IS the security policy (approvals.mode, yolo, allowlist) and the config cache is mtime-keyed,
# so a write takes effect mid-session. Terminal-side coverage (sed -i, tee, >, cp) pairs the file_tools deny.
_ULAK_CONFIG_PATH = (
    r'(?:~\/\.ulak/|(?:\$home|\$\{home\})/\.ulak/|(?:\$ulak_home|\$\{ulak_home\})/)' r'config\.yaml\b'
)
...
_SENSITIVE_WRITE_TARGET = (
    rf'(?:{_SYSTEM_CONFIG_PATH}|/dev/sd|{_SSH_SENSITIVE_PATH}|{_ULAK_ENV_PATH}|{_ULAK_CONFIG_PATH}|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]ulak[^\n]*\.env\b', "access to Ulak secrets (Windows path)"),
    # ── end of Windows tier
    ...
    # between `ulak` and `gateway` (`ulak -p ade gateway restart`) are allowed so a profile flag can't slip past.
    (r'\bulak\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart ulak gateway (kills running agents)"),
    (r'\bulak\s+update\b', "ulak update (restarts gateway, kills running agents)"),
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart ulak-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart ulak-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(ulak|gateway|cli\.py)\b', "kill ulak/gateway process (self-termination)"),
    ...
    # launchctl-driven gateway stop/restart on macOS (label `ai.ulak.gateway`). Two independent lookaheads, NOT a
    # sequential match: a for-loop building the label from a list defined EARLIER (`for item in 'ai.ulak...'; do
    # launchctl bootout "$label"`) never has "ulak" after the verb, and that slipped past and restarted 4 gateways
    # with zero approval. Erring broad is correct for an approval gate: an extra prompt is cheap.
    (r'(?=[\s\S]*\blaunchctl\s+(?:stop|kickstart|bootout|unload|kill|disable|remove)\b)(?=[\s\S]*\b(?:ulak|ai\.ulak)\b)', "stop/restart ulak launchd service (kills running agents)"),
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a credential/SSH/shell-rc/Ulak file (key implant, login-time
    ...
    # ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files, ~/.ulak/config.yaml/.env), but cp/mv/install was
    ...
    # sed -i on Ulak config/.env bypasses the redirection/tee rules; pairs the file_tools
    # write_file/patch deny so the terminal side is not an open door.
    # In-place edit of a Ulak-managed security file (~/.ulak/config.yaml or .env). sed -i bypasses the
    # redirection/tee patterns above because it mutates the file directly. See #14639.
    (rf'\bsed\s+-[^\s]*i.*(?:{_ULAK_CONFIG_PATH}|{_ULAK_ENV_PATH})', "in-place edit of Ulak config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_ULAK_CONFIG_PATH}|{_ULAK_ENV_PATH})', "in-place edit of Ulak config/env (long flag)"),
    ...
    # coverage from #14639.
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_ULAK_CONFIG_PATH}|{_ULAK_ENV_PATH})', "in-place edit of Ulak config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    command = re.sub(r'\\\r?\n', '', command)
    # Fold absolute user/Ulak home prefixes to ~/ and ~/.ulak/ so the static patterns catch /home/alice/.bashrc
    # and C:\Users\alice\.bashrc. Resolved at detection time (not import time) so it tracks HOME/ULAK_HOME set
    # later. MUST run before the backslash strip (which would dissolve C:\Users\alice to C:Usersalice). Ulak home
    # first: on Windows it nests under the user home, and folding the user home first would eat the prefix it needs.
    command = _rewrite_resolved_ulak_home(command)
...
def _rewrite_resolved_ulak_home(command: str) -> str:
    """Resolved ULAK_HOME (and its realpath) -> ``~/.ulak/`` so the _ULAK_CONFIG_PATH /
    _ULAK_ENV_PATH rules match Docker/gateway deployments that spell the absolute path."""
    try:
        ...
        return command
    return _fold_home_prefixes(command, paths, "~/.ulak")
...
_GATEWAY_LIFECYCLE_SPLICE_DESCRIPTION = "stop/restart ulak gateway via shell-spliced verb (kills running agents)"
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Ulak ad-hoc temp script."""
    try:
    ...
    return (
        ...
        and re.fullmatch(r"ulak-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
    )
...
def _is_shell_token_spliced_gateway_lifecycle(command: str) -> bool:
    ...
    verb is an ARGUMENT, so ``launchctl kick"start" -k gui/501/ai.ulak.gateway`` auto-approved.
    Delegates to ``cron.lifecycle_guard`` (shlex-tokenized, anchored on a ulak-gateway
