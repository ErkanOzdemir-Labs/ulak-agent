...
# ladders, parked-server self-probes and repeated `ulak mcp test` runs re-run the preflight
...
# The cache is also persisted under the Ulak home so separate processes and gateway
...
def _disk_cache_path() -> Optional[Path]:
    ...
    Uses ``ulak_constants.get_ulak_home()`` so the cache follows the
    active profile and is isolated across Ulak homes. The cache directory
    is created on demand. Returns ``None`` when Ulak home cannot be
...
def _query_osv(package: str, ecosystem: str, version: Optional[str] = None) -> list:
    ...
    req = urllib.request.Request(
        ...
        headers={"Content-Type": "application/json", "User-Agent": "ulak-agent-osv-check/1.0"},
        method="POST")
