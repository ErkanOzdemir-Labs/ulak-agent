...
refresh; this module supplies ``UlakTokenStorage`` (on-disk persistence), the localhost callback
listener and ``build_oauth_auth()`` (legacy entry point). client_id is Ulak' Client ID Metadata
...
from ulak_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__ulak_user_skipped__"


def _get_token_dir(ulak_home: str | Path | None = None) -> Path:
    """``ULAK_HOME/mcp-tokens/`` — per-profile token directory."""
    from ulak_constants import get_ulak_home

    return Path(ulak_home if ulak_home is not None else get_ulak_home()) / "mcp-tokens"
...
def _cached_client_info(storage: "UlakTokenStorage | None") -> dict | None:
...
def _cached_redirect(storage: "UlakTokenStorage | None") -> "tuple[str | None, int | None]":
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    ``lead`` is the boundary-specific first sentence; this helper appends the shared, actionable ``ulak
    mcp login`` next-step so the guidance wording lives in one place across every non-interactive OAuth
    boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} Run `ulak mcp login <server>` interactively to (re)authorize, then restart or reload the gateway."
        )
...
def _write_json(path: Path, data: dict) -> None:
    ...
    # secure_parent_dir refuses to chmod /, top-level dirs, or the ulak-agent install tree (#25821,
    # #93050).
    ...
    # ulak-agent install tree (#25821, #93050).
...
class UlakTokenStorage:
    """Persist OAuth state as ``ULAK_HOME/mcp-tokens/<server_name>`` + ``.json`` (tokens),
    ``.client.json`` (client info), ``.meta.json`` (server metadata), ``.cimd-off`` (CIMD refused)."""

    def __init__(self, server_name: str, *, ulak_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._ulak_home = Path(ulak_home) if ulak_home is not None else None

    def _path(self, suffix: str) -> Path:
        return _get_token_dir(self._ulak_home) / f"{self._server_name}{suffix}"
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._ulak_home)
...
def _make_callback_handler() -> tuple[type, dict]:
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = ("<h2>Authorization Successful</h2><p>You can close this tab and return to Ulak.</p>" if parsed["code"]
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `ulak mcp login <server>` later to authenticate, "
...
_SSH_HINT_LOOPBACK = (
    ...
    "\n"
    "  See: https://ulak-agent.github.com/ErkanOzdemir-Labs/docs/guides/oauth-over-ssh\n")
...
def _callback_outcome(result: dict, cimd_url: str | None):
    ...
    if result["auth_code"] is None:
        hint = (
            ...
            f"server rejected Ulak' Client ID Metadata Document ({cimd_url}); set ``cimd: false`` under that "
...
UlakOAuthClientProvider: Any = None


def remove_oauth_tokens(server_name: str, *, ulak_home: str | Path | None = None) -> None:
    """Delete stored OAuth tokens and client info for a server."""
    UlakTokenStorage(server_name, ulak_home=ulak_home).remove()
    logger.info("OAuth tokens removed for '%s'", server_name)


# CIMD (OAuth Client ID Metadata Documents): the client_id IS an HTTPS URL the server fetches for our
# name/logo/redirect URIs, replacing per-install DCR. The SDK does the protocol; Ulak only decides
# eligibility. Published from ``website/static/oauth/client-metadata.json``; the github.io origin is
# deliberate — servers MUST NOT follow redirects when fetching it, and ulak-agent.github.com/ErkanOzdemir-Labs/docs/* 301s here.
_CIMD_CLIENT_METADATA_URL = "https://erkanozdemir-labs.github.io/ulak-agent/docs/oauth/client-metadata.json"
...
def _server_declined_cimd(storage: "UlakTokenStorage | None") -> bool:
    """True when cached metadata shows this server doesn't advertise CIMD. The SDK decides CIMD vs DCR
    in its 401 branch — after Ulak must fix the redirect URI — so cached metadata closes the gap;
...
def _maybe_use_cimd(cfg: dict, storage: "UlakTokenStorage | None" = None) -> "tuple[str, int] | None":
...
def _configure_callback_port(cfg: dict, storage: "UlakTokenStorage | None" = None) -> int:
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    metadata_kwargs: dict[str, Any] = {
        "client_name": cfg.get("client_name", "Ulak Agent"),
...
def _invalidate_tokens_on_client_change(
    storage: "UlakTokenStorage", new_client_id: str, new_client_secret: str | None) -> None:
    ...
    if removed:
        logger.warning(
            ...
            "the previous client. Re-authorize with: ulak mcp login %s",
            storage._server_name, old_client_id, new_client_id, storage._server_name)


def _maybe_preregister_client(storage: "UlakTokenStorage", cfg: dict, client_metadata: "OAuthClientMetadata") -> None:
...
def humanize_oauth_registration_error(
    ...
    when the user overrode it or an older Ulak is running."""
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(server_name: str, server_url: str, oauth_config: dict | None = None) -> "OAuthClientProvider | None":
    ...
    global UlakOAuthClientProvider
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"requires browser authorization. Run `ulak mcp login {server_name}` interactively first to complete "
            "initial authorization, then cached tokens will be reused.")
    kwargs = build_provider_kwargs(cfg, storage, ssh_proxy_hint=True)
    if UlakOAuthClientProvider is None:
        from tools.mcp_oauth_provider import UlakProviderMixin

        UlakOAuthClientProvider = type("UlakOAuthClientProvider", (UlakProviderMixin, _sdk_class("OAuthClientProvider")), {
            "__doc__": "SDK provider plus Ulak' token-endpoint fixes (see ``UlakProviderMixin``).",
            "__module__": __name__, "_ulak_logger": logger})
    return UlakOAuthClientProvider(server_url=server_url, **kwargs)
