...
The LLM writes a Python script that calls Ulak tools via RPC, collapsing
...
# Loopback TCP replaces AF_UNIX on Windows, so execute_code runs on every platform Ulak does.
...
def _spill_full_stdout(stdout_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_ulak_dir("cache/exec", "exec_spill")
...
def _missing_ulak_tools_import_hint(m, enabled_tools) -> str:
    missing = m.group(1)
...
# these as the top execute_code failure classes: ulak_tools import misuse, importing the built-in
# helpers, treating tool results as strings, importing third-party packages absent from the sandbox.
_FAILURE_HINT_RULES = (
    (r"cannot import name '(\w+)' from 'ulak_tools'", _missing_ulak_tools_import_hint),
...
def generate_ulak_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """Source of the ulak_tools.py stub module for SANDBOX_ALLOWED_TOOLS ∩ *enabled_tools*.
...
"""Auto-generated Ulak tools RPC stubs."""
...
def _connect():
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["ULAK_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("ULAK_RPC_TOKEN", ""),
    }) + "\\n"
    ...
    # re-accepts (ULAK_RPC_PERSISTENT=1); retry once on a fresh socket.
    _attempts = 2 if os.environ.get("ULAK_RPC_PERSISTENT") == "1" else 1
    with _call_lock:
...
"""Auto-generated Ulak tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("ULAK_RPC_DIR") or os.path.join(tempfile.gettempdir(), "ulak_rpc")
_seq = 0
...
def _run_remote_per_call(env, env_type: str, code: str, effective_task_id: str,
    ...
    """Per-call script ship: stage ulak_tools.py + script.py in a fresh remote sandbox dir,
    serve file-RPC from a polling thread, run, clean up."""
    sandbox_dir = f"{_env_temp_dir(env)}/ulak_exec_{uuid.uuid4().hex[:12]}"
    ...
    try:
        ...
        _ship_file_to_remote(env, f"{sandbox_dir}/ulak_tools.py",
                             generate_ulak_tools_module(list(sandbox_tools), transport="file"))
        ...
        rpc_thread.start()
        env_prefix = (f"ULAK_RPC_DIR={quoted_rpc_dir} ULAK_RPC_TOKEN={shlex.quote(rpc_token)} "
                      "PYTHONDONTWRITEBYTECODE=1")
        tz = os.getenv("ULAK_TIMEZONE", "").strip()
        if tz:
...
def _execute_remote(code: str, task_id: Optional[str], enabled_tools: Optional[List[str]],
    ...
    try:
        ...
        try:
            # --- Session-kernel path (ulak-agent#96873) ------------------- Same always-on model as
...
def execute_code(
    ...
    with RPC access to a subset of Ulak tools; returns the JSON result string. "Sandbox" means
    ...
    # execute_code is a straight bypass — the terminal() path refuses `launchctl bootout ai.ulak.gateway`,
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    if mode == "strict":
        cwd_note = (
            ...
            "(os.path.expanduser('~/.ulak/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
            ...
            "Ulak's own python (the common case — stdlib plus Ulak's "
    ...
    description = (
        "Run Python that calls Ulak tools programmatically. Use when you "
        ...
        f"Available via `from ulak_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "parameters": {
            ...
            "properties": {
                "code": {"type": "string", "description": (
                    ...
                    f"`from ulak_tools import {import_str}` "
