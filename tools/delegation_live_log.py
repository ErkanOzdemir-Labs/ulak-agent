...
One append-only log per child under ``<ulak_home>/cache/delegation/live/
...
def live_transcript_root() -> Path:
    """Root directory for live transcripts (profile-safe, never ~/.ulak)."""
    from ulak_constants import get_ulak_dir
    return get_ulak_dir("cache/delegation", "delegation_cache") / "live"
