#!/usr/bin/env python3
"""One parent-death supervisor per Ulak process, shared by all stdio MCP servers.

Why this exists
---------------
When Ulak dies without running its cleanup path (SIGKILL, OOM killer, a hard
crash), stdio MCP servers it spawned are reparented to init and keep running
forever.  macOS has no ``PR_SET_PDEATHSIG``, so something has to outlive Ulak
and reap them.

This module is deliberately standard-library-only and must not import anything
from ``tools/``: it runs after Ulak may already be dead, and pulling in
...
with a single supervisor per Ulak process:

* **Death detection is a blocking read on a pipe.**  Ulak holds the only write
  end.  When Ulak dies -- by any means, including SIGKILL -- the write end
...
Ulak never unregistered *is* the orphan set, so a clean Ulak shutdown --
...
an unrelated process owned by the same user.  If Ulak then dies ungracefully
...
Two things narrow the window.  Ulak prunes registrations whose group has no
...
exposure already exists in Ulak's own killpg-based orphan cleanup, which this
...
def _is_safe_target(pgid: int, *, own_pgid: int, parent_pgid: int) -> bool:
    ...
    Defensive only -- Ulak already filters non-MCP children before it
    ...
    Ulak process tree or the session.
    """
    if pgid <= 1:
        return False
...
def _serve(stream, *, own_pgid: int, parent_pgid: int) -> set[int]:
    ...
    stopped. Nothing in Ulak can produce that today, but this process is the
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Process group of the spawning Ulak process; never signalled.",
    )
