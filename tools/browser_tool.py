...
from ulak_cli.config import DEFAULT_CONFIG, cfg_get
...
# dependency could read every Ulak secret from process.env.
...
def _build_browser_env() -> dict:
    ...
    env = ulak_subprocess_env(inherit_credentials=False)
...
# than a concrete path (also compared in ulak_cli/tools_config.py and doctor.py).
...
_REAL_PROFILE_SESSION = "ulak-real-profile"
...
def _socket_safe_tmpdir() -> str:
    """Short temp dir for Unix sockets: macOS ``TMPDIR`` + ``agent-browser-ulak_…``
...
# Owner Ulak home per session: the janitor is one process-global thread, so each
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        if not cfg_get(read_raw_config(), "browser", "record_sessions", default=False):
            return
        recordings_dir = ulak_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    screenshots_dir = get_ulak_dir("cache/screenshots", "browser_screenshots")
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_ulak_home_override': ('ulak_constants', 'get_ulak_home_override'),
    'ulak_home_key': ('ulak_constants', 'ulak_home_key'),
    ...
    'node_tool_runnable': ('ulak_constants', 'node_tool_runnable'),
