...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to ULAK_KANBAN_TASK from the env "
    "(the task the dispatcher spawned you to work on)."
)

_DESC_BOARD = (
    "Kanban board slug to target. When omitted, the call resolves the "
    "active board the usual way: ULAK_KANBAN_DB env → "
    "ULAK_KANBAN_BOARD env → the 'current' symlink under the kanban "
...
KANBAN_ATTACH_URL_SCHEMA = _schema(
    "kanban_attach_url",
    (
        "Attach a file to a task by URL — Ulak downloads it server-side "
...
KANBAN_CREATE_SCHEMA = _schema(
    ...
    ),
    {
        ...
        "tenant": _prop("string", (
                ...
                "Defaults to ULAK_TENANT env if set."
        )),
