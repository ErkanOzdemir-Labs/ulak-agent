...
def _emergency_cleanup_all_sessions():
    ...
    ulak processes — every clean exit reaps accumulated orphans, not only
    ...
    # owned by other live ulak processes.
    _best_effort("Orphan reap on exit", _reap_orphaned_browser_sessions)


@contextlib.contextmanager
def _session_owner_scope(task_id: str):
    """Run under the Ulak home + secret scope owning ``task_id``'s session (no-op if unrecorded).
    ...
    home_token = set_ulak_home_override(owner_home)
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record this ulak PID in ``<socket_dir>/<session>.owner_pid`` so the orphan
...
def _reap_socket_dir(socket_dir: str, session_name: str, tracked_names: set) -> bool:
    ...
    A live ``owner_pid`` means another ulak process owns it — leave it UNLESS untracked
...
def _reap_orphaned_browser_sessions():
    """Kill agent-browser daemons whose owning ulak process is gone (an unclean exit loses
    ...
    for prefix in ("agent-browser-h_*", "agent-browser-cdp_*", "agent-browser-ulak_*",
...
def _update_session_activity(task_id: str):
    """Touch the activity timestamp and record the owning Ulak home on first sight (the
    ...
    with _bt._cleanup_lock:
        ...
        _bt._session_owner_homes.setdefault(task_id, str(get_ulak_home()))
...
def _cleanup_old_recordings(max_age_hours=72):
    ...
    try:
        recordings_dir = get_ulak_home() / "browser_recordings"
    except Exception as e:
