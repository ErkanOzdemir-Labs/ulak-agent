...
def get_env_value(name, default=None):
    ...
    try:
        from ulak_cli.config import get_env_value as _get_env_value
    except ImportError:
...
def _get_default_output_dir() -> str:
    ...
    return str(get_ulak_dir("cache/audio", "audio_cache"))
...
def _default_output_dir() -> str:
    ...
    ULAK_HOME and later scope requests to a different profile via
    ``ulak_constants.set_ulak_home_override()`` — a frozen module constant keeps writing synthesized
    ...
    re-resolve from the live profile-scoped ULAK_HOME on every call.
    """
...
def _load_tts_config() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug("ulak_cli.config not available, using default TTS config")
...
_BUILTIN_DISPATCH: Dict[str, tuple] = {
    ...
    "mistral": (lambda: _importable(_import_mistral_client), "Mistral Voxtral TTS", "_generate_mistral_tts",
                ...
                "Run `ulak setup` to install Mistral support."),
    ...
    "neutts": (lambda: _check_neutts_available(), "NeuTTS (local)", "_generate_neutts",
               ...
               "Run ulak setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."),
    "kittentts": (lambda: _importable(_import_kittentts), "KittenTTS (local, ~25MB)", "_generate_kittentts",
                  ...
                  "Run 'ulak setup tts' and choose KittenTTS, or install manually: "
    ...
    "piper": (lambda: _importable(_import_piper), "Piper (local)", "_generate_piper_tts",
              ...
              "Run 'ulak tools' and select Piper under TTS, or install manually: "
...
def _session_platform() -> tuple:
    ...
    platform = get_session_env("ULAK_SESSION_PLATFORM", "").lower()
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_ulak_home()}/audio_cache/<timestamp>.mp3"
            },
            "speed": {
                "type": "number",
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'ulak_xai_user_agent': ('tools.xai_http', 'ulak_xai_user_agent'),
