...
def _ulak_tags(fm: dict) -> Any:
    """``metadata.ulak.tags`` from parsed frontmatter, or ``[]`` (unvalidated type)."""
    metadata = fm.get("metadata", {})
    ulak_meta = metadata.get("ulak", {}) if isinstance(metadata, dict) else None
    return ulak_meta.get("tags", []) if isinstance(ulak_meta, dict) else []
