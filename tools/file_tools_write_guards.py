...
_ulak_config_resolved: str | None = None
_ulak_config_resolved_loaded = False
_real_ulak_home_cached: str | None = None
_real_ulak_home_loaded = False
...
def _ulak_home_real() -> str:
    from ulak_constants import get_ulak_home
    return os.path.realpath(str(get_ulak_home()))


def _get_ulak_config_resolved() -> str | None:
    """Return the resolved absolute path of the Ulak config file (cached)."""
    return _cached_lookup("_ulak_config_resolved", "_ulak_config_resolved_loaded", _config_path_resolved,
                          lambda: str(Path(_expand_tilde("~/.ulak/config.yaml")).resolve()))


def _get_real_ulak_home() -> str | None:
    """Return the realpath of the authoritative Ulak home (cached)."""
    return _cached_lookup("_real_ulak_home_cached", "_real_ulak_home_loaded", _ulak_home_real,
                          lambda: os.path.realpath(_expand_tilde("~/.ulak")))
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    ulak_config = _get_ulak_config_resolved()
    if ulak_config and ulak_config in candidates:
        return (
            f"Refusing to write to Ulak config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.ulak/config.yaml directly or use 'ulak config' instead.")
    return None
...
# vector (AGENTS.md / CLAUDE.md / SOUL.md / .cursorrules / project .ulak tree).
...
def _protected_instruction_config() -> tuple[bool, list[str]]:
    ...
    try:
        from ulak_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # ~/.ulak itself is governed by its own guards (config.yaml hard-block,
    # mirror guard, write_approval); this gate targets PROJECT-LOCAL files only.
    # Must run before the ``.ulak`` component rule, which would match the home.
    real_home = _get_real_ulak_home()
    ...
    for candidate in (normalized, resolved):
        ...
        if base_lower in _PROTECTED_INSTRUCTION_BASENAMES or any(
            ...
            return base
        # Project-local .ulak config dirs (<repo>/.ulak/config.yaml) steer
        # behavior too. Only the IMMEDIATE parent counts — matching any ancestor
        # would gate every write inside a checkout living under ~/.ulak.
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".ulak":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Ulak mirror prefix for persistent Docker file tools."""
    try:
        ...
        if env is not None:
            ...
            return "/root/.ulak" if persistent_docker else None
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    Ulak state (a write the host never reads). Not profile isolation — that guard was
