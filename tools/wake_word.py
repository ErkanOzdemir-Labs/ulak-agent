"""Wake-word ("Hey Ulak") detection — hands-free session trigger.
...
# Minimum gap between two wake fires, so one "hey ulak" can't retrigger across
...
_DEFAULTS: Dict[str, Any] = {
    ...
    "provider": "openwakeword", "phrase": "hey ulak", "sensitivity": 0.6,
    "confirmation_frames": _DEFAULT_CONFIRMATION_FRAMES, "start_new_session": True,
}

# Bundled "hey ulak" model (tools/wakewords/) — the default; alias names resolve
# to it, not to an openWakeWord built-in.
_BUNDLED_MODEL_NAME = "hey_ulak"
_BUNDLED_MODEL_ALIASES = frozenset({"", "hey_ulak", "hey ulak", "ulak"})


def _bundled_wakeword_path(framework: str = "onnx") -> str:
    """Path to the shipped hey_ulak model (.onnx/.tflite) for ``framework``."""
...
def wake_phrase(cfg: Optional[Dict[str, Any]] = None) -> str:
    ...
    return str(_get(cfg, "phrase")) or "hey ulak"
...
def enrolled_profile_phrases() -> Dict[str, str]:
    ...
    with suppress(Exception):
        ...
        from ulak_cli.profiles import get_profile_dir, list_profiles
...
def silent_audio_hint(details: Dict[str, Any]) -> str:
    ...
    if sys.platform == "darwin":
        return ("Microphone delivers only silence. Grant the Ulak backend "
...
def check_wake_word_requirements(cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    ladder = (
        ...
        (bool(missing), lambda: (f"Wake word needs {missing} configured — run `ulak tools` "
...
# ── Process-wide singleton (mirrors ulak_cli.voice's continuous API) ──
...
def _lock_path() -> Path:
    ...
    return get_default_ulak_root() / "runtime" / "wake-word.lock"
