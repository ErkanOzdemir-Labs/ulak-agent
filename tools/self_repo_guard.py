...
def _block_message(operation: str, root: Path) -> str:
    ...
    scratch = (Path(ulak_home).expanduser() if ulak_home else Path.home() / ".ulak") / "scratch"
    return (
        f"Blocked: `{operation}` would rewrite Ulak's live source checkout "
        ...
        "is pushed. To change this checkout, stop Ulak, run the command externally, then restart "
        "Ulak.")
