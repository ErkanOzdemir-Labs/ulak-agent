...
_FOREGROUND_GUIDANCE = (
    (
        ...
        "notify_on_complete=true) so Ulak tracks the process, then run readiness "
...
def gateway_lifecycle_block(
    *,
    ...
    ``systemctl``/``launchctl``/``ulak gateway restart|stop|uninstall``
    targeting ulak-gateway would SIGTERM the gateway — and this very
    ...
    SUPERVISED-gateway probe, not the raw ``_ULAK_GATEWAY`` marker: that
    marker leaks into every process that merely imports gateway.run (ulak
    serve, CLI, web server), which must still be able to restart the gateway;
    an unsupervised foreground ``ulak gateway run`` has no KeepAlive to turn
    ...
    if lifecycle_scan_root_within_budget(command) and contains_launchctl_submit_command(command):
        return _blocked_json(
            ...
            "Use Ulak cron for one-shot delayed work, or install an "
    ...
    if contains_gateway_lifecycle_command_or_referenced_script(
        ...
        return _blocked_json(
            ...
            "to child processes). Run `ulak gateway restart` from a "
