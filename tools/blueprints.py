...
``metadata.ulak.blueprint`` (``schedule`` required; optional ``deliver`` [default "origin"],
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.ulak.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.ulak.blueprint`` is a mapping with a
    ...
    ulak = meta.get("ulak") if isinstance(meta, dict) else None
    blueprint = ulak.get("blueprint") if isinstance(ulak, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.ulak.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    ``metadata.ulak.blueprint`` block) ready for ``ulak skills publish``.
    ...
    frontmatter = {
        ...
        "metadata": {"ulak": {"tags": ["blueprint", "automation"], "blueprint": block}},
    }
