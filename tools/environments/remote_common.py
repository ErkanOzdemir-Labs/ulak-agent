...
from tools.environments.local_env_policy import _ULAK_PROVIDER_ENV_BLOCKLIST, _is_ulak_internal_secret


def load_ulak_env_vars() -> dict[str, str]:
    """``~/.ulak/.env`` values, or ``{}`` — a broken .env must not fail command execution."""
    try:
...
def resolve_passthrough_env(explicit_forward: Iterable[str] = (),
                            ulak_env_loader: Callable[[], dict[str, str]] = load_ulak_env_vars,
    ...
    is filtered through the Ulak provider-credential blocklist and the dynamic internal-secret
    ...
    try:
        ...
        pass
    implicit_forward = {k for k in passthrough_keys if not _is_ulak_internal_secret(k)}
    forward_keys = set(explicit_forward) | (implicit_forward - _ULAK_PROVIDER_ENV_BLOCKLIST)
    ulak_env = ulak_env_loader() if forward_keys else {}
    exec_env: dict[str, str] = {}
    unset_names: set[str] = set()
    for key in sorted(forward_keys):
        value = os.getenv(key) or ulak_env.get(key)
