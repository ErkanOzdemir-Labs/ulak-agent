"""Secret-scrub policy for Ulak child processes: pure data + predicates for which env
names are Ulak-managed credentials. The env *builders* applying it (``_make_run_env``,
``_sanitize_subprocess_env``, ``ulak_subprocess_env``) live in ``tools.environments.local``."""

import os

# Prefix a caller uses in ``extra_env`` to force a blocklisted var through.
_ULAK_PROVIDER_ENV_FORCE_PREFIX = "_ULAK_FORCE_"

# Ulak-managed AWS *inference* credentials for ``auth_type="aws_sdk"`` (Bedrock):
...
_STATIC_PROVIDER_ENV_BLOCKLIST = frozenset({
    ...
    "EMAIL_HOME_ADDRESS", "EMAIL_HOME_ADDRESS_NAME", "ULAK_DASHBOARD_SESSION_TOKEN",
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    # Claude Code install, not Ulak: stripping it made agent-spawned ``claude`` CLIs
    ...
    # It is set and owned by the user's Claude Code install (subscription OAuth), not a Ulak-managed
    # inference credential — Claude subscription auth is not a working Ulak provider path. It arrives via
...
_ULAK_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
# ``_sanitize_subprocess_env``); execute_code, ulak_subprocess_env, docker and
...
# ``buzz`` (the gateway's ``ULAK_SESSION_PLATFORM`` ContextVar; concurrency safe under a multi-session
...
# scope. Every other surface stays sealed — execute_code scrubbing, :func:`ulak_subprocess_env` (browser /
...
# the blocklist entirely because it is NOT a Ulak credential; these ARE Ulak-managed first-party
...
def _buzz_terminal_context_active() -> bool:
    ...
    try:
        ...
        return get_session_env("ULAK_SESSION_PLATFORM", "").strip().lower() == "buzz"
    except Exception:
        return False
...
# ANOTHER project's deps into the Ulak venv (still reachable via PATH, so stripping
# is safe); PYTHONHOME redirects a child interpreter's stdlib to the Ulak venv
# (version-mismatch crashes). PYTHONPATH is handled separately (Ulak-owned entries only).
...
# dependencies into the Ulak venv path instead of the project's own ``.venv`` — silently clobbering the
# Ulak environment (e.g. a project pinned to a different Python version overwrites it and breaks the
# gateway). PYTHONHOME is included because a gateway-inherited value redirects the standard-library search
# of ANY child interpreter — including unrelated system/venv Pythons — to the Ulak venv's stdlib, which
# crashes with version-mismatch errors before a child script even imports a package (#75018). Ulak itself
...
# _strip_ulak_owned_pythonpath() which removes only Ulak-owned entries, preserving user-set paths.
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX", "PYTHONHOME")


def _is_ulak_internal_secret(key: str) -> bool:
    """True for Ulak-internal secrets injected under *dynamic* names the static
...
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_ulak_internal_secret, but _ID has no secret suffix, so it must be
    # enumerated here to stay stripped on the inherit_credentials=True path.
    "GATEWAY_RELAY_ID", "GATEWAY_RELAY_SECRET", "GATEWAY_RELAY_DELIVERY_KEY",
    "HASS_TOKEN", "EMAIL_PASSWORD", "ULAK_DASHBOARD_SESSION_TOKEN",
