...
def _new_output_collector(proc, bounded_capture: bool) -> _BoundedOutputCollector:
    ...
    spill file under ``$ULAK_HOME/cache/terminal-output`` (created only on actual overflow;
    ...
    spill_path = None
    try:
        spill_dir = get_ulak_home() / "cache" / "terminal-output"
...
def _pipe_stdin(proc: subprocess.Popen, data: str) -> None:
    ...
    ``proc._ulak_stdin_errors`` (surfaced by ``_wait_for_process`` as ``stdin_error``) and
    stdin is still closed in ``finally`` so the child sees EOF instead of hanging.
    """
    errors: list[BaseException] = []
    proc._ulak_stdin_errors = errors
    ...
    proc._ulak_stdin_thread = thread
    thread.start()
