...
_FORCE_SYNC_ENV = "ULAK_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.ulak") -> list[tuple[str, str]]:
    """Enumerate all (host_path, remote_path) pairs to sync to a remote. Credential paths are
    remapped from the hardcoded /root/.ulak to *container_base* (remote home may differ)."""
    ...
    files = [
        (entry["host_path"], entry["container_path"].replace("/root/.ulak", container_base, 1))
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        per ``sync_interval`` unless *force* or ``ULAK_FORCE_FILE_SYNC=1``. Transactional:
    ...
    def sync_back(self, ulak_home: Path | None = None) -> None:
        """Pull remote changes back to the host: download the remote ``.ulak/`` as a tar and
        ...
        with self._transaction_lock:
            self._sync_back_transaction(ulak_home=ulak_home)

    def _sync_back_transaction(self, ulak_home: Path | None = None) -> None:
        ...
        # to avoid retry storms against an uninitialized remote .ulak/.
        if not self._pushed_hashes and not self._synced_files:
            logger.debug("sync_back: no prior push state — skipping")
            return

        lock_path = (ulak_home or get_ulak_home()) / ".sync.lock"
    ...
    def _sync_back_impl(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="ulak-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str, file_mapping: list[tuple[str, str]] | None = None, *,
        ...
        substitution (``/root/.ulak/skills/b.md`` -> ``~/.ulak/skills/b.md``)."""
