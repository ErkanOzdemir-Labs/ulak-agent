...
# Module-level binding: tests patch ``ssh._load_ulak_env_vars`` to fake the .env file.
...
class SSHEnvironment(BaseEnvironment):
    ...
    def __init__(self, host: str, user: str, cwd: str = "~",
        ...
        self.control_dir = Path(tempfile.gettempdir()) / "ulak-ssh"
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files(f"{self._remote_home}/.ulak"),
    ...
    def _ensure_remote_dirs(self) -> None:
        """Create base ~/.ulak directory tree on remote in one SSH call."""
        base = f"{self._remote_home}/.ulak"
    ...
    def _ssh_bulk_upload(self, files: list[tuple[str, str]]) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="ulak-ssh-bulk-") as staging:
    ...
    def _ssh_bulk_download(self, dest: Path) -> None:
        """Download remote .ulak/ as a tar archive."""
        # Tar from / with the full path so archive entries keep absolute paths
        # (home/user/.ulak/skills/f.py), matching _pushed_hashes keys.
        rel_base = f"{self._remote_home}/.ulak".lstrip("/")
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False, timeout: int = 120,
        ...
        values, unset_names = resolve_passthrough_env(ulak_env_loader=_load_ulak_env_vars)
