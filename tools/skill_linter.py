...
def _check_frontmatter(frontmatter: Dict[str, Any], skill_dir: Optional[Path]) -> Iterator[LintFinding]:
    ...
    ulak_meta = meta.get("ulak") if isinstance(meta, dict) else None
    if not isinstance(ulak_meta, dict):
        yield _warn("missing-metadata", "frontmatter is missing metadata.ulak.{tags, related_skills}.")
    elif "tags" not in ulak_meta:
        yield _warn("missing-metadata", "metadata.ulak.tags is missing.")
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("ulak", "agent", "ulak agent") and (
        author != "Ulak Agent"):
        yield _warn("author-caps", f"author '{author}' should be 'Ulak Agent' (proper caps) "
