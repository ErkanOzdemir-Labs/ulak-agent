...
# Ulak session id (observability identity, distinct from the gateway routing session_key), forwarded to approval
...
# os.environ["ULAK_INTERACTIVE"] races: one session's `finally` restore can clobber another's set mid-run, dropping
...
_ulak_interactive_ctx: contextvars.ContextVar[str | None] = _ctx("ulak_interactive", None)


def set_ulak_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context instead of mutating os.environ."""
    return _ulak_interactive_ctx.set("1" if interactive else "")


def reset_ulak_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_ulak_interactive_context`."""
    _ulak_interactive_ctx.reset(token)


def _is_interactive_cli() -> bool:
    """True for an interactive CLI/ACP session (contextvar first, env fallback)."""
    ctx_val = _ulak_interactive_ctx.get()
    return is_truthy_value(ctx_val) if ctx_val is not None else env_var_enabled("ULAK_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("ULAK_SESSION_KEY", default)
...
def _is_unattended_platform_approval_context() -> bool:
    ...
    Webhook, msgraph_webhook, and api_server sessions bind ``ULAK_SESSION_PLATFORM`` like chat gateways
...
def _is_single_query_approval_context() -> bool:
    """True for a single-query (-q) session: ``ulak chat -q`` exports
    ``ULAK_INTERACTIVE=1`` (so sudo password prompts work) but nobody is waiting
    ...
    return is_truthy_value(_session_env("ULAK_SINGLE_QUERY_SESSION"))


def _is_gateway_approval_context() -> bool:
    """True inside a gateway/API session that can answer an approval.

    Legacy integrations set ULAK_GATEWAY_SESSION; concurrent paths bind
    ULAK_SESSION_PLATFORM via contextvars. Cron is NEVER a gateway approval
    ...
    if _is_cron_approval_context() or _is_unattended_platform_approval_context():
        return False
    return env_var_enabled("ULAK_GATEWAY_SESSION") or bool(_get_session_platform())
...
def _should_fall_through_to_cli_approval(*, is_cli: bool, approval_callback, notify_cb) -> bool:
    ...
    ``ULAK_EXEC_ASK`` (or a platform marker) can leak into an interactive CLI
