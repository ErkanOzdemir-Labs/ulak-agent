...
_ULAK_CONFIG_FILES = r'\.ulak/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.ulak/\.env|\~/\.ulak/\.env',
     "ulak_env_access", "critical", "exfiltration", "directly references Ulak secrets file"),
    ...
    # quarantine only acts on "dangerous") but high for Ulak/other config (setup docs routinely say
    ...
    (_prose_modify_re(_ULAK_CONFIG_FILES),
     "ulak_config_mod", "high", "persistence", "modification language aimed at Ulak configuration files (verify intent)"),
    (_shell_write_re(_ULAK_CONFIG_FILES),
     "ulak_config_mod_shell", "critical", "persistence", "shell write (redirect/sed -i/tee/cp/mv) targeting Ulak configuration files"),
    (r'\.ulak/config\.yaml|\.ulak/SOUL\.md',
     "ulak_config_ref", "low", "persistence", "references Ulak configuration files (informational; only modification intent is scored)"),
...
# `.skillignore` is Ulak-native; `.clawhubignore` is honored for skills published through ClawHub.
