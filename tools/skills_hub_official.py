"""Skills Hub official sources: repo-shipped optional skills and the centralized Ulak index."""
...
from tools.skills_hub_models import (
    SkillBundle, SkillMeta, SkillSource, _ulak_tags, _matches_query, _memo_json, _parse_frontmatter, hub,
)
...
class OptionalSkillSource(SkillSource):
    ...
    activated by default — absent from the system prompt and not copied to ~/.ulak/skills/ at
    setup. Discoverable via the Skills Hub as source "official" with "builtin" trust."""

    SOURCE_ID = "official"
    TRUST_LEVEL = "builtin"
    OFFICIAL_REPO = "ErkanOzdemir-Labs/ulak-agent"
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        # (metadata.ulak.upstream); install pulls the live content from there.
        try:
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``main``; rather than demanding ``ulak update`` first, resolve against the live repo.
    ...
    def _upstream_pointer_from_content(self, content: Union[str, bytes]) -> Optional[Dict[str, str]]:
        """Parse ``metadata.ulak.upstream: {repo: owner/name, path: ...}`` out of SKILL.md content
        ...
        ulak_meta = meta_block.get("ulak") if isinstance(meta_block, dict) else None
        upstream = ulak_meta.get("upstream") if isinstance(ulak_meta, dict) else None
...
class UlakIndexSource(SkillSource):
    """Skill source backed by the centralized Ulak Skills Index: a JSON catalog on the docs site,
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            from tools.skills_hub_search import _load_ulak_index
            self._index, self._loaded = _load_ulak_index(), True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        for github_id in filter(None, candidates):
            ...
            if bundle:
                bundle.source = entry.get("source", "ulak-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            ...
            source=entry.get("source", "ulak-index"), identifier=entry.get("identifier", ""),
