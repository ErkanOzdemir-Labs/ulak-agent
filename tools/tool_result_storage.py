...
is ALWAYS host-side ``$ULAK_HOME/cache/spillover/{id}.txt`` (works for sessions that never
...
def get_spillover_dir():
    """Return $ULAK_HOME/cache/spillover as a Path (not created)."""
    from ulak_constants import get_ulak_home
    return get_ulak_home() / SPILLOVER_SUBDIR
...
def _write_to_spillover(content: str, filename: str):
    """Write host-side to $ULAK_HOME/cache/spillover; returns path str or None."""
    try:
...
def _resolve_storage_dir(env) -> str:
    ...
    return f"{temp_dir.rstrip('/') or '/'}/ulak-results" if temp_dir else STORAGE_DIR
...
HEREDOC_MARKER = "ULAK_PERSIST_EOF"
# ---- END PLUGIN-COMPAT ----
