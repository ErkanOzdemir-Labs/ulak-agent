...
Ships **no in-tree provider**: enable a plugin and select it in ``ulak tools`` → Video
    ...
    if configured:
        return json.dumps(error_response(
            error=(f"video_gen.provider='{configured}' is set but no plugin registered that name. "
                   f"Run `ulak plugins list` to see installed video gen backends, or "
                   f"`ulak tools` → Video Generation to pick one."),
            error_type="provider_not_registered", provider=configured))
    return json.dumps(error_response(
        error=("No video generation backend is configured. Run `ulak tools` → "
...
_GENERIC_DESCRIPTION = (
    ...
    "`ulak tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if provider is None:
        parts.append(
            ...
            "until the user picks one via `ulak tools` → Video Generation.")
