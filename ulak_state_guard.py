...
_STATE_DB_GUARD_BYPASS_ENV = "ULAK_STATE_DB_GUARD_BYPASS"


def _real_platform_state_root() -> Optional[Path]:
    """The REAL platform-default Ulak root. Avoids ``Path.home()`` /
    ``ulak_constants`` (tests monkeypatch Path.home to a tempdir); ``expanduser``
    reads HOME/passwd, which the conftest never rewrites."""
    try:
        home = Path(os.path.expanduser("~"))
        if sys.platform == "win32":
            base = os.environ.get("LOCALAPPDATA", "").strip()
            root = Path(base) / "ulak" if base else home / "AppData" / "Local" / "ulak"
        else:
            root = home / ".ulak"
        return root.resolve()
    except Exception:
...
#: Exported by the hermetic conftest alongside the ULAK_HOME redirect. Unlike
...
# : Env marker exported by the hermetic test conftest at the same moment it : redirects ``ULAK_HOME`` to
...
# under Ulak test : isolation", and it inherits into subprocess children by default — so a : child that
# received the patched ``ULAK_HOME`` also received the marker, : and a child that resolves a production DB
# while carrying it is, by : definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "ULAK_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    rebuilt env loses PYTEST_* and the ULAK_HOME redirect together, ancestry
    ...
    loses at the same moment it loses the ``ULAK_HOME`` redirect: that child aims at the production DB
