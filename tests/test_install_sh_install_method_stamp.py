...
($INSTALL_DIR), not into the shared $ULAK_HOME.

Background (shared-$ULAK_HOME bug)
------------------------------------
$ULAK_HOME is a data directory users frequently bind-mount into a Docker
gateway as well (``~/.ulak:/opt/data``). The published image stamps 'docker'
there on boot, so if install.sh had written its 'git' marker into the same
$ULAK_HOME the two installs would fight over one slot — and the container,
booting last, would win and wrongly make the host install look like 'docker'
(blocking ``ulak update``).
...
``~/.ulak/ulak-agent``), which is unique to this install and immune to the
shared data dir.
"""
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$ULAK_HOME/\.install_method"', text), (
        "install.sh must not stamp $ULAK_HOME/.install_method — that data "
        "dir may be shared with a Docker gateway whose 'docker' stamp would "
        "clobber it and block host-side `ulak update`"
    )
