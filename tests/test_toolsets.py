...
class TestToolsetConsistency:
    ...
    def test_ulak_platforms_share_core_tools(self):
        """All ulak-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on ulak-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["ulak-cli", "ulak-telegram", "ulak-discord", "ulak-whatsapp", "ulak-slack", "ulak-signal", "ulak-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_ulak_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("ulak-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("ulak-cli")
        second = resolve_toolset("ulak-cli")
        ...
        assert (
            "ulak-cli", True, registry_id, generation
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("ulak-cli", include_registry=False)
        second = resolve_toolset("ulak-cli", include_registry=False)
        assert first == second
