...
def test_reference_messages_drops_system_but_renders_tools_as_text():
    ...
    messages = [
        {"role": "system", "content": "huge ulak system prompt"},
    ...
    assert all("huge ulak system prompt" not in m["content"] for m in view)
...
def test_slot_runtime_anthropic_oauth_routes_through_provider_branch(monkeypatch):
    ...
    monkeypatch.setattr(
        "ulak_cli.runtime_provider.resolve_runtime_provider", fake_resolve
    )
...
def test_reference_messages_flattens_cache_decorated_content():
    ...
    plain = [
        {"role": "system", "content": "ulak system prompt"},
        {"role": "user", "content": "Can we get codex usage resets into ulak?"},
    ]
