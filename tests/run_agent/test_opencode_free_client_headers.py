...
@patch("agent.process_bootstrap.OpenAI")
def test_opencode_free_sends_ulak_attribution(mock_openai):
    """Keyless requests still identify as Ulak (attribution headers match
    ...
    headers = _zen_call_headers(mock_openai)
    assert headers.get("X-Title") == "Ulak Agent"
    assert str(headers.get("User-Agent", "")).startswith("UlakAgent/")
