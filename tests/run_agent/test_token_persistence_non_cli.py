...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    ulak_state = ModuleType("ulak_state")
    ulak_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "ulak_state", ulak_state)
    ulak_state_registry = ModuleType("ulak_state_registry")
    ulak_state_registry.acquire = lambda db_path=None: sentinel_db
    monkeypatch.setitem(sys.modules, "ulak_state_registry", ulak_state_registry)
    ...
    result = json.loads(agent._invoke_tool(
        "session_search",
        {"query": "Ulak", "detail": "full"},
        "task-id",
    ))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Ulak"
...
def test_sequential_session_search_forwards_detail(monkeypatch):
    ...
    tool_call = SimpleNamespace(
        ...
        function=SimpleNamespace(
            name="session_search",
            arguments=json.dumps({"query": "Ulak", "detail": "full"}),
        ),
    )
