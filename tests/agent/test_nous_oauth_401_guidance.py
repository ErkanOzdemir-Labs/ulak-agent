...
to a generic "Your API key was rejected... run ulak setup" message, which is
...
def test_nous_401_guidance_strings_present():
    ...
    # Must give a concrete re-auth command, not a generic "ulak setup".
    assert "ulak portal" in source

    # Must point at the portal so users can check account/credit status.
    assert "portal.github.com/ErkanOzdemir-Labs" in source
