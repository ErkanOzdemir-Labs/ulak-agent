...
blocks that Ulak synthesises from ``reasoning_content``.

See ulak-agent#16748.
"""
...
class TestDeepSeekAnthropicPreservesThinking:
    ...
    def test_cache_control_stripped_from_thinking_block(self) -> None:
        ...
        upstreams that do check them, so Ulak strips them everywhere.
        """
        from agent.anthropic_message_convert import convert_messages_to_anthropic
...
@pytest.mark.parametrize("url", [None, "https://api.anthropic.com", "https://inference-api.github.com/ErkanOzdemir-Labs/anthropic"])
