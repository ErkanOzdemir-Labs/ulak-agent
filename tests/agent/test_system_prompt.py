...
class TestNamedProfileHintIntegration:
    ...
    ``TestNamedProfileHint`` mocks ``get_ulak_home``,
    ``get_default_ulak_root`` and ``_resolve_active_profile_name``, so it
    ...
    ``ULAK_HOME`` and no resolver mocks.
    """

    def test_real_ulak_home_under_profiles_renders_correct_paths(
        ...
        assert get_default_ulak_root() == root

        agent = _make_agent(valid_tool_names=["read_file"])
        with patch("agent.coding_context._coding_mode", return_value="off"):
            prompt = "\n\n".join(_prompt_parts(agent).values())

        assert "Active Ulak profile: coder." in prompt
    ...
    def test_real_default_home_renders_default_branch(self, tmp_path, monkeypatch):
        """ULAK_HOME at the root resolves to the default profile, unchanged."""
        root = tmp_path / ".ulak"
        ...
        assert "Active Ulak profile: default." in prompt
...
def test_coding_prompt_preserves_legacy_workspace_order(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "ULAK_AGENT_HELP_GUIDANCE", "HELP")
    monkeypatch.setattr(system_prompt, "ULAK_AGENT_HELP_GUIDANCE_NO_SKILLS", "HELP")
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_ulak_home", lambda: Path("/ulak"))

    # Production renders this as str(get_ulak_home()) + "/profiles/<name>/",
    # and str(Path("/ulak")) is platform-dependent (backslash on Windows) —
    # build the expectation the same way instead of hardcoding "/ulak".
    _home_str = str(Path("/ulak"))
    expected_profile = (
    ...
    with (
        ...
        patch("ulak_time.now", return_value=datetime(2026, 1, 2)),
    ):
...
class TestTelegramRichMessagesHint:
    ...
    def test_base_hint_without_rich_messages(self, monkeypatch):
        ...
        with patch("ulak_cli.config.load_config_readonly") as mock_cfg:
    ...
    def test_gateway_rich_messages_integration_via_real_config(self, tmp_path, monkeypatch):
        ...
        under ULAK_HOME with ``gateway.platforms.telegram.extra.rich_messages``
        ...
        from ulak_cli import config as _cfgmod
...
def test_conversation_start_uses_session_start_not_build_time(monkeypatch):
    ...
    with (
        ...
        patch("ulak_time.now", return_value=datetime(2026, 1, 2, 9, 0)),
    ):
        prompt = build_system_prompt(agent, system_message="SYSTEM_MESSAGE")
...
class TestConversationStartedTwoLine:
    ...
    def test_same_day_session_keeps_single_line(self):
        from ulak_time import now as ulak_now
        sid = ulak_now().strftime("%Y%m%d_%H%M%S_fresh")
