...
def test_clock_failure_omits_rule_but_compaction_still_runs():
    ...
    with patch.object(ulak_time, "now", _boom), patch(
...
def test_anchoring_rule_uses_date_from_ulak_time_now():
    """The date is taken from ulak_time.now(), which respects the user's TZ."""
    compressor = _compressor()
    fixed = datetime(2025, 12, 31, 23, 30, tzinfo=timezone.utc)
    with patch.object(ulak_time, "now", lambda: fixed), patch(
        "agent.context_compressor.call_llm", return_value=_response("summary")
    ) as mock_call:
