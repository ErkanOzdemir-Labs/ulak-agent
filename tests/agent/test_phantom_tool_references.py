...
Covers:
  * ULAK_AGENT_HELP_GUIDANCE degrades to the docs-only variant when the
...
class TestUlakAgentHelpGuidance:
    def test_skill_variant_used_when_skill_view_present(self):
        from agent.prompt_builder import ULAK_AGENT_HELP_GUIDANCE
        assert "skill_view(name='ulak-agent')" in ULAK_AGENT_HELP_GUIDANCE

    def test_no_skills_variant_has_no_skill_view_reference(self):
        from agent.prompt_builder import ULAK_AGENT_HELP_GUIDANCE_NO_SKILLS
        assert "skill_view" not in ULAK_AGENT_HELP_GUIDANCE_NO_SKILLS
        assert "ulak-agent.github.com/ErkanOzdemir-Labs/docs" in ULAK_AGENT_HELP_GUIDANCE_NO_SKILLS
...
class TestEssentialSkillsUndisableable:
    def test_agent_side_reader_strips_essential(self, monkeypatch, tmp_path):
        ...
        assert "ulak-agent" not in disabled
...
class TestEssentialOnlySync:
    def test_opted_out_sync_seeds_only_essential(self, monkeypatch, tmp_path):
        """A profile with .no-bundled-skills still gets the ulak-agent skill."""
        ...
        for cat, name in [
            ("autonomous-ai-agents", "ulak-agent"),
            ("media", "gif-search"),
        ]:
            d = bundled / cat / name
        ...
        monkeypatch.setattr(ss, "_ulak_home", lambda: home)
        ...
        assert result["copied"] == ["ulak-agent"]
        assert (home / "skills" / "autonomous-ai-agents" / "ulak-agent" / "SKILL.md").exists()
