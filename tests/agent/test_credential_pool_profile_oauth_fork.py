...
Real imports, real temp ULAK_HOME root + named profile, real auth.json I/O.
...
def fleet(tmp_path, monkeypatch):
    """Root ULAK_HOME with an expired-but-refreshable Anthropic pool row."""
    ...
    # against $HOME/.ulak/auth.json; our root is elsewhere, so writes go.
    import ulak_constants
    ulak_constants._default_ulak_root_memo = None  # type: ignore[attr-defined]
    ...
    store = {
        ...
        "credential_pool": {
            "anthropic": [{
                ...
                "priority": 0, "source": "manual:ulak_pkce",
    ...
    import ulak_cli.auth as _auth_mod
...
def _profile(fleet, name, **kw):
    from ulak_cli.profiles import create_profile
...
def test_strip_helper_fails_closed_when_root_store_cannot_be_resolved(fleet, monkeypatch):
    ...
    monkeypatch.setattr(
        ulak_constants, "get_default_ulak_root",
...
def test_borrower_prune_never_deletes_root_singleton_grant(fleet, tmp_path):
    """Root's ulak_pkce row is seeded from ROOT's .anthropic_oauth.json; a
    ...
    assert [e.source for e in root_rows] == ["ulak_pkce"]

    kid = _profile(fleet, "kid")
    fleet["use"](kid)
    pool = load_pool("anthropic")
    assert [e.source for e in pool.entries()] == ["ulak_pkce"], "borrowed root grant was pruned"
    assert fleet["rows"](root) and fleet["rows"](root)[0]["source"] == "ulak_pkce"
...
def test_profile_auth_add_owns_only_its_own_rows(fleet):
    ...
    pool.add_entry(PooledCredential(
        ...
        priority=0, source="manual:ulak_pkce", access_token="sk-ant-oat01-MINE",
...
def test_heal_consolidates_existing_forks_to_the_live_copy(fleet, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="ulak_cli.auth"):
        fleet["use"](forge)
        sel = load_pool("anthropic").select()
...
def test_heal_is_idempotent_and_logs_once(fleet, caplog):
    import logging
    from agent.credential_pool import load_pool
    from ulak_cli.auth import consume_oauth_heal_notices, heal_forked_single_use_oauth_grants
...
def test_heal_never_deletes_the_only_surviving_copy(fleet):
    """Root lost its grant (user ran `ulak auth remove` at root); the profile's
...
def test_heal_pkce_singleton_shape_commits_live_pair_to_root_singleton(fleet):
    """`ulak auth` PKCE shape: root + profile each have .anthropic_oauth.json +
    a ulak_pkce-seeded row; the profile's copy is the rotated (live) one."""
    from agent.credential_pool import load_pool
    ...
    fleet["use"](root)
    load_pool("anthropic")  # seeds root's ulak_pkce row from the singleton
...
def test_heal_is_a_noop_in_classic_mode(fleet):
    from ulak_cli.auth import heal_forked_single_use_oauth_grants
    fleet["use"](fleet["root"])
...
def test_heal_skips_profile_auth_json_symlinked_to_the_root_store(fleet):
    """#101356: `ln -s ~/.ulak/auth.json <profile>/auth.json` shares ONE store.
