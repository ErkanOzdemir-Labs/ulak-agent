"""Regression tests for ErkanOzdemir-Labs/ulak-agent#2765.
...
service that didn't inherit ``~/.ulak/.env``) used to be dropped silently.
...
def test_warns_once_and_dedupes(caplog):
    ...
    assert "ulak memory status" in msg
