...
def isolated_home(monkeypatch):
    """Temp ULAK_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="ulak_test_31179_")
    ulak_home = os.path.join(test_home, ".ulak")
...
def _fresh_modules():
    """Drop cached ulak modules so each test reloads against current env."""
    _drop_reload_targets()
