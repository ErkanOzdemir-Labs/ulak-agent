...
def test_load_review_credentials_cfg_missing_section(monkeypatch):
    monkeypatch.setattr(
        "ulak_cli.config.load_config_readonly", lambda: {"auxiliary": {}}
    )
    assert re_mod._load_review_credentials_cfg() is None
...
def test_collect_skills_from_preloaded_prompt_and_history():
    ...
    parent.ephemeral_system_prompt = (
        ...
        '"ulak-agent-dev" skill preloaded. Treat its instructions as '
    ...
    assert names == ["ulak-agent-dev", "github-pr-workflow"]
...
def test_briefing_includes_loaded_skills_instruction():
    ...
    _, context = build_review_task(snap, "", ["ulak-agent-dev", "xitter"])
    assert "ulak-agent-dev, xitter" in context
...
def test_start_review_threads_loaded_skills_into_context(monkeypatch):
    ...
    parent = _fake_parent()
    parent.ephemeral_system_prompt = (
        'session with the "ulak-agent-dev" skill preloaded.'
    )
    msgs = [
    ...
    assert result["status"] == "dispatched"
    assert "ulak-agent-dev" in built["context"]
