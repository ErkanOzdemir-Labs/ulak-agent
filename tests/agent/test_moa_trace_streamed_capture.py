...
path via ``ulak chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp ULAK_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point ULAK_HOME at a temp dir and turn moa.save_traces on."""
    ulak_home = tmp_path / ".ulak"
    ulak_home.mkdir()
    monkeypatch.setenv("ULAK_HOME", str(ulak_home))

    # save_moa_turn reads config via ulak_cli.config.load_config; stub it to
