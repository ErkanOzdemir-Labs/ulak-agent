...
The official Codex endpoint must receive Ulak' own harness identity, rather
...
These tests pin:
- the required Ulak originator
- the versioned Ulak User-Agent
...
class TestCodexCloudflareHeaders:
    ...
    def test_user_agent_advertises_ulak_version(self):
...
class TestPrimaryClientWiring:
    ...
    def test_apply_client_headers_on_base_url_change(self):
        ...
        with patch("agent.process_bootstrap.OpenAI") as mock_openai:
            ...
            assert headers.get("originator") == "ulak-agent"
            assert headers.get("ChatGPT-Account-ID") == "acct-rotation"
            assert headers.get("User-Agent") == f"UlakAgent/{__version__}"
