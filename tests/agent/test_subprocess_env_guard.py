...
``ulak_subprocess_env`` / ``_sanitize_subprocess_env``, which the factory
...
``ULAK_HOME`` or secret-scrub propagation.
...
SCAN_DIRS = ("agent", "ulak_cli", "tools", "gateway", "cron", "tui_gateway")
SCAN_ROOT_FILES = ("cli.py", "ulak_constants.py")
...
ALLOWED_RAW_SPAWN_ENV_FILES = {
    # THE owner module: _sanitize_subprocess_env / ulak_subprocess_env /
