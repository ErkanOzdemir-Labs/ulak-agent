...
def test_bounded_fast_window_policy(monkeypatch):
    ...
    for provider, base_url in (
        ...
        ("nous", "https://inference-api.github.com/ErkanOzdemir-Labs/v1"),
...
def test_fast_auto_and_cold_parse_and_slash_command(monkeypatch):
    ...
    from ulak_cli.commands import COMMAND_REGISTRY
    ...
    cli_mod.UlakCLI._handle_fast_command(stub, "/fast auto")
    assert stub.service_tier == "auto"
    assert stub.agent is None
    cli_mod.UlakCLI._handle_fast_command(stub, "/fast status")
    assert any("auto" in line for line in printed)
    cli_mod.UlakCLI._handle_fast_command(stub, "/fast cold")
    ...
    assert cli_mod.UlakCLI._resolve_turn_agent_config(route_stub, "hi")["request_overrides"] is None
    route_stub.service_tier = "priority"
    assert cli_mod.UlakCLI._resolve_turn_agent_config(route_stub, "hi")["request_overrides"] == {
        "service_tier": "priority"
    }
