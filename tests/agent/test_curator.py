...
def curator_env(tmp_path, monkeypatch):
    """Isolated ULAK_HOME + freshly reloaded curator + skill_usage modules."""
    home = tmp_path / ".ulak"
    ...
    # save_state() resolves the state path from ULAK_HOME at write time, so a
    # straggler thread that outlives this test would write into whatever home
    # the *next* test has configured (or the default ~/.ulak once monkeypatch
    ...
    # thread here, while ULAK_HOME is still pinned to this test's tmp home
...
def test_cli_pin_refuses_bundled_skill(curator_env, capsys):
    from ulak_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
# `ulak model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_curator_slot_is_canonical_aux_task():
    ...
    from ulak_cli.main_provider_setup import _AUX_TASKS
    from ulak_cli.web_server_config import _AUX_TASK_SLOTS
    ...
    # 2. ulak_cli/main.py _AUX_TASKS — CLI picker
    aux_keys = {k for k, _name, _desc in _AUX_TASKS}
    assert "curator" in aux_keys, "curator missing from _AUX_TASKS (CLI picker)"

    # 3. ulak_cli/web_server.py _AUX_TASK_SLOTS — REST API allowlist
...
def test_review_fork_restricts_toolsets_to_skills_only(curator_env, monkeypatch):
    ...
    ``ulak curator rollback`` restored a hollow skill. Removing the toolset
...
def test_review_fork_toolset_surface_excludes_execution_tools():
    ...
    already-stripped package, and ``ulak curator rollback`` restores a
...
def test_review_prompt_does_not_steer_terminal_writes():
    ...
    ``mkdir -p ~/.ulak/skills/<umbrella>/references/ && mv ...`` its
