...
def _agent(**kw):
    a = SimpleNamespace(provider="nous", model="anthropic/claude-fable-5.1", api_mode="chat_completions",
                        api_key="k", base_url="https://inference-api.github.com/ErkanOzdemir-Labs/v1", session_id="s")
    for k, v in kw.items():
        setattr(a, k, v)
    return a
...
def test_real_agent_usage_recorder_calls_the_hook_once(tmp_path, monkeypatch):
    ...
    a = AIAgent(api_key="jwt", base_url="https://inference-api.github.com/ErkanOzdemir-Labs/v1", provider="nous",
