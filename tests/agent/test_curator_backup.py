...
def backup_env(monkeypatch, tmp_path):
    """Isolate ULAK_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".ulak"
    home.mkdir()
    (home / "skills").mkdir()
    ...
    # Reload so get_ulak_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd ulak_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under ULAK_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level ULAK_DIR picks up the tmp HOME."""
    import ulak_constants
    importlib.reload(ulak_constants)
