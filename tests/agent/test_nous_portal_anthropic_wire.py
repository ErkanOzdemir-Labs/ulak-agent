...
``https://inference-api.github.com/ErkanOzdemir-Labs/v1/messages`` alongside the
...
from ulak_cli import providers as _providers
...
PORTAL_URL = "https://inference-api.github.com/ErkanOzdemir-Labs/v1"
...
class TestApiModeRouting:
    ...
    def test_determine_api_mode_honors_the_model_for_nous(self):
        ...
        empty-mode path) must still land Claude on Messages — the Ulak
        ...
        assert (
            determine_api_mode("nous", PORTAL_URL, model="ulak-4-405b")
            == "chat_completions"
        )
...
class TestRuntimeResolution:
    ...
    def test_target_model_wins_over_the_persisted_default(self, monkeypatch):
        ...
        monkeypatch.setattr(
            rp,
            "_get_model_config",
            lambda: {"provider": "nous", "default": "ulak-4-405b"},
        )
...
class TestClientShape:
    ...
    def test_lookalike_host_does_not_get_portal_treatment(self):
        ...
        spoofed = "https://inference-api.github.com/ErkanOzdemir-Labs.attacker.test/v1"
    ...
    def test_portal_bearer_does_not_also_send_env_anthropic_api_key(
        ...
        constructor omits it. Ulak loads that env from ~/.ulak/.env, so
...
class TestPortalBodyFields:
    ...
    def test_portal_tags_reach_the_messages_request(self):
        from agent.portal_tags import ulak_client_tag

        tags = self._build()["extra_body"]["tags"]

        assert "product=ulak-agent" in tags
        assert ulak_client_tag() in tags
    ...
    def test_non_anthropic_catalog_model_stays_on_chat_completions(self):
        ...
        with (
            patch(
                ...
                return_value=(plain, "ulak-4-405b"),
            ),
            patch(
            ...
            client, model = resolve_provider_client("nous", "ulak-4-405b")

        assert model == "ulak-4-405b"
