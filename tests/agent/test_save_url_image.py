...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate ULAK_HOME under tmp_path."""
    monkeypatch.setenv("ULAK_HOME", str(tmp_path / ".ulak"))
    (tmp_path / ".ulak").mkdir()

    # Force the constants/image cache helpers to re-read ULAK_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("ulak_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_ulak_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under ULAK_HOME — gateway cleanup
