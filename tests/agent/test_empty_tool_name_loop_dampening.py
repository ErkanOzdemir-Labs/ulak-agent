...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated ULAK_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="ulak_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".ulak"))
    prev_home = os.environ.get("ULAK_HOME")
    os.environ["ULAK_HOME"] = os.path.join(test_home, ".ulak")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("ulak_"):
            del sys.modules[mod]
    ...
    try:
        ...
        # references into ulak_cli.*/tools.* and their monkeypatches would otherwise
        ...
        if prev_home is None:
            ...
            os.environ["ULAK_HOME"] = prev_home
