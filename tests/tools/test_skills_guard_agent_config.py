...
skills-guard-v2 scores tiers by confidence:
  ...
  * prose instructing modification of Ulak/other-agent config files,
...
class TestTruePositivesStillCaught:
    ...
    def test_ulak_config_redirection_is_dangerous(self, tmp_path):
        result = _scan(tmp_path, "cat payload >> .ulak/SOUL.md")
        assert result.verdict == "dangerous"

    def test_ulak_config_imperative_is_caution(self, tmp_path):
        result = _scan(tmp_path, "Write the override key into .ulak/config.yaml")
        assert result.verdict == "caution"
