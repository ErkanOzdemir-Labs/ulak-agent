...
- _seed_from_env reads API keys from ~/.ulak/.env when not in os.environ
- _resolve_api_key_provider_secret falls back to credential_pool when env vars are empty
- ~/.ulak/.env takes priority over os.environ for Ulak-managed credentials
...
@pytest.fixture
def isolated_ulak_home(tmp_path, monkeypatch):
    """Point ULAK_HOME at a temp dir and clear known API key env vars.
...
def _write_env_file(home: Path, **kwargs) -> None:
    """Write key=value pairs to ~/.ulak/.env."""
    lines = [f"{k}={v}" for k, v in kwargs.items()]
    (home / ".env").write_text("\n".join(lines) + "\n")


class TestCredentialPoolSeedsFromDotEnv:
    """_seed_from_env must read keys from ~/.ulak/.env, not just os.environ.
    ...
    load_ulak_dotenv, the credential pool must still discover it.
    """

    def test_deepseek_key_from_dotenv_only(self, isolated_ulak_home):
        """Key in .env but not os.environ → _seed_from_env adds a pool entry."""
        _write_env_file(isolated_ulak_home, DEEPSEEK_API_KEY="sk-dotenv-only-12345")
    ...
    def test_empty_dotenv_no_entries(self, isolated_ulak_home):
    ...
    def test_dotenv_wins_over_stale_os_environ(self, isolated_ulak_home, monkeypatch):
        """Regression for #20591: a fresh key rotated into ~/.ulak/.env must
        ...
        """
        _write_env_file(isolated_ulak_home, DEEPSEEK_API_KEY="sk-dotenv-fresh")
...
class TestAuthResolvesFromDotEnv:
    """_resolve_api_key_provider_secret must also read from ~/.ulak/.env."""

    def test_key_from_dotenv_only(self, isolated_ulak_home):
        """Key in .env but not os.environ → _resolve returns it with the env var source."""
        _write_env_file(isolated_ulak_home, DEEPSEEK_API_KEY="sk-dotenv-resolve-789")
        assert "DEEPSEEK_API_KEY" not in os.environ

        from ulak_cli.auth import _resolve_api_key_provider_secret
    ...
    def test_dotenv_wins_over_stale_os_environ_on_resolve(
        self, isolated_ulak_home, monkeypatch
    ):
        """Regression for #20591: when both ~/.ulak/.env and os.environ define
        ...
        """
        _write_env_file(isolated_ulak_home, DEEPSEEK_API_KEY="dotenv-fresh-deepseek")
    ...
    def test_get_anthropic_key_prefers_dotenv_over_stale_os_environ(
        ...
        prefer ~/.ulak/.env over a stale shell export. This path resolves
        ...
        """
        _write_env_file(isolated_ulak_home, ANTHROPIC_API_KEY="dotenv-fresh-anthropic")
...
class TestAuthCredentialPoolFallback:
    ...
    def test_credential_pool_fallback_structure(self, isolated_ulak_home):
    ...
    def test_credential_pool_empty_returns_empty(self, isolated_ulak_home):
    ...
    def test_env_var_takes_priority_over_pool(self, isolated_ulak_home, monkeypatch):
    ...
    def test_dotenv_takes_priority_over_pool(self, isolated_ulak_home):
        """Key in .env beats credential pool — pool only fires when both env sources are empty."""
        _write_env_file(isolated_ulak_home, DEEPSEEK_API_KEY="sk-dotenv-priority-xyz")
        assert "DEEPSEEK_API_KEY" not in os.environ
...
class TestAnthropicEnvAuthTypeClassification:
    ...
    def test_admin_key_classified_as_api_key(self, isolated_ulak_home):
        ...
        _write_env_file(isolated_ulak_home, ANTHROPIC_API_KEY="sk-ant-admin-fake-12345")
        entry = self._seed("ANTHROPIC_API_KEY", "sk-ant-admin-fake-12345")
        assert entry.auth_type == AUTH_TYPE_API_KEY

    def test_standard_api_key_classified_as_api_key(self, isolated_ulak_home):
        """sk-ant-api- key → AUTH_TYPE_API_KEY (unchanged behaviour)."""
        from agent.credential_pool import AUTH_TYPE_API_KEY
        _write_env_file(isolated_ulak_home, ANTHROPIC_API_KEY="sk-ant-api-fake-12345")
