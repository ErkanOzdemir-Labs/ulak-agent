...
was gated on ``[ -f "$ULAK_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'ULAK_HOME="{home}"\n'
        # In tests we run unprivileged; as_ulak is a passthrough then.
        'as_ulak() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Ulak loads $ULAK_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/ulak/.env.example -> $ULAK_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $ULAK_HOME/.env from it on first boot"
    )
