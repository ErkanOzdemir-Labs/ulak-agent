"""Cross-session ULAK_SESSION_ID leak via the shared bash snapshot.
...
``ULAK_SESSION_ID`` into the snapshot, so every LATER session ``source``d that
stale value and its ``echo $ULAK_SESSION_ID`` reported a FOREIGN session's id
— overriding the correct per-command Popen env injected by
``_inject_session_context_env``.

The fix strips the per-session bridged vars (ULAK_SESSION_* / UI /
...
def test_export_snippet_shape():
    snippet = _export_dump_excluding_session_vars('"$__ulak_snap_tmp"')
    ...
    assert "${!ULAK_SESSION_*}" in snippet
    assert "${!ULAK_CRON_AUTO_DELIVER_*}" in snippet
    assert "${!ULAK_BROWSER_CONTROL_*}" in snippet
    assert "ULAK_UI_SESSION_ID" in snippet
    assert "grep -vE" not in snippet
    assert '"$__ulak_snap_tmp"' in snippet
    ...
    assert snippet.rstrip().endswith('> "$__ulak_snap_tmp"')
    ...
    try:
        def run_as(sid):
            ...
            def worker():
                ...
                out["r"] = env.execute('echo "[$ULAK_SESSION_ID]"')
        ...
        if os.path.exists(snap):
            with open(snap) as f:
                assert "ULAK_SESSION_ID" not in f.read()
    finally:
        env.cleanup()
