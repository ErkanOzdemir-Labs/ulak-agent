...
def ulak_home(monkeypatch, tmp_path):
    """Isolate ULAK_HOME so the tests don't pollute the real config.
...
class TestPatchCRLFPreservation:
    def test_patch_on_crlf_file_stays_pure_crlf(self, ulak_home, tmp_path):
    ...
    def test_patch_multiline_replacement_on_crlf(self, ulak_home, tmp_path):
...
class TestWriteFileCRLFPreservation:
    ...
    def test_overwrite_lf_file_stays_lf(self, ulak_home, tmp_path):
