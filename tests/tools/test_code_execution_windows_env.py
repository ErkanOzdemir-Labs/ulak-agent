...
``ulak_tools.py`` and ``script.py`` into a temp dir, and those files
...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``ULAK_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # ULAK_* handling (#27303): only the operational allowlist passes;
        # every other ULAK_* is dropped (the broad prefix was removed).
        "ULAK_HOME": "/home/alice/.ulak",        # allowlisted → kept
        "ULAK_PROFILE": "default",                 # allowlisted → kept
        "ULAK_INTERACTIVE": "1",                   # not allowlisted → dropped
        "ULAK_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "ULAK_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
...
# ``ulak_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``ulak_tools.py`` and ``script.py`` writes in
    ...
    def test_stub_source_roundtrips_through_utf8(self):
        ...
        import tempfile, ast
        stub = generate_ulak_tools_module(
    ...
    def test_windows_default_encoding_would_have_failed(self):
        ...
        *without* ``encoding="utf-8"`` would corrupt the file.  If this
        ...
        stub = generate_ulak_tools_module(["terminal"], transport="uds")
...
# let the child import ulak_tools, a user script that printed non-ASCII
