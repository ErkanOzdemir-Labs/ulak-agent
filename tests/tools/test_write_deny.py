...
class TestWriteDenyExactPaths:
    ...
    def test_ulak_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.ulak/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "ulak_root"
        ...
        # Sanity check: ULAK_HOME does point to the profile dir, not the root.
        from ulak_constants import get_ulak_home, get_default_ulak_root
...
class TestWriteAllowed:
    ...
    def test_ulak_control_files_requested_writable(self):
