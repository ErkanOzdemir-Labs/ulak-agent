"""Tests for tools.wake_word — the "Hey Ulak" hotword detector.
...
def test_config_defaults_and_clamping():
    ...
    assert ww.wake_phrase({"phrase": "hey ulak"}) == "hey ulak"
    assert ww.wake_phrase({}) == "hey ulak"
...
def test_looks_like_path():
    ...
    assert _looks_like_path("models/hey_ulak.onnx")
...
def test_load_wake_word_config_guards_non_dict(monkeypatch):
    monkeypatch.setattr(
        "ulak_cli.config.load_config", lambda: {"wake_word": "oops"}
    )
...
def test_requirements_openwakeword_available(monkeypatch):
    ...
    r = ww.check_wake_word_requirements(
        {"provider": "openwakeword", "phrase": "hey ulak"}
    )
    assert r["available"] is True
    assert r["provider"] == "openwakeword"
    assert r["phrase"] == "hey ulak"
...
def test_openwakeword_ensures_base_models_for_custom_path(monkeypatch):
    ...
    eng = ww._OpenWakeWordEngine(
        {"provider": "openwakeword", "openwakeword": {"model": "/models/hey_ulak.onnx"}}
    )
    assert calls["download"] == [["/models/hey_ulak.onnx"]]
    assert eng._labels == ["hey_ulak"]


def test_bundled_hey_ulak_model_ships_on_disk():
    # The "hey ulak" wake word works out of the box only if the model is
