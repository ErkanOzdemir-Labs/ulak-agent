...
Covers:
- :class:`UlakTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.UlakMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _ULAK_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# UlakTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = UlakTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ULAK_HOME", str(tmp_path))
        storage = UlakTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (UlakMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: UlakTokenStorage, **context_attrs):
    ...
    provider = _ULAK_PROVIDER_CLS.__new__(_ULAK_PROVIDER_CLS)
    provider._ulak_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ULAK_HOME", str(tmp_path))
        storage = UlakTokenStorage("mgr-srv")
        ...
        with patch.object(
            _ULAK_PROVIDER_CLS.__bases__[-1], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ULAK_HOME", str(tmp_path))
        storage = UlakTokenStorage("flow-srv")
        ...
        with patch.object(
            _ULAK_PROVIDER_CLS.__bases__[-1],
            "async_auth_flow",
