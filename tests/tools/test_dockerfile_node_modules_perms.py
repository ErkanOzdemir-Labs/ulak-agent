...
Older images made /opt/ulak/ui-tui and /opt/ulak/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV ULAK_TUI_DIR=/opt/ulak/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R ulak:ulak /opt/ulak/ui-tui" not in text
    assert "chown -R ulak:ulak /opt/ulak/node_modules" not in text
