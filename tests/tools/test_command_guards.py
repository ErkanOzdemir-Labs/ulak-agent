...
def _mode_manual(monkeypatch):
    ...
    The test conftest redirects ULAK_HOME to an empty tempdir, so the
...
def _clean_state():
    ...
    saved = {}
    for k in ("ULAK_INTERACTIVE", "ULAK_GATEWAY_SESSION", "ULAK_EXEC_ASK", "ULAK_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ULAK_INTERACTIVE"] = "1"
        session_key = os.getenv("ULAK_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ULAK_INTERACTIVE or ULAK_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
