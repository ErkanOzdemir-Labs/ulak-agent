...
Verifies:
  - Tools are gated on ULAK_KANBAN_TASK: a normal chat session sees
...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    """Normal `ulak chat` sessions (no ULAK_KANBAN_TASK) must have
    ...
    schema = registry.get_definitions(set(resolve_toolset("ulak-cli")), quiet=True)
...
def worker_env(monkeypatch, tmp_path):
    """Simulate being a worker: ULAK_HOME isolated, ULAK_KANBAN_TASK set
    after we've created the task."""
    home = tmp_path / ".ulak"
    home.mkdir()
    monkeypatch.setenv("ULAK_HOME", str(home))
    monkeypatch.setenv("ULAK_PROFILE", "test-worker")
    monkeypatch.delenv("ULAK_SESSION_ID", raising=False)
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("ULAK_KANBAN_TASK", tid)
    return tid
...
def test_complete_goal_mode_rejected_by_judge(monkeypatch, tmp_path):
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("ULAK_KANBAN_TASK", goal_task_id)
...
def _make_goal_mode_worker_env(monkeypatch, tmp_path):
    """Set up an isolated ULAK_HOME with one claimed goal_mode task,
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to ULAK_PROFILE env we set in the fixture
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    derived from ``ULAK_PROFILE`` so a worker can't forge a comment
    under an authoritative-looking name like ``ulak-system`` and
    ...
    from tools import kanban_tools as kt
    out = kt._handle_comment({
        "task_id": worker_env, "body": "hi", "author": "ulak-system",
    })
    ...
    try:
        comments = kb.list_comments(conn, worker_env)
        # Author comes from ULAK_PROFILE in the fixture, not the
        # caller-supplied "ulak-system" override.
        assert comments[0].author == "test-worker"
    finally:
        conn.close()
...
def test_unblock_with_pending_parents_returns_todo(monkeypatch, tmp_path):
    ...
    home.mkdir()
    monkeypatch.setenv("ULAK_HOME", str(home))
    monkeypatch.setenv("ULAK_PROFILE", "orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
# ---------------------------------------------------------------------------
#
# A worker process has ULAK_KANBAN_TASK set to its own task id. The
...
# are unrestricted.
#
# Orchestrator profiles (no ULAK_KANBAN_TASK in env) are intentionally
...
def test_worker_can_comment_on_foreign_task(worker_env):
    ...
    # ULAK_PROFILE — never to a caller-controlled string.
    conn = kbc.connect()
    try:
...
def test_orchestrator_complete_any_task_allowed(monkeypatch, tmp_path):
    """Orchestrator profiles (no ULAK_KANBAN_TASK) can still complete
...
# ---------------------------------------------------------------------------
#
# The dispatcher pins the active board via ULAK_KANBAN_BOARD env var,
...
# restarting Ulak. These tests pin that ``board=<slug>`` argument
...
def multi_board_env(monkeypatch, tmp_path):
    """Isolated Ulak home with two distinct kanban boards seeded.

    Returns ``("default", "alt")`` slugs. The default board has one
    pre-existing task ``seed_default``; ``alt`` has ``seed_alt``. No
    ULAK_KANBAN_TASK is pinned (orchestrator context) — workers test
    the env-task case via the existing ``worker_env`` fixture.
    """
    home = tmp_path / ".ulak"
    home.mkdir()
    monkeypatch.setenv("ULAK_HOME", str(home))
    # Make sure neither ULAK_KANBAN_DB nor ULAK_KANBAN_BOARD pin a
    # board — the test is specifically about the per-call override.
    monkeypatch.delenv("ULAK_KANBAN_DB", raising=False)
    monkeypatch.delenv("ULAK_KANBAN_BOARD", raising=False)
    monkeypatch.delenv("ULAK_KANBAN_TASK", raising=False)
    monkeypatch.setenv("ULAK_PROFILE", "test-orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
# - Gateway sessions: ULAK_SESSION_PLATFORM + ULAK_SESSION_CHAT_ID set.
# - TUI sessions: ULAK_SESSION_KEY (or ULAK_SESSION_ID) set, with
...
def _list_subs_for_task(task_id):
    from ulak_cli import kanban_db as kb
    from ulak_cli import kanban_db_connect as kbc
    from ulak_cli import kanban_db_notify as kbn
    conn = kbc.connect()
    try:
...
def test_create_subscribes_gateway_session(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("ULAK_SESSION_CHAT_ID", "chat-42")
    monkeypatch.setenv("ULAK_SESSION_THREAD_ID", "thread-7")
    monkeypatch.setenv("ULAK_SESSION_USER_ID", "user-9")
    monkeypatch.setenv("ULAK_SESSION_USER_ID_ALT", "alt-user-9")
    monkeypatch.setenv("ULAK_SESSION_CHAT_TYPE", "forum")
...
def test_create_subscribes_tui_session_via_session_key(monkeypatch, worker_env):
    ...
    local channel), but the parent process exports ULAK_SESSION_KEY.
    We should still auto-subscribe, with platform='tui' and
    chat_id=<key>."""
    from tools import kanban_tools as kt
    monkeypatch.delenv("ULAK_SESSION_PLATFORM", raising=False)
    monkeypatch.delenv("ULAK_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("ULAK_SESSION_THREAD_ID", raising=False)
    monkeypatch.delenv("ULAK_SESSION_USER_ID", raising=False)
    monkeypatch.setenv("ULAK_SESSION_KEY", "tui-session-abc")
    monkeypatch.delenv("ULAK_SESSION_ID", raising=False)
...
def test_create_does_not_subscribe_in_cli_session(monkeypatch, worker_env):
    ...
    monkeypatch.delenv("ULAK_SESSION_PLATFORM", raising=False)
    monkeypatch.delenv("ULAK_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("ULAK_SESSION_KEY", raising=False)
    monkeypatch.delenv("ULAK_SESSION_ID", raising=False)
...
def test_create_respects_auto_subscribe_on_create_false(monkeypatch, worker_env, tmp_path):
    ...
    # worker_env already created <tmp>/.ulak; use a fresh sibling
    # home to avoid mkdir() colliding with the worker's directory.
    home = tmp_path / "gate-home" / ".ulak"
    home.mkdir(parents=True)
    (home / "config.yaml").write_text(
        "kanban:\n  auto_subscribe_on_create: false\n"
    )
    monkeypatch.setenv("ULAK_HOME", str(home))
    monkeypatch.setenv("ULAK_SESSION_PLATFORM", "discord")
    monkeypatch.setenv("ULAK_SESSION_CHAT_ID", "channel-1")
...
def default_url_guard(monkeypatch):
    ...
    Clears ULAK_ALLOW_PRIVATE_URLS and resets url_safety's process-lifetime
    cache on both sides so a prior test's opt-in can't leak in.
    """
    from tools import url_safety

    monkeypatch.delenv("ULAK_ALLOW_PRIVATE_URLS", raising=False)
    url_safety._reset_allow_private_cache()
    yield
    url_safety._reset_allow_private_cache()
