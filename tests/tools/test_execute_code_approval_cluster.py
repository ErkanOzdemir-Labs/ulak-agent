...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_env._scrub_child_env — broad ULAK_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: ULAK_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_ulak_allowlist_and_secret_blocks():
    from tools.code_execution_env import _scrub_child_env

    env = {
        # operational allowlist → kept
        "ULAK_HOME": "/h", "ULAK_PROFILE": "p",
        "ULAK_CONFIG": "/c.yaml", "ULAK_ENV": "/e",
        "ULAK_DELEGATED_CHILD_CONTEXT": "1",
        # other ULAK_* → dropped (broad prefix removed)
        "ULAK_BASE_URL": "https://x", "ULAK_INTERACTIVE": "1",
        "ULAK_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "ULAK_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ULAK_BASE_URL", "ULAK_INTERACTIVE", "ULAK_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped ULAK_* vars."""
    import logging
