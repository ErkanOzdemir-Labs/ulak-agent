...
class TestRmtreeWritableScopeGuard:
    ...
    exception — the result is a silent ``shutil.rmtree(~/.ulak/)``
    ...
    def test_refuses_anything_that_is_not_a_strict_child_of_skills(self, tmp_path):
        """``/``, ``~/.ulak`` itself, a sibling dir, and the skills root
        ...
        sibling = ulak / "kanban.db"  # any non-skills path
        sibling.mkdir()

        with patch("tools.skills_sync.SKILLS_DIR", skills):
            for target in (Path("/"), ulak, sibling, skills):
...
class TestGetBundledDir:
    def test_env_var_override_with_default_fallback(self, tmp_path, monkeypatch):
        ...
        custom_dir.mkdir()
        monkeypatch.setenv("ULAK_BUNDLED_SKILLS", str(custom_dir))
        assert _get_bundled_dir() == custom_dir

        # Empty or unset falls back to the relative path from __file__.
        monkeypatch.setenv("ULAK_BUNDLED_SKILLS", "")
        assert _get_bundled_dir().name == "skills"
        monkeypatch.delenv("ULAK_BUNDLED_SKILLS", raising=False)
        assert _get_bundled_dir().name == "skills"
...
class TestNoBundledSkillsOptOut:
    ...
    This is what `ulak profile create --no-skills` (named profiles) and the
    installer's `--no-skills` flag (default ~/.ulak) rely on so bundled
    skills are never seeded at install time NOR re-injected by `ulak update`.
    """
    ...
    def test_marker_skips_sync_and_removal_seeds_normally(self, tmp_path):
        ...
        skills_dir = tmp_path / "user_skills"
        manifest_file = skills_dir / ".bundled_manifest"
        ulak_home = tmp_path / "home"
        ulak_home.mkdir()
        marker = ulak_home / ".no-bundled-skills"
        ...
        def _patches():
            ...
            stack.enter_context(patch("tools.skills_sync.SKILLS_DIR", skills_dir))
            stack.enter_context(patch("tools.skills_sync.MANIFEST_FILE", manifest_file))
            stack.enter_context(patch("tools.skills_sync.ULAK_HOME", ulak_home))
            return stack
...
class TestOptOutToggleAndRemove:
    """`ulak skills opt-out/opt-in` core: marker toggle + safe removal."""
    ...
    def test_marker_toggle(self, tmp_path):
        ...
        with patch("tools.skills_sync.ULAK_HOME", home):
    ...
    def test_remove_keeps_user_modified(self, tmp_path):
        ...
        with patch("tools.skills_sync._get_bundled_dir", return_value=bundled), \
             ...
             patch("tools.skills_sync.ULAK_HOME", home):
...
class TestCallTimeDirResolution:
    ...
    ULAK_HOME at import, so a long-lived dashboard/TUI process serving a
    ...
    The accessors must follow set_ulak_home_override() at call time, while
    an explicitly patched module global (tests, _profile_scope retargeting)
    still wins.
    """

    def test_accessors_follow_ulak_home_override(self, tmp_path):
        ...
        try:
            assert ss._ulak_home() == profile_home
    ...
    def test_explicit_module_patch_wins_over_override(self, tmp_path):
        ...
        token = set_ulak_home_override(str(tmp_path / "other-profile"))
        try:
