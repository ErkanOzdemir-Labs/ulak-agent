...
class TestApprovalModeParsing:
    ...
    def test_config_bool_false_maps_to_off(self):
        with mock_patch("ulak_cli.config.load_config_readonly", return_value={"approvals": {"mode": False}}):
...
class TestSmartApproval:
    ...
    def test_smart_approval_does_not_allowlist_the_pattern_for_session(self, monkeypatch):
        ...
        monkeypatch.setenv("ULAK_SESSION_KEY", session_key)
        monkeypatch.setenv("ULAK_EXEC_ASK", "1")
        monkeypatch.delenv("ULAK_CRON_SESSION", raising=False)
        monkeypatch.setattr(
...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
            for prefix in ("ulak-verify-", "ulak-ad-hoc-"):
    ...
    def test_symlinked_temp_dir_only_exempts_canonical_target(self, tmp_path):
        ...
        basename = "ulak-verify-example.py"
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            "rm -rf /tmp/ulak-verify-example.py",
            "rm -f /tmp/ulak-verify-example.py /tmp/other.py",
            "rm -f /tmp/nested/../ulak-verify-example.py",
            "rm -f /tmp/a/../../tmp/ulak-verify-example.py",
            "rm -f /var/tmp/ulak-verify-example.py",
            "rm -f /tmp/ulak-verify-*",
            "rm -f /tmp/ulak-verify-$(touch>/tmp/pwned).py",
            "rm -f /tmp/ulak-ad-hoc-`touch>/tmp/pwned`.py",
            "rm -f /tmp/ulak-verify-example.py; touch /tmp/pwned",
        )
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
...
class TestWindowsShellDestructiveCommands:
    def test_windows_destructive_requires_approval(self):
        cases = [
            (r"cmd /c del /f /q C:\tmp\ulak-victim\file.txt", "Windows cmd destructive delete"),
            (r"cmd.exe /k rmdir /s /q C:\tmp\ulak-victim", "Windows cmd destructive delete"),
            ...
            (r"powershell Remove-Item -Recurse -Force C:\tmp\ulak-victim",
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"ULAK_SESSION_KEY": "bob"}, clear=False):
...
class TestUlakConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.ulak/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_write_idioms_against_config(self):
        for command in (
            "echo 'approvals:' > ~/.ulak/config.yaml",
            "echo '  mode: off' >> ~/.ulak/config.yaml",
            "echo x | tee ~/.ulak/config.yaml",
            "echo x | tee $ULAK_HOME/config.yaml",
            "cp /tmp/evil.yaml ~/.ulak/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
            assert key is not None, command
    ...
    def test_reads_and_unrelated_writes_are_safe(self):
        # Reading config is not a write; a non-Ulak absolute config.yaml is
        # handled by the project patterns, not the Ulak-home rule.
        for cmd in (
            "cat ~/.ulak/config.yaml",
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.ulak/config.yaml/.env must require approval — the
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Ulak-home prefixes must fold to ~/ and
    ~/.ulak/ in dangerous-command detection.
    ...
    SSH, and Ulak config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_ulak_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/ULAK_HOME so the fold is verified on the POSIX CI runner too."""
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(self, monkeypatch, capsys):
        monkeypatch.setenv("ULAK_LANGUAGE", "tr")
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        cmd = "kill 1605 && cd ~/.ulak/ulak-agent && source venv/bin/activate && python -m ulak_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc
        for variant in (
            "python -m ulak_cli.main gateway run --replace &",
            "nohup python -m ulak_cli.main gateway run --replace",
        ):
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart ulak-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestWebhookApprovalExclusion:
    ...
    def test_webhook_platform_returns_false(self, monkeypatch):
        ...
        monkeypatch.setenv("ULAK_SESSION_PLATFORM", "webhook")
    ...
    def test_all_unattended_platforms_return_false(self, monkeypatch):
        ...
        for platform in _UNATTENDED_APPROVAL_PLATFORMS:
            monkeypatch.setenv("ULAK_SESSION_PLATFORM", platform)
    ...
    def test_non_webhook_gateway_session_returns_true(self, monkeypatch):
        ...
        monkeypatch.delenv("ULAK_CRON_SESSION", raising=False)
        monkeypatch.setenv("ULAK_GATEWAY_SESSION", "1")
        monkeypatch.setenv("ULAK_SESSION_PLATFORM", "telegram")
    ...
    def test_cron_session_returns_false_regardless_of_platform(self, monkeypatch):
        ...
        monkeypatch.setenv("ULAK_CRON_SESSION", "1")
        monkeypatch.setenv("ULAK_SESSION_PLATFORM", "telegram")
    ...
    def test_webhook_dangerous_command_denies_by_default(self, monkeypatch):
        ...
        monkeypatch.setenv("ULAK_SESSION_PLATFORM", "webhook")
        monkeypatch.setenv("ULAK_SESSION_KEY", "test-webhook-session")
    ...
    def test_api_server_dangerous_command_denies_by_default(self, monkeypatch):
        ...
        monkeypatch.delenv("ULAK_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("ULAK_INTERACTIVE", raising=False)
        monkeypatch.setenv("ULAK_SESSION_PLATFORM", "api_server")
        monkeypatch.setenv("ULAK_SESSION_KEY", "test-api-session")
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_forms_still_flagged_dangerous(self):
        for cmd in (
            ...
            # In-place edit of the Ulak security config via IFS.
            "sed${IFS}-i ~/.ulak/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(cmd)
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep ulak) bypasses the pkill/killall name-matching
    ...
    def test_kill_pidof_expansion_detected(self):
        """`kill $(pidof ulak)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        dangerous, _, desc = detect_dangerous_command("kill -TERM $(pidof ulak_cli.main)")
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()
        assert detect_dangerous_command("kill -9 `pidof ulak`")[0] is True
...
class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Ulak service
    label achieves the same effect as `ulak gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_against_ulak_label_detected(self):
        for cmd in (
            "launchctl stop ai.ulak.gateway",
            "launchctl kickstart -k system/ai.ulak.gateway",
            "launchctl bootout system/ai.ulak.gateway",
            "launchctl unload ~/Library/LaunchAgents/ai.ulak.gateway.plist",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd

    def test_unrelated_labels_not_flagged(self):
        """Read-only inspection, and lifecycle ops on non-Ulak labels, are
    ...
    def test_spliced_detection_does_not_flag_prose_or_other_services(self):
        ...
        a ulak-gateway identifier, so quoted prose and non-gateway ulak
    ...
    def test_label_built_before_verb_detected(self):
        ...
        "ulak"/"ai.ulak" to appear AFTER the verb and missed this
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("ULAK_GATEWAY_SESSION", "ULAK_CRON_SESSION",
                      "ULAK_YOLO_MODE",
                      "ULAK_SESSION_KEY", "ULAK_INTERACTIVE")
        }
        os.environ.pop("ULAK_YOLO_MODE", None)
        os.environ.pop("ULAK_INTERACTIVE", None)
        # ULAK_CRON_SESSION takes priority over ULAK_GATEWAY_SESSION in
        ...
        os.environ.pop("ULAK_CRON_SESSION", None)
        os.environ["ULAK_GATEWAY_SESSION"] = "1"
        os.environ["ULAK_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_import_error_allows_when_fail_open_or_disabled(self, enabled, fail_open):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("ulak_cli.config.load_config_readonly", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"ULAK_INTERACTIVE": "1"}, clear=False):
...
GATEWAY_LIFECYCLE_LAUNCHCTL = (
    ...
    "launchctl unload ~/Library/LaunchAgents/ai.ulak.gateway.plist",
    "launchctl load ~/Library/LaunchAgents/ai.ulak.gateway.plist",
    "launchctl stop ai.ulak.gateway",
    "launchctl restart ai.ulak.gateway",
    "launchctl bootout gui/501/ai.ulak.gateway",
    "launchctl remove ai.ulak.gateway",
    "launchctl disable gui/501/ai.ulak.gateway",
)
...
class TestLifecycleGuardLaunchctlParity:
    ...
    under ``_ULAK_GATEWAY == "1"``, the ``cron.lifecycle_guard`` block is
    ...
    def test_unrelated_labels_are_not_blocked(self):
        ...
        services, including other Ulak ones, stay runnable."""
        from cron.lifecycle_guard import contains_gateway_lifecycle_command

        for cmd in (
            "launchctl bootout gui/501/com.example.unrelated",
            "launchctl remove ai.ulak.update-checker",
