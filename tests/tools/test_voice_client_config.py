...
Real config files in a temp ULAK_HOME, real resolution chains (no mocked
...
def voice_home(tmp_path, monkeypatch):
    """Isolated ULAK_HOME + reloaded config modules; yields a config writer."""
    home = tmp_path / ".ulak"
    home.mkdir()
    ...
    for var in (
        ...
        "DEEPINFRA_API_KEY", "ULAK_LOCAL_STT_LANGUAGE",
    ):
