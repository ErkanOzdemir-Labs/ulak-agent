...
warm_agent_browser_npx_cache() is the fire-and-forget helper `ulak update` /
`ulak doctor --fix` call to pre-fetch agent-browser via npx so the first real
...
`ulak update` — not only when a browser tool is actually used), must pass
...
def test_returns_false_without_spawning_when_npx_unresolvable():
    with patch("tools.browser_tool_install._resolve_npx_bin", return_value=None), patch(
        "subprocess.Popen"
    ) as mock_popen:
...
def test_invokes_npx_with_ignore_scripts_prefer_offline_and_pinned_spec():
    with patch("tools.browser_tool_install._resolve_npx_bin", return_value="/usr/bin/npx"), patch(
...
def test_captures_stdout_and_stderr_instead_of_inheriting_parent_fds():
    """The npx registry fetch runs on every `ulak update` — its stdout/
...
def test_uses_credential_scrubbed_environment():
    ...
    os.environ with every provider/gateway credential Ulak holds."""
...
def test_merges_extended_path_so_managed_only_npx_can_find_sibling_node():
    """If npx was resolved via the Ulak-managed/extended search (not the
    ...
    with patch("tools.browser_tool_install._resolve_npx_bin", return_value="/opt/ulak/node/bin/npx"), \
         patch("tools.browser_tool._build_browser_env", return_value={"PATH": "/usr/bin"}), \
         patch(
             "tools.browser_tool_install._merge_browser_path",
             return_value="/opt/ulak/node/bin:/usr/bin",
         ) as mock_merge, \
         patch("subprocess.Popen", return_value=_mock_proc()) as mock_popen:
        warm_agent_browser_npx_cache()
    ...
    _args, kwargs = mock_popen.call_args
    assert kwargs["env"]["PATH"] == "/opt/ulak/node/bin:/usr/bin"
...
def test_timeout_kills_the_whole_process_tree_not_just_the_pid():
    ...
    with patch("tools.browser_tool_install._resolve_npx_bin", return_value="/usr/bin/npx"), \
         ...
         patch("tools.browser_tool_lifecycle._kill_process_tree") as mock_kill:
...
def test_timeout_cleanup_communicate_itself_raising_does_not_propagate():
    ...
    fire-and-forget caller (ulak_cli/doctor.py calls this bare)."""
    proc = _mock_proc(
        communicate_side_effect=[
            subprocess.TimeoutExpired(cmd=["npx"], timeout=60.0),
...
def test_returns_false_instead_of_raising_on_unexpected_communicate_exception():
    """Fire-and-forget contract: ulak_cli/doctor.py calls this bare (no
