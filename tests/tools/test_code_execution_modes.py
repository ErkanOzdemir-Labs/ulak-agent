...
class TestExecuteCodeModeIntegration(unittest.TestCase):
    ...
    def test_strict_mode_runs_in_tmpdir(self):
        ...
        path stages in ulak_sandbox_*, the session kernel in
        ulak_kernel_* — either satisfies strict mode's isolation promise."""
        result = self._run("import os; print(os.getcwd())", mode="strict")
        self.assertEqual(result["status"], "success")
        cwd = result["output"].strip()
        self.assertTrue(
            "ulak_sandbox_" in cwd or "ulak_kernel_" in cwd,
    ...
    def test_project_mode_interpreter_is_venv_python(self):
        ...
        # The ulak-agent venv is always active during tests, so this also
    ...
    def test_project_mode_can_still_import_ulak_tools(self):
        """Regression: ulak_tools still importable from non-tmpdir CWD.

        This is the PYTHONPATH fix — without it, switching to session CWD
        breaks `from ulak_tools import terminal`.
        """
        import tempfile
        with tempfile.TemporaryDirectory() as td:
            code = (
                "from ulak_tools import terminal\n"
                "r = terminal('echo x')\n"
    ...
    def test_strict_mode_can_still_import_ulak_tools(self):
...
# ---------------------------------------------------------------------------
# _python_environment_prefix / _uses_ulak_python_environment
# ---------------------------------------------------------------------------
...
class TestPythonEnvironmentPrefix(unittest.TestCase):
    ...
    def test_failure_is_not_cached(self):
        ...
        A sticky cached failure would silently drop the ulak root from
...
class TestUsesUlakPythonEnvironment(unittest.TestCase):
    """Unit tests for _uses_ulak_python_environment."""
    ...
    def test_true_for_current_interpreter(self):
        ...
        self.assertTrue(_uses_ulak_python_environment(sys.executable))
    ...
    def test_true_for_current_interpreter_without_probe(self):
        ...
        must never drop the ulak root for the interpreter Ulak itself runs.
        """
        with patch("subprocess.run",
                   side_effect=subprocess.TimeoutExpired(cmd=[], timeout=5)) as mock_run:
            self.assertTrue(_uses_ulak_python_environment(sys.executable))
    ...
    def test_false_for_different_prefix(self):
        ...
        with patch("tools.code_execution_env._python_environment_prefix",
            ...
            self.assertFalse(_uses_ulak_python_environment("/other/python"))

    def test_false_when_prefix_is_empty(self):
        """If prefix cannot be determined (error path), treat as external."""
        with patch("tools.code_execution_env._python_environment_prefix",
                   return_value=""):
            self.assertFalse(_uses_ulak_python_environment("/bad/python"))

    def test_true_when_prefix_matches_sys_prefix(self):
        ulak_prefix = os.path.realpath(sys.prefix)
        with patch("tools.code_execution_env._python_environment_prefix",
                   return_value=ulak_prefix):
            self.assertTrue(_uses_ulak_python_environment("/same/env/python"))


# ---------------------------------------------------------------------------
# PYTHONPATH composition — ulak root included only for same-env interpreters
# ---------------------------------------------------------------------------

class TestPythonPathComposition(unittest.TestCase):
    """Verify ulak root inclusion in PYTHONPATH depends on env match.

    Patches ``_uses_ulak_python_environment`` directly so these tests are
    ...
    def _capture_pythonpath(self, same_env: bool) -> tuple:
        ...
        def _fake_popen(cmd, **kwargs):
            ...
            # [python, <staging_dir>/ulak_kernel_runner.py] (session
    ...
    def test_ulak_root_included_when_same_env(self):
        """When interpreter is in the Ulak env, ulak root is in PYTHONPATH."""
        pythonpath, _ = self._capture_pythonpath(same_env=True)
        parts = pythonpath.split(os.pathsep)
        self.assertIn(self._ulak_root(), parts,
                      "ulak root must be in PYTHONPATH for same-env interpreters")

    def test_ulak_root_excluded_when_external_env(self):
        """When interpreter is external, ulak root must NOT be in PYTHONPATH."""
        pythonpath, _ = self._capture_pythonpath(same_env=False)
        parts = pythonpath.split(os.pathsep)
        self.assertNotIn(self._ulak_root(), parts,
                         "ulak root must not leak into an external interpreter's PYTHONPATH")
