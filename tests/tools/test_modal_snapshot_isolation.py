...
def _restore_tool_modules():
    original_ulak_home = os.environ.get("ULAK_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_ulak_home is None:
            os.environ.pop("ULAK_HOME", None)
        else:
            os.environ["ULAK_HOME"] = original_ulak_home
        _reset_modules(("tools", "ulak_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    ulak_cli = types.ModuleType("ulak_cli")
    ulak_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["ulak_cli"] = ulak_cli
    ulak_home = tmp_path / "ulak-home"
    os.environ["ULAK_HOME"] = str(ulak_home)
    sys.modules["ulak_cli.config"] = types.SimpleNamespace(
        get_ulak_home=lambda: ulak_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="ulak-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": ulak_home / "modal_snapshots.json",
