...
def test_output_path_rejects_ulak_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    ulak_home = tmp_path / "ulak-home"
    ulak_home.mkdir()
    monkeypatch.setattr(file_safety, "_ulak_home_path", lambda: ulak_home)
    monkeypatch.setattr(file_safety, "_ulak_root_path", lambda: ulak_home)

    target = ulak_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    ulak_home = tmp_path / "ulak-home"
    token_dir = ulak_home / "mcp-tokens"
