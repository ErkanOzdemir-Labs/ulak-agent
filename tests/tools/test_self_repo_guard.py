...
class TestBlocksMutationsInSourceRepo:
    ...
    def test_relative_cd_into_repo(self, repo):
        hit, _ = _detect("cd ulak-agent && git pull", repo.parent, repo)
        assert hit is True
    ...
    def test_tilde_dash_c_path(self, repo, monkeypatch, tmp_path):
        ...
        hit, _ = _detect("git -C ~/ulak-agent checkout main", tmp_path, repo)
        assert hit is True
...
class TestBlockMessageGuidance:
    ...
    def test_scratch_hint_honors_ulak_home(self, repo, monkeypatch):
        monkeypatch.setenv("ULAK_HOME", "/custom/ulak-home")
        hit, msg = _detect("git rebase origin/main", repo, repo)
        assert hit is True
        assert "/custom/ulak-home/scratch" in msg
