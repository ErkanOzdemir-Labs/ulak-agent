...
- Signal forwarding runs through the init so ``docker stop`` triggers
  ulak's own graceful-shutdown path.
...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, ulak accumulates zombie
    ...
    assert installed, (
        ...
        "reap orphaned subprocesses, ulak accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    PID 1 and ulak would run without zombie reaping.
    """
    ...
    assert "/opt/ulak/docker/entrypoint-dispatch.sh" in entrypoint_line, (
    ...
    assert "exec /init /opt/ulak/docker/main-wrapper.sh" in dispatcher, (
