...
Uses a throwaway ULAK_HOME so it never touches ~/.ulak. No real names.
"""
...
def _fresh_home():
    """Point ULAK_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_ulak_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["ULAK_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER ULAK_HOME is set.
...
# a different get_ulak_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"ULAK_HOME (throwaway): {HOME}")
