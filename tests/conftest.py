"""Shared fixtures for the ulak-agent test suite.
...
2. **Isolated ULAK_HOME.** ULAK_HOME points to a per-test tempdir so
   code reading ``~/.ulak/*`` via ``get_ulak_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".ulak"`` instead of the canonical
   ``get_ulak_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ULAK_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox ULAK_HOME before ANY test module is imported ──────────────────
# `ulak_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_ulak_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.ulak/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes ULAK_HOME, but fixtures run
...
# the REAL Ulak root — capture it BEFORE the sandbox rewires ULAK_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.ulak (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("ULAK_KANBAN_HOME", "").strip()
_PRE_SANDBOX_ULAK_HOME = os.environ.get("ULAK_HOME", "")


def _ulak_home_points_at_production(value: str) -> bool:
    """True when a pre-set ULAK_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    ULAK_HOME=~/.ulak``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``ulak_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.ulak`` — the escape vector that landed pytest
    ...
    ULAK_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".ulak").resolve()
    except Exception:
        return True
...
if _ulak_home_points_at_production(os.environ.get("ULAK_HOME", "")):
    _SESSION_ULAK_HOME = tempfile.mkdtemp(prefix="ulak-test-home-")
    os.environ["ULAK_HOME"] = _SESSION_ULAK_HOME
    atexit.register(shutil.rmtree, _SESSION_ULAK_HOME, True)
...
# like a real CLI") — which used to disarm ulak_state's live-DB guard in
# the child at the same moment the child lost the ULAK_HOME redirect.
# ULAK_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# ulak_state_guard._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export ULAK_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["ULAK_TEST_ISOLATION"] = os.environ.get("ULAK_HOME", "") or "1"

#: ULAK_HOME as it stood when conftest was imported - i.e. before any test
...
ULAK_HOME_AT_CONFTEST_IMPORT = os.environ.get("ULAK_HOME", "")
...
# ULAK_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ULAK_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``ulak_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Ulak agent session inherits them and
    # ulak_constants home-resolution helpers prefer them over monkeypatched
    ...
    "ULAK_MAX_ITERATIONS",
    "ULAK_SESSION_PLATFORM",
    "ULAK_SESSION_CHAT_ID",
    "ULAK_SESSION_CHAT_NAME",
    "ULAK_SESSION_CHAT_TYPE",
    "ULAK_SESSION_THREAD_ID",
    "ULAK_SESSION_SOURCE",
    "ULAK_SESSION_KEY",
    "ULAK_GATEWAY_SESSION",
    "ULAK_CRON_SESSION",
    "_ULAK_GATEWAY",
    "ULAK_PLATFORM",
    "ULAK_MODEL",
    "ULAK_INFERENCE_MODEL",
    "ULAK_INFERENCE_PROVIDER",
    "ULAK_TUI_PROVIDER",
    "ULAK_MANAGED",
    "ULAK_MANAGED_DIR",
    "ULAK_DEV",
    "ULAK_CONTAINER",
    "ULAK_EPHEMERAL_SYSTEM_PROMPT",
    "ULAK_TIMEZONE",
    "ULAK_REDACT_SECRETS",
    "ULAK_BACKGROUND_NOTIFICATIONS",
    "ULAK_EXEC_ASK",
    "ULAK_HOME_MODE",
    "ULAK_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.ulak/kanban.db instead of the per-test ULAK_HOME.
    "ULAK_KANBAN_DB",
    "ULAK_KANBAN_BOARD",
    "ULAK_KANBAN_HOME",
    "ULAK_KANBAN_WORKSPACES_ROOT",
    "ULAK_KANBAN_LOGS_ROOT",
    "ULAK_KANBAN_TASK",
    "ULAK_KANBAN_WORKSPACE",
    "ULAK_KANBAN_RUN_ID",
    "ULAK_KANBAN_CLAIM_LOCK",
    "ULAK_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    # otherwise-unrelated config tests away from the default "ulak" host.
    "ULAK_HONCHO_HOST",
    ...
    "ULAK_DASHBOARD_OAUTH_CLIENT_ID",
    "ULAK_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ULAK_HOME to per-test tempdirs so code that
    reads ``~/.ulak/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ULAK_* vars that could change test semantics.
    for name in _ULAK_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("ULAK_HONCHO_HOST", "ulak")

    # 3. Redirect ULAK_HOME to a per-test tempdir. Code that reads
    #    ``~/.ulak/*`` via ``get_ulak_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.ulak/*`` via
    #    ``Path.home() / ".ulak"`` instead of ``get_ulak_home()``
    #    is a bug to fix at the callsite.
    fake_ulak_home = tmp_path / "ulak_test"
    fake_ulak_home.mkdir()
    (fake_ulak_home / "sessions").mkdir()
    (fake_ulak_home / "cron").mkdir()
    (fake_ulak_home / "memories").mkdir()
    (fake_ulak_home / "skills").mkdir()
    monkeypatch.setenv("ULAK_HOME", str(fake_ulak_home))
    ...
    # ulak_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("ULAK_TEST_ISOLATION", str(fake_ulak_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("ULAK_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. ulak_state computes ``DEFAULT_DB_PATH = get_ulak_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from ulak_state import ...``) that
    ...
    ulak_state_mod = sys.modules.get("ulak_state")
    if ulak_state_mod is not None and hasattr(ulak_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            ulak_state_mod, "DEFAULT_DB_PATH", fake_ulak_home / "state.db"
        )
    ...
    monkeypatch.setenv("ULAK_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.ulak/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import ulak_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path ULAK_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_ulak_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from ulak_cli import kanban_db_dispatch as _kbd_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.ulak. This autouse
...
# allow-list because test-level fixtures legitimately move ULAK_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session ULAK_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``ULAK_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Ulak root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_ULAK_HOME and not _ulak_home_points_at_production(
        _PRE_SANDBOX_ULAK_HOME
    ):
        # ULAK_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_ulak_root().resolve()
    # No pre-existing ULAK_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".ulak").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.ulak`` captured at import time. Hermetic tests that legitimately
    move ULAK_HOME to sibling tempdirs are unaffected.

    Only patches when ``ulak_cli.kanban_db_connect`` is *already imported*
    ...
    """
    _kdb = sys.modules.get("ulak_cli.kanban_db")
    _kdbc = sys.modules.get("ulak_cli.kanban_db_connect")
    if _kdb is None or _kdbc is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import ulak_cli.kanban_db`
...
# ``ulak_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Ulak root. This fixture wires the
# test-side knobs:
...
#     installs where ULAK_HOME is not ~/.ulak) into the guard's
...
# so subprocess children that import ulak_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("ulak_state")
    ...
    if _PRE_SANDBOX_ULAK_HOME and not _ulak_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_ULAK_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Ulak home override redirects every later
    # ``get_ulak_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from ulak_constants import get_ulak_home_override, set_ulak_home_override

        if get_ulak_home_override() is not None:
            set_ulak_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal ulak config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``ulak-gateway`` process
...
# correlated with a ``tests/ulak_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> ulak-gateway``
...
def _wal_is_usable() -> bool:
    """True when Ulak will actually put a database into WAL mode here.

    Ulak refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Ulak can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Ulak managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``ulak_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_ulak_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_ulak_home`` fixture
    redirects ``ULAK_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.ulak/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``ulak_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["ULAK_VOICE_TTS"] = "1"``. The
...
#      ``ulak_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``ulak_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``ulak_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``ulak_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Ulak runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.ulak``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Ulak falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Ulak uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart ulak-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting ulak/python patterns are also blocked.
    """
    ...
    def _matches_ulak_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ULAK_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_ulak_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch ulak-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m ulak_cli.main".
                if (
                    "ulak" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live ulak-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting ulak/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `ulak update` (or the
        # equivalent `python -m ulak_cli.main update`).  These commands
        ...
        if "update" in low and (
            # ulak update / ulak update --gateway / setsid bash -c ... ulak update
            ("ulak" in low and "update" in low.split())
            or
            # python -m ulak_cli.main update --gateway
            ("ulak_cli" in low and "update" in low.split())
            or
            # venv/bin/ulak update  (absolute path variant used in tests)
            (".venv/bin/ulak" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`ulak update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_ULAK_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import ulak_cli.voice as _voice
    except Exception:
