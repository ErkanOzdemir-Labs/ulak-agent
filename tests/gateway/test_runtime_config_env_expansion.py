...
def gateway_home(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("ULAK_PREFILL_MESSAGES_FILE", raising=False)
    monkeypatch.delenv("ULAK_EPHEMERAL_SYSTEM_PROMPT", raising=False)
    monkeypatch.delenv("ULAK_GATEWAY_BUSY_INPUT_MODE", raising=False)
    monkeypatch.delenv("ULAK_RESTART_DRAIN_TIMEOUT", raising=False)
    monkeypatch.delenv("ULAK_BACKGROUND_NOTIFICATIONS", raising=False)
    return tmp_path
