...
class TestDMTopicFallbackReplyToMode:
    ...
    Regression tests for https://github.com/ErkanOzdemir-Labs/ulak-agent/issues/23994:
    reply_to_mode 'off' was ignored when sending via Ulak-created DM topic
...
class TestDMTopicSyntheticSendRouting:
    ...
    Regression tests for https://github.com/ErkanOzdemir-Labs/ulak-agent/issues/87051:
    ...
    response in a different chat lane than the Ulak topic the session runs
    in. The no-anchor branch must prefer the Ulak topic's
    ``message_thread_id`` whenever it resolves.
    """

    def test_no_anchor_prefers_ulak_topic_thread_id(self):
