...
Proves, on windows-latest:
  1. `GatewayControlServer` binds ``\\\\.\\pipe\\ulak-gateway-<hash>`` via
...
os.environ["ULAK_HOME"] = sys.argv[2]
...
def test_named_pipe_identify_status_and_fleet_consumer(live_server, monkeypatch):
    ...
    assert ident["kind"] == "ulak-gateway"
    ...
    monkeypatch.setattr("ulak_cli.profiles._get_default_ulak_home", lambda: home)
    monkeypatch.setattr(
        "ulak_cli.profiles._get_profiles_root", lambda: home / "no-profiles"
    )
...
def test_pipe_gone_after_kill_falls_back(live_server, monkeypatch):
    ...
    (home / "gateway_state.json").write_text(
        json.dumps(
            {"pid": os.getpid(), "code_sha": "OLD", "kind": "ulak-gateway"}
        ),
        encoding="utf-8",
    )
    monkeypatch.setattr(
