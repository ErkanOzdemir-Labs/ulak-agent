...
(/opt/ulak/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable ULAK_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_ulak_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable ULAK_HOME."""
    ...
    monkeypatch.setattr(
        "ulak_constants.get_ulak_home", lambda: ulak_home
    )
    ...
    expected = ulak_home / "scripts" / "whatsapp-bridge"
