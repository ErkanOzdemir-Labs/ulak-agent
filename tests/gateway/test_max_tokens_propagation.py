...
Precedence verified here:
    ULAK_MAX_TOKENS env  >  model.max_tokens  >  per-provider
...
def isolated_home(tmp_path, monkeypatch):
    """Isolated ULAK_HOME with a writable config.yaml and a clean module cache.

    These tests deliberately re-import ``ulak_cli`` / ``gateway`` so each
    ...
    monkeypatch.delenv("ULAK_MAX_TOKENS", raising=False)

    _saved = {
        k: v
        for k, v in sys.modules.items()
        if k.startswith(("ulak_cli", "gateway"))
    }

    def write_cfg(body: str) -> None:
        (ulak_home / "config.yaml").write_text(textwrap.dedent(body))

    def fresh_gateway():
        for mod in list(sys.modules.keys()):
            if mod.startswith(("ulak_cli", "gateway")):
                del sys.modules[mod]
