...
Guards against the 60-vs-500 bug where a stale `.env ULAK_MAX_ITERATIONS=60`
...
def _run_gateway_import(ulak_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    script = textwrap.dedent(
        ...
        for k in (
            "ULAK_MAX_ITERATIONS",
            "ULAK_AGENT_TIMEOUT",
            "ULAK_AGENT_TIMEOUT_WARNING",
            "ULAK_TURN_LEASE_TIMEOUT",
            "ULAK_SESSION_STALL_TIMEOUT",
            "ULAK_GATEWAY_BUSY_INPUT_MODE",
            "ULAK_GATEWAY_BUSY_TEXT_MODE",
            "ULAK_GATEWAY_PLATFORM_CONNECT_TIMEOUT",
            "ULAK_TIMEZONE",
        ):
            v = os.environ.get(k)
            if v is not None:
    ...
    # otherwise intentionally clean so stale Ulak settings cannot leak in.
    for k in (
        "PATH",
        "PYTHONPATH",
        "VIRTUAL_ENV",
...
@pytest.fixture
def ulak_home(tmp_path: Path) -> Path:
    home = tmp_path / ".ulak"
    home.mkdir()
    return home


def test_config_gateway_timeout_wins_over_stale_env(ulak_home: Path) -> None:
    """Every agent.* bridge key must be config-authoritative, not .env-authoritative."""
    _write_config(ulak_home, agent_cfg={
        "gateway_timeout": 1800,
        "gateway_timeout_warning": 900,
        "session_stall_timeout": 300,
    })
    _write_env(ulak_home, {
        "ULAK_AGENT_TIMEOUT": "60",
        "ULAK_AGENT_TIMEOUT_WARNING": "30",
        "ULAK_SESSION_STALL_TIMEOUT": "15",
    })

    env = _run_gateway_import(ulak_home, initial_env={})

    assert env.get("ULAK_AGENT_TIMEOUT") == "1800"
    assert env.get("ULAK_AGENT_TIMEOUT_WARNING") == "900"
    assert env.get("ULAK_SESSION_STALL_TIMEOUT") == "300"


def test_config_turn_lease_timeout_wins_over_stale_env(ulak_home: Path) -> None:
    ...
    _write_env(
        ulak_home,
        {"ULAK_TURN_LEASE_TIMEOUT": "60"},
    )

    env = _run_gateway_import(ulak_home, initial_env={})

    assert env.get("ULAK_TURN_LEASE_TIMEOUT") == "600"
...
def test_default_turn_lease_timeout_overrides_stale_env_when_key_is_omitted(
    ...
    assert env.get("ULAK_TURN_LEASE_TIMEOUT") == "5"
...
def test_config_platform_connect_timeout_supplies_env_when_unset(ulak_home: Path) -> None:
    ...
    _write_config(ulak_home, gateway_cfg={"platform_connect_timeout": 90})

    env = _run_gateway_import(ulak_home, initial_env={})

    assert env.get("ULAK_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "90"


def test_env_platform_connect_timeout_wins_over_config(ulak_home: Path) -> None:
    """Unlike the agent.*/display.*/timezone bridges (config-authoritative),
    ULAK_GATEWAY_PLATFORM_CONNECT_TIMEOUT is the manual-override escape hatch:
    ...
    env = _run_gateway_import(
        ulak_home,
        initial_env={"ULAK_GATEWAY_PLATFORM_CONNECT_TIMEOUT": "120"},
    )

    assert env.get("ULAK_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "120"
