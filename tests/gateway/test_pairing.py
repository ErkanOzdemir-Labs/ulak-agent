...
class TestSplitPairingDirMigration:
    def test_merges_new_approved_into_active_legacy_dir(self, tmp_path):
        ...
        with patch("gateway.pairing.PAIRING_DIR", legacy), patch("gateway.pairing.get_ulak_home", return_value=home):
            store = PairingStore()
...
class TestProfileScopedDiscovery:
    def test_list_approved_scopes_platform_discovery_to_profile_dir(self, tmp_path):
        ...
        # A profile's store anchors to the ulak ROOT, not the current
        # ULAK_HOME — the current home may itself be a profile, and nesting
        # profiles inside profiles is how a `-p work` CLI and its gateway end
        # up reading different files. Patch that seam, not get_ulak_home.
        with patch("gateway.pairing.PAIRING_DIR", global_dir), patch(
            "gateway.pairing.get_default_ulak_root", return_value=home
        ):
            store = PairingStore(profile="alice")
            # Scoped under the mocked root's profile dir, using the same
            # consolidated layout a standalone `ulak -p alice` resolves —
...
class TestProfileScopedStorage:
    ...
    under each profile's own Ulak home so a multiplexing gateway can keep
    ...
    def test_default_store_uses_global_dir(self, tmp_path, monkeypatch):
        ...
        ``ulak pairing`` CLI continues to work without a profile context."""
        from ulak_constants import get_ulak_home
        monkeypatch.setattr("ulak_constants.get_ulak_home", lambda: tmp_path)
    ...
    def test_default_store_is_not_frozen_at_first_use(self, tmp_path, monkeypatch):
        ...
        whatever ULAK_HOME resolved to the first time this module's
        default path was computed. A long-lived process (the gateway,
        started once at container/process boot) can construct a
        PairingStore before ULAK_HOME/profile context is fully
        ...
        monkeypatch.setattr("ulak_constants.get_ulak_home", lambda: first_home)
        first_store = PairingStore()
        assert first_store._dir == first_home / "platforms" / "pairing"

        monkeypatch.setattr("ulak_constants.get_ulak_home", lambda: second_home)
    ...
    def test_profile_store_uses_profiles_subdir(self, tmp_path, monkeypatch):
        """Explicit profile stores use that profile's normal Ulak layout."""
        monkeypatch.setenv("ULAK_HOME", str(tmp_path))
    ...
    def test_profile_store_matches_profile_cli_home(self, tmp_path, monkeypatch):
        """Gateway and ``ulak -p`` must resolve the same pairing store."""
