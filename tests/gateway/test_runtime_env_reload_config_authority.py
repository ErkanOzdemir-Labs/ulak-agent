...
ULAK_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env ULAK_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``ulak config get terminal.backend`` still says local.
    """
    ulak_home = tmp_path / ".ulak"
    ulak_home.mkdir()
    (ulak_home / "config.yaml").write_text(
    ...
    (ulak_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
