...
def test_session_store_default_db_uses_runtime_ulak_home(tmp_path, monkeypatch):
    """SessionStore must honor runtime ULAK_HOME when opening the default DB.

    Regression for the import-time DEFAULT_DB_PATH freeze: importing
    ulak_state before a fixture redirected ULAK_HOME used to pin every
    default SessionDB() at the developer's real ~/.ulak/state.db.
    """
    config = GatewayConfig(default_reset_policy=SessionResetPolicy(mode="none"))
    fake_home = tmp_path / "alt_ulak_home"
    fake_home.mkdir()
    monkeypatch.setenv("ULAK_HOME", str(fake_home))
...
class TestReadmeSentinel:
    ...
    def test_save_writes_readme_sentinel_first(self, tmp_path):
        ...
        assert "ulak sessions list" in raw["_README"]
