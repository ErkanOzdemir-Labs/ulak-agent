...
#
# Installs a real, earlier Ulak the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `ulak`.
#
...
# Keep sandbox state out of the default .ulak-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".ulak-sandbox-e2e-$ROUTE"
export ULAK_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/ulak/.ulak/ulak-agent"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/ulak-agent.git"
...
LOG_DIR="${ULAK_E2E_LOG_DIR:-$(mktemp -d -t ulak-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED ulak accept FLAG on `ulak update`?
#
...
# that silently rots. `ulak update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "ulak update --help 2>&1" | grep -qF -- "$flag"
}
...
require_ulak_works() {
  local when="$1" out
  out="$(in_sandbox "ulak --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "ulak --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "ulak runs $when"
}

# ── install the earlier Ulak ─────────────────────────────────────────────
...
require_ulak_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: ulak update'
    ...
    # ulak which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="ulak update --yes"
    else
      update_cmd="ulak update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "ulak update failed ($update_cmd)"
    fi
    require_landed_on_target 'ulak update'
    require_ulak_works 'after ulak update'
    ;;
  installer)
    ...
    require_ulak_works 'after installer re-run'
    ;;
esac
