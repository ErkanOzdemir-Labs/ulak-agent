...
def test_repair_refuses_while_another_connection_holds_the_db(tmp_path):
    ...
    (``ulak_state_holders.live_writer_holds_db``) first scans for foreign
    holders — deleted sidecar generations, uninspectable Ulak processes —
    ...
    SQLite builds carrying the WAL-reset bug (and on NFS/SMB) Ulak runs
...
def test_repair_checks_foreign_holders_before_opening_sqlite(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(ulak_state_repair, "_connect_repair_durable", _unexpected_probe)
...
def test_linux_holder_scan_does_not_require_psutil(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(ulak_state_holders, "psutil", None)

    holders = ulak_state_holders.foreign_state_db_holders(
...
def test_incomplete_holder_scan_keeps_unknown_sentinel(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(ulak_state_holders.os, "readlink", lambda _path: str(db))
    real_stat = ulak_state_holders.os.stat
    ...
    holders = ulak_state_holders.foreign_state_db_holders(db)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (["python3", "-m", "ulak_cli.main", "gateway"], True),
    ),
)
def test_uninspectable_unknown_descriptor_uses_ulak_identity_at_repair_boundary(
    tmp_path, monkeypatch, argv, should_block
):
    """An unknown fd target blocks only when argv identifies Ulak."""
    db = _make_wal_db(tmp_path)
    ...
    if should_block:
        ...
        def _unexpected_probe(*_args, **_kwargs):
            pytest.fail("repair opened SQLite with an unproven Ulak descriptor")

        monkeypatch.setattr(
            ulak_state_repair, "_connect_repair_durable",
            _unexpected_probe,
        )
        report = repair_state_db_schema(db, backup=False)
        ...
        real_connect = ulak_state_repair._connect_repair_durable
        probe_reached = False
...
def test_uninspectable_watched_identity_blocks_alias_before_sqlite(
    ...
    monkeypatch.setattr(ulak_state_holders.os, "listdir", _listdir)
    monkeypatch.setattr(
        ulak_state_holders.os, "readlink", lambda _path: str(alias)
    )
    monkeypatch.setattr(ulak_state_holders.os, "stat", _stat)
...
@pytest.mark.linux_only
def test_uninspectable_alias_descriptor_for_ulak_blocks_before_sqlite(
    tmp_path, monkeypatch
):
    """Ulak cannot make an aliased fd safe when its identity is unreadable."""
    db = _make_wal_db(tmp_path)
    alias = tmp_path / "namespace-alias" / "state.db"
    ...
    monkeypatch.setattr(
        ulak_state_holders,
        "_read_proc_argv",
        lambda _pid: ["python3", "-m", "ulak_cli.main", "gateway"],
    )

    def _unexpected_probe(*_args, **_kwargs):
        pytest.fail("repair opened SQLite with an unproven Ulak alias fd")
