...
def progress(tmp_path):
    ...
    status = tmp_path / "ulak-update-status"
...
# Stands in for `ulak update`, and reports the stage that was on screen
...
n="$(cat "$ULAK_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$ULAK_TEST_CALLS"
for f in "$TMPDIR"/ulak-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$ULAK_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$ULAK_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "ulak-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    ulak = install_root / "venv" / "bin" / "ulak"
    ulak.write_text(FAKE_ULAK)
    ulak.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "ULAK_TEST_CAPTURE": str(capture),
        "ULAK_TEST_CALLS": str(calls),
        "ULAK_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".ulak-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`ulak update` is the longest wait in the hand-off and the one the
