...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.ulak/ulak-agent/utils.py)
...
2. ``ulak update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``ulak_cli/main.py`` (the ``ulak update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``ulak_constants`` after the
