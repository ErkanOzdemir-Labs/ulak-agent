...
The ``ulak update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m ulak_cli.main gateway run`` — image name
    ``pythonw``, not ``ulak.exe`` — so the ``taskkill /IM ulak.exe`` guard
    ...
    # The ulak.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM ulak.exe' in text
