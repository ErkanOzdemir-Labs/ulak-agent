...
The slash worker that parses ``/heartbeat`` runs a UlakCLI whose watchdog queues the due prompt
...
@pytest.fixture()
def server(ulak_home):
    with patch.dict("sys.modules", {"ulak_cli.env_loader": MagicMock(), "ulak_cli.banner": MagicMock()}):
...
def _arm_due(key: str):
    from ulak_cli.heartbeat import HeartbeatManager, save_heartbeat
...
def test_notification_poller_fires_due_heartbeat_when_idle(server, session):
    ...
    from ulak_cli.heartbeat import load_heartbeat
...
def test_heartbeat_tick_defers_when_busy_or_not_due(server, session, running, due):
    ...
    if not due:
        ...
        from ulak_cli.heartbeat import save_heartbeat
...
def test_abandon_fire_never_overwrites_a_concurrent_pause(ulak_home):
    from ulak_cli.heartbeat import HeartbeatManager, load_heartbeat
