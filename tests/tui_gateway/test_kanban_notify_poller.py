...
``platform="tui"`` / ``chat_id=ULAK_SESSION_KEY``, but no component ever
...
class TestCollectKanbanNotifications:
    ...
    def test_profile_scoped_session_reads_the_shared_board(self, tmp_path):
        ...
        ulak_cli/kanban_db.py module docstring): ``kanban_home()`` anchors on
        ``get_default_ulak_root()``, which resolves the process env and
        ...
        token = set_ulak_home_override(str(other_profile_home))
        try:
