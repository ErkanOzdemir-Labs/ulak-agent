...
def ulak_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves ULAK_HOME.
...
def server(ulak_home, monkeypatch):
    ...
    # Pin config resolution to the isolated ULAK_HOME. Sibling test
    ...
    # module-level ``_ulak_home = get_ulak_home()`` snapshot freezes
...
def test_moa_bare_returns_usage(server, session, ulak_home):
    _write_moa_config(ulak_home, """
moa:
