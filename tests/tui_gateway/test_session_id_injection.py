...
session id into ULAK_SESSION_ID so terminal/execute_code subprocesses can
...
saw an empty ULAK_SESSION_ID even though agent_init had set it via
...
def test_set_session_context_injects_agent_session_id(monkeypatch):
    """ULAK_SESSION_ID must equal the live agent.session_id after binding."""
    _install_session(
    ...
    assert get_session_env("ULAK_SESSION_ID") == "20260722_deadbeef"
