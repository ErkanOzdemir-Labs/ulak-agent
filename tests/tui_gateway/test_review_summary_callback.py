    ...
    with patch.dict(
        "sys.modules",
        {
            "ulak_constants": MagicMock(
                get_ulak_home=MagicMock(return_value="/tmp/ulak_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'ulak-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'ulak-release' patched"
    }
