...
guard is covered in ``tests/ulak_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``ulak_cli.model_switch_providers.prewarm_picker_cache_async``
...
import ulak_cli.model_switch_providers
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from ulak_cli.model_switch,
