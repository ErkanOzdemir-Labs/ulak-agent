...
``ULAK_HOME`` with named profile dirs — no mocks of the config/mcp layer — and
...
@pytest.fixture
def ulak_root(tmp_path, monkeypatch):
    """A temp ULAK_HOME root with two named profiles: 'work' and 'other'.

    Pointing ULAK_HOME at a dir outside ~/.ulak makes it the profile ROOT
    (get_default_ulak_root's Docker/custom branch), so named profiles live at
    ``<root>/profiles/<name>/`` and the launch/default profile is ``<root>``.
    """
    root = tmp_path / "ulak_home"
...
def test_add_lands_in_named_profile_only(ulak_root):
    root = ulak_root
    resp = _call(
        "mcp.servers.add",
        {
...
def test_list_reflects_the_scoped_profile(ulak_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_status_is_profile_scoped_and_credential_safe(ulak_root):
    _result(
        _call(
            "mcp.servers.add",
            {"profile": "work", "name": "svc-a", "config": {"command": "svc-a-bin"}},
...
def test_status_does_not_mix_launch_runtime_into_another_profile(ulak_root):
...
def test_status_includes_named_profile_runtime_in_multiplex(ulak_root):
    ...
    work_token = set_ulak_home_override(ulak_root / "profiles" / "work")
    try:
        work_scope = ulak_home_key()
    finally:
        reset_ulak_home_override(work_token)
...
def test_set_api_key_writes_env_and_header_to_right_profile(ulak_root):
    root = ulak_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_set_api_key_stdio_references_env_block(ulak_root):
    root = ulak_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_remove_scoped_to_profile(ulak_root):
    root = ulak_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_duplicate_and_missing_errors(ulak_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_requires_transport(ulak_root):
    resp = _call("mcp.servers.add", {"profile": "work", "name": "empty", "config": {}})
    assert "error" in resp
    assert resp["error"]["code"] == 4063


def test_default_profile_add_when_profile_omitted(ulak_root):
    root = ulak_root
    _result(
        _call(
            "mcp.servers.add",
    ...
    # Omitted profile → launch/default profile == ULAK_HOME root config.yaml.
    default_cfg = _read_yaml(root / "config.yaml")
