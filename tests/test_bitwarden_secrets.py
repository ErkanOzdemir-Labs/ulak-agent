...
binary download are exercised manually by `ulak secrets bitwarden
...
def ulak_home(tmp_path, monkeypatch):
    """Point Ulak at an isolated home directory."""
    home = tmp_path / ".ulak"
    home.mkdir()
    monkeypatch.setenv("ULAK_HOME", str(home))
    # Some modules cache get_ulak_home; clear if needed.
    import ulak_constants
    if hasattr(ulak_constants, "_ULAK_HOME_CACHE"):
        ulak_constants._ULAK_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(ulak_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from ulak_cli.env_loader import _apply_external_secret_sources
