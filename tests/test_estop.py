"""Global emergency stop (`ulak pause` / `ulak resume`) — agent/estop.py.
...
already in flight is never touched. Removing the sentinel (`ulak resume`)
...
def ulak_home(tmp_path, monkeypatch):
    """Point ULAK_HOME at a temp dir and reset estop module log state."""
...
def test_engage_creates_sentinel_and_is_engaged(ulak_home):
    assert estop.is_engaged() is False
    estop.engage()
    assert (ulak_home / "ESTOP").exists()
    assert estop.is_engaged() is True


def test_disengage_removes_sentinel(ulak_home):
    estop.engage()
    assert estop.disengage() is True
    assert not (ulak_home / "ESTOP").exists()
    assert estop.is_engaged() is False
    # Disengaging when not engaged is a no-op that reports False.
    assert estop.disengage() is False


def test_reason_and_timestamp_stored(ulak_home):
    ...
    raw = json.loads((ulak_home / "ESTOP").read_text(encoding="utf-8"))
    assert raw["reason"] == "runaway cron fan-out"


def test_get_state_none_when_disengaged(ulak_home):
    assert estop.get_state() is None


def test_corrupt_sentinel_still_engages(ulak_home):
    """A hand-touched/corrupt ESTOP file must still pause (fail safe)."""
    (ulak_home / "ESTOP").write_text("not json", encoding="utf-8")
    assert estop.is_engaged() is True
    state = estop.get_state()
    assert state is not None
...
def test_paused_reply_none_when_disengaged(ulak_home):
    assert estop.paused_reply() is None


def test_paused_reply_surfaces_reason_and_resume_hint(ulak_home):
    ...
    assert "ulak resume" in notice


def test_paused_reply_without_reason(ulak_home):
    estop.engage()
    notice = estop.paused_reply()
    assert notice is not None
    assert "paused" in notice.lower()
...
def test_check_paused_logs_once_per_engagement(ulak_home, caplog):
...
def test_cron_tick_skips_dispatch_when_engaged(ulak_home, monkeypatch):
...
def test_cron_tick_resumes_after_disengage(ulak_home, monkeypatch):
    from cron import scheduler
...
def test_kanban_dispatch_blocked_when_engaged(ulak_home):
...
@pytest.mark.asyncio
async def test_gateway_new_turn_gets_paused_reply(ulak_home):
...
@pytest.mark.asyncio
async def test_gateway_internal_events_bypass_estop(ulak_home):
...
# ── CLI: ulak pause / ulak resume ───────────────────────────────────────


def test_cli_pause_engages_with_reason(ulak_home, capsys):
    from ulak_cli.subcommands.pause import cmd_pause
...
def test_cli_pause_idempotent(ulak_home, capsys):
    from ulak_cli.subcommands.pause import cmd_pause
...
def test_cli_resume_disengages(ulak_home, capsys):
    from ulak_cli.subcommands.pause import cmd_pause, cmd_resume
...
def test_cli_resume_when_not_paused(ulak_home, capsys):
    from ulak_cli.subcommands.pause import cmd_resume
...
def test_builtin_subcommands_include_pause_resume():
    from ulak_cli.main import _BUILTIN_SUBCOMMANDS

    assert "pause" in _BUILTIN_SUBCOMMANDS
    assert "resume" in _BUILTIN_SUBCOMMANDS


# ── ulak status surfacing ─────────────────────────────────────────────────


def test_status_line_when_paused(ulak_home):
    from ulak_cli.status import _estop_status_line
...
def test_is_engaged_fails_safe_on_stat_error(ulak_home, monkeypatch):
    """A stat failure must report ENGAGED (fail safe) — the pause has to
    hold even when ULAK_HOME is misbehaving, matching the module's
...
@pytest.mark.asyncio
async def test_gateway_slash_commands_bypass_estop(ulak_home):
    ...
    try:
        ...
        return
    assert reply is None or "ulak is paused" not in (reply or "").lower()
...
@pytest.mark.asyncio
async def test_gateway_pause_command_engages_and_resumes(ulak_home):
    from gateway.run import GatewayRunner
...
def test_pause_command_registered_for_gateway():
    from ulak_cli.commands import GATEWAY_KNOWN_COMMANDS, resolve_command
...
def test_profile_gateway_honors_canonical_root_estop(tmp_path, monkeypatch):
    """fleet-analyst-class: ULAK_HOME is a profile dir; pause lives at root.

    A process launched with ULAK_HOME=~/.ulak/profiles/fleet-analyst must
    still treat ~/.ulak/ESTOP as engaged. Otherwise `ulak pause` is not
