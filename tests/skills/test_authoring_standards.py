...
def test_required_frontmatter_fields(p):
    ...
    ulak = (fm.get("metadata") or {}).get("ulak") or {}
    if not (ulak.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.ulak.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    ulak = (fm.get("metadata") or {}).get("ulak") or {}
    dangling = [
        rs for rs in (ulak.get("related_skills") or []) if rs not in _all_names()
    ]
