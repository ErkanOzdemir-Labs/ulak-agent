...
def test_frontmatter_required_fields():
    ...
    ulak = fm["metadata"]["ulak"]
    assert ulak["tags"]
    assert "related_skills" in ulak
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("Ulak Agent"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["ulak"]["related_skills"]:
