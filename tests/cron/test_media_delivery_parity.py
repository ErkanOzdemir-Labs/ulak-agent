...
text-only on manual ``ulak cron run <job-id>``. Same box, same token, same
...
def _install_fake_slack_sender(monkeypatch, result_factory):
    ...
    if entry is None:
        ...
        import ulak_cli.plugins as hp_boot
...
def slack_platform_config(monkeypatch, tmp_path):
    ...
    # Config caches are process-global; clear them so the temp ULAK_HOME wins.
    try:
...
class TestMediaPolicyEnvBridge:
    ...
    def test_bridge_helper_exists_and_applies_config(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("ULAK_HOME", str(home))
        for var in (
            "ULAK_MEDIA_DELIVERY_STRICT",
            "ULAK_MEDIA_ALLOW_DIRS",
            "ULAK_MEDIA_TRUST_RECENT_FILES",
        ):
        ...
        assert os.environ.get("ULAK_MEDIA_DELIVERY_STRICT") == "1"
        assert str(allow_dir) in os.environ.get("ULAK_MEDIA_ALLOW_DIRS", "")
        assert os.environ.get("ULAK_MEDIA_TRUST_RECENT_FILES") == "0"
    ...
    def test_standalone_filter_honors_bridged_allowlist(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("ULAK_HOME", str(home))
        for var in ("ULAK_MEDIA_DELIVERY_STRICT", "ULAK_MEDIA_ALLOW_DIRS"):
            monkeypatch.delenv(var, raising=False)
    ...
    def test_deliver_result_runs_bridge(self, monkeypatch, tmp_path, media_file):
        ...
        Realistic enterprise shape: ULAK_MEDIA_DELIVERY_STRICT=1 arrives via
