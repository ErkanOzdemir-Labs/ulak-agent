...
- Unlimited timeout (ULAK_CRON_TIMEOUT=0)
- Backward compat: ULAK_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """ULAK_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """ULAK_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("ULAK_CRON_TIMEOUT", "1200")
...
class TestSysPathOrdering:
    ...
    def test_ulak_time_importable(self):
        """ulak_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _ulak_now
        assert callable(_ulak_now)
