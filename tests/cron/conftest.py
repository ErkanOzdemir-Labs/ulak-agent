...
Provides a default ``ULAK_MODEL`` for cron run_job tests so each one
doesn't have to spell out a model. The global conftest blanks
ULAK_MODEL hermetically; without this autouse fixture every cron test
that exercises ``run_job`` would hit the fail-fast guard added in
``cron/scheduler.py`` (see issue #23979) and have to be rewritten.

Tests that specifically need ``ULAK_MODEL`` unset — model-resolution
edge cases — call ``monkeypatch.delenv("ULAK_MODEL", raising=False)``
...
def _default_cron_test_model(monkeypatch):
    """Pin a default ULAK_MODEL so cron run_job tests have a resolvable model."""
    monkeypatch.setenv("ULAK_MODEL", "test-cron-default-model")
    yield
