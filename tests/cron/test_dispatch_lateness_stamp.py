...
kind) on every recurring dispatch so `ulak cron list` and
`ulak cron status` can surface late catch-ups.
"""
...
def cron_store(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("cron.jobs._ulak_now", lambda: FIXED_NOW)
    return tmp_path
...
class TestDueScanDispatchStamp:
    def test_catch_up_beyond_grace_stamped_and_persisted(self, cron_store):
        ...
        # Persisted, so a separate `ulak cron list` process can read it.
