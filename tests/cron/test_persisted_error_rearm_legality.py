...
def cron_store(tmp_path, monkeypatch):
    ...
    (ulak_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("ULAK_HOME", str(ulak_home))
    monkeypatch.setattr(J, "ULAK_DIR", ulak_home)
    monkeypatch.setattr(J, "CRON_DIR", ulak_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", ulak_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", ulak_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_ulak_now", lambda: now):
            due = J.get_due_jobs()
