...
CronScheduler plan, .ulak/plans/cron-scheduler-provider-interface.md). They
...
No production code is exercised beyond the two ticker entry points:
  ...
  - ulak_cli/web_server.py::_start_desktop_cron_ticker  (desktop fallback)
...
def test_desktop_ticker_calls_tick_then_stops():
    ...
    no adapters/loop."""
    from ulak_cli.web_server import _start_desktop_cron_ticker
...
def test_multiplex_recovery_isolates_profile_failures(tmp_path):
    ...
    def _tick(*args, **kwargs):
        tick_homes.append(str(get_ulak_home()))
        if len(tick_homes) >= 2:
            stop.set()
        return 0
