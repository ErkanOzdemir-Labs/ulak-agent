...
profile's ULAK_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.ulak/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active ULAK_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever ULAK_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    ULAK_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import ulak_constants

    monkeypatch.setattr(
        ulak_constants, "_get_platform_default_ulak_home", lambda: root
    )
    monkeypatch.setenv("ULAK_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile ULAK_HOME (<root>/profiles/<name>), the cron store
    ...
    assert ulak_constants.get_ulak_home().resolve() == profile_home.resolve()
    assert ulak_constants.get_default_ulak_root().resolve() == root.resolve()

    # cron/jobs.py computes ULAK_DIR from get_ulak_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.ULAK_DIR.resolve() == profile_home.resolve()
        assert (
