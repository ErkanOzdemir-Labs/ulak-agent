...
(``ulak cron list`` / ``ulak cron status``); the agent path did not.
...
def ulak_env(tmp_path, monkeypatch):
    """Isolate ULAK_HOME for each test so jobs don't leak."""
    home = tmp_path / ".ulak"
    home.mkdir()
    (home / "cron").mkdir()
...
class TestCreateSurfacesGatewayLiveness:
    def test_create_with_gateway_running_has_no_warning(self, ulak_env):
    ...
    def test_create_without_gateway_warns_not_scheduled(self, ulak_env):
        with (
    ...
    def test_non_builtin_provider_is_exempt(self, ulak_env):
    ...
    def test_failed_probe_stays_neutral(self, ulak_env):
...
class TestListSurfacesGatewayLiveness:
    ...
    def test_list_with_gateway_running_has_no_warning(self, ulak_env):
    ...
    def test_list_without_gateway_warns_jobs_inert(self, ulak_env):
        _create_job()
    ...
    def test_list_empty_without_gateway_stays_quiet(self, ulak_env):
    ...
    def test_list_non_builtin_provider_is_exempt(self, ulak_env):
        _create_job()
        with patch_liveness(provider="chronos", pids=[]):
            result = self._list_jobs()
...
class _LivenessPatches:
    ...
    def __enter__(self):
        ...
        self._stack.enter_context(
            patch(
                "ulak_cli.cron._active_cron_provider_name",
        ...
        self._stack.enter_context(
            patch(
                "ulak_cli.gateway.named_profile_served_by_running_multiplexer",
                return_value=False,
            )
        )
        self._stack.enter_context(
...
class TestRuntimeLockFirstLiveness:
    ...
    def test_lock_active_reports_alive_despite_empty_pid_scan(self, ulak_env):
    ...
    def test_lock_inactive_falls_back_to_pid_scan(self):
        ...
        import ulak_cli.cron as cron_cli

        with (
            patch("ulak_cli.cron._active_cron_provider_name", return_value="builtin"),
            patch("gateway.status.is_gateway_runtime_lock_active", return_value=False),
            patch("ulak_cli.gateway.find_gateway_pids", return_value=[424242]),
        ):
    ...
    def test_no_lock_no_pids_is_false(self):
        ...
        with (
            patch("ulak_cli.cron._active_cron_provider_name", return_value="builtin"),
            patch("gateway.status.is_gateway_runtime_lock_active", return_value=False),
            patch("ulak_cli.gateway.find_gateway_pids", return_value=[]),
            patch(
                "ulak_cli.gateway.named_profile_served_by_running_multiplexer",
                return_value=False,
            ),
...
class TestCronStatusLockFirst:
    """`ulak cron status` shares the lock-first false-alarm fix (#95947).
    ...
    def _run_status(self, *, pids, lock_active, lock_pid=None):
        ...
        with (
            patch("ulak_cli.cron._active_cron_provider_name", return_value="builtin"),
            patch("ulak_cli.gateway.find_gateway_pids", return_value=list(pids)),
            patch(
                "gateway.status.is_gateway_runtime_lock_active",
    ...
    def test_lock_active_suppresses_not_running_false_alarm(self, ulak_env):
    ...
    def test_no_lock_no_pids_still_warns(self, ulak_env):
