"""display_ulak_home() renders POSIX separators on every platform.

Maintainer catch (#95681 arc): on Windows with a custom ULAK_HOME under
the user profile (e.g. AppData/Local/ulak), ``"~/" +
str(home.relative_to(Path.home()))`` produced the mixed-separator chimera
``~/AppData\\Local\\ulak`` — which then leaked into every consumer that
appends sub-paths (the skill_manage schema showed the agent
``~/AppData\\Local\\ulak/skills/``). The ``~/`` shorthand implies POSIX
...
class TestDisplayUlakHomePosix(unittest.TestCase):
    def test_nested_home_renders_forward_slashes(self):
        ...
        nested = fake_userhome / "AppData" / "Local" / "ulak"
        with patch.object(hc, "get_ulak_home", return_value=nested):
            out = hc.display_ulak_home()
        self.assertEqual(out, "~/AppData/Local/ulak")
        self.assertNotIn("\\", out)

    def test_default_home_unchanged(self):
        import ulak_constants as hc

        with patch.object(hc, "get_ulak_home",
                          return_value=Path.home() / ".ulak"):
            out = hc.display_ulak_home()
        self.assertEqual(out, "~/.ulak")

    def test_outside_home_falls_back_to_absolute(self):
        import ulak_constants as hc

        outside = Path("/opt/ulak-custom") if os.name != "nt" else Path("C:/opt/ulak-custom")
        with patch.object(hc, "get_ulak_home", return_value=outside):
            out = hc.display_ulak_home()
