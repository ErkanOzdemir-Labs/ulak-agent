"""Guard: Ulak-owned subprocesses must not resolve managed runtimes by bare PATH.

Ulak installs runtimes for itself — ``uv`` at ``$ULAK_HOME/bin/uv``, Node at
``$ULAK_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Ulak's own
...
* when a system copy also exists, the one Ulak does not own wins — which is
...
``iter_ulak_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Ulak provisions into ULAK_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Ulak-owned subprocess code: plugins ship their own
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("ulak_cli/update_cmd_deps.py", "uv"): (
    ...
    ("ulak_cli/update_cmd_deps.py", "npm"): (
...
def _source_files() -> list[Path]:
    ...
    # (ulak_agent-<version>/) gets created and deleted concurrently, and
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Ulak-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$ULAK_HOME/bin (uv) and $ULAK_HOME/node are not on an "
        ...
        "  node/npm -> ulak_constants.find_node_executable()\n"
        "  PATH env -> ulak_constants.iter_ulak_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(ulak_constants, helper))


def test_managed_uv_helpers_exist():
    from ulak_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
