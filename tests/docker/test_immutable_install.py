...
Build the real image and verify at runtime:
  ...
  1. /opt/ulak is not writable by the ulak user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and ULAK_DISABLE_LAZY_INSTALLS are set
  3. /opt/ulak/.install_method contains "docker" (code-scoped stamp)
  4. $ULAK_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $ULAK_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_ulak(
    built_image: str, container_name: str,
) -> None:
    """The ulak user must not be able to modify /opt/ulak.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/ulak as the ulak user
        "touch /opt/ulak/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"ulak user can write to /opt/ulak (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/ulak/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"ulak user can write to /opt/ulak/.venv: {r.stdout}"
    )


def test_ulak_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
    """The container must set PYTHONDONTWRITEBYTECODE and
    ULAK_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $ULAK_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $ULAK_HOME was not healed on boot: "
        f"{r.stdout}"
    )
