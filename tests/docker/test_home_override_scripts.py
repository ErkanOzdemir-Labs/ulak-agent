...
def test_dashboard_service_resets_home(
    ...
    container with ULAK_DASHBOARD=1 and verify the dashboard process
...
def test_dashboard_does_not_auto_insecure_from_host(
    ...
    ULAK_DASHBOARD_HOST. The auth gate is the authority now.
    ...
    """
    start_container(built_image, container_name, "ULAK_DASHBOARD=1", "ULAK_DASHBOARD_HOST=0.0.0.0")
...
def test_stage2_repairs_profiles_and_cron_ownership(
    ...
    The stage2 hook chowns these dirs to ulak:ulak on every boot.
    ...
    # Verify files are now owned by ulak.
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/profiles/testprof/marker '
        '/opt/data/cron/root_owned.json',
    ...
    assert "ulak" in r.stdout, (
        f"expected ulak-owned files after restart, got: {r.stdout!r} — "
