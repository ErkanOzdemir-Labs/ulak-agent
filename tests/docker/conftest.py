...
Override the image with ``ULAK_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("ULAK_TEST_IMAGE", "ulak-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``ULAK_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("ULAK_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged ulak user
# ---------------------------------------------------------------------------
#
# Background: every Ulak runtime path inside the container drops to UID
# 10000 (the ``ulak`` user) via ``s6-setuidgid ulak``. ``docker exec``
...
# fails (PermissionError swallowed) for ulak, so a test that ran as root
...
# helpers below run every probe under ``-u ulak`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: ulak).
