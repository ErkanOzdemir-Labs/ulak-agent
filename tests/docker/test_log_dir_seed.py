"""Runtime smoke test for Docker $ULAK_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the ulak user after container boot.
...
registered later runs its log service as the dropped ulak user and
...
def test_logs_gateways_seeded_and_ulak_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by ulak after boot."""
    ...
    assert "logs=ulak" in r.stdout, (
        f"logs/ not owned by ulak: {r.stdout}"
    )
    assert "gateways=ulak" in r.stdout, (
        f"logs/gateways/ not owned by ulak: {r.stdout}"
    )
