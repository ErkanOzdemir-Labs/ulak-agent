...
from ulak_state_common import FTS_CJK_STALE_KEY
...
def db(cjk_so, tmp_path, monkeypatch):
    monkeypatch.setenv("ULAK_FTS5_CJK_SO", str(cjk_so))
...
def test_config_toggle_disables_cjk(cjk_so, tmp_path, monkeypatch):
    monkeypatch.setenv("ULAK_FTS5_CJK_SO", str(cjk_so))
    monkeypatch.setenv("ULAK_CJK_FTS", "0")
    d = SessionDB(db_path=tmp_path / "state.db")
    try:
...
def test_existing_v23_db_gains_cjk_via_optimize(cjk_so, tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ULAK_FTS5_CJK_SO", str(tmp_path / "absent.so"))
