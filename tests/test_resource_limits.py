...
from ulak_cli import resource_limits
...
def test_fresh_process_import_without_posix_resource_is_a_safe_noop():
    code = textwrap.dedent(
        ...
        spec = importlib.util.spec_from_file_location(
            "ulak_cli._resource_limits_without_posix_resource",
            module_path,
        )
...
def test_serve_startup_applies_limit_before_web_server(monkeypatch):
    ...
    import ulak_cli.plugins
    import ulak_cli.web_server

    # cmd_dashboard(headless_backend=True) exports ULAK_SERVE_HEADLESS=1 into
    ...
    # 404 path.
    monkeypatch.setenv("ULAK_SERVE_HEADLESS", "0")
    ...
    monkeypatch.setattr(ulak_cli.plugins, "discover_plugins", lambda: None)
    monkeypatch.setattr(
        ulak_cli.web_server,
        "start_server",
...
def test_named_profile_reroute_defers_limit_to_final_process(monkeypatch, tmp_path):
    ...
    assert calls == []
    assert exec_call["argv"][1:5] == ["-m", "ulak_cli.main", "-p", "default"]
    assert exec_call["env"]["ULAK_HOME"] == str(tmp_path)
...
def test_dashboard_lifecycle_flags_skip_limit_adjustment(monkeypatch, lifecycle_flag):
    ...
    from ulak_cli import main as cli_main
    import ulak_cli.main_dashboard as ulak_cli_main_dashboard
    ...
    monkeypatch.setattr(ulak_cli_main_dashboard, "_find_stale_dashboard_pids", lambda: [])
