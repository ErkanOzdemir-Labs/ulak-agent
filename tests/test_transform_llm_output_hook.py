...
from ulak_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(ulak_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <ulak_home>/plugins/<name> and opt it in."""
    plugin_dir = ulak_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ULAK_HOME", str(tmp_path / "ulak_empty"))
