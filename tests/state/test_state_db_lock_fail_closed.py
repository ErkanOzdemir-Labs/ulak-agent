...
work on a file several Ulak processes share (gateway service, the Desktop
app's `ulak serve` backend, CLI sessions, the TUI slash worker):

* `ulak_state_common.fts_rebuild_admission` — full structural FTS rebuilds
* `ulak_state_repair._cross_process_repair_lock`   — writable_schema surgery / VACUUM
