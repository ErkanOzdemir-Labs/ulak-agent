...
import ulak_state
import ulak_state_holders
import ulak_state_schema
from ulak_state import SessionDB
from ulak_state_common import FTS_REBUILD_DEFERRAL_KEY, FTS_STALE_KEY, LEGACY_FTS_SQL, LEGACY_FTS_TRIGRAM_SQL, SCHEMA_SQL, _FTS_TRIGGERS
from ulak_state_dbfile import _concrete_state_db_holder_pids, _is_inactive_orphan_desktop_holder
...
class TestRuntimeFtsRebuild:
    def test_reap_candidates_exclude_uninspectable_holder_suspicions(
        ...
        assert _concrete_state_db_holder_pids(
            db_path,
            [
                (222, "uninspectable holder: python -m ulak_cli.main serve --port 0"),
    ...
    @pytest.mark.parametrize(
        "argv",
        (
            ("journalctl", "-u", "ulak-agent.service"),
            ("grep", "ulak-agent", "/var/log/syslog"),
            (
                "/usr/sbin/tailscaled",
                "be-child",
                "ssh",
                "--cmd=python -m ulak_cli.main gateway",
            ),
            ("tmux", "new-session", "/opt/ulak-agent/.venv/bin/ulak gateway"),
            ("python3", "/opt/ulak-agent/tools/check_state.py"),
            ("ulak-monitor", "gateway"),
            ("ulakctl", "serve"),
            ("python3", "worker.py", "ulak_cli.main"),
            ("python3", "-m", "other.module", "ulak_cli.main"),
            ("python3", "-c", "ulak_cli.main"),
            ("python3", "-Icprint('ulak_cli.main')", "ulak_cli/main.py"),
        ),
    )
    def test_uninspectable_non_ulak_process_is_not_a_holder(self, argv):
        assert not ulak_state_holders._looks_like_ulak(argv)

    @pytest.mark.parametrize(
        "argv",
        (
            ("/usr/local/bin/ulak", "gateway"),
            ("/usr/local/bin/ulak-agent", "serve"),
            ("/usr/local/bin/ulak-acp", "--stdio"),
            ("/usr/bin/python3", "-m", "ulak_cli.main", "gateway"),
            ("/usr/bin/python3", "-m", "acp_adapter"),
            ("/usr/bin/python3", "-Im", "ulak_cli.main", "gateway"),
            ("/usr/bin/python3", "-mulak_cli.main", "gateway"),
            ("/usr/bin/python3", "-W", "ignore", "-m", "ulak_cli.main"),
            ("/usr/bin/python3", "-Xdev", "-m", "ulak_cli.main"),
            (
                "/opt/ulak-agent/.venv/bin/python",
                "/opt/ulak-agent/ulak_cli/main.py",
                "gateway",
            ),
            ("python.exe", "--", "ulak_cli/main.py", "gateway"),
            ("python3", "/opt/ulak-agent/run_agent.py", "--query", "hello"),
        ),
    )
    def test_uninspectable_ulak_process_remains_a_holder(self, argv):
        assert ulak_state_holders._looks_like_ulak(argv)
    ...
    def test_foreign_holder_detection_proc_readlink_deleted_wal(
        ...
        monkeypatch.setattr(ulak_state_holders.os, "getpid", lambda: 111)
        real_listdir = os.listdir
        def _listdir(path):
            if isinstance(path, str):
                path = path.replace("/proc", str(proc_root))
            return real_listdir(path)
        monkeypatch.setattr(ulak_state_holders.os, "listdir", _listdir)
        real_readlink = os.readlink
        def _readlink(path):
            path = path.replace("/proc", str(proc_root))
            return real_readlink(path)
        monkeypatch.setattr(ulak_state_holders.os, "readlink", _readlink)
        real_stat = os.stat
        ...
        monkeypatch.setattr(ulak_state_holders.os, "stat", _stat)

        holders = ulak_state_holders.foreign_state_db_holders(db_path)
    ...
    def test_foreign_holder_ignores_same_path_with_different_file_identity(
        ...
        assert ulak_state_holders.foreign_state_db_holders(db_path) == []
    ...
    def test_foreign_holder_uninspectable_process_cmdline_fallback(
        ...
        flagged when /proc/<pid>/cmdline identifies it as a Ulak process."""
        db_path = tmp_path / "state.db"
        ...
        # PID 222's cmdline is world-readable and looks like Ulak
        cmdline_path = proc_root / "222" / "cmdline"
        cmdline_path.write_bytes(
            b"python3\x00-m\x00ulak_cli.main\x00chat\x00"
        )
        ...
        def _fake_argv(pid):
            ...
            try:
                ...
                return None
        monkeypatch.setattr(ulak_state_holders, "_read_proc_argv", _fake_argv)

        holders = ulak_state_holders.foreign_state_db_holders(db_path)
        # Should include PID 222 with the cmdline info
        assert len(holders) == 1
        assert holders[0][0] == 222
        assert "ulak_cli.main" in holders[0][1]
    ...
    def test_structural_corruption_propagates_without_live_fts_mutation(
        ...
        from ulak_state import StateDbCorruptError
    ...
    def test_repeated_deferrals_reap_inactive_orphan_then_rebuild(
        ...
        monkeypatch.setattr(ulak_state_schema.time, "time", lambda: 120.0)
...
class TestPhysicalCorruptionAcceptance:
    ...
    def test_canonical_btree_corruption_fails_closed(
        ...
        try:
            caplog.clear()
            with caplog.at_level("WARNING", logger="ulak_state"):
                with pytest.raises(sqlite3.DatabaseError) as caught:
