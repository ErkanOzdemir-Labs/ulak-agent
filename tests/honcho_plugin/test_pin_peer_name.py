...
Under a gateway (Telegram, Discord, Slack, ...) Ulak passes the
...
class TestPinPeerNameConfigParsing:
    ...
    def test_root_level_true(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ULAK_HOME", str(tmp_path / "isolated"))
...
class TestCrossPlatformMemoryUnification:
    """The same physical user talking to Ulak via Telegram AND Discord
...
class TestProfilePeerUniqueness:
    """Each Ulak profile can pin to its own unique peerName.

    Profile cloning copies host blocks, but operators routinely diverge them
    afterwards (e.g. `ulak -p partner` pinned to a different person's peer).
