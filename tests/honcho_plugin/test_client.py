...
from ulak_cli.profiles import _get_default_ulak_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "ulak"
        assert config.workspace_id == "ulak"
...
class TestFromGlobalConfig:
    ...
    def test_base_url_host_block_overrides_root_and_env(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"ulak": {"baseUrl": "http://host-block:9001"}},
        }))
    ...
    def test_base_url_full_precedence_chain(self, tmp_path):
        ...
        config_file = tmp_path / "config.json"
        layers = {
            "hosts": {"ulak": {"baseUrl": "http://host:1"}},
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="ulak-agent"
        ):
            result = config.resolve_session_name("/home/user/ulak-agent/subdir")
        assert result == "ulak-agent"


class TestResolveConfigPath:
    def test_prefers_ulak_home_when_exists(self, tmp_path):
        ulak_home = tmp_path / "ulak"
        ulak_home.mkdir()
        local_cfg = ulak_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: ULAK_HOME points at ~/.ulak/profiles/<name>, so
        # _get_default_ulak_home() must resolve back to ~/.ulak — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".ulak"
        ...
        assert _get_default_ulak_home() == default_home
        assert result == default_cfg


class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "ulak_coder"
        assert profile_host_key("default") == "ulak"


    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"ULAK_HONCHO_HOST": "ulak.coder"}):
            assert resolve_active_host() == "ulak.coder"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False), patch(
            ...
            os.environ.pop("ULAK_HONCHO_HOST", None)
            # Temporarily remove ulak_cli.profiles to simulate import failure
            saved = sys.modules.get("ulak_cli.profiles")
            sys.modules["ulak_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "ulak"
            finally:
                if saved is not None:
                    sys.modules["ulak_cli.profiles"] = saved
                else:
                    sys.modules.pop("ulak_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="ulak_coder")
        assert config.host == "ulak_coder"
        assert config.workspace_id == "ulak"  # shared workspace
        assert config.ai_peer == "ulak_coder"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"ulak": {"enabled": True, "aiPeer": "ulak"}},
        }))
...
class TestGetHonchoClient:
    ...
    def test_dot_form_legacy_host_key_keeps_local_api_key(self):
        """Regression for #37436: a legacy dot-form host block (ulak.work)
        ...
        cfg = HonchoClientConfig(
            ...
            host="ulak_work",
            workspace_id="ulak",
            raw={"hosts": {"ulak.work": {"apiKey": "explicit-local-key"}}},
        )
    ...
    def test_timeout_change_triggers_client_rebuild(self):
        ...
        cfg_yaml = get_ulak_home() / "config.yaml"
    ...
    def test_managed_config_timeout_does_not_thrash_singleton(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ULAK_MANAGED_DIR", str(managed_dir))
...
class TestGetHonchoClientBaseUrlDoublePrefixFix:
    ...
    def test_lan_default_host_empty_key_uses_local_placeholder(self, tmp_path):
        ...
        with patch.dict(os.environ, {}, clear=True), \
             patch("ulak_cli.profiles.get_active_profile_name", return_value="default"), \
