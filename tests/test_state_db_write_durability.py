...
The defect: ulak_state already knows macOS `fsync()` does not guarantee
...
import ulak_state
from ulak_state import repair_state_db_schema
from ulak_state_repair import _connect_repair_durable
...
def test_repair_path_has_no_bare_connects() -> None:
    ...
    """
    # The repair/probe helpers live in ulak_state_repair; ulak_state only
    # re-imports them.
    import ulak_state_repair

    source = Path(ulak_state_repair.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(ulak_state_repair.__file__))
