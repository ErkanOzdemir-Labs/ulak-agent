...
def _isolate(tmp_path, monkeypatch):
    """Isolate ULAK_HOME so tests don't touch real config."""
    home = tmp_path / ".ulak"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal UlakCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        "display": {"tool_progress": "new"},
        "terminal": {},
    }), patch("cli.get_tool_definitions", return_value=[]), \
         patch("ulak_cli.banner.build_welcome_banner"):
        from cli import UlakCLI
        obj = UlakCLI.__new__(UlakCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["ulak-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Ulak' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Ulak' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Ulak' minimum context length."""
