...
def _real_finalize(cli_obj):
    """Call the real UlakCLI.finalize_preloaded_skills on a dummy object."""
    return _REAL_FINALIZE(cli_obj)


def _capture_real_finalize():
    import cli as cli_mod
    return cli_mod.UlakCLI.__dict__["finalize_preloaded_skills"]
...
def test_main_applies_preloaded_skills_to_system_prompt(monkeypatch):
    ...
    monkeypatch.setattr(cli_mod, "UlakCLI", fake_cli)
    monkeypatch.setattr(
        cli_mod,
        "build_preloaded_skills_prompt",
        lambda skills, task_id=None: ("skill prompt", ["ulak-agent-dev", "github-auth"], []),
    )

    with pytest.raises(SystemExit):
        cli_mod.main(skills="ulak-agent-dev,github-auth", list_tools=True)
    ...
    assert cli_obj.preloaded_skills == ["ulak-agent-dev", "github-auth"]
...
def test_show_banner_does_not_print_skills():
    ...
    cli_obj.preloaded_skills = ["ulak-agent-dev", "github-auth"]
    cli_obj.console = MagicMock()

    with patch("ulak_cli.banner.build_welcome_banner") as mock_banner, patch(
