...
minimal ``UlakCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def ulak_home(tmp_path, monkeypatch):
    """Isolated ULAK_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".ulak"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("ULAK_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves ULAK_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal UlakCLI stub with an active goal wired in."""
    from cli import UlakCLI
    from ulak_cli.goals import GoalManager

    cli = UlakCLI.__new__(UlakCLI)
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, ulak_home):
        ...
        with patch("ulak_cli.goals.judge_goal"):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, ulak_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, ulak_home):
        ...
        src = inspect.getsource(UlakCLI.chat)
