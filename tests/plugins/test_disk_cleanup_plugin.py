...
def _isolate_env(tmp_path, monkeypatch):
    """Isolate ULAK_HOME for each test.

    The global hermetic fixture already redirects ULAK_HOME to a tempdir,
    but we want the plugin to work with a predictable subpath. We reset
    ULAK_HOME here for clarity.
    """
...
def _load_plugin_init():
    ...
    spec = importlib.util.spec_from_file_location(
        "ulak_plugins.disk_cleanup",
        plugin_dir / "__init__.py",
        submodule_search_locations=[str(plugin_dir)],
    )
    # Ensure parent namespace package exists for the relative `. import disk_cleanup`
    import types
    if "ulak_plugins" not in sys.modules:
        ns = types.ModuleType("ulak_plugins")
        ns.__path__ = []
        sys.modules["ulak_plugins"] = ns
    mod = importlib.util.module_from_spec(spec)
    mod.__package__ = "ulak_plugins.disk_cleanup"
    mod.__path__ = [str(plugin_dir)]
    sys.modules["ulak_plugins.disk_cleanup"] = mod
    spec.loader.exec_module(mod)
    return mod
...
class TestIsSafePath:
    def test_accepts_path_under_ulak_home(self, _isolate_env):
        dg = _load_lib()
    ...
    def test_rejects_outside_ulak_home(self, _isolate_env):
        dg = _load_lib()
...
class TestBundledDiscovery:
    def _write_enabled_config(self, ulak_home, names):
    ...
    def test_disk_cleanup_discovered_but_not_loaded_by_default(self, _isolate_env):
        ...
        from ulak_cli import plugins as pmod
