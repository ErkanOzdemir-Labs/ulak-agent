...
def provider(monkeypatch, tmp_path):
    ...
    p.initialize("session-1", ulak_home=str(tmp_path), platform="cli")
    return p
...
def test_format_prefetch_context_deduplicates_overlap():
    result = _format_prefetch_context(
        ...
        dynamic_facts=["Jordan prefers short answers", "Uses Ulak"],
        search_results=[{"memory": "Uses Ulak", "similarity": 0.9}],
        max_results=10,
    )
    assert result.count("Jordan prefers short answers") == 1
    assert result.count("Uses Ulak") == 1
...
def test_prefetch_includes_profile_on_first_turn(provider):
    provider._client.profile_response = {
        ...
        "search_results": [{"memory": "Working on Ulak memory provider", "similarity": 0.88}],
    }
...
def test_merge_metadata_stamps_sm_source():
    # sm_source routes Ulak writes into the "Ulak" Space in the Supermemory
...
def test_identity_template_resolved_in_container_tag(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("SUPERMEMORY_API_KEY", "test-key")
    monkeypatch.setattr("plugins.memory.supermemory._SupermemoryClient", FakeClient)
    _save_supermemory_config({"container_tag": "ulak-{identity}"}, str(tmp_path))
    p = SupermemoryMemoryProvider()
    p.initialize("s1", ulak_home=str(tmp_path), platform="cli", agent_identity="coder")
    assert p._container_tag == "ulak_coder"
...
def test_container_tag_env_var_override(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("plugins.memory.supermemory._SupermemoryClient", FakeClient)
    p = SupermemoryMemoryProvider()
    p.initialize("s1", ulak_home=str(tmp_path), platform="cli")
...
def test_probe_supermemory_connection_missing_key(tmp_path):
    ...
    assert status["container_tag"] == "ulak"
...
def test_post_setup_writes_config_and_prints_summary(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr("ulak_cli.config.save_config", fake_save_config)
