...
_EXPECTED_USER_AGENT = f"openviking-memory-ulak/{_ULAK_VERSION}"
...
def test_openviking_provider_config_loader_uses_readonly_config(monkeypatch):
    ...
    config = openviking_module._load_ulak_openviking_config()
...
""",
    ...
    settings = openviking_module._resolve_connection_settings(
        openviking_module._load_ulak_openviking_config()
    )
...
def test_post_setup_existing_profile_picker_validates_and_links_saved_profile(
    ...
    from ulak_cli import memory_setup
    ...
    OpenVikingMemoryProvider().post_setup(str(ulak_home), config)
...
def test_start_local_openviking_server_strips_pythonpath_from_child_env(monkeypatch):
    """The spawned server must not inherit Ulak's PYTHONPATH (#78153).

    Inheriting it makes openviking-server import packages from the Ulak
    venv instead of its own, and on Windows locks Ulak venv DLLs so the
    venv cannot be rebuilt during `ulak update`.
    """
    popen_calls = []
    ...
    monkeypatch.setattr(openviking_module.shutil, "which", lambda name: "/usr/local/bin/openviking-server")
    monkeypatch.setattr(openviking_module.subprocess, "Popen", fake_popen)
    monkeypatch.setenv("PYTHONPATH", "/opt/ulak/.venv/Lib/site-packages")
    monkeypatch.setenv("ULAK_PROFILE", "test-profile")
    ...
    assert child_env.get("ULAK_PROFILE") == "test-profile"
...
def test_https_local_endpoint_is_not_runtime_autostart_eligible(monkeypatch):
    ...
    assert warnings == [
        ...
        "OpenViking memory is temporarily unavailable; Ulak will retry on a later access or when "
        "the config changes. "
...
def test_runtime_does_not_autostart_when_local_server_reports_unhealthy(monkeypatch):
    ...
    assert warnings == [
        ...
        "OpenViking memory is temporarily unavailable; Ulak will retry on a later access "
...
def test_tool_add_resource_rejects_ulak_credential_file_upload(tmp_path, monkeypatch):
    import agent.file_safety as fs

    ulak_home = tmp_path / "ulak_home"
    ulak_home.mkdir()
    auth_json = ulak_home / "auth.json"
    auth_json.write_text('{"OPENROUTER_API_KEY":"sk-test-secret"}', encoding="utf-8")
    monkeypatch.setattr(fs, "_ulak_home_path", lambda: ulak_home)
...
def test_viking_client_delete_uses_identity_headers(monkeypatch):
    ...
    assert captured["kwargs"]["headers"]["X-OpenViking-Actor-Peer"] == "ulak"
...
# ---------------------------------------------------------------------------
# on_session_switch — flush + commit + rotate behavior (ulak-agent#28296)
# ---------------------------------------------------------------------------
...
def test_sync_turn_captures_session_id_before_worker_runs():
    ...
    assert captured_payloads == [{
        "messages": [
            ...
            {"role": "assistant", "parts": [{"type": "text", "text": "a"}], "peer_id": "ulak"},
        ]
    }]
...
def test_session_needs_commit_guard_wins_over_stale_turn_count():
    """Regression for ulak-agent#28296 review (M3): once a session is marked
...
def test_concurrent_providers_claim_unlocked_pending_owner_once(
    ...
    for provider in providers:
        ...
        provider._ulak_home = str(tmp_path)
...
def test_is_available_false_without_any_endpoint(monkeypatch):
    _clear_openviking_env(monkeypatch)
    monkeypatch.setattr(
        openviking_module, "_load_ulak_openviking_config", lambda: {}
    )
...
class TestOpenVikingEnvWriter:
    ...
    def test_bom_prefixed_env_updates_in_place(self, tmp_path):
        ...
        # The stale value must be gone, not left as a duplicate. Ulak and
