...
Run from the PR worktree:
    ...
    cd ~/.ulak/ulak-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.ulak/ulak-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.ulak/ulak-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a ulak-agent checkout"
)
...
# Isolated ULAK_HOME for the config write.
