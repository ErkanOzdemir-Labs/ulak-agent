...
# worktree), ``MAIN_DIR`` falls back to a sibling ``ulak-agent-main``
...
def _resolve_main_dir() -> Path:
    ...
    if (candidate / "tools" / "image_generation_tool.py").exists() and candidate != REPO_ROOT:
        return candidate
    sibling = REPO_ROOT.parent / "ulak-agent-main"
...
assert (PR_DIR / "tools" / "image_generation_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a ulak-agent checkout"
)
...
# Isolated ULAK_HOME so the config write is hermetic.
...
SCENARIOS: list[tuple[str, str, dict[str, str]]] = [
    ...
    (
        ...
        "",
        {
            "TOOL_GATEWAY_DOMAIN": "github.com/ErkanOzdemir-Labs",
...
def main() -> int:
    ...
    if MAIN_DIR == PR_DIR:
        print(
            ...
            "      Set up a sibling 'ulak-agent-main' checkout pinned to "
