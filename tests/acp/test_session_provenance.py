...
def test_root_session_no_compression(db):
    ...
    assert prov["currentUlakSessionId"] == "root1"
    assert prov["rootUlakSessionId"] == "root1"
    assert prov["parentUlakSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_ulak_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentUlakSessionId"] == "old"
    assert prov["rootUlakSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousUlakSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootUlakSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"ulak"}
    assert "sessionProvenance" in meta["ulak"]
    assert meta["ulak"]["sessionProvenance"]["currentUlakSessionId"] == "root1"
