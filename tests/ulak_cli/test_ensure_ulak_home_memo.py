"""ensure_ulak_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".ulak"
    second = tmp_path / "b" / ".ulak"

    monkeypatch.setenv("ULAK_HOME", str(first))
    cfg.ensure_ulak_home()

    # Profile switch: ULAK_HOME moves → the new path is ensured too.
