...
calls ``os._exit(0)`` ~5ms after ``ULAK_BACKEND_READY``, which no in-process
...
def test_live_backend_survives_timezone_drifted_parent_marker(tmp_path):
    ...
    try:
        ...
        env.update(
            ...
            PYTHONUNBUFFERED="1",
            ULAK_PARENT_PID=str(parent.pid),
            ULAK_PARENT_START_MARKER=drifted_marker,
            ULAK_PARENT_NONCE="nonce-95693",
            ULAK_SERVE_WATCHDOG_POLL_S="0.5",
        )
        env.pop("ULAK_DESKTOP", None)
        code = (
        ...
        assert _read_until(serve, "ULAK_BACKEND_READY"), "backend never announced READY"
