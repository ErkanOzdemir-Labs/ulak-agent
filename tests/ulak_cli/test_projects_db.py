"""Tests for the per-profile Projects store (ulak_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Ulak Agent", folders=["/tmp/ulak"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "ulak-agent"
    assert proj.name == "Ulak Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/ulak"
    assert [f.path for f in proj.folders] == ["/tmp/ulak"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "ulak-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' ULAK_HOME.
