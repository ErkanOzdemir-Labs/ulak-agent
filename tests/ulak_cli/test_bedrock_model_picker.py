...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (ULAK_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via ULAK_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in ULAK_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from ulak_cli.providers import ULAK_OVERLAYS
        assert "bedrock" in ULAK_OVERLAYS
