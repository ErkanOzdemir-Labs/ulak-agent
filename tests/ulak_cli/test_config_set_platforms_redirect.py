...
`ulak config set platforms.<name>.<display_setting> <value>` must write to
...
def _write_config(ulak_home: Path, data: dict) -> Path:
...
def _set(monkeypatch, ulak_home, key, value, force=False):
    """Isolated call to set_config_value against a temp ULAK_HOME."""
    monkeypatch.setenv("ULAK_HOME", str(ulak_home))
    # set_config_value resolves the home live via get_config_path()/get_ulak_home()
...
class TestPerPlatformDisplayRedirect:
    def test_streaming_redirects_to_display_platforms(self, ulak_home, monkeypatch):
        """platforms.telegram.streaming must land under display.platforms."""
        _set(monkeypatch, ulak_home, "platforms.telegram.streaming", "false")

        result = yaml.safe_load((ulak_home / "config.yaml").read_text())
    ...
    def test_show_reasoning_redirects(self, ulak_home, monkeypatch):
        _set(monkeypatch, ulak_home, "platforms.telegram.show_reasoning", "false")
        result = yaml.safe_load((ulak_home / "config.yaml").read_text())
        assert result["display"]["platforms"]["telegram"]["show_reasoning"] is False

    def test_tool_progress_redirects(self, ulak_home, monkeypatch):
        ...
        # is the bool.
        _set(monkeypatch, ulak_home, "platforms.discord.tool_progress", "off")
        result = yaml.safe_load((ulak_home / "config.yaml").read_text())
        assert result["display"]["platforms"]["discord"]["tool_progress"] is False

    def test_connection_key_not_redirected(self, ulak_home, monkeypatch):
        """A real connection key (token) stays in top-level platforms.<name>."""
        _set(monkeypatch, ulak_home, "platforms.telegram.token", "new-token")
        result = yaml.safe_load((ulak_home / "config.yaml").read_text())
...
class TestRedirectSiblingSurfaces:
    ...
    def test_get_mirrors_gateway_resolution_after_set(self, ulak_home, monkeypatch, capsys):
        from gateway.display_config import resolve_display_setting
        from ulak_cli.config import get_config_value
        ...
        raw = yaml.safe_load((ulak_home / "config.yaml").read_text())
        assert resolve_display_setting(raw, "telegram", "streaming") is False

    def test_unset_removes_the_redirected_leaf(self, ulak_home, monkeypatch):
    ...
    def test_unset_missing_redirected_leaf_exits_nonzero(self, ulak_home, monkeypatch):
        from ulak_cli.config import unset_config_value
    ...
    def test_set_prints_redirect_note(self, ulak_home, monkeypatch, capsys):
        _set(monkeypatch, ulak_home, "platforms.telegram.streaming", "false")
    ...
    def test_redirect_helper_only_touches_known_display_keys(self):
        ...
        from ulak_cli.config import _redirect_platform_display_key
