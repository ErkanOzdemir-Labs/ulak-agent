...
``_reexec_dependency_sync_off_windows_shim`` with ``ULAK_UPDATE_REEXEC=1``)
...
def _run_cmd_update(monkeypatch, impl, *, reexec: bool):
    ...
    monkeypatch.setattr("ulak_cli.config.detect_install_method", lambda root: "git")
    monkeypatch.setattr("ulak_cli.update_lock.UpdateLock", lambda: _FakeLock(events["order"]))
    ...
    monkeypatch.setattr("ulak_cli.update_receipt.finalize_pending_update_receipt", fake_receipt)
    monkeypatch.setattr("os._exit", fake_exit)
    if reexec:
        monkeypatch.setenv("ULAK_UPDATE_REEXEC", "1")
    else:
        monkeypatch.delenv("ULAK_UPDATE_REEXEC", raising=False)
