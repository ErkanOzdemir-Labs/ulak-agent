...
When ``ULAK_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.ulak/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.ulak            (has its own auth.json fixture)
    * Profile     -> tmp_path/.ulak/profiles/coder   (active, ULAK_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Ulak is not
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (ULAK_HOME != ~/.ulak, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    ulak_home = tmp_path / "classic"
    ulak_home.mkdir()
