...
def _stage(home_root, name):
    # Machine-scoped models dir (the shared root — tmp ULAK_HOME IS the root here).
    from ulak_cli.local_runtime.bootstrap import models_dir
...
def test_not_ours_returns_none(ulak_home):
    from ulak_cli.local_runtime.capabilities import managed_model_supports_vision

    assert managed_model_supports_vision("gpt-4o") is None


def test_catalog_vision_model_with_projector_on_disk(ulak_home):
    """Staged catalog model with an mmproj present: True (server down —
    the catalog + on-disk projector answer)."""
    from ulak_cli.local_runtime.bootstrap import assets_dir
    from ulak_cli.local_runtime.capabilities import managed_model_supports_vision
    ...
    _stage(ulak_home, variant.model_id)
    adir = assets_dir()
...
def test_catalog_vision_model_missing_projector_is_blind(ulak_home):
...
def test_lookup_chain_consults_managed_runtime(ulak_home, monkeypatch):
    ...
    monkeypatch.setattr(
        "ulak_cli.local_runtime.capabilities.managed_model_supports_vision",
        lambda mid: True)
...
def test_webp_transcodes_to_png_for_managed_provider(ulak_home, monkeypatch, tmp_path):
...
def test_webp_passes_through_for_cloud_providers(ulak_home, monkeypatch, tmp_path):
...
def test_vision_analyze_normalization_narrows_for_managed(ulak_home, monkeypatch, tmp_path):
