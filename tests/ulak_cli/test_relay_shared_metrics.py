"""Focused tests for the Ulak shared-metrics durable store."""
...
from ulak_cli.observability import shared_metrics as shared_metrics_module
from ulak_cli.observability.shared_metrics import SharedMetricsStore
from ulak_cli.observability.shared_metrics_contract import (
...
LEGACY_SCHEMA_PATH = SCHEMA_PATH.with_name("ulak.shared_metrics.v1.schema.json")
...
def test_model_call_counter_survives_restart_and_exports_only_new_deltas(tmp_path):
    ...
    assert first_package["schema_version"] == "ulak.shared_metrics.v2"
...
def test_package_schema_matches_the_model_call_contract():
    ...
    assert schema["properties"]["schema_version"]["const"] == "ulak.shared_metrics.v2"
...
def test_client_active_mark_accepts_only_an_empty_allowlisted_payload():
    event = SimpleNamespace(
        ...
        scope_category=None,
        metadata={
            "ulak.metrics.schema_version": "ulak.metrics.event.v2",
        },
        data={},
    )
    ...
    wrong_schema.metadata["ulak.metrics.schema_version"] = "unknown"
...
def test_v1_package_schema_retains_the_legacy_model_contract():
    ...
    assert schema["properties"]["schema_version"]["const"] == "ulak.shared_metrics.v1"
...
def test_auxiliary_logical_scope_projects_one_normalized_terminal_route():
    event = SimpleNamespace(
        ...
        metadata={
            ...
            "ulak.call_role": "auxiliary:compression",
        },
    )
    ...
    event.metadata["ulak.call_role"] = "primary"
...
def test_tool_subscriber_contract_accepts_only_bounded_events():
    terminal = SimpleNamespace(
        ...
        name="ulak.tool_call",
        scope_category="end",
        metadata={"ulak.metrics.schema_version": "ulak.metrics.event.v2"},
        data={
...
def test_skill_subscriber_contract_accepts_only_bounded_marks():
    ...
    assert skill_counter(load) == ("ulak.skill.load.count", load.data)
...
def test_store_exports_task_started_and_terminal_counters(tmp_path):
    ...
    store.record_counter("ulak.task_run.finished", terminal, _resource())
...
def test_retention_prunes_only_expired_exported_history(tmp_path):
    ...
    with sqlite3.connect(database_path) as connection:
        connection.execute(
            ...
            WHERE ulak_version = 'expired-version'
            """
        )
        connection.execute(
            """
    ...
    with sqlite3.connect(database_path) as connection:
        ...
        aggregate_versions = {
            ...
            for row in connection.execute(
                "SELECT ulak_version FROM counter_aggregates"
            ).fetchall()
        }
