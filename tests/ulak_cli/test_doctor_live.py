"""Tests for ``ulak doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from ulak_cli import doctor_live
from ulak_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `ulak doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("ulak_cli.doctor.ULAK_HOME", tmp_path / "home")
        monkeypatch.setattr("ulak_cli.doctor.PROJECT_ROOT", tmp_path / "root")
