...
def ulak_home(tmp_path, monkeypatch):
    ...
    # /etc/ulak on the dev/CI box can't influence the test.
    monkeypatch.setenv("ULAK_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(ulak_home, monkeypatch):
...
def test_env_expansion_in_user_config(ulak_home, monkeypatch):
    from ulak_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_ulak_dotenv(ulak_home=str(home))
