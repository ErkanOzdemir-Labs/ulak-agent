...
the SAME provider universe as ``ulak model`` (the CLI/TUI picker).
...
def test_every_ulak_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `ulak model` universe."""
    ...
    assert not missing, (
        "providers shown in `ulak model` but not configurable in the desktop "
