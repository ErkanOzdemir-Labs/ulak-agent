...
Before the fix, ``ulak whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``ulak gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def _env_value(ulak_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from ulak_cli.main_platform_setup import cmd_whatsapp
    ...
    monkeypatch.setattr("ulak_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``ulak whatsapp`` with an existing paired session and
