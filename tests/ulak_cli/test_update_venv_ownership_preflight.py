"""Venv ownership preflight for ``ulak update`` (#83529).

A venv touched by ``sudo pip`` / ``sudo ulak`` contains root-owned files
(e.g. ``site-packages/ulak_agent-*.dist-info/INSTALLER``). A later normal
``ulak update`` then dies mid-mutation inside ``uv pip install -e .``
("Permission denied (os error 13)") with ``venv/bin/ulak`` already deleted,
...
def _make_fake_venv(tmp_path):
    ...
    (venv / "bin" / "ulak").write_text("#!stub\n")
    (venv / "bin" / "python").write_text("#!stub\n")
    sp = venv / "lib" / "python3.12" / "site-packages"
    dist_info = sp / "ulak_agent-1.0.0.dist-info"
...
def test_foreign_owned_dist_info_child_detected(tmp_path, monkeypatch):
    ...
    installer = str(
        ...
        / "ulak_agent-1.0.0.dist-info" / "INSTALLER"
    )
...
def test_foreign_owned_refuses_with_chown_hint(tmp_path, monkeypatch, capsys):
    venv = _make_fake_venv(tmp_path)
    ulak_bin = str(venv / "bin" / "ulak")
    real_uid = update_cmd._path_uid
    monkeypatch.setattr(
        update_cmd,
        "_path_uid",
        lambda p: 0 if str(p) == ulak_bin else real_uid(p),
    )
    monkeypatch.setattr(
        update_cmd_deps,
        "_path_uid",
