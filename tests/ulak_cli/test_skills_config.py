"""Tests for ulak_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("ulak_cli.skills_config.save_config")
...
class TestIsSkillDisabled:
    ...
    @patch("ulak_cli.config.load_config")
    ...
    @patch("ulak_cli.config.load_config")
    @patch.dict("os.environ", {"ULAK_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """ULAK_SESSION_PLATFORM should be used when ULAK_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_ulak_platform_takes_precedence(self, tmp_path, monkeypatch):
        """ULAK_PLATFORM should win over ULAK_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from ulak_cli.skills_config import _get_categories
        skills = [
