...
from ulak_cli.slack_cli import _build_full_manifest, slack_manifest_command
from ulak_cli.subcommands.slack import build_slack_parser
from ulak_cli import main_platform_setup


def _parse_slack_args(argv):
    """Build the real `ulak slack` parser and parse argv against it."""
...
def _run_console_entrypoint(*argv: str) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(
        [
            sys.executable,
            "-c",
            "from ulak_cli.main import main; raise SystemExit(main())",
            *argv,
        ],
...
def test_slack_dispatcher_propagates_manifest_failure(monkeypatch):
    from ulak_cli import main as main_module
    from ulak_cli import slack_cli
...
class TestSlackManifestArgparse:
    ...
    def test_long_description_file_reports_tilde_expansion_errors(
        ...
        source = "~ulak-user-that-does-not-exist-20260716/AGENTS.md"
...
class TestSlackFullManifest:
    """Generated full Slack app manifest used by `ulak slack manifest`."""
    ...
    def test_assistant_features_remain_enabled(self):
        manifest = _build_full_manifest("Ulak", "Your Ulak agent on Slack")
    ...
    def test_no_assistant_preserves_core_surface(self):
        ...
        manifest = _build_full_manifest(
            "Ulak", "Your Ulak agent on Slack", include_assistant=False
        )
