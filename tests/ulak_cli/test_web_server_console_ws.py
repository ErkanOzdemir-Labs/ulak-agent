"""Dashboard Ulak Console websocket tests."""
...
@pytest.fixture
def console_client(monkeypatch, _isolate_ulak_home):
...
def test_console_ws_cancel_returns_to_prompt(console_client, monkeypatch):
    from ulak_cli.console_engine import ConsoleResult, UlakConsoleEngine

    def slow_execute(self, line: str, *, confirmed: bool = False):
        time.sleep(0.2)
        return ConsoleResult("ok", output="late", command=line)

    monkeypatch.setattr(UlakConsoleEngine, "execute", slow_execute)
