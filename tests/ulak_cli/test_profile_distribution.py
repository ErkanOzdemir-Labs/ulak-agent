"""Tests for ulak_cli.profile_distribution — git-based profile installs.
...
from ulak_cli.profile_distribution import (
    DEFAULT_DIST_OWNED,
    DistributionError,
...
# ---------------------------------------------------------------------------
# Isolated profile env (matches tests/ulak_cli/test_profiles.py)
# ---------------------------------------------------------------------------
...
class TestVersionRequires:
    ...
    def test_check_matrix(self, spec, cur, ok):
        if ok:
            check_ulak_requires(spec, cur)
        else:
            with pytest.raises(DistributionError, match="requires Ulak"):
                check_ulak_requires(spec, cur)
...
class TestInstall:
    ...
    def test_update_respects_distribution_owned_allowlist(self, profile_env):
        ...
        from ulak_cli.profile_distribution import read_manifest as _read
    ...
    def test_install_enforces_ulak_requires(self, profile_env, monkeypatch):
        # Pin current Ulak version to something well below the requirement
        import ulak_cli
        monkeypatch.setattr(ulak_cli, "__version__", "0.1.0", raising=False)

        mf = DistributionManifest(
            name="future",
            version="1.0.0",
            ulak_requires=">=99.0.0",
        )
...
class TestInstalledAtStamp:
    ...
    def test_update_refreshes_installed_at(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr(
            "ulak_cli.profile_distribution.datetime", _FakeDT, raising=True
        )

        from ulak_cli.profile_distribution import update_distribution
...
class TestProfileInfoDistribution:
    ...
    def test_malformed_manifest_does_not_break_list(self, profile_env):
        from ulak_cli.profiles import create_profile, list_profiles, get_profile_dir
