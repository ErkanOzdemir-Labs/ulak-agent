...
holding ``ulak.exe`` open WITHOUT FILE_SHARE_DELETE, exactly like a
...
def held_shim(tmp_path: Path):
    ...
    shim = scripts / "ulak.exe"
    shim.write_bytes(b"MZ fake shim")
    (scripts / "ulak-gateway.exe").write_bytes(b"MZ fake shim")
...
def test_locked_shim_really_cannot_be_renamed(held_shim):
    ...
    with pytest.raises(OSError):
        os.rename(shim, shim.with_name("ulak.exe.old.premise"))
...
def test_release_then_strict_quarantine_succeeds(tmp_path, monkeypatch):
    ...
    scripts = tmp_path / "venv" / "Scripts"
    scripts.mkdir(parents=True)
    (scripts / "ulak.exe").write_bytes(b"MZ fake shim")

    holder = subprocess.Popen(
        [sys.executable, "-c", _HOLDER_CODE, str(scripts / "ulak.exe")],
        stdout=subprocess.PIPE,
        text=True,
    )
