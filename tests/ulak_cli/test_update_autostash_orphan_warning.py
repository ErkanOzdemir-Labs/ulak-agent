...
``ulak update`` can legitimately leave an autostash behind (--keep-stash
parks it; a conflicted restore preserves it), but nothing ever mentioned those
entries again — they persisted invisibly for weeks. ``ulak update`` now
warns about ``ulak-update-autostash-*`` entries older than the threshold.
...
def _make_repo_with_autostash(tmp_path, age_days: float):
    """Real repo with one ulak-update-autostash entry aged ``age_days``."""
    import shutil
    ...
    name = f"ulak-update-autostash-{stamp}"
...
def test_non_ulak_stash_is_ignored(tmp_path, capsys):
    import shutil
...
def test_unparseable_autostash_timestamp_left_alone(tmp_path, capsys):
    ...
    _git(tmp_path, "stash", "push", "-m", "ulak-update-autostash-notadate")
    count = update_cmd._warn_orphaned_update_autostashes(["git"], tmp_path)
    assert count == 0
