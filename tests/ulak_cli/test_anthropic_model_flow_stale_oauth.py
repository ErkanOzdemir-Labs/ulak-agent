"""Tests for Bug #12905 fix — stale OAuth token detection in ulak model flow.

Bug 3: `ulak model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.ulak/.env but no valid
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr("ulak_cli.secret_prompt.masked_secret_prompt", lambda _: "")

        from ulak_cli.model_setup_flows import _model_flow_anthropic
        cfg = {}
