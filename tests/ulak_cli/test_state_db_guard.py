...
from ulak_cli.backup import verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    ULAK_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def ulak_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("ulak_cli.config") or mod == "ulak_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, ulak_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, ulak_home, capsys, monkeypatch):
