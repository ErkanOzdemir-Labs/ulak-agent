"""Tests for ulak_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import ulak_cli.gui_uninstall as gu


def _make_agent(ulak_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = ulak_home / "ulak-agent"
    (agent_root / "ulak_cli").mkdir(parents=True)
    (agent_root / "ulak_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(ulak_home: Path) -> None:
    """Create the source-built GUI artifacts a `ulak desktop` run produces."""
    desktop = ulak_home / "ulak-agent" / "apps" / "desktop"
    ...
    (ulak_home / "ulak-agent" / "node_modules").mkdir(parents=True)
    (ulak_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(ulak_home: Path) -> None:
    (ulak_home / "config.yaml").write_text("x: 1\n")
    (ulak_home / ".env").write_text("KEY=secret\n")
    (ulak_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(ulak_home)
    _make_gui_build(ulak_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(ulak_home)
    ...
    assert summary["ulak_home"] == str(ulak_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `ulak desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    ulak_home = tmp_path / ".ulak"
    _make_agent(ulak_home)
    icon = lde.icon_path(ulak_home / "ulak-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(ulak_home)
    ...
    assert lde.icon_path(ulak_home / "ulak-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (ulak_home / "ulak-agent" / "ulak_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".ulak")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import ulak_cli.uninstall as uninstall
