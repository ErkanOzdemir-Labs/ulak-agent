...
def test_installed_tags_newest_first(ulak_home):
    from ulak_cli.local_runtime.binaries import installed_tags

    assert installed_tags() == []
    _install_fake_tag(ulak_home, "b10290")
    _install_fake_tag(ulak_home, "b10412")
    assert installed_tags() == ["b10412", "b10290"]


def test_default_tag_flows_from_default_config(ulak_home):
    """Unpinned users inherit the Ulak-release default (deep-merge);
...
def test_update_available_requires_enabled_and_installed(ulak_home, monkeypatch):
    ...
    import ulak_cli.web_routers.local_models as lm
...
def test_boot_never_downloads_missing_tag(ulak_home, monkeypatch):
...
def test_prune_keeps_n_minus_one(ulak_home):
    from ulak_cli.local_runtime.binaries import installed_tags, prune_old_tags

    for tag in ("b10100", "b10200", "b10290"):
        _install_fake_tag(ulak_home, tag)
    prune_old_tags(["b10290", "b10200"])
    assert installed_tags() == ["b10290", "b10200"]
    # downloads/ cache dir must survive pruning when present.
    downloads = ulak_home / "runtimes" / "llamacpp" / "downloads"
