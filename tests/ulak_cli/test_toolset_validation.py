"""Unit tests for ulak_cli.toolset_validation (see #38798).

Pure logic — the validity predicate is injected, so these tests need neither the
tool registry nor a running Ulak.
"""
...
# A representative set of real toolset names. `ulak` is deliberately absent —
# that is the corruption #38798 reported (`ulak-cli` rewritten to `ulak`).
_KNOWN = {
    "ulak-cli",
    "ulak-telegram",
    "ulak-discord",
...
def test_38798_corruption_warns_and_suggests_correct_name():
    # The exact reported shape: cli holds 'ulak' instead of 'ulak-cli'.
    warnings = validate_platform_toolsets({"cli": ["ulak"]}, _is_valid)
    unknown = [w for w in warnings if "unknown toolset 'ulak'" in w]
    assert len(unknown) == 1
    # Actionable: points at the valid name the entry should have been.
    assert "did you mean 'ulak-cli'?" in unknown[0]
    # And the zero-valid-toolsets safety net fires.
    assert any("zero valid toolsets" in w for w in warnings)


def test_mixed_valid_and_invalid_flags_only_the_invalid():
    cfg = {"cli": ["ulak-cli"], "discord": ["bogus"]}
...
def test_empty_list_on_a_platform_warns_even_when_others_are_valid():
    ...
    cfg = {
        "cli": [],
        "telegram": ["ulak-telegram"],
        "discord": ["ulak-discord"],
    }
    warnings = validate_platform_toolsets(cfg, _is_valid)
...
def test_empty_list_warns_for_each_affected_platform():
    cfg = {"cli": [], "discord": [], "telegram": ["ulak-telegram"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)
...
def test_null_platform_warns_even_when_others_are_valid():
    ...
    cfg = {"cli": None, "telegram": ["ulak-telegram"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)

    assert any(
        "platform 'cli'" in w
        and "null toolset value" in w
        and "falling back to 'ulak-cli'" in w
        for w in warnings
    )
    assert not any("zero valid toolsets" in w for w in warnings)
...
def test_null_platform_uses_canonical_default_for_alias():
    cfg = {"whatsapp_cloud": None, "telegram": ["ulak-telegram"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)

    assert any(
        "platform 'whatsapp_cloud'" in w
        and "falling back to 'ulak-whatsapp'" in w
        for w in warnings
    )
    assert not any("zero valid toolsets" in w for w in warnings)
...
def test_scalar_platform_value_warns_but_uses_platform_default():
    cfg = {"cli": "bogus", "telegram": ["ulak-telegram"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)
...
def test_platform_restricted_toolset_warns_when_other_platform_is_valid():
    cfg = {"telegram": ["discord"], "cli": ["ulak-cli"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)
...
def test_null_plugin_platform_uses_synthetic_default():
    ...
    try:
        cfg = {platform: None, "telegram": ["ulak-telegram"]}
        warnings = validate_platform_toolsets(cfg, _is_valid)

        assert resolve_toolset(f"ulak-{platform}")
        assert any(
            f"platform '{platform}'" in w
            and f"falling back to 'ulak-{platform}'" in w
            for w in warnings
        )
        assert not any("zero valid toolsets" in w for w in warnings)
    finally:
...
def test_populated_platforms_produce_no_empty_list_warning():
    cfg = {"cli": ["ulak-cli"], "telegram": ["ulak-telegram"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)
    assert warnings == []
