"""Tests for _purge_stale_ulak_modules — the class fix for stale
sys.modules breaking the gateway auto-restart after `ulak update`.

Field failure (2026-08-20, Teknium's Linux box): `ulak update` pulled a
checkout where ulak_cli/gateway.py newly imports `line_input` from
ulak_cli.cli_output, but the updater process had cli_output cached from
before that symbol existed. The function-level `from ulak_cli.gateway
...
the Ulak package prefixes so later imports rebuild a self-consistent
...
def _restore_sys_modules():
    ...
    The purge under test evicts real Ulak modules from the cache; later
...
def test_purge_evicts_ulak_prefixed_modules():
    victims = [
        "ulak_cli.cli_output",
        "ulak_cli.gateway",
        "gateway.status",
    ...
    for name in victims:
        if name not in sys.modules:
            ...
            added.append(name)
    try:
        cli_main._purge_stale_ulak_modules()
        for name in victims:
...
def test_purge_protects_executing_modules():
    ...
    cli_main._purge_stale_ulak_modules()
    assert sys.modules.get("ulak_cli.update_cmd") is update_cmd
    assert sys.modules.get("ulak_cli.main") is cli_main
    assert "ulak_cli" in sys.modules
...
def test_purge_preserves_active_update_receipt(tmp_path, monkeypatch):
    ...
    import ulak_cli.update_receipt as receipt
    ...
    try:
        ...
        cli_main._purge_stale_ulak_modules()
        post_purge_receipt = importlib.import_module("ulak_cli.update_receipt")
...
def test_purge_never_raises_on_weird_sys_modules():
    ...
    sys.modules["ulak_cli._purge_test_none"] = None  # type: ignore[assignment]
    try:
        cli_main._purge_stale_ulak_modules()
    finally:
        sys.modules.pop("ulak_cli._purge_test_none", None)
...
def test_stale_symbol_scenario_end_to_end():
    ...
    try:
        ...
        try:
            from ulak_cli.cli_output import line_input  # noqa: F401
            raised = False
        except ImportError:
            raised = True
...
def test_purge_keeps_plan_record_class_identity():
    ...
    from ulak_cli.update_inventory import RuntimeRecord as before

    cli_main._purge_stale_ulak_modules()
    from ulak_cli.update_inventory import RuntimeRecord as after
