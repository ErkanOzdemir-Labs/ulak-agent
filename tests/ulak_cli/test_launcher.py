"""Tests for the top-level `./ulak` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./ulak` should use `ulak_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "ulak"
    called = []

    fake_main_module = types.ModuleType("ulak_cli.main")

    def fake_main():
        called.append("ulak_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "ulak_cli.main", fake_main_module)
