"""Tests for ulak_cli/completion.py — shell completion script generation."""
...
from ulak_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real ulak structure."""
    p = argparse.ArgumentParser(prog="ulak")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_ulak_completion()" in out
        assert "complete -F _ulak_completion ulak" in out
...
class TestGenerateZsh:
    ...
    def test_preserves_valid_zsh_arguments_alias_syntax(self):
        ...
        assert "'(-)'{-p,--profile}'[Profile name]:profile:_ulak_profiles'" in out
...
class TestProfileCompletion:
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'ulak profile use', complete with profile names."""
        out = generate_bash(_make_parser())
        # The profile case should have _ulak_profiles for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_ulak_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _ulak_profiles"
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__ulak_profiles" in out
        count = out.count("(__ulak_profiles)")
