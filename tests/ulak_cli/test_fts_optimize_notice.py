...
def test_update_notice_offers_v1_trigram_tool_calls_rebuild(tmp_path, monkeypatch, capsys):
    ...
    class FakeSessionDB:
        ...
        _db_needs_fts_storage_upgrade = staticmethod(
            ulak_state.SessionDB._db_needs_fts_storage_upgrade
        )

    monkeypatch.setattr(ulak_constants, "get_ulak_home", lambda: tmp_path)
    monkeypatch.setattr(ulak_state, "SessionDB", FakeSessionDB)
    ...
    assert "ulak sessions optimize-storage" in capsys.readouterr().out
    conn.close()
