"""Tests for the Command Installation check in ulak doctor."""
...
import ulak_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal ULAK_HOME + PROJECT_ROOT for doctor tests."""
    home = tmp_path / ".ulak"
    ...
    ulak_bin = venv_bin_dir / "ulak"
    ulak_bin.write_text("#!/usr/bin/env python\n# entry point\n")
...
class TestDoctorCommandInstallation:
    ...
    def test_fix_repairs_wrong_symlink(self, monkeypatch, tmp_path):
        home, project, ulak_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create a symlink pointing to wrong target
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "ulak"
        wrong_target = tmp_path / "wrong_ulak"
        ...
        assert cmd_link.resolve() == ulak_bin.resolve()
