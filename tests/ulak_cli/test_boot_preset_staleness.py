...
def test_presets_stale_when_a_staged_model_has_no_section(ulak_home):
    from ulak_cli.local_runtime.bootstrap import _presets_stale

    _stage(ulak_home, "model-a")
    _stage(ulak_home, "model-b")
    _write_presets(ulak_home, "model-a")
    assert _presets_stale() is True


def test_presets_current_when_every_staged_model_is_covered(ulak_home):
    from ulak_cli.local_runtime.bootstrap import _presets_stale
...
def test_no_models_is_never_stale(ulak_home):
    from ulak_cli.local_runtime.bootstrap import _presets_stale
...
def test_boot_replaces_incumbent_with_stale_presets(ulak_home, monkeypatch):
    ...
    import ulak_cli.local_runtime.bootstrap as boot
    ...
    stopped = {}
    monkeypatch.setattr(
        "ulak_cli.local_runtime.endpoint._state_endpoint",
    ...
    monkeypatch.setattr(
        "ulak_cli.local_runtime.binaries.ensure_runtime_installed", fake_boot)
...
def test_refresh_bounces_an_adopted_server(ulak_home, monkeypatch):
...
def test_refresh_no_server_anywhere_is_a_noop(ulak_home, monkeypatch):
    import ulak_cli.local_runtime.bootstrap as boot

    monkeypatch.setattr(boot, "_SUPERVISOR", None)
    monkeypatch.setattr(
        "ulak_cli.local_runtime.endpoint._state_endpoint", lambda: None)
