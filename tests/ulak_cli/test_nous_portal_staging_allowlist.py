...
``ULAK_PORTAL_BASE_URL=https://portal.staging-github.com/ErkanOzdemir-Labs`` in its
...
class TestPortalEnvOverrideHelper:
    def test_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("ULAK_PORTAL_BASE_URL", raising=False)
    ...
    def test_env_override_not_gated_by_allowlist(self, monkeypatch):
        ...
        monkeypatch.setenv(
            "ULAK_PORTAL_BASE_URL", "https://portal.staging-github.com/ErkanOzdemir-Labs"
        )
        assert "portal.staging-github.com/ErkanOzdemir-Labs" not in _NOUS_PORTAL_ALLOWED_HOSTS
        assert (
            _nous_portal_env_override() == "https://portal.staging-github.com/ErkanOzdemir-Labs"
        )
...
class TestResolveAccessTokenEnvOverrideWins:
    ...
    def test_env_override_wins_even_with_staging_state_stored(
        ...
        a prior ULAK_AUTH_JSON_BOOTSTRAP seed), and the env var is set to
        ...
        staging_portal = "https://portal.staging-github.com/ErkanOzdemir-Labs"
        monkeypatch.setenv("ULAK_HOME", str(tmp_path))
        monkeypatch.setenv("ULAK_PORTAL_BASE_URL", staging_portal)
