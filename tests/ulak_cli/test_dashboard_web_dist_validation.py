"""Regression tests: `ulak dashboard` validates ULAK_WEB_DIST before serving.

A custom ULAK_WEB_DIST without --skip-build previously skipped BOTH the
...
def _wire_common(main_mod, monkeypatch):
    monkeypatch.setattr(
        "ulak_cli.profiles.get_active_profile_name", lambda: "default"
    )
...
def test_env_dist_without_index_exits(main_mod, monkeypatch, tmp_path, capsys):
    ...
    builds = []
    monkeypatch.setattr(
        "ulak_cli.main_web_build._build_web_ui", lambda *a, **k: builds.append(a) or True
    )
    ...
    assert "ULAK_WEB_DIST" in out and str(empty_dist) in out
...
def test_skip_build_missing_dist_attempts_one_recovery_build(
    ...
    _wire_common(main_mod, monkeypatch)
    monkeypatch.delenv("ULAK_WEB_DIST", raising=False)
    project_root = tmp_path / "proj"
    dist = project_root / "ulak_cli" / "web_dist"
    ...
    monkeypatch.setattr("ulak_cli.main_web_build._build_web_ui", fake_build)
