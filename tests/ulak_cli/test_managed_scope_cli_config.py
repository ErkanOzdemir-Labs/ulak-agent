...
ulak_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `ulak config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_ulak_home = get_ulak_home()`` at import time (module
    singleton), so monkeypatching ULAK_HOME after import doesn't move it.
    ...
    this only matters for tests that swap ULAK_HOME.)
    """
    import cli
