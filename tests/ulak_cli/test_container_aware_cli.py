...
def container_env(tmp_path, monkeypatch):
    """Set up a fake ULAK_HOME with .container-mode file."""
    ulak_home = tmp_path / ".ulak"
    ulak_home.mkdir()
    monkeypatch.setenv("ULAK_HOME", str(ulak_home))
    monkeypatch.delenv("ULAK_DEV", raising=False)

    container_mode = ulak_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=ulak-agent\n"
        "exec_user=ulak\n"
        "ulak_bin=/data/current-package/bin/ulak\n"
    )
    return ulak_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("ulak_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "ulak-agent"
    assert info["exec_user"] == "ulak"
    assert info["ulak_bin"] == "/data/current-package/bin/ulak"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from ulak_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "ulak"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "ulak-agent" in cmd
    assert "/data/current-package/bin/ulak" in cmd
    assert "chat" in cmd
