...
* Be supported-platform-only — no-op silently elsewhere so ``ulak update``
...
class TestCuaDriverRuntimeContract:
    def test_current_manifest_is_ready(self):
        from ulak_cli import tools_config_cua as tools_config
...
class TestInstallCuaDriverUpgrade:
    ...
    def test_install_target_writability_is_probed_for_real_on_macos(self):
        ...
        ``macos_only``: ``/Applications`` is the only install target Ulak
    ...
    def test_missing_explicit_override_does_not_install_standard_driver(
        ...
        monkeypatch.setenv("ULAK_CUA_DRIVER_CMD", "/missing/custom/cua-driver")
        with patch.object(
                 tools_config,
                 "_resolved_cua_driver_cmd",
                 return_value=None,
...
class TestRequireConfirmedUpdate:
    """`ulak update` passes require_confirmed_update=True: the full
    ...
    Explicit `ulak computer-use install --upgrade` keeps the old
    ...
    def test_explicit_upgrade_still_falls_through_on_indeterminate(self):
        # `ulak computer-use install --upgrade` (default flag): the old
    ...
    def test_incompatible_driver_repairs_on_posix_despite_indeterminate_check(self):
        """Ulak' own version floor is the confirmation. When the installed
        driver fails the runtime contract, the `ulak update` refresh must
...
class TestUpdateCheckTimeoutDefaults:
    ...
    full reinstall fall-through during `ulak update`.
    """
...
class TestInstallerTimeoutDrainIsBounded:
    ...
    def test_drain_that_times_out_still_surfaces_the_run_timeout(self):
        ...
        so the existing manual re-run hint prints and ``ulak update``
...
class TestCuaVersionSummary:
    """`ulak computer-use status` prints one line, whatever the binary says.

    A binary chosen by ULAK_CUA_DRIVER_CMD is under no obligation to answer
