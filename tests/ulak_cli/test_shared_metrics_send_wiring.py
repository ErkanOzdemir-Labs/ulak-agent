...
from ulak_cli.observability import relay_shared_metrics as mod
...
def capture_sender(monkeypatch):
    ...
    monkeypatch.setattr(
        "ulak_cli.observability.shared_metrics_sender.SharedMetricsSender",
        FakeSender,
    )
    return record
...
def _set_config(monkeypatch, config):
    monkeypatch.setattr(
        "ulak_cli.config.read_raw_config_readonly", lambda: config, raising=False
    )
...
class TestConsentWindows:
    ...
    def test_reconcile_runs_even_when_collection_is_disabled(
        ...
        # ULAK_HOME: the boot reconciler probes the default path (without
        ...
        home = tmp_path / "home"
        monkeypatch.setattr(
            "ulak_constants.get_ulak_home", lambda: home
        )
        ...
        monkeypatch.setattr(
            "ulak_cli.observability.shared_metrics.SharedMetricsStore",
            lambda *a, **k: store,
        )
...
class TestFailureIsolation:
    ...
    def test_an_unreadable_config_does_not_break_export(self, runtime, monkeypatch, capture_sender):
        ...
        monkeypatch.setattr(
            "ulak_cli.config.read_raw_config_readonly", explode, raising=False
        )
        runtime._export()
        assert runtime.subscriber.store.exported == 1
        assert capture_sender["passes"] == []
