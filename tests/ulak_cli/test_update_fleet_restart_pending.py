...
A ``ulak update`` killed after git pull advanced HEAD but before the
...
- next ``ulak update`` with git already up to date still runs the
...
import ulak_cli.main_web_build as main_web_build
import ulak_cli.main_install_repair as main_install_repair
from ulak_cli import update_cmd
import ulak_cli.update_cmd_fleet as update_cmd_fleet
...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    ...
    monkeypatch.setattr(ulak_main.subprocess, "run", run_side_effect)
    monkeypatch.setattr(ulak_main, "PROJECT_ROOT", tmp_path)
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(ulak_main, "_resolve_update_branch", lambda args: "main")
    monkeypatch.setattr(ulak_main, "_is_windows", lambda: False)
    ...
    monkeypatch.setattr(
        ulak_main,
        "_get_origin_url",
        lambda *a, **k: "https://github.com/ErkanOzdemir-Labs/ulak-agent.git",
    )
    monkeypatch.setattr(update_cmd, "_is_fork", lambda *a, **k: False)
    monkeypatch.setattr(
        ulak_main, "_stash_local_changes_if_needed", lambda *a, **k: None
    )
    monkeypatch.setattr(ulak_main, "_clear_bytecode_cache", lambda *a, **k: 0)
    monkeypatch.setattr(
        ulak_main, "_record_bytecode_fingerprint", lambda *a, **k: None
    )
    monkeypatch.setattr(
        main_web_build, "_record_bytecode_fingerprint", lambda *a, **k: None
    )
    monkeypatch.setattr(ulak_main, "_run_pre_update_backup", lambda *a, **k: None)
    monkeypatch.setattr(
        ulak_main, "_pause_windows_gateways_for_update", lambda: None
    )
    monkeypatch.setattr(
        ulak_main, "_resume_windows_gateways_after_update", lambda *a, **k: None
    )
    monkeypatch.setattr(ulak_main, "_write_update_incomplete_marker", lambda: None)
    monkeypatch.setattr(ulak_main, "_clear_update_incomplete_marker", lambda: None)
    ...
    monkeypatch.setattr(ulak_main, "_build_web_ui", lambda *a, **k: None)
    ...
    monkeypatch.setattr(update_cmd, "_purge_stale_ulak_modules", lambda: None)
    monkeypatch.setattr(ulak_main, "_purge_stale_ulak_modules", lambda: None)

    import ulak_cli.gateway as ulak_gateway

    monkeypatch.setattr(
        ulak_gateway, "find_gateway_pids", lambda **_kwargs: []
    )
    monkeypatch.setattr(ulak_gateway, "supports_systemd_services", lambda: False)
    monkeypatch.setattr(
        ulak_gateway, "find_profile_gateway_processes", lambda *a, **k: []
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "ulak_cli.update_inventory.collect_runtime_inventory",
...
def test_marker_round_trip_under_ulak_home():
    path = update_cmd._fleet_restart_pending_marker_path()
    assert path.parent == get_ulak_home()
...
def test_pending_needed_when_unfinished_receipt_runtime_sha_skews(monkeypatch):
    ...
    receipt_dir = get_ulak_home() / "logs" / "update_receipts"
...
def test_run_pending_restart_true_when_no_gateways(monkeypatch, capsys):
    monkeypatch.setattr(
        "ulak_cli.gateway.find_gateway_pids", lambda **k: []
    )
    monkeypatch.setattr(ulak_main, "_purge_stale_ulak_modules", lambda: None)
...
def test_clean_update_escalates_surviving_serve_as_unaccounted(
    ...
    ``ulak-gateway.service``) and an unmanaged ``serve`` on the same
    ...
    import ulak_cli.update_inventory as ui
    ...
    import ulak_cli.process_identity as pi
    ...
    latest = get_ulak_home() / "logs" / "update_receipts" / "latest.json"
...
def test_interrupt_between_pull_and_restart_leaves_marker(
    ...
    monkeypatch.setattr(ulak_main, "_clear_bytecode_cache", _interrupt)
...
def test_startup_warn_prints_when_marker_present(capsys):
    ...
    assert "ulak gateway restart" in err
