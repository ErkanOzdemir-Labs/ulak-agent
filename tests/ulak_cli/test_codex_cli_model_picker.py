...
Covers:
 - Normal path (tokens already in Ulak auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Ulak
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `ulak auth openai-codex`. The old
...
@pytest.fixture()
def ulak_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Ulak auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(ulak_auth_only_env):
    """openai-codex appears when tokens are already in Ulak auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Ulak
    auth store."""
    ulak_home = tmp_path / ".ulak"
    ulak_home.mkdir()
