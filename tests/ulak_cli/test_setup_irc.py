"""Tests for IRC gateway configuration via `ulak setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Ulak install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Ulak install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#ulak")
            monkeypatch.setenv("IRC_NICKNAME", "ulak-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `ulak setup gateway` experience with IRC present."""
