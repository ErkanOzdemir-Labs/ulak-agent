...
def test_fast_serve_launch_dispatches_only_unambiguous_serve(monkeypatch) -> None:
    ...
    monkeypatch.setattr(sys, "argv", ["ulak", "serve", "--host", "127.0.0.1", "--port", "0"])
    ...
    for argv in (["serve", "--future-flag"], ["serve", "--help"], ["chat"]):
        monkeypatch.setattr(sys, "argv", ["ulak", *argv])
        assert main_mod._try_fast_serve_launch() is False
    monkeypatch.setenv("ULAK_DISABLE_FAST_SERVE_LAUNCH", "1")
    monkeypatch.setattr(sys, "argv", ["ulak", "serve"])
    assert main_mod._try_fast_serve_launch() is False
    monkeypatch.delenv("ULAK_DISABLE_FAST_SERVE_LAUNCH")
