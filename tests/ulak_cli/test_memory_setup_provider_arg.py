"""Tests for `ulak memory setup [provider]` routing.
...
``ulak memory setup honcho``) without the interactive picker — which matters
because the per-provider ``ulak <provider>`` subcommand is only registered
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "ulak memory setup" in out
...
class TestInstallDependenciesRunner:
    ...
    def _run_with_missing_dep(self, tmp_path, which_side_effect, run_behavior=None):
        ...
        # The hermetic conftest sets ULAK_DISABLE_LAZY_INSTALLS=1 so no test
        ...
        with _patch.dict(os.environ, {"ULAK_DISABLE_LAZY_INSTALLS": "0"}), \
             patch("plugins.memory.find_provider_dir", return_value=tmp_path), \
             patch("ulak_cli.tools_config_cua.shutil.which", side_effect=which_side_effect), \
             patch("ulak_cli.tools_config_cua.subprocess.run", fake_run):
