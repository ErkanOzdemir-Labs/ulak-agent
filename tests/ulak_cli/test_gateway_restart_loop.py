...
Covers:
- Defense 1: gateway stop/restart refuse when _ULAK_GATEWAY=1
...
from ulak_cli.cron import cron_command
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "ulak gateway stop",
        "ulak gateway uninstall",
        "ulak  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "ULAK GATEWAY RESTART",           # uppercase
    ])
    def test_ulak_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.ulak.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.ulak/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l ulak-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.ulak.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.ulak.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.ulak.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.ulak/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.ulak.gateway-apollo:/a.plist' "
            "'ai.ulak.gateway-cronus:/c.plist' 'ai.ulak.gateway-plutus:/p.plist' "
            "'ai.ulak.gateway:/Users/botuser/Library/LaunchAgents/ai.ulak.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated ulak note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.ulak.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.ulak.gateway',
        "launchctl kick'start' -k gui/501/ai.ulak.gateway",
        "launchctl kick\\start -k gui/501/ai.ulak.gateway",
        'launchctl "kickstart" -k gui/501/ai.ulak.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.ulak.gateway',
        "launchctl dis\\able gui/501/ai.ulak.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.ulak."gateway"',
        # Same class on the systemctl and ulak-CLI branches.
        'systemctl re"start" ulak-gateway',
        'ulak gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.ulak.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.ulak.update-checker',
        'systemctl re"start" ulak-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "ulak cron list",
        "ulak update",
        "ulak config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `ulak gateway start` is benign — starting a gateway from inside a
        ...
        "ulak gateway start",
        "ulak gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway ulak
        # services must not be falsely blocked (the old `.*ulak` matched any
        # ulak token).
        "launchctl unload ai.ulak.update-checker.plist",
        "launchctl restart ai.ulak.daemon",
        ...
        "systemctl restart ulak-meta.service",
        "systemctl restart ulak-cron-helper",
        ...
        "echo after the ulak gateway restarted cleanly",
        "the ulak gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "ulak skill view gateway-notes && echo ulak gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `ulak` is a path component
        # there, not a command.
        "cat '/docs/ulak gateway restart-notes.md'",
        "less /home/user/notes/ulak gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "ulak gateway restart",
        "ulak gateway restart; echo done",
        "ulak gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;ulak gateway restart",
        "true && ulak gateway stop",
        "echo $(ulak gateway restart)",
        "echo `ulak gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.ulak.gateway"])',
        'subprocess.run(["ulak", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.ulak.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["ulak", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: ulak gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `ulak -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry ULAK_HOME/ULAK_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("ULAK_PROFILE", "zeus")
        monkeypatch.delenv("ULAK_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "ulak -p zeus gateway stop",
        "ulak -p zeus gateway restart",
        "ulak --profile zeus gateway restart",
        "ulak --profile zeus gateway stop",
        "ulak --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "ulak -v -p zeus gateway restart",
        "ulak -p zeus -v gateway restart",
        "ulak --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "ulak -p venus gateway stop",
        "ulak -p venus gateway restart",
        "ulak --profile venus gateway restart",
        "ulak --profile=venus gateway stop",
        "ulak -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "ulak -p zeus gateway start",
        "ulak -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("ulak gateway restart")
        assert _contains_gateway_lifecycle_command("ulak gateway stop")

    def test_ulak_home_derived_profile(self, monkeypatch):
        # Without ULAK_PROFILE the guard falls back to the ULAK_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("ULAK_PROFILE", raising=False)
        monkeypatch.delenv("ULAK_PROFILE_NAME", raising=False)
        import ulak_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("ulak -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("ulak -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("ulak -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("ulak gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_ulak_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade ulak then run ulak gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_ulak_gateway_once.sh). The script must live under
        # ULAK_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("ULAK_HOME", str(tmp_path / ".ulak"))
        scripts_dir = tmp_path / ".ulak" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_ULAK_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _ULAK_GATEWAY=1.

    Issue #37453: systemctl --user restart ulak-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart ulak-gateway",
        "systemctl --user restart ulak-gateway",
        "systemctl stop ulak-gateway.service",
        "ulak gateway restart",
        "ulak gateway uninstall",
        "launchctl kickstart gui/501/ai.ulak.gateway",
        "launchctl bootout gui/501/ai.ulak.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.ulak.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.ulak/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.ulak.gateway.restart-once.plist",
        "pkill -f ulak.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart ulak-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.ulak.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-ulak label: label-independent detection is the point
        # (#62891 second reproduction used `ai.ulak.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.ulak.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _ULAK_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _ULAK_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_ULAK_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="ulak gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["ulak gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.ulak.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.ulak.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-ulak systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run ulak gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("ulak gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfeulak gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under ULAK_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("ulak gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/ulak\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 ulak gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_ulak_home()` falls back to Path.home(), which raises
        RuntimeError when neither ULAK_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart ulak-gateway' .",
        "grep -rn 'ulak gateway restart' ./logs",
        "rg 'ulak gateway restart' ../archive",
        "grep -c 'systemctl stop ulak-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM ulak_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell ulak gateway restart"',
        'sqlite3 ./db ".system systemctl restart ulak-gateway"',
        'psql ./x -c "\\! systemctl restart ulak-gateway"',
        "grep -r 'ulak gateway restart' . | sh",
        "grep -r 'ulak gateway restart' ./logs | bash",
        "grep -r 'ulak gateway restart' . | sudo sh",
        "grep -r 'x' . ; ulak gateway restart",
        "grep -r 'x' . && systemctl restart ulak-gateway",
        'grep -r "$(ulak gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart ulak-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell ulak gateway restart"',
        'sqlite3 db ".system systemctl restart ulak-gateway"',
        'psql -c "\\! systemctl restart ulak-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    ulak_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run ulak gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ULAK_HOME", str(tmp_path / ".ulak"))
        (tmp_path / ".ulak").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run ulak gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop ulak-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart ulak-gateway' /var/log/syslog",
        "rg 'ulak gateway restart' /home/user/.ulak/logs/",
        "journalctl -u ulak-gateway --grep 'systemctl restart ulak-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'ulak_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell ulak gateway restart"',
        'psql -c "\\! systemctl restart ulak-gateway"',
        "grep 'systemctl restart ulak-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart ulak-gateway",
        'grep "$(systemctl restart ulak-gateway)" f',
        "grep 'restart' log; systemctl restart ulak-gateway",
        'sqlite3 db "SELECT 1"; ulak gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "ulak gateway restart",
        "sudo systemctl stop ulak-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart ulak-gateway%'\""
        )
