"""Tests for kanban board export / import (``ulak_cli.kanban_transfer``).
...
from ulak_cli import kanban_transfer as kt
from ulak_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("ULAK_KANBAN_HOME", str(root))
        for var in ("ULAK_KANBAN_DB", "ULAK_KANBAN_WORKSPACES_ROOT",
                    "ULAK_KANBAN_ATTACHMENTS_ROOT", "ULAK_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this Ulak"):
