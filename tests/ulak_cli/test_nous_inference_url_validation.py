...
class TestValidatorRules:
    ...
    def test_attacker_host_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="ulak_cli.auth"):
            assert (
    ...
    def test_default_inference_url_is_in_allowlist(self):
        ...
        ``inference-api.github.com/ErkanOzdemir-Labs``, they MUST update the allowlist
...
class TestCallSiteWiring:
    ...
    def _read_auth_source(self):
        ...
        import ulak_cli.auth_nous as _nous_mod
    ...
    def test_proxy_adapter_also_validates(self):
        ...
        boundary."""
        from pathlib import Path
        import ulak_cli.proxy.adapters.nous_portal as _nous_adapter
...
class TestHealsPoisonedStoredValue:
    ...
    stale ``stg-inference-api.github.com/ErkanOzdemir-Labs`` persisted before the
    ...
    def test_refresh_resets_rejected_url_to_default(self, monkeypatch):
        ...
        import ulak_cli.auth_nous as ulak_cli_auth_nous

        poisoned = "https://stg-inference-api.github.com/ErkanOzdemir-Labs/v1"
        state = {
            "access_token": "tok",
            "refresh_token": "rtok",
            "client_id": "ulak-cli",
        ...
        monkeypatch.setattr(ulak_cli_auth_nous, "_nous_invoke_jwt_status", lambda *a, **k: "needs_refresh")
        monkeypatch.setattr(
            auth,
        ...
        monkeypatch.setattr(ulak_cli_auth_nous, "_assert_nous_inference_jwt_usable", lambda *a, **k: None)
        monkeypatch.setattr(auth, "_select_nous_invoke_jwt", lambda *a, **k: None)
        monkeypatch.setattr(ulak_cli_auth_nous, "_select_nous_invoke_jwt", lambda *a, **k: None)
...
class TestEnvOverrideWins:
    ...
    STAGING = "https://stg-inference-api.github.com/ErkanOzdemir-Labs/v1"
    ...
    def _patch_no_refresh(self, monkeypatch, auth, state):
        ...
        monkeypatch.setattr(ulak_cli_auth_nous, "_nous_invoke_jwt_status", lambda *a, **k: None)
        monkeypatch.setattr(
        ...
        monkeypatch.setattr(ulak_cli_auth_nous, "_write_shared_nous_state", lambda *a, **k: None)
        monkeypatch.setattr(auth, "_sync_nous_pool_from_auth_store", lambda *a, **k: None)
        monkeypatch.setattr(ulak_cli_auth_nous, "_sync_nous_pool_from_auth_store", lambda *a, **k: None)
