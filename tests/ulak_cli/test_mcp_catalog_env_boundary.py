...
@pytest.fixture
def catalog_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, _isolate_ulak_home):
    ...
    monkeypatch.setenv("ULAK_OPTIONAL_MCPS", str(catalog))
...
def test_catalog_rejects_undeclared_key_before_any_write_or_install(
    ...
    import ulak_cli.mcp_catalog as mcp_catalog
...
def test_catalog_cannot_declare_reserved_control_key(
    ...
    catalog_root = Path(os.environ["ULAK_OPTIONAL_MCPS"])
    ...
    response = client.post(
        "/api/mcp/catalog/install",
        headers=HEADERS,
        json={"name": "demo", "env": {"ULAK_YOLO_MODE": "1"}},
    )
    ...
    assert installs == []
    env_path = catalog_env / ".env"
    assert not env_path.exists() or "ULAK_YOLO_MODE" not in env_path.read_text(
        encoding="utf-8"
    )
...
def test_rejected_copilot_controls_do_not_change_live_resolvers(
    ...
    monkeypatch.setenv("ULAK_COPILOT_ACP_COMMAND", "/opt/trusted/copilot")
    monkeypatch.setenv("ULAK_COPILOT_ACP_ARGS", "--acp --stdio")
    expected_command = _resolve_command()
    expected_args = _resolve_args()

    attempts = {
        "ULAK_COPILOT_ACP_COMMAND": "/tmp/attacker-command",
        "ULAK_COPILOT_ACP_ARGS": "--malicious-transport",
    }
...
def test_preexisting_copilot_controls_remain_usable(
    ...
    monkeypatch.setenv("ULAK_COPILOT_ACP_COMMAND", "parent-placeholder")
    monkeypatch.setenv("ULAK_COPILOT_ACP_ARGS", "--parent-placeholder")
    (catalog_env / ".env").write_text(
        "ULAK_COPILOT_ACP_COMMAND=/opt/operator/copilot\n"
        "ULAK_COPILOT_ACP_ARGS=--acp --stdio --operator-mode\n",
        encoding="utf-8",
    )

    load_ulak_dotenv(
        ulak_home=catalog_env,
