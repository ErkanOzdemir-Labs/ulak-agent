...
def test_code_scoped_stamp_wins_over_home_stamp(tmp_path):
    """The stamp next to the running code is authoritative over $ULAK_HOME.

    Models a host git install whose $ULAK_HOME is shared with (and stamped
    'docker' by) a co-located container. The code-scoped stamp must win so the
    host install is correctly identified as 'git' and 'ulak update' works.
    """
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    ...
    with patch("ulak_cli.config.get_managed_system", return_value=None), \
         patch("ulak_cli.config.get_ulak_home", return_value=home):
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    image, so ``ulak update`` refused to run. With a ``.git`` checkout it
    must resolve to ``git``.
    """
    (tmp_path / ".git").mkdir()
    with patch("ulak_cli.config.get_managed_system", return_value=None), \
         patch("ulak_cli.config.get_ulak_home", return_value=tmp_path), \
         patch("ulak_constants.is_container", return_value=True):
        from ulak_cli.config import detect_install_method
