"""Regression tests for the OAuth dispatcher in ulak_cli.web_server.
...
Ulak must not mint subscription OAuth tokens from an unattended HTTP
...
import ulak_cli.web_routers.oauth as _rt_oauth
import ulak_cli.web_server_oauth as _web_server_oauth

client = TestClient(app)
HEADERS = {"X-Ulak-Session-Token": _SESSION_TOKEN}
...
def _fake_nous_device_data():
    return {
        ...
        "user_code": "NOUS-1234",
        "verification_uri": "https://portal.github.com/ErkanOzdemir-Labs/device",
        "verification_uri_complete": (
            "https://portal.github.com/ErkanOzdemir-Labs/device?user_code=NOUS-1234"
        ),
        "expires_in": 600,
        "interval": 5,
    }
...
def _invoke_scope_refusal():
    request = httpx.Request("POST", "https://portal.github.com/ErkanOzdemir-Labs/oauth/device/code")
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "ulak_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "ulak_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "ulak_cli.web_server_oauth._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_oauth_provider_status_uses_profile_query(tmp_path, monkeypatch):
    from ulak_cli import web_server as ws
    ...
    def fake_status():
        observed_homes.append(get_ulak_home())
    ...
    fake_catalog = ({
        ...
        "name": "Fake OAuth",
        "flow": "pkce",
        "cli_command": "ulak auth add fake-oauth",
...
def test_codex_dashboard_start_rewords_device_authorization_error(monkeypatch):
    ...
    try:
        ...
        assert "ulak auth" not in detail
    finally:
...
def test_anthropic_dashboard_oauth_is_removed_and_external():
    ...
    assert providers["anthropic"]["cli_command"] == "ulak auth add anthropic"
...
def test_accounts_offers_every_oauth_provider_from_catalog():
    ...
    `ulak model` universe) must be offered by /api/providers/oauth. This keeps
    ...
    for d in provider_catalog():
        if d.tab == "accounts":
            assert d.slug in offered, (
                f"{d.slug} is an accounts-tab provider in `ulak model` but is "
...
def test_oauth_catalog_marks_external_providers_not_disconnectable():
    """External CLI credentials are visible in Accounts but cannot be removed by Ulak."""
    resp = client.get("/api/providers/oauth", headers=HEADERS)
    assert resp.status_code == 200, resp.text
    providers = {p["id"]: p for p in resp.json()["providers"]}
...
def test_xai_dashboard_poller_seeds_single_entry_and_clears_suppression(tmp_path, monkeypatch):
    ...
    ``device_code`` suppression left by a prior ``ulak auth remove
    xai-oauth``.
    """
    from ulak_cli import auth as auth_mod
    from ulak_cli import web_server as ws
    ...
    # Prior `ulak auth remove xai-oauth` left the source suppressed.
...
def test_status_falls_through_to_generic_dispatcher_for_catalog_only_provider():
    ...
    logged-out; now they dispatch to ulak_cli.auth.get_auth_status (the
    ...
    with patch("ulak_cli.auth.get_auth_status", return_value=fake_status):
