...
import ulak_cli.local_runtime.catalog as cat
...
def _doc_from(entries):
    ...
    doc = json.loads(files("ulak_cli.local_runtime")
...
def test_refresh_swaps_in_memory_only(monkeypatch, tmp_path):
    ...
    packaged_path = files("ulak_cli.local_runtime").joinpath("catalog.json")
...
def test_min_engine_gate(monkeypatch):
    from ulak_cli.web_routers.local_models import _engine_too_old

    monkeypatch.setattr("ulak_cli.local_runtime.binaries.installed_tags",
                        lambda: ["b10362"])
