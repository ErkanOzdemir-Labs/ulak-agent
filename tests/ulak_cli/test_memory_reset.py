"""Tests for the `ulak memory reset` CLI command.
...
- Profile-scoped reset (uses ULAK_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ULAK_HOME with memory files."""
    ulak_home = tmp_path / ".ulak"
    memories = ulak_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("ULAK_HOME", str(ulak_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nUlak repo is at ~/.ulak/ulak-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `ulak memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        ulak_home = tmp_path / ".ulak"
        (ulak_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ULAK_HOME", str(ulak_home))
