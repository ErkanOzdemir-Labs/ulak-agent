...
ULAK_HOME (tests/conftest.py isolation) and assert every store agrees
afterwards.
...
@pytest.fixture
def ulak_home(monkeypatch, tmp_path):
    """Fresh ULAK_HOME with .env + auth.json + config.yaml fixtures."""
...
def test_delete_clears_provider_models_cache(ulak_home):
    _write_env(ulak_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(ulak_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    cache_path = ulak_home / "provider_models_cache.json"
...
def test_update_rotates_config_yaml_model_mirror(ulak_home):
    old = "sk-oe-" + "f" * 24
    new = "sk-oe-" + "g" * 24
    _write_env(ulak_home, OPENAI_API_KEY=old)
    _write_config(
        ulak_home,
        "model:\n"
        "  provider: custom\n"
    ...
    cfg_text = ulak_home.joinpath("config.yaml").read_text(encoding="utf-8")
...
# load_pool() or a separate `ulak auth add`.
# ---------------------------------------------------------------------------
...
def test_put_api_env_materializes_credential_pool_entry(ulak_home):
    ...
    the old cached credential until a separate ``ulak auth add opencode-go``
...
def test_put_api_env_writes_auth_json_for_provider(ulak_home):
    """Sentinel for #96058: PUT /api/env must modify auth.json on disk.

    The reported symptom was ``stat -c '%y' ~/.ulak/auth.json`` returning
...
def test_update_rotates_keyed_providers_mirror(ulak_home):
    old = "sk-kp-" + "n" * 24
    new = "sk-kp-" + "o" * 24
    _write_env(ulak_home, OPENAI_API_KEY=old)
    _write_config(
...
def test_delete_scrubs_keyed_providers_mirror(ulak_home):
    old = "sk-kp-" + "p" * 24
    _write_env(ulak_home, OPENAI_API_KEY=old)
    _write_config(
        ulak_home,
...
def test_scrub_never_touches_providers_base_url_alias(ulak_home):
