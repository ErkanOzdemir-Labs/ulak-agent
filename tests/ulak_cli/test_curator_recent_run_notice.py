...
The notice prints the most recent curator run summary on `ulak update`,
...
session start) so users normally never see the rename map. `ulak update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    from ulak_cli import update_cmd_maint
