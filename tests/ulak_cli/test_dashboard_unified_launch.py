...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "ulak_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # ULAK_HOME.  For a standard install (ULAK_HOME unset) that root is
        # the platform-native default (~/.ulak), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from ulak_constants import get_default_ulak_root
        assert env.get("ULAK_HOME") == str(get_default_ulak_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (ULAK_DESKTOP=1) must NOT
