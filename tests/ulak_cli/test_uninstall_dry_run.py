...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_ulak_home", lambda: ulak_home)
    monkeypatch.setattr(uninstall, "_is_default_ulak_home", lambda home: False)
    ...
    assert str(ulak_home) in output
    assert project_root.exists()
    assert ulak_home.exists()
