...
- ``ulak_cli/_subprocess_compat.pid_is_ulak`` / ``kill_process_tree``
- ``ulak_cli/dashboard_procs._kill_stale_dashboard_processes`` (win32)
- ``ulak_cli/update_cmd._stop_process_trees``
...
from ulak_cli import _subprocess_compat
...
class TestPidIsUlak:
    ...
    def test_non_windows_is_unconditional_pass(self):
        ...
        with mock.patch.object(_subprocess_compat, "IS_WINDOWS", False):
            assert _subprocess_compat.pid_is_ulak(1234) is True
    ...
    def test_non_windows_still_rejects_recycled_identity(self):
        ...
        with mock.patch.object(_subprocess_compat, "IS_WINDOWS", False), mock.patch.object(
            ...
            assert _subprocess_compat.pid_is_ulak(
                1234, expected_start_time=123
            ) is False

    def test_ulak_match_requires_token_boundary(self):
        # "ulak" buried inside an unrelated path segment must not match.
        assert _subprocess_compat._text_names_ulak(
            r"c:\users\sulaka\app.exe"
        ) is False
        assert _subprocess_compat._text_names_ulak(
            r"C:\Users\x\.ulak-runtime\python.exe -m ulak_cli.main"
        ) is True
        assert _subprocess_compat._text_names_ulak(
    ...
    def test_invalid_pid_inputs_do_not_crash(self):
        with mock.patch.object(_subprocess_compat, "IS_WINDOWS", True):
            assert _subprocess_compat.pid_is_ulak(-1) is False
            assert _subprocess_compat.pid_is_ulak(0) is False
            assert _subprocess_compat.pid_is_ulak("not-a-pid") is False
            assert _subprocess_compat.pid_is_ulak(True) is False

    def test_probe_matches_ulak_like_process(self):
        with mock.patch.object(_subprocess_compat, "IS_WINDOWS", True), mock.patch.object(
            ...
            _subprocess_compat, "_process_command_is_ulak", return_value=True
        ):
            assert _subprocess_compat.pid_is_ulak(1234) is True
    ...
    def test_probe_rejects_foreign_process(self):
        with mock.patch.object(_subprocess_compat, "IS_WINDOWS", True), mock.patch.object(
            ...
            _subprocess_compat, "_process_command_is_ulak", return_value=False
        ):
            assert _subprocess_compat.pid_is_ulak(1234) is False
    ...
    def test_missing_pid_real_probe_fails_closed(self):
        # A PID that cannot exist must never be judged Ulak-owned.
        assert _subprocess_compat.pid_is_ulak(2**24) is False
...
class TestKillProcessTree:
    ...
    def test_retained_handle_is_taskkilled_without_probe(self):
        with mock.patch.object(_subprocess_compat, "IS_WINDOWS", True), mock.patch.object(
            _subprocess_compat, "pid_is_ulak"
...
class TestStopProcessTrees:
    ...
    def test_foreign_pids_only_probed(self):
        with mock.patch(
            ...
            "ulak_cli._subprocess_compat.pid_is_ulak", return_value=False
        ), mock.patch.object(update_cmd.subprocess, "run") as run:
            update_cmd._stop_process_trees([1111, 2222])
        run.assert_not_called()

    def test_ulak_pid_probed_then_taskkilled(self):
        with mock.patch(
            "gateway.status.get_process_start_time", return_value=123
        ), mock.patch(
            "ulak_cli._subprocess_compat.pid_is_ulak", return_value=True
        ), mock.patch.object(
...
class TestKillStaleDashboardProcesses:
    ...
    def test_foreign_pid_reported_not_killed(self):
        ...
        assert result["failed"] == [
            (12345, "not ulak-owned or process identity changed")
        ]
        run.assert_not_called()

    def test_ulak_pid_killed(self):
        with self._patch_find(), mock.patch.object(
            dashboard_procs.sys, "platform", "win32"
        ), mock.patch(
            "gateway.status.get_process_start_time", return_value=123
