...
Run: pytest tests/ulak_cli/test_dashboard_auth_native_flow.py
"""
...
from ulak_cli.dashboard_auth import native_flow
from ulak_cli.dashboard_auth.base import Session
...
def _walk_native_login(client, *, redirect_uri, challenge, state="cli-state"):
    ...
    assert "ulak_session_at" not in set_cookie, (
...
def test_native_authorize_empty_provider_password_only_brokers_to_login(
    ...
    # in it.
    from ulak_cli.dashboard_auth.cookies import parse_pkce_payload
...
@pytest.fixture
def pw_gated_client():
    from ulak_cli.dashboard_auth.routes import _reset_password_rate_limit
    from tests.ulak_cli.test_dashboard_auth_password_login import (
        PasswordProvider,
    )
...
def test_password_login_without_broker_still_mints_cookies(pw_gated_client):
    ...
    assert r.json()["next"] == "/"
    set_cookie = r.headers.get("set-cookie", "")
    assert "ulak_session_at" in set_cookie
