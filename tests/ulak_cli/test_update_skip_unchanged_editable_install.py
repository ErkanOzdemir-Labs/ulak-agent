"""``ulak update`` skips the editable reinstall when the pull can't affect it.
...
that rewrite is the only reason the running ``ulak.exe`` gets quarantined,
...
from ulak_cli.update_cmd import _editable_install_is_current
