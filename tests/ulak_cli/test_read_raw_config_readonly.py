...
@pytest.fixture()
def isolated_ulak_home():
    """Per-test ULAK_HOME dir (already redirected by the autouse conftest
...
def test_freshness_after_config_edit(isolated_ulak_home):
    from ulak_cli.config import read_raw_config_readonly

    cfg = _write_config(isolated_ulak_home, {"display": {"ephemeral_system_ttl": 1}})
    first = read_raw_config_readonly()
    assert first["display"]["ephemeral_system_ttl"] == 1

    _write_config(isolated_ulak_home, {"display": {"ephemeral_system_ttl": 7}})
...
def test_missing_config_returns_empty(isolated_ulak_home):
    from ulak_cli.config import read_raw_config_readonly

    cfg = isolated_ulak_home / "config.yaml"
    if cfg.exists():
        cfg.unlink()
