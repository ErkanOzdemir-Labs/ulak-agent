"""Tests for ulak_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $ULAK_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Ulak-managed node dir.
"""
...
def _make_ulak_node(ulak_home: Path) -> Path:
    """Create a fake $ULAK_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = ulak_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    ulak_home = fake_home / ".ulak"
    _make_ulak_node(ulak_home)
    ...
    removed = uninstall.remove_node_symlinks(ulak_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    ulak_home = fake_home / ".ulak"
    node_bin = _make_ulak_node(ulak_home)
