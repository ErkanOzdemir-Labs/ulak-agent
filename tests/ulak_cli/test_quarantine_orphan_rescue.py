"""Regression tests: a failed quarantine restore must never strand `ulak`.

On Windows the updater renames the live ``ulak*.exe`` shims aside
(``ulak.exe.old.<unix-ms>``) so uv can write replacements. Gaps in the
recovery path ended with ``ulak`` gone from PATH — and, because the command
that repairs it IS ``ulak update``, unrecoverable without a manual reinstall
(#75584):
...
from ulak_cli import _early_recovery as er
...
def test_cleanup_rescue_reports_when_it_cannot_recover(tmp_path, capsys):
    ...
    assert "FAILED to restore ulak.exe" in captured.err
...
def test_cleanup_rescues_newest_by_parsed_stamp_not_lexicographic(tmp_path):
    ...
    """
    scripts = _make_scripts_dir(tmp_path)
    (scripts / "ulak.exe.old.999").write_bytes(b"MZ-stray-short-stamp")
    (scripts / f"ulak.exe.old.{_stamp(60_000)}").write_bytes(b"MZ-genuine")

    with _run_cleanup(scripts):
        cli_main._cleanup_quarantined_exes(scripts)

    assert (scripts / "ulak.exe").read_bytes() == b"MZ-genuine"


def test_cleanup_ignores_names_it_did_not_create(tmp_path):
    """An unparseable suffix is not ours: never rescued, never deleted."""
    scripts = _make_scripts_dir(tmp_path)
    (scripts / "ulak.exe").write_bytes(b"MZ-live")
    foreign = scripts / "ulak.exe.old.backup"
    ...
    assert (scripts / "ulak.exe").read_bytes() == b"MZ-live"
...
def test_cleanup_does_not_rescue_from_a_foreign_name(tmp_path):
    ...
    assert not (scripts / "ulak.exe").exists()
...
def test_cleanup_leaves_fresh_quarantine_for_concurrent_update(tmp_path):
    ...
    scripts = _make_scripts_dir(tmp_path)
    (scripts / "ulak.exe").write_bytes(b"MZ-live")
    fresh = scripts / f"ulak.exe.old.{_stamp()}"
...
def test_cleanup_still_sweeps_genuinely_stale_quarantine(tmp_path):
    ...
    stale = scripts / f"ulak.exe.old.{_stamp(ancient_ms)}"
...
def test_quarantine_stamp_ms_parses_and_rejects():
    assert main_install_repair._quarantine_stamp_ms(Path("ulak.exe.old.1787020473885")) == 1787020473885
    assert main_install_repair._quarantine_stamp_ms(Path("ulak.exe.old.backup")) is None
    assert main_install_repair._quarantine_stamp_ms(Path("ulak.exe")) is None
...
def test_helper_retries_then_succeeds(tmp_path):
    scripts = _make_scripts_dir(tmp_path)
    quarantined = scripts / "ulak.exe.old.123"
    quarantined.write_bytes(b"MZ-old-ulak")
    original = scripts / "ulak.exe"
    ...
    assert failed == []
    assert original.read_bytes() == b"MZ-old-ulak"
    assert calls["n"] >= 2
...
def test_helper_reports_failure_and_returns_the_pair(tmp_path, capsys):
    ...
    assert "FAILED to restore ulak.exe" in captured.err
    assert "ulak.exe.old.123" in captured.err
