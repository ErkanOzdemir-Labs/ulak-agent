...
#88251: ``ulak update`` treated a failed desktop pack as non-fatal, printed
...
import ulak_cli.update_cmd_maint as update_cmd_maint
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "ulak_constants.with_ulak_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "ulak_constants.display_ulak_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_ulak_home", lambda: tmp_path)
