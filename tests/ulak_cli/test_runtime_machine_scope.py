...
def profile_home(tmp_path, monkeypatch):
    """A NAMED-profile ULAK_HOME under <root>/profiles/<name>."""
    root = tmp_path / ".ulak"
    profile = root / "profiles" / "coder"
    profile.mkdir(parents=True)
    monkeypatch.setenv("ULAK_HOME", str(profile))
    # ulak_constants memoizes root resolution per (native, env) pair;
...
def test_models_and_runtimes_resolve_to_the_shared_root(profile_home):
    ...
    import ulak_cli.local_runtime.binaries as binaries
    import ulak_cli.local_runtime.bootstrap as bootstrap
...
def test_all_runtime_state_follows_runtimes_root(profile_home):
    ...
    root, profile = profile_home
    from ulak_cli.local_runtime.growth import window_overrides_path
...
def test_default_profile_paths_unchanged(tmp_path, monkeypatch):
    """ULAK_HOME at the root itself (default profile) resolves exactly
