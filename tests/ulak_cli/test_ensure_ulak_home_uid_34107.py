"""Regression tests for #34107 — Docker UID/GID handling in ensure_ulak_home.

When Ulak runs in Docker with ``ULAK_UID=1000`` / ``ULAK_GID=911``,
the entrypoint chowns the top-level ``ULAK_HOME`` once at startup. But
subdirectories created at runtime by ``ensure_ulak_home()`` — especially
...
The fix is a ``_chown_to_ulak_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_ulak_uid_gid
# ---------------------------------------------------------------------------


class TestResolveUlakUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("ULAK_UID", "1000")
        monkeypatch.setenv("ULAK_GID", "911")
        from ulak_cli.config import _resolve_ulak_uid_gid
...
class TestChownToUlakUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_ulak_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("ULAK_UID", raising=False)
        monkeypatch.delenv("ULAK_GID", raising=False)
        from ulak_cli import config as cfg
