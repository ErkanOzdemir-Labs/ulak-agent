...
def test_mechanism_ids_are_machine_readable_and_described():
    ...
    assert describe_restart_mechanism("manual", "default") == "ulak gateway restart"
    assert "sc.exe" in describe_restart_mechanism("windows-service", "default")


def test_windows_service_supervisor_classification():
    from ulak_cli.update_inventory import _detect_supervisor_for_pid
...
def test_windows_service_runtime_reconciles_via_service_profiles():
    ...
    outcomes = match_runtime_outcomes(
        ...
        restarted_services=["ulak-gateway"], relaunched_profiles=["default"],
...
def test_failed_unit_is_failed():
    outcomes = match_runtime_outcomes(
        ...
        failed_units=["ulak-gateway-work.service"],
    )
...
def test_restarted_service_unit_matches_profile():
    outcomes = match_runtime_outcomes(
        ...
        restarted_services=["ulak-gateway.service"], relaunched_profiles=[],
        externally_supervised_profiles=[], killed_pids=set(), failed_units=[],
    )
    assert outcomes[0]["outcome"] == "restarted"


def test_launchd_default_gateway_restarted_via_ai_ulak_label():
    """macOS restart bookkeeping records ``ai.ulak.gateway``, which does not
    contain the substring ``ulak-gateway``. The default-profile gateway must
    ...
    outcomes = match_runtime_outcomes(
        ...
        restarted_services=["ai.ulak.gateway"], relaunched_profiles=[],
        externally_supervised_profiles=[], killed_pids=set(), failed_units=[],
    )
    assert outcomes[0]["outcome"] == "restarted"
    assert report_unaccounted_runtimes(outcomes) is False
...
def test_launchd_named_profile_and_failed_label():
    restarted = match_runtime_outcomes(
        ...
        restarted_services=["ai.ulak.gateway-work"], relaunched_profiles=[],
        externally_supervised_profiles=[], killed_pids=set(), failed_units=[],
    )
...
def test_untouched_runtime_is_unaccounted_and_escalates(capsys):
    ...
    outcomes = match_runtime_outcomes(
        _plan(_rt("coder", 500)),
        restarted_services=["ulak-gateway.service"],
    ...
    assert "ulak -p <profile> gateway restart" in out
...
def test_unmanaged_serve_runtime_under_default_profile_is_unaccounted():
    ...
    outcomes = match_runtime_outcomes(
        ...
        restarted_services=["ulak-gateway"], relaunched_profiles=[],
        externally_supervised_profiles=[], killed_pids=set(), failed_units=[],
    )
...
def test_named_profile_serve_does_not_match_gateway_profile_unit():
    """``ulak-gateway-work.service`` restarted must not credit the ``work``
    serve — the old substring match (``"work" in unit``) did exactly that."""
    outcomes = match_runtime_outcomes(
        _plan(_rt("work", 101, supervisor="systemd"), _serve("work", 901)),
        restarted_services=["ulak-gateway-work.service"], relaunched_profiles=[],
        externally_supervised_profiles=[], killed_pids=set(), failed_units=[],
    )
    by_pid = {o["pid"]: o["outcome"] for o in outcomes}
...
def test_serve_reconciles_against_its_own_unit_vocabulary():
    """A serve IS covered when a ``ulak-serve*`` unit for its profile was
    ...
    outcomes = match_runtime_outcomes(
        ...
        restarted_services=["ulak-gateway", "user/ulak-serve",
                            "ulak-serve-work.service", "ulak-dashboard-ops"],
        relaunched_profiles=[], externally_supervised_profiles=[],
        killed_pids=set(), failed_units=["ulak-serve-qa.service"],
    )
    by_pid = {o["pid"]: o["outcome"] for o in outcomes}
    assert by_pid == {900: "restarted", 901: "restarted", 902: "restarted", 903: "failed"}
    # exact names: ``work`` must not claim ``ulak-serve-workbench``
    outcomes = match_runtime_outcomes(
        _plan(_serve("work", 901)),
        restarted_services=["ulak-serve-workbench.service"], relaunched_profiles=[],
        externally_supervised_profiles=[], killed_pids=set(), failed_units=[],
    )
    assert outcomes[0]["outcome"] == "unaccounted"
...
def test_serve_outcome_follows_incarnation_probe_when_provided():
    ...
    alive is unaccounted — even when a ulak-serve unit was restarted."""
    plan = _plan(_serve("default", 900), _serve("default", 901, kind="dashboard"))
    outcomes = match_runtime_outcomes(
        plan, restarted_services=["ulak-serve.service"], relaunched_profiles=[],
        externally_supervised_profiles=[], killed_pids=set(), failed_units=[],
...
def test_unaccounted_serve_report_names_serve_remedy_not_gateway_restart(capsys):
    ...
    assert "ulak-serve.service" in out
    assert "ulak gateway restart" not in out
