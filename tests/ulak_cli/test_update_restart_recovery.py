...
def test_abort_recovery_hands_managed_profiles_to_a_fresh_process(monkeypatch):
    ...
    assert argv[1:4] == ["-m", "ulak_cli.update_restart_recovery", "--stdin"]
    ...
    assert kwargs["env"]["ULAK_UPDATE_RESTART_RECOVERY"] == "1"
...
def test_service_matching_is_exact_for_overlapping_profile_names():
    assert update_cmd._gateway_service_matches_profile(
        "foo", "ulak-gateway-foo.service"
    )
    assert not update_cmd._gateway_service_matches_profile(
        "foo", "ulak-gateway-foobar.service"
    )
    assert update_cmd._gateway_service_matches_profile(
        "default", "ai.ulak.gateway"
    )
    assert not update_cmd._gateway_service_matches_profile(
        "default", "ai.ulak.gateway-foo"
    )
    # Scope-qualified identities the restart phase may record.
    assert update_cmd._gateway_service_matches_profile(
        "default", "gui/501/ai.ulak.gateway"
    )
    assert update_cmd._gateway_service_matches_profile(
        "foo", "user/ulak-gateway-foo.service"
    )


def test_recovery_child_restarts_each_profile_with_a_fresh_main(monkeypatch):
    recovery = importlib.import_module("ulak_cli.update_restart_recovery")
    calls = []
    ...
    assert [call[0] for call in calls] == [
        [sys.executable, "-m", "ulak_cli.main", "-p", "coder", "gateway", "restart"],
        [sys.executable, "-m", "ulak_cli.main", "-p", "default", "gateway", "restart"],
    ]
    for _, kwargs in calls:
        ...
        assert kwargs["check"] is False
        assert kwargs["env"]["ULAK_UPDATE_RESTART_RECOVERY"] == "1"
        assert "_ULAK_GATEWAY" not in kwargs["env"]
...
def test_recovery_child_verifies_systemd_profiles_via_is_active(monkeypatch):
    ...
    def fake_run(argv, **kwargs):
        ...
        if argv[0].endswith("systemctl"):
            unit = argv[-1]
            active = unit == "ulak-gateway.service"
...
def test_recovery_module_end_to_end_in_a_real_fresh_process(tmp_path):
    ...
    grandchild ``ulak_cli.main … gateway restart`` invocations (recording
    ...
    env["_ULAK_GATEWAY"] = "1"  # must be scrubbed before the grandchild runs
