...
def test_managed_node_swap_defines_staged_and_backup_as_siblings() -> None:
    ...
    assert re.search(r'\$staged\s*=\s*"\$UlakHome\\node\.new-\$stamp"', swap), (
        "$staged must be defined as a sibling of the live tree "
        '("$UlakHome\\node.new-$stamp")'
    )
    assert re.search(r'\$backup\s*=\s*"\$UlakHome\\node\.old-\$stamp"', swap), (
        "$backup must be defined as a sibling of the live tree "
        '("$UlakHome\\node.old-$stamp")'
    )


def test_managed_node_swap_renames_stay_within_ulak_home() -> None:
    swap = _swap_block()
