"""Tests for ulak_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Ulak entry point
(ulak, ulak-agent, ulak-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Ulak entry point imports ulak_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported ulak_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("ulak_bootstrap", None)
    import ulak_bootstrap  # noqa: WPS433
    return ulak_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Ulak CLI
...
class TestEntryPointsImportBootstrap:
    """Every Ulak entry point must import ulak_bootstrap as its
    ...
    # Entry points that invoke Ulak as a process.  Each one must
    # import ulak_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "ulak_cli/main.py",   # ulak CLI (console_script)
        "run_agent.py",          # ulak-agent (console_script)
        "acp_adapter/entry.py",  # ulak-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import ulak_bootstrap' and that
        ...
        half-finished ``ulak update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``ulak_bootstrap`` as a top-level module) leaves ulak
        ...
        """
        # Resolve relative to the ulak-agent repo root.  Tests live
        # at tests/test_ulak_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # ulak start even when ulak_bootstrap hasn't been
        ...
        assert first_import_name == "ulak_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'ulak_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import ulak_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Ulak's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("ULAK_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["ULAK_PYTHON_SRC_ROOT"] = env
            elif "ULAK_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["ULAK_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/ulak")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ULAK_PYTHON_SRC_ROOT", None)
            else:
                os.environ["ULAK_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/ulak", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Ulak root.  The guard must relocate Ulak to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/ulak"])
        assert result[0] == "/opt/ulak"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Ulak root.
        assert result.index("/opt/ulak") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["ULAK_PYTHON_SRC_ROOT"] = "/env/ulak"
            hb.harden_import_path()
            assert sys.path[0] == "/env/ulak"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ULAK_PYTHON_SRC_ROOT", None)
