#!/usr/bin/env python3
"""SWE Runner with Ulak Trajectory Format

Runs tool-calling agent tasks in Ulak-Agent's execution environments (local,
docker, modal) and writes trajectories in Ulak format (from/value pairs with
...
def create_environment(env_type: str = "local", image: str = "python:3.11-slim", cwd: str = "/tmp", timeout: int = 60, **kwargs):
    """Create a Ulak execution environment (``local`` ignores ``image``/``kwargs``)."""
    if env_type == "local":
...
class MiniSWERunner:
    """Tool-calling agent loop over a Ulak execution environment, emitting Ulak trajectories."""
    ...
    def _convert_to_ulak_format(self, messages: List[Dict[str, Any]], user_query: str) -> List[Dict[str, Any]]:
        """Convert the OpenAI-style message list to the Ulak trajectory format used by batch_runner.py."""
        system_msg = ULAK_SYSTEM_PREFIX + f"<tools>\n{self._format_tools_for_system_message()}\n</tools>\n" + ULAK_SYSTEM_SUFFIX
    ...
    def run_task(self, task: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self._cleanup_env()
        return {"conversations": self._convert_to_ulak_format(messages, task), "completed": completed, "api_calls": api_call_count,
...
def main(
    ...
    """
    Run SWE tasks with Ulak trajectory format output.
    ...
    """
    print("🚀 Mini-SWE Runner with Ulak Trajectory Format")
    print("=" * 60)
