"""CLI entry point for the ulak-agent ACP adapter.

Loads ``~/.ulak/.env``, routes logging to stderr (stdout is reserved for ACP
JSON-RPC), and starts the ACP agent server.

Usage::

    python -m acp_adapter.entry   # or: ulak acp / ulak-acp
"""

# IMPORTANT: ulak_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See ulak_bootstrap.py for full rationale.
try:
    import ulak_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Partial ``ulak update`` (git-reset landed, ``uv pip install -e .`` did not):
    # UTF-8 stdio setup is skipped on Windows; POSIX is unaffected.
    pass
else:
    # Stop a ``utils/``/``proxy/``/``ui/`` package in the launch cwd from shadowing Ulak modules.
...
def _load_env() -> None:
    """Load .env from ULAK_HOME (default ``~/.ulak``)."""
    from ulak_cli.env_loader import load_ulak_dotenv

    ulak_home = get_ulak_home()
    loaded = load_ulak_dotenv(ulak_home=ulak_home)
    log = logging.getLogger(__name__)
    for env_file in loaded or ():
        log.info("Loaded env from %s", env_file)
    if not loaded:
        log.info("No .env found at %s, using system env", ulak_home / ".env")


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="ulak-acp", description="Run Ulak Agent as an ACP stdio server.")
    parser.add_argument("--version", action="store_true", help="Print Ulak version and exit")
    parser.add_argument("--check", action="store_true", help="Verify ACP dependencies and adapter imports, then exit")
    parser.add_argument("--setup", action="store_true",
                        help="Run interactive Ulak provider/model setup for ACP terminal auth")
    parser.add_argument("--setup-browser", action="store_true",
                        help="Install agent-browser + Playwright Chromium into ~/.ulak/node/ "
...
def _print_version() -> None:
    from ulak_cli import __version__ as ulak_version
...
def _run_check() -> None:
    ...
    from acp_adapter.server import UlakACPAgent  # noqa: F401

    print("Ulak ACP check OK")


def _run_setup() -> None:
    from ulak_cli.main import main as ulak_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "ulak", "model"]
        ulak_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting ulak-agent ACP adapter")
    ...
    import acp
    from .server import UlakACPAgent
    ...
    if os.environ.get("ULAK_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
