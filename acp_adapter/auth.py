"""ACP auth helpers — detect and advertise Ulak authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "ulak-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Ulak runtime provider, or None if unavailable.
...
def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Ulak.

    The ACP registry requires at least one usable auth method in the initial
    handshake. A fresh Zed install may have no Ulak credentials yet, so the
    ...
    if provider:
        methods.append(AuthMethodAgent(
            ...
            description=f"Authenticate Ulak using the currently configured {provider} runtime credentials.",
        ))
    methods.append(TerminalAuthMethod(
        id=TERMINAL_SETUP_AUTH_METHOD_ID, name="Configure Ulak provider", type="terminal", args=["--setup"],
        description=("Open Ulak' interactive model/provider setup in a terminal. "
                     "Use this when Ulak has not been configured on this machine yet."),
    ))
    return methods
...
def has_provider() -> bool:
    """Return True if Ulak can resolve any runtime provider credentials."""
