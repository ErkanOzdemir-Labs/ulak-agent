...
Additive Ulak extension under ACP ``_meta.ulak`` (unknown to other clients,
...
rotates the internal Ulak head, ``build_session_provenance`` exposes the
...
def build_session_provenance(
    db: Any, acp_session_id: str, current_ulak_session_id: str, *, previous_ulak_session_id: Optional[str] = None,
) -> Optional[Dict[str, Any]]:
    """Build ``_meta.ulak.sessionProvenance`` for an ACP session.

    ``db`` must expose ``get_session``. ``current_ulak_session_id`` is the live
    internal id (``state.agent.session_id``); ``previous_ulak_session_id`` is
    the id before the most recent turn, supplied by ``prompt()`` to flag a
    rotation. Returns ``None`` if the session can't be read."""
    row = _get_row(db, current_ulak_session_id)
    if not row:
        return None
    ...
    root_id, compression_depth, cursor_parent = current_ulak_session_id, 0, parent_id
    seen = {current_ulak_session_id}
    ...
    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id, "currentUlakSessionId": current_ulak_session_id,
        "rootUlakSessionId": root_id, "parentUlakSessionId": parent_id,
        "sessionKind": "continuation" if is_continuation else "root", "compressionDepth": compression_depth,
    }
    if previous_ulak_session_id:
        provenance["previousUlakSessionId"] = previous_ulak_session_id
        if previous_ulak_session_id != current_ulak_session_id:
...
def session_provenance_meta(
    ...
    """Return a ready ``_meta`` payload: ``{"ulak": {"sessionProvenance": ...}}``."""
    prov = build_session_provenance(db, acp_session_id, current_ulak_session_id,
                                    previous_ulak_session_id=previous_ulak_session_id)
    return None if prov is None else {"ulak": {"sessionProvenance": prov}}
