...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with ulak-agent)
2. User plugins: ``$ULAK_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``ulak_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$ULAK_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$ULAK_HOME/plugins/model-providers/`` if it exists."""
    try:
        from ulak_constants import get_ulak_home

        d = get_ulak_home() / "plugins" / "model-providers"
...
def _installed_plugins_dir() -> Path | None:
    """Return ``$ULAK_HOME/plugins/`` if it exists.

    This is where ``ulak plugins install`` clones a plugin — flat, one
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple ULAK_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_ulak_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``ulak_agent.plugins``
    ...
    * **Provider targets only.** The ``ulak_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$ULAK_HOME``) keep their
    ...
    try:
        from ulak_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$ULAK_HOME/plugins/model-providers/<name>/``
      2b. Plugins installed by ``ulak plugins install`` at
          ``$ULAK_HOME/plugins/<name>/`` that declare ``kind: model-provider``
    ...
    # 0. Pip-installed plugins — entry points in the ``ulak_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$ULAK_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with ulak-agent.
    ...
    # 2. User plugins — under $ULAK_HOME/plugins/model-providers/<name>/.
    ...
    # 2b. Plugins installed by ``ulak plugins install`` / the plugin index.
    #     Those clone into $ULAK_HOME/plugins/<name>/ — flat, NOT under
