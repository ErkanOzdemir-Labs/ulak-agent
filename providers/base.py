...
def _profile_user_agent() -> str:
    """Return a ``ulak-cli/<version>`` UA string, with a stable fallback.
    ...
    """
    try:
        from ulak_cli import __version__ as _ver  # lazy: avoid layer cycle at import time
        return f"ulak-cli/{_ver}"
    except Exception:
        return "ulak-cli"
...
class ProviderProfile:
    ...
    # describe how to launch it; ulak_cli/auth.py's
    ...
    def create_client(self, **client_kwargs: Any) -> Any | None:
        ...
        a profile registered from ``~/.ulak/plugins/model-providers/`` or a
    ...
    def fetch_models(
        ...
        *,
        ...
        # ulak-cli UA so the catalog endpoint is reachable.
