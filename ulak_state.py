#!/usr/bin/env python3
"""SQLite state store for Ulak Agent: session metadata, message history, model
...
from ulak_state_common import escape_like as _escape_like, stat_db_file_identity as _stat_db_file_identity
from ulak_state_errors import (
...
from ulak_state_readpool import _READ_POOL_MAX, _proc_fd_targets, _read_budget_for
from ulak_state_sessions import SessionSessionsMixin
from ulak_state_fts import SessionFtsSetupMixin, load_fts5_cjk_extension
from ulak_state_portability import SessionPortabilityMixin
from ulak_state_telegram import SessionTelegramTopicsMixin
from ulak_state_schema import SessionSchemaMixin
import ulak_state_holders as _state_holders
from ulak_state_dbfile import (
...
from ulak_state_messages import SessionMessagesMixin
from ulak_state_wal import _WAL_INCOMPAT_MARKERS, apply_database_pragmas, apply_wal_with_fallback
from ulak_state_repair import _claim_repair_attempt, preflight_db_writability, repair_state_db_schema
from ulak_state_titles import SessionTitlesMixin
from ulak_state_usage import SessionUsageMixin
from ulak_state_maintenance import SessionMaintenanceMixin
from ulak_state_gateway import SessionGatewayMixin
from ulak_state_compression import SessionCompressionMixin
from ulak_state_search import SessionSearchMixin
...
DEFAULT_DB_PATH = _IMPORT_DEFAULT_DB_PATH = get_ulak_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    ``get_ulak_home()`` is resolved fresh (a runtime ULAK_HOME redirect works regardless of import)."""
    return DEFAULT_DB_PATH if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH else get_ulak_home() / "state.db"


# Live-DB guard knobs live HERE (not in ulak_state_guard): the hermetic conftest monkeypatches
# ``ulak_state._STATE_DB_GUARD_BYPASS`` (``@pytest.mark.live_system_guard_bypass`` escape hatch)
# and ``_EXTRA_DENY_ROOTS`` (the pre-sandbox root, so custom-ULAK_HOME deployments are covered).
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    ``ULAK_HOME`` together, which is precisely the state in which it writes to production (#82770).
    """
    ...
    for root in roots:
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Ulak root {root}). "
                "Tests must run against a temporary ULAK_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "ULAK_HOME. If this test genuinely needs the live database, mark it with "
...
def divert_session_transcript_jsonl(session_id: str, messages) -> "Optional[Path]":
    """Append pending messages to ULAK_HOME/sessions/<id>.jsonl (state.db was replaced under a
    live process). Returns the path, or None if nothing to write."""
    sid = str(session_id or "").strip()
    if not sid or not messages:
        return None
    sessions_dir = get_ulak_home() / "sessions"
...
# connection per resolved path via ulak_state_registry.acquire(); one-shots use SessionDB() + close().
...
# ── Process-wide shared SessionDB registry (#90837) ── lives in ulak_state_registry.py (acquire /
# release / close_all / release_or_close). Long-lived in-process callers (gateway, tui_gateway, cron,
# in-process tools) share ONE writer connection per resolved path via ulak_state_registry.acquire(); CLI
...
class SessionDB(
    ...
    # SQLite's deterministic busy handler convoys under many ulak processes: keep its
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        # Opened via ulak_state_registry.acquire(): close() releases a refcount instead.
        # Set True when this instance is opened via ulak_state_registry.acquire(). Makes close() a no-op so the
    ...
    def _open_writer(self) -> None:
        ...
        try:
            ...
            if not repair_state_db_schema(self.db_path).get("repaired"):
                raise
            self._connect_and_init_with_lock_patience()
        # FTS optimization is OPT-IN (`ulak db optimize`); no background worker races session lifecycle.
    ...
    def _handle_quarantine_if_zeroed(self, already_locked: bool = False) -> None:
        ...
        msg = (
            ...
            f"Restore from {self.db_path.parent / 'state-snapshots'} via `ulak snapshot list` / "
            f"`ulak snapshot restore <id>` if available. "
            ...
            try:
                ...
                if isinstance(exc, sqlite3.OperationalError):
                    if "locked" in err_msg or "busy" in err_msg:
                        ...
                        raise sqlite3.OperationalError(
                            f"database is locked (another Ulak process held the "
    ...
    def _halt_db_corrupt(self, exc: BaseException) -> None:
        ...
        logger.error(
            ...
            "gateway and run `ulak sessions recover --source %s --inspect-only`.", self.db_path, exc,
            self.db_path,
        )
    ...
    def _foreign_state_db_holders(self) -> List[Tuple[int, str]]:
        """Foreign processes holding this DB or its WAL sidecars (see ulak_state_holders)."""
    ...
    def close(self):
        ...
        When this instance is shared (opened via ``ulak_state_registry.acquire``), ``close()`` RELEASES one
        ...
        if self._shared_registry_owned:
            from ulak_state_registry import release
            release(self)
            return
            ...
            if self._conn:
                if self._db_corrupt:  # quarantined: no checkpoint over a damaged image
                    logger.warning(
                        ...
                        "then run `ulak sessions recover --source %s --inspect-only`.", self.db_path,
...
_PLUGIN_COMPAT_LAZY = {
    'AUTO_VACUUM_MIN_FREELIST_RATIO': ('ulak_state_common', 'AUTO_VACUUM_MIN_FREELIST_RATIO'),
    'ActivityProvenance': ('agent.session_activity', 'ActivityProvenance'),
    'CompressionSessionBusyError': ('ulak_state_errors', 'CompressionSessionBusyError'),
    'CompressionSessionClosedError': ('ulak_state_errors', 'CompressionSessionClosedError'),
    'DEFERRED_INDEX_SQL': ('ulak_state_common', 'DEFERRED_INDEX_SQL'),
    'FTS_CJK_STALE_KEY': ('ulak_state_common', 'FTS_CJK_STALE_KEY'),
    'FTS_CJK_TABLE_SQL': ('ulak_state_fts', 'FTS_CJK_TABLE_SQL'),
    'FTS_CJK_TRIGGER_SQL': ('ulak_state_fts', 'FTS_CJK_TRIGGER_SQL'),
    'FTS_REBUILD_DEFERRAL_KEY': ('ulak_state_common', 'FTS_REBUILD_DEFERRAL_KEY'),
    'FTS_SQL': ('ulak_state_common', 'FTS_SQL'),
    'FTS_STALE_KEY': ('ulak_state_common', 'FTS_STALE_KEY'),
    'FTS_STORAGE_VERSION': ('ulak_state_common', 'FTS_STORAGE_VERSION'),
    'FTS_TRIGRAM_SQL': ('ulak_state_common', 'FTS_TRIGRAM_SQL'),
    'LEGACY_FTS_SQL': ('ulak_state_common', 'LEGACY_FTS_SQL'),
    'LEGACY_FTS_TRIGRAM_SQL': ('ulak_state_common', 'LEGACY_FTS_TRIGRAM_SQL'),
    'MAX_FTS5_QUERY_CHARS': ('ulak_state_common', 'MAX_FTS5_QUERY_CHARS'),
    'PERSISTENCE_ERROR_CAUSES': ('ulak_state_errors', 'PERSISTENCE_ERROR_CAUSES'),
    'SCHEMA_SQL': ('ulak_state_common', 'SCHEMA_SQL'),
    'SCHEMA_VERSION': ('ulak_state_common', 'SCHEMA_VERSION'),
    'SESSION_STATUS_COMPLETE': ('ulak_state_sessions', 'SESSION_STATUS_COMPLETE'),
    'SESSION_STATUS_EMPTY': ('ulak_state_sessions', 'SESSION_STATUS_EMPTY'),
    'SESSION_STATUS_ERROR': ('ulak_state_sessions', 'SESSION_STATUS_ERROR'),
    'SESSION_STATUS_INTERRUPTED': ('ulak_state_sessions', 'SESSION_STATUS_INTERRUPTED'),
    ...
    'SessionTurnLeaseLostError': ('ulak_state_errors', 'SessionTurnLeaseLostError'),
    'WalUnsupportedError': ('ulak_state_wal', 'WalUnsupportedError'),
    'apply_durability_barriers': ('ulak_state_repair', 'apply_durability_barriers'),
    'classify_session_status': ('ulak_state_sessions', 'classify_session_status'),
    'collect_state_db_stats': ('ulak_state_dbfile', 'collect_state_db_stats'),
    'count_db_holders': ('ulak_state_dbfile', 'count_db_holders'),
    'describe_skill_invocation': ('agent.skill_commands', 'describe_skill_invocation'),
    'fts5_cjk_so_path': ('ulak_state_fts', 'fts5_cjk_so_path'),
    'is_advisory_lock_contention': ('ulak_state_common', 'is_advisory_lock_contention'),
    'is_automatic_end_reason': ('ulak_state_common', 'is_automatic_end_reason'),
    'is_disk_full_error': ('ulak_state_errors', 'is_disk_full_error'),
    'is_sqlite_wal_reset_vulnerable': ('ulak_state_wal', 'is_sqlite_wal_reset_vulnerable'),
    'is_transient_sqlite_error': ('ulak_state_errors', 'is_transient_sqlite_error'),
    'iter_deleted_sqlite_sidecar_holders': ('ulak_state_dbfile', 'iter_deleted_sqlite_sidecar_holders'),
    'release_or_close': ('ulak_state_registry', 'release_or_close'),
    'report_startup_progress': ('ulak_startup_watchdog', 'report_startup_progress'),
    'resolve_journal_mode': ('ulak_state_wal', 'resolve_journal_mode'),
    'resolve_synchronous_level': ('ulak_state_wal', 'resolve_synchronous_level'),
    'sanitize_context': ('agent.memory_manager', 'sanitize_context'),
    'sqlite_source_id': ('ulak_state_wal', 'sqlite_source_id'),
    'workspace_key': ('ulak_state_sessions', 'workspace_key'),
}
