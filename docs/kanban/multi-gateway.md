...
Ulak supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.ulak/config.yaml`:
...
Or set the env var: `ULAK_KANBAN_DISPATCH_IN_GATEWAY=false`
