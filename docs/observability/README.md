...
Ulak observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Ulak' planner, model providers, memory, tool registry,
...
Ulak also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "ulak.observer.v1"
```

Hook callbacks are fail-open. Ulak catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Ulak only builds
...
The native NeMo Relay SDK integration maps Ulak session, turn, LLM, and tool
