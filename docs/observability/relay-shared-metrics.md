...
Ulak includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Ulak and does not require a Ulak observability plugin. Ulak remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Ulak execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `ulak-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the Ulak `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `ULAK_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `ULAK_NEMO_RELAY_ATOF_*` and `ULAK_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Ulak does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Ulak requires NeMo Relay 0.8.3 or later within the 0.8 release line. That
line provides the provider-codec and canonical tool-result contracts Ulak
uses for managed provider and tool calls.

## Runtime Dependency and Data Boundary

Ulak installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Ulak process so configured interceptors can
...
Collection remains off unless Ulak policy enables it:
...
opt-in. Set `ULAK_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Ulak does not invoke Relay's plugin initializer, so Relay
...
selected file cannot be loaded, Ulak reports the error and does not invoke
...
retain the same `session_id` and add `ulak.session.segment` plus
`ulak.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a Ulak
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Ulak process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Ulak logs:
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Ulak
process and share the same policy boundary.

Run profiles in separate Ulak processes when they require different trust
...
Ulak core owns one Relay host and one isolated Relay session scope per Ulak
...
marks do not require Ulak plugin registration. Shared-metrics marks must
...
```text
Ulak turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Ulak shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Ulak sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Ulak used for the logical call, such as
...
New calls use `ulak.model_route.count`. The previous
`ulak.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `ulak.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Ulak
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Ulak processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `ulak.task_run`, parented to
the owning Ulak session. The start counter contains only bounded execution
...
provider attempts for the same Ulak API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Ulak tool-call
...
cancellations. Active task ownership follows the task ID if Ulak rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`ulak.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Ulak
...
tool or plugin names. Ulak does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `ulak.tool_approval` marks and
...
Successful skill mutations emit `ulak.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `ulak.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Ulak derives reuse and patch-generation
...
```text
$ULAK_HOME/telemetry/shared_metrics/metrics.sqlite3
$ULAK_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Ulak version,
...
schema field name, its current scope is one `ULAK_HOME`, so it is more
...
Deleting `$ULAK_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `ULAK_HOME`. To reset it, stop Ulak
processes and remove `$ULAK_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Ulak CLI turn against the deterministic local model server:
...
`$ULAK_HOME/telemetry/shared_metrics/outbox/` to the Ulak telemetry ingest
...
Removing `$ULAK_HOME/telemetry/shared_metrics` still resets local identity,
