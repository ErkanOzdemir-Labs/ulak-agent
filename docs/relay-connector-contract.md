...
This document is the formal interface between the **Ulak gateway** (Python,
`gateway/relay/`) and the **connector** (Node/TypeScript,
`ErkanOzdemir-Labs/gateway-gateway`). The connector implementer's first action is to
read this file.
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `ulak gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`ulak gateway enroll` → connector `/relay/enroll`, or
...
(`ErkanOzdemir-Labs/gateway-gateway`); this section is the gateway-facing summary.
...
driven by the managed Portal / `ulak` CLI.
