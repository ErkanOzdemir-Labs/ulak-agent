...
Ulak middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Ulak'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Ulak calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Ulak retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Ulak guardrails,
...
- `telemetry_schema_version`: currently `ulak.observer.v1`
- `middleware_schema_version`: currently `ulak.middleware.v1`
...
Ulak stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Ulak runs
them as a nested chain in registration order. Middleware failures are fail-open:
Ulak logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Ulak applies middleware in this order:
...
For each tool call, Ulak applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Ulak pre-execution path against the effective arguments:
...
For isolated local testing, use one `ULAK_HOME` for plugin enablement and the
agent run:

```bash
export ULAK_HOME=/tmp/ulak-middleware-test
mkdir -p "$ULAK_HOME"
ulak plugins enable <plugin-name>
ulak chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run ulak plugins enable <plugin-name>
uv run ulak chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["ulak_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/ulak-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Ulak expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Ulak treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Ulak preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Ulak does not convert downstream
