...
or change arbitrary Ulak configuration.
...
small host fact requested by the RFC. Settings writes use Ulak'
...
`ULAK_HOME`, so one globally loaded plugin context follows multiplexed
...
```text
<ULAK_HOME>/plugin-data/<plugin-data-namespace>/state.json
```
...
The implementation is covered with real temporary-Ulak-home tests for:
...
- [Issue #64227](https://github.com/ErkanOzdemir-Labs/ulak-agent/issues/64227)
- [RFC PR #58542](https://github.com/ErkanOzdemir-Labs/ulak-agent/pull/58542) by Topher Ross
