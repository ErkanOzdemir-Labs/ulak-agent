...
a provider run fails, or delivery starts failing. `ulak cron list` shows some of
...
Add `ulak cron doctor` as a read-only diagnostic command that summarizes cron
...
- `ulak cron doctor` returns `0` and prints a healthy message when active jobs
...
- It returns `1` and prints grouped job-level issues when any active job has:
  ...
  - script path missing/outside `ULAK_HOME/scripts`, or
