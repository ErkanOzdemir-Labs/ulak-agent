...
When running Ulak inside Docker, the default `network_mode: host` gives the
...
The Ulak [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   ULAK_UID=$(id -u) ULAK_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=ulak,ulak-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://ulak-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Ulak trust model and vulnerability reporting
