...
`ulak doctor` reports the explicit repair command.
...
repaired or restored file. Do not run `ulak doctor --fix` while the gateway
is still up. Next steps:

```bash
ulak gateway stop
ULAK_HOME="$HOME/.ulak" ulak sessions recover --source "$HOME/.ulak/state.db" --inspect-only
# if recoverable:
ULAK_HOME="$HOME/.ulak" ulak sessions recover --source "$HOME/.ulak/state.db" --output "$HOME/recovered-state.db"
```
...
```bash
ulak gateway stop
ULAK_HOME="$HOME/.ulak" ulak sessions repair --check-only
ULAK_HOME="$HOME/.ulak" ulak sessions repair
```
