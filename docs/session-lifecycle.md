...
The canonical transcript store is SQLite via `SessionDB` (from `ulak_state`). The
...
Counts consecutive restarts via a JSON file (`{ULAK_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `ulak update`, `ulak gateway restart`,
or `/restart`.
