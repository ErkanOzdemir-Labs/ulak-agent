...
`ulak config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.ulak/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `ULAK_HOME`, secret scope, and
