...
_profile_runtime_scope(profile_home)           (gateway/run.py)
   ├── set_ulak_home_override(home)          config / state.db / skills /
...
- `get_secret(name)` resolves: global allowlist → active scope → fallback.
  ...
  - multiplexing **on**, no scope installed: **raises `UnscopedSecretError`**
    ...
    value.
- A small allowlist (`ULAK_HOME`, `ULAK_PROFILE`, proxy settings,
...
reload helper: `ulak_cli.env_loader.load_ulak_dotenv` skips the
...
## The ULAK_HOME override

`ulak_constants.py` holds a context-local override consulted by
`get_ulak_home()` before the `ULAK_HOME` env var. Everything that resolves
...
automatically. `get_process_ulak_home()` exists for the few machine-level
assets that must not follow the override. `ulak_home_key()` gives
...
`profiles_to_serve(multiplex, profile_allowlist)` in `ulak_cli/profiles.py`
...
DB handles are resolved at call time through the active ULAK_HOME override —
...
ULAK_HOME override. Asset writes are atomic, type- and size-capped.
...
| Built-in tool registry | Built-ins are process-global; plugin-registered tools are overlaid per profile via `ulak_home_key()`. |
...
- `agent/secret_scope.py`, `ulak_constants.py`, `gateway/profile_routing.py`,
  `gateway/run.py` (`_profile_runtime_scope`), `ulak_cli/profiles.py`
