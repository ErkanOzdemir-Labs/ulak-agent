...
has to be thrown away or summarized. Ulak has always done this in one batch:
...
Ulak folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Ulak's normal session flush is
...
With `micro_compact` unset or `false` Ulak behaves exactly as it always has:
...
Defaults to `$ULAK_HOME/logs/agent.log`. It reports passes, outcome mix, net
