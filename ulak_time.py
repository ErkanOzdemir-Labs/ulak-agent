"""Timezone-aware clock for Ulak.

``now()`` returns a tz-aware datetime in the user's configured IANA timezone. Resolution order:
``ULAK_TIMEZONE`` env var, then ``timezone`` in ``~/.ulak/config.yaml``, else server-local
...
# ULAK_HOME, so one unkeyed global would leak the first profile's timezone into later
...
def _timezone_cache_identity() -> Tuple[str, str]:
    tz_env = os.getenv("ULAK_TIMEZONE", "").strip()
