...
an unlinked SQLite DB/WAL/SHM generation.  ``ulak_state`` supplies only the
...
_ULAK_EXECUTABLES = frozenset({"ulak", "ulak-agent", "ulak-acp"})
_ULAK_PYTHON_MODULES = frozenset({"acp_adapter", "ulak_cli.main"})
_ULAK_PYTHON_SCRIPTS = frozenset({"ulak_cli/main.py", "run_agent.py"})
...
def _looks_like_ulak(argv: Sequence[str]) -> bool:
    """Return whether argv identifies a supported Ulak execution target."""
    if not argv:
        return False
    program = os.path.basename(argv[0]).lower().removesuffix(".exe")
    if program in _ULAK_EXECUTABLES:
        return True
    ...
    if kind == "module":
        return normalized in _ULAK_PYTHON_MODULES
    return any(
        normalized == script or normalized.endswith(f"/{script}")
        for script in _ULAK_PYTHON_SCRIPTS
    )
        ...
        try:
            ...
            for pid_str in os.listdir("/proc"):
                ...
                try:
                    ...
                    if argv is not None and _looks_like_ulak(argv):
                        cmdline = " ".join(argv)
