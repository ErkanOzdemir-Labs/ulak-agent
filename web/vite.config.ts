...
const BACKEND = process.env.ULAK_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `ulak dashboard` server injects a one-shot
 * session token into `index.html` (see `ulak_cli/web_server.py`). The
 * Vite dev server serves its own `index.html`, so unless we forward that
 * token, every protected `/api/*` call 401s.
 *
 * This plugin fetches the running dashboard's `index.html` on each dev page
 * load, scrapes the `window.__ULAK_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function ulakDevToken(): Plugin {
  const TOKEN_RE = /window\.__ULAK_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__ULAK_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;

  return {
    name: "ulak:dev-session-token",
    apply: "serve",
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[ulak] Could not find session token in ${BACKEND} — ` +
              `is \`ulak dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__ULAK_SESSION_TOKEN__="${match[1]}";` +
              `window.__ULAK_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[ulak] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`ulak dashboard\` or set ULAK_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@ulak/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
  ...
  server: {
    proxy: {
      ...
      // Same host as `ulak dashboard` must serve these; Vite has no
