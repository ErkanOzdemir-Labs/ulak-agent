...
import { UlakConsoleModal } from "@/components/UlakConsoleModal";
...
export default function SystemPage() {
  ...
  // spawned `ulak import` runs non-interactively (stdin is /dev/null), so
  ...
  const checkForUpdate = useCallback(
    async (force = false) => {
      if (status?.can_update_ulak === false) return;
      setCheckingUpdate(true);
      try {
        const info = await api.checkUlakUpdate(force);
        setUpdateInfo(info);
        if (force) {
    ...
    [showToast, status?.can_update_ulak],
  );
  ...
  const applyUpdate = async () => {
    ...
    if (status?.can_update_ulak === false) {
      showToast(
        "Ulak updates are managed outside this dashboard.",
        "success",
      );
      return;
    }
    try {
      const resp = await api.updateUlak();
      if (!resp.ok) {
        showToast(
          resp.message ??
      ...
      setActiveAction(resp.name ?? "ulak-update");
  ...
  const canUpdateUlak = status?.can_update_ulak !== false;
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <ConfirmDialog
        open={canUpdateUlak && updateConfirmOpen}
        onCancel={() => setUpdateConfirmOpen(false)}
        onConfirm={() => void applyUpdate()}
        title="Update Ulak?"
        description={
          updateInfo && updateInfo.behind && updateInfo.behind > 0
            ? `This will run 'ulak update' (${updateInfo.update_command}) and pull ${updateInfo.behind} new commit${updateInfo.behind === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`
            : `This will run 'ulak update' (${updateInfo?.update_command ?? "ulak update"}) and restart the gateway when it finishes.`
        }
        ...
        <Card>
          <CardContent className="py-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-3 gap-x-6 text-sm">
              ...
              <div>
                <div className="text-xs uppercase tracking-wider text-muted-foreground">Ulak</div>
                <div className="flex items-center gap-2">
                  <span>v{stats?.ulak_version}</span>
                  {canUpdateUlak &&
                    updateInfo &&
        ...
        <Card>
          <CardContent className="flex flex-col gap-3 py-4">
            <div className="flex items-center gap-3">
              ...
              <a
                href={portal?.subscription_url || "https://portal.github.com/ErkanOzdemir-Labs/manage-subscription"}
                target="_blank"
                rel="noreferrer"
            ...
            {!portal?.logged_in && (
              <p className="text-xs text-muted-foreground">
                Log in with <span className="font-mono">ulak portal</span>.
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="flex flex-col gap-4 py-4">
            ...
            <div className="flex flex-col gap-3 border-t border-border pt-4 sm:flex-row sm:items-end">
              <div className="grid min-w-0 flex-1 gap-2">
                ...
                <Input
                  ...
                  placeholder="$ULAK_HOME/backups/ulak-backup.zip"
                />
              </div>
              <Button
                size="sm"
            ...
            <ConfirmDialog
              ...
              title="Restore full Ulak backup?"
              description={`This will overwrite your current Ulak configuration, skills, sessions, and data with the contents of ${backupImportLabel(importConfirmTarget)}. This cannot be undone.`}
              destructive
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground max-w-prose">
                    ...
                    returns links to send the Ulak team. Pastes auto-delete
                    after 6 hours.
                  </span>
                </div>
              </div>
