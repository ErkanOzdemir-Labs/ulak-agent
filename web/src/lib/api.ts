import { buildUlakWebSocketUrl } from "@ulak/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/ulak/). The Python backend
// injects ``window.__ULAK_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__ULAK_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const ULAK_BASE_PATH = readBasePath();
const BASE = ULAK_BASE_PATH;
...
declare global {
  interface Window {
    __ULAK_SESSION_TOKEN__?: string;
    __ULAK_BASE_PATH__?: string;
    ...
    __ULAK_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Ulak-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__ULAK_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``ulak update``, ``ulak gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__ULAK_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__ULAK_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__ULAK_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``ulak_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__ULAK_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Ulak-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``ulak_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__ULAK_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateUlak: () =>
    fetchJSON<ActionResponse>("/api/ulak/update", { method: "POST" }),
  checkUlakUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/ulak/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** ulak-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``ulak update``. */
  can_update_ulak?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the ULAK_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the ULAK_HOME volume — no staleness dimension, so no
 * sampled_at. */
