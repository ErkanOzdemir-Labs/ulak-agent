...
interface UlakConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function UlakConsoleModal({ open, onClose }: UlakConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("ulak> ");
  const inputPromptRef = useRef("ulak> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "ulak> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Ulak Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="ulak-console-title"
    >
      <div
        className={cn(
          themedBody,
