...
import { api, ULAK_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "ulak:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${ULAK_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${ULAK_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?ulak_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-ulak-plugin", manifest.name);
