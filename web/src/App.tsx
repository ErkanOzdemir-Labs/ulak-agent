...
const SIDEBAR_COLLAPSED_KEY = "ulak-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="ulak-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="ulak-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateUlak = status?.can_update_ulak === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'ulak update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "ulak update";
    return (
      t.status.updateUlakConfirmMessage ??
      `This will run 'ulak update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateUlakConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateUlak) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateUlak,
      runningLabel: t.status.updatingUlak,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Ulak gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateUlakConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateUlakConfirmTitle ?? `${t.status.updateUlak}?`}
    />
    </>
  );
}
