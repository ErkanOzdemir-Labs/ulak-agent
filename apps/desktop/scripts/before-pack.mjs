 ...
 * product name (`Ulak`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Ulak'
 *
 * This is a hard failure with no obvious cause for the user — `ulak desktop`
 ...
 * (the mac-unpacked Ulak.app bundle) and Windows (win-unpacked), so we
 ...
 * Ulak.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `ulak desktop --build-only` (ulak_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Ulak.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Ulak'}.exe`
