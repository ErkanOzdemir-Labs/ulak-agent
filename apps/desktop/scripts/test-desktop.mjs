...
// no longer bundles the Ulak Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Ulak.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Ulak'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Ulak.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Ulak'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default ULAK_HOME for non-sandboxed runs -- matches main.ts's
// resolveUlakHome(). On Windows it's %LOCALAPPDATA%\ulak; elsewhere
// it's ~/.ulak. The fresh-install sandbox launchFresh() sets its own
// ULAK_HOME and never touches this.
const DEFAULT_ULAK_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'ulak')
  }
  return path.join(os.homedir(), '.ulak')
})()
const VENV_ROOT = path.join(DEFAULT_ULAK_HOME, 'ulak-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'ulak-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.ULAK_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Ulak-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Ulak-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Ulak-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Ulak-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.ULAK_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.ULAK_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const ulakHome = path.join(sandbox, 'ulak-home')
  ...
  env.ULAK_DESKTOP_CWD = cwd
  env.ULAK_DESKTOP_IGNORE_EXISTING = '1'
  env.ULAK_DESKTOP_TEST_MODE = 'fresh-install'
  env.ULAK_DESKTOP_USER_DATA_DIR = userDataDir
  env.ULAK_HOME = ulakHome
  delete env.ULAK_DESKTOP_ULAK
  delete env.ULAK_DESKTOP_ULAK_ROOT
  ...
  console.log(`  ULAK_HOME: ${ulakHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(ulakHome, 'ulak-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Ulak Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of ulak_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'ulak-agent', 'ulak_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Ulak
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + ULAK_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  ULAK_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
