 ...
 *
 * Stamps the Ulak icon + identity onto the packed Windows Ulak.exe via
 * rcedit (delegated to set-exe-identity.mjs). This runs for EVERY packed build
 * — first install, `ulak desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Ulak')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Ulak'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Ulak.exe keeps the stock Electron icon`)
  }
}
