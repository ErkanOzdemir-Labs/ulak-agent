#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Ulak icon + version metadata onto the
// built Ulak.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Ulak.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `ulak desktop`, the installer's
...
// path (which rebuilds via `ulak desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Ulak.exe>
//
...
// Stamp the Ulak icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Ulak',
      FileDescription: 'Ulak',
      CompanyName: 'Ulak Agent',
      LegalCopyright: 'Copyright (c) 2026 Ulak Agent'
    }
  })

  console.log('[set-exe-identity] done — Ulak icon + identity stamped')
}
