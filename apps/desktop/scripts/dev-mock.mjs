...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `ulak-dev-mock-${Date.now()}`))
  ...
  return { root, ulakHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(ulakHome, mockUrl) {
  fs.writeFileSync(
    path.join(ulakHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(ulakHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.ulakHome, mock.url)
  console.log(`  ULAK_HOME: ${sandbox.ulakHome}`)
  ...
  const env = {
    ...
    ULAK_DESKTOP_APP_NAME: `UlakDevMock-${Date.now()}`,
  }
