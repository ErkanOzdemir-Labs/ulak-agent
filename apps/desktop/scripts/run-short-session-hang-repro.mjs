...
const UPSTREAM_URL = 'https://github.com/ErkanOzdemir-Labs/ulak-agent.git'
...
const LEGACY_STORAGE_KEY = 'ulak.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'ulak.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.ulakDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `ulak-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const ulakHome = join(sandbox, 'ulak-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(ulakHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(ulakHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        ULAK_DESKTOP_APP_NAME: `UlakShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        ULAK_DESKTOP_ULAK_ROOT: target.targetRoot,
        ULAK_DESKTOP_IGNORE_EXISTING: '1',
        ULAK_DESKTOP_USER_DATA_DIR: userData,
        ULAK_HOME: ulakHome,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'ulak-short-session-ab-'))
  const prepared = []
