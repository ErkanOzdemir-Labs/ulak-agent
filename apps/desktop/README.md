...
<p align="center">
  <a href="https://github.com/ErkanOzdemir-Labs/ulak-agent/releases"><img src="https://img.shields.io/badge/Download-macOS%20%C2%B7%20Windows%20%C2%B7%20Linux-FFD700?style=for-the-badge" alt="Download"></a>
...
**The native desktop app for [Ulak Agent](../../README.md) — the self-improving AI agent from [Nous Research](https://github.com/ErkanOzdemir-Labs).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other Ulak surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to Ulak and hear it back.</td></tr>
...
### Install with Ulak (recommended)

Already have the Ulak CLI? Just run:

```bash
ulak desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Ulak gateway or install Ulak locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the Ulak Desktop website.](https://ulak-agent.github.com/ErkanOzdemir-Labs/).
...
```bash
# throwaway ULAK_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Ulak Agent runtime into `ULAK_HOME`
(`~/.ulak`, or `%LOCALAPPDATA%\ulak` on Windows), using the same layout as a
CLI install.
...
- **Ulak Agent** runs as a headless `ulak serve` process and exposes the
...
1. `ULAK_DESKTOP_ULAK_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `ULAK_DESKTOP_ULAK`, or `ulak` on `PATH`
5. a system Python that can import the Ulak runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Ulak
...
screen offers **Connect to existing Ulak** before starting the local installer.
...
terminal commands, and file operations run against the remote Ulak host, not
...
transport- or Ulak-managed header names such as `Authorization`, `Cookie`,
`Host`, `Origin`, `Referer`, and `X-Ulak-Session-Token`.
...
Boot logs land in `ULAK_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\ulak\ulak-agent\.ulak-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\ulak\ulak-agent\venv"
```

> The default Ulak home on Windows is `%LOCALAPPDATA%\ulak`. Set the `ULAK_HOME` env var if you've relocated it.
...
- 📖 [Documentation](https://ulak-agent.github.com/ErkanOzdemir-Labs/docs/)
