...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Ulak E2E'], { cwd: repo })
...
function configureRepoCwd(ulakHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(ulakHome, mockUrl)
  fs.appendFileSync(path.join(ulakHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(ulakHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.ulakHome, mock.url, repo)
