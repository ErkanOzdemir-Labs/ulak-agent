...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  fs.appendFileSync(
    path.join(sandbox.ulakHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.ulakHome)
  const builder = await RealSessionBuilder.start(sandbox.ulakHome)
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.ulakHome, mock.url)
  fs.appendFileSync(path.join(sandbox.ulakHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
