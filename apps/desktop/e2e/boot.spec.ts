 ...
 * packaged binary) with a real `ulak serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → ulak serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Ulak title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Ulak')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { ulakDesktop?: Record<string, unknown> }).ulakDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → ulak serve is spawned → WS connects → config
