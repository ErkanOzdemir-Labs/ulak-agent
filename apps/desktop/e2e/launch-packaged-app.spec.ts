...
/**
 * E2E smoke tests for the packaged Ulak desktop app.
 *
 ...
 * (credential stripping, isolated ULAK_HOME + userData, unique app name).
 *
...
test('window opens with the Ulak title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      ulakDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).ulakDesktop?.hud?.open({ sessionId: null })
  )
