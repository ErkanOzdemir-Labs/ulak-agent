/**
 * Shared E2E fixtures for the Ulak desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → ulak serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `ulak desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `ulak-e2e-${prefix}-${Math.random()}`))
  const ulakHome = path.join(root, 'ulak-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(ulakHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(ulakHome, 'config.yaml')
 ...
 */
export function writeEnvFile(ulakHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(ulakHome, '.env')
 ...
 */
function writeEmptyConfig(ulakHome: string): void {
  const configPath = path.join(ulakHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - ULAK_HOME → sandbox ulak-home (isolated config/sessions)
 *  - ULAK_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - ULAK_DESKTOP_IGNORE_EXISTING=1 → don't pick up `ulak` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - ULAK_DESKTOP_ULAK_ROOT → REPO_ROOT (dev checkout resolution)
 *  - ULAK_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    ULAK_HOME: sandbox.ulakHome,
    ULAK_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    ULAK_DESKTOP_IGNORE_EXISTING: '1',
    ULAK_DESKTOP_ULAK_ROOT: REPO_ROOT,
    ULAK_DESKTOP_APP_NAME: `UlakE2E-${Date.now()}`,
    ...
    ULAK_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated ULAK_HOME + userData
 * @param env      - the process environment (already has ULAK_HOME etc.)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.ulakHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via ULAK_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`ulak serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.ulakHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { ULAK_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Ulak backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Ulak.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Ulak.app', 'Contents', 'MacOS', 'Ulak')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'ulak')
}
 ...
 * progress without spawning a real Ulak backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * ULAK_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    ULAK_DESKTOP_BOOT_FAKE: '1',
    ULAK_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + ulak-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).ULAK_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).ULAK_DESKTOP_ULAK
  delete (env as Record<string, string | undefined>).ULAK_DESKTOP_ULAK_ROOT
