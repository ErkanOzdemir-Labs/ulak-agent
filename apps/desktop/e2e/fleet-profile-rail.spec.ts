 ...
 * second gateway, "Homelab", is a REAL second `ulak serve` this spec spawns
 * with its own ULAK_HOME, profiles and session token, registered in the v2
...
function findUlakBinary(): string {
  const venv = path.join(REPO_ROOT, '.venv', 'bin', 'ulak')

  if (fs.existsSync(venv)) {
    return venv
  }

  const result = spawnSync('which', ['ulak'], { encoding: 'utf8' })

  if (result.status === 0 && result.stdout.trim()) {
    return result.stdout.trim()
  }

  throw new Error('ulak binary not found: create the repo venv (uv sync) or put ulak on PATH')
}
...
/**
 * Spawn a second, fully real `ulak serve` as the remote gateway. Its
 * session token is pinned through ULAK_DASHBOARD_SESSION_TOKEN so the
...
async function startRemoteGateway(root: string, mockUrl: string, profiles: string[]): Promise<RemoteGateway> {
  ...
  const child: ChildProcess = spawn(
    ...
    ['serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'],
    {
      ...
      env: {
        ...process.env,
        ULAK_HOME: home,
        ULAK_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN,
      },
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`remote ulak serve exited early (${child.exitCode}):\n${log}`)
    }

    try {
      const response = await fetch(`${url}/api/status`, {
        headers: { 'X-Ulak-Session-Token': REMOTE_TOKEN },
      })
  ...
  if (Date.now() >= deadline) {
    throw new Error(`remote ulak serve never became ready:\n${log}`)
  }
...
test.describe('fleet profile rail — two registered gateways', () => {
  ...
  test.beforeAll(async () => {
    ...
    writeMockProviderConfig(sandbox.ulakHome, mock.url)
    ...
    seedProfiles(sandbox.ulakHome, ['research'])
