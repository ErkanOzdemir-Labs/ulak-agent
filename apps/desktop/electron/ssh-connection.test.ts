...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  const line = 'setsid env ULAK_DASHBOARD_SESSION_TOKEN=abc123deadbeef ULAK_DESKTOP=1 ulak dashboard'
  const out = redactSecrets(line)
  assert.ok(!out.includes('abc123deadbeef'))
  assert.match(out, /ULAK_DASHBOARD_SESSION_TOKEN=<redacted>/)
  // non-secret env vars are preserved
  assert.match(out, /ULAK_DESKTOP=1/)
})
...
test('redactSecrets scrubs Authorization and X-Ulak-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-Ulak-Session-Token: hdr_888'), /X-Ulak-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-Ulak-Session-Token: hdr_888').includes('hdr_888'))
})
...
test('controlSocketPath default base stays under sun_path even with the temp-listener suffix', () => {
  ...
  const p = controlSocketPath('ulak', 'remote-build-server', 22) // no baseDir → default
...
test('buildExecArgs ends with host then the remote command', () => {
  ...
  const args = buildExecArgs(conn, 'command -v ulak', 15000)
  assert.equal(args[args.length - 1], 'command -v ulak')
...
test('close() removes the control socket when -O exit fails', async () => {
  const dir = path.join(os.tmpdir(), `ulak-ssh-close-${process.pid}-${Date.now()}`)
...
test('open() creates the control-socket directory if it does not exist', async () => {
  const dir = path.join(os.tmpdir(), `ulak-ssh-test-${process.pid}-${Date.now()}`)
