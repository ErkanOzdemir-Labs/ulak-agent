...
function fakeSsh(rules: any[] = []) {
  ...
  return {
    ...
    async exec(cmd) {
      ...
      if (cmd.includes('.ulak-update-in-progress') && !cmd.includes('marker_clear()') && !/setsid|nohup/.test(cmd)) {
        return 'CLEAR'
      }
...
test('POSIX relaunch gate refuses live and uncertain install markers without executing Ulak', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = {
      async exec(command) {
        ...
        if (command.includes('ULAK_HOME')) {
          return '/home/alice/.ulak\n'
        }

        if (command.includes('.ulak-update-in-progress')) {
          return observation
        }
...
test('POSIX relaunch gate permits absent/dead markers and normalizes named-profile homes install-wide', async () => {
  ...
  await assertRemoteInstallUpdateClear(ssh, '/home/alice/.ulak/profiles/research')
  assert.match(commands[0], /home\.parent\.name/)
  assert.match(commands[0], /profiles/)
  assert.match(commands[0], /\.ulak-update-in-progress/)
})
...
test('listRemoteUlakProfiles inventories Mini-style profile dirs without spawning a dashboard', async () => {
  const ssh = fakeSsh([
    [/ULAK_HOME/, '/Users/zillajr/.ulak\n'],
    [/ls -1/, 'bob\ndixie\ngoose\nrambo\nbob.rollback-old\n']
  ])

  assert.deepEqual(await listRemoteUlakProfiles(ssh), ['default', 'bob', 'dixie', 'goose', 'rambo'])
  assert.equal(
...
test('listRemoteUlakProfiles rejects a hostile ULAK_HOME', async () => {
  const ssh = fakeSsh([[/ULAK_HOME/, '/tmp/x; echo pwned\n']])

  await assert.rejects(
    () => listRemoteUlakProfiles(ssh),
    (err: any) => {
...
test('locateUlak prefers the explicit profile path when executable', async () => {
  const ssh = fakeSsh([[/\[ -x .*\/opt\/ulak/, 'OK']])
  assert.equal(await locateUlak(ssh, '/opt/ulak'), '/opt/ulak')
})

test('locateUlak throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
  // command -v WOULD find a different install, but an explicit path must not
  // silently fall back to it — that is the "connected to the wrong ulak" bug.
  const ssh = fakeSsh([
    [/command -v ulak/, '/home/u/.local/bin/ulak\n'],
    [/\[ -x .*\.local\/bin\/ulak/, 'OK']
  ])

  await assert.rejects(
    () => locateUlak(ssh, '/bad/path/ulak'),
    (err: any) => {
      assert.equal(err.kind, 'ulak-not-found')
      assert.match(err.message, /\/bad\/path\/ulak/)
...
test('locateUlak falls back to the login-shell command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v ulak/, '/home/u/.local/bin/ulak\n'],
    [/\[ -x .*\.local\/bin\/ulak/, 'OK']
  ])

  assert.equal(await locateUlak(ssh, ''), '/home/u/.local/bin/ulak')
})

test('locateUlak preserves an installer wrapper instead of resolving its interpreter', async () => {
  // install.sh venv mode writes: exec "$ULAK_BIN" "$ULAK_ENTRYPOINT" "$@",
  // where $ULAK_BIN is the venv python. The old canonicalization returned
  ...
  const ssh = fakeSsh([
    ...
    [/python3 -c/, '/home/u/.ulak/ulak-agent/venv/bin/python\n']
  ])

  assert.equal(await locateUlak(ssh, ''), '/home/u/.local/bin/ulak')
  assert.ok(
    !ssh.calls.some(cmd => cmd.includes('python3 -c')),
    'locateUlak must not shell out to a python3 parser to rewrite the launcher'
  )
})

test('locateUlak returns an explicit remoteUlakPath unchanged', async () => {
  // The override half of #74411: an explicit remoteUlakPath pointing at a
  // wrapper was also canonicalized to its interpreter, so overriding to
  // ~/.local/bin/ulak changed nothing for affected users.
  const ssh = fakeSsh([
    [/\[ -x .*\.local\/bin\/ulak/, 'OK'],
    [/python3 -c/, '/home/u/.ulak/ulak-agent/venv/bin/python\n']
  ])

  assert.equal(await locateUlak(ssh, '~/.local/bin/ulak'), '~/.local/bin/ulak')
  assert.ok(!ssh.calls.some(cmd => cmd.includes('python3 -c')), 'an explicit remoteUlakPath must never be rewritten')
})

test('locateUlak falls back to ~/.local/bin/ulak when the login-shell probe misses', async () => {
  ...
  assert.equal(await locateUlak(ssh, ''), '~/.local/bin/ulak')
})

test('locateUlak tries the conventional venv path last', async () => {
  const ssh = fakeSsh([[/\[ -x .*venv\/bin\/ulak/, 'OK']])
  assert.equal(await locateUlak(ssh, ''), '~/.ulak/ulak-agent/venv/bin/ulak')
})

test('locateUlak throws a ulak-not-found error with an install hint', async () => {
...
test('locateUlak uses a login shell for the command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v ulak/, '/x/ulak'],
    [/\[ -x/, 'OK']
  ])

  await locateUlak(ssh, '')
  assert.ok(
...
test('ownership paths are isolated by ownership ID and spawn nonce', () => {
  assert.equal(ownershipDirectory(OWNERSHIP_ID), `~/.ulak/desktop-ssh/${OWNERSHIP_ID}`)
  assert.equal(lockfilePath(OWNERSHIP_ID), `~/.ulak/desktop-ssh/${OWNERSHIP_ID}/backend.lock.json`)
  assert.equal(spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE), `~/.ulak/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.log`)
})
...
test('metadata and process proof transport failures remain indeterminate', async () => {
  ...
  await assert.rejects(
    () => pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, failure]]), 5, SPAWN_NONCE, '/x/ulak'),
    (error: any) => error.kind === 'transient-transport-error'
  )
})

test('pidIsOurDashboard requires the exact serve ownership nonce', async () => {
  const ours = `/x/ulak serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'OWNED\n']]), 5, SPAWN_NONCE, '/x/ulak'), true)
  assert.equal(
...
test('pidIsOurDashboard accepts the venv entrypoint an installer wrapper execs into', async () => {
  ...
  assert.equal(
    await pidIsOurDashboard(ssh, 5, SPAWN_NONCE, '~/.local/bin/ulak', '/Users/cd9c/.ulak', OWNERSHIP_ID, 'ops'),
    true
  )
  assert.match(ownershipProbe, /ulak-agent.*venv.*bin.*ulak/)
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    const temp = await mkdtemp(path.join(os.tmpdir(), 'ulak wrapper ownership '))
    ...
    const entrypoint = path.join(installDir, 'ulak')
    const launcher = path.join(temp, 'ulak launcher')
    ...
    try {
      ...
      assert.equal(
        await pidIsOurDashboard(ssh, child.pid, SPAWN_NONCE, launcher, '/unrelated/ulak-home', OWNERSHIP_ID, 'ops'),
        true
      )
      assert.equal(
        await pidIsOurDashboard(
...
test('buildSpawnCommand is headless serve, detached, token not in argv', () => {
  const cmd = buildSpawnCommand('/x/ulak', 'work', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  ...
  assert.ok(!cmd.includes('ULAK_DASHBOARD_SESSION_TOKEN'), 'token env var must not appear')
})
...
test('buildSpawnCommand atomically reserves the ownership slot through spawn and lock publication', () => {
  const cmd = buildSpawnCommand('/x/ulak', 'work', {
    ulakHome: '~/.ulak',
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    ownershipId: OWNERSHIP_ID,
  ...
  assert.ok(cmd.includes('.ulak-update-in-progress.mutex'))
  assert.match(cmd, /fcntl\.flock\(fd,fcntl\.LOCK_EX\)/)
  assert.match(cmd, /os\.O_CLOEXEC/)
  assert.match(
    cmd,
    /subprocess\.run\(\["sh","-c",payload,"ulak-update-mutex",str\(fd\)\],pass_fds=\(fd,\),check=False\)/
  )
  assert.doesNotMatch(cmd, /os\.set_inheritable\(fd,True\)/)
  assert.match(cmd, /ulak-update-child "\$1"/)
...
test.skipIf(process.platform === 'win32')('detached backend does not inherit the update mutex descriptor', async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'ulak-update-mutex-'))
  const ulakPath = path.join(directory, 'ulak')
...
for fd in /proc/$$/fd/*; do
  ...
  case "$target" in
    *ulak-update-in-progress.mutex) printf '%s\\n' "$target" >> ${reportPath} ;;
  esac
done
`,
      { mode: 0o700 }
    ...
    const command = buildSpawnCommand(ulakPath, '', {
      ulakHome: path.join(directory, 'home'),
      logPath
    })
...
test('spawnRemoteDashboard always spawns serve (legacy dashboard path removed)', async () => {
  ...
  await spawnRemoteDashboard(ssh, { ulakPath: '/x/ulak', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID })
...
test('READY_RE accepts both serve and dashboard sentinels', () => {
  assert.equal(READY_RE.exec('ULAK_BACKEND_READY port=4321')?.[1], '4321')
  assert.equal(READY_RE.exec('ULAK_DASHBOARD_READY port=8765')?.[1], '8765')
})
...
test('spawnRemoteDashboard rejects when no pid is returned', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { ulakPath: '/x/ulak', profile: '', token: 't', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('connect() spawns fresh when there is no lockfile, adopts the served token', async () => {
  const ssh = fakeSsh([
    ...
    [/setsid/, '777\n'],
    [/kill -0 777/, 'ALIVE'],
    [/cat .*\.log/, 'ULAK_DASHBOARD_READY port=51999\n']
  ])
...
test('managed SSH maps a local scope to a different non-default remote profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '778\n'],
    [/kill -0 778/, 'ALIVE'],
    [/cat .*\.log/, 'ULAK_BACKEND_READY port=52000\n']
  ])
  ...
  assert.match(spawn, /\.ulak\/desktop-ssh\/[0-9a-f]{32}\/[0-9a-f]{16}\.token/)
...
test('connect() respawns when the requested remote profile differs from the lockfile profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/kill 333/, ''],
    [/--version/, 'Ulak Agent v0.18.2\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/setsid/, '890\n'],
    [/kill -0 890/, 'ALIVE'],
    [/cat .*\.log/, 'ULAK_DASHBOARD_READY port=52050\n']
  ])
...
test('connect() respawns when the lockfile ulakPath differs from the resolved path', async () => {
  const reuseToken = 'stored-token'
  const lock = ownedLock({ ulakPath: '/old/stale/ulak', tokenFingerprint: fingerprintToken(reuseToken) })
  ...
  const result = await connect(
    connectDeps(ssh, { reuseToken, remoteUlakPath: '/new/ulak', adoptServedToken: async () => 'fresh' })
  )
...
test('connect() respawns when the lockfile protocolVersion is incompatible', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '901\n'],
    [/kill -0 901/, 'ALIVE'],
    [/cat .*\.log/, 'ULAK_DASHBOARD_READY port=44100\n']
  ])
...
test('connect() fresh spawn writes ulakHome + protocolVersion into the lockfile', async () => {
  const writes: string[] = []
  ...
  const ssh = fakeSsh([
    ...
    [/\[ -x/, 'OK'],
    [/cat .*lock\.json/, ''], // no lockfile
    [/ULAK_HOME/, '/home/alice/.ulak\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/printf '%s\\n'/, ''],
    [/setsid/, '700\n'],
    [/kill -0 700/, 'ALIVE'],
    [/cat .*\.log/, 'ULAK_DASHBOARD_READY port=45500\n'],
    [
      /printf '%s' '/,
      c => {
        writes.push(c)
  ...
  assert.match(lockWrite, /"ulakHome":"\/home\/alice\/\.ulak"/)
})
...
test('connect() respawns when the lockfile pid is dead (killed dashboard)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '888\n'],
    [/kill -0 888/, 'ALIVE'],
    [/cat .*\.log/, 'ULAK_DASHBOARD_READY port=42000\n']
  ])
...
test('connect() respawns when the dashboard is wedged (alive pid, probe fails)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '999\n'],
    [/kill -0 999/, 'ALIVE'],
    [/cat .*\.log/, 'ULAK_DASHBOARD_READY port=43000\n']
  ])
...
test('validateRemotePath accepts absolute POSIX paths', () => {
  assert.doesNotThrow(() => validateRemotePath('/usr/bin/ulak'))
  assert.doesNotThrow(() => validateRemotePath('/home/user/.ulak/ulak-agent/venv/bin/ulak'))
})

test('validateRemotePath accepts ~/ prefix paths', () => {
  assert.doesNotThrow(() => validateRemotePath('~/bin/ulak'))
  assert.doesNotThrow(() => validateRemotePath('~/.ulak/logs/desktop-ssh.log'))
  assert.doesNotThrow(() => validateRemotePath('~'))
})

test('validateRemotePath accepts paths with spaces and quotes', () => {
  assert.doesNotThrow(() => validateRemotePath('/home/user/my project/ulak'))
...
test('validateRemotePath rejects relative paths', () => {
  assert.throws(() => validateRemotePath('ulak'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('./bin/ulak'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('../etc/passwd'), /absolute|relative/i)
})

test('validateRemotePath rejects NUL and newline', () => {
  assert.throws(() => validateRemotePath('/usr/bin/ulak\x00'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/ulak\n'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/ulak\r'), /unsafe/i)
})

test('validateRemotePath preserves shell metacharacters as path data', () => {
  for (const p of ['/usr/$(whoami)/ulak', '/usr/`id`/ulak', '/usr/a;b|c&d<e>f']) {
...
test('expandRemotePath expands ~/ to "$HOME"/', () => {
  const result = expandRemotePath('~/.ulak/logs/desktop-ssh.log')
...
test('expandRemotePath returns quoted absolute paths unchanged', () => {
  const result = expandRemotePath('/usr/local/bin/ulak')
  assert.ok(result.includes('/usr/local/bin/ulak'))
  assert.ok(!result.includes('eval'))
})

test('expandRemotePath preserves spaces as data', () => {
  const result = expandRemotePath('/home/user/my project/ulak')
...
test('buildSpawnCommand does not embed the token in the command string', () => {
  ...
  assert.ok(!cmd.includes('ULAK_DASHBOARD_SESSION_TOKEN'), 'env var name must not appear')
})

test('buildSpawnCommand includes --ssh-session-token-file when tokenFilePath is provided', () => {
  const cmd = buildSpawnCommand('/x/ulak', 'work', {
    tokenFilePath: `~/.ulak/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.token`,
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    spawnNonce: SPAWN_NONCE
  })

  assert.match(cmd, /--ssh-session-token-file/)
  assert.match(cmd, /\.ulak\/desktop-ssh\//)
})

test('buildSpawnCommand always uses serve, never dashboard', () => {
  const cmd = buildSpawnCommand('/x/ulak', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /serve --isolated/)
  assert.doesNotMatch(cmd, /\bdashboard\b/)
  assert.doesNotMatch(cmd, /--skip-build/)
...
test('buildSpawnCommand raises the SSH child file limit before execing Ulak', () => {
  const cmd = buildSpawnCommand('/x/ulak', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /ulimit -n 65536 2>\/dev\/null \|\| true; exec env ULAK_DESKTOP=1/)
...
test('spawnRemoteDashboard removes a token file when upload reporting fails', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { ulakPath: '/x/ulak', profile: '', token: 'tok', ownershipId: OWNERSHIP_ID }),
    /channel closed/
  )
...
test('spawnRemoteDashboard fails with update-required when remote lacks --ssh-session-token-file', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { ulakPath: '/x/ulak', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('readLockfile treats a log path outside the exact ownership and spawn path as skew', async () => {
  const lock = ownedLock({ logPath: '~/.ulak/desktop-ssh/other.log' })
...
test('cleanupStale never deletes a lock-supplied unexpected log path', async () => {
  ...
  await cleanupStale(ssh, OWNERSHIP_ID, ownedLock({ logPath: '~/.ulak/unrelated.log' }))
...
test('pidIsOurDashboard requires an exact nonce option value', async () => {
  const prefix = `/x/ulak serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}ff`
  const suffix = `/x/ulak serve --isolated --ssh-owner-nonce xx${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/ulak'), false)
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/ulak'), false)
})
...
test('remote SSH ownership capability requires both secure bootstrap flags', async () => {
  ...
  assert.equal(await remoteSupportsSshOwnership(supported, '/x/ulak'), true)
  ...
  assert.equal(await remoteSupportsSshOwnership(unsupported, '/x/ulak'), false)
})
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    ...
    const root = await mkdtemp(path.join(os.tmpdir(), 'ulak-argv-shim-'))
    ...
    try {
      ...
      assert.equal(
        argv[2],
        `${fakeHome}/.ulak/.ulak-update-in-progress.mutex`,
      ...
      const base = `${fakeHome}/.ulak/desktop-ssh/${OWNERSHIP_ID}`
