...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'ulak serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/ulak serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m ulak_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/ulak --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Ulak Runtime\\ulak.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('ulak chat --query serve'), false)
