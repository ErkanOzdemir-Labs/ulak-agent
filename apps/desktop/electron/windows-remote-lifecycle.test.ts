...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command ulak.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          ulakHome: 'C:\\Users\\alice\\.ulak',
          ulakPath: 'C:\\Ulak\\ulak.exe',
          python: 'C:\\Ulak\\python.exe'
        })
      }

      if (script.includes('.ulak-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('ulak_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.ulak\\profiles\\research')
  assert.match(script, /\.ulak-update-in-progress/)
...
test('Windows probe validates Ulak and Python topology before selection', async () => {
  let script = ''
  ...
  const fallbackJoin = script.indexOf('Join-Path $ulakHome')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('Ulak is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /Ulak is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\Ulak's\\python.exe" }, 'inspect', ['C:\\x y\\ulak.exe'])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'ulak_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /Ulak''s/)
  assert.match(script, /C:\\x y\\ulak\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { ulakPath: lock.ulakPath, ulakHome: lock.ulakHome }
