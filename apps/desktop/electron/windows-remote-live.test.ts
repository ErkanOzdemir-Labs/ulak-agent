...
//   ULAK_WIN_SSH_HOST   ssh alias/host of the Windows box
//   ULAK_WIN_SSH_USER   remote user
//   ULAK_WIN_SSH_ULAK absolute path to the remote ulak.exe under test
const liveHost = process.env.ULAK_WIN_SSH_HOST || ''
const liveUser = process.env.ULAK_WIN_SSH_USER || ''
const configuredUlak = process.env.ULAK_WIN_SSH_ULAK || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Ulak-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredUlak)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredUlak.replace('ulak.exe', 'python.exe')}' -m ulak_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredUlak.replace('ulak.exe', 'python.exe')
        const terminate = `& '${python}' -m ulak_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.ulakPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m ulak_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m ulak_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
