// Unit tests for the pure Windows `ulak` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvUlakCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `ulak` shim shadows
//      the real ulak.cmd/ulak.exe.
...
//   3. resolveVenvUlakCommand() — must probe the venv python via
//      canImportUlakCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvUlak: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvUlak: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvUlak: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvUlak: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvUlak: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvUlakCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportUlakCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    ulakHome: '/fake/ulak-home',
...
test('resolveVenvUlakCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvUlakCommand('/root/venv/Scripts/ulak.exe', [], deps), null)
})

test('resolveVenvUlakCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvUlakCommand('/root/venv/Scripts/ulak.cmd', [], deps), null)
})

test('resolveVenvUlakCommand: returns null when the basename is not ulak/ulak.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvUlakCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvUlakCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvUlakCommand('/root/venv/bin/ulak.exe', [], deps), null)
})

test('resolveVenvUlakCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvUlakCommand('/root/venv/Scripts/ulak.exe', [], deps), null)
})

test('resolveVenvUlakCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportUlakCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvUlakCommand('/root/venv/Scripts/ulak.exe', ['serve'], deps)
...
test('resolveVenvUlakCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvUlakCommand('/root/venv/Scripts/ulak.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'ulak_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvUlakCommand: is case-insensitive on ulak.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvUlakCommand('/root/venv/Scripts/ULAK.EXE', [], deps))
  assert.ok(resolveVenvUlakCommand('/root/venv/SCRIPTS/ulak.exe', [], deps))
})
