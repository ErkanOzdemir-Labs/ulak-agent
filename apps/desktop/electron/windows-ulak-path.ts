/**
 * windows-ulak-path.ts
 *
 * Pure, dependency-injected pieces of Windows `ulak` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvUlakCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `ulak` shim shadowed the real
 *      ulak.cmd/ulak.exe; the shim then failed the --version probe and
 ...
 *      and Ulak launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvUlakCommand() — unwrapWindowsVenvUlakCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `ulak`) must
 * not shadow `ulak.cmd`/`ulak.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the Ulak launcher that drives `ulak update`.
...
export function chooseUpdaterArgs(signals: BootstrapRecoverySignals, branch: string): string[] {
  const canRunUpdater = signals.hasVenvUlak && signals.hasVenvPython
...
export interface ResolveVenvUlakCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportUlakCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `ulak`/`ulak.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/ulak(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m ulak_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportUlakCli(). A
 * venv whose update died mid-`pip install` still has python.exe + ulak.exe
 ...
 *
 * Returns null when `command` is not a venv ulak shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvUlakCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvUlakCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^ulak(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Ulak at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Ulak Python at ${python}`,
    command: python,
