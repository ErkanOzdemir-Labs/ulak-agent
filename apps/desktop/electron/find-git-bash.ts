 ...
 *   1. ULAK_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\ulak\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect ULAK_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.ULAK_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'ulak', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'ulak', 'git', 'usr', 'bin', 'bash.exe'))
  }
