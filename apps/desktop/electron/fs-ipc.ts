...
// hardening, ULAK_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('ulak:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('ulak:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMain.handle('ulak:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('ulak:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<ULAK_HOME>/desktop-plugins`,
  // resolved from the main-process ULAK_HOME (see resolveUlakHome) — NOT from
  // the connected backend. A remote backend reports its own `ulak_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    // profiles/<name>/, matching the profile-scoped ulak_home the backend
    // reported before this resolver existed. 'default'/unset pins the global root.
    const profile = readActiveDesktopProfile()
    const base = profile && profile !== 'default' ? path.join(ulakHome, 'profiles', profile) : ulakHome
  ...
  ipcMain.handle('ulak:fs:desktopPluginsRoot', async () => localPluginsRoot('desktop-plugins'))

  // The LOCAL logs root (`<ULAK_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where ULAK_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('ulak:fs:logsRoot', async () => localPluginsRoot('logs'))

  // The LOCAL agent-plugin root (`<ULAK_HOME>/plugins`), same Electron-local
  ...
  ipcMain.handle('ulak:fs:agentPluginsRoot', async () => localPluginsRoot('plugins'))

  ipcMain.handle('ulak:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('ulak:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('ulak:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('ulak:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('ulak:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
