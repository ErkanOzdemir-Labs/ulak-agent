...
const SCAN_MODULE = 'ulak_cli._scan_venv_blockers'
 ...
 * preview servers. Unknown Python/Ulak processes are deliberately ignored.
 */
export async function stopSafeVenvBlockers(
  ...
  for (const process of result.processes) {
    ...
    try {
      await execFn(
        pythonPath,
        ['-m', 'ulak_cli._scan_venv_blockers', '--terminate-safe', String(process.pid), String(process.createTime)],
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Ulak process is using this installation.',
    '',
...
export function formatProbeFailedMessage(error?: string): string {
  ...
  return (
    'Update aborted: Desktop could not verify the Ulak installation is free.' +
    timeoutDetail +
    '\n\n' +
    'Close other Ulak windows and terminals, then retry.  If the problem\n' +
    'persists, run `ulak update` in a terminal for detailed diagnostics.'
  )
}
