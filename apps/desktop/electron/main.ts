...
import { isUlakOwnedVenvDaemon } from './venv-holder-select'
...
const USER_DATA_OVERRIDE = process.env.ULAK_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.ULAK_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.ULAK_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with ULAK_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[ulak] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[ulak] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. ULAK_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[ulak] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[ulak] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via ULAK_DESKTOP_PASSWORD_STORE, bridged by the
// `ulak desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[ulak] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[ulak] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `ulak desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Ulak sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[ulak] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[ulak] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[ulak] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[ulak] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[ulak] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('ulak:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Ulak burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[ulak] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[ulak] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[ulak] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[ulak] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// ULAK_HOME — the user-facing root for everything Ulak-related. Mirrors
// scripts/install.ps1's $UlakHome and scripts/install.sh's $ULAK_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\ulak (matches install.ps1)
//   macOS / Linux: ~/.ulak (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.ulak directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\ulak yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// ULAK_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// ULAK_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.ulak / %LOCALAPPDATA%\ulak.
function resolveUlakHome() {
  if (process.env.ULAK_HOME) {
    return normalizeUlakHomeRoot(process.env.ULAK_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'ulak-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a ULAK_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\ulak and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('ULAK_HOME')

    if (fromRegistry) {
      return normalizeUlakHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'ulak')
    const legacy = path.join(app.getPath('home'), '.ulak')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.ulak setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.ulak')
}

const ULAK_HOME = resolveUlakHome()

function pathWithUlakManagedNode(...entries) {
  const managed = ulakManagedNodePathEntries(ULAK_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_ULAK_ROOT — the canonical mutable Ulak install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_ULAK_ROOT = path.join(ULAK_HOME, 'ulak-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_ULAK_ROOT, 'venv')
...
// boots, going straight to `resolveUlakBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_ULAK_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_ULAK_ROOT, '.ulak-bootstrap-complete')
...
// active-profile.json records which Ulak profile the desktop launches its
// local backend as. When set, startUlak() passes `ulak --profile <name>
// dashboard …`, which deterministically pins ULAK_HOME (see
// _apply_profile_override in ulak_cli/main.py) and bypasses the sticky
// ~/.ulak/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors ulak_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// ulakDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under ULAK_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by ulak_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(ULAK_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (ulak_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.ULAK_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.ULAK_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.ULAK_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.ULAK_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.ULAK_DESKTOP_APP_NAME || 'Ulak'
...
// 'ulak:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.erkanozdemir-labs.ulak) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.erkanozdemir-labs.ulak')
}

// Seed the native About panel with the live Ulak version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `ulak update` mid-session is reflected without an app restart;
...
// startUlak(); this pool only holds EXTRA profile
...
// The legacy ULAK_DESKTOP_POOL_* env vars remain the initial-value fallback
...
function readPersistedPoolLimits() {
  try {
    ...
    const fromEnv = clampPoolLimits({
      maxBackends: Number(process.env.ULAK_DESKTOP_POOL_MAX) || undefined,
      idleMs: Number(process.env.ULAK_DESKTOP_POOL_IDLE_MS) || undefined
    })
...
function logPoolSpawnFailure(label: string, error: unknown): void {
  if (isBackgroundSlotWaitTimeout(error)) {
    ...
    rememberLog(
      `Ulak backend for profile ${label} failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
  }
}
...
//   * WSL2 IPC stall  = the renderer's `ulak:backend:touch` roundtrips
...
const POOL_KEEPALIVE_FRESH_MS = Math.max(
  120_000,
  Number(process.env.ULAK_DESKTOP_POOL_KEEPALIVE_FRESH_MS) || 4 * 60_000
)
...
// onto the error so subsequent startUlak() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning ulak serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Ulak backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  if (ulakLog.length > 300) {
    ulakLog.splice(0, ulakLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-ulak-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('ulak:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the ulak:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('ulak:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_ULAK_ROOT
  })
}
...
// The Tauri updater writes ULAK_HOME/.ulak-update-in-progress for the whole
...
// (`force_kill_other_ulak`, taskkill /IM ulak.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(ULAK_HOME)),
...
const BUNDLE_SWAP_RELAUNCH_FLAG = '--ulak-bundle-swap-relaunched'
...
// The detached updater swaps the packaged bundle on disk AFTER `ulak update`
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Ulak will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(ULAK_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'Ulak update did not finish',
        `${result.message}\n\nDetails: ${path.join(ULAK_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
  ...
  if (outcome === 'timeout') {
    ...
    await advanceBootProgress('backend.update-restart', 'Restarting Ulak to load the updated app…', 14)
...
function findOnPath(command) {
  ...
  // shell-script shim named `ulak` — must not shadow `ulak.cmd`/`ulak.exe`.
...
function unwrapWindowsVenvUlakCommand(command, backendArgs) {
  return resolveVenvUlakCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
...
// spawns `ulak serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `ulak` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'ulak_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `ulak --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, ULAK_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveUlakBackend step 4); execFileSync of a .cmd without
...
function isUlakSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'ulak_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.ULAK_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Ulak deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the ulak-import
...
// findGitBash — locate bash.exe on Windows. Resolves ULAK_GIT_BASH_PATH
...
// normal ULAK_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\ulak\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'ulak', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'ulak', 'git', 'bin', 'git.exe'))
  }
...
function recentUlakLog() {
  return ulakLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's ulak root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_ULAK_ROOT.
// ULAK_DESKTOP_ULAK_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.ULAK_DESKTOP_ULAK_ROOT && path.resolve(process.env.ULAK_DESKTOP_ULAK_ROOT),
    !IS_PACKAGED && isUlakSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isUlakSourceRoot(ACTIVE_ULAK_ROOT) ? ACTIVE_ULAK_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_ULAK_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('ulak:updates:progress', merged)
  }
}
      ...
      const req = https.get(
        url,
        {
          headers: {
            ...
            'User-Agent': 'ulak-desktop-update-check'
          },
          timeout: 10_000
        },
        res => {
...
// Windows that binary owns ALL repo mutation — running `ulak update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(ULAK_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `ulak update` can write
// fresh entry points. On Windows this is the file the running backend
// `ulak.exe` holds open; on POSIX it's never mandatory-locked.
function venvUlakShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'ulak.exe')
    : path.join(updateRoot, 'venv', 'bin', 'ulak')
}
...
// Kill only Ulak-OWNED venv daemons (the memory plugin's hindsight daemon:
...
// venv files mapped. External holders (a user terminal running `ulak`,
...
function killUlakOwnedVenvDaemons(updateRoot) {
  if (!IS_WINDOWS) {
    return
  }
  ...
  try {
    ...
    holders = (Array.isArray(parsed) ? parsed : [parsed]).filter(p =>
      isUlakOwnedVenvDaemon(p?.ExecutablePath, p?.CommandLine, scriptsDir)
    )
  } catch {
  ...
  for (const holder of holders) {
    ...
    if (Number.isInteger(pid) && pid > 0) {
      rememberLog(`[updates] stopping Ulak-owned venv daemon (hindsight) PID ${pid} before hand-off`)
...
// only signals the direct child, so on Windows a backend `ulak.exe` that
// spawned its own grandchildren (a `ulak` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `Ulak backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live Ulak backend PID ${child.pid}; ` +
  ...
  try {
    ...
    child.ulakBackendIdentity = identity
...
function releaseBackendChild(child) {
  const identity = child?.ulakBackendIdentity
...
// `ulakProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `ulak.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a ulak
// REPL, a pty terminal, the gateway) keep `ulak.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const ulakProcess = backendConnectionState.getProcess()
  ...
  // unlock — the shim probe only covers venv\Scripts\ulak.exe, but the
  // backend is `python.exe -m ulak_cli.main serve`, which need not hold
  // the shim at all (#74805 first-attempt race).
  const initialPids = []

  if (ulakProcess && Number.isInteger(ulakProcess.pid)) {
    initialPids.push(ulakProcess.pid)
  }
  ...
  stopBackendTreesForUpdate(ulakProcess, {
  ...
  // `ulak gateway stop --all`: the CLI discovers every profile's gateway
  ...
  stopGatewayBeforeUpdate(venvUlakShimPath(updateRoot), ULAK_HOME)

  // Reap Ulak-OWNED venv daemons the tree-kill above cannot reach: the
  ...
  killUlakOwnedVenvDaemons(updateRoot)

  const shim = venvUlakShimPath(updateRoot)
  ...
  const gate = await waitForBackendRelease(
    initialPids,
    {
      ...
      collectStragglerPids: () => {
        ...
        const currentUlakProcess = backendConnectionState.getProcess()

        if (currentUlakProcess && Number.isInteger(currentUlakProcess.pid)) {
          stragglers.push(currentUlakProcess.pid)
        }
...
// `ulak update`). Instead we spawn the staged Ulak-Setup binary with
// --update and quit, so it can run `ulak update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `ulak update` → relaunch),
      ...
      // in: the ULAK_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `ulak update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `ulak desktop`, never the Tauri installer that self-copies
      // ulak-setup.exe into ULAK_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `ulak` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `ulak update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `ulak update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, ulakRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(ULAK_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Ulak — this window will close and the updater will open. Don’t reopen Ulak yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(ULAK_HOME, rememberLog)

    // Stop our own backend(s) and wait for the venv shim to unlock BEFORE we
    // spawn the updater. Without this the updater races a still-locked
    // ulak.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running ulak, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Ulak install open ' +
        '(a second Ulak window or a terminal running ulak?). Close it and retry.'
      ...
      if (IS_WINDOWS) {
        ...
        startGatewaysAfterUpdateAbort(venvUlakShimPath(updateRoot))
      }
    ...
    // Ulak processes running from this venv.  The updater normally refuses
    ...
    // `ulak update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `ulak update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          ULAK_HOME,
          ULAK_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithUlakManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `ulak update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(ULAK_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another Ulak update is already running (PID <itself>)" —
      ...
      if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
        writeUpdateMarker(ULAK_HOME, child.pid)
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. Ulak will keep running — try again, or run \`ulak update\` from a terminal.`
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvUlak = path.join(venvBin, IS_WINDOWS ? 'ulak.exe' : 'ulak')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's Ulak launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the ulak
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.ulak-bootstrap-complete')),
      hasVenvUlak: fileExists(venvUlak),
...
// Python-level pre-update snapshot inside `ulak update`.
function preflightStateDb(ulakHome, rememberLog) {
  const stateDbPath = path.join(ulakHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(ulakHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(ulakHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(ulakHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `ulak update`, swaps/relaunches the app bundle, and writes
// .ulak-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'ulak update', percent: null })

    return { ok: true, manual: true, command: 'ulak update', ulakRoot: updateRoot }
  }
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      ULAK_HOME,
      ULAK_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithUlakManagedNode(path.join(updateRoot, 'venv', 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Ulak — this window will close. Don’t reopen Ulak yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_ULAK_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isUlakSourceRoot(ACTIVE_ULAK_ROOT) &&
    fileExists(venvPython) &&
    canImportUlakCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_ULAK_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `ulak
...
function resolveWebDist() {
  const override = process.env.ULAK_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set ULAK_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: ulak desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: ulak desktop --force-build`
  )
...
function resolveUlakCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Ulak.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveUlakCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'ulak_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_ULAK_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Ulak at ${ACTIVE_ULAK_ROOT}`,
    command,
    args: ['-m', 'ulak_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      ulakHome: ULAK_HOME,
      pythonPathEntries: [ACTIVE_ULAK_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_ULAK_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveUlakBackend(backendArgs) {
  // 1. Explicit override -- ULAK_DESKTOP_ULAK_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.ULAK_DESKTOP_ULAK_ROOT && path.resolve(process.env.ULAK_DESKTOP_ULAK_ROOT)

  if (overrideRoot && isUlakSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Ulak source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `ulak` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isUlakSourceRoot.)
  if (!IS_PACKAGED && isUlakSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Ulak source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_ULAK_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\ulak\\ulak-agent (Windows) or ~/.ulak/ulak-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Ulak runtime at ${ACTIVE_ULAK_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `ulak` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    ULAK_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.ULAK_DESKTOP_IGNORE_EXISTING !== '1') {
    let ulakCommand = null
    const ulakOverride = process.env.ULAK_DESKTOP_ULAK

    if (ulakOverride) {
      const resolvedOverride = findOnPath(ulakOverride)

      if (resolvedOverride) {
        ulakCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(ulakOverride, { isWsl: IS_WSL })) {
        ulakCommand = ulakOverride
      } else {
        rememberLog(`Ignoring Windows Ulak override under WSL: ${ulakOverride}`)
      }
    } else {
      ulakCommand = findOnPath('ulak')
    }

    if (ulakCommand) {
      if (looksLikeDesktopAppBinary(ulakCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Ulak CLI: ${ulakCommand}`)
        ulakCommand = null
      }
    }

    if (ulakCommand) {
      const unwrapped = unwrapWindowsVenvUlakCommand(ulakCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `ulak` shim
      ...
      const shellForProbe = isCommandScript(ulakCommand)

      // ULAK_DESKTOP_ULAK is an explicit deployment override (used by
      ...
      if (shouldTrustUlakOverride(ulakOverride) || verifyUlakCli(ulakCommand, { shell: shellForProbe })) {
  ...
  // 5. Last-ditch: pip-installed ulak_cli module via system Python.
  ...
  if (python) {
    ...
    // having ulak_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\ulak\ulak-agent\venv.
    if (canImportUlakCli(python)) {
      return {
        kind: 'python',
        label: `installed ulak_cli module via ${python}`,
        command: python,
        args: ['-m', 'ulak_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: ulak_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveUlakBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Ulak Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveUlakBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Ulak install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Ulak recovery was handed off to Ulak Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      ulakHome: ULAK_HOME,
      logRoot: path.join(ULAK_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Ulak install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Ulak bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(ULAK_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startUlak() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // ulak:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveUlakBackend(backend.args))
  }
  ...
  if (!isUlakSourceRoot(ACTIVE_ULAK_ROOT)) {
    throw new Error(
      `Ulak install at ${ACTIVE_ULAK_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Ulak' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\ulak\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `ulak` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Ulak on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isUlakSourceRoot()
    // plus an importable ulak_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `Ulak venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Ulak at ${ACTIVE_ULAK_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Ulak runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----ulak-${crypto.randomBytes(12).toString('hex')}`
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported Ulak backend URL protocol: ${parsed.protocol}`))
                ...
                reject(
                  new Error(
                    `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                      'The endpoint is likely missing on the Ulak backend.'
                  )
                )
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to Ulak backend after ${timeoutMs}ms`))
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        agent,
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-Ulak-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Ulak-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('ulak:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveUlakCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('ulak:preview-file-changed', payload)
}
...
// waitForUlakReady how to read a 401 — rejected session vs gated route.
...
async function waitForUlak(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForUlakReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('ulak:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('ulak:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('ulak:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('ulak:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Ulak...".
function sendPowerResume() {
  ...
  webContents.send('ulak:power-resume')
}
...
// 'ulak:power-battery' push below.
ipcMain.handle('ulak:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('ulak:power-battery', next)
    }
  }
}
...
async function showPluginCompatNoticeOnce() {
  ...
  try {
    notice = pendingPluginCompatNotice(ULAK_HOME, app.getPath('userData'))
  } catch (err) {
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('ulak:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('ulak:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('ulak:context-menu-spellcheck', {
...
//     alongside the ~15-min access token (Portal NAS #293 / ulak #37247).
//     Both are set as HttpOnly cookies (``ulak_session_at`` ~15 min,
//     ``ulak_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
// "not signed in", which on the initial boot path (startUlak → the renderer's
// single-shot boot() with no retry) surfaced as the "Ulak couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Ulak
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Ulak Cloud agent…' : 'Sign in to Ulak gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Ulak backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in ulak_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Ulak Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:ulak-remote-oauth`:
...
// convention (ulak_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every Ulak surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.github.com/ErkanOzdemir-Labs'

function resolvePortalBaseUrl() {
  const raw = process.env.ULAK_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the Ulak gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for ulak_session_at/rt that the portal
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing Ulak Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Ulak Cloud sign-in.'))
...
// Discover the hosted (Ulak Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Ulak Cloud. Open Settings → Gateway, choose Ulak Cloud, and sign in.'
    ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Ulak Cloud session has expired. Sign in to Ulak Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Ulak Cloud org tag on cloud-mode entries so Settings can
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every ulak:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remoteUlakPath`/`remoteProfile` —
...
// a valid stored value (pins the root ULAK_HOME explicitly); null means "no
...
// contains "ulak", avoiding false positives from stale gateway.pid files
// whose PID was recycled by the OS to an unrelated process.
function isUlakProcess(pid) {
  try {
...
//   1. Legacy ~/.ulak/active_profile (explicit CLI choice via ulak profile use)
//   2. Running gateway (gateway.pid with verified liveness + ulak identity)
...
function migrateActiveProfileIfMissing() {
  migrateActiveProfileIfMissingPure(DESKTOP_PROFILE_CONFIG_PATH, {
    legacyActivePath: path.join(ULAK_HOME, 'active_profile'),
    ulakHome: ULAK_HOME,
    profilesRoot: path.join(ULAK_HOME, 'profiles'),
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.ULAK_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.ULAK_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Ulak Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteUlakPath: (ssh || savedSsh)?.remoteUlakPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by ULAK_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Ulak Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Ulak Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteUlakPath: input.sshRemoteUlakPath ?? existingBlock.remoteUlakPath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (ulak #37247), and the gateway middleware transparently
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Ulak gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Ulak gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget(webContentsId?: number) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.ULAK_DESKTOP_REMOTE_TOKEN,
      url: process.env.ULAK_DESKTOP_REMOTE_URL
    },
    profile,
    registry: readDesktopConnectionsRegistry()
  })
...
async function rollbackSshBootstrapResult(ssh, result, profile, sshConfig, boundaryError) {
  ...
  try {
    const expected = {
      ...
      ulakPath: result.ulakPath,
      ulakHome: result.ulakHome,
    ...
    if (result.platform?.os === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        ssh,
        { ulakPath: result.ulakPath, ulakHome: result.ulakHome, python: result.pythonPath },
        expected
      )
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, metadata, fingerprint, lease) {
  ...
  try {
    ...
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteUlakPath || '')
    const lifecycle = platform.os === 'Windows' ? connectWindowsRemote : remoteLifecycle.connect
    result = await lifecycle({
      ssh,
      profile: resolveRemoteSshDashboardProfile(sshConfig.remoteProfile, profile),
      remoteUlakPath: sshConfig.remoteUlakPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForUlak: (baseUrl, token) => waitForUlak(baseUrl, token, lease.signal, 'token'),
  ...
  await fenceManagedSshBootstrapPublication({
    ...
    publish: () => {
      ...
      sshConnections.set(scope, {
        ...
        host: sshConfig.host,
        hostLabel,
        ulakVersion: result.ulakVersion || '',
        ...
        // per-profile pools also use bare keys. Only startUlak' explicit call
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.ulakVersion || 'ulak (version unknown)'} at ${result.ulakPath || '?'}`
  )
  ...
  return {
    ...connection,
    remoteUlakVersion: result.ulakVersion || '',
    ssh: {
      effectiveConfigFingerprint: sshConfig.effectiveConfigFingerprint,
      host: sshConfig.host,
...
//   2. env override (ULAK_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
function globalRemoteActive() {
  if (process.env.ULAK_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startUlak() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Ulak
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteUlakPath: input.sshRemoteUlakPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteUlakPath || '')
          let ulakPath
          let ulakVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            ulakPath = runtime.ulakPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.ulakPath])
            ulakVersion = inspection.version
            supported = inspection.supported
          } else {
            ulakPath = await remoteLifecycle.locateUlak(ssh, sshConfig.remoteUlakPath || '')
            ulakVersion = await remoteLifecycle.probeUlakVersion(ssh, ulakPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, ulakPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Ulak on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteUlakPath: ulakPath,
            remoteUlakVersion: ulakVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startUlak())
  ...
  // connect to Ulak gateway". Mirror the renderer's connect here so the test
...
function resetUlakConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const ulakProcess = backendConnectionState.invalidate()
  stopBackendChild(ulakProcess)
...
// startUlak() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetUlakConnection() invalidates it.
  const ulakProcess = backendConnectionState.getProcess()
  const dying = ulakProcess && !ulakProcess.killed ? ulakProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('ulak:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'saved' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('ulak:connections:changed', payload)
    }
  }
}
...
async function ensureBackend(profile, opts: { spawnPriority?: LocalBackendSpawnPriority } = {}) {
  ...
  if (route.backend === 'primary') {
    const connection = await startUlak()
  ...
  // spawning, so two children never share one profile's ULAK_HOME.
  ...
  const resolveRegistrySshConfig = () => {
    ...
    if (!resolvedRegistrySshConfig) {
      resolvedRegistrySshConfig = normalizeSshConfig({
        ...
        remoteUlakPath: source.remoteUlakPath,
...
async function connectRegistryBackend(
  ...
  if (source.kind === 'ssh') {
    ...
    return {
      ...
      // is only the routing label. ulak:api uses it to translate explicit
      // self-profile query filters into the backend's namespace.
      remoteProfile: sshConfig.remoteProfile || '',
      logs: ulakLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  await waitForUlak(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
function remoteUpdateTargetFromState(state): RemoteUpdateTarget {
  if (!state?.ssh || !state?.ulakPath || !state?.ulakHome) {
  ...
  return {
    ...
    ulakPath: state.ulakPath,
    ulakHome: state.ulakHome,
...
async function openManagedSshUpdateTransport(
  ...
  try {
    const platform: any = await detectRemotePlatform(ssh, config.remoteUlakPath || '')

    if (platform.os === 'Windows') {
      const runtime = platform.ulakPath ? platform : await probeWindowsRemote(ssh, config.remoteUlakPath || '')
      ...
      return {
        ...
        target: {
          ...
          platform: 'Windows',
          ulakPath: runtime.ulakPath,
          ulakHome: runtime.ulakHome,
          pythonPath: runtime.python
        }
      }
    }

    const ulakPath = await remoteLifecycle.locateUlak(ssh, config.remoteUlakPath || '')
    const ulakHome = await remoteLifecycle.probeRemoteUlakHome(ssh)

    return {
      close: () => ssh.close(),
      target: { ssh, platform: platform.os, ulakPath, ulakHome }
    }
  } catch (error) {
    await ssh.close()
    throw error
...
async function drainManagedSshScope(scope) {
  ...
  try {
    ...
    if (state.remotePlatform === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        state.ssh,
        { ulakPath: state.ulakPath, ulakHome: state.ulakHome, python: state.pythonPath },
        expected
      )
    ...
    if (state && state.localPort && state.remotePort && scope.reuseToken) {
      try {
        ...
        if (!forwardClosed) {
          await waitForUlak(`http://127.0.0.1:${state.localPort}`, scope.reuseToken, undefined, 'token')
        } else {
...
// local-spawn portion of startUlak() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForUlak(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited ULAK_HOME env (see _apply_profile_override
  // step 3 in ulak_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveUlakBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const ulakCwd = resolveUlakCwd()
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(ULAK_HOME, 'profiles', key))
  )
  rememberLog(`Starting Ulak backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: ulakCwd,
        ULAK_DASHBOARD_SESSION_TOKEN: token,
        ...
        ...parentIdentityEnv,
        ULAK_WEB_DIST: webDist,
        ...(readyFile ? { ULAK_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Ulak backend for profile "${profile}" failed to start: ${error.message}`)
    void teardownFailedLocalBackend(poolKey, entry).catch(cleanupError => {
      rememberLog(
        `Ulak backend for profile "${profile}" cleanup failed: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`
      )
    })
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Ulak backend for profile "${profile}" exited (${signal || code})`)
    releaseLocalBackendSlot(entry)
    releaseBackendChild(child)
    ...
    if (!ready) {
      rejectStart?.(
        new Error(
          `Ulak backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  await Promise.race([waitForUlak(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Ulak backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Ulak backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_ulak_home() recreates the deleted profile directory.
//
...
async function startUlak() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Ulak Desktop is already running in another window.')
  }

  await reapOrphanedBackendsOnce()

  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startUlak() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Ulak backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      if (!backendConnectionState.isCurrentAttempt(connectionAttempt)) {
        throw new Error('Ulak backend start was superseded by a newer connection attempt.')
      }

      await advanceBootProgress('backend.remote', `Connecting to remote Ulak backend at ${remote.baseUrl}`, 24)
      await waitForUlak(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Ulak backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, ulakLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.ulak/active_profile file) and
    // resolves ULAK_HOME the same way `ulak -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Ulak runtime', 28)

        return resolveUlakBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting Ulak backend via ${backend.label}`, 84)
    rememberLog(`Starting Ulak backend via ${backend.label}`)
    ...
    const ulakProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin ULAK_HOME for the child so Python's get_ulak_home()
          // resolves to the SAME location our resolveUlakHome() picked. Without
          // this pin, Python falls back to ~/.ulak on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\ulak, which differs from C:\Users\<u>\.ulak.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets ULAK_HOME via setx; the desktop
    ...
    primaryOutputTail.attach(ulakProcess)
    ...
    const portAnnouncement = waitForDashboardPortAnnouncement(ulakProcess, {
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, ulakProcess)

    if (!processOwner) {
      stopBackendChild(ulakProcess)
      await waitForBackendExit(ulakProcess)
      releaseBackendChild(ulakProcess)
      throw new Error('Ulak backend start was superseded by a newer connection attempt.')
    }

    ulakProcess.stdout.on('data', rememberLog)
    ulakProcess.stderr.on('data', rememberLog)
    ...
    ulakProcess.once('error', error => {
      releaseBackendChild(ulakProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Ulak backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Ulak backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Ulak backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Ulak backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    ulakProcess.once('exit', (code, signal) => {
      releaseBackendChild(ulakProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Ulak backend exit (${signal || code})`)
      ...
      rememberLog(`Ulak backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Ulak backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Ulak backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentUlakLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Ulak backend to launch', 86)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Ulak backend to become ready', 90)
    await Promise.race([waitForUlak(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => ulakProcess.exitCode === null && !ulakProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Ulak backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Ulak backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
    ...
    // The backend's plugin discovery just ran and refreshed ULAK_HOME/.plugin-compat-report.json.
...
function wireCommonWindowHandlers(win, { zoom = true }: { zoom?: boolean } = {}) {
  ...
  // links arrive via `ulak:openExternal`, not here. See window-open-policy.ts.
...
function notifyBrowserPopoutClosed(tabId) {
  ...
  for (const other of BrowserWindow.getAllWindows()) {
    if (!other.isDestroyed()) {
      other.webContents.send('ulak:browser-popout:closed', tabId)
    }
  }
}
...
// here so it can leave the app's bounds and stay visible while Ulak is
...
// pushes pet state over IPC (ulak:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via ulak:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Ulak app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Ulak icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see ulak:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Ulak in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('ulak:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so Ulak can be driven
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('ulak:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const push = (state: { active: boolean; app: string }) => {
    if (!win.isDestroyed()) {
      win.webContents.send('ulak:hud:game-overlay', state)
    }
  }
  ...
  const enumerate = async () => {
    ...
    if (!reported) {
      reported = true
      console.warn(`[ulak] HUD cannot enumerate windows: ${windows.reason}`)
    }
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('ulak:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `ulak:hud:set-bounds`, which flips resizable on for the call — the
  ...
  // grab, ulak:hud:workspace-transfer temporarily makes the X11 window
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('ulak:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('ulak:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('ulak:quick-entry:shown')
}
...
function createWindow() {
    ...
    callbacks: {
      ...
      onFailedLoadBudgetExhausted: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          ...
          repairHint: 'ulak desktop --force-build',
  ...
  startUlak().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('ulak:connection', async (_event, profile, extra) => {
...
ipcMain.handle('ulak:connection:for', async (_event, payload) => {
  ...
  // Same single-owner claim as 'ulak:connection', keyed by the composite
...
ipcMain.on('ulak:connection:active-route', (event, route) => {
...
// "Starting Ulak…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('ulak:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: () => resetUlakConnection({ soft: true }),
...
ipcMain.handle('ulak:backend:touch', async (_event, profile) => {
...
ipcMain.handle('ulak:pool-limits:get', async () => ({ ...poolLimits }))
ipcMain.handle('ulak:pool-limits:set', async (_event, raw) => {
...
ipcMain.handle('ulak:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('ulak:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('ulak:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('ulak:window:openBrowser', async (_event, tabId) => {
...
ipcMain.handle('ulak:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = resolveUlakBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'Ulak is not installed yet' }
    }
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, ULAK_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('ulak:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('ulak:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('ulak:zoom:get', event => {
...
ipcMain.on('ulak:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('ulak:backend:recycle', async (_event, profile) => {
...
ipcMain.handle('ulak:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startUlak() call restarts the
...
ipcMain.handle('ulak:bootstrap:repair', async () => {
  // Forceful repair: force the next startUlak() through the full installer
  ...
  // "bypass active runtime" path inside startUlak() and fall through
...
ipcMain.handle('ulak:boot-progress:get', async () => bootProgressState)
ipcMain.handle('ulak:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('ulak:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('ulak:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('ulak:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('ulak:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('ulak:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('ulak:secret-storage:get', async () => ({ on: secretStoragePolicy().on }))
ipcMain.handle('ulak:secret-storage:set', async (_event: any, on: any) => applySecretStorageEncryption(on === true))
...
ipcMain.handle('ulak:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('ulak:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('ulak:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('ulak:connections:set-primary', async (_event, id) => {
...
ipcMain.handle('ulak:connections:set-launch-mode', async (_event, mode) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('ulak:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('ulak:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteUlakPath: entry.remoteUlakPath
    })
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteUlakPath: connection.remoteUlakPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteUlakProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root ULAK_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('ulak:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// ulak:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
...
ipcMain.handle('ulak:gateway:ws-url-for', async (_event, payload) => {
...
ipcMain.handle('ulak:connections:update-managed', async (_event, rawId) => requestManagedSshUpdate(rawId))

// Fan out `ulak update` to every eligible registered connection at once.
...
ipcMain.handle('ulak:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/ulak/update', {}, { timeoutMs: 15_000 })
...
// analogue of the ulak:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('ulak:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('ulak:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startUlak() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('ulak:connection-config:oauth-logout', async (_event, rawUrl) => {
  const baseUrl = normalizeRemoteBaseUrl(rawUrl)
...
// --- Ulak Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('ulak:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('ulak:connection-config:save', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('ulak:connection-config:apply', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('ulak:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
// Persistence-only sibling of ulak:profile:set: records the profile the
...
// the gateway (#79886).
ipcMain.handle('ulak:profile:remember', async (_event, name) => ({
  profile: writeActiveDesktopProfile(name)
}))
ipcMain.handle('ulak:profile:set', async (_event, name) => {
  assertCanMutateManagedPrimaryRouting()
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new ULAK_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('ulak:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('ulak:requestMicrophoneAccess', async () => {
...
ipcMain.handle('ulak:window:readBelow', async event => {
...
async function handleUlakApiRequest(request) {
  ...
  // backend calls ensure_ulak_home() which recreates the profile directory,
...
ipcMain.handle('ulak:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its ULAK_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleUlakApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleUlakApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('ulak:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('ulak:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('ulak:focus-session', payload.sessionId)
    }

    // Plugin / session-less activation — serializable path (+ optional notifyId
    // for renderer callbacks). Same vocabulary as ulak://index-network/….
    if (payload?.activate || payload?.notifyId) {
      mainWindow.webContents.send('ulak:notification-activate', {
  ...
  notification.on('action', (_actionEvent, index) => {
    ...
    if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
      mainWindow.webContents.send('ulak:notification-action', { sessionId: payload.sessionId, actionId: action.id })
...
ipcMain.handle('ulak:data-url-read-max:get', () => ({
...
ipcMain.handle('ulak:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('ulak:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('ulak:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('ulak:readFileText', async (_event, filePath) => {
...
// `ulak:readFileText` is the *preview* read and silently truncates at
...
ipcMain.handle('ulak:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('ulak:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('ulak:writeClipboard', (_event, text) => {
...
ipcMain.handle('ulak:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('ulak:readClipboard', () => clipboard.readText())

ipcMain.handle('ulak:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('ulak:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('ulak:context-menu:edit', (event, command) => {
...
ipcMain.handle('ulak:context-menu:copy-image', event => {
...
ipcMain.handle('ulak:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('ulak:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('ulak:capturePreview', async (_event, payload) => {
  const guest = electronWebContents.fromId(Number(payload?.webContentsId))

  return capturePreviewContents(guest, payload?.rect, payload?.viewport)
})

ipcMain.handle('ulak:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('ulak:saveClipboardImage', async () => {
...
ipcMain.handle('ulak:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('ulak:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('ulak:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('ulak:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('ulak:active-work', (event, payload) => {
  const id = event.sender.id
...
ipcMain.on('ulak:titlebar-theme', (_event, payload) => {
...
ipcMain.on('ulak:native-theme', (_event, mode) => {
...
ipcMain.on('ulak:translucency:support', event => {
...
ipcMain.on('ulak:keep-awake', (_event, on) => {
...
ipcMain.handle('ulak:quick-entry:settings:get', async () => {
...
ipcMain.handle('ulak:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('ulak:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('ulak:quick-entry:submit', {
...
ipcMain.on('ulak:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('ulak:quick-entry:state', payload)
  }
})

ipcMain.on('ulak:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('ulak:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('ulak:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `ulak:window:openSession`).
...
ipcMain.handle('ulak:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('ulak:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('ulak:preview:reach', async (event, url) => reachablePreviewUrl(event.sender.id, String(url || '')))

ipcMain.handle('ulak:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveUlakCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('ulak:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveUlakCwd()
}))

ipcMain.handle('ulak:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('ulak:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('ulak:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('ulak:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('ulak:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('ulak:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('ulak:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: ulakLog.slice(-200) }))
...
ipcMain.on('ulak:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (ulak:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('ulak:updates:check', async () =>
...
ipcMain.handle('ulak:updates:apply', async (_event, payload) =>
...
ipcMain.handle('ulak:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('ulak:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Ulak version (the one `release.py` bumps in
// ulak_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Ulak version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveUlakVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'ulak_cli', '__init__.py')
...
// Renderer-bundle skew: `ulak update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live Ulak version and push it into the native About panel
// just before showing it, so an in-place `ulak update` is reflected without
...
ipcMain.handle('ulak:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolveUlakVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    ulakRoot: resolveUpdateRoot(),
...
// The About page's "Restart Ulak" button (shown when bundleSwapPending):
...
ipcMain.handle('ulak:app:relaunch', async () => {
...
// the actual removal via `ulak uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (ulak_cli/uninstall.py + ulak_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_ULAK_ROOT
  ...
  const fallback = () => ({
    ulak_home: ULAK_HOME,
    agent_installed: isUlakSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'ulak_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, ULAK_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Ulak agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import ulak_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_ULAK_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `ulak-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `ulak-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('ulak:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('ulak:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('ulak:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('ulak:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// ulak:// deep links (e.g. ulak://blueprint/morning-brief?time=08:00,
// ulak://mcp/install?name=NAME&config=B64 — the vendor "Add to Ulak"
// button, or ulak://plugin/install?repo=owner/repo). Dev
// (`ULAK_DESKTOP_DEV_SERVER`) registers ulak-dev:// instead — bare
// Electron or a stale OS handler often owns ulak:// on dev machines.
...
// ---------------------------------------------------------------------------
const ULAK_PROTOCOL = DEV_SERVER ? 'ulak-dev' : 'ulak'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['ulak-dev', 'ulak'] : ['ulak']
...
function handleDeepLink(url) {
  ...
  // ulak://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('ulak:deep-link', payload)
...
ipcMain.handle('ulak:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${ULAK_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(ULAK_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(ULAK_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${ULAK_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `ulak://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startUlak), whose
...
app.whenReady().then(() => {
  ...
  // startUlak() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching ulak:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting Ulak never keeps another app's chord hostage.
