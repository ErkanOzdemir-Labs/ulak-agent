 ...
 * `iter_ulak_node_dirs()` in ulak_constants.py, which the Electron main
 * process cannot import.
 */
function ulakManagedNodePathEntries(
  ulakHome,
  ...
  const root = pathModule.join(ulakHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const ulakNodeDirs = ulakManagedNodePathEntries(ulakHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([ulakNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeUlakHomeRoot(ulakHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!ulakHome) {
    return ulakHome
  }

  const resolved = pathModule.resolve(String(ulakHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). ulak_bootstrap sets this inside the child too, but only
