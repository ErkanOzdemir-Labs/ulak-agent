 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Ulak Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.ulak-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(ulakHome) {
  return path.join(ulakHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.ulak/ulak-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(ulakHome) {
  if (!ulakHome) {
    return null
  }

  const candidate = path.join(ulakHome, 'ulak-agent', 'scripts', installScriptName())
...
function cachedScriptPath(ulakHome, commit) {
  return path.join(bootstrapCacheDir(ulakHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/ErkanOzdemir-Labs/ulak-agent/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(ulakHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(ulakHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, ulakHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass ULAK_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          ULAK_HOME: ulakHome || process.env.ULAK_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, ulakHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, ulakHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--ulak-home', ulakHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, ulakHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, ulakHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, ulakHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(ulakHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, ulakHome, emit })
