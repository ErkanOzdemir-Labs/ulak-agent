...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'ulak-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/ulak-runner-test',
    sourceRepoRoot: null,
    ulakHome: '/tmp/ulak-runner-test',
    logRoot: '/tmp/ulak-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'ulak-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'ulak-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/ulak-agent',
      ulakHome: '/tmp/ulak'
    }),
    ['--dir', '/tmp/ulak-agent', '--ulak-home', '/tmp/ulak', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/ulak-agent', '--ulak-home', '/tmp/ulak', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/ulak', '--ulak-home', '/tmp/home', '--branch', 'main']
  )
})
