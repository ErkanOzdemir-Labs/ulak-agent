// Profile-delete routing logic for the `ulak:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_ulak_home() and
...
/** Parse a `ulak:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its ULAK_HOME.
 *
...
/**
 * Route the next `ulak:api` request away from the primary/window backend
 ...
 * ensure_ulak_home() recreates the directory the delete just removed.
 */
