 ...
 *      top of startUlak() in main.ts, before primaryProfileKey() is read); the
...
function baseDeps(overrides: Record<string, unknown> = {}) {
  ...
  return {
    legacyActivePath: '/home/u/.ulak/active_profile',
    ulakHome: '/home/u/.ulak',
    profilesRoot: '/home/u/.ulak/profiles',
...
test('findRunningGatewayProfiles returns [] when no pid files exist', () => {
  const fs = makeFs({ '/home/u/.ulak/profiles/coder': { dir: true } })
  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.ulak/profiles', ['coder'], {
      ...fs,
      isUlakProcess: () => true
    }),
    []
...
test('findRunningGatewayProfiles drops stale (non-ulak) recycled PIDs', () => {
  // Two profiles have pid files, but only coder's pid is a live ulak process.
  // The recycled PID at 5678 belongs to an unrelated process (e.g. Chrome).
  const fs = makeFs({
    '/home/u/.ulak/profiles/coder/gateway.pid': { content: '{"pid":1234}' },
    '/home/u/.ulak/profiles/writer/gateway.pid': { content: '{"pid":5678}' }
  })

  const deps = {
    ...fs,
    isUlakProcess: (pid: number) => pid === 1234
  }

  assert.deepEqual(findRunningGatewayProfiles('/home/u/.ulak/profiles', ['coder', 'writer'], deps), ['coder'])
})

test('findRunningGatewayProfiles tolerates malformed pid files', () => {
  const fs = makeFs({
    '/home/u/.ulak/profiles/coder/gateway.pid': { content: '{not json' }
  })
...
test('findRunningGatewayProfiles drops non-integer and non-positive PIDs', () => {
  const fs = makeFs({
    '/home/u/.ulak/profiles/coder/gateway.pid': { content: '{"pid":-1}' },
    '/home/u/.ulak/profiles/writer/gateway.pid': { content: '{"pid":1.5}' },
    '/home/u/.ulak/profiles/extra/gateway.pid': { content: '{"pid":0}' }
  })

  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.ulak/profiles', ['coder', 'writer', 'extra'], {
      ...fs,
      isUlakProcess: () => true
    }),
    []
...
test('findRunningGatewayProfiles preserves order of allProfiles', () => {
  const fs = makeFs({
    '/home/u/.ulak/profiles/coder/gateway.pid': { content: '{"pid":1}' },
    '/home/u/.ulak/profiles/writer/gateway.pid': { content: '{"pid":2}' }
  })
...
test('listProfileDirs includes default and any name passing the regex', () => {
  const fs = makeFs({
    '/home/u/.ulak/profiles/default': { dir: true },
    '/home/u/.ulak/profiles/coder': { dir: true },
    '/home/u/.ulak/profiles/writer': { dir: true }
  })
...
test('listProfileDirs skips files (not directories) and invalid names', () => {
  const fs = makeFs({
    '/home/u/.ulak/profiles/default': { dir: true },
    '/home/u/.ulak/profiles/some-file.txt': { dir: false },
    '/home/u/.ulak/profiles/UPPERCASE': { dir: true } // regex rejects
  })
...
test('decideMigration suppresses write when best is default (single-profile fallback)', () => {
  ...
  // default-primary and we leave it alone. Default's DB is $ULAK_HOME/state.db,
  // not profiles/default/state.db.
  const deps = baseDeps()

  const d = decideMigration(null, [], ['default', 'coder'], deps, p => (p.endsWith('/.ulak/state.db') ? 99 : 50))
...
test('migrateActiveProfileIfMissing is a no-op when a user-selected preference file exists', () => {
  ...
  const fs = makeFs({
    ...
    '/home/u/.ulak/profiles/coder': { dir: true },
    '/home/u/.ulak/profiles/writer': { dir: true },
    '/home/u/.ulak/profiles/writer/state.db': { size: 400 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing writes legacy choice with no _migrated flag', () => {
  ...
  const fs = makeFs({
    '/home/u/.ulak/active_profile': { content: 'coder' },
    '/home/u/.ulak/profiles/coder': { dir: true },
    '/home/u/.ulak/profiles/writer': { dir: true }
  })
...
test('migrateActiveProfileIfMissing writes heuristic choice with _migrated=true', () => {
  ...
  const fs = makeFs({
    '/home/u/.ulak/profiles/coder/state.db': { size: 50 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.ulak/profiles/writer/state.db': { size: 200 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing is a no-op for single-profile (default-only) installs', () => {
  ...
  // Production default DB is ~/.ulak/state.db, not profiles/default/state.db.
  let written: unknown = null

  const fs = makeFs({
    '/home/u/.ulak/state.db': { size: 10 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing prefers a single running gateway over heuristics', () => {
  ...
  const fs = makeFs({
    '/home/u/.ulak/profiles/coder/gateway.pid': { content: '{"pid":42}' },
    '/home/u/.ulak/profiles/writer/state.db': { size: 500 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })

  const deps = baseDeps({
    ...fs,
    isUlakProcess: (pid: number) => pid === 42,
    writeJson: (_p: string, payload: unknown) => {
      written = payload
    }
  })
...
// ---------------------------------------------------------------------------
// Production layout: default is ~/.ulak, not ~/.ulak/profiles/default
// ---------------------------------------------------------------------------

test('profileStateDbPath puts default at ulakHome, named under profilesRoot', () => {
  assert.equal(profileStateDbPath('default', '/home/u/.ulak', '/home/u/.ulak/profiles'), '/home/u/.ulak/state.db')
  assert.equal(
    profileStateDbPath('conduit', '/home/u/.ulak', '/home/u/.ulak/profiles'),
    '/home/u/.ulak/profiles/conduit/state.db'
  )
})
...
test('findRunningGatewayProfiles sees default gateway.pid at ulakHome', () => {
  const fs = makeFs({
    '/home/u/.ulak/gateway.pid': { content: '{"pid":99}' },
    '/home/u/.ulak/profiles/coder/gateway.pid': { content: '{"pid":11}' }
  })

  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.ulak/profiles', ['default', 'coder'], {
      ...fs,
      ulakHome: '/home/u/.ulak',
      isUlakProcess: pid => pid === 99
    }),
    ['default']
  )
})
...
test('migrateActiveProfileIfMissing does not pin a tiny named profile over a large default DB', () => {
  ...
  // ~/.ulak/profiles/<name>, never scored ~/.ulak/state.db, and wrote
  // { profile: named, _migrated: true }.
  let written: unknown = null

  const fs = makeFs({
    '/home/u/.ulak/profiles/conduit': { dir: true },
    '/home/u/.ulak/state.db': { size: 409 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.ulak/profiles/conduit/state.db': { size: 2 * 1024 * 1024, mtime: NOW - 60_000 }
  })
...
test('migrateActiveProfileIfMissing still pins a named profile that actually beats default', () => {
  ...
  const fs = makeFs({
    '/home/u/.ulak/profiles/work': { dir: true },
    '/home/u/.ulak/state.db': { size: 5 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.ulak/profiles/work/state.db': { size: 200 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing does not pin default when only default gateway is running', () => {
  ...
  const fs = makeFs({
    '/home/u/.ulak/gateway.pid': { content: '{"pid":7}' },
    '/home/u/.ulak/state.db': { size: 10 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })

  const deps = baseDeps({
    ...fs,
    isUlakProcess: (pid: number) => pid === 7,
    writeJson: (_p: string, payload: unknown) => {
      written = payload
    }
  })
