 ...
 * the three user-facing uninstall modes to the `ulak uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m ulak_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `ulak_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'ulak_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Ulak.app/Contents/MacOS/Ulak  → …/Ulak.app
 *   Windows: …\Ulak\Ulak.exe                 → …\Ulak  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Ulak.app/Contents/MacOS/Ulak → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Ulak.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Ulak.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Ulak$/i.test(dir) || /[\\/]ulak-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/ulak
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import ulak_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, ulakHome }) {
 ...
 * modes + `pythonPath`=agentRoot so `import ulak_cli` resolves from source
 ...
 */
function buildWindowsCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, ulakHome }) {
  const pid = Number(desktopPid) || 0
  // cmd.exe has no string escaping inside quotes; strip embedded quotes (paths
  // under %LOCALAPPDATA% never contain them). `&`/`^` in a path would still be
  // a problem, but Ulak install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "ULAK_HOME=${String(ulakHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
