 ...
 *
 * The Python side (ulak_cli/plugin_compat.py) statically scans the user's enabled external plugins on
 * every CLI/gateway/TUI start and writes ULAK_HOME/.plugin-compat-report.json when any plugin imports a
...
export function readReport(ulakHome: string): PluginCompatReport | null {
  try {
    const raw = fs.readFileSync(path.join(ulakHome, REPORT_FILE), 'utf8')
...
export function pendingNotice(ulakHome: string, userData: string): PendingNotice | null {
  const report = readReport(ulakHome)
