...
export interface UlakReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForUlakReady`).
   */
...
export const REMOTE_UNSIGNED_OAUTH_MESSAGE =
  'Remote Ulak gateway uses OAuth, but you are not signed in. ' +
 ...
 *  - readiness-probe exhaustion in waitForUlakReady().
 *
 * Returns null unless the backend is a *.agents.github.com/ErkanOzdemir-Labs host AND the
...
export function makeNousCloudBackendDownError(baseUrl: string, error: unknown): Error | null {
  ...
  const err = new Error(
    `Nous Cloud agent ${hostname} is down ` +
      ...
      'Check https://portal.github.com/ErkanOzdemir-Labs for backend status, ' +
      'or switch to Local mode in Settings → Gateway. ' +
      'You can also reach out on Discord at discord.gg/ErkanOzdemir-Labs ' +
...
/**
 * True when the backend URL points at a Nous-managed Ulak Cloud instance
 * (e.g. ares-3009.agents.github.com/ErkanOzdemir-Labs). These are Fly.io-hosted machines
...
export function isNousCloudAgentUrl(baseUrl: string): boolean {
  try {
    ...
    return host.endsWith('.agents.github.com/ErkanOzdemir-Labs')
  } catch {
    return false
  }
}
 ...
 * `isReauthRequired` so startUlak latches; `needsOauthLogin` alone only
...
export async function waitForUlakReady(baseUrl: string, options: UlakReadyOptions): Promise<void> {
  ...
  throw new Error(`Ulak backend did not become ready: ${detail}`)
}
