 ...
 * origin=git@github.com:ErkanOzdemir-Labs/ulak-agent.git. A background
...
test('canonicalGitHubRemote normalizes SSH and HTTPS forms to the same value', () => {
  assert.equal(canonicalGitHubRemote('git@github.com:ErkanOzdemir-Labs/ulak-agent.git'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('git@github.com:ErkanOzdemir-Labs/ulak-agent'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('ssh://git@github.com/ErkanOzdemir-Labs/ulak-agent.git'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('https://github.com/ErkanOzdemir-Labs/ulak-agent.git'), OFFICIAL_REPO_CANONICAL)
  // Case-insensitive: an uppercased owner still canonicalizes to the same repo.
  assert.equal(canonicalGitHubRemote('git@github.com:erkanozdemir-labs/ulak-agent.git'), OFFICIAL_REPO_CANONICAL)
  // Trailing slashes are stripped.
  assert.equal(canonicalGitHubRemote('https://github.com/ErkanOzdemir-Labs/ulak-agent/'), OFFICIAL_REPO_CANONICAL)
})
...
test('isSshRemote detects scp-like and ssh:// forms only', () => {
  assert.equal(isSshRemote('git@github.com:ErkanOzdemir-Labs/ulak-agent.git'), true)
  assert.equal(isSshRemote('ssh://git@github.com/ErkanOzdemir-Labs/ulak-agent.git'), true)
  assert.equal(isSshRemote('https://github.com/ErkanOzdemir-Labs/ulak-agent.git'), false)
...
test('isOfficialSshRemote is true only for the official repo over SSH', () => {
  assert.equal(isOfficialSshRemote('git@github.com:ErkanOzdemir-Labs/ulak-agent.git'), true)
  assert.equal(isOfficialSshRemote('git@github.com:ErkanOzdemir-Labs/ulak-agent'), true)
  assert.equal(isOfficialSshRemote('ssh://git@github.com/ErkanOzdemir-Labs/ulak-agent.git'), true)
...
test('isOfficialSshRemote does NOT match forks, other hosts, or HTTPS', () => {
  ...
  assert.equal(isOfficialSshRemote('git@github.com:someuser/ulak-agent.git'), false)
  // Same repo name on a different host is not the official repo.
  assert.equal(isOfficialSshRemote('git@gitlab.com:ErkanOzdemir-Labs/ulak-agent.git'), false)
  ...
  assert.equal(isOfficialSshRemote('https://github.com/ErkanOzdemir-Labs/ulak-agent.git'), false)
