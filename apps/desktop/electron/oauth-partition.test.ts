...
// evict each other's `ulak_session*` cookies when they ride the single
// shared `persist:ulak-remote-oauth` partition — and, worse, gateway A's
...
describe('resolveOauthPartition (#92183 per-connection cookie jars)', () => {
  ...
  it('keeps cloud connections on the legacy partition (silent portal cascade needs the shared jar)', () => {
    const reg = registry('local', [
      { id: 'cloud-1', kind: 'cloud', url: 'https://agent.github.com/ErkanOzdemir-Labs', authMode: 'oauth' }
    ])

    expect(resolveOauthPartition('https://agent.github.com/ErkanOzdemir-Labs/api/status', { registry: reg })).toBe(
      LEGACY_OAUTH_PARTITION
    )
  })
  ...
  it('falls back to the legacy partition for unmatched, portal, and malformed inputs', () => {
    ...
    expect(resolveOauthPartition('https://portal.github.com/ErkanOzdemir-Labs/api/agents', { registry: reg })).toBe(
      LEGACY_OAUTH_PARTITION
    )
