...
export function registerGitIpc({ resolveGitBinary, resolveGhBinary }: GitIpcDeps) {
  ...
  ipcMain.handle('ulak:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

  ipcMain.handle('ulak:git:worktreeAdd', async (_event, repoPath, options) =>
    addWorktree(repoPath, options || {}, resolveGitBinary())
  )

  ipcMain.handle('ulak:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
    removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
  )

  ipcMain.handle('ulak:git:branchSwitch', async (_event, repoPath, branch) =>
  ...
  ipcMain.handle('ulak:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
  ...
  ipcMain.handle('ulak:git:review:list', async (_event, repoPath, scope, baseRef) =>
    reviewList(repoPath, scope, baseRef, resolveGitBinary())
  )
  ipcMain.handle('ulak:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
  ...
  ipcMain.handle('ulak:git:fileDiff', async (_event, repoPath, filePath) =>
    fileDiffVsHead(repoPath, filePath, resolveGitBinary())
  )
  ipcMain.handle('ulak:git:review:stage', async (_event, repoPath, filePath) =>
    reviewStage(repoPath, filePath ?? null, resolveGitBinary())
  )
  ipcMain.handle('ulak:git:review:unstage', async (_event, repoPath, filePath) =>
    reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
  )
  ipcMain.handle('ulak:git:review:revert', async (_event, repoPath, filePath) =>
    reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
  )
  ipcMain.handle('ulak:git:review:revParse', async (_event, repoPath, ref) =>
    reviewRevParse(repoPath, ref, resolveGitBinary())
  )
  ipcMain.handle('ulak:git:review:commit', async (_event, repoPath, message, push) =>
    reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
  )
  ipcMain.handle('ulak:git:review:commitContext', async (_event, repoPath) =>
    reviewCommitContext(repoPath, resolveGitBinary())
  )
  ipcMain.handle('ulak:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
  ipcMain.handle('ulak:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
  ipcMain.handle('ulak:git:review:prList', async (_event, repoPath, branches, numbers) =>
    reviewPrList(repoPath, resolveGhBinary(), branches, numbers)
  )
  ipcMain.handle('ulak:git:review:fetchPrComment', async (_event, repoPath, url) =>
    reviewFetchPrComment(repoPath, resolveGhBinary(), url)
  )
  ipcMain.handle('ulak:git:review:createPr', async (_event, repoPath) =>
  ...
  ipcMain.handle('ulak:git:scanRepos', async (_event, roots, options) => {
    try {
