 ...
 *
 * The pre-update handoff kills Ulak-OWNED venv daemons (the memory plugin's
 * hindsight daemon) so the updater never races a mapped shim. External
 * holders (a user terminal running `ulak`, unrelated scripts) must NOT be
...
/**
 * True when a process is a Ulak-owned venv daemon: its exe lives under
 ...
 * spawns DETACHED — it outlives Ulak and holds venv shims mapped).
 */
export function isUlakOwnedVenvDaemon(
