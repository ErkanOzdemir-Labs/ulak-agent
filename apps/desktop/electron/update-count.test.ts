...
function createTempGitRepo() {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'ulak-update-count-'))
  ...
  try {
    ...
    git('config', 'user.name', 'Ulak Test')
    git('config', 'user.email', 'ulak@example.invalid')
...
test('compareApiUrl builds the GitHub compare URL for HTTPS origins', () => {
  assert.equal(
    compareApiUrl({
      currentSha: SHA_A,
      originUrl: 'https://github.com/ErkanOzdemir-Labs/ulak-agent.git',
      targetSha: SHA_B
    }),
    `https://api.github.com/repos/ErkanOzdemir-Labs/ulak-agent/compare/${SHA_A}...${SHA_B}`
  )
})
...
test('compareApiUrl handles SSH origin forms', () => {
  for (const originUrl of [
    ...
    'ssh://git@github.com/ErkanOzdemir-Labs/ulak-agent.git',
