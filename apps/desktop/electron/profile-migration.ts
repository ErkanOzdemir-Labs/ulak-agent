...
export interface MigrationDeps {
  ...
  /** Default profile home (`~/.ulak`). Default's state.db and gateway.pid live here. */
  ulakHome: string
  /** Named-profile root (`~/.ulak/profiles`). Does not contain `default`. */
  profilesRoot: string
  ...
  isUlakProcess: (pid: number) => boolean
  now: () => number
...
/**
 * Production layout: default IS `ulakHome`; named profiles are children of
 * `profilesRoot`. There is no `profiles/default` directory on a normal install.
 */
export function profileStateDbPath(name: string, ulakHome: string, profilesRoot: string): string {
  return name === 'default' ? `${ulakHome}/state.db` : `${profilesRoot}/${name}/state.db`
}

export function profileGatewayPidPath(name: string, ulakHome: string, profilesRoot: string): string {
  return name === 'default' ? `${ulakHome}/gateway.pid` : `${profilesRoot}/${name}/gateway.pid`
}

function resolveUlakHome(profilesRoot: string, ulakHome?: string): string {
  if (ulakHome) {
    return ulakHome
  }

  // Tests that predate ulakHome pass only profilesRoot.
...
/**
 * Return the profile names whose gateway.pid file points to a live ulak process.
 ...
 *
 * `ulakHome` is optional so existing call sites that only pass `profilesRoot`
 * still work: it is derived as the parent of `…/profiles`.
 */
export function findRunningGatewayProfiles(
  profilesRoot: string,
  allProfiles: string[],
  deps: Pick<MigrationDeps, 'existsSync' | 'readFileSync' | 'isUlakProcess'> & { ulakHome?: string }
): string[] {
  const ulakHome = resolveUlakHome(profilesRoot, deps.ulakHome)
  const running: string[] = []

  for (const name of allProfiles) {
    const pidFile = profileGatewayPidPath(name, ulakHome, profilesRoot)
...
export function decideMigration(
  ...
  for (const name of candidates) {
    const s = score(profileStateDbPath(name, deps.ulakHome, deps.profilesRoot))
...
/** Default is always a candidate; it is `$ULAK_HOME`, not `$ULAK_HOME/profiles/default`. */
...
export function migrateActiveProfileIfMissing(desktopProfileConfigPath: string, deps: MigrationDeps): boolean {
  ...
  // launches `ulak --profile default` and is worse than writing nothing
