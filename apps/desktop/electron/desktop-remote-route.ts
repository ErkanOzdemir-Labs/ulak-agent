...
export function resolveDesktopRemoteRoute({
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'ULAK_DESKTOP_REMOTE_URL is set but ULAK_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Ulak backend.'
      )
    }
  ...
  if (!modeIsRemoteLike(config.mode)) {
    ...
    // resolves local and spawns a loopback `ulak serve` the desktop never
