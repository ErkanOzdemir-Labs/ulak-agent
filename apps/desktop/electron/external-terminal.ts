// Launching the Ulak TUI in the user's OWN terminal emulator.
//
...
// `ulak --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Ulak runtime is often a venv Python invoked
//     as `python -m ulak_cli.main`, not a `ulak` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Ulak
