 ...
 *     `X-Ulak-Session-Token` header; WS uses `?token=`.
...
// ulak_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - ulak_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - ulak_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / ulak #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-ulak_session_at', '__Secure-ulak_session_at', 'ulak_session_at']
const RT_COOKIE_VARIANTS = ['__Host-ulak_session_rt', '__Secure-ulak_session_rt', 'ulak_session_rt']

// The Nous portal (NAS) does NOT use Ulak gateway session cookies — it is a
...
// Keep this aligned with ulak_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['ulak', 'test', 'tmp', 'root', 'sudo'])
...
/** Which Ulak profile the remote SSH dashboard should actually run as.
...
// Ulak Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteUlakPath = String(entry.remoteUlakPath || '').trim()

  if (remoteUlakPath) {
    out.remoteUlakPath = remoteUlakPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Ulak installation. Preserve an explicit mapping
  // when it is a valid Ulak profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_ulak_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `ULAK_HOME` scopes it.
 *
...
function resolveProfileBackendRoute(profile, opts: ProfileRouteOptions = {}): ProfileBackendRoute {
  ...
  if (scopedProfile === primaryProfile) {
    ...
    // the wire: the dashboard's process ULAK_HOME can belong to a different
 ...
 * `ulak:api` handler — both routes reach a backend whose namespace is the
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://ulak.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `ulak:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a ulak session ACCESS-token cookie
 ...
 * the Ulak gateway session cookies, so cloud sign-in / discovery liveness
