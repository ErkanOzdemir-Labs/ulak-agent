export type ParentWatchdogEnv = {
  ULAK_PARENT_PID: string
  ULAK_PARENT_START_MARKER?: string
  ULAK_PARENT_NONCE?: string
  /** Spawn tag consumed by ulak_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { ULAK_PARENT_PID: String(pid) }
  env.ULAK_SPAWN = spawnTag(pid, startMarker)
  ...
  env.ULAK_PARENT_START_MARKER = startMarker
  env.ULAK_PARENT_NONCE = nonce

  return env
}

/** Build the `ULAK_SPAWN` tag mirrored by ulak_cli/process_identity.py.
