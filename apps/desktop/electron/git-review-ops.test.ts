...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ulak-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'ulak-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Ulak Test'], { cwd: dir })
