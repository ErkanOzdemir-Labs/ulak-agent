 ...
 * runtime, remote gateways, Ulak Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@ulak/shared`) — tsconfig project
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remoteUlakPath || '').trim() !== String(activeSsh.remoteUlakPath || '').trim() ||
...
/** Turn `ls ~/.ulak/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. Ulak Cloud
 * instances are platform-managed — we never run `ulak update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteUlakPath: input.remoteUlakPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/Ulak-managed names; an empty result stores nothing.
 ...
 * `remoteUlakPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Ulak Cloud' : 'Remote gateway'),
