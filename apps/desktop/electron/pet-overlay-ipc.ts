...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('ulak:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('ulak:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('ulak:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('ulak:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('ulak:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('ulak:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('ulak:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('ulak:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('ulak:pet-overlay:control', payload)
  })
}
