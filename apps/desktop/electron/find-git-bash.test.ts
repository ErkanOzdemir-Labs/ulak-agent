...
test('ULAK_GIT_BASH_PATH override takes precedence', () => {
  const result = findGitBash({
    isWindows: true,
    env: { ULAK_GIT_BASH_PATH: 'D:\\CustomGit\\bin\\bash.exe' },
    fileExists: yes,
    findOnPath: () => null
  })
...
test('ULAK_GIT_BASH_PATH invalid path falls through to candidates', () => {
  const env = {
    ULAK_GIT_BASH_PATH: 'X:\\Missing\\bash.exe',
...
test('ULAK_GIT_BASH_PATH empty string is ignored', () => {
  const result = findGitBash({
    isWindows: true,
    env: { ULAK_GIT_BASH_PATH: '', LOCALAPPDATA: '' },
    fileExists: no,
