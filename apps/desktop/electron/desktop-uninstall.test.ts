...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'ulak_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'ulak_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'ulak_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Ulak.app/Contents/MacOS/Ulak', 'darwin'),
    '/Applications/Ulak.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Ulak.app/Contents/MacOS/Ulak', 'darwin'),
    '/Users/x/Applications/Ulak.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Ulak\\Ulak.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Ulak'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\ulak-desktop\\Ulak.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\ulak-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Ulak.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_UlakXXXX/ulak', 'linux', { APPIMAGE: '/home/x/Apps/Ulak.AppImage' }),
    '/home/x/Apps/Ulak.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(resolveRemovableAppPath('/opt/ulak/linux-unpacked/ulak', 'linux', {}), '/opt/ulak/linux-unpacked')
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/ulak', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Ulak.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Ulak.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.ulak/ulak-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.ulak/ulak-agent',
    uninstallArgs: ['-m', 'ulak_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/ulak/linux-unpacked',
    ulakHome: '/home/x/.ulak'
  })
  ...
  assert.match(script, /'-m' 'ulak_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/ulak\/linux-unpacked'/)
  assert.match(script, /export ULAK_HOME='\/home\/x\/\.ulak'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.ulak/ulak-agent',
    agentRoot: '/home/x/.ulak/ulak-agent',
    uninstallArgs: ['-m', 'ulak_cli.uninstall', '--mode', 'full'],
    appPath: null,
    ulakHome: '/home/x/.ulak'
  })

  // System python + source on PYTHONPATH so import ulak_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.ulak\/ulak-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'ulak_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'ulak_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    ulakHome: '/h'
  })
  ...
  assert.match(script, /'-m' 'ulak_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\ulak',
    uninstallArgs: ['-m', 'ulak_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Ulak',
    ulakHome: 'C:\\Users\\x\\AppData\\Local\\ulak'
  })

  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import ulak_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\ulak;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "ulak_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Ulak" >nul 2>&1/)
