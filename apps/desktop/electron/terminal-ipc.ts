...
// registry, and the ulak:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // ULAK_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.ULAK_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip Ulak' TUI into
    ...
    env.TERM_PROGRAM = 'Ulak'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a ulak/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from ULAK_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.ULAK_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `ulak:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('ulak:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('ulak:terminal:attach', (event, id) => {
  ...
  ipcMain.handle('ulak:terminal:write', (_event, id, data) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('ulak:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('ulak:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('ulak:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
