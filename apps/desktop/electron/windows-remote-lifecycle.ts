...
const READY_RE = /^ULAK_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitUlakPath = '') {
  const explicit = psLiteral(explicitUlakPath)
  ...
  const script = [
    ...
    'if(-not $ulakHome){$ulakHome=Join-Path $env:LOCALAPPDATA "ulak"}',
    'Assert-NoReparse $ulakHome $true',
    '$candidate=[IO.Path]::Combine($ulakHome, "ulak-agent\\venv\\Scripts\\ulak.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "ulak-agent\\.venv\\Scripts\\ulak.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $ulakHome "ulak-agent\\venv\\Scripts\\ulak.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "ulak-agent\\.venv\\Scripts\\ulak.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command ulak.exe -ErrorAction SilentlyContinue',
    ...
    '$ulak=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$ulak=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $ulak){throw "Ulak is not installed on the remote Windows host."}',
    'Assert-NoReparse $ulak $false',
    'if($explicit -and $ulak -ne $explicit){throw "The configured Ulak path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($ulak), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;ulakHome=$ulakHome;ulakPath=$ulak;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(ulakHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class UlakMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$home=${psLiteral(ulakHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".ulak-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[UlakMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, ulakHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(ulakHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitUlakPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitUlakPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'ulak_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'ulak_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'ulak_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$home=${psLiteral(runtime.ulakHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".ulak-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};ulakPath=${psLiteral(reservation.ulakPath)};ulakHome=${psLiteral(reservation.ulakHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.ulakPath === 'string' &&
    typeof lock.ulakHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.ulakPath === runtime.ulakPath &&
    lock.ulakHome === runtime.ulakHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteUlakPath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.ulakHome)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.ulakPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Ulak on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.ulakPath = inspection.path
  const ulakVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located ulak at ${runtime.ulakPath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, ulakPath: runtime.ulakPath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForUlak(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Ulak SSH"', 'powershell.exe -NoLogo')
