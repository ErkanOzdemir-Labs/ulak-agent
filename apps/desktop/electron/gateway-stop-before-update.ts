 ...
 *
 * So we delegate to `ulak gateway stop --all`: the CLI discovers every
 ...
 * force-kills survivors — the same logic `ulak update`'s
...
export function stopGatewayBeforeUpdate(
  ...
  return runGatewayLifecycleCommand(ulakCliPath, ['gateway', 'stop', '--all'], deps)
}
 ...
 */
export function startGatewaysAfterUpdateAbort(ulakCliPath: string, deps: StopGatewayBeforeUpdateDeps = {}): boolean {
  return runGatewayLifecycleCommand(ulakCliPath, ['gateway', 'start', '--all'], deps)
}

function runGatewayLifecycleCommand(ulakCliPath: string, args: string[], deps: StopGatewayBeforeUpdateDeps): boolean {
  ...
  if (deps.spy) {
    deps.spy(ulakCliPath, args)
  }

  if (!existsSync(ulakCliPath)) {
    return false
  }

  try {
    exec(ulakCliPath, args, {
