 ...
 *
 * 1. TORN BUNDLE after an update (#95575): `ulak update` replaces the app
...
export interface RendererLoadErrorDetails {
  ...
  /** Repair command hint, e.g. `ulak desktop --force-build`. */
  repairHint?: string
  /**
...
export function buildRendererLoadErrorPage(details: RendererLoadErrorDetails = {}): string {
  ...
  const title = 'Ulak couldn\u2019t start the desktop UI'
  ...
  const repair = details.repairHint ? `<p>Repair with: <code>ulak desktop --force-build</code></p>` : ''
...
<main>
  ...
  <code>ulak desktop --force-build</code>, then restart the app.</p>
