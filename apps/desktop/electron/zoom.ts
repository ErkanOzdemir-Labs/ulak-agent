...
export const ZOOM_STORAGE_KEY = 'ulak:desktop:zoomLevel'
 ...
 * but forgot to emit 'ulak:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('ulak:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `ulak:zoom:changed`, which never fired —
