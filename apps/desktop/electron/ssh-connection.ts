...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(ULAK_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Ulak-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.ulak/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'ulak-desktop-ssh')
  }

  return path.join(os.homedir(), '.ulak', 'desktop-ssh')
}
