...
describe('resolvePluginGitUrl', () => {
  it('maps owner/repo shorthand to github git url', () => {
    expect(resolvePluginGitUrl('ErkanOzdemir-Labs/ulak-example-plugins')).toEqual({
      gitUrl: 'https://github.com/ErkanOzdemir-Labs/ulak-example-plugins.git',
      subdir: null
    })
  })
...
describe('resolveSubdirWithin', () => {
  it('rejects path traversal', () => {
    const root = mkdtemp('ulak-plugin-root-')
...
describe('findDesktopEntry', () => {
  it('finds root plugin.js', () => {
    const root = mkdtemp('ulak-plugin-detect-')
...
describe('detectPluginComponents', () => {
  ...
  it('detects agent-only layout', async () => {
    const root = mkdtemp('ulak-plugin-agent-')
    roots.push(root)
  ...
  it('detects dual layout', async () => {
    const root = mkdtemp('ulak-plugin-dual-')
    roots.push(root)
    fs.mkdirSync(path.join(root, 'desktop'), { recursive: true })
