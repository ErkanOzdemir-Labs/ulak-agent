// Backend subcommand routing for the desktop-managed Ulak process.
//
// The desktop app launches its own headless backend via `ulak serve` — it
...
// app hasn't updated yet, or an older `ulak` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Ulak profile to pin via `--profile`.
 */
 ...
 * `-m ulak_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `ulak_cli/subcommands/dashboard.py` source registers
