 ...
 * console. It writes ULAK_HOME/.ulak-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Ulak within 30 minutes.
...
export function handoffResultPath(ulakHome: string): string {
  return path.join(ulakHome, '.ulak-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(ulakHome)
  let raw: string
