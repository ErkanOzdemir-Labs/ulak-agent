 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `ulak update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.ulakHome, 'Ulak home')
  const ulakPath = validateRemoteValue(target.ulakPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(ulakPath)

  const updateCommand =
    `env ULAK_HOME=${homeWord} ` +
    `ULAK_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'ULAK_UPDATE_ORIGIN_PROFILE=default ' +
    `ULAK_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `ULAK_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:ULAK_HOME=${psLiteral(home)}`,
    `$env:ULAK_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:ULAK_UPDATE_ORIGIN_PROFILE="default"',
    `$env:ULAK_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:ULAK_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:ULAK_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:ULAK_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:ULAK_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(ulakPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.ulak-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote Ulak updated and every managed SSH profile is ready.'
        : restoreOk
