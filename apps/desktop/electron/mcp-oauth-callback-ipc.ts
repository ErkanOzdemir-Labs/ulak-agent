 ...
 *   - the browser sees only a minimal "return to Ulak" page.
 */
...
const DONE_HTML =
  ...
  '<p>You can close this window and return to Ulak.</p>' +
...
export function registerMcpOauthCallbackIpc() {
  ...
  ipcMain.handle('ulak:mcp-oauth:listen', async () => {
  ...
  ipcMain.handle('ulak:mcp-oauth:wait', async (_event, id, timeoutMs) => {
  ...
  ipcMain.handle('ulak:mcp-oauth:cancel', (_event, id) => {
    dispose(String(id || ''))
