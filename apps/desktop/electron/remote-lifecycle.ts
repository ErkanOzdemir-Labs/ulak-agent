 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Ulak dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Ulak install on the remote (login-shell probe),
 ...
 *     `ULAK_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForUlak in.
 *
 * The minted ULAK_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^ULAK_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.ulak/desktop-ssh'
...
// Resolve the remote ulak executable. An EXPLICIT path is honored strictly
...
async function locateUlak(ssh, remoteUlakPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The ulak binary or wrapper script
    ...
    //     the Ulak version, and
    //   - capability probing: `<python> serve --help` failed entirely.
    // See https://github.com/ErkanOzdemir-Labs/ulak-agent/issues/74411
    return candidate
  }
  ...
  if (remoteUlakPath) {
    if (await isExecutable(remoteUlakPath)) {
      return resolveLauncher(remoteUlakPath)
    }

    const err: any = new Error(
      `The Ulak path you set is not an executable on the remote host: "${remoteUlakPath}". ` +
        'Check the path (it must be the full path to the `ulak` binary on the remote, e.g. ' +
        '~/ulak-agent/.venv/bin/ulak), or clear it to auto-detect.'
    )

    err.kind = 'ulak-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v ulak')}`)).trim()
  ...
  candidates.push('~/.local/bin/ulak')
  candidates.push('/usr/local/bin/ulak')
  candidates.push('~/.ulak/ulak-agent/venv/bin/ulak')
  ...
  const err: any = new Error(
    'Ulak is not installed on the remote host (could not find a `ulak` executable). ' +
      'Install it on the remote with:  curl -fsSL https://ulak-agent.github.com/ErkanOzdemir-Labs/install.sh | sh  ' +
      '— or set the Ulak path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<ulak> --version`,
// e.g. "Ulak Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH ulak a
// connection uses, so a stale/unexpected install is visible.
async function probeUlakVersion(ssh, ulakPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(ulakPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Ulak Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The ULAK_HOME the remote dashboard will use (explicit env wins, else
// ~/.ulak). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteUlakHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${ULAK_HOME:-$HOME/.ulak}"')).trim().split('\n').pop()

    return out || '~/.ulak'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Ulak home.')
...
marker=home/'.ulak-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing Ulak checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, ulakHome) {
  const home = assertSafeRemoteHome(ulakHome)
  let observation = ''
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote Ulak install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote Ulak update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote Ulak update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteUlakProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteUlakHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Ulak profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Ulak home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'ulakPath', 'ulakHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(ssh, pid, spawnNonce, ulakPath = '', ulakHome = '', ownershipId = '', profile = '') {
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(ulakPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/ulak`, leaving neither
      // launcher nor ULAK_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `ulak_home=os.path.expanduser(${shq(ulakHome)}) if ${shq(ulakHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if ulak_home:\n' +
      ' expected_entries.add(os.path.join(ulak_home,"ulak-agent","venv","bin","ulak"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.ulakPath)
  const expectedHome = lock.ulakHome ? expandRemotePath(lock.ulakHome) : "''"
  ...
  const executableMatch = lock.ulakHome
...
function lockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.ulakPath === expected.ulakPath &&
    lock.ulakHome === expected.ulakHome
  )
}
...
expected_path=os.path.expanduser(${py(lock.ulakPath)})
ulak_home=os.path.expanduser(${py(lock.ulakHome)})
expected_entries={expected_path,os.path.join(ulak_home,"ulak-agent","venv","bin","ulak")}
...
// execing Ulak. mutexPath is expandRemotePath() output — a complete shell
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"ulak-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(ulakPath, profile, opts: any = {}) {
  const ulak = expandRemotePath(ulakPath)
  ...
  const marker = expandRemotePath(`${remoteInstallRoot(opts.ulakHome || '~/.ulak')}/.ulak-update-in-progress`)

  const updateMutex = expandRemotePath(
    `${remoteInstallRoot(opts.ulakHome || '~/.ulak')}/.ulak-update-in-progress.mutex`
  )
  ...
  const dashCmd =
    ...
    `exec env ULAK_DESKTOP=1 ${ulak} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} ulak-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, ulakPath) {
  const ulak = expandRemotePath(ulakPath)

  const out = await ssh.exec(
    `help="$(${ulak} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(
  ssh,
  { ulakPath, profile, token, ownershipId, ulakHome = '~/.ulak', assertInstallClear = async () => {} }
) {
  if (!(await remoteSupportsSshOwnership(ssh, ulakPath))) {
    const err: any = new Error(
      'The remote Ulak install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Ulak on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(ulakPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        ulakHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForUlak, a token-gated
...
async function connect(deps) {
  ...
  const ulakHome = await probeRemoteUlakHome(ssh)
  await assertRemoteInstallUpdateClear(ssh, ulakHome)
  const ulakPath = await locateUlak(ssh, remoteUlakPath)
  log(`located ulak at ${ulakPath}`)
  const ulakVersion = await probeUlakVersion(ssh, ulakPath)

  if (ulakVersion) {
    log(`remote ulak version: ${ulakVersion}`)
  }
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this Ulak Desktop build (${lock.reason}). ` +
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.ulakPath === ulakPath &&
      lock.ulakHome === ulakHome
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    ownershipId,
    ulakHome,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, ulakHome)
  })
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForUlak(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
