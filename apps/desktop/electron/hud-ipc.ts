...
export function registerHudIpc({
  ...
  ipcMain.on('ulak:hud:native-drag', event => {
    event.returnValue = hudWindowing().move === 'native-drag'
  })

  ipcMain.on('ulak:hud:windowing', event => {
  ...
  ipcMain.on('ulak:hud:workspace-transfer', (event, transferring) => {
  ...
  ipcMain.handle('ulak:hud:open', async (_event, request) => {
    openHudWindow(
  ...
  ipcMain.handle('ulak:hud:frost', (_event, showing) => {
  ...
  // reaches the bar.
  ipcMain.on('ulak:hud:ignore-mouse', (_event, ignore) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('ulak:hud:begin-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('ulak:hud:end-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('ulak:hud:move-by', (event, delta) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('ulak:hud:set-bounds', (event, bounds) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('ulak:hud:reset-layout', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('ulak:hud:session', (event, sessionId) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('ulak:hud:close', async () => {
    closeHudWindow()
