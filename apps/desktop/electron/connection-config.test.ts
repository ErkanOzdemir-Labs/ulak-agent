...
test('normalizeRemoteHeaders keeps safe proxy headers and drops transport/auth headers', () => {
  assert.deepEqual(
    normalizeRemoteHeaders({
      ...
      'X-Ulak-Session-Token': { encoding: 'plain', value: 'token' },
...
test('remoteRequestMatchesBaseUrl treats HTTPS and WSS as the same gateway origin', () => {
  assert.equal(
    remoteRequestMatchesBaseUrl('wss://ulak.example.com/gateway/api/ws?ticket=abc', 'https://ulak.example.com/gateway'),
    true
  )
  assert.equal(remoteRequestMatchesBaseUrl('ws://ulak.example.com/api/ws', 'http://ulak.example.com'), true)
  assert.equal(
    remoteRequestMatchesBaseUrl('wss://ulak.example.com/other/api/ws', 'https://ulak.example.com/gateway'),
    false
  )
  assert.equal(
    remoteRequestMatchesBaseUrl('wss://other.example.com/gateway/api/ws', 'https://ulak.example.com/gateway'),
    false
  )
})
...
test('profileRemoteOverride returns the per-profile remote with defaulted auth mode', () => {
  const config = {
    profiles: {
      coder: { mode: 'remote', url: '  https://coder.example.com/ulak  ', token: { value: 'sek' } }
    }
  }

  assert.deepEqual(profileRemoteOverride(config, 'coder'), {
    url: 'https://coder.example.com/ulak',
    authMode: 'token',
    token: { value: 'sek' }
  })
})
...
test('profileRemoteOverride treats a cloud entry as a remote override', () => {
  ...
  const config = {
    profiles: {
      coder: { mode: 'cloud', url: 'https://agent-1.agents.github.com/ErkanOzdemir-Labs', authMode: 'oauth' }
    }
  }

  assert.deepEqual(profileRemoteOverride(config, 'coder'), {
    url: 'https://agent-1.agents.github.com/ErkanOzdemir-Labs',
    authMode: 'oauth',
    token: undefined
  })
})
...
test('localProfileEntry preserves inactive SSH drafts but drops Cloud state', () => {
  const ssh = { mode: 'ssh', host: 'box', user: 'alice', remoteUlakPath: '/ulak' }
...
// --- translateSelfProfileQuery (registry SSH-scoped ulak:api contract) ---
...
test('resolveProfileApiRequest routes action-status polls with the action-spawning routes', () => {
  ...
  // install): _spawn_ulak_action registers the dynamic action name only in
...
test('normalizeRemoteBaseUrl strips trailing slashes, hash, and query', () => {
  ...
  assert.equal(normalizeRemoteBaseUrl('https://gw.example.com/ulak/'), 'https://gw.example.com/ulak')
  assert.equal(normalizeRemoteBaseUrl('https://gw.example.com/ulak?x=1#frag'), 'https://gw.example.com/ulak')
})

test('normalizeRemoteBaseUrl preserves a path prefix', () => {
  assert.equal(normalizeRemoteBaseUrl('https://host/ulak'), 'https://host/ulak')
})
...
test('normalizeRemoteBaseUrl auto-prepends http:// for scheme-less host:port input', () => {
  ...
  assert.equal(normalizeRemoteBaseUrl('gw.example.com/ulak/'), 'http://gw.example.com/ulak')
})
...
test('buildGatewayWsUrl honors a path prefix', () => {
  assert.equal(buildGatewayWsUrl('https://host/ulak', 't'), 'wss://host/ulak/api/ws?token=t')
})
...
test('buildGatewayWsUrlWithTicket uses ?ticket= not ?token=', () => {
  const url = buildGatewayWsUrlWithTicket('https://gw.example.com/ulak', 'tkt-9')
  assert.equal(url, 'wss://gw.example.com/ulak/api/ws?ticket=tkt-9')
...
test('cookiesHaveSession detects the bare access-token cookie', () => {
  assert.equal(cookiesHaveSession([{ name: 'ulak_session_at', value: 'x' }]), true)
})

test('cookiesHaveSession detects the __Host- and __Secure- prefixed variants', () => {
  assert.equal(cookiesHaveSession([{ name: '__Host-ulak_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveSession([{ name: '__Secure-ulak_session_at', value: 'x' }]), true)
})

test('cookiesHaveSession is false for an empty value', () => {
  assert.equal(cookiesHaveSession([{ name: 'ulak_session_at', value: '' }]), false)
})
...
test('cookiesHaveSession ignores unrelated cookies (AT-only by design)', () => {
  ...
  assert.equal(cookiesHaveSession([{ name: 'ulak_session_rt', value: 'x' }]), false)
...
test('AT_COOKIE_VARIANTS covers all three deploy shapes', () => {
  assert.deepEqual(AT_COOKIE_VARIANTS, ['__Host-ulak_session_at', '__Secure-ulak_session_at', 'ulak_session_at'])
})

test('RT_COOKIE_VARIANTS covers all three deploy shapes', () => {
  assert.deepEqual(RT_COOKIE_VARIANTS, ['__Host-ulak_session_rt', '__Secure-ulak_session_rt', 'ulak_session_rt'])
})

// --- cookiesHaveLiveSession (AT or RT — the connectivity check) ---

test('cookiesHaveLiveSession is true for a live access-token cookie', () => {
  assert.equal(cookiesHaveLiveSession([{ name: 'ulak_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Host-ulak_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Secure-ulak_session_at', value: 'x' }]), true)
})
...
test('cookiesHaveLiveSession is true for an RT cookie even with NO access-token cookie', () => {
  ...
  assert.equal(cookiesHaveLiveSession([{ name: 'ulak_session_rt', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Host-ulak_session_rt', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Secure-ulak_session_rt', value: 'x' }]), true)
})

test('cookiesHaveLiveSession is true when both AT and RT are present', () => {
  assert.equal(
    cookiesHaveLiveSession([
      { name: 'ulak_session_at', value: 'a' },
      { name: 'ulak_session_rt', value: 'r' }
    ]),
    true
  )
})
...
test('cookiesHaveLiveSession is false for empty values', () => {
  assert.equal(cookiesHaveLiveSession([{ name: 'ulak_session_at', value: '' }]), false)
  assert.equal(cookiesHaveLiveSession([{ name: 'ulak_session_rt', value: '' }]), false)
  assert.equal(
    cookiesHaveLiveSession([
      { name: 'ulak_session_at', value: '' },
      { name: 'ulak_session_rt', value: '' }
    ]),
    false
  )
})
...
test('cookiesHavePrivySession does NOT treat ulak gateway cookies as a portal session', () => {
  // The whole point of Q7: a gateway session cookie is NOT a portal sign-in.
  assert.equal(cookiesHavePrivySession([{ name: 'ulak_session_at', value: 'x' }]), false)
  assert.equal(cookiesHavePrivySession([{ name: '__Host-ulak_session_rt', value: 'x' }]), false)
})
...
test('cookiesHavePrivyAccessToken is false for empty values, gateway cookies, and non-arrays', () => {
  ...
  assert.equal(cookiesHavePrivyAccessToken([{ name: 'ulak_session_at', value: 'x' }]), false)
...
test('gateway WS URL IPC result serializes success and the auth-vs-transport matrix', async () => {
  ...
  for (const error of [
    ...
    new Error('Timed out connecting to Ulak backend after 8000ms'),
...
// BEFORE waitForUlakReady. This mirrors main.ts buildRemoteConnection's
...
test('OAuth ticket-mint 503 surfaces the Cloud-down error (startup boundary)', () => {
  const baseUrl = 'https://ares-3009.agents.github.com/ErkanOzdemir-Labs'
  ...
  if (cloudError !== null) {
    ...
    assert.ok(cloudError.message.includes('Nous Cloud agent ares-3009.agents.github.com/ErkanOzdemir-Labs is down'))
