...
test('environment URL without its token keeps the existing error', () => {
  assert.throws(
    ...
    /ULAK_DESKTOP_REMOTE_TOKEN is not/
  )
})
...
test('global SSH treats an omitted port as 22 and checks the primary route', () => {
  const route = resolveDesktopRemoteRoute({
    config: { mode: 'ssh', remote: { mode: 'ssh', host: 'box.test', user: 'ulak' } },
    registry: registry('ssh-primary', [
      { id: 'ssh-primary', kind: 'ssh', label: 'SSH primary', host: 'box.test', user: 'ulak', port: 22 }
    ])
  })
...
test('profile remote wins over a registry-backed global SSH route', () => {
  const route = resolveDesktopRemoteRoute({
    config: {
      mode: 'ssh',
      remote: { mode: 'ssh', host: 'global-box.test', user: 'ulak' },
      profiles: {
        worker: { mode: 'remote', url: 'https://worker.test', authMode: 'token', token: tokenA }
      }
    },
    ...
    registry: registry('global-ssh', [
      { id: 'global-ssh', kind: 'ssh', label: 'Global SSH', host: 'global-box.test', user: 'ulak' },
...
test('profile SSH wins over a different registry primary SSH route', () => {
  const route = resolveDesktopRemoteRoute({
    config: {
      ...
      remote: { mode: 'ssh', host: 'global-box.test', user: 'ulak' },
      profiles: {
        worker: { mode: 'ssh', host: 'worker-box.test', user: 'ulak' }
      }
    },
    profile: 'worker',
    registry: registry('global-ssh', [
      { id: 'global-ssh', kind: 'ssh', label: 'Global SSH', host: 'global-box.test', user: 'ulak' },
      { id: 'worker-ssh', kind: 'ssh', label: 'Worker SSH', host: 'worker-box.test', user: 'ulak' }
    ])
  })
...
test('local route does not inherit an unrelated registry SSH connection', () => {
  const route = resolveDesktopRemoteRoute({
    config: { mode: 'local' },
    registry: registry('local', [
      { id: 'unused-ssh', kind: 'ssh', label: 'Unused SSH', host: 'box.test', user: 'ulak' }
    ])
  })
...
// remote transport, or startUlak() spawns a loopback `ulak serve` the
