 ...
 *
 * The staged Tauri `ulak-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `ulak update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `ulak update`, swaps/relaunches
 * the app, and writes .ulak-update-result.json. With the app gone before
 * the update starts, the ULAK_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `ulak update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into ULAK_HOME on *every* platform
 * (`ulak-setup.exe` on Windows, `ulak-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_ulak_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `ulak update`,
 ...
 */
export function resolveStagedUpdaterBinary(ulakHome: string, deps: ResolveStagedUpdaterBinaryDeps = {}): string | null {
  const isWindows = deps.isWindows ?? process.platform === 'win32'
  ...
  const candidate = path.join(ulakHome, 'ulak-setup.exe')
 ...
 *
 * `copy_self_to_ulak_home` deliberately no-ops during `--update`
 ...
 * "Another Ulak update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `ulak update`, which
 ...
 * the installer is written to ULAK_HOME at install/repair time, making mtime
