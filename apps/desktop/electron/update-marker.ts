...
export function markerPath(ulakHome) {
  return path.join(ulakHome, '.ulak-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(ulakHome)
  let raw
 ...
 *
 * The Tauri-based ulak-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(ulakHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(ulakHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * Ulak to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(ulakHome, opts)
