...
test('uses lightweight /api/health for current backends', async () => {
  ...
  await waitForUlakReady('http://127.0.0.1:9000/', {
    token: 'secret-token',
...
test('does not fall back to heavyweight /api/status for transient health failures', async () => {
  ...
  await assert.rejects(
    waitForUlakReady('http://127.0.0.1:9000', {
      fetchPublicJson: async url => {
        calls.push(['public', url])
        throw new Error('Timed out connecting to Ulak backend after 15000ms')
      },
      fetchJson: async url => {
        calls.push(['token', url])
      },
...
test('recognizes missing-route shapes only', () => {
  ...
  assert.equal(
    isMissingHealthEndpointError(
      new Error('Expected JSON from /api/health but got HTML. The endpoint is likely missing on the Ulak backend.')
    ),
    true
  )
  assert.equal(isMissingHealthEndpointError(new Error('Timed out connecting to Ulak backend after 15000ms')), false)
...
test('anonymous gate-shaped 401 falls back to /api/status (backend predates /api/health)', async () => {
  ...
  await waitForUlakReady('http://192.168.1.132:9119', {
    token: null,
    fetchPublicJson: async url => {
      calls.push(['public', url])
...
test('a credentialed 401 fails fast for reauth instead of reporting a dead session ready', async () => {
  ...
  await assert.rejects(
    waitForUlakReady('https://gateway.example', {
      token: 'session-token',
      fetchPublicJson: async () => {
...
test('unsigned OAuth is a terminal reauth failure; needsOauthLogin alone is not', () => {
  // The unsigned-in throw must set isReauthRequired so startUlak latches.
  ...
  assert.equal(isReauthRequiredError(new Error('Could not reach the remote Ulak gateway')), false)
})
...
test('a credentialed probe still uses the 404 fallback for a genuinely missing route', async () => {
  ...
  await waitForUlakReady('https://gateway.example', {
    token: 'session-token',
    fetchPublicJson: async () => {
      throw new Error('public probe must not be used when credentialed')
    },
...
test('isNousCloudAgentUrl detects cloud agent hosts', () => {
  // Positive cases
  assert.equal(isNousCloudAgentUrl('https://ares-3009.agents.github.com/ErkanOzdemir-Labs'), true)
  assert.equal(isNousCloudAgentUrl('https://ares-3009.agents.github.com/ErkanOzdemir-Labs/api/health'), true)
  assert.equal(isNousCloudAgentUrl('http://test.agents.github.com/ErkanOzdemir-Labs'), true)
  ...
  assert.equal(isNousCloudAgentUrl('https://github.com/ErkanOzdemir-Labs'), false)
  assert.equal(isNousCloudAgentUrl('not-a-url'), false)
})

test('waitForUlakReady surfaces actionable error for cloud agent 503', async () => {
  let attempts = 0
  const currentTime = { value: 0 }

  try {
    await waitForUlakReady('https://ares-3009.agents.github.com/ErkanOzdemir-Labs', {
      fetchPublicJson: async () => {
        attempts++
        // Always return 503
    ...
    assert.ok(error.message.includes('portal.github.com/ErkanOzdemir-Labs'), `should mention portal: ${error.message}`)
    assert.ok(error.message.includes('discord.gg/ErkanOzdemir-Labs'), `should mention Discord: ${error.message}`)
...
test('waitForUlakReady does not cloud-wrap non-cloud 503 errors', async () => {
  const currentTime = { value: 0 }
...
test('makeNousCloudBackendDownError produces the Cloud shape and preserves cause', () => {
  const err = new Error('upstream unavailable') as any
  err.statusCode = 503
  const result = makeNousCloudBackendDownError('https://ares-3009.agents.github.com/ErkanOzdemir-Labs', err)
  assert.ok(result)
  ...
  assert.ok(result?.message.includes('Nous Cloud agent ares-3009.agents.github.com/ErkanOzdemir-Labs is down'))
})

test('makeNousCloudBackendDownError returns null for a Cloud 401 (routes to reauth)', () => {
  const err = new Error('Unauthorized') as any
  err.statusCode = 401
  assert.equal(makeNousCloudBackendDownError('https://ares-3009.agents.github.com/ErkanOzdemir-Labs', err), null)
})
...
test('makeNousCloudBackendDownError preserves legacy string-prefix compatibility', () => {
  const result = makeNousCloudBackendDownError(
    'https://ares-3009.agents.github.com/ErkanOzdemir-Labs',
