...
export interface I18nConfigClient {
  getConfig: () => Promise<UlakConfigRecord>
  saveConfig: (config: UlakConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.ulakDesktop?.api) {
      return Promise.resolve({})
    }

    return getUlakConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.ulakDesktop?.api) {
...
export function getConfigDisplayLanguage(config: UlakConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: UlakConfigRecord, locale: Locale): UlakConfigRecord {
