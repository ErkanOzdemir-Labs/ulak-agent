...
export const ru = defineLocale({
  sessionImport: {
    ...
    subtitle: 'Перенесите разговор в Ulak и продолжите с того места, где остановились.',
    ...
    chooseHelp: 'Выберите сессию, чтобы прочитать историю перед импортом в Ulak.',
    ...
    you: 'Вы',
    snapshot: 'Этот разговор уже есть в Ulak. Откройте существующую копию, чтобы продолжить.',
    copyNotice:
      'Копируется текст разговора. Исходные файлы не меняются. Вывод инструментов и рассуждения не переносятся.',
    importing: 'Импорт…',
    open: 'Открыть в Ulak',
    continue: 'Продолжить в Ulak',
  ...
  boot: {
    ready: 'Ulak Desktop готов',
    desktopBootFailedWithMessage: message => `Не удалось запустить приложение: ${message}`,
    steps: {
      connectingGateway: 'Подключение к шлюзу',
      loadingSettings: 'Загрузка настроек Ulak',
      loadingSessions: 'Загрузка последних сеансов',
      retryingRemoteBackend: 'Переподключение к удалённому бэкенду Ulak…',
      startingDesktopConnection: 'Запуск подключения приложения',
      startingUlakDesktop: 'Запуск Ulak Desktop…'
    },
    errors: {
      backgroundExited: 'Фоновый процесс Ulak завершён.',
      backgroundExitedDuringStartup: 'Фоновый процесс Ulak завершился при запуске.',
    ...
    failure: {
      title: 'Ulak не удалось запустить',
      description:
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      'Ваш бэкенд Ulak старше этой сборки приложения и может работать некорректно. Обновите их, чтобы они совпали.',
    installMethodUnsupportedTitle: 'Неподдерживаемый способ установки',
    updateUlak: 'Обновить Ulak',
    ...
    errors: {
      ...
      methodNotAllowed:
        'Бэкенд приложения отклонил запрос (405 Method Not Allowed). Попробуйте перезапустить Ulak Desktop.',
    ...
    native: {
      ...
      inputBody: 'Ulak ожидает ваш ответ.',
      turnDoneTitle: 'Ulak завершил',
      turnDoneBody: '',
  ...
  settings: {
    ...
    resetConfirm: 'Сбросить все настройки к значениям Ulak по умолчанию?',
    ...
    plugins: {
      ...
      agent: {
        ...
        blurb:
          'Плагины, установленные в бэкенд Ulak — инструменты, навыки, MCP-серверы, хуки и slash-команды. Переносимые — пакеты Agent Plugins (навыки + MCP-бандлы, работающие и в других агентах). Переключатели действуют для новых сеансов.',
        ...
        updateBackendToManage: 'Обновите бэкенд Ulak, чтобы управлять этим плагином из приложения.',
        sources: {
          bundled: 'встроенный',
      ...
      installModal: {
        ...
        agentTargetLocal: profile => `Устанавливается в локальный бэкенд ${profile} (~/.ulak/plugins/)`,
    ...
    notifications: {
      ...
      focusedHint: 'Оповещения о завершении срабатывают только пока Ulak в фоне.',
      kinds: {
        approval: {
        ...
        input: {
          label: 'Нужен ввод',
          description: 'Ulak задал вопрос или требует пароль/секрет.'
        },
        turnDone: {
          label: 'Ответ готов',
          description: 'Ход завершился, пока Ulak был в фоне.'
        },
        turnError: {
          label: 'Ход не удался',
        ...
        plugin: {
          ...
          description: 'Плагин приложения отправил уведомление, пока Ulak был в фоне.'
        }
      },
    ...
    appearance: {
      ...
      colorModeDesc: 'Выберите фиксированный режим или позвольте Ulak следовать настройкам системы.',
      ...
      reactionsDesc: 'Эмодзи-тапбеки в стиле iMessage — реагируйте на сообщения, и Ulak сможет реагировать на ваши.',
      ...
      pet: {
        title: 'Питомец',
        intro:
          'Заберите анимированного питомца из petdex, который парит над приложением и реагирует на действия Ulak — «бегает», пока выполняются инструменты, радуется успеху и хмурится при ошибках.',
        restartHint:
          'Питомцам нужен быстрый перезапуск — текущее приложение запустилось до появления этой функции. Выйдите из Ulak и откройте снова, затем вернитесь сюда.',
        on: 'Вкл',
        off: 'Выкл',
        scaleTitle: 'Размер',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      agent: {
        ...
        maxTurns: 'Верхний предел ходов с вызовами инструментов, после которого Ulak останавливает запуск.'
      },
      terminal: {
      ...
      fileReadMaxChars: 'Максимальное число символов, которые Ulak может прочитать из одного запроса к файлу.',
      approvals: {
        mode: 'Как Ulak обрабатывает команды, требующие явного подтверждения.',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'Когда Ulak обновляет себя из приложения (без запроса в терминале), сохранять локальные правки исходников (stash) или выбрасывать (discard). Обновления из терминала всегда спрашивают.'
      }
    }),
    about: {
      heading: 'Ulak Desktop',
      version: value => `Версия ${value}`,
      versionUnavailable: 'Версия недоступна',
      bundleOutOfSync: 'Сборка приложения устарела',
      bundleOutOfSyncDesc:
        'Рантайм Ulak обновлён, но само приложение — ещё старая сборка: новые функции интерфейса (например, Bot Mode) не появятся до обновления. Запустите обновление ниже, чтобы пересобрать приложение. Если предупреждение не исчезнет, переустановите с последнего установщика.',
      ...
      automaticUpdatesDesc: 'Ulak автоматически проверяет обновления в фоне и сообщает, когда они готовы.',
    ...
    config: {
      ...
      loading: 'Загрузка конфигурации Ulak…',
    ...
    quickEntry: {
      ...
      enabledDesc:
        'Глобальным горячим ключом вызывайте маленький композер откуда угодно и отправляйте запрос, не открывая Ulak.',
    ...
    connections: {
      ...
      intro:
        'Управляйте этим устройством и всеми шлюзами Ulak, до которых можно дотянуться через удалённые, SSH или Cloud-соединения.',
      stagedNote:
      ...
      updateSkippedCloud: 'Управляется Ulak Cloud',
      kindLocal: 'Локальный',
      kindRemote: 'Удалённый шлюз',
      kindCloud: 'Ulak Cloud',
      kindSsh: 'SSH',
      kindLocalDesc: 'Рантайм Ulak, которым управляет это приложение.',
      kindRemoteDesc: 'Шлюз Ulak, доступный по HTTP(S) — LAN, Tailscale или интернет.',
      kindCloudDesc: 'Хостинговая инстанция, обнаруженная через ваш аккаунт Ulak Cloud.',
      kindSshDesc: 'Установка Ulak, доступная по SSH.',
      labelTitle: 'Имя',
      labelDesc:
      ...
      headersDesc:
        'Отправляются с каждым HTTP- и WebSocket-запросом к этому шлюзу — например, для прокси доступа вроде Cloudflare Access (CF-Access-Client-Id / CF-Access-Client-Secret). Значения хранятся зашифрованными. Заголовки, которыми управляет Ulak (Authorization, Cookie, Host…), игнорируются.',
      ...
      cloudAddHint:
        'Подсказка: вход в Ulak Cloud выше автоматически обнаруживает ваших агентов — эту форму используйте только для ручной регистрации известного URL инстанса.',
    ...
    gateway: {
      ...
      intro:
        'По умолчанию — локальный. Используйте удалённый, когда приложение должно управлять бэкендом Ulak в другом месте. Соединения шлюзов — на уровне машины; профили обнаруживаются из подключённых шлюзов.',
      envOverrideTitle: 'Переменные окружения управляют этой сессией приложения.',
      envOverrideDesc:
        'Сбросьте ULAK_DESKTOP_REMOTE_URL и ULAK_DESKTOP_REMOTE_TOKEN, чтобы использовать сохранённую настройку ниже.',
      modeTitle: 'Режим подключения',
      localTitle: 'Локальный шлюз',
      localDesc: 'Запускает приватный бэкенд Ulak на localhost. Это значение по умолчанию, работает офлайн.',
      remoteTitle: 'Удалённый шлюз',
      remoteDesc: 'Подключает это приложение к удалённому бэкенду Ulak.',
      remoteAuthHint:
        'Хостинговые шлюзы используют OAuth или логин/пароль; самохостинговые могут использовать токен сессии.',
      cloudTitle: 'Ulak Cloud',
      cloudDesc: 'Войдите в Ulak Cloud один раз и выбирайте агентов из своего аккаунта — без вставки URL.',
      cloudSignInTitle: 'Ulak Cloud',
      cloudSignIn: 'Войти в Ulak Cloud',
      cloudSignedIn: 'Вы вошли в Ulak Cloud',
      cloudNeedsSignIn: 'Войдите в Ulak Cloud, чтобы обнаружить агентов в вашем аккаунте.',
      ...
      cloudDiscoverFailed: 'Не удалось загрузить агентов Ulak Cloud',
      cloudConnectFailed: 'Не удалось подключиться к этому агенту',
      cloudSignInFailed: 'Не удалось войти в Ulak Cloud',
      cloudSignedOutTitle: 'Выход из Ulak Cloud',
      cloudSignedOutMessage: 'Сессия Ulak Cloud сброшена.',
      ...
      remoteUrlDesc: 'Базовый URL удалённого бэкенда дашборда. Поддерживаются префиксы пути, например /ulak.',
      ...
      restartingMessage: 'Ulak Desktop переподключится с сохранёнными настройками — оболочка останется открытой.',
      savedMessage: 'Сохранено для следующего перезапуска.',
      connectedTo: (baseUrl, version) => `Подключено к ${baseUrl}${version ? ` · Ulak ${version}` : ''}`,
      ...
      sshDesc:
        'Ulak запускается на удалённой машине по SSH и туннелируется в это приложение — ничего не нужно запускать или открывать самим. Требуется рабочая SSH-аутентификация по ключу на хост.',
      sshTrustHint:
      ...
      sshUlakPathTitle: 'Путь к Ulak (необязательно)',
      sshUlakPathDesc: 'Полный путь к бинарнику ulak на удалённой машине. Пусто = автоопределение.',
      sshUlakPathPlaceholder: 'автоопределение',
      ...
      sshReachable: (host, platform) => `Доступен: ${host} (${platform}) — Ulak найден`,
      ...
      sshErrAuth:
        'SSH-аутентификация не удалась. Загрузите ключ в ssh-agent (ssh-add) или задайте IdentityFile в ~/.ssh/config — Ulak запускает ssh в неинтерактивном режиме.',
      sshErrHostKey:
      ...
      sshErrNotInstalled:
        'Ulak не установлен на удалённой машине. Установите его там (curl -fsSL https://ulak-agent.github.com/ErkanOzdemir-Labs/install.sh | sh) или задайте путь к Ulak.',
      sshErrPlatform:
        'Неподдерживаемая удалённая платформа. SSH-режим Ulak Desktop поддерживает удалённые хосты Linux, macOS и Windows.',
      sshErrTimeout: 'SSH-соединение истекло. Хост может быть недоступен или «спит».',
      sshErrUpdateRequired: 'Перед подключением через SSH обновите Ulak на удалённой машине.',
    ...
    mcp: {
      ...
      deepLinkDescription:
        'Ссылка запросила добавить этот MCP-сервер в Ulak. Проверьте конфигурацию ниже — она пришла из ссылки, а не из Ulak.',
    ...
    providers: {
      ...
      intro:
        'Войдите по подписке — копировать API-ключ не нужно. Ulak проведёт вход в браузере прямо здесь, в приложении.',
      ...
      localEndpoint: {
        ...
        description: 'Направьте Ulak на любой OpenAI-совместимый эндпоинт (Zyphra, vLLM, llama.cpp, Ollama и т. д.).'
      },
  ...
  skills: {
    ...
    skillArchivedMessage: 'Восстановить через ulak curator restore.',
    hub: {
  ...
  starmap: {
    ...
    emptyDesc: 'По мере того как Ulak создаёт навыки и память для вашей работы, они появятся здесь.',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'Перезапустите Ulak, чтобы использовать питомцев — бэкенд старше этой функции.',
    ...
    generatePet: {
      ...
      staleBackend: 'Обновите Ulak, чтобы генерировать питомцев.',
      backgroundHint: 'Можно закрыть — Ulak уведомит, когда будет готово.',
    ...
    nav: {
      ...
      settings: { title: 'Настройки', detail: 'Настройка Ulak desktop' },
    ...
    ulakActiveSessions: (version, count) => `Ulak ${version} · Активные сеансы: ${count}`,
  ...
  messaging: {
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'ID пользователя бота', placeholder: '@ulak:example.org' },
  ...
  profiles: {
    ...
    deleting: 'Удаление...',
    createDesc: 'Профили — это независимые среды Ulak: отдельные настройки, навыки и SOUL.md.',
    nameLabel: 'Имя',
  ...
  cron: {
    ...
    modelImpact: {
      ...
      saveFailed: 'Ulak не сохранил это изменение модели.'
    },
    ...
    newCron: 'Новый cron',
    emptyDescNew:
      'Запланируйте промпт, который будет выполняться по cron-выражению. Ulak выполнит его и доставит результаты в выбранное вами место.',
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'Обновите бэкенд Ulak, чтобы создавать проекты — ваш бэкенд старше этого desktop-приложения (Настройки → Обновления → Бэкенд).',
      deleteConfirm: 'Это удалит сохранённый проект из Ulak. Файлы, git-репозитории и worktrees не пострадают.',
      ...
      worktreeStaleBackend:
        'Обновите бэкенд Ulak, чтобы создавать worktrees по этому удалённому соединению — он старше git worktree API.',
  ...
  composer: {
    ...
    placeholderStarting: 'Запуск Ulak...',
    placeholderReconnecting: 'Переподключение к Ulak…',
    ...
    attachUrlDesc: 'Ulak загрузит страницу и добавит её как контекст для этого хода.',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: 'Попросить Ulak открыть PR',
  ...
  updates: {
    stages: {
      ...
      pull: 'Почти готово…',
      pydeps: 'Завершаем…',
      update: 'Обновляем Ulak…',
      rebuild: 'Пересобираем desktop-приложение…',
      restart: 'Перезапускаем Ulak…',
    ...
    unsupportedMessage: 'Эта версия Ulak не может обновлять себя из приложения.',
    ...
    availableBody: 'Новая версия Ulak готова к установке.',
    availableTitleBackend: 'Доступно обновление бэкенда',
    availableBodyBackend: 'Новая версия подключённого бэкенда Ulak готова к установке.',
    ...
    manualBody:
      'Ulak установлен из командной строки, поэтому обновления тоже выполняются там. Вставьте это в терминал:',
    manualPickedUp: 'Ulak подхватит новую версию при следующем запуске.',
    guiSkewTitle: 'Обновите desktop-приложение',
    guiSkewBody:
      'Бэкенд обновлён, но пакет этого desktop-приложения не изменился. Обновите или переустановите desktop-приложение Ulak (ваш AppImage / .deb / .rpm), чтобы версии совпали.',
    copy: 'Копировать',
    copied: 'Скопировано',
    done: 'Готово',
    applyingBody:
      'Обновлятор Ulak возьмёт процесс на себя в своём окне и автоматически перезапустит Ulak, когда закончит. Пожалуйста, не открывайте Ulak вручную во время обновления.',
    applyingBodyBackend:
      'Удалённый бэкенд применяет обновление и перезапустится. Ulak переподключится автоматически, когда он снова заработает.',
    applyingClose: 'Это окно закроется во время обновления, затем Ulak откроется сам.',
    errorTitle: 'Обновление не завершилось',
    errorBody: 'Не переживайте — ничего не потеряно. Можно попробовать снова прямо сейчас.',
    blockerTitle: 'Закрыть локальные предпросмотры, чтобы обновить Ulak?',
    blockerBody:
      'Ulak нужно остановить эти локальные предпросмотры перед обновлением. Ваши файлы не будут изменены или удалены.',
    foreignBlockerTitle: 'Закрыть другие процессы, чтобы обновить Ulak',
    foreignBlockerBody:
      'Ulak не может безопасно закрыть эти процессы автоматически. Закройте приложение, терминал или службу, которой принадлежит каждый из них, и повторите обновление.',
    mixedBlockerBody:
      'Ulak может закрыть локальные предпросмотры, перечисленные ниже. Остальные процессы нужно закрыть вручную, прежде чем обновление сможет продолжиться.',
  ...
  install: {
    ...
    oneTimeTitle: 'Ulak требует одноразовой установки',
    ...
    setupChoiceTitle: 'Настройка Ulak Desktop',
    setupChoiceDesc:
      'Подключите это приложение к уже работающему шлюзу Ulak или установите Ulak локально на этот компьютер.',
    connectExistingTitle: 'Подключиться к существующему Ulak',
    ...
    installLocalTitle: 'Установить Ulak локально',
    installLocalDesc: 'Скачайте Ulak, создайте его Python-окружение и запустите бэкенд на этом компьютере.',
    localStartUnavailable: 'Не удалось начать локальную установку. Перезапустите Ulak Desktop и попробуйте снова.',
    remoteSetupTitle: 'Подключиться к существующему Ulak',
    remoteSetupDesc: 'Введите URL вашего шлюза. Ulak Desktop определит, нужен токен или вход через браузер.',
    remoteUrlTitle: 'URL шлюза',
    remoteUrlDesc: 'Используйте базовый URL шлюза Ulak, включая https:// для удалённых.',
    remoteUrlPlaceholder: 'https://gateway.example.com/ulak',
    probing: 'Определяем аутентификацию шлюза...',
    probeError: 'Не удалось подключиться к этому шлюзу Ulak.',
    ...
    settingUpTitle: 'Настройка Ulak Agent',
    finishingTitle: 'Завершаем',
    failedDesc:
      'Один из шагов установки завершился ошибкой. На Windows это может произойти, если запущена другая инстанция Ulak CLI или desktop. Остановите все работающие инстанции Ulak и повторите. Подробности — ниже или в журнале desktop.',
    activeDesc:
      'Это одноразовая настройка. Установщик Ulak скачивает зависимости и настраивает вашу машину. При следующих запусках этот шаг будет пропущен.',
  ...
  onboarding: {
    headerTitle: 'Настроим для вас Ulak Agent',
    headerDesc: 'Подключите провайдера модели, чтобы начать общение. Большинство вариантов — в один клик.',
    preparingInstall: 'Ulak завершает установку. Обычно это занимает меньше минуты при первом запуске.',
    starting: 'Запускаем Ulak…',
    ...
    recommended: 'Рекомендуется',
    connected: 'Подключено',
    featuredPitch: 'Одна подписка, 300+ передовых моделей — рекомендуемый способ запускать Ulak',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'self-hosted',
        description:
          'Укажите Ulak локальный или self-hosted OpenAI-совместимый endpoint (vLLM, llama.cpp, Ollama и т.д.).'
      }
    },
    ...
    flowSubtitles: {
      ...
      device_code: 'Откроет страницу подтверждения в браузере — Ulak подключится автоматически',
    ...
    authorizeThere: 'Авторизуйте Ulak там.',
    ...
    autoBrowser: provider =>
      `Мы открыли ${provider} в вашем браузере. Авторизуйте Ulak там, и подключение произойдёт автоматически — ничего копировать и вставлять не нужно.`,
  ...
  shell: {
    ...
    statusbar: {
      ...
      desktopVersion: version => `Ulak Desktop v${version}`,
      ...
      connectionCloudTooltip: host => `Ulak Cloud · ${host}`,
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} — ${size}. Ulak покажет только первые 512 КБ.`,
    ...
    web: {
      ...
      tryAgain: 'Попробовать снова',
      restarting: 'Ulak перезапускается...',
      askRestart: 'Попросить Ulak перезапустить сервер',
      lookingRestart: taskId => `Ulak ищет сервер предпросмотра для перезапуска (${taskId})`,
      restartingTitle: 'Перезапуск сервера предпросмотра',
      restartingMessage: 'Ulak работает в фоне. Следите за прогрессом в консоли предпросмотра.',
      ...
      blankPageBody: 'Введите адрес выше, чтобы просматривать, или попросите Ulak открыть страницу.',
      finishedRestarting: message => `Ulak завершил перезапуск сервера предпросмотра${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'Ulak не смог перезапустить сервер.',
      stillWorking:
        'Ulak всё ещё работает, но результата перезапуска пока нет. Команда сервера может выполняться в foreground.',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'Ulak загружает ответ',
    ...
    approval: {
      gatewayDisconnected: 'Шлюз Ulak не подключён',
      ...
      alwaysDescription: pattern =>
        `Это добавит паттерн «${pattern}» в ваш постоянный список разрешений (~/.ulak/config.yaml). Ulak больше не будет спрашивать о подобных командах — ни в этом сеансе, ни в будущем.`,
  ...
  prompts: {
    ...
    sudoDesc:
      'Ulak нужен ваш пароль sudo, чтобы выполнить команду с повышенными правами. Он отправляется только вашему локальному агенту.',
    sudoPlaceholder: 'пароль sudo',
    secretTitle: 'Требуется секрет',
    secretDesc: 'Ulak нужны учётные данные, чтобы продолжить.',
  ...
  desktop: {
    ...
    restartToUseSaveImage: 'Перезапустите Ulak Desktop, чтобы использовать «Сохранить изображение».',
    restartToSaveImages: 'Перезапустите Ulak Desktop, чтобы сохранять изображения',
    ...
    handoff: {
      ...
      timedOut: 'Превышено время ожидания шлюза. Выполняется ли `ulak gateway`?'
    }
  },
  errors: {
