/**
 * Live skin sync from the Ulak backend.
 *
 * The backend resolves the active skin (built-in or `$ULAK_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Ulak
 * authoring/activating a skin from a prompt, or `/skin` elsewhere) repaints.
 */

import type { UlakSkin } from '@ulak/shared/skin'
...
const BACKEND_THEMES_KEY = 'ulak-desktop-backend-themes-v1'
 ...
 */
export function ingestBackendSkin(skin: UlakSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as UlakSkin)
