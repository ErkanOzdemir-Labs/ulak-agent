...
const SKIN_KEY = 'ulak-desktop-theme-v2'
const MODE_KEY = 'ulak-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'ulak-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'ulak-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'ulak-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.ulakTheme = skinName
  root.dataset.ulakMode = rendered
  ...
  window.ulakDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('ulak-boot-background', chromeBg)
    window.localStorage.setItem('ulak-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.ulakThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.ulakDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Ulak authoring/activating a skin from a
