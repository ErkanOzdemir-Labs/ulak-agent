...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Ulak doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in ulak_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `ulak model` uses — instead of
...
export type UlakConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `ulak model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// non-desktop) Ulak simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (ulak.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors ulak_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Ulak, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with Ulak but install on demand. */
...
export interface ToolProvider {
   ...
   *  CLI `ulak tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Ulak); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** ulak-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** ulak-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/ulak/update/check` — the backend's own update state.
