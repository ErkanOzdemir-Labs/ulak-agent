import { JsonRpcGatewayClient } from '@ulak/shared'

import type { UlakApiRequest } from '@/global'
...
// "Timed out connecting to Ulak backend" that hangs the UI (#48504).
//
...
export class UlakGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Ulak gateway connection closed',
      connectErrorMessage: 'Could not connect to Ulak gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Ulak gateway is not connected',
...
// process to route them to that host (ulak:api's registry branch). Null
...
export function ulakApi<T>(request: UlakApiRequest): Promise<T> {
  return window.ulakDesktop.api<T>({ ...connectionScoped(), ...request })
}
