...
import { capabilityScoped, getApiRequestConnection, type ProfileScope, profileScoped, ulakApi } from './client'
...
export async function listSessions(
  ...
  const result = await ulakApi<PaginatedSessions>({
    ...profileScoped(),
    path:
...
export async function listSidebarSessions(req: SidebarSessionsRequest): Promise<SidebarSessionsResponse> {
  ...
  try {
    result = await ulakApi<SidebarSessionsResponse>({
      ...profileScoped(),
...
export function searchSessions(query: string): Promise<SessionSearchResponse> {
  return ulakApi<SessionSearchResponse>({
...
export function getSessionMessages(
  ...
  return ulakApi<SessionMessagesResponse>({
    ...sessionScope,
...
export function renameSession(
  ...
  return ulakApi<{ ok: boolean; title: string }>({
    ...(profile ? { profile } : {}),
    path: `/api/sessions/${encodeURIComponent(id)}`,
    method: 'PATCH',
