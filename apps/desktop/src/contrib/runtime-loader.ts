 ...
 *   (`@ulak/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default UlakPlugin -> register(ctx)
 *
 ...
 * and the two on-disk doors — `<ulak home>/desktop-plugins/<name>/plugin.js`
 * and the unified agent-plugin half `<ulak home>/plugins/<name>/desktop/
...
import { createPluginContext, type UlakPlugin } from './plugin'
...
interface LoadOptions {
   ...
   *  unified agent-plugin root sets this so `~/.ulak/plugins` keeps its
...
/** Rewrite ONLY mapped import specifiers (@ulak/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default UlakPlugin export`)
    }

    // A disk/runtime copy of a plugin that now ships BUNDLED (e.g. a
    // standalone install of ulak-bots predating its adoption in-tree) must
...
//  - `<ulak home>/desktop-plugins/<name>/plugin.js` — the standalone door
//    (agent- or user-written desktop-only plugins);
//  - `<ulak home>/plugins/<name>/desktop/plugin.js` — the desktop HALF of a
 ...
 *  backend's ulak_home — #66899). `agentPluginsRoot` is optional: older
...
async function diskRoots(): Promise<DiskRoot[]> {
  ...
  if (unified) {
    // Opt-in by default: `~/.ulak/plugins` is installed-but-inert until the
...
async function readPluginSourceText(file: string): Promise<string> {
  ...
  if (result.truncated) {
    throw new PluginSourceOversizeError(
      "plugin.js exceeds this shell's 512 KiB read limit — update Ulak Desktop to load larger plugins"
    )
  }
