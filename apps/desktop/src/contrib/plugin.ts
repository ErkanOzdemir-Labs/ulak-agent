 ...
 * `UlakPlugin`; it never touches the registry directly — it receives a
 ...
 *  under `ulak.plugin.<id>.` — plugins can't read or clobber each other. */
 ...
 *  in one attributed namespace instead of the raw `window.ulakDesktop`
...
export interface PluginOs {
   ...
   *  the user is away from Ulak — use `host.notify` for the in-app toast.
   *  Throttled per plugin; reserve it for genuinely notable events.
   *  Supports `icon`, `activate` (e.g. `ulak://index-network/intent/1`),
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `ulak.plugin.${pluginId}.${key}`
...
function createPluginOs(pluginId: string): PluginOs {
  const attempt = async (run: (bridge: NonNullable<typeof window.ulakDesktop>) => Promise<boolean>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.ulakDesktop
  ...
  const attemptPath = async (run: (bridge: NonNullable<typeof window.ulakDesktop>) => Promise<null | string>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.ulakDesktop
