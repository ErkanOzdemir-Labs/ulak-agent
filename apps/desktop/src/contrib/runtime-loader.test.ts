...
import type { UlakReadDirResult } from '@/global'
...
// getStatus would supply the connected backend's ulak_home — a REMOTE path in
// remote mode. The disk scanner must NOT derive the plugin root from it (#66899).
const getStatus = vi.fn(async () => ({ ulak_home: '/remote/box/.ulak' }))

vi.mock('@/ulak', async importActual => ({
  ...(await importActual<typeof UlakModule>()),
  getStatus: () => getStatus()
}))

const desktopPluginsRoot = vi.fn<() => Promise<string>>()
const agentPluginsRoot = vi.fn<() => Promise<string>>()
const readDir = vi.fn<(path: string) => Promise<UlakReadDirResult>>()
...
beforeEach(() => {
  ...
  getStatus.mockClear()
  ;(window as unknown as { ulakDesktop: unknown }).ulakDesktop = {
    agentPluginsRoot,
    desktopPluginsRoot,
...
afterEach(() => {
  delete (window as unknown as { ulakDesktop?: unknown }).ulakDesktop
})

describe('scanDiskPlugins (#66899)', () => {
  it('scans the Electron-resolved local roots, never the backend ulak_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.ulak/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('/local/.ulak/plugins')
    readDir.mockResolvedValue({ entries: [] })

    await discoverRuntimePlugins()

    expect(desktopPluginsRoot).toHaveBeenCalled()
    expect(readDir).toHaveBeenCalledWith('/local/.ulak/desktop-plugins')
    expect(readDir).toHaveBeenCalledWith('/local/.ulak/plugins')
    // The remote backend's ulak_home must never feed the local plugin scan.
    expect(getStatus).not.toHaveBeenCalled()
    expect(readDir).not.toHaveBeenCalledWith('/remote/box/.ulak/desktop-plugins')
  })
  ...
  it('treats a package without a Desktop half as metadata, not a throwing file read', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.ulak/plugins') {
        return { entries: [{ isDirectory: true, name: 'my-feature', path: '/local/.ulak/plugins/my-feature' }] }
      }

      if (dir === '/local/.ulak/plugins/my-feature') {
        return {
          entries: [{ isDirectory: false, name: 'plugin.yaml', path: '/local/.ulak/plugins/my-feature/plugin.yaml' }]
        }
      }
    ...
    expect(readDir).toHaveBeenCalledWith('/local/.ulak/plugins/my-feature')
    expect(readDir).not.toHaveBeenCalledWith('/local/.ulak/plugins/my-feature/desktop')
  ...
  it('a DIRECTORY named plugin.js is not a plugin entry (metadata walk rejects it)', async () => {
    ...
    agentPluginsRoot.mockResolvedValue('')
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.ulak/desktop-plugins') {
        return { entries: [{ isDirectory: true, name: 'odd', path: '/local/.ulak/desktop-plugins/odd' }] }
      }

      if (dir === '/local/.ulak/desktop-plugins/odd') {
        // A folder literally named plugin.js — must resolve to "no entry".
        return {
          entries: [{ isDirectory: true, name: 'plugin.js', path: '/local/.ulak/desktop-plugins/odd/plugin.js' }]
        }
      }
  ...
  it('still scans the standalone root when agentPluginsRoot is absent (older shell)', async () => {
    delete (window.ulakDesktop as unknown as { agentPluginsRoot?: unknown }).agentPluginsRoot
    desktopPluginsRoot.mockResolvedValue('/local/.ulak/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })
  ...
  it('loads a unified desktop half OPT-IN: inventoried but not activated by default', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.ulak/plugins') {
        return { entries: [{ isDirectory: true, name: 'uni', path: '/local/.ulak/plugins/uni' }] }
      }

      if (dir === '/local/.ulak/plugins/uni') {
        return {
          entries: [{ isDirectory: true, name: 'desktop', path: '/local/.ulak/plugins/uni/desktop' }]
        }
      }

      if (dir === '/local/.ulak/plugins/uni/desktop') {
        return {
            ...
            ? [
                {
                  isDirectory: false,
                  name: 'plugin.js',
                  path: '/local/.ulak/plugins/uni/desktop/plugin.js'
                }
              ]
            : []
        }
    ...
    try {
      ...
      // ~/.ulak/plugins stays installed-but-inert until the user toggles it.
...
describe('watchRuntimePlugins dir watch (#66899)', () => {
  it('watches both Electron-resolved local roots, never the backend ulak_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.ulak/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('/local/.ulak/plugins')
    readDir.mockResolvedValue({ entries: [] })
    ...
    expect(watchDirectory).toHaveBeenCalledWith('/local/.ulak/desktop-plugins')
    expect(watchDirectory).toHaveBeenCalledWith('/local/.ulak/plugins')
    expect(watchDirectory).not.toHaveBeenCalledWith('/remote/box/.ulak/desktop-plugins')
    expect(getStatus).not.toHaveBeenCalled()
  })
})
...
describe('plugin source reads (512 KiB preview-cap bug)', () => {
  ...
  const standaloneRootWith = (name: string) => {
    const folder = `/local/.ulak/desktop-plugins/${name}`
  ...
  it('loads the full source via readPluginSource when the shell offers it', async () => {
    ;(window.ulakDesktop as unknown as { readPluginSource: unknown }).readPluginSource = readPluginSource
    desktopPluginsRoot.mockResolvedValue('/local/.ulak/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('')
    ...
    try {
      ...
      expect(readPluginSource).toHaveBeenCalledWith('/local/.ulak/desktop-plugins/big/plugin.js')
      expect(register).toHaveBeenCalledTimes(1)
  ...
  it('older shell without readPluginSource: a truncated preview read fails LOUDLY, never evaluates', async () => {
    ...
    try {
      ...
      expect($pluginRecords.get().huge).toMatchObject({
        kind: 'disk',
        status: 'error',
        file: '/local/.ulak/desktop-plugins/huge/plugin.js'
      })
...
describe('bundled-shadowed disk copies', () => {
  it('skips a disk copy of a bundled plugin but publishes a visible inventory row', async () => {
    ...
    publishPlugin({ id: 'ulak-bots', name: 'Bot Mode', kind: 'bundled', status: 'loaded' })
    ...
    try {
      const id = await loadRuntimePlugin(
        'export default { id: "ulak-bots", name: "Bot Mode", register() {} }',
        'ulak-bots',
        { file: '/local/.ulak/desktop-plugins/ulak-bots/plugin.js' }
      )

      // Skipped — the bundled copy stays the only live registration...
      expect(id).toBeNull()
      expect($pluginRecords.get()['ulak-bots']).toMatchObject({ kind: 'bundled', status: 'loaded' })
      ...
      expect($pluginRecords.get()['ulak-bots:disk-shadowed']).toMatchObject({
        kind: 'disk',
        status: 'disabled',
        file: '/local/.ulak/desktop-plugins/ulak-bots/plugin.js'
      })
