/**
 * @ulak/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__ULAK_PLUGIN_SDK__` and maps the import to it, so a
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend Ulak profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.ulakDesktop?.getAgentRoster
...
async function pluginRouteStillRegistered(route: PluginProfileRoute): Promise<boolean> {
  const getProfileRoutes = window.ulakDesktop?.getProfileRoutes
...
//
// ensureGatewayProfile awaits the store's dial, and UlakGateway.connect() has
...
export const host = {
  ...
  deleteProfile: async (profile: string | PluginProfileRoute): Promise<void> => {
    ...
    // restores on relaunch and re-creates the deleted profile (ulak-agent#94235).
    dropTilesForProfile(
  ...
  connections: async () => {
    const bridge = window.ulakDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Ulak Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.ulakDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Ulak Desktop.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update Ulak Desktop to open another Bot chat.' })
   ...
   *
   *  `isStaleTile` (ulak-agent#90102): the caller's reconciliation probe
  ...
  profileRoutes: async () => {
    ...
    if (!getProfileRoutes) {
      throw new Error('Ulak Desktop connection routing unavailable')
    }
  ...
  listPersistedSessions: async (
    ...
    return ulakApi<PaginatedSessions>({
  ...
  setPersistedSessionHidden: async (
    ...
    return ulakApi<{ ok: boolean; hidden: boolean }>({
      ...(route ? { connectionId: route.connectionId } : {}),
 ...
 *  "ULAK AGENT" with it, a `chat.empty` contribution writes its own name. */
