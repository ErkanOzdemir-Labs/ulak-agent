...
vi.mock('@/ulak', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn(), ulakApi: vi.fn() }))
...
const { deleteProfile, ulakApi } = await import('@/ulak')
...
describe('connection-aware plugin host APIs', () => {
  it('retires a profile gateway before deleting it', async () => {
    ...
    // profile's backend, re-creating its ULAK_HOME (#94235).
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { ulakDesktop: unknown }).ulakDesktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(ulakApi).toHaveBeenNthCalledWith(
      1,
    ...
    expect(ulakApi).toHaveBeenNthCalledWith(2, {
      connectionId: 'source-a',
      path: '/api/sessions/bot-chat',
      method: 'PATCH',
      body: { hidden: true, profile: 'backend-worker' }
    })
    expect(vi.mocked(ulakApi).mock.calls.every(([request]) => !('profile' in request))).toBe(true)
