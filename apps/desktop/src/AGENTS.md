...
## The desktop is its own chat surface on a `ulak serve` backend
...
(`@ulak/shared`: `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard also consumes.
The desktop has **no build/runtime dependency on the dashboard frontend**: it spawns a headless
`ulak serve` (`headless_backend=True` → `cmd_dashboard` skips `_build_web_ui` and exports
`ULAK_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists).
`dashboard` and `serve` share `cmd_dashboard`/`start_server` but neither launches the other. It does
NOT embed `ulak --tui` — own composer, transcript, slash pipeline.
...
and ONLY when it does not (older managed install / PATH `ulak` not yet updated) rewrites argv to
...
## Bot Mode (`src/plugins/ulak-bots/`) — one bot = ONE canonical forever-chat, identified by NAME

Each bot is a Ulak **profile** with a persistent identity. This invariant regressed repeatedly,
...
**There is NO session-id pin.** The old design stored a pointer in `ui_meta['ulak-bots'].chat`;
...
in `src/plugins/ulak-bots/`: `canonical-chat-registry.test.ts` (tripwire: the open path never
