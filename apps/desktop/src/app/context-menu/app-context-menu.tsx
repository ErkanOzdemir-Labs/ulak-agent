...
import { ULAK_CONTEXT_MENU_TRIGGER_ATTR } from '@/components/ui/context-menu'
    ...
    [
      ...
      terminal.paste ? (
        <Item
          ...
          onSelect={() =>
            void window.ulakDesktop?.readClipboard().then(text => (text ? terminal.paste?.(text) : undefined))
          }
        />
      ) : null,
      <Item
...
function domSections(open: Extract<OpenContextMenu, { kind: 'dom' }>, t: Translations): ReactNode[][] {
  ...
  const editableCommand = (command: 'copy' | 'cut' | 'paste') => {
    withEditableFocus(() => void window.ulakDesktop?.contextMenuEdit?.(command))
  }
  ...
  const spellcheckAction = (action: { kind: 'add' | 'replace'; word: string }) => {
    withEditableFocus(() => void window.ulakDesktop?.contextMenuSpellcheck?.(action))
  }
    ...
    sections.push(
      [
        ...
        <Item
          ...
          onSelect={() => void window.ulakDesktop?.contextMenuCopyImage?.()}
        />,
        target.imageUrl ? (
          <Item
            icon="copy"
        ...
        target.imageUrl ? (
          <Item
            ...
            onSelect={() => void window.ulakDesktop?.saveImageFromUrl?.(target.imageUrl)}
          />
        ) : null
      ].filter(Boolean)
    )
      ...
      [
        ...
        imageUrl ? (
          <Item
            ...
            label={copy.image.saveImageAs}
            onSelect={() => void window.ulakDesktop?.saveImageFromUrl?.(imageUrl)}
          />
        ) : null
      ].filter(Boolean)
    )
  ...
  return [
    ...
    [
      <Item
        icon="cloud-download"
        key="shell-update"
        label={t.commandCenter.updateUlak}
...
export function AppContextMenu() {
  ...
  useEffect(() => {
    ...
    const onContextMenu = (event: MouseEvent) => {
      ...
      if (element?.closest(`[${ULAK_CONTEXT_MENU_TRIGGER_ATTR}], [data-slot="context-menu-trigger"]`)) {
        return
      }
  ...
  useEffect(() => window.ulakDesktop?.onContextMenuSpellcheck?.(augmentSpellcheck), [])
