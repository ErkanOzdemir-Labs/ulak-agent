...
import { getLatestSessionMessages } from '@/ulak'
...
import { useUlakConfigRecord } from '../hooks/use-config-record'
...
import { useUlakConfig } from '../session/hooks/use-ulak-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshUlakConfig, sttEnabled, voiceMaxRecordingSeconds } = useUlakConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('ulak:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('ulak:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshUlakConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshUlakConfig])
  ...
  const configRecord = useUlakConfigRecord()
