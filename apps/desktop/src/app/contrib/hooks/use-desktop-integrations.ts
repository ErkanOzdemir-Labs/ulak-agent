...
import { pathFromUlakDeepLink, resolveUlakOpenPath } from '@/lib/ulak-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    // "About Ulak" — it is the OS-standard affordance for updating THIS app,
    ...
    const unsubscribe = window.ulakDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow('client'))
  ...
  useEffect(() => {
    window.ulakDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.ulakDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.ulakDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // ulak://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.ulakDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future ulakDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveUlakOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.ulakDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (ulak://index-network/intent/1, ulak://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromUlakDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.ulakDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.ulakDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.ulakDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.ulakDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
