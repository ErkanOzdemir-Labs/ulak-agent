...
import { getUlakConfigRecord, saveMcpServers } from '@/ulak'

import { setUlakConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `ulak://mcp/install` deep links. The payload is
 ...
 * confirming lets Ulak spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getUlakConfigRecord())
      ...
      setUlakConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
