...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshUlakConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshUlakConfig — two REST calls (config + defaults) per event, per
