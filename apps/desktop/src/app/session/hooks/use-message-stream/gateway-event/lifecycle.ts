...
export function handleLifecycleEvent(ctx: GatewayEventContext): boolean {
  ...
  if (event.type === 'gateway.ready') {
    ...
    ingestBackendSkin((payload as { skin?: UlakSkin } | undefined)?.skin, { apply: false })
  ...
  if (event.type === 'skin.changed') {
    // A runtime skin switch (Ulak activating an authored skin, or `/skin`
    // on another surface). Only the active source+profile's change repaints.
    if (fromActiveSource()) {
      ingestBackendSkin(payload as UlakSkin | undefined, { apply: true })
    }
