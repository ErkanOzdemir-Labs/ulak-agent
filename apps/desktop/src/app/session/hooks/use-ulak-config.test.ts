...
import { useUlakConfig } from './use-ulak-config'

vi.mock('@/ulak', () => ({
  getUlakConfig: vi.fn(),
  getUlakConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'ulak.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getUlakConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getUlakConfig>>)

describe('useUlakConfig refreshUlakConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to Ulak' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useUlakConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshUlakConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useUlakConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getUlakConfig>>>()
    vi.mocked(getUlakConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useUlakConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshUlakConfig(true)
    })
    expect(getUlakConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getUlakConfig>>)
  ...
  it('does not publish config after its switch loses ownership', async () => {
    const staleConfig = deferred<Awaited<ReturnType<typeof getUlakConfig>>>()
    vi.mocked(getUlakConfig).mockReturnValueOnce(staleConfig.promise)
    const { result } = renderHook(() => useUlakConfig({ activeSessionIdRef: { current: null } }))
    let ownsSwitch = true

    let refresh!: Promise<void>
    act(() => {
      refresh = result.current.refreshUlakConfig(false, () => ownsSwitch)
    })
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getUlakConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getUlakConfig>>>()
    vi.mocked(getUlakConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshUlakConfig(true)
      refreshC = result.current.refreshUlakConfig(true)
    })
