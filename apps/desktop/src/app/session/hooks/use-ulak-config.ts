...
import { getUlakConfig, getUlakConfigDefaults } from '@/ulak'
...
interface UlakConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useUlakConfig({ activeSessionIdRef }: UlakConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshUlakConfig = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getUlakConfig(), getUlakConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Ulak' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshUlakConfig, sttEnabled, voiceMaxRecordingSeconds }
}
