        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.ulakDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const annotateGuest = useCallback((): null | PreviewAnnotateGuest => {
    ...
    return {
      capture: async rect => {
        ...
        const dataUrl = await window.ulakDesktop.capturePreview?.({ rect, viewport, webContentsId })
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Ulak to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.ulakDesktop?.watchPreviewFile ||
      !window.ulakDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.ulakDesktop.onPreviewFileChanged(payload => {
    ...
    void window.ulakDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.ulakDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.ulakDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:ulak-preview')
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.ulakDesktop?.zoom?.factor?.() || 1
      ...
      openGuestContextMenu(
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.ulakDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.ulakDesktop?.contextMenuCopyImage?.(),
          ...
          <PreviewBrowserBar
            ...
            onOpenExternal={
              !isBrowserWindow() && !canOpenBrowserWindow()
                ? () => void window.ulakDesktop?.openExternal(currentUrl)
                : undefined
            }
