...
const preamble = () => `  var w = window;
  ...
  w.__ulakActHolder = w.__ulakActHolder || {};
  w.__ulakAct = ${actEngineSource()};
  w.__ulakWatch_fn = (${watchInPage.toString()});
  w.__ulakWatchTag = ${WATCH_TAG};
  var holder = w.__ulakActHolder;
  var act = function (a) { return w.__ulakAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__ulakWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__ulakHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'ULAK-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
