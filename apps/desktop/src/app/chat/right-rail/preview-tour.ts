...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__ulakTourEngine) {
    ${driverIife}
    w.__ulakTourHolder = {};
    w.__ulakTourCollect = (${collectTourTargets.toString()});
    w.__ulakTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__ulak-tour-style')) {
    var style = document.createElement('style');
    style.id = '__ulak-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__ulakTourEngine(
    w.driver.js.driver,
    w.__ulakTourHolder,
