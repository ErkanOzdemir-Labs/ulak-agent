...
import type { UlakReadDirEntry, UlakReadDirResult } from '@/global'
...
export type ProjectTreeEntry = UlakReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: UlakReadDirEntry) {
...
async function filterIgnored(entries: UlakReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<UlakReadDirResult> {
  if (!window.ulakDesktop) {
