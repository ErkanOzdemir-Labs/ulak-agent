...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { ulakDesktop: { readDir: typeof readDir } }).ulakDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): UlakReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { ulakDesktop: unknown }).ulakDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: UlakReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<UlakReadDirResult>(resolve => {
  ...
  it('discards a stale live refresh after the active registered connection changes', async () => {
    let resolveRefreshFromA: ((result: UlakReadDirResult) => void) | undefined
  ...
  it('discards a stale child read after the active registered connection changes', async () => {
    let resolveChildFromA: ((result: UlakReadDirResult) => void) | undefined
  ...
  it('discards a stale root read after the active registered connection changes', async () => {
    let resolveFirst: ((result: UlakReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<UlakReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.ulakDesktop is missing', async () => {
    delete (window as unknown as { ulakDesktop?: unknown }).ulakDesktop
  ...
  it('keeps an unreadable root on screen while it re-probes', async () => {
    ...
    let releaseProbe: ((value: UlakReadDirResult) => void) | undefined
  ...
  it('keeps loaded rows on screen while the root refreshes', async () => {
    ...
    let releaseRefresh: ((value: UlakReadDirResult) => void) | undefined
  ...
  it('clears the rows when the same path is re-read from another backend', async () => {
    ...
    let releaseFromB: ((value: UlakReadDirResult) => void) | undefined
