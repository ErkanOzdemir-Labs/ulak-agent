...
const ULAK_PATHS_MIME = 'application/x-ulak-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(ULAK_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(ULAK_PATHS_MIME)
  ...
  const getPath = window.ulakDesktop?.getPathForFile
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.ulakDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (ulak --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.ulakDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[ulak-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.ulakDesktop?.terminal?.write(sessionId, `${command}\r`)
