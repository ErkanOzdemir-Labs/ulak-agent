...
import { ulakConfigCacheWriter, invalidateUlakConfig, useUlakConfigRecord } from '../hooks/use-config-record'
...
// backend now surfaces the full `ulak model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in ulak_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged, scopeProfile }: ModelSettingsProps) {
  ...
  const { data: config } = useUlakConfigRecord(scopeProfile)
  const setConfig = useMemo(() => ulakConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const refresh = useCallback(
    async ({ replaceSelection = false }: { replaceSelection?: boolean } = {}) => {
      ...
      try {
        ...
        void invalidateUlakConfig(scopeProfile)
      } catch (err) {
        if (profileEpoch.current === epoch) {
  ...
  const writeAgentDefault = useCallback(
    async (key: string, value: string) => {
      ...
      try {
        await saveUlakConfig(next, scopeProfile)
      } catch (err) {
        setConfig(prev)
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `ulak model` curation). Best-effort — fall through to the refreshed
  ...
  const recycleStaleBackend = useCallback(async () => {
    ...
    try {
      await window.ulakDesktop?.recycleBackend?.(scopeProfile)
        ...
        {needsSetup && !setupIsApiKey && selectedProviderRow && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Ulak runs the flow for you.`}
          </p>
        )}
