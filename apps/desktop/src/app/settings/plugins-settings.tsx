...
// User-installed plugins first — mirrors `ulak plugins list --user`.
...
function reveal(file: string) {
  void window.ulakDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // ulak_home breaks against a remote backend (#66899).
    const dir = await window.ulakDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.ulakDesktop?.openDir?.(dir)
...
// Agent plugins live under the BACKEND's ulak home (profile-aware), so the
// path comes from the gateway — not from the renderer's local ULAK_HOME.
...
async function revealAgentPluginsDir(request: GatewayRequest) {
  try {
    ...
    const opened = await window.ulakDesktop?.openDir?.(`${home}/plugins`)
            ...
            <SelectContent>
              {profiles.map(profile => (
                <SelectItem key={profile.name} value={profile.name}>
                  {profile.is_default ? 'Ulak (default)' : profile.name}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
