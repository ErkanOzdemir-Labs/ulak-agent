...
vi.mock('@/ulak', () => ({
  ...
  saveUlakConfig: (config: unknown, profile?: string) => saveUlakConfig(config, profile),
...
beforeEach(() => {
  ...
  getUlakConfigSchema.mockResolvedValue({ fields: {} })
...
describe('ConfigSettings autosave', () => {
  it('sends a later revert instead of diffing it away against the stale page-load baseline', async () => {
    getUlakConfigRecord.mockResolvedValue({ checkpoints: { enabled: false }, other: 'untouched' })
    ...
    try {
      ...
      await waitFor(() => expect(saveUlakConfig).toHaveBeenCalledTimes(1))
      expect(saveUlakConfig.mock.calls[0][0]).toEqual({ checkpoints: { enabled: true } })

      // Revert: flip it back to its original value and let autosave fire again.
      toggle.click()
      await vi.advanceTimersByTimeAsync(700)

      await waitFor(() => expect(saveUlakConfig).toHaveBeenCalledTimes(2))
      ...
      expect(saveUlakConfig.mock.calls[1][0]).toEqual({ checkpoints: { enabled: false } })
    } finally {
      vi.useRealTimers()
    }
  })
