...
import { disconnectOAuthProvider, listOAuthProviders } from '@/ulak'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/ulak'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.ulakDesktop?.terminal)
...
//      in ulak_cli/provider_catalog.py) — the SAME provider identity
//      `ulak model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Ulak can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Ulak can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Ulak never deletes creds another tool owns behind a silent API call.
