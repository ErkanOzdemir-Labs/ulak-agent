 ...
 * the default browser's profile (cookies/logins) into a Ulak-owned copy;
...
export function BrowserRealProfilePanel({ profile }: BrowserRealProfilePanelProps) {
  ...
  const { data: config } = useUlakConfigRecord(profile)
  ...
  const toggle = useCallback(
    async (on: boolean) => {
      ...
      try {
        await saveUlakConfigRecord(next, profile)
        notify({
          kind: 'info',
