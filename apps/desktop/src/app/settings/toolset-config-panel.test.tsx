...
import type { ToolsetConfig } from '@/types/ulak'
...
const getUlakConfigSchema = vi.fn()
...
vi.mock('@/ulak', () => ({
  ...
  getUlakConfigSchema: () => getUlakConfigSchema(),
...
beforeEach(() => {
  ...
  getUlakConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getUlakConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveUlakConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveUlakConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
  ...
  describe('managed Nous provider activation', () => {
    ...
    it('drives the existing Nous OAuth device-code flow from the sign-in action and refetches', async () => {
      ...
      startOAuthLogin.mockResolvedValue({
        ...
        session_id: 'sess-1',
        user_code: 'NOUS-1234',
        verification_url: 'https://portal.github.com/ErkanOzdemir-Labs/device?user_code=NOUS-1234',
        poll_interval: 5,
        expires_in: 600
      })
