...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: UlakConfigRecord = { display: { theme: 'mono' } }
  ...
  describe('providerGroup', () => {
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // ULAK_QWEN_ shares the ULAK_ stem with other integrations.
      expect(providerGroup('ULAK_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
  ...
  describe('enumOptionsFor — backend selector dropdowns', () => {
    ...
    it('surfaces user-defined command-type TTS providers (canonical providers nesting + legacy)', () => {
      const withCustom: UlakConfigRecord = {
        tts: {
          provider: 'neutts',
    ...
    it('never offers a built-in name as a command provider, even one absent from the dropdown list', () => {
      const shadowing: UlakConfigRecord = {
        tts: {
          provider: 'edge',
          providers: {
  ...
  describe('sectionFieldEntries', () => {
    it('renders memory.provider from config even when the backend schema omits it', () => {
      ...
      const config: UlakConfigRecord = { memory: { memory_enabled: true, provider: '' } }
    ...
    it('infers the field type from the config value when the schema omits the key', () => {
      const config: UlakConfigRecord = { memory: { provider: '', memory_enabled: true, memory_char_limit: 2200 } }
    ...
    it('prefers the backend schema entry over inference when both exist', () => {
      ...
      const config: UlakConfigRecord = { memory: { provider: 'honcho' } }
  ...
  describe('clearsEnabledToolsets', () => {
    it('flags a non-empty → empty transition', () => {
      const prev: UlakConfigRecord = { toolsets: ['memory', 'terminal', 'web_search'] }
      const next: UlakConfigRecord = { toolsets: [] }
    ...
    it('does not flag a non-empty → missing transition (deep-merge preserves the key)', () => {
      ...
      const prev: UlakConfigRecord = { toolsets: ['memory'] }
      const next: UlakConfigRecord = {}

      expect(clearsEnabledToolsets(prev, next)).toBe(false)
    })

    it('does not flag when at least one toolset remains', () => {
      const prev: UlakConfigRecord = { toolsets: ['memory', 'terminal'] }
      const next: UlakConfigRecord = { toolsets: ['memory'] }

      expect(clearsEnabledToolsets(prev, next)).toBe(false)
    })

    it('does not flag when the list was already empty', () => {
      const prev: UlakConfigRecord = { toolsets: [] }
      const next: UlakConfigRecord = { toolsets: [] }
    ...
    it('does not flag an unrelated edit that never touched toolsets', () => {
      const prev: UlakConfigRecord = { model: 'a', toolsets: ['memory'] }
      const next: UlakConfigRecord = { model: 'b', toolsets: ['memory'] }
  ...
  describe('diffConfig', () => {
    it('omits a top-level key the draft never touched', () => {
      ...
      // an agent set via `ulak config set` while the page sat open must not
      // come back in the patch just because it's still present in the draft.
      const baseline: UlakConfigRecord = { fallback_providers: ['nara1'], timezone: 'UTC' }
      const draft: UlakConfigRecord = { fallback_providers: ['nara1'], timezone: 'America/New_York' }
    ...
    it('includes a nested key only when it actually changed, leaving siblings out', () => {
      const baseline: UlakConfigRecord = { display: { personality: 'default', show_reasoning: true } }
      const draft: UlakConfigRecord = { display: { personality: 'default', show_reasoning: false } }
    ...
    it('sends a new key that was absent from the baseline', () => {
      const baseline: UlakConfigRecord = {}
      const draft: UlakConfigRecord = { timezone: 'UTC' }
    ...
    it('returns an empty object when the draft matches the baseline exactly', () => {
      const baseline: UlakConfigRecord = { toolsets: ['memory'], display: { personality: 'default' } }
      const draft: UlakConfigRecord = { toolsets: ['memory'], display: { personality: 'default' } }

      expect(diffConfig(baseline, draft)).toEqual({})
    })

    it('treats an array as a whole value, not diffed element by element', () => {
      const baseline: UlakConfigRecord = { toolsets: ['memory', 'terminal'] }
      const draft: UlakConfigRecord = { toolsets: ['memory'] }
