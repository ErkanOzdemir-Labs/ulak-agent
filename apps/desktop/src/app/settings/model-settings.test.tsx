...
const getUlakConfigRecord = vi.fn()
const saveUlakConfig = vi.fn()
...
vi.mock('@/ulak', () => ({
  ...
  getUlakConfigRecord: () => getUlakConfigRecord(),
  saveUlakConfig: (config: unknown) => saveUlakConfig(config),
...
beforeEach(() => {
  getGlobalModelInfo.mockResolvedValue({ provider: 'nous', model: 'ulak-4' })
  getGlobalModelOptions.mockResolvedValue({
    providers: [
      {
        name: 'Nous',
        slug: 'nous',
        models: ['ulak-4', 'ulak-4-mini'],
        authenticated: true,
        capabilities: { 'ulak-4': { reasoning: true, fast: true } }
      }
    ]
  })
  getAuxiliaryModels.mockResolvedValue({
    main: { provider: 'nous', model: 'ulak-4' },
    tasks: [{ task: 'vision', provider: 'auto', model: '', base_url: '' }]
  })
  getMoaModels.mockResolvedValue(null)
  setModelAssignment.mockResolvedValue({ ok: true, provider: 'nous', model: 'ulak-4', gateway_tools: [] })
  getRecommendedDefaultModel.mockResolvedValue({ provider: 'nous', model: 'ulak-4', free_tier: null })
  setEnvVar.mockResolvedValue({ ok: true })
  getUlakConfigRecord.mockResolvedValue({ agent: { reasoning_effort: 'medium', service_tier: 'normal' } })
  saveUlakConfig.mockResolvedValue({ ok: true })
})
...
describe('ModelSettings', () => {
  ...
  it('replaces the selected provider and model when the active profile changes', async () => {
    getGlobalModelInfo
      ...
      .mockResolvedValueOnce({ provider: 'nous', model: 'ulak-4' })
  ...
  it('writes the profile default speed (service_tier) when the fast switch is toggled', async () => {
    await renderModelSettings()
    await waitFor(() => expect(getUlakConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() =>
      expect(saveUlakConfig).toHaveBeenCalledWith(
      ...
      providers: [
        {
          ...
          models: ['ulak-4'],
          authenticated: true,
          capabilities: { 'ulak-4': { reasoning: false, fast: false } }
        }
      ]
    })
  ...
  it('warns when a main switch leaves auxiliary tasks pinned to another provider', async () => {
    setModelAssignment.mockResolvedValueOnce({
      ...
      gateway_tools: [],
      stale_aux: [{ task: 'compression', provider: 'nous', model: 'ulak-4' }]
    })
...
describe('ModelSettings MoA preset editor', () => {
  ...
  it('does not clear the model or save when the same provider is re-selected', async () => {
    ...
    try {
      ...
      expect(saveMoaModels).not.toHaveBeenCalled()
      expect(screen.getByText('nous · ulak-4')).toBeTruthy()
    } finally {
      vi.useRealTimers()
    }
  })
        ...
        expect.objectContaining({
          presets: expect.objectContaining({
            default: expect.objectContaining({
              reference_models: [
                expect.objectContaining({ provider: 'nous', model: 'ulak-4', enabled: false }),
...
describe('ModelSettings code-skew 503', () => {
  const skewError = new Error(
    'Error invoking remote method \'ulak:api\': Error: 503: {"detail":"Restart required: This process is running code from 08b4875f4a but the checkout on disk is now 48d2528066. The model picker would risk a stale-module crash — restart the Desktop-owned backend to load the new code (use Restart backend in Ulak Desktop, or quit and reopen the app)"}'
  )
  ...
  it('unwraps the stale-backend 503 instead of dumping IPC JSON', async () => {
    ...
    expect(screen.queryByText(/ulak:api/)).toBeNull()
  ...
  it('recycles the Desktop-owned backend and reloads the catalog', async () => {
    ...
    ;(window as unknown as { ulakDesktop: { recycleBackend: typeof recycleBackend } }).ulakDesktop = {
      recycleBackend
    }
