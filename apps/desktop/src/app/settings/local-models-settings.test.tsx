...
import * as ulak from '@/ulak'

const mocked = vi.mocked(ulak)
...
describe('BrowseSection', () => {
  it('searches HF after a pause and shows fit-priced files on demand', async () => {
    ...
    try {
      vi.mocked(ulak.searchHFModels).mockResolvedValue({
        hits: [{ downloads: 872724, gated: false, likes: 47, repo: 'unsloth/Qwen3.8-27B-GGUF', updated: '2026-08-18' }]
      })
      vi.mocked(ulak.listHFRepoFiles).mockResolvedValue({
        files: [
      ...
      expect(ulak.searchHFModels).not.toHaveBeenCalled()
      await act(async () => {
        await vi.advanceTimersByTimeAsync(400)
      })
      expect(ulak.searchHFModels).toHaveBeenCalledWith('qwen')
      ...
      vi.mocked(ulak.downloadBrowsedModel).mockResolvedValue({ job_id: 'j1', model_id: 'Qwen3.8-27B-Q4_K_M' })
      fireEvent.click(q4Btn)
      await act(async () => {
        await vi.runOnlyPendingTimersAsync()
      })
      expect(ulak.downloadBrowsedModel).toHaveBeenCalledWith('unsloth/Qwen3.8-27B-GGUF', ['Qwen3.8-27B-Q4_K_M.gguf'])
    } finally {
      vi.useRealTimers()
    }
  })
...
describe('added-by-you rows', () => {
  it('staged models outside the catalog get the full action set', async () => {
    vi.mocked(ulak.getLocalModelsStatus).mockResolvedValue({
      ...BASE_STATUS,
      loaded_models: { 'Ulak-4.3-36B-Q5_K_M': 'loaded' },
      models: [{ id: 'Ulak-4.3-36B-Q5_K_M', size_bytes: 25 * 2 ** 30, size_label: '25.0 GB' }],
      placement: {
        'Ulak-4.3-36B-Q5_K_M': {
    ...
    vi.mocked(ulak.getLocalCatalog).mockResolvedValue({ models: [] })

    renderPane()
    await screen.findByText('Ulak-4.3-36B-Q5_K_M')
