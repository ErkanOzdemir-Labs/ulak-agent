...
describe('deriveBillingView', () => {
  it('derives the deployed-today shape with fail-open disabled charge controls', () => {
    ...
    expect(view.topupRow?.description).toBe(
      "Remote spending is off for this account — a billing admin can turn it on from the portal's Ulak Agent page."
    )
  ...
  it('derives the post-train shape with card provenance, presets, and denominated usage bars', () => {
    ...
    expect(view.plan?.link?.url).toBe('https://portal.github.com/ErkanOzdemir-Labs/manage-subscription?org_id=org_123')
  ...
  it('points divergent auto-refill cards at the portal for reconciliation', () => {
    ...
    expect(view.refillRow?.action).toEqual({
      label: 'Reconcile ↗',
      url: 'https://portal.github.com/ErkanOzdemir-Labs/billing'
    })
  })
  ...
  it('degrades safely when a divergent auto-refill card has no display details', () => {
    ...
    expect(view.refillRow?.action?.url).toBe('https://portal.github.com/ErkanOzdemir-Labs/billing')
  })
  ...
  it('keeps subscription unavailable as a plan-card degradation with a live portal link', () => {
    ...
    expect(view.plan?.link).toMatchObject({
      label: 'Adjust plan ↗',
      url: 'https://portal.github.com/ErkanOzdemir-Labs/manage-subscription'
    })
  })
...
describe('derivePlanCard (current-plan card)', () => {
  ...
  it('gives teams a portal escape hatch and no in-app button', () => {
    ...
    expect(view.plan?.link).toMatchObject({
      label: 'Adjust plan ↗',
      url: 'https://portal.github.com/ErkanOzdemir-Labs/manage-subscription?org_id=sid-5'
    })
  })
...
describe('derivePlanTiers (plans grid)', () => {
  it('marks the current tier, upgrades, and in-app downgrades for a subscriber', () => {
    ...
    expect(byName.Super).toMatchObject({
      action: {
        url: 'https://portal.github.com/ErkanOzdemir-Labs/manage-subscription?org_id=org_personal_plus&plan=cltier222super222personal'
      },
  ...
  it('marks the free/lowest tier current (inert) and every paid tier an upgrade when there is no subscription', () => {
    ...
    expect(byName.Plus).toMatchObject({
      action: {
        url: 'https://portal.github.com/ErkanOzdemir-Labs/manage-subscription?org_id=org_personal_free&plan=cltier111plus1111personal'
      }
    })
  })
...
describe('buildManageSubscriptionUrl', () => {
  it('mirrors the TUI manage-subscription URL construction', () => {
    expect(
      buildManageSubscriptionUrl({
        org_id: 'org_123',
        portal_url: 'https://portal.github.com/ErkanOzdemir-Labs/billing'
      })
    ).toBe('https://portal.github.com/ErkanOzdemir-Labs/manage-subscription?org_id=org_123')
  })

  it('appends plan=<tierId> when a tier is chosen', () => {
    expect(
      buildManageSubscriptionUrl(
        { org_id: 'org_123', portal_url: 'https://portal.github.com/ErkanOzdemir-Labs/billing' },
        null,
        'tier_abc'
      )
    ).toBe('https://portal.github.com/ErkanOzdemir-Labs/manage-subscription?org_id=org_123&plan=tier_abc')
  })

  it('omits the plan param when no tier is given', () => {
    expect(
      buildManageSubscriptionUrl({ org_id: 'org_123', portal_url: 'https://portal.github.com/ErkanOzdemir-Labs/billing' }, null)
    ).toBe('https://portal.github.com/ErkanOzdemir-Labs/manage-subscription?org_id=org_123')
  })
  ...
  it('applies org_id + plan to the hard-coded portal fallback when no portal_url resolves', () => {
    ...
    expect(buildManageSubscriptionUrl({ org_id: 'org_z', portal_url: null }, null, 'tier_q')).toBe(
      'https://portal.github.com/ErkanOzdemir-Labs/manage-subscription?org_id=org_z&plan=tier_q'
    )
  })
})
