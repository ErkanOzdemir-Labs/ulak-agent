  ...
  it('subscribes for verification, opens the verification URL, cleans up, and invalidates on completion', async () => {
    ...
    act(() => {
      gatewayMock.emit('billing.step_up.verification', {
        payload: {
          user_code: 'ABCD-1234',
          verification_url: 'https://portal.github.com/ErkanOzdemir-Labs/device'
        },
    ...
    expect(result.current.verification).toEqual({
      code: 'ABCD-1234',
      url: 'https://portal.github.com/ErkanOzdemir-Labs/device'
    })

    result.current.openVerification()
    expect(window.ulakDesktop?.openExternal).toHaveBeenCalledWith('https://portal.github.com/ErkanOzdemir-Labs/device')
