 ...
 * agent sources (local runtime + any number of remote gateways / Ulak Cloud
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.ulakDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.ulakDesktop.oauthLoginConnectionConfig(editorUrl)
  ...
  // Fan out `ulak update` to every eligible source; per-connection results
