...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<UlakConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useUlakConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => ulakConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  // never touched — possibly changed out-of-band by `ulak config set`
  // while this page sat open — is never resent with its stale value.
  const configBaselineRef = useRef<UlakConfigRecord | null>(null)
    ...
    const t = window.setTimeout(() => {
      ...
      saveQueueRef.current = saveQueueRef.current.then(async () => {
        try {
          ...
          const result = await saveUlakConfig(patch, scopeProfile)
  ...
  const applyConfig = (next: UlakConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: UlakConfigRecord) => {
