...
//
// Distinct from session/hooks/use-ulak-config.ts, which is side-effecting —
// it pushes personality/cwd/voice/… into the session stores for live chat.
export const ULAK_CONFIG_KEY = ['ulak-config-record'] as const
...
// share a cache row.
export const ulakConfigKey = (profile?: ProfileScope) =>
  profile == null ? ULAK_CONFIG_KEY : ([...ULAK_CONFIG_KEY, profileScopeKey(profile)] as const)
...
export const useUlakConfigRecord = (profile?: ProfileScope) =>
  useQuery({
    queryKey: ulakConfigKey(profile),
    ...
    queryFn: () => getUlakConfigRecord(profile ?? undefined),
    staleTime: 0
  })

// setUlakConfigCache writes the app-wide (base-key) record. Pass a profile to
