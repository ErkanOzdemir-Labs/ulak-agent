...
function setWorkspaceTransfer(transferring: boolean): void {
  window.ulakDesktop?.hud?.setWorkspaceTransfer?.(transferring)
}

function moveHud(state: PressState): void {
  window.ulakDesktop?.hud?.moveBy?.({
    width: state.originW,
...
function armGrab(state: PressState, workspaceTransfer: boolean): void {
  ...
  window.ulakDesktop?.hud?.beginMove?.()
}
 ...
 * (see ulak:hud:move-by — a transparent frameless window drifts wider on
...
export function useHudComposerDrag(
  ...
  const reset = useCallback(() => {
    ...
    if (state) {
      if (state.armed) {
        window.ulakDesktop?.hud?.endMove?.()
      }
