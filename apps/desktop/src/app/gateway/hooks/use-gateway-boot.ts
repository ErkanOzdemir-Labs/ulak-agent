import { isGatewayReauthRequired, JsonRpcGatewayError, resolveGatewayWsUrl } from '@ulak/shared'
...
export function primaryRuntimeConnectionId(connection: Pick<UlakConnection, 'connectionId' | 'mode'>): null | string {
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.ulakDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: UlakGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: UlakConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Ulak...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Ulak…". The probe is a no-op for a healthy or local backend.
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to Ulak backend'
        )
        ...
        // as an opaque "Could not connect to Ulak gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshUlakConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshUlakConfig(false, ownsSwitch).catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startUlak re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new UlakGateway()
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshUlakConfig(),
          ...
          // retry it), not brick boot behind the "Ulak couldn't start"
