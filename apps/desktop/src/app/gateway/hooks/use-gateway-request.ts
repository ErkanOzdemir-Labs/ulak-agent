...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as UlakGateway | null
  const gatewayRef = useRef<UlakGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.ulakDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<UlakGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as UlakGateway | null
      }),
    []
  )
