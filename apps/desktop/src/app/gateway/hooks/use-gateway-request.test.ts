...
vi.mock('@/ulak', async importOriginal => {
  const actual = await importOriginal<typeof UlakModule>()

  class FakeUlakGateway {
...
const fakeGateway = { connectionState: 'open' } as unknown as UlakGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as UlakGateway, 'default')
  $gateway.set(primary as unknown as UlakGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.ulakDesktop, 'getGatewayWsUrlFor')
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as UlakGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { ulakDesktop: unknown }).ulakDesktop = { getConnection }
    $gateway.set(dropped)
