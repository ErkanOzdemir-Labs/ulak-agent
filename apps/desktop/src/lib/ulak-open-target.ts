 ...
 * `ulak://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `ulak://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `ulak://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type UlakOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

const ULAK_PROTOCOL = 'ulak:'
...
export function normalizeUlakOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('ulak://') || trimmed.startsWith(`${ULAK_PROTOCOL}//`)) {
    try {
      ...
      // ulak://index-network/intent/1 → /index-network/intent/1
...
export function resolveUlakOpenPath(target: UlakOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeUlakOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeUlakOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeUlakOpenString(target.path)
 ...
 * (`ulak://<kind>/<name>?…` → kind/name/params).
 */
export function pathFromUlakDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  if (!kind || !name) {
    return null
  }

  if (kind === 'open') {
    return resolveUlakOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveUlakOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `ulak://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromUlakDeepLink('open', name, params)
}
