...
function connectionCacheKey(connection: UlakConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: UlakConnection | null = $connection.get()) {
...
function bridge() {
  ...
  if (!desktop) {
    throw new Error('Ulak Desktop bridge is unavailable')
  }
...
export async function readDesktopDir(path: string): Promise<UlakReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<UlakReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<UlakReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<UlakReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.ulakDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: UlakSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
