...
//
// When someone is in a hands-free "Hey Ulak" voice chat, the natural way to
...
// phrase (optionally addressed to Ulak), so a real turn that merely contains
...
// Optional address prefixes so "ulak stop" / "ok stop" / "hey ulak, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey ulak', 'hey ulak,', 'ulak', 'ulak,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("ulak") is not a stop command on its own.
      continue
    }
 ...
 * to Ulak). Returns false for anything that merely contains "stop" as part of
