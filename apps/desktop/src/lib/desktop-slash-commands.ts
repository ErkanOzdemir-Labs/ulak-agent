...
/** Why a known Ulak command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Ulak profile', surface: action('profile') },
  {
    name: '/skin',
...
function isKnownUlakSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Ulak' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownUlakSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Ulak and were never asked for. It is
