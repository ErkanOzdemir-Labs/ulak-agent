...
beforeEach(() => {
  ...
  document.querySelector('ulak-watch')?.remove()
  delete (window as unknown as Record<string, unknown>).__ulakWatch
})

/** The overlay lives in a closed shadow root, so a test can only see the host. */
const host = () => document.querySelector('ulak-watch')
...
describe('watchInPage', () => {
  it('mounts one host on documentElement and reuses it across stages', () => {
    ...
    expect(document.querySelectorAll('ulak-watch')).toHaveLength(1)
  ...
  it('rebuilds its chrome when the injected source changes underneath it', () => {
    ...
    const stamp = (value: number) => {
      ;(window as unknown as Record<string, unknown>).__ulakWatchTag = value
    }
