...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __ulakWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__ulakWatch || (win.__ulakWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('ulak-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('ulak-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __ulakWatchTag?: number }).__ulakWatchTag
