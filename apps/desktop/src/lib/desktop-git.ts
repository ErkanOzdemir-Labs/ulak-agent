import type {
  ...
  UlakGitBranch,
  UlakGitWorktree,
  UlakRepoPullRequests,
  UlakRepoStatus,
  UlakReviewList,
  UlakReviewShipInfo
} from '@/global'
import { ulakApi } from '@/ulak'
...
// (window.ulakDesktop.git); on a remote gateway that's the wrong filesystem,
...
type GitBridge = NonNullable<NonNullable<Window['ulakDesktop']>['git']>
...
const remoteGit: GitBridge = {
  worktreeList: async repoPath =>
    (await gitGet<{ worktrees: UlakGitWorktree[] }>('worktrees', { path: repoPath })).worktrees,
  ...
  repoStatus: repoPath => gitGet<UlakRepoStatus | null>('status', { path: repoPath }),
  ...
  review: {
    ...
    shipInfo: repoPath => gitGet<UlakReviewShipInfo>('review/ship-info', { path: repoPath }),

    prList: (repoPath, branches, numbers) =>
      gitPost<UlakRepoPullRequests>('review/pr-list', { branches, numbers: numbers ?? [], path: repoPath }),
...
export function desktopGit(): GitBridge | undefined {
  ...
  return isDesktopFsRemoteMode() ? remoteGit : window.ulakDesktop?.git
}
