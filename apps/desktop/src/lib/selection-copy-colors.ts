...
// Ulak is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.ulakMode
 ...
 * the only carriers of Ulak' palette — are dropped so the paste target's
