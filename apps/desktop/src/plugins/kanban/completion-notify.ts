 ...
 *    desktop shell fires only while the user is AWAY from Ulak. This is the
...
import { host, type PluginOs, type PluginRestOptions, type PluginTranslate } from '@ulak/plugin-sdk'
...
function notifyOne(kind: string, spec: { titleKey: string; toast: ToastKind }, ev: CompletionEvent): void {
  ...
  // is away from Ulak (the toast above covers the foreground case). Isolated:
