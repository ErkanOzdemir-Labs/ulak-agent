 ...
 * `{type:'ulak-skill-pick'}` back and the plugin installs via skills.manage.
 *
...
vi.mock('@ulak/plugin-sdk', async importOriginal => {
  const original = await importOriginal<typeof UlakSdk>()
...
describe('hub pick messages', () => {
  it('installs the picked skill when it comes from our own frame', () => {
    ...
    postPick(
      { identifier: 'nous/web-research', name: 'Web Research', type: 'ulak-skill-pick' },
      {
  ...
  it('regression: refuses identifiers outside the slug charset', () => {
    ...
    for (const identifier of ['../../etc/passwd', 'skill; rm -rf /', '-flag', 'name with spaces', '']) {
      postPick({ identifier, name: 'Web Research', type: 'ulak-skill-pick' }, { source: frame.contentWindow })
    }
  ...
  it('ignores messages that are not a skill pick', () => {
    ...
    postPick({ identifier: 'nous/web-research', type: 'ulak-skill-pick' }, { source: frame.contentWindow })
