...
import { atom, host } from '@ulak/plugin-sdk'
...
const GROUP_CHAT_SYNC_META_KEY = 'ulak-bots-groups'
...
/** The v3 envelope stored under the default profile's `ulak-bots-groups`
 ...
 *  clients (Ulak Go, headless backends) see rooms regardless of which
 ...
 *  renamed bot never shows up as its raw profile id or a stale "Ulak"
 *  (community report, Aug 21 2026: renamed default still read "Ulak is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as Ulak (matching displayName and the
 *  @ulak handle) so the main agent never loses its name in rooms. */
export function groupSpeakerLabel(name?: null | string) {
  ...
  // Core profile display_name (`ulak profile rename …` / dashboard) from
  ...
  return trimmed.toLowerCase() === 'default' ? 'Ulak' : trimmed
}
