...
describe('cachedUnionRoster', () => {
  it('reads the entry useRoster wrote under the connection-suffixed key', async () => {
    ...
    seed(['ulak-bots', 'roster', 'local'], { profiles: [{ name: 'default' }] })
    ...
    expect(cache.has(JSON.stringify(['ulak-bots', 'roster']))).toBe(false)
  })

  it('falls back to another connection’s entry when the window has moved', async () => {
    const { cachedUnionRoster } = await import('./data')

    seed(['ulak-bots', 'roster', 'vera'], { profiles: [{ connectionId: 'vera', name: 'default' }] })
    connection.id = 'local'
  ...
  it('prefers the freshest snapshot among several cached connections', async () => {
    ...
    seed(['ulak-bots', 'roster', 'old'], { fetchedAt: 1_000, profiles: [{ name: 'stale' }] })
    seed(['ulak-bots', 'roster', 'new'], { fetchedAt: 9_000, profiles: [{ name: 'fresh' }] })
