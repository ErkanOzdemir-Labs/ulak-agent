...
function published(room: Room) {
  ...
  return (configure?.params.ui_meta as Record<string, Record<string, unknown>>)?.['ulak-bots-groups']
}
...
describe('speaker labels', () => {
  it('the default profile speaks as Ulak in transcripts, not @default', async () => {
    ...
    expect(line).toBe('Ulak: hello room')
  ...
  it('honor friendly identity: Bot Mode title, then display_name, never a stale Ulak', async () => {
    const { chat, rounds } = await loadRoom()
    const data = await import('./data')

    // A renamed default (core display_name via `ulak profile rename`) must
    // read as its new name — the community report was "Lucy" still showing
    // "Ulak is thinking…" in group rooms.
    ...
    // Untitled rows keep today's behavior: default → Ulak, others verbatim.
    data.$botMeta.set({})

    expect(chat.groupSpeakerLabel('default')).toBe('Ulak')
...
describe('sync worker', () => {
  it('retries a gateway CAS conflict and publishes the merged room', async () => {
    ...
    await drain(() => (room.gateway.uiMetaRevisions['ulak-bots-groups'] || 0) < 2, 60)

    const stored = room.gateway.uiMeta['ulak-bots-groups'] as {
      rooms: Record<string, { log: GroupMessage[] }>
    }

    expect(room.gateway.uiMetaRevisions['ulak-bots-groups']).toBe(2)
  ...
  it('fans a room write out to every reachable default-profile gateway', async () => {
    ...
    host.requestProfile = async (route: { connectionId: string }, method: string) => {
      ...
      if (method === 'profiles.configure') {
        return { applied: { ui_meta: true, ui_meta_revisions: { 'ulak-bots-groups': 1 } } }
      }
