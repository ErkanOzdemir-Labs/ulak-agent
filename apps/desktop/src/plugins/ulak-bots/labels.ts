...
export function displayName(bot: Partial<RosterRow>, meta?: BotMeta | null): string {
  ...
  // "Ulak". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `ulak profile rename
  ...
  // never generic "Ulak" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as Ulak (the agent it is)
