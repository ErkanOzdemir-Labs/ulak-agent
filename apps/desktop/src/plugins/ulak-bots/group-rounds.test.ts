...
describe('routing', () => {
  ...
  it('resolves @ulak to the default member', async () => {
    const { rounds } = await loadRoom()
    ...
    const parsed = rounds.parseGroupChatMentions('@ulak take a look', members)
...
describe('turn prompt', () => {
  it('addresses the default profile as @ulak', async () => {
    const { rounds } = await loadRoom()
    ...
    expect(own).toMatch(/You are @ulak,/)
    ...
    expect(peer).toMatch(/group chat with @ulak/)
  })
