/**
 * Ulak Bot Mode — a "one chat per agent" roster for the Ulak desktop.
 *
 * Left pane "Bots": one row per Ulak profile (a bot = an agent profile) with
 ...
 *
 * Right tile "Routines": scheduled tasks (Ulak cron jobs) scoped to the
 ...
 * bot-initiated sends use `ulak -p <bot> chat --in ~ -c "Bot Chat"`.
 */

import { CHAT_EMPTY_AREA, COMPOSER_AREAS, host, PALETTE_AREA, translateNow } from '@ulak/plugin-sdk'
import type { ChatEmptyProps, PluginContext } from '@ulak/plugin-sdk'
...
export default {
  ...
  register(ctx: PluginContext) {
    ...
    // excluded (a bot doesn't @ itself); 'default' surfaces as @ulak.
    ctx.register({
            ...
            try {
              const registry =
                typeof window !== 'undefined'
                  ? await Promise.resolve(window.ulakDesktop?.connections?.list?.()).catch(() => null)
                  : null
    ...
    try {
      if (typeof window !== 'undefined') {
        unbindConnectionsChanged =
          window.ulakDesktop?.connections?.onChanged?.(payload => {
    ...
    // A bot's chat before it has spoken: core's splash is Ulak' wordmark and
      ...
      data: {
        handler: async (draft: ComposerDraftPayload): Promise<ComposerDraftPayload> => {
          ...
          const lines = mentionedBots.map(bot => {
            ...
            const title = String(
              botRosterMeta(bot, $botMeta.get())?.title || bot.ui_meta?.['ulak-bots']?.title || bot.title || ''
            ).trim()

            // message_agent only resolves canonical identities: the relay
            // matches a roster row's handle/profile (± @connection-id), the
            // local path a bare profile name or 'ulak'. botHandle() prefers
