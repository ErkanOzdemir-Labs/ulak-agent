...
import { Button, host, Input } from '@ulak/plugin-sdk'
import { useEffect, useRef, useState } from 'react'

import { useBots } from './i18n'

// ── skills hub section: the REAL hub page (docs) embedded as a picker ──────
// https://ulak-agent.github.com/ErkanOzdemir-Labs/docs/skills?embed=picker hides the
// docs chrome and adds "+ Add to this Agent" per card, posting
// {type: 'ulak-skill-pick', ...} to us (ulak-agent#86243). We validate
...
export function HubSkillsSection({ forProfile, onInstalled }: HubSkillsSectionProps) {
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (!data || data.type !== 'ulak-skill-pick' || !data.name) {
        return
      }
