...
import { host, LruCache } from '@ulak/plugin-sdk'
...
// to (local, remote URL, SSH, Ulak Cloud, docker) must be able to find
...
// ulak_cli/config_defaults.py and tui_gateway/methods_bot_relay.py and fails
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['ulak-bots']?.title || profile?.display_name || ''),
