...
import { Button, host, Input, useI18n } from '@ulak/plugin-sdk'
...
// and the row falls back to the "run ulak mcp / Settings" hint. profile is
...
export function McpSetupButton({ profile, entry, onDone, ensureProfile }: McpSetupButtonProps) {
  ...
  const beginOAuth = async () => {
    ...
    const mcpOauthBridge =
      typeof window !== 'undefined' && window.ulakDesktop && window.ulakDesktop.mcpOauth
        ? window.ulakDesktop.mcpOauth
        : null
    ...
    try {
      ...
      } else if (typeof window !== 'undefined' && window.ulakDesktop && window.ulakDesktop.openExternal) {
        window.ulakDesktop.openExternal(authUrl)
      } else {
