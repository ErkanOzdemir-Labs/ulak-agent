/**
 * Routines: the Ulak cron jobs scoped to the bot you're chatting with — the
...
// ulak -p <bot> chat delegation wrapper so the run reaches that bot's
...
export function routinePrompt(
  ...
  return (
    ...
    `ulak -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// Ulak-native schedule string; Advanced exposes it raw.
...
/** Compose the Ulak schedule string from picker state. */
