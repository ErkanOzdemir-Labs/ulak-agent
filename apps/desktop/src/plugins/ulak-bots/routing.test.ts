 ...
 * the raw Cloud hostname, or as generic "Ulak" when Cloud was the only
 * source.
 *
...
vi.mock('@ulak/plugin-sdk', () => ({ host: hostMock }))
...
describe('alias identity survives the hosted handoff (#89131)', () => {
  ...
  it('renders the sole Cloud-only default as the alias, not "Ulak"', () => {
  ...
  it('never leaks the alias onto a same-named default on another connection', () => {
    ...
    // Local default while the ACTIVE gateway is local: untouched "Ulak".
    expect(displayName({ name: 'default' }, null)).toBe('Ulak')
  })
