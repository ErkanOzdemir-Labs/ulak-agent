...
import { forgetSessionUnread, host, queryClient } from '@ulak/plugin-sdk'
...
export function mergeServerMeta(roster: RosterRow[], fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['ulak-bots']
...
/** Permanently delete a bot's Ulak profile, then remove plugin-local state
 ...
 * directory (ulak-agent#52279). That is the "can't delete a bot" error. */
