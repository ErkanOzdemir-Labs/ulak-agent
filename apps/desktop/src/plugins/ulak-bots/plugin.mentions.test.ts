...
async function contributions({
  ...
  cache.set(JSON.stringify(['ulak-bots', 'roster', cacheKeyConnection]), {
    key: ['ulak-bots', 'roster', cacheKeyConnection],
    value: { profiles }
  })
...
describe('@-mention completions', () => {
  ...
  it('surfaces default as @ulak and prefix-filters on the handle', async () => {
    const { provide } = await contributions({
      focused: 'researcher',
      profiles: [
        { name: 'default' },
    ...
    expect(provide('').map(item => item.insert)).toEqual(expect.arrayContaining(['@ulak', '@writer-homelab']))
...
describe('the mention middleware', () => {
  it('identifies a remote @name-device mention without delivering anything', async () => {
    ...
    expect(result.text).not.toMatch(/ulak -p '?default/)
  ...
  it('annotates the resolvable handle for a local row whose UI alias differs', async () => {
    ...
    // The local resolver only knows bare profile names / 'ulak'.
    const { handler } = await contributions({
      focused: 'ops',
      profiles: [
    ...
    expect(result.text).toMatch(/message_agent target: "ulak"/)
  ...
  it('teaches no shellout and forbids forwarding the user’s text verbatim', async () => {
    ...
    // a `ulak -p …` handoff, giving the model a second send path and a way
    // to relay the raw draft.
    const { handler } = await contributions({ focused: 'research', profiles: [{ name: 'research' }, { name: 'ops' }] })
    const result = await handler({ text: 'ask @ops to summarize' })

    expect(result.text).not.toMatch(/ulak -p/)
  ...
  it('keeps a poisoned bot title inert prose', async () => {
    ...
    // the invariant that matters is that no ulak command is ever emitted.
    const { handler } = await contributions({
      focused: 'ops',
      profiles: [
        { name: 'ops' },
    ...
    expect(result.text).not.toMatch(/`ulak/)
  })
