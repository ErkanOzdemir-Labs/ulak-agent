...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `UlakPlugin` and
...
`$ULAK_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`ulak-desktop-plugins` skill.
