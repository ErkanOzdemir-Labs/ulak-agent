import type { ConnectionState } from '@ulak/shared'
...
import type { SessionInfo, UsageStats } from '@/types/ulak'
...
const COMPOSER_MODEL_KEY = 'ulak.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'ulak.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'ulak.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'ulak.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'ulak.desktop.composer.fast'
...
function composerScopeForConnection(connection: UlakConnection | null): string | null {
  if (!connection) {
    return null
  }
...
const LAST_SESSION_KEY = 'ulak.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'ulak.desktop.lastRoute'
...
function workspaceCwdKey(connection: UlakConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(shouldPublish: () => boolean = () => true): Promise<string> {
  const settings = window.ulakDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(shouldPublish: () => boolean = () => true): Promise<void> {
  const sanitize = window.ulakDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<UlakConnection | null>(null)
...
const SESSION_OWNER_HINTS_KEY = 'ulak.desktop.sessionOwnerHints.v1'
 ...
 * UlakConnection lookup is unavailable. */
...
export const setConnection = (next: Updater<UlakConnection | null>) => {
...
// until config loads, and re-seeded on every profile switch by useUlakConfig.
