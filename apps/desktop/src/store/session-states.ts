...
import { LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@ulak/shared'
...
const TILES_KEY = 'ulak.desktop.sessionTiles.v2'
const LEGACY_TILES_KEY = 'ulak.desktop.sessionTiles.v1'
...
const BOT_CHAT_SCOPE_KEY = 'ulak.desktop.botChatSessions.v1'
 ...
 *  (ulak-agent#90102). The tile bucket is a Local Storage cache, and a
 ...
 * restore re-dials the profile's backend, whose ensure_ulak_home() re-creates
 * the profile directory the delete just removed (ulak-agent#94235). Same
...
export function dropTilesForProfile(
  ...
  const ownerMatches = (owner: SessionProfileRoute | undefined): boolean => {
    ...
    // would orphan a live conversation (ulak-agent#94235). Tiles persisted
...
export const $focusedStoredSessionId = computed(
  ...
  (groupId, tree, selected, workspaceMode) => {
    ...
    // it to the sidebar group, whose active pane ('ulak-bots:pane') is not a
...
// Dev hook for automation (mirrors __ULAK_LAYOUT_TREE__).
if ((import.meta.env.DEV || import.meta.env.VITE_PERF_PROBE === '1') && typeof window !== 'undefined') {
  ;(window as unknown as Record<string, unknown>).__ULAK_SESSION_TILES__ = {
