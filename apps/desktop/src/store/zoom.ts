...
export function setZoomPercent(percent: number): void {
  window.ulakDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.ulakDesktop?.zoom) {
  void window.ulakDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.ulakDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
