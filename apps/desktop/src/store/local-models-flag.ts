 ...
 *
 * Local models ship on main behind `--local` (either `ulak desktop --local`
 * or the flag on Ulak.exe itself). The flag is strict: without it the GUI
...
export const $localModelsEnabled = atom<boolean>(
  typeof window !== 'undefined' && window.ulakDesktop?.localModelsEnabled === true
)
