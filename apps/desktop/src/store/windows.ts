...
export function canOpenSessionWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.ulakDesktop?.openSessionWindow === 'function'
}

// True when the shell can open a full peer app window (⌘⇧N / "New Window").
export function canOpenNewWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.ulakDesktop?.openWindow === 'function'
}

// True when the shell can pop the in-app Browser into its own OS window.
export function canOpenBrowserWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.ulakDesktop?.openBrowserWindow === 'function'
}
...
export function canOpenSessionInTerminal(): boolean {
  return typeof window !== 'undefined' && typeof window.ulakDesktop?.openSessionInTerminal === 'function'
}
...
export async function openSessionInNewWindow(sessionId: string, opts?: { watch?: boolean }): Promise<void> {
  ...
  await runWindowOpen(
    () => window.ulakDesktop.openSessionWindow(sessionId, { ...opts, profile }),
...
export async function openNewWindow(): Promise<void> {
  ...
  await runWindowOpen(() => window.ulakDesktop.openWindow(), 'Could not open a new window')
}
...
export async function openBrowserInNewWindow(tabId: string): Promise<boolean> {
  ...
  return runWindowOpen(() => window.ulakDesktop.openBrowserWindow(tabId), 'Could not pop out browser')
}
...
export async function openSessionInTerminal(
  ...
  await runWindowOpen(
    () => window.ulakDesktop.openSessionInTerminal(sessionId, opts),
