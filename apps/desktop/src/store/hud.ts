 ...
 * composer with the reply scrolling above it, so Ulak can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.ulakDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.ulakDesktop?.hud
...
export function resetHudLayout(): void {
  void window.ulakDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.ulakDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.ulakDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
