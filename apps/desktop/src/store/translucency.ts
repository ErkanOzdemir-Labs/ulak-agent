 ...
 * `@ulak/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-ulak-glass]`
 * block in styles.css).
 */
 ...
 * sets `ulakDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.ulakDesktop?.glassSupported === 'boolean'
    ? window.ulakDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.ulakDesktop?.translucencySupported === 'boolean'
    ? window.ulakDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-ulak-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-ulak-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-ulak-glass-on', glassLive)
  root.toggleAttribute('data-ulak-glass', glassOn)
  root.toggleAttribute('data-ulak-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-ulak-glass-scope', scope)
  } else {
    root.removeAttribute('data-ulak-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.ulakDesktop?.setTranslucency?.(state)
  })
