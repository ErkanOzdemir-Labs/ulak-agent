...
import type { UlakGitWorktree, UlakRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: UlakGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, UlakRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, UlakGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<UlakRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<UlakGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<UlakRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<UlakGitWorktree[]>>()
const $noRepoStatus = atom<UlakRepoStatus | null>(null)
const $noWorktrees = atom<UlakGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<UlakRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<UlakGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: UlakRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: UlakGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<UlakRepoStatus | null>
  seq: number
  target: string
}
