...
import type { UlakBranchPullRequest } from '@/global'
import { scanSessionPullRequests, type SessionInfo } from '@/ulak'
...
export const $pullRequestsByBranch = atom<Record<string, UlakBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'ulak.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('ulak.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: UlakBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
