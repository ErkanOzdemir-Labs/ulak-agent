...
import type { UlakReviewFile, UlakReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Ulak' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'ulak.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'ulak.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'ulak.desktop.reviewTreeMode'
const SELECTED_KEY = 'ulak.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<UlakReviewFile[]>([])
...
export const $reviewShipInfo = atom<UlakReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['ulakDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: UlakReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly UlakReviewFile[], path: string): UlakReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.ulakDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.ulakDesktop?.openExternal?.(url)
    }
