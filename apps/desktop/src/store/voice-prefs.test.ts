...
vi.mock('@/ulak', () => ({
  getUlakConfigRecord: vi.fn(async () => ({})),
  saveUlakConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye ulak', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye ulak')
  })
