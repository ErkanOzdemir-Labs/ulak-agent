...
/**
 * Feature store for backend (agent) plugins — the native Ulak plugins plus
 ...
 * primitives `ulak plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native Ulak. */
  portable?: boolean
}
 ...
 *  ULAK_HOME. Always refetches (it's a cheap local disk scan on the
