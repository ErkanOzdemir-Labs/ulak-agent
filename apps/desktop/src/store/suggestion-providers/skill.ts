 ...
 *  Working in `~/www/ulak-agent`, the draft says "ulak-agent" constantly
 *  — a "Use skill: ulak-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`ulak-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/ulak-agent-dev`, `plugin:skill`). */
