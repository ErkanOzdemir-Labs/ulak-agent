...
const agentConn = (over: Partial<UlakConnection> = {}): UlakConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as UlakConnection

const localConn = (over: Partial<UlakConnection> = {}): UlakConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as UlakConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<UlakConnection>>()

const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<UlakConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { ulakDesktop: { getConnection, getConnectionFor }, localStorage })
...
describe('ensureGatewayAgent commit hook (beforeActivate) — the Sessions switcher door (#93937)', () => {
  ...
  it('a descriptor lookup that never answers is bounded, fails open, and releases the mutex', async () => {
    ...
    try {
      getConnectionFor.mockImplementationOnce(() => new Promise<UlakConnection>(() => undefined))
