 ...
 * app's bounds and stays visible while Ulak is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'ulak.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'ulak.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.ulakDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.ulakDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.ulakDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.ulakDesktop?.petOverlay?.close()
}
