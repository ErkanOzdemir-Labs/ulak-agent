...
const { getProfiles } = await import('@/ulak')
...
const remoteConn = (over: Partial<UlakConnection> = {}): UlakConnection =>
  ({ baseUrl: 'https://ulak-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as UlakConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { ulakDesktop: { getConnection } })
