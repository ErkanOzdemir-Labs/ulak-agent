import { type ConnectionState, type GatewayEvent, registryBackendScopeKey, resolveGatewayWsUrl } from '@ulak/shared'
...
import { setApiRequestConnection, UlakGateway } from '@/ulak'
...
function dialProfile(
  desktop: NonNullable<typeof window.ulakDesktop>,
  profile: string,
...
const isOpen = (gateway: UlakGateway | null): boolean => gateway?.connectionState === 'open'
...
interface RegistryConfig {
  ...
  onActiveConnectionChanged?: (connection: UlakConnection) => void
  /**
...
interface Secondary {
  ...
  connection: UlakConnection | null
  gateway: UlakGateway
...
interface GatewayRegistryState {
  ...
  primaryGateway: UlakGateway | null
  ...
  $gateway: ReturnType<typeof atom<UlakGateway | null>>
  $activeProfile: ReturnType<typeof atom<string>>
}

const STATE_KEY = Symbol.for('ulak.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<UlakGateway | null>(null),
...
export function setPrimaryGateway(gateway: UlakGateway | null, profile = 'default'): void {
...
export function setPrimaryGatewayConnection(connection: Pick<UlakConnection, 'connectionId'> | null): void {
...
async function requestOnPrimaryGateway<T>(
  ...
  if (!gateway || !isOpen(gateway)) {
    throw new Error('Ulak gateway unavailable')
  }
...
export function activeGateway(): UlakGateway | null {
  if (g.activeKey === g.primaryProfile) {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the ulak module (null
...
function publishActiveConnection(connection: UlakConnection): void {
...
async function gatewayForProfile(
  ...
  leaseRequest = false,
  spawnPriority: SpawnPriority = 'background'
): Promise<{ gateway: UlakGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
  const key = normKey(profile)
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Ulak gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.ulakDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Ulak Desktop.')
  }
...
export async function ensureActiveGatewayOpen(): Promise<UlakGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Ulak gateway is not connected" on the Sessions `+` action (#88880).
