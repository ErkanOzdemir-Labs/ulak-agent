import { LOCAL_CONNECTION_ID } from '@ulak/shared'
...
// /api/profiles/active `current`). "default" is the root ~/.ulak. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'ulak.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'ulak.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await ulakApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new ULAK_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.ulakDesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<UlakConnection | null> {
  const getConnection = window.ulakDesktop?.getConnection
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<UlakConnection | null> {
  const getConnectionFor = window.ulakDesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'ulak.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.ulakDesktop?.profile?.remember(target)
      }
...
async function isLocalDesktopProfile(target: string): Promise<boolean> {
  const getConnectionConfig = window.ulakDesktop?.getConnectionConfig
...
// Switch to the default (root ~/.ulak) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.ulakDesktop?.touchBackend?.(target).catch(() => undefined)
}
