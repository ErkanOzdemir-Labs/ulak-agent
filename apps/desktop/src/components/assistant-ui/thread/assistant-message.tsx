...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's ULAK_HOME/logs. On a
  ...
  const openLogs = useCallback(async () => {
    try {
      const root = await window.ulakDesktop?.logsRoot?.()
      ...
      const result = await window.ulakDesktop?.openDir?.(root)
