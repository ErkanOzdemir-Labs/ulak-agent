...
describe('buildToolView web-search query', () => {
  it('keeps the query separate from structured search results', () => {
    const view = buildToolView(
      part({
        args: { query: 'Ulak Agent Desktop tool calls' },
        result: { web: [{ snippet: 'Desktop docs', title: 'Ulak docs', url: 'https://example.com/docs' }] },
        toolName: 'web_search'
      }),
      ''
    )

    expect(view.searchQuery).toBe('Ulak Agent Desktop tool calls')
    expect(view.searchHits).toEqual([{ snippet: 'Desktop docs', title: 'Ulak docs', url: 'https://example.com/docs' }])
  })
})
  ...
  it('shows failed title when navigate returns success=false', () => {
    const view = buildToolView(
      part({
        ...
        args: { url: 'https://ulak-agent.github.com/ErkanOzdemir-Labs/docs' },
    ...
    expect(view.title).toBe('Failed to open ulak-agent.github.com/ErkanOzdemir-Labs/docs')
  })
  ...
  it('shows opened title on success', () => {
    const view = buildToolView(
      part({
        toolName: 'browser_navigate',
        args: { url: 'https://ulak-agent.github.com/ErkanOzdemir-Labs/docs' },
        result: { success: true, url: 'https://ulak-agent.github.com/ErkanOzdemir-Labs/docs', title: 'Docs' }
      }),
      ''
    )

    expect(view.status).toBe('success')
    expect(view.title).toBe('Opened ulak-agent.github.com/ErkanOzdemir-Labs/docs')
  })
})
