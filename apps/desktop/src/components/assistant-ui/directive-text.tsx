...
const ULAK_REF_TYPES = WIRE_REFERENCE_KINDS
type UlakRefType = (typeof ULAK_REF_TYPES)[number]
...
const ULAK_DIRECTIVE_RE = referenceRe()
...
export const ulakDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(ULAK_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...ulakDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Ulak directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.ulakDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.ulakDesktop?.readFileDataUrl(id)
