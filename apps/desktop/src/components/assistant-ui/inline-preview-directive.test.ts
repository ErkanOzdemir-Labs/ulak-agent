...
describe('intentFromMessage', () => {
  const msg = (over: Record<string, unknown> = {}) => ({
    type: 'ulak-inline-preview-intent',
    token: 'tok',
  ...
  it('rejects wrong token, wrong type, empty, and hostile shapes', () => {
    ...
    expect(intentFromMessage(msg({ type: 'ulak-inline-preview-size' }), 'tok')).toBeNull()
...
describe('withInlineChrome intent wiring', () => {
  it('injects ulak.send and the data-ulak-send click bridge', () => {
    const framed = withInlineChrome('<html><body><h1>w</h1></body></html>', 'tok', '')

    expect(framed).toContain('window.ulak={send:send}')
    expect(framed).toContain('data-ulak-send')
    expect(framed).toContain('ulak-inline-preview-intent')
  })
})
