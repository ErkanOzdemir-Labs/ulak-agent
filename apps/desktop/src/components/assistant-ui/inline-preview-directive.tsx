 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.ulak.send(prompt)` (or declarative
 * `data-ulak-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'ulak-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'ulak-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `ulak.send(prompt)`.
 ...
 *  wires `data-ulak-send` so declarative HTML works with zero script:
 *  `<button data-ulak-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.ulak={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-ulak-send]"):null;' +
    'if(el)send(el.getAttribute("data-ulak-send")||"")},true)})()</script>'
  )
}
...
export function InlinePreviewDirective({
  ...
  // URL connection — including a same-machine `ulak serve` — renders live.)
