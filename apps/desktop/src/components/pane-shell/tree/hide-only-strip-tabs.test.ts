...
function sessionsBotsTree() {
  ...
  registerPane('ulak-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'ulak-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('ulak-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('ulak-bots:pane')
    expect($hiddenStripTabs.get()).toContain('ulak-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('ulak-bots:pane')

    expect(setStripTabHidden('ulak-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('ulak-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('ulak-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('ulak-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('ulak.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('ulak-bots:pane')
    ...
    revealTreePane('ulak-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('ulak-bots:pane')
    expect(window.localStorage.getItem('ulak.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'ulak-bots:pane', title: 'ulak-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'ulak-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
