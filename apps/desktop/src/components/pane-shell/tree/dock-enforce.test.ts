...
const TREE_KEY = 'ulak.desktop.layoutTree.v2'
const USER_PLACED_KEY = 'ulak.desktop.userPlacedPanes.v1'
const LEGACY_HEAL_KEY = 'ulak.desktop.paneDockHeals.v1'
  ...
  children: [
    {
      ...
      children: [
        ...
        { type: 'group', id: 'g-bots', panes: ['ulak-bots:pane'], active: 'ulak-bots:pane' }
      ]
    },
...
describe('enforced dock (stacked Bots pane → sessions-zone tab, every boot)', () => {
  ...
  it('re-homes a stacked bots pane into the sessions tab strip, keeping sessions active', async () => {
    ...
    const group = model.findGroupOfPane(tree.$layoutTree.get()!, 'ulak-bots:pane')!

    expect(group.panes).toEqual(['sessions', 'ulak-bots:pane'])
    ...
    expect(JSON.stringify(persisted)).toContain('"panes":["sessions","ulak-bots:pane"]')
  })

  it('re-homes even a USER-PLACED pane — the owner invariant beats the drag record', async () => {
    window.localStorage.setItem(USER_PLACED_KEY, JSON.stringify(['ulak-bots:pane']))
  ...
  it('re-homes even when the retired heal token was already burned, and clears the stale ledger', async () => {
    window.localStorage.setItem(LEGACY_HEAL_KEY, JSON.stringify(['ulak-bots:pane:sessions-tab-v1']))
  ...
  it('is idempotent within a boot and does not fight an intra-session drag', async () => {
    ...
    expect(model.findGroupOfPane(tree.$layoutTree.get()!, 'ulak-bots:pane')!.panes).toContain('sessions')
    ...
    expect(group.panes).toEqual(['ulak-bots:pane'])
  })
  ...
  it('re-homes again on the NEXT boot after a drag persisted the stacked shape', async () => {
    ...
    const group = second.model.findGroupOfPane(second.tree.$layoutTree.get()!, 'ulak-bots:pane')!
      ...
      children: [
        {
          type: 'group',
          id: 'g-left',
          panes: ['sessions', 'ulak-bots:pane'],
          active: 'ulak-bots:pane',
          headerHidden: true
        },
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' }
      ]
  ...
  it('re-homes an edge-enforced pane stranded in the sessions tab strip', async () => {
      ...
      children: [
        {
          type: 'group',
          id: 'g-sessions',
          panes: ['sessions', 'ulak-bots:pane', 'ulak-bots:routines'],
          active: 'ulak-bots:pane'
        },
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' }
      ]
    ...
    const botsGroup = model.findGroupOfPane(tree.$layoutTree.get()!, 'ulak-bots:pane')!
    const routinesGroup = model.findGroupOfPane(tree.$layoutTree.get()!, 'ulak-bots:routines')!

    expect(botsGroup.panes).toEqual(['sessions', 'ulak-bots:pane'])
    expect(botsGroup.active).toBe('ulak-bots:pane')
    expect(routinesGroup.panes).toEqual(['ulak-bots:routines'])
    ...
    const dockedRoutinesTree = {
      ...
      children: [
        ...
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' },
        {
          type: 'group',
          id: 'g-routines',
          panes: ['ulak-bots:routines'],
          active: 'ulak-bots:routines'
        }
      ]
    }
