...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Ulak gateway',
    fakeMode: false,
    message: 'boot failed',
...
describe('BootFailureOverlay', () => {
  ...
  it('recovers a cloud connection through the portal cascade instead of native OAuth', async () => {
    const gatewayUrl = 'https://agent-1.agents.github.com/ErkanOzdemir-Labs'
    const logout = vi.fn().mockResolvedValue({ ok: true, connected: false })
    const nativeLogin = vi.fn().mockResolvedValue({ ok: true, connected: false })
    const cloudStatus = vi.fn().mockResolvedValue({ portalBaseUrl: 'https://portal.github.com/ErkanOzdemir-Labs', signedIn: false })

    const cloudLogin = vi.fn().mockResolvedValue({
      ok: true,
      portalBaseUrl: 'https://portal.github.com/ErkanOzdemir-Labs',
      signedIn: true
    })
  ...
  it('shows the Nous Cloud down recovery when the backend flags isCloudBackendDown', async () => {
    const restore = stubDesktop(remoteToken)
    $desktopBoot.set({
      error: 'Nous Cloud agent ares-3009.agents.github.com/ErkanOzdemir-Labs is down (HTTP 503: server-side fault).',
      fakeMode: false,
    ...
    try {
      ...
      expect(screen.getByText(/ares-3009\.agents\.erkanozdemir-labs\.com/i)).toBeTruthy()
    } finally {
      restore()
    }
  })
