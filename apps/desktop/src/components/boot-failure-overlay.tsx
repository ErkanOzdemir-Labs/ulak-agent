...
export function BootFailureOverlay() {
  ...
  const retry = async () => {
    setBusy('retry')
    await window.ulakDesktop?.resetBootstrap().catch(() => undefined)
    window.location.reload()
  }

  const repair = async () => {
    setBusy('repair')
    await window.ulakDesktop?.repairBootstrap().catch(() => undefined)
    window.location.reload()
  }
  ...
  const switchToLocalGateway = async () => {
    ...
    await window.ulakDesktop?.applyConnectionConfig({ mode: 'local' }).catch(() => undefined)
    setBusy(null)
  }

  // Clear this gateway's stale auth first, then re-establish it through the
  // connection's owning login flow. Ulak Cloud must reuse its portal session
  ...
  const openLogs = () => void window.ulakDesktop?.revealLogs().catch(() => undefined)
    ...
    actions = [
      {
        ...
        onClick: () => openExternalLink('https://portal.github.com/ErkanOzdemir-Labs'),
        icon: <ExternalLink />
      },
      localAction,
      { ...retryAction, variant: 'secondary' },
      {
        ...
        onClick: () => openExternalLink('https://discord.gg/ErkanOzdemir-Labs'),
        variant: 'ghost'
      },
