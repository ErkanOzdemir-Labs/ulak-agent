...
import type { TranslucencyState } from '@ulak/shared/translucency'
...
declare global {
  interface Window {
    ulakDesktop: {
      ...
      // Resume this session in the user's own terminal emulator (`ulak --tui
      ...
      // Ulak Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new ULAK_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: UlakApiRequest) => Promise<T>
      notify: (payload: UlakNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<UlakReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<UlakReadFileTextResult>
      selectPaths: (options?: UlakSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<UlakPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<UlakPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<UlakPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: UlakActiveWork) => void
      setTitleBarTheme?: (payload: UlakTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<UlakReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<ULAK_HOME>/desktop-plugins),
      ...
      /** LOCAL `<ULAK_HOME>/logs` (profile-aware) — error card "Open Logs". */
      logsRoot?: () => Promise<string>
      // Local AGENT-plugin root (<ULAK_HOME>/plugins), same Electron-local
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<UlakGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<UlakGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<UlakGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<UlakRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: UlakReviewScope, baseRef?: null | string) => Promise<UlakReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: UlakReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<UlakReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<UlakRepoPullRequests>
          ...
          fetchPrComment: (repoPath: string, url: string) => Promise<UlakPrComment | null>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: UlakTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<UlakTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: UlakWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: UlakPreviewFileChanged) => void) => () => void
...
export interface UlakTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface UlakTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `ulak update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  ulakRoot?: string
...
export interface UlakConnection {
  baseUrl: string
  darwinMajor?: number
...
export interface UlakTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface UlakActiveWork {
  count: number
  titles: string[]
}

export interface UlakWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Ulak Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Ulak Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Ulak Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the ulak gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Ulak Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface UlakApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface UlakNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface UlakPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface UlakReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface UlakPreviewWatch {
  id: string
  path: string
}
...
export interface UlakGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface UlakGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface UlakGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface UlakRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface UlakRepoStatus {
  branch: null | string
...
// turn began.
export type UlakReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface UlakReviewFile {
  path: string
  added: number
  removed: number
...
export interface UlakReviewList {
  files: UlakReviewFile[]
...
export interface UlakBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface UlakRepoPullRequests {
  ghReady: boolean
  prs: UlakBranchPullRequest[]
}
...
export interface UlakPrComment {
  author: string
  body: string
  diffHunk: string
  kind: 'issue' | 'review'
...
export interface UlakReviewShipInfo {
  ghReady: boolean
  pr: UlakReviewPr | null
}

export interface UlakReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface UlakReadDirResult {
  entries: UlakReadDirEntry[]
  error?: string
}

export interface UlakPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface UlakSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
