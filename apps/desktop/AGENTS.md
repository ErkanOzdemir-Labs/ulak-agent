...
How to build Ulak Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Ulak surface can also
...
- A **runtime home change** (switching the underlying `ULAK_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Ulak — do not assume
