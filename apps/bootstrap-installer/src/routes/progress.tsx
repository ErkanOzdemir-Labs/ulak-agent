...
export default function ProgressScreen({ bootstrap }: ProgressProps) {
  ...
  const title = bootstrap.status === 'completed' ? 'Done' : isUpdate ? 'Updating Ulak' : 'Setting up Ulak Agent'

  const description = isUpdate
    ? 'Ulak is updating to the latest version — this only takes a moment.'
    : 'This is a one-time setup. The Ulak installer is downloading dependencies and configuring your machine. Subsequent launches will skip this step.'

  const pct = Math.round(progress.fraction * 100)

  return (
    <div className="ulak-fade-in flex h-full flex-col">
