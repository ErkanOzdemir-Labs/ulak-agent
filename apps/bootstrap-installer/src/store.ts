...
export const $ulakHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.ulak/logs/bootstrap-installer.log')
    $ulakHome.set('~/.ulak')
  ...
  try {
    const [logPath, ulakHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_ulak_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $ulakHome.set(ulakHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Ulak" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Ulak-Setup.exe --update);
...
export async function launchUlakDesktop(): Promise<void> {
  ...
  await invoke('launch_ulak_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Ulak repository' },
