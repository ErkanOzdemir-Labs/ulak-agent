...
/// read side does not see EOF until the last of them closes it. `ulak update`
...
/// stranded with it.
///
/// Same bound `Invoke-UlakStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `ulak_home_override` propagates to the child as $ULAK_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, ulak_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = ulak_home_override {
        cmd.env("ULAK_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, ulak_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = ulak_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_ulak_home() {
    ...
    /// child. `ulak update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
