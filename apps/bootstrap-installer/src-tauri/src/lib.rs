...
/// desktop app handing off via `Ulak-Setup.exe --update`).
///
...
pub fn run() {
    // Tracing → bootstrap-installer.log under ULAK_HOME/logs/ so install
    ...
    // Ulak is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "Ulak installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Ulak is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "Ulak" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "Ulak" shortcuts that install.ps1 creates, and a
            // reliable detached relaunch there needs the DETACHED_PROCESS +
            // startup-grace handling used by launch_ulak_desktop — out of
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::ulak_home().join("ulak-agent");
                if bootstrap::ulak_is_installed(&install_root) {
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        Ok(()) => {
                            // Brief grace so the spawned app is registered
                            // before we exit (mirrors launch_ulak_desktop).
                            std::thread::sleep(std::time::Duration::from_millis(200));
                            tracing::info!(
                                "ulak already installed — relaunched desktop; exiting installer"
                            );
                            app.handle().exit(0);
                            return Ok(());
                        }
        ...
        .expect("error while running Ulak Setup");
}
