...
//!
//! Mirrors `ulak_constants.get_ulak_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\ulak
//!   macOS:   ~/.ulak
//!   Linux:   ~/.ulak  (override via $ULAK_HOME)
//!
//! NOTE (macOS): Python's get_ulak_home(), scripts/install.sh, and the
//! Electron desktop's resolveUlakHome() ALL use ~/.ulak on macOS — there
...
/// Returns the canonical Ulak home directory, respecting $ULAK_HOME if set.
pub fn ulak_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("ULAK_HOME") {
    ...
    {
        // %LOCALAPPDATA%\ulak — matches scripts/install.ps1's $UlakHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("ulak");
        }
    }

    // macOS + Linux + fallback: ~/.ulak (matches Python get_ulak_home(),
    // install.sh, and the Electron desktop's resolveUlakHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".ulak");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    ulak_home().join("bootstrap-cache")
}
...
/// ULAK_HOME so it survives repo checkout deletion (unlike anything under
/// ulak-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\ulak\ulak-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_ulak`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under ULAK_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves ULAK_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    ulak_home().join(".ulak-update-in-progress")
}
...
pub fn copy_self_to_ulak_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to ULAK_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_ULAK_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".ulak-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under ULAK_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[ulak-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("ULAK_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_ulak_home() -> String {
    ulak_home().to_string_lossy().into_owned()
}
