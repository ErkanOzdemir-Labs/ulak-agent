...
# Ulak WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: ULAK_TINI_SHIM_TARGET overrides the `/init` path (and
# ULAK_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${ULAK_TINI_SHIM_TARGET:-/init}"
WRAPPER="${ULAK_TINI_SHIM_WRAPPER:-/opt/ulak/docker/main-wrapper.sh}"
