...
#
# Wired into the image as /etc/cont-init.d/01-ulak-setup by the
...
ULAK_HOME="${ULAK_HOME:-/opt/data}"
INSTALL_DIR="/opt/ulak"

# Drop to ulak via s6-setuidgid, but skip it when already non-root.
as_ulak() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid ulak "$@"; }
...
# starts non-root. The baked install tree under /opt/ulak is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $ULAK_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass ULAK_UID/ULAK_GID — or the PUID/PGID aliases — which the
...
# `id -u ulak` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# ulak UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u ulak)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-ulak UID).
...
and the baked /opt/ulak install tree is intentionally root-owned and
...
# --- Bootstrap ULAK_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid ulak mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom ULAK_HOME paths whose parents only
# root can create (e.g. `ULAK_HOME=/home/ulak/.ulak` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for ULAK_UID/ULAK_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  ULAK_UID/
# ULAK_GID still win when both are set.  See #15290, salvages #25872.
ULAK_UID="${ULAK_UID:-${PUID:-}}"
ULAK_GID="${ULAK_GID:-${PGID:-}}"

if [ -n "${ULAK_UID:-}" ] && validate_uid_gid "$ULAK_UID" && [ "$ULAK_UID" != "$(id -u ulak)" ]; then
    echo "[stage2] Changing ulak UID to $ULAK_UID"
    usermod -u "$ULAK_UID" ulak
fi
if [ -n "${ULAK_GID:-}" ] && validate_uid_gid "$ULAK_GID" && [ "$ULAK_GID" != "$(id -g ulak)" ]; then
    echo "[stage2] Changing ulak GID to $ULAK_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$ULAK_GID" ulak 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised ulak user (UID 10000)
...
# ulak process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes ulak. Idempotent across container
...
#   - ulak is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G ulak 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] ulak already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" ulak 2>/dev/null; then
        echo "[stage2] Added ulak to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group ulak failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When ULAK_UID is remapped or the top-level $ULAK_HOME isn't owned by
# the runtime ulak UID, restore ownership to ulak — but ONLY for the
# directories ulak actually writes to. The full $ULAK_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of ulak-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_ulak_uid=$(id -u ulak)
...
chown_ulak_tree() {
    ...
    chown -R ulak:ulak "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_ulak_owner() {
    target="$1"
    find "$target" \( ! -user ulak -o ! -group ulak \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$ULAK_HOME" 2>/dev/null)" != "$actual_ulak_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $ULAK_HOME (targeted) to ulak ($actual_ulak_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $ULAK_HOME: chown the directory itself (not its contents)
    # so ulak can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$ULAK_HOME"; then
        :
    else
        chown ulak:ulak "$ULAK_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $ULAK_HOME failed (rootless container?) — continuing"
    fi
    # Ulak-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by ulak (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$ULAK_HOME/$sub" ] && tree_has_non_ulak_owner "$ULAK_HOME/$sub"; then
            chown_ulak_tree "$ULAK_HOME/$sub"
        fi
    done
fi
...
# ulak user. Hosted/container instances keep mutable state under
# $ULAK_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# ULAK_DISABLE_LAZY_INSTALLS=1. Keeping /opt/ulak root-owned and
...
# $ULAK_HOME/lazy-packages dir on the data volume (Dockerfile sets
# ULAK_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to ulak
...
# Always reset ownership of $ULAK_HOME/profiles to ulak on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> ulak …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as ulak and walks
...
if [ -d "$ULAK_HOME/profiles" ] && tree_has_non_ulak_owner "$ULAK_HOME/profiles"; then
    chown_ulak_tree "$ULAK_HOME/profiles"
fi

# Always reset ownership of $ULAK_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged ulak runtime even
...
# gate as profiles/).
if [ -d "$ULAK_HOME/cron" ] && tree_has_non_ulak_owner "$ULAK_HOME/cron"; then
    chown_ulak_tree "$ULAK_HOME/cron"
fi

# Always ensure logs/gateways is ulak-owned (#45258). Formerly healed by
...
# top-level $ULAK_HOME is mis-owned, so a warm volume with ulak-owned
# ULAK_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid ulak mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as ulak.
if [ -d "$ULAK_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$ULAK_HOME/logs/gateways"; then
        :
    else
        chown ulak:ulak "$ULAK_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# ulak pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged ulak gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $ULAK_HOME is
...
# and cron/.
if [ -d "$ULAK_HOME/platforms/pairing" ] && tree_has_non_ulak_owner "$ULAK_HOME/platforms/pairing"; then
    chown_ulak_tree "$ULAK_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$ULAK_HOME/pairing" ] && tree_has_non_ulak_owner "$ULAK_HOME/pairing"; then
    chown_ulak_tree "$ULAK_HOME/pairing"
fi

# Reset ownership of ulak-owned top-level state files on every boot.
# The targeted data-volume chown above only covers ulak-owned
# *subdirectories*; loose state files living directly under $ULAK_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> ulak …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged ulak runtime then hits
...
# sweep so host-owned files in a bind-mounted $ULAK_HOME are never
...
# entries of ulak_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$ULAK_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$ULAK_HOME/$f"; then
            :
        else
            chown ulak:ulak "$ULAK_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the ulak runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$ULAK_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$ULAK_HOME/config.yaml"; then
        :
    else
        chown ulak:ulak "$ULAK_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$ULAK_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as ulak user ---
# Run as ulak via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $ULAK_HOME values
...
as_ulak mkdir -p \
    ...
    "$ULAK_HOME/home" \
    "$ULAK_HOME/pairing" \
    "$ULAK_HOME/platforms/pairing" \
    "$ULAK_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/ulak/.install_method (see Dockerfile), NOT written here into
# $ULAK_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $ULAK_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.ulak:/opt/data) and sometimes shared with a
...
# 'ulak update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $ULAK_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/ulak stamp, so deleting the data-dir copy is safe).
if [ -f "$ULAK_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$ULAK_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$ULAK_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$ULAK_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$ULAK_HOME/$dest"; then
            :
        else
            as_ulak cp "$INSTALL_DIR/$src" "$ULAK_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/ulak/.env.example being present
...
# not generate one. Ulak loads $ULAK_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$ULAK_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$ULAK_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $ULAK_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$ULAK_HOME/.env" ] && ! refuse_symlinked_path "clean" "$ULAK_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$ULAK_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$ULAK_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$ULAK_HOME/.env"; then
        :
    else
        if [ ! -f "$ULAK_HOME/.env" ]; then
            ...
            (umask 077 && as_ulak touch "$ULAK_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$ULAK_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$ULAK_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$ULAK_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $ULAK_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $ULAK_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$ULAK_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$ULAK_HOME/.env"; then
        :
    else
        chown ulak:ulak "$ULAK_HOME/.env" 2>/dev/null || true
        chmod 600 "$ULAK_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `ulak` user — without this it gets EACCES on every
...
# ulak group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp ulak /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted ulak group access to the Fly Machines API socket"
    else
...
# $ULAK_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `ulak update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set ULAK_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$ULAK_HOME/config.yaml" ]; then
    s6-setuidgid ulak "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$ULAK_HOME/auth.json" ] && [ -n "${ULAK_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$ULAK_HOME/auth.json"; then
        :
    else
        printf '%s' "$ULAK_AUTH_JSON_BOOTSTRAP" > "$ULAK_HOME/auth.json"
        chown ulak:ulak "$ULAK_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$ULAK_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via ULAK_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$ULAK_HOME/auth.json" ] && [ -n "${ULAK_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$ULAK_HOME/auth.json"; then
        :
    else
        s6-setuidgid ulak "$INSTALL_DIR/.venv/bin/python" \
...
# can set ULAK_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# ULAK_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$ULAK_HOME/gateway_state.json" ] && \
        [ "${ULAK_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$ULAK_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$ULAK_HOME/gateway_state.json"
        chown ulak:ulak "$ULAK_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$ULAK_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_ulak "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/ulak/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Ulak spawns for the
...
# main-wrapper.sh propagates it into the supervised ``ulak`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-ulak, dashboard, etc.).
