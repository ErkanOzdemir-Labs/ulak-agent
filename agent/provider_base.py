...
Every tool-provider ABC shares the same identity + ``ulak tools`` picker surface.
...
class ProviderBase(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``ulak tools``. Defaults to ``name``."""
        return self.name

    def get_setup_schema(self) -> Dict[str, Any]:
        """Provider row for the ``ulak tools`` picker.
...
class CatalogProviderBase(ProviderBase):
    ...
    @property
    def display_name(self) -> str:
        """Human-readable label shown in ``ulak tools``. Defaults to ``name.title()``."""
    ...
    def is_available(self) -> bool:
        ...
        ``ulak setup`` call it on every paint.
        """
        return True
