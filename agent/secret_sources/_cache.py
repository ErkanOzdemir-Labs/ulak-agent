...
a cache problem must never block Ulak startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """``home_path`` as resolved by ``load_ulak_dotenv()``, else ``$ULAK_HOME``/``~/.ulak``."""
    if home_path is None:
        from ulak_constants import get_ulak_home

        home_path = get_ulak_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend at ``<ulak_home>/cache/<basename>``::
