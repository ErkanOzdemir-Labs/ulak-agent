...
env-var-shaped values at process startup, AFTER ``~/.ulak/.env`` has loaded
and BEFORE the rest of Ulak reads ``os.environ``. The contract is deliberately
narrow: read-only; startup-time and synchronous (one ``fetch()`` per process per
ULAK_HOME, under a registry-enforced wall-clock timeout, no background
...
_SOURCE_ENVIRONMENT = ContextVar("ulak_secret_source_environment", default=None)
...
_GENERIC_REMEDIATION = {
    ErrorKind.NOT_CONFIGURED: "Run `ulak secrets {name} setup` to finish configuration.",
    ErrorKind.BINARY_MISSING: "Run `ulak secrets {name} setup` to install the helper CLI.",
    ErrorKind.AUTH_FAILED: "Credentials rejected — run `ulak secrets {name} setup` to re-authenticate.",
    ErrorKind.AUTH_EXPIRED: "Credentials expired — run `ulak secrets {name} setup` to re-authenticate.",
