...
def build_gateway_health_snapshot(
    ...
    base = {
        ...
        "ulak.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
    ...
    metrics: list[GatewayMetric] = [
        metric("ulak.gateway.up", int(bool(gateway_running))),
        metric("ulak.gateway.active_agents", active_agents),
        metric("ulak.gateway.busy", int(bool(busy))),
        metric("ulak.gateway.drainable", int(bool(drainable))),
        metric("ulak.gateway.restart_requested", int(bool(runtime.get("restart_requested")))),
        metric("ulak.gateway.state", 1, **{"ulak.gateway.state": gateway_state}),
    ]
    fatal_count = 0
    events: list[GatewayHealthEvent | GatewayDiagnosticEvent] = []
    ...
    for platform, pdata in platforms.items():
        ...
        pattrs = {"ulak.platform": str(platform), "ulak.platform.state": state}
        metrics.append(metric("ulak.platform.up", int(state in _RUNNING_PLATFORM_STATES), **pattrs))
        metrics.append(metric("ulak.platform.degraded", int(is_degraded), **pattrs, **{"ulak.error_code": error_code}))
        if is_degraded:
