...
_OBSERVABLE_METRIC_NAMES = (
    "ulak.gateway.up", "ulak.gateway.state", "ulak.gateway.active_agents", "ulak.gateway.busy",
    "ulak.gateway.drainable", "ulak.gateway.restart_requested", "ulak.gateway.background_work",
    "ulak.gateway.background_delegations", "ulak.platform.up", "ulak.platform.degraded",
    "ulak.cron.scheduler.heartbeat_age_seconds", "ulak.cron.scheduler.last_success_age_seconds",
    "ulak.cron.scheduler.catch_up_occurrences", "ulak.cron.jobs.enabled", "ulak.cron.jobs.running",
    "ulak.cron.jobs.overdue",
)
...
class GatewayHealthExportRuntime:
    ...
    def shutdown(self) -> None:
        ...
        if closeables:
            worker = threading.Thread(target=_close, name="ulak-gateway-health-export-shutdown", daemon=True)
            worker.start()
...
def _read_runtime_snapshot(config: Dict[str, Any]):
    ...
    try:
        ...
        for name, read in (
            ("ulak.gateway.background_work", _read_background_work_count),
            ("ulak.gateway.background_delegations", _read_background_delegations_count),
        ):
...
def _start_metric_provider(config: Dict[str, Any], sdk: Dict[str, Any]) -> Any:
    ...
    meter = provider.get_meter("ulak.gateway.health")
...
class GatewayDiagnosticLogStreamer(EmitterStreamer):
    ...
    def __init__(self, config: Dict[str, Any], sdk: Dict[str, Any]):
        ...
        self._logger = self._provider.get_logger("ulak.gateway.diagnostics")
        self._sdk = sdk
        self.exported = 0
...
def start_gateway_health_export(config: Dict[str, Any]) -> GatewayHealthExportRuntime:
    ...
    if metrics_on or diagnostics_on:
        try:
            ...
            logger.warning("monitoring.gateway_health_export.enabled but OTLP SDK is unavailable; install 'ulak-agent[otlp]'", exc_info=True)
    ...
    if diagnostics_on:
        try:
            ...
            thread = threading.Thread(target=_run, name="ulak-gateway-health-export", daemon=True)
            thread.start()
