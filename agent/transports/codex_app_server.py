...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # codex needs LLM provider creds but must not receive Tier-1 Ulak secrets (gateway/GitHub/infra tokens).
        ...
        # `os.environ.copy()` also handed it every Tier-1 Ulak secret — gateway bot tokens, GitHub auth,
        ...
        spawn_env = ulak_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("ULAK_KANBAN_TASK"):
            kanban_db = spawn_env.get("ULAK_KANBAN_DB")
            default_root = os.path.join(spawn_env.get("ULAK_HOME", os.path.expanduser("~/.ulak")), "kanban")
            kanban_root = os.path.dirname(kanban_db) if kanban_db else spawn_env.get("ULAK_KANBAN_ROOT", default_root)
            cmd += [
    ...
    def initialize(
        self, client_name: str = "ulak", client_title: str = "Ulak Agent",
