...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Ulak finds ``~/.openclaw/``: migrate first, cleanup (which breaks OpenClaw) after."""
    return (
        "A legacy OpenClaw directory was detected at ~/.openclaw/.\n"
        "To port your config, memory, and skills over to Ulak, run `ulak claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, run `ulak claw cleanup` "
