...
def _record_codex_app_server_usage(agent, turn, messages=None) -> dict[str, Any]:
    """Translate Codex app-server token usage into Ulak accounting. Prompt bucket = uncached + cached
    ...
    estimate grows monotonically and ulak-mode fires thread compaction on tiny threads (#100381)."""
...
def _record_codex_app_server_compaction(agent, turn, *, approx_tokens: int | None = None, force: bool = False) -> bool:
    ...
    if compressor is not None:
        ...
        # Codex owns this summary: a prior Ulak deterministic-fallback flag must not leak into it.
...
# --- Codex app-server → Ulak UI bridge -------------------------------------
# The app-server bypasses the Ulak tool loop, so the bridge translates JSON-RPC notifications
# into the callbacks the standard runtime fires (tool_progress_callback, _fire_stream_delta, ...).

# Item types that project to a Ulak tool_call (keep in sync with agent/transports/codex_event_projector.py
...
# Internal MCP server wrapping Ulak' native tools: its inner dispatch has no tool_progress_callback, so the
# codex-level mcpToolCall IS the display event and the mcp.ulak-tools.* prefix is stripped (users see Ulak tools).
_INTERNAL_MCP_SERVER = "ulak-tools"
...
def _codex_item_to_tool_name(item: dict) -> str:
    """Synthetic Ulak tool name for a codex item (mirrors CodexEventProjector)."""
...
def _ensure_codex_session(agent) -> None:
    ...
    # Approval callback: Ulak' standard prompt flow when a CLI thread installed one.
    ...
    # explicit approval bypass (approvals.mode: off, /yolo, --yolo, ULAK_YOLO_MODE) hands policy to codex's sandbox.
    ...
    # Ulak' gateway UI callbacks (tool_progress_callback, _fire_stream_delta,
...
def _bypass_sdk_request_transform(stream_kwargs: dict) -> dict:
    ...
    request without the walk. ULAK_CODEX_SDK_TRANSFORM=1 disables."""
    if os.environ.get("ULAK_CODEX_SDK_TRANSFORM", "").strip().lower() in {"1", "true", "yes", "on"}:
