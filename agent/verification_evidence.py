...
_AD_HOC_SCRIPT_NAME_PREFIXES = ("ulak-verify-", "ulak-ad-hoc-")
...
def _db_path() -> Path:
    return get_ulak_home() / "verification_evidence.db"
...
def record_verify_run(
    *, root: str | Path, session_id: str | None = None, ok: bool, command: str = "ulak verify",
    scope: str = "full", output: str = "",
) -> Optional[dict[str, Any]]:
    """Record a completed ``ulak verify`` run as verification evidence.
    ...
    return _insert_evidence(VerificationEvidence(
        command=command, canonical_command="ulak verify", kind="verify",
