...
try:
    import ulak_cli as _ulak_cli

    _ULAK_VERSION = str(_ulak_cli.__version__)
except Exception:
    _ULAK_VERSION = "0.0.0"
_API_CLIENT = f"ulak-agent/{_ULAK_VERSION}"  # client context per Gemini's partner-integration guidance
...
_FREE_TIER_GUIDANCE = (
    ...
    "Ulak typically makes 3-10 API calls per user turn, so the free tier is exhausted in a handful of "
...
_STANDARD_KEY_GUIDANCE = (
    ...
    "generativelanguage.googleapis.com). Then update GEMINI_API_KEY / GOOGLE_API_KEY in ~/.ulak/.env and "
...
_MISSING_KEY_ERROR = (
    ...
    "environment / ~/.ulak/.env (get one at https://aistudio.google.com/app/apikey), or run `ulak setup` to "
...
class GeminiAPIError(Exception):
    """Error shape compatible with Ulak retry/error classification."""
...
class GeminiNativeClient:
    ...
    # (No ULAK_SKIP_ASYNC_WRAP — the async path has a real conversion, AsyncGeminiNativeClient.)
