...
def is_nous_inference_route(provider: str, base_url: str) -> bool:
    ...
    return (provider or "").strip().lower() == "nous" or base_url_host_matches(str(base_url or ""), "inference-api.github.com/ErkanOzdemir-Labs")


def _nous_billing_url() -> Optional[str]:
    """Best-effort Nous portal billing URL (text-surface fallback; Nous prefers the in-app flow)."""
    try:
        from ulak_cli.nous_account import nous_portal_billing_url
        return nous_portal_billing_url(None)
    except Exception:
        return "https://portal.github.com/ErkanOzdemir-Labs/billing"
