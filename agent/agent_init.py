...
from ulak_cli.route_identity import normalize_route_base_url
from ulak_cli.timeouts import get_provider_request_timeout
...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    logger.warning(
        ...
        "provider's credentials/config with 'ulak memory status'. Note: "
        "systemd/gateway services do not inherit ~/.ulak/.env automatically; set "
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    with suppress(Exception):
        from ulak_cli.providers import ULAK_OVERLAYS, get_provider
        overlay = ULAK_OVERLAYS.get(provider)
    ...
    with suppress(Exception):
        ...
        from ulak_cli.models import normalize_provider as normalize_model_provider
        from ulak_cli.providers import normalize_provider as normalize_registry_provider
...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: ulak config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    """Per-profile marker path (``$ULAK_HOME`` is profile-scoped; not a config key)."""
    return get_ulak_home() / ".codex_gpt55_autoraise_notice"
...
def _resolve_api_mode(agent, api_mode, provider_name, base_url):
    ...
    if api_mode in _EXPLICIT_API_MODES:
    ...
    elif agent.provider in {"nous", "nous-portal", "erkanozdemir-labs"}:
        ...
        try:
            ...
            from ulak_cli.providers import host_mandated_api_mode as _host_mandated_api_mode
...
def _finalize_routing(agent, api_mode, credential_pool):
    ...
    with suppress(Exception):
        from ulak_cli.model_normalize import (
...
_TURN_STATE: Dict[str, Any] = {
    ...
    # Credits tracking (dev-only, ULAK_DEV_CREDITS) from x-nous-credits-* headers; session
...
def _init_prompt_cache_config(agent):
    ...
    agent._cache_ttl = "5m"
    with suppress(Exception):
        from ulak_cli.config import load_config_readonly as _load_pc_cfg
...
def _setup_logging(agent):
    ...
    from ulak_logging import setup_logging, setup_verbose_logging
    setup_logging(ulak_home=_ra()._ulak_home)
    ...
    # would starve the root file handlers. Noise reduction belongs in ulak_logging.
...
def _init_anthropic_client(agent, api_key, base_url, _provider_timeout):
    ...
    if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
        try:
            from ulak_cli.auth import build_minimax_oauth_token_provider
...
def _init_bedrock_client(agent, base_url):
    ...
    with suppress(Exception):
        from ulak_cli.config import load_config_readonly as _load_br_cfg
...
def _routed_client_kwargs(agent, fallback_model, _provider_timeout) -> Dict[str, Any]:
    ...
    if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
        ...
        raise RuntimeError(
            ...
            f"variable, or switch to a different provider with `ulak model`."
        )
    raise RuntimeError(
        "No LLM provider configured. Run `ulak model` to "
        "select a provider, or run `ulak setup` for first-time "
        "configuration."
    )
...
_HOST_DEFAULT_HEADERS: List[tuple[str, Callable[[Any, str], Dict[str, str]]]] = [
    ...
    ("githubcopilot.com", _lazy_headers("ulak_cli.models", "copilot_default_headers")),
    ...
    ("x.ai", _lazy_headers("tools.xai_http", "ulak_xai_default_headers")),
]
...
def _load_tools(agent, enabled_toolsets, disabled_toolsets):
    # A multiplexed gateway may have switched ULAK_HOME since model_tools was imported;
    ...
    # Kanban guidance is session-static (kanban_show iff ULAK_KANBAN_TASK); resolve once.
...
def _publish_session_id(session_id: str) -> None:
    ...
    try:
        ...
        if not delegated_child:
            os.environ["ULAK_SESSION_ID"] = session_id
...
def _init_session_state(agent, session_id, session_db, parent_session_id, reasoning_config, max_tokens,
    ...
    # ~/.ulak/sessions/ — kept unconditionally for request_dump_*.json debug breadcrumbs.
    agent.logs_dir = get_ulak_home() / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot is opt-in (sessions.write_json_snapshots); state.db is canonical.
    agent._session_json_enabled = False
    with suppress(Exception):
        from ulak_cli.config import load_config_readonly as _load_sess_cfg
    ...
    # Process-scoped --yolo is persisted so `ulak --resume` restores the bypass
...
def _compression_codex_settings(cfg: Dict[str, Any]) -> tuple[str, bool, Optional[int]]:
    ...
    if app_server_auto not in {"native", "ulak", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, ulak, off.",
            app_server_auto,
        )
...
def _configured_default_base_url(_agent_cfg, _model_cfg, _custom_providers) -> str:
    ...
    if _norm in _RUNTIME_FIRST_PROVIDER_IDS:
        ...
        with suppress(Exception):
            from ulak_cli.auth import resolve_provider as resolve_auth_provider
...
def _select_context_engine(_agent_cfg):
    ...
    if _selected_engine is None:
        try:
            from ulak_cli.plugins import get_plugin_context_engine
...
def _enforce_minimum_context(agent):
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by Ulak Agent.  Choose a model with at least "
...
def _warn_nonagentic_ulak_model(agent):
    # Nous Ulak 3/4 are chat models, not tool-call-tuned. cli.py show_banner() already
    ...
    if agent.quiet_mode or (agent.platform or "cli") == "cli":
        return
    with suppress(Exception):
        from ulak_cli.model_switch import _check_ulak_model_warning
    ...
    if agent.context_compressor:
        try:
            agent.context_compressor.on_session_start(
                agent.session_id, ulak_home=str(get_ulak_home()),
...
def init_agent(
    ...
    Non-obvious parameters:
      ...
      skip_context_files: skip SOUL.md/.ulak.md/AGENTS.md/CLAUDE.md/.cursorrules injection;
        load_soul_identity keeps ~/.ulak/SOUL.md as identity regardless.
    """
    _install_safe_stdio()
    ...
    try:
        from ulak_cli.config import load_config_readonly as _load_agent_config
    ...
    _warn_nonagentic_ulak_model(agent)
