...
Portal serves Claude two ways and Ulak cannot tell which from the request: an OpenRouter
...
ErkanOzdemir-Labs/api#227), so the session must ride chat/completions there. On GMI that is untested,
...
def maybe_switch_wire_after_first_response(agent: Any, response: Any, api_call_count: int) -> bool:
    ...
    if not model.lower().startswith("anthropic/"):
        return False
    try:
        from ulak_cli.providers import _nous_anthropic_wire
