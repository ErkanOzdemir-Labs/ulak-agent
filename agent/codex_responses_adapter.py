...
# long"). Codex message ids can run 400+ chars; Ulak ``msg_...`` ids stay under the cap.
...
def _chat_messages_to_responses_input(
    ...
    decision was wrong — xAI explicitly relies on Ulak threading encrypted reasoning back across turns for
    ...
    Ulak' local history is never truncated by native compaction, so the full conversation is still on the
    wire.
    """
