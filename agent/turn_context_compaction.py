...
def _codex_native_auto_compaction(agent: Any) -> bool:
    """Codex app-server threads are compacted by the codex agent itself; Ulak only
    initiates compaction in "ulak" mode."""
    return (
        # See #36801.
...
def _preflight_compression(
    ...
    if _preflight_deferred:
        ...
        logger.info(
            "Skipping Ulak preflight compression for codex app-server "
            "(mode=%s); Ulak will not start thread compaction here.",
