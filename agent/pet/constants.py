...
inferred from the atlas shape so Ulak renders both legacy 8-row sheets and
...
class PetState(str, Enum):
    """Animation state a pet can be shown in (Ulak names; Codex rows say ``jumping``/``running`` for ``jump``/``run``)."""
...
# Legacy Ulak/petdex row order (top -> bottom) for the older 8-row, 9-column atlas.
...
# Canonical Ulak names -> accepted row-name aliases in descending preference.
