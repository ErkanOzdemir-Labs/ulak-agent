...
``ulak pause`` writes a sentinel at ``$ULAK_HOME/ESTOP``; ``ulak resume``
...
(fail safe, e.g. ``touch ~/.ulak/ESTOP``). Ported from gastownhall/gastown estop.go (MIT).
"""
...
# Same profile-aware / fleet-root resolvers the file-safety guards use (fail-open to ~/.ulak).
from agent.file_safety import _ulak_home_path as _ulak_home, _ulak_root_path as _canonical_root
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel this process would write on `ulak pause`."""
    return _ulak_home() / SENTINEL_NAME


def _candidate_sentinel_paths() -> list:
    """Profile home first, then the fleet root if it is a different directory: a profile
    gateway (ULAK_HOME=~/.ulak/profiles/<n>) must still honor an operator's ~/.ulak/ESTOP."""
...
def paused_reply() -> Optional[str]:
    ...
    return f"⏸️ Ulak is paused{tag}. New work is on hold; run `ulak resume` to pick things back up."
...
def check_paused(component: str, logger: logging.Logger) -> bool:
    ...
    if first:
        ...
        logger.info(
            "%s dispatch paused by global emergency stop%s — remove with `ulak resume` (%s)",
