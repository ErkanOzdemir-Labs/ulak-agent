...
YAML files in ``<ULAK_HOME>/skill-bundles/`` (``name``, ``description``,
...
def _bundles_dir() -> Path:
    """Bundles directory: ``ULAK_BUNDLES_DIR`` override (tests) or ``<ULAK_HOME>/skill-bundles``."""
    override = os.environ.get("ULAK_BUNDLES_DIR")
    return Path(override).expanduser() if override else get_ulak_home() / "skill-bundles"
...
# File-level CRUD — used by `ulak bundles`.
