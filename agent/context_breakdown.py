...
def compute_context_details(agent: Any) -> Dict[str, Any]:
    ...
    Reuses the ``ulak prompt-size`` attribution (index-line bytes from the
    live skills block; schema bytes via the registry's tool→toolset map).
    """
    from ulak_cli.prompt_size import _compute_skills_breakdown, _compute_toolsets_breakdown
    from agent.system_prompt import build_system_prompt_parts
