"""OpenAI-shape bridge shared by Ulak' ACP clients.

ACP has no OpenAI-style ``tools``/``tool_calls`` channel, so Ulak' tool schemas travel INTO the
...
read/edit/execute tools forwards only Ulak' agent-level tools, since re-offering overlapping ones
makes Ulak redo finished work.
"""
...
class StreamChunks(list):
    """Chunk list that also carries response-level attributes (e.g. ``ulak_projected_messages``)
    Ulak reads off the ``create`` result; a plain list would drop them on the stream path."""
