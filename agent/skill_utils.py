...
from ulak_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_kanban() -> bool:
    ...
    if os.getenv("ULAK_KANBAN_TASK") or os.getenv("ULAK_KANBAN_BOARD"):
        try:
...
def _load_raw_config() -> Dict[str, Any]:
    """Read config.yaml with an mtime+size keyed cache (no ulak_cli.config import)."""
...
def _home_relative(p: Path) -> Path:
    """Anchor a relative config path at ULAK_HOME; absolute paths pass through."""
    from ulak_constants import get_ulak_home
    return p if p.is_absolute() else get_ulak_home() / p


# Never disableable: `ulak-agent` is the agent's own operating manual and the
# system prompt points at it unconditionally.
ESSENTIAL_SKILLS: frozenset = frozenset({"ulak-agent"})


def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    """Disabled skill names from config.yaml: global list ∪ platform list
    (*platform* defaults to ``ULAK_PLATFORM`` / ``ULAK_SESSION_PLATFORM``)."""
    skills_cfg = _skills_cfg()
    if skills_cfg is None:
        return set()
    from gateway.session_context import get_session_env
    resolved_platform = platform or os.getenv("ULAK_PLATFORM") or get_session_env("ULAK_SESSION_PLATFORM")
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``ulak config set`` stores lists as quoted JSON/Python-literal strings;
...
def get_external_skills_dirs() -> List[Path]:
    ...
    are ``~``/``${VAR}`` expanded, relative to ULAK_HOME; the local skills dir is skipped."""
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_skill_create_dir() -> Optional[Path]:
    ...
    relative to ULAK_HOME; a value equal to the local skills dir counts as unset."""
...
def display_skill_create_dir() -> str:
    ...
    if create_dir is None:
        return f"{display_ulak_home()}/skills/"
...
def get_all_skills_dirs() -> List[Path]:
    """Skill dirs: local ``~/.ulak/skills/`` first, then create_dir, then external.
...
# Project-local skills (<root>/.ulak/skills, <root>/.agents/skills; root = nearest
...
PROJECT_SKILLS_SUBDIRS = (os.path.join(".ulak", "skills"), os.path.join(".agents", "skills"))
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    (ULAK_HOME itself may live inside a git checkout)."""
    local_skills = get_skills_dir().resolve()
    dirs: List[Path] = []
...
# cached under ULAK_HOME, never inside the repo); "dangerous" excludes the
# skill from index, list, view and slash commands ("caution" loads, as on the hub).

# ── Project skill quarantine (scan-time injection defense) ──────────────── Trust (`ulak skills trust`)
...
# high-confidence findings only. The scan cache lives under ULAK_HOME, never inside the repo (we don't
...
def is_quarantined_project_skill(skill_md) -> bool:
    ...
    try:
        ...
        from ulak_constants import get_ulak_home
        cache_dir = get_ulak_home() / "cache" / "project_skill_scans"
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    # (which follows the live profile-scoped ULAK_HOME), so normalization
    ...
    # ~/.ulak/skills/<name> may be a symlink to a checkout elsewhere, and
...
def _ulak_metadata(frontmatter: Dict[str, Any]) -> Dict[str, Any]:
    """``metadata.ulak`` mapping from frontmatter, or ``{}`` when malformed."""
    metadata = frontmatter.get("metadata")
    ulak = metadata.get("ulak") if isinstance(metadata, dict) else None
    return ulak if isinstance(ulak, dict) else {}
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    ulak = _ulak_metadata(frontmatter)
    return {key: ulak.get(key, []) for key in _CONDITION_KEYS}


def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Extract ``metadata.ulak.config`` declarations (key/description/default/prompt).
    Entries missing ``key`` or ``description`` are skipped; ``prompt`` defaults to the description."""
    raw = _ulak_metadata(frontmatter).get("config")
