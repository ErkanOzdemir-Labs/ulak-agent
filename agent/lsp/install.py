...
Installs go to a Ulak-owned staging dir, ``<ULAK_HOME>/lsp/bin/``, so the
...
missing binary skips the server and ``ulak lsp status`` reports it).
...
# ``bin`` in ``<ULAK_HOME>/lsp/bin/`` first, then on PATH.  ``extra_pkgs``
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # the host so `ulak lsp status` reports its presence.
...
def ulak_lsp_bin_dir() -> Path:
    """Return the Ulak-owned bin staging dir for LSP servers."""
    from ulak_constants import get_ulak_home

    p = get_ulak_home() / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(ulak_lsp_bin_dir() / name):
...
def _link_into_bin(target: Path) -> str:
    ...
    link = ulak_lsp_bin_dir() / target.name
    if not link.exists():
        try:
...
def _install_npm(pkg: str, bin_name: str, extra_pkgs: Optional[list] = None) -> Optional[str]:
    ...
    # Managed npm first: $ULAK_HOME/node isn't on an arbitrary process's
    # PATH, so a bare which() would miss the Node that Ulak installed.
    npm = find_node_executable("npm")
    if npm is None:
        logger.info("[install] cannot install %s: no usable npm found", pkg)
        return None
    staging = ulak_lsp_bin_dir().parent  # <ULAK_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    ...
    pip_target = ulak_lsp_bin_dir().parent / "python-packages"
...
def detect_status(pkg: str) -> str:
    """Return ``installed``, ``missing``, or ``manual-only`` (for ``ulak lsp status``; spawns nothing)."""
    recipe = INSTALL_RECIPES.get(pkg)
...
__all__ = ["INSTALL_RECIPES", "try_install", "detect_status", "ulak_lsp_bin_dir"]
