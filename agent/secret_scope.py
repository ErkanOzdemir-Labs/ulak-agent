...
_GLOBAL_ENV_EXACT = frozenset({
    # Ulak runtime / deployment
    "ULAK_HOME", "ULAK_PROFILE", "ULAK_GATEWAY_LOCK_DIR",
    "ULAK_MAX_ITERATIONS", "ULAK_MAX_TOKENS", "ULAK_API_TIMEOUT",
    "ULAK_REDACT_SECRETS", "ULAK_NOUS_TIMEOUT_SECONDS",
    "_ULAK_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "ULAK_KANBAN_DB", "ULAK_KANBAN_WORKSPACES_ROOT", "ULAK_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "ULAK_KANBAN_",
    "ULAK_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    from ulak_cli.config import _parse_env_value
...
def build_profile_secret_scope(ulak_home: Path) -> Dict[str, str]:
    ...
    secrets = load_env_file(Path(ulak_home) / ".env")
    try:
        from ulak_cli.env_loader import get_secret_source_values
        external_secrets = get_secret_source_values(Path(ulak_home))
    except Exception:
        external_secrets = {}
