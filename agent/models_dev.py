...
thread refreshes) → disk cache (~/.ulak/models_dev_cache.json, any age) →
...
# Ulak provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # opencode-free is Zen-hosted (ulak_cli/models.py) and models.dev's "opencode" catalog lists
    ...
    # Ulak provider is "meta-ai"; both aliases are needed or muse-spark-*
...
# Reverse mapping: models.dev id → Ulak ids (built lazily; many-to-one).
_MODELS_DEV_TO_PROVIDER: Optional[Dict[str, List[str]]] = None


def _models_dev_to_ulak_ids(mdev_id: str) -> List[str]:
    """Return the Ulak provider ids that map to *mdev_id* (may be [])."""
    global _MODELS_DEV_TO_PROVIDER
    if _MODELS_DEV_TO_PROVIDER is None:
        _MODELS_DEV_TO_PROVIDER = {}
        for ulak_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            _MODELS_DEV_TO_PROVIDER.setdefault(mapped, []).append(ulak_id)
...
def _ulak_path(name: str) -> Path:
    from ulak_constants import get_ulak_home
    return get_ulak_home() / name


def _get_cache_path() -> Path:
    return _ulak_path("models_dev_cache.json")


def _get_etag_path() -> Path:
    return _ulak_path("models_dev_cache.etag")
...
def _get_provider_models(provider: str, *, allow_network: bool = False) -> Optional[Dict[str, Any]]:
    """Resolve a Ulak provider ID to its models dict, or None if unknown.
...
# models the catalog does not know and never displace catalog data. Provider keys accept the Ulak
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    """Override section for *provider* (keyed by Ulak OR models.dev id), or None."""
    overrides = _load_model_overrides()
    provider_key = (provider or "").strip()
    if not overrides or not provider_key:
        return None
    # Forward (Ulak → models.dev id) and reverse (caller passed a models.dev id, config keyed by Ulak id) aliases.
    candidates = [provider_key, PROVIDER_TO_MODELS_DEV.get(provider_key), *_models_dev_to_ulak_ids(provider_key)]
...
def get_provider_info(provider_id: str, *, allow_network: bool = True) -> Optional[ProviderInfo]:
    """Provider metadata by Ulak or models.dev ID, or None if not cataloged. ``allow_network`` defaults to True (interactive setup)."""
...
def get_model_info(provider_id: str, model_id: str, *, allow_network: bool = False) -> Optional[ModelInfo]:
    """Full model metadata by Ulak or models.dev provider ID (exact match, then case-insensitive), or
