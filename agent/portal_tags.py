...
Every Ulak request to the Nous Portal (main loop, auxiliary client, fallback
paths) must carry the same product-attribution tags, sent in OpenAI-compatible
``extra_body['tags']``: ``["product=ulak-agent", "client=ulak-client-v<__version__>"]``.
The version is read live from ``ulak_cli.__version__`` — do NOT pre-compute it
...
_affinity_scope: ContextVar[Optional[str]] = ContextVar("ulak_affinity_scope", default=None)
...
def ulak_client_tag() -> str:
    """``client=ulak-client-v<MAJOR>.<MINOR>.<PATCH>`` ("unknown" if ulak_cli is unimportable)."""
    try:
        from ulak_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"client=ulak-client-v{__version__}"
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    """
    tags = ["product=ulak-agent", ulak_client_tag()]
