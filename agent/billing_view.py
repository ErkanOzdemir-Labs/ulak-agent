...
def build_billing_state(*, timeout: float = 15.0) -> BillingState:
    """Fetch + parse ``/api/billing/state``; fail-open. ``ULAK_DEV_BILLING_FIXTURE`` short-circuits to a fixture."""
...
def _dev_fixture_billing_state() -> Optional[BillingState]:
    """``ULAK_DEV_BILLING_FIXTURE`` -> :class:`BillingState` for offline UX; None when unset.
    ...
    """
    name = (os.getenv("ULAK_DEV_BILLING_FIXTURE") or "").strip().lower()
    if not name:
        return None
    ...
    common: dict[str, Any] = dict(
        ...
        charge_presets=(Decimal("10"), Decimal("25"), Decimal("50")), portal_url="https://portal.github.com/ErkanOzdemir-Labs/billing?topup=open",
    )
    ...
    if name not in overrides:
        return BillingState(logged_in=False, error=f"unknown ULAK_DEV_BILLING_FIXTURE: {name}")
