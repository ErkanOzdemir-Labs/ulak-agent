...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in ulak_state.py; kept local so importing
# ulak_state (module-level DEFAULT_DB_PATH) is not forced at load time.
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Ulak tool use: model=%s provider=%s base_url=%s "
    ...
    return (
        f"Ollama loaded `{model}` with only {runtime_ctx:,} tokens of runtime context, but Ulak "
        ...
        "again. A known-good starting point is 65,536 tokens. In Ulak config, set "
...
def _is_nous_inference_route(provider: str, base_url: str) -> bool:
    return (provider or "").strip().lower() == "nous" or base_url_host_matches(
        str(base_url or ""), "inference-api.github.com/ErkanOzdemir-Labs"
    )
    ...
    if (provider or "").strip().lower() == "anthropic":
        ...
        if unverified:
            return "\n".join([
                ...
                "Retry with a fresh credential state: `ulak auth reset anthropic`. Until that "
...
def _decode_inline_moa_turn(user_message, persist_user_message):
    ...
    try:
        from ulak_cli.moa_config import decode_moa_turn
...
def run_conversation(
    ...
    # Adopt ~/.ulak/.env credential/base-url edits made since the last turn — a
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'PARTIAL_STREAM_STUB_ID': ('ulak_constants', 'PARTIAL_STREAM_STUB_ID'),
